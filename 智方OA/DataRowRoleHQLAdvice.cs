...
namespace ZhiFang.RowFilter.Interceptor
{
    public class DataRowRoleHQLAdvice : IMethodInterceptor
    {
        ...
        public object Invoke(IMethodInvocation invocation)
        {
            ...
            {
                HttpContext.Current.Response.Write("{ErrorInfo:\"无此功能权限！请尝试重新登录后再操作！\",success:false ,ResultDataFormatType:\"JSON\",ResultDataValue:\"\"}");
                return null;
            }
        }
        public void SetCacheRowFilterHQL(IMethodInvocation invocation)
        {
            ...
            //if (isExec == true)
            //    empRolesIdStr = GetRBACEmpRolesIdStrByCurEmpId(strEmployeeID);
            //if (!string.IsNullOrEmpty(empRolesIdStr))
            //{
            //    isExec = true;
            //}
            //else
            //{
            //    isExec = false;
            //    Common.Log.Log.Debug("UserAccount为:" + strUserAccount + "," + "EmployeeID为:" + strEmployeeID + "的员工角色为空!");
            //}
            long moduleID = 0;
        ...
        /// <returns></returns>
        public string GetRBACEmpRolesIdStrByCurEmpId(string empId)
        {
            StringBuilder strb = new StringBuilder();
            string empRolesIdStr = "";
            IApplicationContext context = ContextRegistry.GetContext();
            IBRBACEmpRoles = (ZhiFang.IBLL.RBAC.IBRBACEmpRoles)context.GetObject("BRBACEmpRoles");

            IList<RBACEmpRoles> tempList = IBRBACEmpRoles.SearchRBACEmpRolesByEmpId(empId);
            foreach (var item in tempList)
            {
                strb.Append(item.Id + ",");
            }
            if (strb != null && strb.ToString().Length > 0)
            {
                empRolesIdStr = strb.ToString().TrimEnd(',');
            }
            return empRolesIdStr;
        }
        /// <summary>
