...
namespace ZhiFang.ProjectProgressMonitorManage.Common
{
    public class MyNPOIHelper
    {
        ...
        public static bool FillExcelMoudleSheet(string strModuleFileName, string strFillFileName, Dictionary<string, string> dicCellValue, bool isWrapText)
        {
            return FillExcelMoudleSheet(strModuleFileName, strFillFileName, dicCellValue, "", isWrapText);
        }

        public static bool FillExcelMoudleSheet(string strModuleFileName, string strFillFileName, Dictionary<string, string> dicCellValue, string isFillEmptyCell, bool isWrapText)
        {
                ...
                using (FileStream fileStream = new FileStream(strModuleFileName, FileMode.Open, FileAccess.Read))
                {
                    ...
                    if (dicCellValue != null && dicCellValue.Count > 0)
                    {
                        ...
                        foreach (KeyValuePair<string, string> kv in dicCellValue)
                        {
                            ...
                            if (array.Length == 2)
                                MyNPOIHelper.FillExcelMoudleSheetCell(sheet, int.Parse(array[0]), int.Parse(array[1]), kv.Value, cellStyle, isFillEmptyCell, isWrapText);
                            else if (array.Length >= 3)
                            {
                                if (array[2].ToUpper() == "P")
                                    MyNPOIHelper.FillExcelMoudleSheetCellImage(sheet, int.Parse(array[0]), int.Parse(array[1]), kv.Value, array[3], isWrapText);
        ...
        public static void FillExcelMoudleSheetCell(ISheet sheet, int row, int col, string value, ICellStyle cellStyle, string isFillEmptyCell, bool isWrapText)
        {
            IRow headerRow;
            ...
            cell.CellStyle.WrapText = isWrapText;//自动换行
        ...
        public static void FillExcelMoudleSheetCellImage(ISheet sheet, int row, int col, string filePath, string empName, bool isWrapText)
        {
            ...
            {
                FillExcelMoudleSheetCell(sheet, row, col, empName, null, "", isWrapText);
            }
        }
