...
(function() {
	...
	/* 显示/隐藏智方图片页签(选择多图片上传时)，默认是隐藏*/
	function setzhifangImageShow() {
		var hidden=(editor.getOpt('zhifangImageHidden')===false?false:true);
		//if(!hidden)hidden=false;
		var zhifangImage=$("#zhifangImage");
		if(zhifangImage)hidden==true?zhifangImage.hide():zhifangImage.show();
	}
	/* 初始化tab标签 */
	function initTabs() {
	...
	function setTabFocus(id) {
		...
		switch(id) {
			...
			case 'online':
				...
				//自定义页签
			case 'online2':
				setAlign(editor.getOpt('imageManagerInsertAlign2'));
				onlineImage2 = onlineImage2 || new OnlineImage2('imageList2');
				onlineImage2.reset();
				break;
			case 'search':
				setAlign(editor.getOpt('imageManagerInsertAlign'));
		...
		dialog.onok = function() {
			...
			switch(id) {
				...
				case 'online':
					...
					//自定义页签
				case 'online2':
					list = onlineImage2.getInsertList();
					break;
				case 'search':
	...
	/* 在线图片二 */
	function OnlineImage2(target) {
		this.container = utils.isString(target) ? document.getElementById(target) : target;
		this.init();
	}
	OnlineImage2.prototype = {
		...
		initEvents: function() {
			...
			/* 滚动拉取图片 */
			domUtils.on($G('imageList2'), 'scroll', function(e) {
				var panel = this;
				if(panel.scrollHeight - (panel.offsetHeight + panel.scrollTop) < 10) {
					_this.getImageData();
				}
		...
		initData: function() {
			...
			/* 拉取数据需要使用的值 */
			this.state = 0;
			this.listSize = editor.getOpt('imageManagerListSize2');
			this.listIndex = 0;
			this.listEnd = false;
		...
		getImageData: function() {
			...
			if(!_this.listEnd && !this.isLoadingData) {
				this.isLoadingData = true;
				//imageManagerActionName2
				var urlPrefix = editor.getOpt('imageManagerUrlPrefix2');
				var actionName = editor.getOpt('imageManagerActionName2');
				var serverOAUrl = editor.getOpt('serverOAUrl');
				var imageUrl = editor.getOpt('imageUrl');
				var url = "";
				if(serverOAUrl) {
					serverOAUrl =  serverOAUrl + (serverOAUrl.indexOf('?') == -1 ? '?' : '&') + 'action=' + (actionName || '');
					url = utils.formatUrl(serverOAUrl);
				}
				isJsonp = utils.isCrossDomainUrl(url);
				ajax.request(url, {
					'timeout': 100000,
		...
		pushData: function(list) {
			var i, item, img, icon, _this = this,
				urlPrefix = editor.getOpt('imageManagerUrlPrefix2');
			for(i = 0; i < list.length; i++) {
				if(list[i] && list[i].url) {
					item = document.createElement('li');
					img = document.createElement('img');
