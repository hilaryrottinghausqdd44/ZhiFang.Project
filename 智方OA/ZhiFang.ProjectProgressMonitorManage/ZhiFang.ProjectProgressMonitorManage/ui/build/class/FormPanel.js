...
Ext.define('Ext.zhifangux.FormPanel',{
	...
	alias:'widget.zhifangux_formpanel',
	/**数据是否加载完成*/
    ...
    submit:function(){
        ...
        url = getRootPath() + "/" + url;
        var values2 = me.getForm().getValues();
        ...
        var v = me.fireEvent('changeValue',values);//用于对需要提交的数据进行处理
        if(v != true){value = v;}
        ...
        if(com.appComID && com.appComID != ""){//弹出的应用Id
        	var callback = function(info){
        		if(info.success){
        			...
        			me.openAppShowWin(null,ClassCode,com,selectId);
        		}else{
     ...
     * @param {} selectId
     */
    openAppShowWin:function(title,classCode,com,selectId){
        var me = this;
        ...
        var config = {
			...
			resizable:true,//可变大小
			draggable:true,//可移动
           	selectId:com.selectId//默认选中
        };
        if(title && title != ""){config.title = title;}
        var win = Ext.create(panel,config);
	...
	setWinformInfo:function(record,com){
        ...
        if(combo.xtype == 'combobox' || combo.xtype == 'combo'){
            combo.loadData([[value,text]]);
           	combo.setValue(value);
           	var dataTimeStampComId = itemId.split('_').slice(0,-1).join('_') + "_DataTimeStamp";
