...
Ext.define('Ext.build.BuildAppPanel',{
	...
	initComponent:function(){
		...
		me.ParserApp = Ext.create(me.ParserUrl);
		//初始化视图
		me.initView();
	...
	createSouth:function(){
		...
		var com = {
			...
			columns:[//列模式的集合
                ...
                {text:'标题栏',dataIndex:'header',width:60,align:'center',value:true,
                    xtype:'checkcolumn',
                    editor:{
                        xtype:'checkbox',
				...
				{text:'region',dataIndex:'region',editor:{}},//border布局时的区域
				{text:'可收缩',dataIndex:'split',editor:{}},//可收缩
				{text:'可收缩',dataIndex:'collapsible',editor:{}},//可收缩
				{text:'默认收缩',dataIndex:'collapsed',editor:{}},//默认收缩
				{text:'开启边框',dataIndex:'border',width:60,align:'center',value:true,
					xtype:'checkcolumn',
                    editor:{
                        xtype:'checkbox',
                        cls:'x-grid-checkheader-editor'
				...
				{text:'顺序号',dataIndex:'sequencenum',width:60,align:'center',
					xtype:'numbercolumn',
					format:'0',
					editor:{
		                xtype:'numberfield',
				...
				{text:'默认页面',dataIndex:'defaultactive',width:60,align:'center',value:true,
					xtype:'checkcolumn',
                    editor:{
                        xtype:'checkbox',
                        cls:'x-grid-checkheader-editor'
				...
				{text:'应用组件ID',dataIndex:me.fieldsObj.AppComID,hidden:false,editor:{readOnly:true}},
				{text:'中文名称',dataIndex:me.fieldsObj.CName,hidden:false,editor:{readOnly:true}},
				{text:'英文名称',dataIndex:me.fieldsObj.EName,hidden:false,editor:{readOnly:true}},
				{text:'功能编码',dataIndex:me.fieldsObj.ModuleOperCode,hidden:false,editor:{readOnly:true}},
				{text:'功能简介',dataIndex:me.fieldsObj.ModuleOperInfo,hidden:false,editor:{readOnly:true}},
...
//				{text:'时间戳',dataIndex:me.fieldsObj.DataTimeStamp,hidden:true},
				...
				{text:'关系实验室ID',dataIndex:'RefLabID',hidden:false},
				{text:'关系GUID',dataIndex:'RefAppComRefID',hidden:false},
				{text:'关系时间戳',dataIndex:'RefDataTimeStamp',hidden:false}
			],
			store:Ext.create('Ext.data.Store',{
		    	fields:me.getSouthStoreFields(false),
		    	proxy:{
		            type:'memory',
	...
	openAppListWin:function(callback,oldRecord){
    	...
    	var doSelectApp = function(id){
    		var call = function(appInfo){
				if(appInfo && appInfo != ""){
					var a = Ext.define('BTDAppComponentsModel',{
					    ...
					    fields:me.getSouthStoreFields(true)
					});
			...
			me.getAppInfoServer(id,true,call);
    	};
	...
	openLinkageConWin:function(){
		...
		var items = [];
		var arr = me.getSouthRocordInfoArray();
		for(var i in arr){
			var id = arr[i][me.fieldsObj.AppComID];
			var com = {
				xtype:'fieldset',
				title:arr[i].title + '【' + arr[i].itemId + '】',
				padding:5,collapsible:true,
				items:Ext.create('Ext.build.AppStructureTree',{rootId:id})
			};
			items.push(com);
		}
		...
		//弹出面板
		var win = Ext.create('Ext.panel.Panel',params);
		win.show();
	},
	/**
	 * 填写内部编号
	...
	openItemIdWin:function(record,callback,oldRecord){
		...
		var DesignCodeStr = record.get(me.fieldsObj.DesignCode);//还原代码
        var DesignCode ="";
        var title ="";
        if(DesignCode!=""){
			DesignCode = Ext.JSON.decode(DesignCodeStr);
			title = DesignCode.panelParams.titleText;
        }
		if(oldRecord){
				...
				items:[
					...
					{xtype:'button',text:'确定',iconCls:'build-button-ok',
						handler:function(){
							...
							//需要新增的记录不存在或者是需要替换的记录
							if(!rec || (rec && oldRecord && rec.get('itemId') == oldRecord.get('itemId'))){
								if(itemId && itemId != ""){
									if(oldRecord){
	...
	addAppPanel:function(record,par){
		...
		var type=record.get(me.fieldsObj.AppType);
		var panelParams = {
		...
		var menuItems = [{
			...
			handler:function(){
				me.switchParamsPanel(record.get('itemId'));
			}
		},{
        ...
        var panel =panel = Ext.create(Class,panelParams);
		center.add(panel);
    ...
    browse:function(){
        ...
        var appPanelClass = me.createPanelClass();
        var appPanel = Ext.create(appPanelClass);
        //解决chrome浏览器的滚动条问题------------
        appPanel.on({
        	boxready:function(){
        		appPanel.hide();
    ...
    createPanelClass:function(){
    	...
    	var layout = "auto";
    	var ClassName = "Ext.panel.Panel";
    	if(layoutType == "1"){//绝对布局
			layout = "absolute";
		}else if(layoutType == "2"){//border布局
			layout = eval("({type:'border',regionWeights:" + me.getBorderRegionWeightsStr() + "})");
		...
		//关系解析代码
		var linkageValue = me.linkageValue;
		var appInfos = [];
		var appInfoList = me.getAppInfoList();
		for(var i in appInfoList){
			var appId = appInfoList[i]['appId'];//功能ID
			var itemId = appInfoList[i]['itemId'];//内部编号
            var header = appInfoList[i]['header'];//标题栏
			var title = appInfoList[i]['title'];//标题文字
			var region = appInfoList[i]['region'];//region
			var split = appInfoList[i]['split'];//可收缩
			var collapsible = appInfoList[i]['collapsible'];//可收缩
			var collapsed = appInfoList[i]['collapsed'];//默认收缩
			var border = appInfoList[i]['border'];//有边框
			
			var x = appInfoList[i]['x'];//x
			var y = appInfoList[i]['y'];//y
			var width = appInfoList[i]['width'];//宽度
			var height = appInfoList[i]['height'];//高度
			
			var sequencenum = appInfoList[i]['sequencenum'];//顺序号
			var defaultactive = appInfoList[i]['defaultactive'];//默认页面
		...
		var par = {
			...
			layout:layout,
			getAppInfoServerUrl:me.getAppInfoServerUrl,
			comNum:0,
			appInfos:appInfos,
			resizable:{handles:'s e'},
			listeners:{
			...
			getCallback:function(appInfo){
				var me1 = this;
				appInfo.draggable = layout == "absolute" ? true : false;
				appInfo.resizable = true;
				var callback = function(obj){
					if(obj.success && obj.appInfo != ""){
						var ModuleOperCode = obj.appInfo[me.fieldsObj.ModuleOperCode];//功能编码
						var ClassCode = obj.appInfo[me.fieldsObj.ClassCode];//类代码
						...
						var record = me.getSouthRecordByKeyValue('itemId',appInfo.itemId);
						//右键菜单
						var menuItems = [{
							text:'属性面板',iconCls:'build-button-configuration-blue',
							tooltip:'点击切换到应用的属性面板',
							handler:function(){
								me.switchParamsPanel(appInfo.itemId);
							}
						},{
							text:'移除应用',iconCls:'build-button-delete',
							tooltip:'移除现有的应用模块',
							handler:function(){
				        		...
				        		me.removeAppPanel(panel);
				        		//删除组件属性面板
				        		me.removeAppParamsPanel(appInfo.itemId);
							}
						},{
							text:"替换应用",iconCls:'build-button-edit',
				        	tooltip:'重新选择应用替换现有的应用',
				        	handler:function(){
				        		var callback = function(record1){
									//移除组件记录
					        		me.removeSouthValueByRecord(appInfo.itemId);
					        		//删除应用组件 
					        		me.removeAppPanel(panel);
					        		//删除组件属性面板
					        		me.removeAppParamsPanel(appInfo.itemId);
					        		...
					        		for(var i in appInfo){
					        			if(appInfo[i] && i != "itemId" && i != "title"){
					        				record1.set(i,appInfo[i]);
					        			}else{
					        				delete appInfo[i];
					        			}
					        		}
									...
									me.addAppParamsPanel(record1);
									//添加元应用到应用中
									me.addAppPanel(record1,appInfo);
				        		};
				        		me.openAppListWin(callback,record);
				        	}
						}];
						...
						appInfo.listeners = {
							...
							move:function(com,x,y,eOpts){
					            me.setSouthRecordByKeyValue(panel.itemId,'x',x);
					            me.setSouthRecordByKeyValue(panel.itemId,'y',y);
							},
							//大小变化监听
							boxready:function(com,width,height,e){
								me.setSouthRecordByKeyValue(panel.itemId,'width',width);
					            me.setSouthRecordByKeyValue(panel.itemId,'height',height);
							},
							//大小变化监听
						...
						me1.add(panel);
						if(ClassName == 'Ext.tab.Panel' && appInfo.defaultactive){
							me1.setActiveTab(panel);
						}
					}else{
						var record = me.getSouthRecordByKeyValue('itemId',appInfo.itemId);
						//右键菜单
			...
			initLink:function(){
				...
				if(me.comNum == length){//内部应用全部都加载完毕
					if(linkageValue && linkageValue != ""){eval(linkageValue);}
		...
		if(par){
			...
			var menuItems = [{
	        	...
	        	handler:function(){
	        		var callback = function(record1){
						//移除组件记录
		        		me.removeSouthValueByRecord(record.get('itemId'));
		        		//删除应用组件 
		        		me.removeAppPanel(panel);
		        		//删除组件属性面板
		        		me.removeAppParamsPanel(record.get('itemId'));
	...
	createBorderPanel:function(){
		...
		if(values.top_show){//展现上部
			items.push({
				...
				collapsible:values.top_split,//可收缩
				collapsed:values.top_collapsed,//默认收缩
				collapsed2:values.top_collapsed,//默认收缩属性
				border:values.top_border,//有边框
				height:values.top_height,//高度
				items:[{
					xtype:'button',
		...
		if(values.bottom_show){//展现下部
			items.push({
				...
				collapsible:values.bottom_split,//可收缩
				collapsed:values.bottom_collapsed,//默认收缩
				collapsed2:values.bottom_collapsed,//默认收缩属性
				border:values.bottom_border,//有边框
				height:values.bottom_height,//高度
				items:[{
					xtype:'button',
		...
		if(values.left_show){//展现左部
			items.push({
				...
				collapsible:values.left_split,//可收缩
				collapsed:values.left_collapsed,//默认收缩
				collapsed2:values.left_collapsed,//默认收缩属性
				border:values.left_border,//有边框
				width:values.left_width,//宽度
				items:[{
					xtype:'button',
		...
		if(values.right_show){//展现右部
			items.push({
				...
				collapsible:values.right_split,//可收缩
				collapsed:values.right_collapsed,//默认收缩
				collapsed2:values.right_collapsed,//默认收缩属性
				border:values.right_border,//有边框
				width:values.right_width,//宽度
				items:[{
					xtype:'button',
	...
	createAppParamsPanel:function(record){
		...
		//组件的属性
		com.items.push(me.createComponentInfo(record));
	 ...
	 * @return {}
	 */
	createComponentInfo:function(record){
		var me = this;
		var com = {
	        ...
	        items:[{
	            ...
	            listeners:{
                	blur:function(field){
                		var itemId = record.get('itemId');
                		me.setSouthRecordByKeyValue(itemId,'title',field.value);
                	}
                }
	        },{
                xtype:'checkbox',itemId:'showTitle',name:'showTitle',boxLabel:'显示标题',
                listeners:{
                	change:function(field,newValue,oldValue){
                		var itemId = record.get('itemId');
                		me.setSouthRecordByKeyValue(itemId,'header',newValue);
                	}
                }
            },{
                xtype:'checkbox',itemId:'hasBorder',name:'hasBorder',boxLabel:'边框',
                listeners:{
                	change:function(field,newValue,oldValue){
                		var itemId = record.get('itemId');
                		me.setSouthRecordByKeyValue(itemId,'border',newValue);
                	}
                }
            }]
		};
    ...
    setParamsPanelValues:function(componentItemId){
        ...
        if(componentItemId != "center"){
            var record = me.getSouthRecordByKeyValue('itemId',componentItemId);
    ...
    setBasicParamsPanelValues:function(componentItemId,record){
        ...
        var values = {
        	...
        	showTitle:record.get('header'),
        	hasBorder:record.get('border')
        };
    ...
    getSouthRocordInfoArray:function(){
        var me = this;
        var records = me.getSouthRecords();
        var fields = me.getSouthStoreFields(false);
     ...
     * @return {}
     */
    getSouthStoreFields:function(bo){
        var me = this;
        
        var fields = [
    		{name:'itemId',type:'string'},//内部编号
    		{name:'title',type:'string'},//标题
            {name:'header',type:'bool'},//是否有标题栏
    		...
    		{name:'sequencenum',type:'int'},//顺序号
    		{name:'defaultactive',type:'bool'},//默认页面
	    	...
	    	{name:'RefLabID',type:'string'},//关系实验室ID
	    	{name:'RefAppComRefID',type:'string'},//关系GUID
	    	{name:'RefDataTimeStamp',type:'string'}//关系时间戳
        ];
    ...
    setAppParams:function(){
        ...
        var callback = function(appInfo){
            //var appParams = Ext.JSON.decode(appInfo[me.fieldsObj.DesignCode.split("_")[1]]);
            ...
            for(var i in southParams){
            	southParams[i].RefLabID = "";
            	southParams[i].RefAppComRefID = "";
            	southParams[i].RefDataTimeStamp = "";
            }
        ...
        //从后台获取应用信息
        //me.getAppInfoServer(me.appId,false,callback);
        me.ParserApp.getAppInfoById(me.appId,callback);
    },
    /**
     * 给组件记录列表赋值
     * @private
    ...
    getBorderRegionWeightsStr:function(){
		...
		for(var i in regionWeights){
			if(Ext.typeOf(regionWeights[i])){
				var bo = (Ext.typeOf(regionWeights[i]) === "number");
				regionWeightsStr += i + ":" + (bo ? regionWeights[i] : 0)  + ",";
			}
		}
	...
	getLinkObj:function(linkArr,objStr){
		...
		if(actEventName == "addClick" || actEventName == "delClick" || actEventName == "saveClick" || actEventName == "resetClick"){
			fun = function(but){
				...
				var functionStr = "";
				functionStr += "var act=" + actObjStr + ";";
				functionStr += "var fun=" + funObjStr + ";";
				if(actEventName == "addClick" || actEventName == "delClick"){
					functionStr += "var records=act.getSelectionModel().getSelection();";
					functionStr += "var reocrd=records[0];";
				}
				if(funFunName == "updateNode"){
					functionStr += "var com=act;" + 
					"var type=com.type;" + 
					"var values=com.getForm().getValues();" + 
					"var type=com.type;" + 
					"var obj={" +
						...
						"text:values[com.objectName+'_CName']" + 
					"};";
					functionStr += "fun." + funFunName + "(type,obj);";
				}else if(funFunName == "setValueByItemId"){
					var par = [];
					for(var i=0,l=actEventParNameArr.length;i<l;i++){
						var v = actEventParNameArr[i];
						if(v[0] == "{" && v.slice(-1) == "}"){
							functionStr += "var _" + i + "=record.get('" + v.slice(1,-1) + "');";
						}else{
							functionStr += "var _" + i + "=" + v + ";";
						}
						par.push("_" + i);
					}
					functionStr += "fun." + funFunName + "(" + par.join(",") + ");";
				}else{
					functionStr += "fun.autoSelect=act.getForm().getValues()[act.objectName+'_Id'];";
					functionStr += "fun." + funFunName + "();";
				}
...
//						functionStr += funObjStr + "." + funFunName + "(" + actEventParNameArr.join(",") + ");";
						...
						functionStr += funObjStr + "." + funFunName + "(" + par.join(",") + ");";
					}
				}
				eval(functionStr);
			};
...
//						functionStr += funObjStr + "." + funFunName + "(" + actEventParNameArr.join(",") + ");";
				...
				functionStr += "}else{alertError('请选择一条数据进行操作！');}";
				eval(functionStr);
			};
		}else if(actEventName == "selectClick"){//查询[高级查询、分组查询]
			fun = function(but){
	...
	save:function(bo){
        ...
        if(!isOk){
           alertError(message);
        	...
        	//设计代码（还原代码）
	    	var appParams = me.getAppParamsStr();
        	//类代码
			...
			//应用关系列表
			var BTDAppComponentsRefList = [];
			var appArr = me.getSouthRocordInfoArray();
			for(var i in appArr){
				var o = {
					RefAppComID:appArr[i][me.fieldsObj.AppComID],//被引用应用GUID
					RefAppComIncID:appArr[i].itemId//被引用应用内部ID
				};
				
				if(bo && me.appId != -1 && appArr[i].RefAppComRefID && appArr[i].RefAppComRefID != ""){
					o.LabID = appArr[i].RefLabID;//关系表实验室ID
					o.Id = appArr[i].RefAppComRefID;//关系表GUID
					o.DataTimeStamp = appArr[i].RefDataTimeStamp.split(",");//关系表时间戳
					o.Creatoro = appArr[i].Creatoro;
					o.Modifier = appArr[i].Modifier;
					o.DataAddTime = appArr[i].DataAddTime;
					o.DataUpdateTime = appArr[i].DataUpdateTime;
				}
				
				BTDAppComponentsRefList.push(o);
				
			}
			//关系列表json串
			//var BTDAppComponentsRefListStr = Ext.JSON.encode(BTDAppComponentsRefList);
			...
			var BTDAppComponents = {
				...
				BTDAppComponentsRefList:BTDAppComponentsRefList//关系列表
			};
			...
			var saveToServer = function(){
				var callback = function(text){
					...
					if(result.success){
						...
						me.ParserApp.getAppInfoById(me.appId,c);
						me.disablePanel(false);//启用面板
						alertInfo('保存成功');
					}else{
						me.disablePanel(false);//启用面板
	            ...
	            me.ParserApp.saveAppInfo(BTDAppComponents,callback);
			};
            //名称重复校验
	...
	createAppClass:function(){
		...
		for(var i in appInfoList){
			...
			if(x && x > 0){str += "x:" + x + ",";}
			if(y && y > 0){str += "y:" + y + ",";}
			if(width && width > 0){str += "width:" + width + ",";}
			if(height && height > 0){str += "height:" + height + ",";}
			...
			str += "collapsed:" + collapsed + ",";
			str += "border:" + border + ",";
			str += "sequencenum:" + sequencenum + ",";
			str += "defaultactive:" + defaultactive;
			str += "}";
			
			appInfos.push(str);
		}
		
		var appInfosStr = "[" + appInfos.join(",") + "]";
	 ...
	 * @param {} callback
	 */
	getAppInfoServer:function(id,isPlanish,callback){
		var me = this;
		
		if(id != -1){
			var url = me.getAppInfoServerUrl + "?isPlanish=" + isPlanish + "&id=" + id;
			//回调函数
            var c = function(text){
	        	var result = Ext.JSON.decode(text);
	        	if(result.success){
                    ...
                    if(result.ResultDataValue && result.ResultDataValue != ""){
                    	if(!isPlanish){
							result.ResultDataValue = result.ResultDataValue.replace(/\\\\\\/g,"\\");
	...
	getAppParamsStr:function(){
		...
		for(var i in panelParams){
			if(Ext.typeOf(panelParams[i])){
				var bo = (Ext.typeOf(panelParams[i]) === "string");
				var v = panelParams[i];
				if(bo){
					panelParamsStr += i + ":'" + v.replace(/\'/g,"\\\'") + "',";
				}else{
					panelParamsStr += i + ":" + v + ",";
				}
				//panelParamsStr += i + ":" + (bo ? "'" : "") + v + (bo ? "'" : "") + ",";
			}
		}
		...
		for(var i in southParams){
			...
			for(var j in southParams[i]){
				if(Ext.typeOf(southParams[i][j])){
					var bo = (Ext.typeOf(southParams[i][j]) === "string");
					var v = southParams[i][j];
					if(bo){
						objStr += j + ":'" + v.replace(/\'/g,"\\\'") + "',";
					}else{
						objStr += j + ":" + v + ",";
					}
					//objStr += j + ":" + (bo ? "'" : "") + southParams[i][j].replace(/\'/g,"\\\'") + (bo ? "'" : "") + ",";
				}
			}
		...
		//总体属性
		var appParamsStr = "{panelParams:" + panelParamsStr + ",southParams:" + southParamsStr + "}";
	...
	changeStoreData: function(response){
  	...
  	getAppInfoList:function(){
  		var me = this;
  		//所有组件信息
		var items = me.getSouthRecords();
  		var list = [];
  		
  		var map = [];
		for(var i in items){
			var kv = {sequencenum:items[i].get('sequencenum'),Index:i};
			map.push(kv);
		}
		
		for(var i=0;i<map.length-1;i++){
			for(var j=i+1;j<map.length;j++){
				if(map[i].sequencenum > map[j].sequencenum){
					var temp = map[i];
					map[i] = map[j];
					map[j] = temp;
				}
		...
		for(var i in map){
			var obj = {};
			var record = items[map[i].Index];
			obj.appId = record.get(me.fieldsObj.AppComID);//功能ID
			obj.itemId = record.get('itemId');//内部编号
            obj.header = record.get('header');//标题栏
			obj.title = record.get('title');//标题文字
			obj.region = record.get('region');//region
			obj.split = record.get('split');//可收缩
			obj.collapsible = record.get('collapsible');//可收缩
			obj.collapsed = record.get('collapsed');//默认收缩
			obj.border = record.get('border');//有边框
			
			obj.x = record.get('x');//x
			obj.y = record.get('y');//y
			obj.width = record.get('width');//宽度
			obj.height = record.get('height');//高度
			
			obj.sequencenum = record.get('sequencenum');//顺序号
			obj.defaultactive = record.get('defaultactive');//默认页面
	...
	setAppInfo:function(appInfo){
        ...
        //关系表数据
        var list = appInfo.BTDAppComponentsRefList;
        for(var i in list){
        	var record = me.getSouthRecordByKeyValue('itemId',list[i].RefAppComIncID);
        	if(record){
        		record.set('RefLabID',list[i].LabID);//关系实验室ID
            	record.set('RefAppComRefID',list[i].Id);//关系GUID
            	record.set('RefDataTimeStamp',list[i].DataTimeStamp);//关系时间戳
            	record.set(me.fieldsObj.AppComID,list[i].RefAppComID);//被引用应用ID
            	record.set('itemId',list[i].RefAppComIncID);//itemId
        	}
        }
	},
	/**
