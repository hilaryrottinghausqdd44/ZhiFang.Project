...
/**机构管理员角色编码*/
JcallShell.System.ORG_ADMIN_ROLE_USECODE = 'ORG_ADMIN_ROLE';

/**登录时的本地时间毫秒数*/
JcallShell.System.LOGIN_DATE_TIMES = null;

/**智方的微信新闻页签是否显示,OA系统为显示:false;QMS等系统不显示:true*/
JcallShell.System.WeiXinTabHidden= true;

JcallShell.System.USER = {
	EXCEL:'用户信息模版V1.xls'
};
...
JcallShell.System.ClassDict = {
	//获取类字典服务
	_classDicUrl:'/SystemCommonService.svc/GetClassDic',
	//获取类字典列表服务
	_classDicListUrl:'/SystemCommonService.svc/GetClassDicList',
	/** @public
	...
	loadClassInfoList:function(classParamList,callback){
		...
		JShell.Server.post(url,Ext.JSON.encode(params),function(data){
			if(data.success){
				for(var i in classParamList){
					me.initClassInfo(classParamList[i].classname,data.value);
				}
			}else{
				for(var i in classParamList){
