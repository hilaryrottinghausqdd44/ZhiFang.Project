 ...
 * @version 2017-05-10
 */
Ext.define('Shell.interface.qms.file.Viewport', {
	...
	initDailyModuleToolbar:function(){
		...
		var url = 'config/DailyModule_QMS.json?t=' + new Date().getTime();
		JShell.Server.get(url,function(data){
			if(data.success){
				me.changeDailyModuleToolbar(data.value.list);
				
				var TopToolbar = me.getItemCom().getComponent('TopToolbar'),
					DailyModuleToolbar = TopToolbar.getComponent('DailyModuleToolbar');
				me.ContentTab.insertTab(DailyModuleToolbar.items.items[1].classInfo);
			}
		});
	},
	/**常用功能变化*/
	...
	onBeginLogin:function(){
		...
		if(!params.ACCOUNT || !params.PASSWORD){
			...
			if(params.ACCOUNTCASE){
				switch(params.ACCOUNTCASE){
					case 'U':Account = Account.toLocaleUpperCase();break;//账号转大写
					case 'L':Account = Account.toLocaleLowerCase();break;//账号转小写
					default:break;
				}
			}
			me.onLogin(Account,params.PASSWORD,function(data){
				me.afterLogin();
			});
		}
	},
