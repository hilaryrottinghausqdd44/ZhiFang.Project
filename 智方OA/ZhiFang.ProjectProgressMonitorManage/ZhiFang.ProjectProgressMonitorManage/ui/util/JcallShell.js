...
/**@description Function,用于存放全局的方法*/
JcallShell.Function = {};
/**@description 空方法*/
...
JcallShell.Page = {
	...
	changeLangage: function(lang) {
		...
		if(!lang || lang.toLocaleUpperCase() == 'CN') {
			//加载EXTJS中文包
			var url = JcallShell.System.Path.UI + '/extjs/locale/ext-lang-zh_CN.js';
			content.push('<script type="text/javascript" src="' + url + '"></script>');
		} else {
			var file = 'locale/' + JShell.System.Lang.toLocaleLowerCase() + '/config/config_System.js';
			var url = JcallShell.System.Path.UI + '/' + file;
			content.push('<script type="text/javascript" src="' + url + '"></script>');
		}
...
JcallShell.Date = {
	 ...
	 * @param {Object} year
	 * @param {Object} month
	 * @param {Object} weekTims 第几周
	 * @author liangyl
	 * @version 2016-09-28
	 * @version 2017-10-12 longfc
	 */
	　　
	getWeekStartDateAndEndDate: function(year, month, weekTims) {　　　　
		var result = {
			StartDate: '',
			EndDate: ''
		};
		if(!year || !month || !weekTims) {
			return result;
		}
		year = parseInt(year);
		month = parseInt(month);
		weekTims = parseInt(weekTims);

		var firstDateStr = year + "-" + month + "-" + "01";
		//选择月的第一天
		var firstDate = this.getMonthFirstDate(year, month, false);
		//选择月的最后一天
		var lastDate = new Date(year, month, 0);
		//选择月的总天数
		var countDays = this.getCountDays(firstDate);
		//选择月的总周数
		var weekCount = this.getMonthTotalWeekByDate(firstDateStr);
		//第一天所属星期几;返回值是 0（周日） 到 6（周六） 之间的一个整数
		...
		if(firstWeekDay == 0) {
			...
			addPrevDays = firstWeekDay - 1;
		}
		//最后一天所属星期几;返回值是 0（周日） 到 6（周六） 之间的一个整数
		...
		if(lastWeekDay == 0) {
			...
			addNextDays = 7 - lastWeekDay;
		}
		var mapDates = {};
		var day = 1;
		//该月总周数
		for(var i = 1; i <= weekCount; i++) {
			if(day > countDays) break;
			var weekDates = [];
			//每周7天的日期
			for(var j = addPrevDays; j < 7; j++) {
				//先填充补齐选择月的第一周的上月日期
				if(i == 1 && addPrevDays > 0) {
					//选择月的上一个月的第一天
					var preFirstDate = new Date(firstDate.getFullYear(), firstDate.getMonth() - 1, 1);
					//选择月的上一个月的年份
					var preYear = preFirstDate.getFullYear();
					//选择月的上一个月的月份
					var preMonth = preFirstDate.getMonth();
					var preCountDays = this.getCountDays(preFirstDate);
					for(var k = 1; k <= addPrevDays; k++) {
						var preDate = new Date(preYear, preMonth, preCountDays);
						preDate = Ext.util.Format.date(preDate, 'Y-m-d');
						preCountDays--;
						weekDates.push(preDate);
					}
					weekDates.reverse();
				}
				addPrevDays = 0;
				var curDay = day;
				if(curDay < 9) curDay = "0" + curDay;
			...
			mapDates[i] = weekDates;
		}
		//选择月最后一周需要填充补齐的下月日期
		if(addNextDays > 0) {
			//选择月的下一个月的第一天
			var nextFirstDate = new Date(firstDate.getFullYear(), firstDate.getMonth() +1, 1);
			//选择月的下个月的年份
			var nextYear =nextFirstDate.getFullYear();
			//选择月的下个月的月份
			var nextMonth = nextFirstDate.getMonth();
			for(var nextDay = 1; nextDay <= addNextDays; nextDay++) {
				var dateNext = new Date(nextYear, nextMonth, nextDay);
				dateNext = Ext.util.Format.date(dateNext, 'Y-m-d');
				mapDates[weekCount].push(dateNext);
			}
			addNextDays = 0;
		}
		//获取当前选择周的日期信息
		var curWeek = mapDates[weekTims];
		//获取当前选择周的第一天
		var fromDate = curWeek[0];
		//获取当前选择周的最后一天
		var toDate = curWeek[curWeek.length - 1];
		if(fromDate) {
	 ...
	 */
	getMonthTotalWeekByDate: function(date) {
		var mDate = this.getDate(date);
		if(!mDate) return null;

		//获取当前日期所在月的最后一天
		mDate = new Date(mDate.getFullYear(), mDate.getMonth() + 1, 0);

		return this.getMonthWeekByDate(mDate);
	},
	/**获取当前年月有多少周
	 * @param {Object} year 年
	 * @param {Object} month 月(1-12)
	 */
	getMonthTotalWeekByYearMonth: function(year, month) {
		//获取当前日期所在月的最后一天
		var date = new Date(year, month, 0);

		return this.getMonthWeekByDate(date);
	},
	/**获取当前日期在当前月是第几周
	 * @param {Object} date 日期
	 */
	getMonthWeekByDate: function(date) {
		var mDate = this.getDate(date);
		if(!mDate) return null;
...
JcallShell.Bytes = {
	...
	toSize: function(bytes) {
		...
		var result = (bytes / Math.pow(k, i)).toPrecision(len) + ' ' + sizes[i];
...
JcallShell.Server = {
	...
	/**后台服务键值对*/
	HEADERS: [{
		key: 'User-Agent-Zip-Fa',
		value: '6D0F3E94-B672-4BFD-B614-00398C73447D'
	}],
	/**状态信息*/
	Status: {
		ERROR_404: '无法找到地址',
	...
	Mapping: function(data) {
		...
		for(var i in value) {
			if(Ext.typeOf(value[i]) != 'string') continue;
...
JcallShell.Templet = {
	 ...
	 * @example
	 * regExp /{Id}|{Name}/g
	 * matchHashMap {"Id":"123","Name":"名称"}
	 */
	replace: function(str, regExp, matchHashMap) {
		if(!regExp) return str;

		return str.replace(regExp, function(matchStr) {
			var value = matchHashMap[matchStr];
			return value !== undefined ? value : matchStr;
		});
	}
};
