...
Ext.define("Shell.class.sysbase.bparameter.Form", {
	...
	createHideItems: function() {
		...
		items.push({
			...
			xtype: "textfield",
			itemId: "BParameter_PDict_Id",
			hidden: true,
			name: "BParameter_PDict_Id",
			fieldLabel: "字典ID"
		});
	...
	getAddParams: function() {
		...
		if(values.BParameter_PDict_Id) {
			entity.PDict = {
				Id: values.BParameter_PDict_Id,
		...
		if(me.formtype=="add"){
			var LabID=JShell.System.Cookie.get(JShell.System.Cookie.map.LABID);//实验室ID
			if(LabID)entity.LabID=LabID;
		}
		return {
			entity: entity
		};
	...
	setPDictIdValue: function(value) {
		var me = this;
		if(me.formtype = "add") {
			PDictId = me.getComponent('BParameter_PDict_Id');
			PDictId.setValue(value);
		}
	},
