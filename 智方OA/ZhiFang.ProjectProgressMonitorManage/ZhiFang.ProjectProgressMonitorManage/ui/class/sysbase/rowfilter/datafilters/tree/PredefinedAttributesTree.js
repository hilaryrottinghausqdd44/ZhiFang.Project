 ...
 * 　　功能要求：
 */
Ext.define('Shell.class.sysbase.rowfilter.datafilters.tree.PredefinedAttributesTree', {
	...
	arrTreeJson: [],
	//模块操作预定义属性更新
	updateServerUrl: JShell.System.Path.ROOT + '/' + 'RBACService.svc/RBAC_UDTO_UpdateRBACModuleOperByField',
	selectServerUrl: JShell.System.Path.ROOT + '/RBACService.svc/RBAC_UDTO_SearchRBACModuleOperByHQL?isPlanish=true&fields=RBACModuleOper_PredefinedField,RBACModuleOper_Id',
	objectPropertyUrl: JShell.System.Path.ROOT + '/RBACService.svc/RBAC_UDTO_GetEntityFrameTree',
	/**哪些树父节点的对象不需要加载,属性选择时过滤一些无关的节点*/
	noClassNameArr: [
		"PGMProgram_PBDictTree_Creator",
		"PGMProgram_SubBDictTree_Creator",
		"PGMProgram_OriginalPGMProgram",
		"PGMProgram_BEquip_PGMProgramList"
	],
	afterRender: function() {
		var me = this;
		me.callParent(arguments);
		if(me.store != null && me.moduleOperId) {
			//外部还原已经勾选的操作
			setTimeout(function() {
				me.defaultChecked()
			}, 2000);
	...
	initComponent: function() {
		...
		if(!me.objectCName) me.objectCName = "数据对象";
		me.initLink();
	...
	loadData: function() {
		var me = this;
		me.store.proxy.url = me.objectPropertyUrl + "?EntityName=" + me.objectName;
		me.store.load();
		if(me.store != null && me.moduleOperId) {
			//外部还原已经勾选的操作
			setTimeout(function() {
	...
	initLink: function() {
		...
		me.listeners.beforeitemexpand = function(node) {
				me.nodeClassName = node.data['InteractionField'];
			},
			me.listeners.beforeload = function(store) {
				//实体对象不能自己关联自己或过滤一些无关的节点
				if(me.nodeClassName != "" && Ext.Array.indexOf(me.noClassNameArr, me.nodeClassName) == -1) {
					store.proxy.url = me.objectPropertyUrl + "?EntityName=" + me.nodeClassName;
				}
			},
	...
	defaultChecked: function() {
		...
		if(me.moduleOperId) {
			...
			if(hqlWhere && hqlWhere != null) {
				url = url + '&where=' + encodeURIComponent(hqlWhere);;
			}
			JShell.Server.get(url, function(data) {
				if(data.success) {
					if(data.value && data.value.list) lists = data.value.list;
	...
	/**隐藏节点*/
	doHideNode: function(data,defaultRootProperty) {
		var me = this;

		var children = data[defaultRootProperty];

		me.changeChildren(children,defaultRootProperty);
	},
	/**修改所有子节点*/
	changeChildren: function(children,defaultRootProperty) {
		var me = this;
		for(var i = 0; i < children.length; i++) {
			var bo = me.changeNode(children[i],defaultRootProperty);
			if(bo) {
				children.splice(i, 1);
				i--;
			}
	...
	/**修改节点*/
	changeNode: function(node,defaultRootProperty) {
		var me = this;
		//需要剔除的节点
		if(Ext.Array.indexOf(me.noClassNameArr, node.InteractionField) > -1) {
			return true;
		}
		var children = node[defaultRootProperty];
		if(children) {
			me.changeChildren(children,defaultRootProperty);
		}
	...
	nodeEachChild: function(node) {
		...
		node.eachChild(function(childNode) {
			if(Ext.Array.indexOf(me.noClassNameArr, childNode.InteractionField) > -1) {
				childNode.data.leaf = true;
				childNode.data.expanded = false;
			}
	...
	createStore: function() {
		...
		var store = Ext.create('Ext.data.TreeStore', {
			...
			proxy: {
				type: 'ajax',
				url: me.objectPropertyUrl + "?EntityName=" + me.objectName,
				extractResponseData: function(response) {
					...
					if(data.ResultDataValue && data.ResultDataValue != "") {
						...
						if(arr) {
							for(var i in arr) {
								var obj = arr[i];
								if(Ext.Array.indexOf(me.noClassNameArr, obj.InteractionField) == -1) {
									arr[i]['expanded'] = true;
									arr[i].checked = false;
	...
	saveClick: function(node) {
		...
		if(me.moduleOperId) {
			...
			if(PredefinedField.length == 0) {
				...
				PredefinedField = JShell.JSON.encode(PredefinedField);
			...
			var params = JShell.JSON.encode({
				entity: entity,
				fields: "PredefinedField,Id"
			});
			JShell.Server.post(me.updateServerUrl, params, function(data) {
				me.fireEvent('onsave', me, data);
			});
		}
	},
	/***
	...
	getPredefinedField: function() {
		...
		//新增的处理
		var arrTreeJson = [];
		var str = JShell.JSON.encode(nodes);
		str = str.replace(/'/g, "");
		str = str.replace(/"/g, "'");
		var nodeStr = "{'Tree':[]}";
		if(str != null && str.length > 0) {
			if(str == nodeStr || str == '') {
				...
				arrTreeJson = [{
					...
					text: me.objectCName,
					leaf: false,
					ParentEName: me.objectName,
					ParentCName: "",
					InteractionField: me.objectName,
					FieldClass: me.objectName
				}];
