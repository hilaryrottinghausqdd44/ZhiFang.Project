 ...
 * 设置数行据过滤条件应用
 */
Ext.define('Shell.class.sysbase.rowfilter.preconditions.AddPanel', {
	...
	createAddForm: function() {
		var me = this;
		me.AddForm = Ext.create('Shell.class.sysbase.rowfilter.preconditions.AddForm', {
			header: false,
			border: false,
			split: false,
			collapsible: true,
			...
			EntityCode: me.EntityCode,
			EntityCName: me.EntityCName
		});
	},
	//数据条件列表
	...
	loadData: function() {
		...
		if(me.PK) {
			...
			var fields = "RBACRowFilter_RowFilterConstruction,RBACRowFilter_CName,RBACRowFilter_EntityCode,RBACRowFilter_EntityCName";
			...
			JShell.Server.get(url, function(data) {
				if(data.success) {
					if(data.value) {
						...
						me.EntityCode = data.value['RBACRowFilter_EntityCode'];
						me.EntityCName = data.value['RBACRowFilter_EntityCName'];
						//数据过滤条件中文名称还原
	...
	getSaveParams: function(button) {
		...
		if(me.appType == "edit") params['fields'] = 'CName,RowFilterCondition,RowFilterConstruction,Id,IsPreconditions';
	...
	save: function(bo, button) {
		...
		if(!me.EntityCode) {
			button.setDisabled(false);
			JShell.Msg.alert('所属分类实体不能为空!', null, 1000);
			return false;
		}
