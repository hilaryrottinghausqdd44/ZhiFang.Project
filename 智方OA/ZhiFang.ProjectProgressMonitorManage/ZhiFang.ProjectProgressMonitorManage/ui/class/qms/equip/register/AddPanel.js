 ...
 * @version 2016-08-26
 */
Ext.define('Shell.class.qms.equip.register.AddPanel', {
	...
	afterRender: function() {
		...
		me.EditTabPanel.on({
			onAddClick:function(grid){//TB列表列表新增
				me.isAdd();
			},
			selectclick:function(record,grid,fields){//TB列表 选择行,加载数据
				me.onSelectTB(record,grid,fields);
			},
			nodata:function(form){//TB列表数据为空时
   		    	var num=0,arr=[];
   		    	me.onResetClick();
	            me.fireEvent('loadSaveDataClick', []);	
   		    },
   		    onDelClick:function(){//TB列表的删除按钮
   		    	me.fireEvent('onDelClick');	
   		    },
   		    loadsearch:function(list){//tb列表数据加载完成后
   		    	me.IsLoadData=false;
   		        if(list>0){
   		        	me.IsLoadData=true;
   		        }
   		    } ,
			blur:function(com){ //时间改变 失去焦点 查询数据
 		    ...
 		    onDailyClick:function(com){//载入按钮
		    	JShell.Action.delay(function() {
		    		me.showMask("正在载入数据...");
		    		if(me.IsTbType!='TB'){
		    			me.getloadDailyData();
   		    ...
   		    resize : function(com,  width,  height,  oldWidth,  oldHeight,  eOpts ){ //窗体大小改变后tab按钮收缩
			    me.setResizeBtn();
   		    }
		});
		me.MemoTabPanel.on({
			uploadClick:function(com){//附件上传完毕事件
	...
	//tb列表选择行
	onSelectTB : function(record,grid,fields){
		var me =this;
		me.showMask();
		me.isTbAdd=1;
		JShell.Action.delay(function() {
	...
	createItems: function() {
		...
		me.EditTabPanel = Ext.create('Shell.class.qms.equip.register.EditTabPanel', {
			border: false,
			title: '操作列表',
			region: 'center',
			header: false,
	...
	saveData: function() {
	    ...
	    entityList=me.EditTabPanel.Form.SaveMaintenanceData(me.NowTabData, me.TempletID,me.NowTabType,operatedate,ItemMemoVal,me.isTbAdd,me.BatchNumber,me.TempletBatNo,me.IsAdd);
	    ...
	    var entity =me.MemoTabPanel.Form.SaveFormData(me.TempletID, me.NowTabType,operatedate,me.isTbAdd,me.BatchNumber,me.TempletBatNo,me.IsAdd);
    ...
    * */
	...
	onSeachTb:function(Operatedate){
		var me=this;
		me.clearMemoData();
		me.EditTabPanel.onSearch(me.TempletID,Operatedate,me.NowTabType,me.IsTbType);
	},
	/**MemoTabPanel赋值*/
...
//		var Sysdate ="2018-08-16";		
	...
	/**根据选择行加载模板
	 * 根据模板id找到模板内容 生成按钮(模板)
	 * */
	changeTab:function(id,CheckType,TempletBatNo){
		var me=this;
		me.BUTTON_CLICK_COUNT=0;
		me.IsTbType='';
		me.TempletID=id;
		me.TempletBatNo = TempletBatNo;
	    me.EditTabPanel.Form.TempletID = id;
        //传入审核类型
		...
		me.getTempletById(id,function(data){
			if(data && data.value){
				...
				me.IsTbType='';
				me.IsLoadData=false;
				me.createTab(buttonsToolbar,data.value.list,numcount);
			}
		});
	},
	/**
	 * 根据模板解析并创建按钮页签*/
	createTab:function(buttonsToolbar,arr,numcount){
		var me =this;
		...
		//判断是否需要滚动条
		me.createBtnScrollBar(numcount);
		//单击按钮加载
		me.onBarClick(buttonsToolbar);
	
	},
	/**生成按钮加滚动条*/
	createBtnScrollBar : function (numcount){
		var me = this;
		//判断是否需要滚动条
	...
	/**单击工具栏按钮*/
	onBarClick : function(buttonsToolbar){
		var me=this;
		var listSaveArr=[];
				...
				buttonsToolbar.items.items[i].on({
					...
					toggle: function (com, pressed, opts) {
			            ...
			            if (pressed) {
						    ...
						    var stratDate=me.getOperateDate();
						    //需要一键保存
		                    if(me.IsOnekey=='1' && com.IsTbType!='TB'){
						    	//取当前页签数据
							    ...
							    if(!isLoad){
							    	me.loadDataByBtn();
		    	                    me.allSaveArr= me.getAllSaveInfo(me.allSaveArr,com);
							    }
						    }else{
								me.loadDataByBtn();
							}
			            }
			            if(!pressed && !com.IsTbType && me.IsOnekey=='1'){//页签切换前的数据（上一页签)
			            	me.allSaveArr= me.getAllSaveInfo(me.allSaveArr,com);
			            }
			            if(!pressed && com.IsTbType=='TB' && me.IsOnekey=='1'){//页签切换前的数据（上一页签)
			            	me.allSaveArr= me.getAllSaveInfo(me.allSaveArr,com);
			            }
			            me.hideMask();
			        }
...
//		me.TempletBatNo = TempletBatNo;
	...
	/**根据类型设置时间*/
	setDateValue:function(Sysdate){
		var me =this;
		var startDate=null,endDate=null;
		if(me.CheckType=='1'){//按天审核
    ...
    */
	getEParaVal: function(callback) {
        ...
        var where="&fields="+fields+"&where=ParaType='QualityRecord' and (ParaNo = 'IsAutoSaveLoadData' or ParaNo = 'IsSaveAllData'  or ParaNo = 'IsSaveDataPreview' or ParaNo = 'IsShowLoadDataButton')";
		url+=where;
   ...
   onChangeResult2:function(data){
	...
	/**还原表单*/
	createTbItemData:function(stratDate){
		var me=this;
		//动态生成表单
		me.createMaintenanceDataForm();
		//还原数据
	...
	onSaveClick: function() {
		...
		me.BUTTON_CLICK_COUNT = me.BUTTON_CLICK_COUNT+1;
		if(me.BUTTON_CLICK_COUNT==1)me.BUTTON_DELAY_TIME =0;
		    else  
		       me.BUTTON_DELAY_TIME =500;   
		...
		}, null, me.BUTTON_DELAY_TIME);
		
	},
	isAdd:function(TempletBatNo){
		var me = this;
		me.IsAdd=true;
		me.TempletBatNo = TempletBatNo;
		me.isTbAdd=0;
