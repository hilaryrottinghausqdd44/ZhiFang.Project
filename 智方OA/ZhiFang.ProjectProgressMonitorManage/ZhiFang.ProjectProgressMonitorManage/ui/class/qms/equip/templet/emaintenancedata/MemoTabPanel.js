 ...
 * @version 2016-08-25
 */
Ext.define('Shell.class.qms.equip.templet.emaintenancedata.MemoTabPanel', {
	...
	createItems: function() {
		var me = this;
		me.Form = Ext.create('Shell.class.qms.equip.templet.emaintenancedata.MemoForm', {
			title: '备注',
			header: false,
			layout: "fit",
			itemId: 'Form'
	 ...
	 * 附件
	 * */
	getMemoCheckType: function(CheckType,Sysdate) {
		var me = this;
		 var startDate=null,endDate=null;
		if(CheckType=='1'){//按天审核
			var beginDate = JcallShell.Date.toString(Sysdate, true);
            startDate=beginDate;
            endDate=JShell.Date.getNextDate(1);
		}else{
			var beginDateStr = JcallShell.Date.toString(Sysdate, true);
			var b = beginDateStr.split("-");
			startDate = b[0] + '-' + b[1] + '-01 00:00:00';
			var endDateStr = JcallShell.Date.getMonthLastDate(b[0], b[1], true);
			endDate=endDateStr+' 23:59:59';
//			if(beginDate && endDate) {
//				params.push("eattachment.FileUploadDate between '" + beginDate + "' and '" + endDate2 + "'");
//			}
            
		}
		me.Attachment.endDate=endDate;
		me.Attachment.startDate=startDate;
	},
