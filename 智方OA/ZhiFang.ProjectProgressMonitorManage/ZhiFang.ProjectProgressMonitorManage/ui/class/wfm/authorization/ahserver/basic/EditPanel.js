...
Ext.define('Shell.class.wfm.authorization.ahserver.basic.EditPanel', {
	...
	/**获取数据服务路径*/
	selectUrl: '/ProjectProgressMonitorManageService.svc/ST_UDTO_SearchAHServerLicenceAndAndDetailsById',
	editUrl: '/ProjectProgressMonitorManageService.svc/ST_UDTO_UpdateAHServerLicenceAndDetailsByField',
		...
		JShell.Server.get(url, function(data) {
			if (data.success) {
				...
				if (data.value != null) {
					...
					//加载服务器授权备注
					if (me.EditTabPanel.MemoForm) {
						if (me.AHServerLicence.Comment) {
							me.EditTabPanel.MemoForm.getForm().setValues({
								"AHServerLicence_Comment": me.AHServerLicence.Comment
							});
						} else {
							me.EditTabPanel.MemoForm.isEdit(me.PK);
						}
					}
				}
			} else {
	...
	onSave: function(Status) {
		...
		if (isSave == true) {
			...
			me.EditTabPanel.ProgramLicenceGrid.store.each(function(record) {
				...
				} else if (isSave == true && LicenceTypeId != info.Id) { //如果授权类型不是商业,截止日期不能为空
		...
		if (isSave == true) {
			...
			//服务器授权备注信息
			var values2 = me.EditTabPanel.MemoForm.getForm().getValues();
			AHServerLicence.Comment = values2.AHServerLicence_Comment;
