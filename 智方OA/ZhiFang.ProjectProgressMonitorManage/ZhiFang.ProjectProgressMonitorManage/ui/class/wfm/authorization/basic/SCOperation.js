 ...
 * @version 2015-07-27
 */
Ext.define('Shell.class.wfm.authorization.basic.SCOperation', {
	...
	autoScroll: true,
	/**获取数据服务路径*/
	selectUrl: '/ProjectProgressMonitorManageService.svc/ST_UDTO_SearchAHOperationByHQL',
	/**获取获取类字典列表服务路径*/
	...
	changeHtml: function(list) {
		...
		for(var i = 0; i < len; i++) {
			...
			var obj = me.getInfoByID(data.Type);
			var Info = null;
			if(obj) {
				var style = [];
				if(obj.BGColor) {
				...
				if(data.Memo) {
					html.push('<b style="' + style.join(';') + '">' + obj.Name + '</b>  ' + '   处理意见：<b>' + data.Memo + '</b>');
				} else {
	...
	getStatusListData: function(callback) {
		...
		JcallShell.Server.post(url, params, function(data) {
			if(data.success) {
				if(data.value) {
					StatusListData = data.value[0].LicenceStatus;
