 ...
 * @version 2017-08-09
 */
Ext.define('Shell.class.wfm.business.projecttrack.follow.basic.ContentPanel', {
	...
	/**获取数据服务路径*/
	selectInfoUrl: '/ProjectProgressMonitorManageService.svc/ST_UDTO_SearchPContractFollowById',
	/**获取附件服务路径*/
	...
	loadInfoData:function(callback){
		...
		fields = "PContractFollow_" + fields.join(",PContractFollow_");
	...
	getInfoHtml:function(){
		...
		if(data.success){
			...
			if(data.value){
				html = html.replace(/{PClientName}/g,data.value.PContractFollow_PClientName || '');
				html = html.replace(/{PayOrg}/g,data.value.PContractFollow_PayOrg || '');
				html = html.replace(/{ContractNumber}/g,data.value.PContractFollow_ContractNumber || '');
				html = html.replace(/{Name}/g,data.value.PContractFollow_Name || '');
				
				html = html.replace(/{Amount}/g,data.value.PContractFollow_Amount || '');
				html = html.replace(/{PayedMoney}/g,data.value.PContractFollow_PayedMoney || '');
				html = html.replace(/{LeftMoney}/g,data.value.PContractFollow_LeftMoney || '');
				html = html.replace(/{InvoiceMoney}/g,data.value.PContractFollow_InvoiceMoney || '');
				html = html.replace(/{Compname}/g,data.value.PContractFollow_Compname || '');
				
				html = html.replace(/{Principal}/g,data.value.PContractFollow_Principal || '');
				html = html.replace(/{SignMan}/g,data.value.PContractFollow_SignMan || '');
				html = html.replace(/{SignDate}/g,JShell.Date.toString(data.value.PContractFollow_SignDate,true) || '');
				html = html.replace(/{PaidServiceStartTime}/g,
					JShell.Date.toString(data.value.PContractFollow_PaidServiceStartTime,true) || '');
				
				html = html.replace(/{IsInvoices}/g,data.value.PContractFollow_IsInvoices || '');
				html = html.replace(/{InvoiceMemo}/g,data.value.PContractFollow_InvoiceMemo || '');
				html = html.replace(/{Comment}/g,data.value.PContractFollow_Comment || '');
				me.LinkEquipInfoListHTML=data.value.PContractFollow_LinkEquipInfoListHTML || '无';
				me.PurchaseDescHTML=data.value.PContractFollow_PurchaseDescHTML || '无';
			}
		}else{
			html = me.getErrorTemplet();
			html = html.replace(/{Title}/g,'项目内容');
	...
	getInfoTemplet: function() {
		...
		'<div class="col-sm-12">' +
			'<div style="' + rDivStyle + '">' +
				...
				'<div style="' + sDivStyle + '">计划签署日期：{SignDate}</div>' +
