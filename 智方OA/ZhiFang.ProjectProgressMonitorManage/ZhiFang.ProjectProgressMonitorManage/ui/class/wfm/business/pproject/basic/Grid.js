...
Ext.define('Shell.class.wfm.business.pproject.basic.Grid', {
	...
	onSaveCopyProject: function(typeID,win) {
		var me = this,
			url = '/ProjectProgressMonitorManageService.svc/PM_UDTO_CopyProject';
	...
	getProjectType:function(){
		...
		url += "&fields=PDict_CName,PDict_Id&where=pdict.PDictType.DictTypeCode='ProjectType'";
		me.ItemTypeEnum = {};
		JShell.Server.get(url,function(data){
			if(data.success){
				if(data.value) {
					Ext.Array.each(data.value.list, function(obj, index) {
						tempArr = [obj.PDict_Id, obj.PDict_CName];
						me.ItemTypeEnum[obj.PDict_Id] = obj.PDict_CName;
