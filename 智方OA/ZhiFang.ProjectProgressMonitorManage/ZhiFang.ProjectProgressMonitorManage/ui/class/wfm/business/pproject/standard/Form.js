...
Ext.define('Shell.class.wfm.business.pproject.standard.Form', {
	...
	autoScroll: false,
	/**获取数据服务路径*/
	selectUrl: '/ProjectProgressMonitorManageService.svc/ST_UDTO_SearchPProjectById?isPlanish=true',
	/**新增服务地址*/
	addUrl: '/ProjectProgressMonitorManageService.svc/ST_UDTO_AddPProject',
	/**修改服务地址*/
	editUrl: '/ProjectProgressMonitorManageService.svc/ST_UDTO_UpdatePProjectByField',
    /**布局方式*/
	layout: 'anchor',
	/**每个组件的默认属性*/
	defaults: {
	...
	createItems: function() {
        ...
        var TypedefaultWhere="pdict.PDictType.DictTypeCode='" + this.ProjectType+"'";
        if(TypeID){
		...
		if(PProject_Type) {
			PProject_Type.on({
				check: function(p, record) {
					PProject_Type.setValue(record ? record.get('PDict_CName') : '');
					PProject_TypeID.setValue(record ? record.get('PDict_Id') : '');
					p.close();
				}
			});
		}
	...
	changeResult: function(data) {
		...
		if(data.PProject_TypeID){
	        me.getProjectType(data.PProject_TypeID,function(data){
	        	if(data.value.list){
	        		PProject_Type.setValue(data.value.list[0].PDict_CName);
	        	}
	        });
		}else{
	...
	isValidProjectType:function(callback,TypeID){
		var me = this,
			values = me.getForm().getValues(),
			url = '/ProjectProgressMonitorManageService.svc/ST_UDTO_SearchPProjectByHQL';
	...
	getTypeById:function(callback){
		var me = this;
		var url = JShell.System.Path.ROOT + '/ProjectProgressMonitorManageService.svc/ST_UDTO_SearchPProjectByHQL?isPlanish=false';
