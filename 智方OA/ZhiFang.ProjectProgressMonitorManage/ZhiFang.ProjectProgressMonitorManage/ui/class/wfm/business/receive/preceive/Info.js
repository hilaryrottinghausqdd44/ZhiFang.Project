 ...
 * @version 2016-12-28
 */
Ext.define('Shell.class.wfm.business.receive.preceive.Info', {
	...
	/**获取数据服务路径*/
	selectInfoUrl: '/ProjectProgressMonitorManageService.svc/ST_UDTO_SearchPClientById',
	...
	loadInfoData:function(callback){
		...
		var fields = [
			'Id','Name','SName','Shortcode','Name','HospitalLevelName',
			'ProvinceName','CityName','SCTypeName','Principal','LinkMan',
			'PhoneNum','PhoneNum2','Address','MailNo','ClientTypename','VATBank',
			'Emall','Bman','ClientAreaName','HospitalTypeName','VATNumber','VATAccount','LicenceCode'
		];
	...
	getInfoTemplet: function() {
		...
		'<div class="col-sm-12">' +
			'<div style="' + rDivStyle + '">' +
				'<div style="' + sDivStyle + '"><b>区域名称：{ClientAreaName}</b></div>' +
				'<div style="' + sDivStyle + '">省份：{ProvinceName}</div>' +
			    '<div style="' + sDivStyle + '">客户类型：{ClientTypename}</div>' +
			    '<div style="' + sDivStyle + '">客户级别：{SCTypeName}</div>' +
				'<div style="' + sDivStyle + '">医院类别：{HospitalTypeName}</div>' +
				'<div style="' + sDivStyle + '">医院等级：{HospitalLevelName}</div>' +
