 ...
 * @version 2015-07-02
 */
Ext.define('Shell.class.wfm.business.expenseaccount.threeaudit.Grid', {
	...
	afterRender: function() {
		...
		me.on({
			itemdblclick: function(view, record) {
				...
				//四审退回
				var isBack = 10;
				if(Status == isApply || Status == isBack) {
					me.openEditForm(id);
				} else {
					me.openShowForm(id);
				}
	...
	onSearch: function(autoSelect) {
		...
		JShell.System.ClassDict.init('ZhiFang.Entity.ProjectProgressMonitorManage','PExpenseAccountStatus',function(){
	        ...
	        me.defaultWhere="(pexpenseaccount.IsSpecially=1   and ((pexpenseaccount.ThreeReviewManID="+UserId +")" +" or (pexpenseaccount.ThreeReviewManID=null and pexpenseaccount.Status=5))) and (pexpenseaccount.IsUse=1)" ;
	...
	openEditForm: function(id) {
		var me = this;
		JShell.Win.open('Shell.class.wfm.business.expenseaccount.threeaudit.EditPanel', {
			PK: id,
			SUB_WIN_NO: '6',
			listeners: {
				save: function(p, id) {
	...
	getStatusData: function(StatusList) {
		...
		for(var i in StatusList) {
			...
			if(obj.Id != 1 && obj.Id != 2 && obj.Id != 4 && obj.Id != 3 && obj.Id != 6 && obj.Id != 8) {
