...
Ext.define('Shell.class.wfm.business.expenseaccount.basic.Grid', {
	...
	/**删除数据服务路径*/
	delUrl: '/ProjectProgressMonitorManageService.svc/ST_UDTO_Delpexpenseaccount',
	/**获取获取类字典列表服务路径*/
		...
		if(PClientName) {
			PClientName.on({
				check: function(p, record) {
					PClientName.setValue(record ? record.get('PDict_CName') : '');
					PClientID.setValue(record ? record.get('PDict_Id') : '');
					p.close();
				},
				change: function() {
					me.onGridSearch();
