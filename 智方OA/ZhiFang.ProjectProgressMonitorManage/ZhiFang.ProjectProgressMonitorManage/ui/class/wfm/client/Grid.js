...
Ext.define('Shell.class.wfm.client.Grid',{
	...
	createButtonToolbarItems:function(){
		...
		var SourceList=[];
		SourceList=me.PClientSourceList;
		SourceList.splice(0,0,["", '请选择', 'text-align:center;']);
		//查询框信息
		me.searchInfo = {
		...
		buttonToolbarItems.unshift({
			xtype:'textfield',itemId:'LicenceID',width:170,labelWidth:60,emptyText:'客户服务编号',fieldLabel:'授权编码'
		},{
		...
		className:'Shell.class.wfm.dict.CheckGrid',emptyText:'区域',
			...
			emptyText: '客户来源',labelWidth:0,
			name: 'ProjectSourceName',
			itemId: 'ProjectSourceName',
			...
			triggerCls:'x-form-search-trigger',
			enableKeyEvents:true,
			itemId:'searchAgentName',
			width:120,emptyText:'代理商名称',
			onTriggerClick:function(){
				me.onSearch();
			},
			listeners:{
	...
	createGridColumns: function() {
		...
		},
		{
			...
			text:'客户服务编号',dataIndex:'PClient_LicenceCode',width:110,
			sortable:false,defaultRenderer:true
		},{
			...
			text:'客户来源',dataIndex:'PClient_ProjectSourceName',width:100,
			sortable:false,defaultRenderer:true
		},{
			text:'代理商',dataIndex:'PClient_AgentName',width:100,
			sortable:false,defaultRenderer:true
		},{
	...
	onAddClick:function(){
		...
		JShell.Win.open('Shell.class.wfm.client.Form',{
			resizable: false,
			formtype:'add',
			PClientSourceList:me.PClientSourceList,
			listeners:{
				save:function(p){
					p.close();
					me.onSearch();
	...
	openEditForm:function(id){
		...
		JShell.Win.open('Shell.class.wfm.client.Form',{
			resizable: false,
			formtype:'edit',
			PK:id,PClientSourceList:me.PClientSourceList,
			listeners:{
				save:function(p){
					p.close();
					me.onSearch();
	...
	getParms:function(){
		var me=this;
			...
			IsRepeat=false,
			IsNotUser=false,
			IsContract=false,ProjectSourceID=null,
			searchBman=null,ProjectSourceName=null,searchAgentName=null,
			params = [];
		...
		if(buttonsToolbar){
			...
			ProjectSourceName=buttonsToolbar.getComponent('ProjectSourceName').getRawValue();
		    ProjectSourceID=buttonsToolbar.getComponent('ProjectSourceName').getValue();

		    searchAgentName=buttonsToolbar.getComponent('searchAgentName').getValue();
		}
		if(buttonsToolbar2){
		...
		//客户来源
		if(ProjectSourceName && ProjectSourceID){
            params.push("pclient.ProjectSourceName='" + ProjectSourceName + "'");
		}
		//代理商
		if(searchAgentName){
			params.push("(pclient.AgentName like '%" + searchAgentName + "%')");
		}
		if(params.length > 0){
	...
	getProjectSource:function(){
		...
		url += "&fields=PDict_Id,PDict_CName&where=pdict.PDictType.Id='5022676312945296120'";
		JShell.Server.get(url,function(data){
			if(data.success){
				if(data.value) {
					var list =data.value.list;
//					me.PClientSourceList.push(["", '请选择', 'text-align:center;']);
					for(var i =0; i<list.length;i++){
                        tempArr = [list[i].PDict_Id, list[i].PDict_CName];
						me.PClientSourceList.push(tempArr);
					}
				}
			}
		},false);
