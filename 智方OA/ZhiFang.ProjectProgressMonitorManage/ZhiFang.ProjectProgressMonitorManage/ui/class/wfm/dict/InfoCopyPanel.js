...
Ext.define('Shell.class.wfm.dict.InfoCopyPanel',{
	...
	getOneInfo:function(record){
		...
		if(typeV == '1'){
			info = record.get('PDict_CName')
		}else if(typeV == '2'){
			info = record.get('PDict_CName') + 
				separatorV + record.get('PDict_Shortcode');
		}else if(typeV == '3'){
			info = record.get('PDict_CName') + 
				separatorV + record.get('PDict_Shortcode') + 
				separatorV + record.get('PDict_Memo');
		}
	...
	getOneRecord:function(info){
		...
		if(typeV == '1'){
			data.PDict_CName = arr[0];
		}else if(typeV == '2'){
			data.PDict_CName = arr[0];
			data.PDict_Shortcode = arr[1];
		}else if(typeV == '3'){
			data.PDict_CName = arr[0];
			data.PDict_Shortcode = arr[1];
			data.PDict_Memo = arr[2];
		}
