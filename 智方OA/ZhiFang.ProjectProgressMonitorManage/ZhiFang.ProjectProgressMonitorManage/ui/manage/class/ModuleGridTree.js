Ext.ns('Ext.manage');
Ext.define('Ext.manage.ModuleGridTree',{
	...
	createColumns:function(){
		...
		var columns = [{
			...
			text:'入口地址',dataIndex:'Url',width:300
		},{
			text:'数据加入时间',dataIndex:'DataAddTime',hidden:true,
			xtype:'datecolumn',
			format:'Y-m-d H:i:s'
		},{
			text:'数据更新时间',dataIndex:'DataUpdateTime',hidden:true,
			xtype:'datecolumn',
			format:'Y-m-d H:i:s'
		},{
	...
	createDockedItems:function(){
		...
		var dockedItems = [{
			...
			dock:'top',
			items:[
				{text:'更新',iconCls:'build-button-refresh',handler:function(button){me.load(button);}},
				{text:'新增一级模块',iconCls:'build-button-add',handler:function(button){me.dockItemsAdd();}},
				{text:'全部展开',iconCls:'build-button-arrow-in',handler:function(button){me.expandAll();}},
				{text:'全部收缩',iconCls:'build-button-arrow-out',handler:function(button){me.collapseAll();}}
			]
		}];
    ...
    load:function(button){
    	var me = this;
        //预防数据频繁刷新,树的数据显示不同步
        if(me.hasResponseData==true){
            me.hasResponseData=false;
            com.disabled=false;//false为可用
        }else{
            setTimeout(function(){me.delayRun(com);},1800);
        }
    },
    delayRun:function(com){
        var me = this;
        var myUrl = me.url;
