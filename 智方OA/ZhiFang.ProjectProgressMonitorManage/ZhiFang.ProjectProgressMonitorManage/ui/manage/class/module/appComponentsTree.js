 ...
 */
Ext.ns('Ext.manage');
Ext.define('Ext.manage.module.appComponentsTree',{
    ...
    getnodesChecked:function(){
        ...
        Ext.Array.each(data,function(record){
            if(record.get('leaf')){//是不是只有子节点才保存
                var strCName=''+record.get('text');
                strCName = strCName.replace(/\(/g,'-');
                strCName = strCName.replace(/\)/g,'');
                var arrCName=strCName.split('-');
                var cname=arrCName[0];
                var moduleOperCode='';
                if(arrCName.length>1){
                    moduleOperCode="'"+arrCName[1]+"'";
                    //moduleOperCode=moduleOperCode.substring(0,moduleOperCode.length-1);//去掉')'
                }
                var item= {
                        text:''+cname,
                        //Id:record.get('Id'),//应用的id
                        tid:record.get('tid'),//应用操作Id
                        moduleOperCode:moduleOperCode//内部编码
                    };
                dataArray.push(item);
            }
        });
