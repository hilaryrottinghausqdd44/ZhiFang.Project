...
namespace ZhiFang.ProjectProgressMonitorManage
{
    ...
    {
        ZhiFang.IBLL.ProjectProgressMonitorManage.IBFFile IBFFile { get; set; }
        ZhiFang.IBLL.ProjectProgressMonitorManage.IBFFileAttachment IBFFileAttachment { get; set; }
        ZhiFang.IBLL.ProjectProgressMonitorManage.IBFFileCopyUser IBFFileCopyUser { get; set; }
        ZhiFang.IBLL.ProjectProgressMonitorManage.IBFFileInteraction IBFFileInteraction { get; set; }
        ZhiFang.IBLL.ProjectProgressMonitorManage.IBFFileOperation IBFFileOperation { get; set; }
        ZhiFang.IBLL.ProjectProgressMonitorManage.IBFFileReadingLog IBFFileReadingLog { get; set; }
        ZhiFang.IBLL.ProjectProgressMonitorManage.IBFFileReadingUser IBFFileReadingUser { get; set; }
        ...
        /// <returns></returns>
        public BaseResultDataValue QMS_UDTO_SearchReviseFFileListByFileID(long fileId, string where, int limit, int page, bool isPlanish, string fields, string sort)
        {
            BaseResultDataValue tempBaseResultDataValue = new BaseResultDataValue();
            EntityList<FFile> tempEntityList = new EntityList<FFile>();
            try
            {
                tempEntityList = IBFFile.SearchReviseFFileListByFileID(fileId, where, page, limit, CommonServiceMethod.GetSortHQL(sort));
        ...
        public BaseResultBool QMS_UDTO_DelFFileOfPhysicalById(long id)
        {
            BaseResultBool tempBaseResultBool = new BaseResultBool();
            try
            {
                tempBaseResultBool = IBFFile.DeleteFFileOfPhysicalById(id);
            }
            catch (Exception ex)
            {
                tempBaseResultBool.success = false;
        ...
        public List<string> GetSrcList(XmlNode xd)
        {
            List<string> srclist = new List<string>();
            if (xd != null && xd.ChildNodes.Count > 0)
            {
                foreach (XmlNode xn in xd.ChildNodes)
                {
                    ZhiFang.Common.Log.Log.Error("GetSrcList.xn:" + xn.InnerXml);
                    if (xn.Attributes != null && xn.Attributes["src"] != null && xn.Attributes["src"].Value != null && xn.Attributes["src"].Value.Trim() != "")
                    {
                        srclist.Add(xn.Attributes["src"].Value.Trim());
                    }
                    if (xn.Attributes != null && xn.Attributes["href"] != null && xn.Attributes["href"].Value != null && xn.Attributes["href"].Value.Trim() != "")
                    {
                        srclist.Add(xn.Attributes["href"].Value.Trim());
                    }
                    if (xn.ChildNodes != null && xn.ChildNodes.Count > 0)
                    {
                        srclist.AddRange(GetSrcList(xn));
                    }
                }
            }
            return srclist;
