...
namespace ZhiFang.BLL.OA
{
	...
	///
	/// </summary>
	public  class BBWeiXinAccount : BaseBLL<BWeiXinAccount>, ZhiFang.IBLL.OA.IBBWeiXinAccount
	{
        ...
        public void PushWeiXinMessage(SysWeiXinTemplate.PushWeiXinMessage pushWeiXinMessageAction, List<long> receiveidlist, Dictionary<string, TemplateDataObject> pdata, string syscode, string url)
        {
            if (receiveidlist.Count > 0)
            {
                IList<ZhiFang.Entity.OA.BWeiXinEmpLink> bwxell = IDBWeiXinEmpLinkDao.GetListByHQL(" EmpID in (" + string.Join(",", receiveidlist.ToArray()) + ") ");
                if (bwxell != null && bwxell.Count > 0)
                {
                    ...
                    {
                        case "pinvoice": break;
                        case "task": Shortcode = SysWeiXinTemplate.新任务提醒; break;
                        case "atleave": Shortcode = SysWeiXinTemplate.请假审批提醒; break;
                        case "ategress": Shortcode = SysWeiXinTemplate.考勤提醒; break;
                        case "attrip": Shortcode = SysWeiXinTemplate.出差审批通知; break;
                        case "atovertime": Shortcode = SysWeiXinTemplate.考勤提醒; break;
                        case "ploanbill": break;
                        case "pexpenseaccount": break;
                        case "prepayment": break;
                        case "contract": break;
                        case "receiveplan": break;
                        case "receive": break;
                        case "pcustomerservice": break;
                        case "pworkloginteraction": break;
                        case "file": break;
                        case "licence": break;
                    }

                    IList<ZhiFang.Entity.OA.BWeiXinPushMessageTemplate> bwxpmtl = IDBWeiXinPushMessageTemplateDao.GetListByHQL(" Shortcode='" + Shortcode + "' ");
