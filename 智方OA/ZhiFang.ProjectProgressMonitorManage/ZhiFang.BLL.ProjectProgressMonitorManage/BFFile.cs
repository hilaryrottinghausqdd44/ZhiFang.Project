...
namespace ZhiFang.BLL.ProjectProgressMonitorManage
{
    ...
    ///
    /// </summary>
    public class BFFile : BaseBLL<FFile>, ZhiFang.IBLL.ProjectProgressMonitorManage.IBFFile
    {
        ...
        IDFFileInteractionDao IDFFileInteractionDao { get; set; }
        IDFFileAttachmentDao IDFFileAttachmentDao { get; set; }
        IDBParameterDao IDBParameterDao { set; get; }
        ...
        /// <returns></returns>
        public FFile GetFFileAndAddFFileCopyUser(long longID, int isAddFFileReadingLog, int isAddFFileOperation)
        {
            FFile ffile = null;
            try
            {
        ...
        {
            string strHqlWhere = GetHQLByWhere(where, isSearchChildNode);
            return ((IDFFileDao)base.DBDao).SearchFFileReadingUserListByHQLAndEmployeeIDCookie(strHqlWhere, page, count);
        }
        /// <summary>
        /// 依员工Id查询阅读文档信息
        /// </summary>
        ...
        private string GetHQLByWhere(string where, bool isSearchChildNode)
        {
            ...
            if (!String.IsNullOrEmpty(where))
                tmpa = where.ToString().Trim().Split('^');
        ...
        public bool FFileWeiXinMessagePushById(SysWeiXinTemplate.PushWeiXinMessage pushWeiXinMessageAction, long id)
        {
                ...
                if (ffilereadinguserlist.Count > 0)
                {
                    ...
                    switch (ffilereadinguserlist[0].Type)
                    {
                        case 1:
                            ...
                            {
                                ZhiFang.Common.Log.Log.Debug("ZhiFang.BLL.ProjectProgressMonitorManage.BFFile.FFileWeiXinMessagePushById：没有员工！");
                            }
                            break;
                        case 2:
                            ...
                            if (deptidlist.Count > 0)
                            {
                                ...
                                {
                                    ZhiFang.Common.Log.Log.Debug("ZhiFang.BLL.ProjectProgressMonitorManage.BFFile.FFileWeiXinMessagePushById：部门：" + string.Join(",", deptidlist.ToArray()) + "内没有员工！");
                                }
                            }
                            else
                            {
                                ZhiFang.Common.Log.Log.Debug("ZhiFang.BLL.ProjectProgressMonitorManage.BFFile.FFileWeiXinMessagePushById：抄送表内没有部门！");
                            }
                            break;
                        case 3:
                            ...
                            if (Roleidlist.Count > 0)
                            {
                                ...
                                {
                                    ZhiFang.Common.Log.Log.Debug("ZhiFang.BLL.ProjectProgressMonitorManage.BFFile.FFileWeiXinMessagePushById：角色：" + string.Join(",", Roleidlist.ToArray()) + "内没有员工！");
                                }
                            }
                            else
                            {
                                ZhiFang.Common.Log.Log.Debug("ZhiFang.BLL.ProjectProgressMonitorManage.BFFile.FFileWeiXinMessagePushById：抄送表内没有角色！");
                            }
                            break;
                        case 4:
                            foreach (FFileReadingUser ffilereadinguser in ffilereadinguserlist)
            ...
            {
                ZhiFang.Common.Log.Log.Debug("ZhiFang.BLL.ProjectProgressMonitorManage.BFFile.FFileWeiXinMessagePushById：参数id为空！");
                return false;
            }
        }
        /// <summary>
        ...
        /// <summary>
        /// 物理删除文档及其相关的记录信息
        /// FFileInteraction,FFileOperation,FFileReadingLog,FFileInteraction,FFileReadingUser,FFileCopyUser,FFileAttachment,FFile
        /// </summary>
        /// <param name="id"></param>
        /// <returns></returns>
        public BaseResultBool DeleteFFileOfPhysicalById(long id)
        {
            BaseResultBool tempBaseResultBool = new BaseResultBool();

            try
            {
                tempBaseResultBool.success = IDFFileInteractionDao.DeleteByFFileId(id);
                if (tempBaseResultBool.success == false)
                {
                    tempBaseResultBool.ErrorInfo = "删除文档交流信息失败!";
                    return tempBaseResultBool;
                }

                tempBaseResultBool.success = IDFFileOperationDao.DeleteByFFileId(id);
                if (tempBaseResultBool.success == false)
                {
                    tempBaseResultBool.ErrorInfo = "删除文档操作记录信息失败!";
                    return tempBaseResultBool;
                }
                tempBaseResultBool.success = IDFFileReadingLogDao.DeleteByFFileId(id);
                if (tempBaseResultBool.success == false)
                {
                    tempBaseResultBool.ErrorInfo = "删除文档阅读记录信息失败!";
                    return tempBaseResultBool;
                }
                tempBaseResultBool.success = IDFFileReadingUserDao.DeleteByFFileId(id);
                if (tempBaseResultBool.success == false)
                {
                    tempBaseResultBool.ErrorInfo = "删除文档阅读人信息失败!";
                    return tempBaseResultBool;
                }
                tempBaseResultBool.success = IDFFileCopyUserDao.DeleteByFFileId(id);
                if (tempBaseResultBool.success == false)
                {
                    tempBaseResultBool.ErrorInfo = "删除文档抄送人信息失败!";
                    return tempBaseResultBool;
                }
                tempBaseResultBool.success = IDFFileAttachmentDao.DeleteByFFileId(id);
                if (tempBaseResultBool.success == false)
                {
                    tempBaseResultBool.ErrorInfo = "删除文档附件信息失败!";
                    return tempBaseResultBool;
                }
                //清空相关OriginalFileID及FFile
                tempBaseResultBool.success = ((IDFFileDao)base.DBDao).DeleteByFFileId(id);
                if (tempBaseResultBool.success == false)
                {
                    tempBaseResultBool.ErrorInfo = "删除文档信息失败!";
                }
            }
            catch (Exception ex)
            {
                ...
                ZhiFang.Common.Log.Log.Error("物理删除文档信息失败:" + ex.Message);
                //throw;
            }
            return tempBaseResultBool;
        }
        public EntityList<FFile> SearchReviseFFileListByFileID(long fileId, string where, int limit, int page, string sort)
        {
            EntityList<FFile> entityList = new EntityList<FFile>();
            IList<FFile> ffileList = new List<FFile>();
            FFile ffile = this.Get(fileId);
            if (ffile == null)
                return entityList;

            if (!ffileList.Contains(ffile) && ffile.IsUse == 1)
                ffileList.Add(ffile);
            //获取某一文档之前的所有已修订的文档信息
            if (ffile.OriginalFileID.HasValue && ffile.Id != ffile.OriginalFileID.Value)
                SearchPFFileListByOriginalFileID(ffile.OriginalFileID.Value, 0, ref ffileList);
            //获取某一文档之后的所有已修订的文档信息
            SearchCFFileListByFileID(ffile.Id, 0, ref ffileList);

            ffileList = ffileList.OrderBy(p => p.DataAddTime).ToList();
            entityList.list = ffileList;
            entityList.count = ffileList.Count;
            return entityList;
        }
        /// <summary>
        /// 获取某一文档之前的所有已修订的文档信息
        ...
        /// <param name="ffileList"></param>
        public void SearchPFFileListByOriginalFileID(long originalFileID, long pfileID, ref IList<FFile> ffileList)
        {
            FFile ffile = this.Get(originalFileID);
            if (ffile == null)
                return;

            if (ffileList == null) ffileList = new List<FFile>();
            if (!ffileList.Contains(ffile) && ffile.IsUse == 1)
                ffileList.Add(ffile);
            //终止递归
            if (ffile.Id == pfileID) return;

            if (ffile.OriginalFileID.HasValue && ffile.Id != ffile.OriginalFileID.Value)
                SearchPFFileListByOriginalFileID(ffile.OriginalFileID.Value, ffile.Id, ref ffileList);
            else
                return;
        }
        /// <summary>
        ...
        /// <param name="ffileList"></param>
        public void SearchCFFileListByFileID(long fileID, long pfileID, ref IList<FFile> ffileList)
        {
            IList<FFile> tempList = this.SearchListByHQL(string.Format("ffile.OriginalFileID=" + fileID));
            if (tempList == null || tempList.Count <= 0)
                return;

            FFile ffile = tempList[0];
            if (ffileList == null) ffileList = new List<FFile>();
            if (!ffileList.Contains(ffile) && ffile.IsUse == 1)
                ffileList.Add(ffile);
            //终止递归
            ...
            if (ffile.OriginalFileID.HasValue && ffile.Id != ffile.OriginalFileID.Value)
                SearchCFFileListByFileID(ffile.Id, fileID, ref ffileList);
            else
                return;
        }
    }
