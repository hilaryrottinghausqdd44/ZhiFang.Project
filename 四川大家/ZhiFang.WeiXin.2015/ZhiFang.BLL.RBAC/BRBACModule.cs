...
{	
	public class BRBACModule : BaseBLL<ZhiFang.Entity.RBAC.RBACModule>, ZhiFang.IBLL.RBAC.IBRBACModule
    {
        ...
        #region 检验小组仪器逻辑被注释（以后要考虑模块如何同其它业务系统衔接）
        /*
        public object SearchTestGroupEquipRBACModuleToTree(string RBACModuleUseCode, bool IsListTree)
        {
            ...
            if ((tmplist != null) && (tmplist.list != null) && (tmplist.list.Count > 0))
            {
                ...
                for (int i = 0; i < TestGrouplist.Count(); i++)
                {
                    ...
                    tree.Tree = GetTestGroupEquipRBACModuleLeafTrees<RBACModule>(TestGrouplist.ElementAt(i).Id, TestGrouplist.ElementAt(i).UseCode, Equiplist, IsListTree);
                    //if (tree.Tree.Length > 0)
                    //{
                    //    tree.leaf = false;
                    //}
