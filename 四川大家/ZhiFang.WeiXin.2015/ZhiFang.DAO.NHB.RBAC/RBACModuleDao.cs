...
namespace ZhiFang.DAO.NHB.RBAC
{
    public class RBACModuleDao : Base.BaseDaoNHBService<RBACModule, long>, IDRBACModuleDao
    {
        ...
        {
            var pl = this.HibernateTemplate.Find<RBACModule>(" select distinct rbacm from RBACModule  rbacm join rbacm.RBACRoleModuleList  rrm join rrm.RBACRole  rr join rr.RBACEmpRoleList rer join rer.HREmployee hre where hre.Id = " + longHREmpID + " and rbacm.IsUse=true order by  rbacm.DispOrder,rbacm.DataAddTime ");
            return pl;
        }
