...
{
    public class UserConsumerItemDetailsDao : BaseDaoNHB<UserConsumerItemDetails, long>, IDUserConsumerItemDetailsDao
    {
        /// <summary>
        /// 查询项目统计报表数据
        /// </summary>
        ...
        /// <returns></returns>
        public IList<UserConsumerItemDetails> SearchUserConsumerItemDetailsList(UserConsumerFormSearch searchEntity, string strWhere, int page, int count)
        {
            IList<UserConsumerItemDetails> tempEntityList = new List<UserConsumerItemDetails>();
            ...
            List<string> paranamea = new List<string> { "ConsumptionStartDate", "ConsumptionEndDate", "BillingStartDate", "BillingEndDate", "SamplingStartDate", "SamplingEndDate", "UOFCode", "OSUserConsumerFormCode", "MRefundFormCode", "BonusFormCode", "AreaID", "UserAccountID", "DoctorAccountID", "OSUserConsumerFormStatus", "IsRefund", "ItemID","strWhere" };
            ...
            if (searchEntity.IsRefund)
            {
                if (paranamea.IndexOf("IsRefund") >= 0)
                    paravaluea[paranamea.IndexOf("IsRefund")] = "1";// searchEntity.IsRefund;
            }
            else
            {
                if (paranamea.IndexOf("IsRefund") >= 0)
            ...
            if (searchEntity.ItemID.HasValue)
            {
                if (paranamea.IndexOf("ItemID") >= 0)
                    paravaluea[paranamea.IndexOf("ItemID")] = searchEntity.ItemID.Value.ToString();
            }
            ...
            tempEntityList = base.HibernateTemplate.FindByNamedQueryAndNamedParam<UserConsumerItemDetails>("SP_Report_UserConsumerItemDetails", paranamea.ToArray(), paravaluea);
            return tempEntityList;
        }
