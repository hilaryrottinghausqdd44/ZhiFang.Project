...
namespace ZhiFang.WeiXin.Common
{
    public class JsonSerializer
    {
        ...
        public static string JsonDotNetSerializer(object obj)
        {
            JsonSerializerSettings settings = new JsonSerializerSettings();
            settings.ReferenceLoopHandling = ReferenceLoopHandling.Ignore;
            //string result = JsonConvert.SerializeObject(obj, Formatting.Indented, settings);  
            return JsonConvert.SerializeObject(obj, Formatting.None, settings);//去掉回车和空格。
        }

        public static T JsonDotNetDeserializeObject<T>(string json)
        {
    ...
    public class JsonSerializerClass
    {
        ...
        private string CombinationJsonStr(object propertyObject, string classTypeName, PropertyInfo tempProperty)
        {
            ...
            Type tempType = tempProperty.PropertyType;
            ...
            else
            {
                ...
                {
                    tempPropertyValue = ZhiFang.Common.Public.JsonSerializer.ByteTypeToString((byte[])propertyObject);
                }
                else if (tempType == typeof(DateTime?))
                {
                    //tempPropertyValue = DateTime.Parse(propertyObject.ToString()).Ticks.ToString();
                    tempPropertyValue = DateTime.Parse(propertyObject.ToString()).ToString(ZhiFang.Common.Public.JsonSerializer.JsonDateTimeFormat, DateTimeFormatInfo.InvariantInfo);
                }
                else
                {
                    tempPropertyValue = ConvertSpecialChar(propertyObject.ToString());
                }
                ResultStr = propertySeparator + classTypeName +
                            TierSeparator + propertyName + propertySeparator + ValueSeparator + CommonSeparator +
        ...
        private string CombinationJsonStrEx(object propertyObject, string classTypeName, PropertyInfo tempProperty)
        {
            ...
            else
            {
                ...
                else if (tempType == typeof(DateTime) || tempType == typeof(DateTime?))
                {
                    //tempPropertyValue = DateTime.Parse(propertyObject.ToString()).Ticks.ToString();
                    tempPropertyValue = DateTime.Parse(propertyObject.ToString()).ToString(ZhiFang.Common.Public.JsonSerializer.JsonDateTimeFormat, DateTimeFormatInfo.InvariantInfo);
                }
                else if (tempType == typeof(Boolean) || tempType == typeof(Boolean?))
                {
        ...
        public string GetObjectListPlanish<T>(IList<T> ObjectList, bool isPOName = true)
        {
            string ResultStr = "";
            if (ObjectList != null)
            {
        ...
        public string GetSingleObjectPlanish<T>(T SingleObject, bool isPOName = true)
        {
            //自定义属性对象序列化后的JSON串
            string tempResultStr = "";
            if (SingleObject != null)
            {
                GetObjectPropertyPlanishEx(SingleObject, SingleObject.GetType().Name, ref tempResultStr, isPOName);
                if (tempResultStr.Length > 0)
                {
                    //组合每个自定义属性对象序列化后的JSON串
        ...
        protected void GetObjectPropertyPlanishEx(Object WillPlanishObject, string TypeName, ref string ResultStr, bool isPOName = true)
        {
            if (ParaList.Count <= 0)
            {
            ...
            foreach (string tempPropertyNameStr in ParaList)
            {
                ...
                for (int i = j; i < tempPropertyArray.Length; i++)
                {
                    tempInt++;
                    ...
                    if (tempPropertyInfo != null)
                    {
                        ...
                        if (tempObject != null)
                        {
                            if (tempInt == tempPropertyArray.Length - j)
                            {
        ...
        /// <returns></returns>
        private string ConvertSpecialChar(string SourceStr)
        {
        ...
        public string GetObjectPropertyNoPlanish<T>(T WillPlanishObject, string fields)
        {
            if (WillPlanishObject != null)
            {
                ...
                return JsonConvert.SerializeObject(WillPlanishObject, Formatting.None, Settings);//去掉回车和空格。
                //return JsonConvert.SerializeObject(WillPlanishObject, Formatting.Indented, Settings);
            }
            else
            {
