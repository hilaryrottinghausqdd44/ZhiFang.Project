...
namespace ZhiFang.WeiXin.Common
{
    public class MyNPOIHelper
    {
                ...
                using (FileStream fileStream = new FileStream(strModuleFileName, FileMode.Open, FileAccess.Read))
                {
                    ...
                    if (dicCellValue != null && dicCellValue.Count > 0)
                    {
                        foreach (KeyValuePair<string, string> kv in dicCellValue)
                        {
                            ...
                            if (array.Length == 2)
                                MyNPOIHelper.FillExcelMoudleSheetCell(sheet, int.Parse(array[0]), int.Parse(array[1]), kv.Value);
                            else if (array.Length == 3 && array[2].ToUpper() == "P")
                                MyNPOIHelper.FillExcelMoudleSheetCellImage(sheet, int.Parse(array[0]), int.Parse(array[1]), kv.Value); ;
                        }
                    }
                }
        ...
        public static void FillExcelMoudleSheetCell(ISheet sheet, int row, int col, string value)
        {
            IRow headerRow;
            headerRow = sheet.GetRow(row);
            if (headerRow != null && (!string.IsNullOrEmpty(value)))
            {
        ...
        public static void FillExcelMoudleSheetCellImage(ISheet sheet, int row, int col, string filePath)
        {
        ...
        {
            IList<string> dt = new List<string>();
            IWorkbook wb;
            try
            {
                ...
                dt = GetExcelMoudleFile(sheet, 0, true);
            }
            catch (Exception ex)
            {
                ...
                for (int i = (HeaderRowIndex + 1); i <= sheet.LastRowNum; i++)
                {
                    try
                    {
                        ...
                        for (int j = row.FirstCellNum; j < cellCount; j++)
                        {
                            ...
                            {
                                //Log.Info("检查Excel文件中的行数据出错：" + ex.Message);
                                throw new Exception("检查Excel文件中的行数据出错：" + ex.Message);
                            }
                        }
                    }
                    catch (Exception ex)
                    {
                        //Log.Info("检查Excel文件循环读取Excel行出错：" + ex.Message);
                        throw new Exception("检查Excel文件循环读取Excel行出错：" + ex.Message);
                    }
                }
            }
            catch (Exception ex)
