...
{
    public class ExportDTtoExcelHelp : MyNPOIHelper
    {
        /// <summary>
        /// DataTable导出到Excel文件
        /// </summary>
        ...
        public static FileStream ExportDTtoExcellHelp(DataTable dtSource, string strHeaderText, string strFileName, Dictionary<string, short> cellFontStyleList)
        {
            FileStream fs = null;
            try
            {
                ...
                if (temp[temp.Length - 1] == "xls" && dtSource.Columns.Count < 256 && dtSource.Rows.Count < 65536)
                {
                    ...
                    {
                        using (fs = new FileStream(strFileName, FileMode.Create, FileAccess.Write))
                        {
                ...
                else
                {
                    ...
                    fs = ExportDTByDataTable(dtSource, strHeaderText, strFileName, cellFontStyleList);
                }
            }
            catch (Exception ee)
            {
                ZhiFang.Common.Log.Log.Error("DataTable导出到Excel文件失败:" + ee.StackTrace);
                return null;
            }
            return fs;
        }
                    ...
                    switch (column.DataType.ToString())
                    {
                        ...
                        case "System.Double":
                            if (!String.IsNullOrEmpty(drValue))
                            {
                                double doubV = 0;
                                double.TryParse(drValue, out doubV);
                                //newCell.Alignment = NPOI.SS.UserModel.HorizontalAlignment.Center;
                                newCell.SetCellValue(doubV);
                            }
                            break;
                        case "System.DBNull": //空值处理
