		...
		report2:{
			...
			get_html:function(callback){
				shell_win.memory.member.get_list(function(data){
					...
					html.push('<div style="margin-bottom:5px;font-size:14px;margin-left:15px;color:gray;">就诊人</div>');
					html.push('<ul id="report2_member_ul" class="list-group">');
					
					for(var i=0;i<len;i++){
						html.push('<li class="list-group-item" id="report2_member_li_');
						html.push(data[i].Id);
						html.push('" on');
						html.push(Shell.util.Event.touch);
						html.push('="shell_win_all.report2.on_member_row_touch(\'');
						html.push(data[i].Id);
						html.push('\',\'');
						html.push(data[i].Name);
						html.push('\',\'');
						...
						html.push('<span class="list_group_item_right_arrow">〉</span>');
						if(data[i].UnReadRFCount > 0){
							html.push('<span class="badge" style="background-color:#3c763d" id="report2_member_li_');
							html.push(data[i].Id);
							html.push('_badge">');
							html.push(data[i].UnReadRFCount);
							html.push('</span>');
					...
					html.push('</ul>');
					html.push('<div style="margin-bottom:5px;font-size:14px;margin-left:15px;color:gray;">报告列表</div>');
					html.push('<ul id="report2_ul_view" class="list-group"></ul>');
			...
			on_member_row_touch:function(id,name,report_ids){
				if(shell_win.report2.last_member_id){
					$("#report2_member_li_" + shell_win.report2.last_member_id).removeClass("button_touch");
				}
				shell_win.report2.last_member_id = id;
				$("#report2_member_li_" + id).addClass("button_touch");
				
				shell_win.report2.get_report_list(id,name,report_ids,function(data){
					var len = data.length,
						html = [];
					
					for(var i=0;i<len;i++){
						html.push('<li class="list-group-item" id="report2_report_li_');
						html.push(data[i][shell_win.report.obj.info][shell_win.report.obj.info_obj.report_id]);
						html.push('" on');
						html.push(Shell.util.Event.touch);
						...
						var has_read = shell_win.report2.has_read(data[i][shell_win.report.obj.info][shell_win.report.obj.info_obj.report_id]);
						if(!has_read){
							html.push(
								'<div id="report2_li_status_"' + data[i][shell_win.report.obj.info][shell_win.report.obj.info_obj.report_id] + 
					...
					$("#report2_ul_view").html(html.join(""));
				});
			},
			/**是否已阅读*/
			has_read:function(id){
