...
$(function() {
	...
	var shell_win = {
		...
		report:{
			...
			/**数据行触摸处理*/
			on_member_row_touch:function(id,name){
				shell_win.report.get_report_list(id,name,function(data){
					var len = data.length,
						html = [];
		...
		account:{
			...
			/**获取页面*/
			get_html:function(callback){
				var list = shell_win.account.get_list();
					
				callback(shell_win.account.get_ul(list));
			},
			/**获取功能列表*/
			get_list:function(){
				var list = [{
