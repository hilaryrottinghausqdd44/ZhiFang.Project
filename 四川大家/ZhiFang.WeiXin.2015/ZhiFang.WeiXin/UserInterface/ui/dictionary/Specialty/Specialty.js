...
$(function(){
    ...
    //初始化专业表
    $('#tbSpecialty').datagrid({
        title:'专业',
        url:Shell.util.Path.rootPath+'/ServerWCF/DictionaryService.svc/ST_UDTO_SearchBSpecialtyByHQL?isPlanish=true',
        method:'GET',
        loadMsg:'数据加载...',
        rownumbers:true,
        pagination:true,
    ...
    function createColumns() {
        var columns = [
            [
                ...
                {field: 'BSpecialty_LabID', title: '实验室ID', width: fixWidth(0.2) },
                {field: 'BSpecialty_Id', title: '专业ID', width: fixWidth(0.2) },
                {field: 'BSpecialty_Name', title: '名称', width: fixWidth(0.2)},
                {field: 'BSpecialty_SName', title: '简称', width: fixWidth(0.2)},
                {field: 'BSpecialty_Shortcode', title: '快捷码', width: fixWidth(0.2)},
                {field: 'BSpecialty_PinYinZiTou', title: '汉字拼音字头', width: fixWidth(0.2)},
                {field: 'BSpecialty_Comment', title: '备注', width: fixWidth(0.2)},

                {field:'BSpecialty_IsUse',title:'是否使用',width:fixWidth(0.2),
                    formatter:function(value,row){
                        var useCName=row.BSpecialty_IsUse=="true"?'是':'否';
                        return useCName;
                    },
                    styler:function(value,row){
                        var color=row.BSpecialty_IsUse=="true"?'black':'red';
                        return 'Color:'+color;
                    }},
    ...
    function setService(serviceType, serviceParam,entity, where) {
        ...
        switch (serviceType) {
            case'delete':
                serviceParam.serviceName = 'ST_UDTO_DelBSpecialty';
                serviceParam.type = 'GET';
                serviceParam.data =  where;
                break;
            case'add':
                serviceParam.serviceName = 'ST_UDTO_AddBSpecialty';
                serviceParam.type = 'POST';
                serviceParam.data =  entity;
                break;
            case'edit':
                serviceParam.serviceName = 'ST_UDTO_UpdateBSpecialtyByField';
                serviceParam.type = 'POST';
                serviceParam.data =  entity;
                break;
            case'validate':
    ...
    function getEntity(getType) {
        ...
        var LabID=$('#LabID').numberbox('getValue');
        if(!LabID)
            return 1;
        result+='"LabID":'+LabID+',';
        fields.push("LabID");

        var SpecialtyID=$('#SpecialtyID').numberbox('getValue');
        if(!SpecialtyID)
            return 1;
        result+='"Id":'+SpecialtyID+',';
        fields.push("Id");
        ...
        var UseCode=$('#UseCode').textbox('getValue');
        if(UseCode){
            result+='"UseCode":"'+UseCode+'",';
            fields.push("UseCode");
        }
        ...
        if(StandCode){
            ...
            fields.push("StandCode");
        }

        var DeveCode=$('#DeveCode').textbox('getValue');
        if(DeveCode){
            result+='"DeveCode":"'+DeveCode+'",';
            fields.push("DeveCode");
        }
    ...
    $('#addRow').bind('click',function addRow(){
        ...
        $('#dlg').window('center');
        $('#frm').form('clear');
        $('#LabID').numberbox('enable');
        $('#SpecialtyID').numberbox('enable');
        setCombobox([]);//设置下拉框
    });

    //删除
    $('#deleteRow').bind('click',function deleteRow(){
        var rows = $('#tbSpecialty').datagrid('getSelections') ||[],
            ...
            $.messager.confirm('确认', '确认删除吗？', function (btn) {
                if (btn) {
                    for (var i = 0; i < length; i++) {
                        askService('delete', null,{Id:rows[i]['BSpecialty_Id']});
                    }
                    $('#tbSpecialty').datagrid('clearSelections');//因为getSelections会记忆选过的记录，所以要清空一下
                }
            });
        }
    });
    ...
    $('#search').bind('searcher',function search(value){
        $('#tbSpecialty').datagrid('load',{fields:value});
    });
...
function setCombobox(row) {
    ...
    //是否使用
    $('#IsUse').combobox({
        valueField: 'BSpecialty_IsUse',
        textField: 'text',
        editable:false,
        panelHeight:50,
        data: [
            {BSpecialty_IsUse: 1, text: '是'},
            {BSpecialty_IsUse: 0, text: '否'}
        ],
        onLoadSuccess: function () {
            if (row.BSpecialty_IsUse=="false") {
...
function editRow(index){
    var curData = $('#tbSpecialty').datagrid('getData'),//返回当前页加载完毕的数据
    ...
    if(curRow){
        ...
        $('#dlg').window('center');
        $('#frm').form('clear');
        $('#LabID').numberbox('disable');
        $('#SpecialtyID').numberbox('disable');
