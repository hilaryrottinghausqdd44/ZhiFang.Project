$(function() {
	//获取医嘱单服务地址
	var GET_DOCTOR_ORDER_LIST_URL = JcallShell.System.Path.ROOT + "/ServerWCF/ZhiFangWeiXinAppService.svc/WXAS_BA_SearchDoctorOrderForm";
	...
	function getDoctorOrderListData(callback) {
		...
		var data = JcallShell.JSON.encode({
			page:1,
			limit:20
		});
		//获取数据
	...
	function changeDoctorOrderListHtml(list) {
		...
		for(var i in list) {
			...
			row = row.replace(/{Dept}/g, list[i].DPN ? ('-' + list[i].DPN) : '');
