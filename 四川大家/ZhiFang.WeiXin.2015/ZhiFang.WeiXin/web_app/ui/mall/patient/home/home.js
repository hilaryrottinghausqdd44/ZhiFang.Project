$(function () {
    ...
    //初始化我的账户信息
    function initMyInfo() {
        var src = MyInfo.BWeiXinAccount_HeadImgUrl || JcallShell.System.Path.UI + '/images/default/head.png';
        $("#AccountHeadImg").width("64px"); //头像
        $("#AccountHeadImg").attr("src", src); //头像

        var Name = MyInfo.BWeiXinAccount_Name || MyInfo.BWeiXinAccount_UserName;
        $("#AccountName").html(Name || ""); //姓名

        //强制密码
        if (MyInfo.BWeiXinAccount_LoginInputPasswordFlag) {
            USE_PASSWORD = true;
    ...
    function initListeners() {       
        ...
        $("#barcodeButton").on("click", function () {
            location.href = '../user/barcode.html';//用户信息二维码页面地址
        });
        //用户详细信息
        ...
        $("#usePassword").on("click", function () {
            ...
            JcallShell.Server.ajax({
                ...
                $.hideLoading();
                if (data.success) {
                    MyInfo.BWeiXinAccount_LoginInputPasswordFlag = USE_PASSWORD;
                } else {
                    $("#usePasswordDiv").hide();
                    $.alert("服务错误！", "错误提示");
                }
