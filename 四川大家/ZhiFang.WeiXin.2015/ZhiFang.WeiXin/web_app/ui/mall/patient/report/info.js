$(function () {
	...
	var GET_REPORT_INFO_URL = JcallShell.System.Path.ROOT + 
		"/ServerWCF/WeiXinAppService.svc/ST_UDTO_GetSearchAccountReportFormById";
	//获取报告PDF服务地址
	var GET_REPORT_PDF_URL = JcallShell.System.Path.ROOT + 
		"/ServerWCF/ZhiFangWeiXinAppService.svc/Get_ReportFormPDFURLByIndexId";
	...
	function getInfoData(callback){
		var url = GET_REPORT_INFO_URL + '?ReportFormIndexId=' + params.ID + 
			"&SearchAccountId=" + JcallShell.Cookie.get(JcallShell.Cookie.map.WEIXINUSERID);
	...
	function getReportUrl(callback){
		//var url = GET_REPORT_PDF_URL + '?ReportFormId=' + params.ID;
		var url = GET_REPORT_PDF_URL + '?ReportFormIndexId=' + params.ID
		//alert(url);
