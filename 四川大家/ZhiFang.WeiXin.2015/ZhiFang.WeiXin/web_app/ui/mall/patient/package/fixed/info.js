$(function () {
	...
	var PACKAGE_RECOMMENDED_URL = JcallShell.System.Path.ROOT + 
		'/ServerWCF/ZhiFangWeiXinService.svc/ST_UDTO_SearchBLabTestItemById';
	var FIELDS = [
		'BLabTestItem_Id','BLabTestItem_CName','BLabTestItem_MarketPrice','BLabTestItem_ItemDesc',
		'BLabTestItem_GreatMasterPrice','BLabTestItem_Price','BLabTestItem_Pic','BLabTestItem_BonusPercent'
	];
	...
	function changeHtml(data){
	    var defaultPic = JcallShell.System.Path.UI + '/images/home/top/001.png';
	    var pricehtml = '<div>折扣价格:￥' + data.Price + '</div>';
	    if (JcallShell.Cookie.get(JcallShell.Cookie.map.DoctorAccountType) && JcallShell.Cookie.get(JcallShell.Cookie.map.DoctorAccountType) == "2") {
	        pricehtml = '<div>大家价格:￥' + data.GreatMasterPrice + '</div>';
	    }
		var html = 
        ...
        pricehtml +
		'<div style="margin-top:10px;">' + (data.ItemDesc || '') + '</div>';
    ...
    function addDoctorOrder(){
    	...
    	var hasValue = JcallShell.LocalStorage.PatientOrder.addPackage(data);
    	if(hasValue){
