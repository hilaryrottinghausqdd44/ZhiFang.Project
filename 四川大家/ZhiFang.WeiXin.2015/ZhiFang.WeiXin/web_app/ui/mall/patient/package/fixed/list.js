$(function() {
	//获取套餐服务地址
	var PACKAGE_RECOMMENDED_URL = JcallShell.System.Path.ROOT + "/ServerWCF/ZhiFangWeiXinAppDoctService.svc/DS_UDTO_SearchOSTestItemByAreaID";
	...
	function getRowTemplet() {
	    var pricehtml = '<div><a>实际价:￥{Price}</a></div>';
	    if (JcallShell.Cookie.get(JcallShell.Cookie.map.DoctorAccountType) && JcallShell.Cookie.get(JcallShell.Cookie.map.DoctorAccountType)=="2") {
	        pricehtml = '<div><a>大家价:￥{GreatMasterPrice}</a></div>';
	    }
		var templet =
			'<div class="list-div">' +
				...
				'<div style="float:left;width:100%;">' +
					'<div class="list-div-button" onclick="addDoctorOrder(\'{Id}\',\'{Name}\',\'{MarketPrice}\',' +
						'\'{GreatMasterPrice}\',\'{Price}\',\'{Pic}\',\'{ItemNo}\',\'{BonusPercent}\')">加入</div>' +
				'</div>' +
			'</div>';
	...
	//加入医嘱单
    function addDoctorOrder(Id,CName,MarketPrice,GreatMasterPrice,Price,Pic,ItemNo,BonusPercent){
    	var data = {
    		Id:Id,
    		CName:CName,
