 ...
 * @version 2021-02-02
 */
Ext.define('Shell.class.weixin.blabSampleType.Form',{
	...
	createItems:function(){
		...
		var items = [
			{fieldLabel:'主键ID',disabled:true,name:'BLabSampleType_Id',itemId:'BLabSampleType_Id',emptyText:'必填项',allowBlank:false},
			{fieldLabel:'样本名称',emptyText:'必填项',allowBlank:false,name:'BLabSampleType_CName',itemId:'BLabSampleType_CName'},
			{fieldLabel:'样本编码',emptyText:'必填项',allowBlank:false,name:'BLabSampleType_LabSampleTypeNo',itemId:'BLabSampleType_LabSampleTypeNo'},
			{fieldLabel:'快捷码',name:'BLabSampleType_ShortCode'},
			{fieldLabel:'排序',name:'BLabSampleType_DispOrder'},
			{fieldLabel:'实验室编码',itemId:'BLabSampleType_LabCode',allowBlank:false,name:'BLabSampleType_LabCode',emptyText:'必填项',disabled:true,hidden:false},
			{fieldLabel:'实验室',xtype: 'uxCheckTrigger',name:'BLabSampleType_LabName',
				itemId: 'BLabSampleType_LabName',				
				className: 'Shell.class.weixin.blabSampleType.ClientCheckGrid',
				listeners: {
					check: function(p, record) {
						me.getComponent('BLabSampleType_LabCode').setValue(record ? record.get('CLIENTELE_Id') : '');
			            me.getComponent('BLabSampleType_LabName').setValue(record ? record.get('CLIENTELE_CNAME') : '');
						p.close();
					}
				}
			},
			{boxLabel:'是否使用',name:'BLabSampleType_Visible',
	...
	getAddParams:function(){
		...
		var BLabSampleType_LabCode = me.getComponent("BLabSampleType_LabCode").getValue();
		var entity = {
			Id:values.BLabSampleType_Id,
			...
			LabCode:BLabSampleType_LabCode,
	...
	getEditParams:function(){
		...
		for(var i in fields){
			if(fields[i] != "BLabSampleType_LabName"){
		...
		entity.entity.Id = me.getComponent("BLabSampleType_Id").getValue();
		return entity;
	},
	/**@overwrite 返回数据处理方法*/
	...
	initFromListeners:function(){
		...
		var BLabSampleType_CName = me.getComponent('BLabSampleType_CName');
		BLabSampleType_CName.on({
			change:function(field,newValue,oldValue,eOpts){
				if(newValue != ""){
					JShell.Action.delay(function(){
						JShell.System.getPinYinZiTou(newValue,function(value){
							me.getForm().setValues({
								BLabSampleType_ShortCode:value
							});
						});
					},null,200);
				}else{
					me.getForm().setValues({
						BLabSampleType_ShortCode:""
					});
				}
			}
		});
		...
		JShell.Server.get(url,function(data){
			...
			if(data.success){
				if(data.value){
					...
					if(data.value.BLabSampleType_LabCode){
						var clenturl = JShell.System.Path.ROOT+'/ServerWCF/ZhiFangWeiXinService.svc/ST_UDTO_SearchCLIENTELEById?isPlanish=true';
						clenturl +="&id="+data.value.BLabSampleType_LabCode+"&fields=CLIENTELE_CNAME";
						JShell.Server.get(clenturl,function(dataa){
							if(dataa.success){
								if(dataa.value){
									dataa.value = JShell.Server.Mapping(dataa.value);
									me.getComponent("BLabSampleType_LabName").setValue(dataa.value.CLIENTELE_CNAME);
								}else{
									me.getComponent("BLabSampleType_LabName").setValue("");
								}
							}
						},false);
					}
	...
	onSaveClick:function(){
		...
		if(me.formtype == 'add' && id){
			...
			sidurl +="&id="+id+"&fields=BLabSampleType_Id";
	...
	isAdd:function(){
		...
		me.getComponent("BLabSampleType_Id").setDisabled(false);
		me.formtype = 'add';
		me.PK = '';
		me.changeTitle();//标题更改
	...
	isEdit:function(id){
		...
		me.showButtonsToolbar(true);//显示功能按钮栏
		me.setReadOnly(false);
		me.getComponent("BLabSampleType_Id").setDisabled(true);
		me.formtype = 'edit';
		me.changeTitle();//标题更改
