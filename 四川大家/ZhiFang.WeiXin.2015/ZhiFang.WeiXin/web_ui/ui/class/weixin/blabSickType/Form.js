 ...
 * @version 2021-02-02
 */
Ext.define('Shell.class.weixin.blabSickType.Form',{
	...
	createItems:function(){
		...
		var items = [
			{fieldLabel:'主键ID',disabled:true,name:'BLabSickType_Id',itemId:'BLabSickType_Id',emptyText:'必填项',allowBlank:false},
			{fieldLabel:'样本名称',emptyText:'必填项',allowBlank:false,name:'BLabSickType_CName',itemId:'BLabSickType_CName'},
			{fieldLabel:'样本编码',emptyText:'必填项',allowBlank:false,name:'BLabSickType_LabSickTypeNo',itemId:'BLabSickType_LabSickTypeNo'},
			{fieldLabel:'快捷码',name:'BLabSickType_ShortCode'},
			{fieldLabel:'排序',name:'BLabSickType_DispOrder'},
			{fieldLabel:'实验室编码',itemId:'BLabSickType_LabCode',allowBlank:false,name:'BLabSickType_LabCode',emptyText:'必填项',disabled:true,hidden:false},
			{fieldLabel:'实验室',xtype: 'uxCheckTrigger',name:'BLabSickType_LabName',
				itemId: 'BLabSickType_LabName',				
				className: 'Shell.class.weixin.blabSampleType.ClientCheckGrid',
				listeners: {
					check: function(p, record) {
						me.getComponent('BLabSickType_LabCode').setValue(record ? record.get('CLIENTELE_Id') : '');
			            me.getComponent('BLabSickType_LabName').setValue(record ? record.get('CLIENTELE_CNAME') : '');
						p.close();
					}
				}
			},
			{boxLabel:'是否使用',name:'BLabSickType_UseFlag',
	...
	getAddParams:function(){
		...
		var BLabSickType_LabCode = me.getComponent("BLabSickType_LabCode").getValue();
		var entity = {
			Id:values.BLabSickType_Id,
	...
	getEditParams:function(){
		...
		for(var i in fields){
			if(fields[i] != "BLabSickType_LabName"){
		...
		entity.entity.Id = me.getComponent("BLabSickType_Id").getValue();
		return entity;
	},
	/**@overwrite 返回数据处理方法*/
	...
	initFromListeners:function(){
		...
		var BLabSickType_CName = me.getComponent('BLabSickType_CName');
		BLabSickType_CName.on({
			change:function(field,newValue,oldValue,eOpts){
				if(newValue != ""){
					JShell.Action.delay(function(){
						JShell.System.getPinYinZiTou(newValue,function(value){
							me.getForm().setValues({
								BLabSickType_ShortCode:value
							});
						});
					},null,200);
				}else{
					me.getForm().setValues({
						BLabSickType_ShortCode:""
					});
				}
			}
		});
		...
		JShell.Server.get(url,function(data){
			...
			if(data.success){
				if(data.value){
					...
					if(data.value.BLabSickType_LabCode){
						var clenturl = JShell.System.Path.ROOT+'/ServerWCF/ZhiFangWeiXinService.svc/ST_UDTO_SearchCLIENTELEById?isPlanish=true';
						clenturl +="&id="+data.value.BLabSickType_LabCode+"&fields=CLIENTELE_CNAME";
						JShell.Server.get(clenturl,function(dataa){
							if(dataa.success){
								if(dataa.value){
									dataa.value = JShell.Server.Mapping(dataa.value);
									me.getComponent("BLabSickType_LabName").setValue(dataa.value.CLIENTELE_CNAME);
								}else{
									me.getComponent("BLabSickType_LabName").setValue("");
								}
							}
						},false);
					}
	...
	onSaveClick:function(){
		...
		if(me.formtype == 'add' && id){
			...
			sidurl +="&id="+id+"&fields=BLabSickType_Id";
	...
	isAdd:function(){
		...
		me.getComponent("BLabSickType_Id").setDisabled(false);
		me.formtype = 'add';
		me.PK = '';
		me.changeTitle();//标题更改
	...
	isEdit:function(id){
		...
		me.showButtonsToolbar(true);//显示功能按钮栏
		me.setReadOnly(false);
		me.getComponent("BLabSickType_Id").setDisabled(true);
		me.formtype = 'edit';
		me.changeTitle();//标题更改
