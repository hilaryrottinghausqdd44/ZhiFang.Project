Ext.define('Shell.class.weixin.dict.lab.BLabDoctor.contrast.IntelligenceGrid', {
	...
	loadDatas:function(){
		...
		for(var i=0;i<labSickType;i++){
			var labSampleTypeCName=me.dictList[i].BLabDoctor_CName;
			var labSampleTypeLabSampleTypeNo=me.dictList[i].BLabDoctor_LabDoctorNo;
			var isContrast=me.dictList[i].BLabDoctor_isContrast;
            var tag='1';
			...
			for(var j=0;j<sickType;j++){
				if(me.ItemList[j]){
					var Id=me.ItemList[j].Doctor_Id;
				    var CName=me.ItemList[j].Doctor_CName;
				    if(Id==labSampleTypeLabSampleTypeNo || CName==labSampleTypeCName ){
				    	obj={
				    		labDoctor_LabDoctorNo:labSampleTypeLabSampleTypeNo,
				    		labDoctor_CName:labSampleTypeCName,
				    		Doctor_NO:Id,
				    		Doctor_CName:CName,
				    		Id:isContrast,
				    		Tag:tag
	...
	createGridColumns:function(){
		...
		var columns = [{
			text:'实验室样本编号',
			dataIndex:'labDoctor_LabDoctorNo',
			width:120,
			sortable:false,
			menuDisabled:true,
	...
	oneAdd:function(record){
        ...
        var labSickTypeNo=record.get('labDoctor_LabDoctorNo');
		var controlSickTypeID=record.get('Doctor_NO');
		var entity={
			DoctorControlNo:me.ClienteleId+"_"+controlSickTypeID+"_"+labSickTypeNo,
