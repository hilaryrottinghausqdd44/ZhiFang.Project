Ext.define("Shell.class.weixin.dict.lab.BLabSampleType.contrast.App",{
	...
	afterRender:function(){
		...
		me.LGrid.on({
			itemclick:function(v, record){
				...
				var sampleTypeId=record.data.BLabSampleType_sampleTypeId;
				
				for(var i =0 ; i< sampleTypeList.length;i++){
					if(sampleTypeId==sampleTypeList[i].data.SampleType_Id){
		...
		me.CBtn.on({
			...
			cancelClick:function(){
				...
				if(row[0] && row[0]!=='undefined'){
					if(row[0].get('BLabSampleType_isContrast')==''){
			...
			intelligenceClick:function(){
				...
				var dictList=me.LGrid.getLabSampleType();
				var itemList=me.RGrid.getSampleType();
				if(!dictList){
	...
	createPitems:function(){
		var me =this;
		me.LGrid=Ext.create("Shell.class.weixin.dict.lab.BLabSampleType.contrast.Grid",{
			region:'center',
		});
		me.CBtn=Ext.create("Shell.class.weixin.dict.lab.BLabSampleType.contrast.Btn",{
			region:'east',
			width:100,
			split: false,
			border:false,
		...
		me.RGrid=Ext.create("Shell.class.weixin.dict.lab.BLabSampleType.contrast.SampleType",{
	...
	createButtonToolbarItems:function(){
		...
		items.push(
			...
			},
			{
				...
				className:"Shell.class.weixin.dict.lab.BLabSampleType.contrast.CheckGrid",
				listeners:{
	...
	openIntelligenceForm:function(dictList,ItemList){
		...
		JShell.Win.open('Shell.class.weixin.dict.lab.BlabSampleType.contrast.IntelligenceGrid', {
