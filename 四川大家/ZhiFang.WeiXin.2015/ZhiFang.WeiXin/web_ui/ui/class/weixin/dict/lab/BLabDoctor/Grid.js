Ext.define("Shell.class.weixin.dict.lab.BLabDoctor.Grid",{
	...
	/**删除数据服务路径*/
	delUrl: '/ServerWCF/DictionaryService.svc/ST_UDTO_DelBLabDoctorAndControl',
	/**是否启用修改按钮*/
	hasEdit: false,
	/**是否启用保存按钮*/
	hasSave: false,
	...
	createDefaultButtonToolbarItems: function() {
		...
		items = [{
				...
				className: 'Shell.class.weixin.dict.lab.BLabDoctor.CheckGrid',
				listeners: {
	...
	onClienteleAcceptId:function(record){
		...
		if(id) {
			me.internalWhere=' blabdoctor.LabCode='+id;// 设置查询的内部条件
			me.onSearch();
		} else {
			me.clearData();
		}
	...
	getLabTestItem:function(){
		...
		for(var i =0;i<row.length;i++){
			arrId.push(row[i].get("BLabDoctor_Id"));
		}
		return arrId;
	},
	...
	onCopyClick:function(LabCodeList, ItemNoList, isall, ClienteleId){
		var me = this;
		JShell.Win.open('Shell.class.weixin.dict.lab.BLabDoctor.MsgFrom', {
			resizable: false,
			formtype:'add',
