 ...
 * @version 2017-02-20
 */
Ext.define('Shell.class.weixin.ordersys.refund.apply.TabPanel', {
	...
	createItems: function() {
		var me = this;
		me.Form = Ext.create('Shell.class.weixin.ordersys.refund.apply.Form', {
			itemId: 'Form',
	...
	saveData: function() {
		...
		if(checkbox == null || checkbox.length < 1) {
			JShell.Msg.alert("退款方式!", null, 1000);
			return;
		}
		var url = Form.formtype == 'add' ? Form.addUrl : Form.editUrl;
		url = (url.slice(0, 4) == 'http' ? '' : JShell.System.Path.ROOT) + url;
		var params = Form.formtype == 'add' ? Form.getAddParams() : Form.getEditParams();
		...
		JShell.Server.post(url, params, function(data) {
			if(data.success) {
				...
				me.hideMask(); //隐藏遮罩层
				JShell.Msg.alert("退款申请保存失败", null, 1000);
			}
		});
	}
});