 ...
 * @version 2017-02-20
 */
Ext.define('Shell.class.weixin.ordersys.refund.apply.Form', {
	...
	createRefundType: function(fieldLabel) {
		...
		me.RefundType = {
			...
			//				me.RefundType.removeAll();
			//				me.RefundType.add(array);
			//			},
	...
	createHideItems: function() {
		...
		items.push({
			...
			itemId: 'RefundApplyManName',
			name: 'RefundApplyManName'
		}, {
			fieldLabel: '申请人ID',
			hidden: true,
			value: (me.formtype == "add" ? userId : ""),
			name: 'RefundApplyManID',
			itemId: 'RefundApplyManID'
		});
	...
	isAdd: function() {
		...
		me.getForm().setValues({
			...
			UOFID: me.UOFID,
			RefundApplyManID: userId,
			RefundApplyManName: userName
		});
	},
	isEdit: function() {
		var me = this;
	...
	getAddParams: function() {
		...
		if(values.UOFID) entity.UOFID = values.UOFID;
		if(values.RefundApplyManID) entity.RefundApplyManID = values.RefundApplyManID;
		if(values.RefundApplyManName) entity.RefundApplyManName = values.RefundApplyManName;
		...
		if(values.RefundApplyTime) {
			entity.RefundApplyTime = JShell.Date.toServerDate(values.RefundApplyTime);
		}
		//概要
		if(values.Memo) {
			entity.Memo = values.Memo.replace(/\\/g, '&#92');
			entity.Memo = entity.Memo.replace(/[\r\n]/g, '<br />');
		}
		//退款原因
		if(values.RefundReason) {
			entity.RefundReason = values.RefundReason.replace(/\\/g, '&#92');
			entity.RefundReason = entity.RefundReason.replace(/[\r\n]/g, '<br />');
		}
	...
	getEditParams: function() {
		var me = this,
			...
			fieldsArr = ['Id', 'RefundApplyManName', 'RefundApplyManID', 'RefundApplyTime', 'RefundPrice', 'Memo', 'RefundReason', 'BankTransFormCode', 'RefundType', 'BankID', 'BankAccount'];
