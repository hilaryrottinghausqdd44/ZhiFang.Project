...
Ext.define("Shell.class.sysbase.bparameter.Form", {
	...
	addUrl: "/ServerWCF/ZhiFangWeiXinService.svc/ST_UDTO_AddBParameter",
	editUrl: "/ServerWCF/ZhiFangWeiXinService.svc/ST_UDTO_UpdateBParameterByField",
	selectUrl: "/ServerWCF/ZhiFangWeiXinService.svc/ST_UDTO_SearchBParameterById?isPlanish=true",
	/**启用表单状态初始化*/
	openFormType: true,
	...
	createHideItems: function() {
		...
		items.push({
			...
			xtype: "textfield",
			itemId: "BParameter_BDict_Id",
			hidden: true,
			name: "BParameter_BDict_Id",
			fieldLabel: "字典ID"
		});
	...
	getAddParams: function() {
		...
		if(values.BParameter_BDict_Id) {
			entity.BDict = {
				Id: values.BParameter_BDict_Id
			};
			if(me.formtype=="add")entity.BDict.DataTimeStamp=strDataTimeStamp.split(',');
		}
		//参数值
	...
	setBDictIdValue: function(value) {
		var me = this;
		if(me.formtype = "add") {
			BDictId = me.getComponent('BParameter_BDict_Id');
