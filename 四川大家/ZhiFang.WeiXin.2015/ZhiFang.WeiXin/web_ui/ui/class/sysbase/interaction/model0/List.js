 ...
 * @version 2015-07-02
 */
Ext.define('Shell.class.sysbase.interaction.model0.List', {
	...
	initComponent: function() {
		...
		me.paramsFields = ['SenderID','SenderName','Contents','DataAddTime'];
		for(var i=0;i<me.paramsFields.length;i++){
			me.paramsFields[i] = me.objectName + '_' + me.paramsFields[i];
		}
		me.paramsFields = me.paramsFields.join(',');
	...
	insertContent:function(value){
		...
		var isSender = value[me.objectName + '_SenderID'] == 
    	...
    	if(!isSender){
    		var name = value[me.objectName + '_SenderName'] + '&nbsp;&nbsp;' + 
    			JShell.Date.toString(value[me.objectName + '_DataAddTime']);
	    ...
	    ContentDiv.innerHTML = value[me.objectName + '_Contents'];
...
//	    parent.appendChild(Div);
	...
	getParams:function(){
		...
		var where = me.objectNameLower + ".IsUse=1 and " + me.objectNameLower + "." + me.fObejctName + ".Id=" + me.fObjectValue;
		params.push('where=' + where);
		
		params.push('fields=' + me.paramsFields);
		
		var sort = '[{"property":"' + me.objectName + '_DataAddTime","direction":"DESC"}]';
