...
JcallShell.Page = {
	...
	changeLangage:function(lang){
		...
		if(!lang || lang.toLocaleUpperCase() == 'CN'){
			//加载EXTJS中文包
			var url = JcallShell.System.Path.ROOT + '/web_src/extjs/locale/ext-lang-zh_CN.js';
...
JcallShell.Msg = {
	...
	show: function(msg, title, icon, times) {
		...
		if (this.delayF) clearTimeout(this.delayF);
		...
		if (times && times > 0) {
			this.delayF = setTimeout(function() {
				view.hide();
			}, times);
		}
	}
...
JcallShell.Date = {
	...
	getWeekStartDateAndEndDate: function(year, month, weekTims) {
		...
		var fristDayStr=year + "-" + month + "-1" + " 12:00:00";
		var fristDay=new Date(Date.parse(fristDayStr.replace(/-/g, "/")));
		
		//该月第一天星期几
		var weekOfDay = fristDay.getDay(); 
		var dayCount =JcallShell.Date.getCountDays(fristDayStr); //该月天数
		var endDay = 0; //结束日
		//周结束日期,初始化为当前时间
		endDay = new Date();
		var dateEnd = "";
         //7*周次 - 第一周中上月天数 > 本月天数
        if (7 * parseInt(weekTims) - weekOfDay > dayCount)
        {
            endDay = 7 * parseInt(weekTims) - weekOfDay - dayCount +1;
            var dateLast = year + '-' + month + '-' + new Date(year, month, 0).getDate(); //获取当月最后一天日期 
			var dateLastDay = new Date(dateLast);
			result.EndDate = Ext.util.Format.date(JcallShell.Date.getNextDate(dateLast, endDay), 'Y-m-d');
        }
		//7*周次 - 第一周中上月天数 > 本月天数
		if(7 * parseInt(weekTims) - weekOfDay <= dayCount) {
			endDay = 7 * parseInt(weekTims) - weekOfDay + 1;
			dateEnd = year + "-" + month + "-" + endDay.toString();
			result.EndDate = dateEnd;
		}
		var StartD = JcallShell.Date.getNextDate(result.EndDate, -6);
		result.EndDate = Ext.util.Format.date(JcallShell.Date.getNextDate(result.EndDate, 0), 'Y-m-d');
		result.StartDate = Ext.util.Format.date(StartD, 'Y-m-d');
		return result;
	},
	/**获取当前日期所在的月有多少周
