...
Ext.define('Ext.build.ParserForm',{
	...
	getClassCode:function(){
		...
		"Ext.define('" + params.appCode + "',{" + 
            ...
            "initComponent:function(){" + 
				...
				ClassCode = ClassCode + (html && html != '' ? ("me.html='" + html.replace(/\'/g,"@@") + "';") : "");
				//挂靠
	...
	createDockedItems:function(){
        ...
        if(params.hasSaveButton){//保存按钮
        	var but = "{xtype:'button',text:'保存',iconCls:'build-button-save',handler:function(but){me.submit();}}";
        	buttons.push(but);
        }
	...
	createTextCom:function(config){
		var me = this;
		var com = "{xtype:'textfield'," + me.createBasicCom(config) + "}";
		return com;
	},
	/**
	 * 创建文本域组件
	...
	createTextAreaCom:function(config){
		var me = this;
		var com = "{xtype:'textarea'," + me.createBasicCom(config) + "}";
		return com;
	},
	/**
	 * 创建数字组件
