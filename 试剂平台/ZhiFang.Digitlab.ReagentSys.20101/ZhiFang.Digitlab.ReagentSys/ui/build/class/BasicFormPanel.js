...
Ext.define('Ext.build.BasicFormPanel',{
     ...
     * @type 
     */
    saveBackgroundPictureServerUrl:getRootPath()+'/PublicService.svc/Public_File_UpLoadByFrom',
    //=====================内部变量=======================
    /**
     * 表单默认的标题
     * @type String
    ...
    comTypeList:[
        ...
        ['comboboxbut','只读下拉框+按钮']//带按钮的只读下拉框
    ],
    /**
     * 面板样式
     * @type 
    ...
    createNorth:function(){
        ...
        var com = {
            ...
            items:[
                '<b>'+me.buildTitle+'</b>',
                '->',
                {xtype:'button',text:'快速布局',itemId:'quickLayout',iconCls:'build-button-config',margin:'0 4 0 0',
                    handler:function(){
                        me.quickLayout();
                    }
                },
    ...
    setComponentLabWidth:function(InteractionField,newValue,oldValue,record,x,y){
        ...
        if(com.xtype== 'checkboxgroup'||com.xtype== 'radiogroup'){
            ...
            var url=getRootPath() + "/" + record.get('ServerUrl').split("?")[0];
...
/*	                    id.on({
    ...
    createSouth1:function(){
        ...
        var com = {
            ...
            columns:[//列模式的集合
                ...
                {text:'交互字段',dataIndex:'InteractionField',width:150,editor:{readOnly:true},locked:true},
                ...
                {text:'必填',dataIndex:'NotNull',width:40,align:'center',
                    xtype:'checkcolumn',
                    editor:{
                        xtype:'checkbox',
                ...
                {text:'定值下拉框数据',dataIndex:'combodata',editor:{}},
                ...
                {text:'文本框绑定的元应用的ID值',dataIndex:'appComID',hidden:true,editor:{readOnly:true}},
                ...
                {text:'空值提示',dataIndex:'EmptyText',
                    editor:{
                        listeners:{
                            change:function(com,newValue){
                                var record = com.ownerCt.editingPlugin.context.record;
                                var InteractionField = record.get('InteractionField');
                                me.setColumnParamsRecord(InteractionField,'EmptyText',newValue);
                                
                                me.setBasicParamsForInteractionField(InteractionField,'EmptyText',newValue);
                            }
                        }
                    }
                },
                {text:'条件',dataIndex:'Where',
                    editor:{
                        listeners:{
                            change:function(com,newValue){
                                var record = com.ownerCt.editingPlugin.context.record;
                                var InteractionField = record.get('InteractionField');
                                me.setColumnParamsRecord(InteractionField,'Where',newValue);
                               	
                                me.setBasicParamsForInteractionField(InteractionField,'Where',newValue);
                            }
                        }
                    }
                }
    ...
    createOther:function(){
        ...
        var com = {
            ...
            items:[{
                ...
                xtype:'numberfield',itemId:'allLabelWidth',name:'allLabelWidth',value:60,
                ...
                mode:'local',editable:false,value:'right',
				displayField:'text',valueField:'value',
                store:new Ext.data.SimpleStore({ 
				    fields:['value','text'], 
                ...
                xtype:'numberfield',itemId:'allWidth',name:'allWidth',value:200,
    ...
    deployBgHtml:function(){
        ...
        //列表配置参数
        var params = me.getPanelParams();
        var src = params.formHtml && params.formHtml != "" ? getHtmlBackgroundPictureRootPath() + "/" + params.formHtml : "";
        var oldSrc = params.formHtml && params.formHtml != "" && params.formHtml.length < 100 ? params.formHtml : "";
        Ext.create('Ext.form.Panel',{
            ...
            oldSrc:oldSrc,
            items:[{
                xtype:'image',name:'image',itemId:'image',src:src
            }],
            dockedItems:[{//停靠
                xtype:'toolbar',dock:'bottom',itemId:'bottomtoolbar',
                items:[{
                	xtype:'filefield',buttonConfig:{iconCls:'search-img-16',text:''},
                	fieldLabel:'背景图片',itemId:'Picture',name:'Picture',labelWidth:60,labelAlign:'right',
                	listeners:{
                		change:function(field,value){
                			var form = field.ownerCt.ownerCt;
                			me.htmlImgChange(form);
                		}
                	}
                },'->',
                {xtype:'button',text:'确定',iconCls:'build-button-ok',itemId:'save',
                    handler:function(but){
                        var form = but.ownerCt.ownerCt;
                        me.setPanelParams({formHtml:form.oldSrc});//修改html参数
                        //修改背景
                        var src = form.oldSrc && form.oldSrc != "" ? getHtmlBackgroundPictureRootPath() + "/" + form.oldSrc : "";
                        if(src != ""){
                        	var center = me.getCenterCom();
    ...
    createForm:function(){
        ...
        var src = formHtml && formHtml != "" ? getHtmlBackgroundPictureRootPath() + "/" + formHtml : "";
        var html = "<img src='" + src + "'/>";
...
//            me.setColumnParamsRecord(com.itemId,'Height',com.height);
    ...
    createComponentsBasicInfo:function(){
        ...
        for(var i in southRecords){
            ...
            if(com.type == 'comboboxbut'){
            	com.width = com.width - 22;
            	com.readOnly = true;
            	var btnCom = {
            		xtype:'button',
            		iconCls:'build-button-configuration-blue',
            		readOnly:record.get('IsReadOnly'),
            		hidden:record.get('IsHidden'),//是否隐藏
            		itemId:com.itemId + '-button',
	                x:com.x + com.width + 1,
	                y:com.y,
	                sortNum:0//光标顺序
            	};
            ...
            //NotNull
            if(record.get('NotNull')) com.allowBlank = false;//必填
            if(record.get('EmptyText'))com.emptyText = record.get('EmptyText');//空值提示
            var defaultValue = record.get('defaultValue');
            if(defaultValue){
            	var b = defaultValue.match(/(\{SYS_.*?\})/g);
            	if(b){
            		defaultValue = Shell.util.SysInfo['get' + b[0].slice(1,-1)]();
            	}
            	com.value = defaultValue;//初始值
            }
        }
    ...
    createComponentsByType:function(type,record){
        ...
        }else if(type == 'comboboxbut'){
            com = me.createComboboxbut(record);//带按钮的只读下拉框
        }
        return com;
    },
    /***
     ...
     * @return {}
     */
    createComboboxbut:function(record){
        var me=this;
...
//                }         
    ...
    createParamsPanel:function(type,componentItemId,title){
        ...
        if(type == 'combobox'){//下拉框
            ...
            otherItems = me.createComboboxbutItems(componentItemId);
        }
     ...
     * @return {}
     */
    createComboboxbutItems:function(componentItemId){
        var me = this;
        //面板ID
        var panelItemId = componentItemId + me.ParamsPanelItemIdSuffix;
        ...
        var items = [{
            xtype:'fieldset',title:'带按钮的只读下拉框特有属性',padding:'0 5 0 5',collapsible:true,
            defaultType:'textfield',
            itemId:'otherParams',
            items:[{
                    ...
                    focus:function(owner,The,eOpts){
                    	//值字段赋值
                     	...
                     	if(newValue && newValue != ""){
                            ...
                            //值字段赋值
                            me.setSouthRecordByKeyValue(componentItemId,'textField',value);
                        }
                    },
				    	...
				    	appList.on({
				    		okClick:function(){
				    			...
				    			if(records.length == 0){
				    				...
				    				but.ownerCt.getComponent('openWinAppName').setValue(records[0].get(me.fieldsObj.CName));
				    				me.setSouthRecordByKeyValue(componentItemId,'appComID',records[0].get(me.fieldsObj.Id));
				    				me.setSouthRecordByKeyValue(componentItemId,'appComName',records[0].get(me.fieldsObj.CName));
				    				appList.close();//关闭应用列表窗口
				    			}
				    		},
				    		itemdblclick:function(view,record,tem,index,e,eOpts){
				    			but.ownerCt.getComponent('openWinAppName').setValue(record.get(me.fieldsObj.CName));
						    	me.setSouthRecordByKeyValue(componentItemId,'appComID',record.get(me.fieldsObj.Id));
						    	me.setSouthRecordByKeyValue(componentItemId,'appComName',record.get(me.fieldsObj.CName));
				    			appList.close();//关闭应用列表窗口
				    		}
				    	});
					}
    ...
    createBasicItems:function(componentItemId){
        ...
        var items = [{
            ...
            items:[{
                ...
                xtype:'checkbox',fieldLabel:'必填',name:'NotNull',labelWidth:60,anchor:'95%',
                itemId:'NotNull',
                listeners:{
                    change:function(com,newValue,oldValue,eOpts){
                        me.setColumnParamsRecord(componentItemId,'NotNull',newValue);
                    }
                }
            },{
                xtype:'textfield',fieldLabel:'初始值',name:'defaultValue',labelWidth:60,anchor:'95%',
                itemId:'defaultValue',
                listeners:{
                    blur:function(com,The,eOpts){
                        me.setColumnParamsRecord(componentItemId,'defaultValue',this.value);
                ...
                xtype:'textfield',fieldLabel:'背景提示',name:'EmptyText',labelWidth:60,anchor:'95%',
                itemId:'EmptyText',
                listeners:{
                    blur:function(com,The,eOpts){
                        me.setColumnParamsRecord(componentItemId,'EmptyText',this.value);
                    }
                }
            },{
                xtype:'textfield',fieldLabel:'条件',name:'Where',labelWidth:60,anchor:'95%',
                itemId:'Where',
                listeners:{
                    blur:function(com,The,eOpts){
                        me.setColumnParamsRecord(componentItemId,'Where',this.value);
                    }
                }
            }]
        }];
   ...
   getEastisFunctionBtn:function(componentItemId){
    ...
    createNumberfieldItems:function(componentItemId){
        ...
        var items = [{
            ...
            items:[{
                ...
                itemId:'NumberMin',value:-Infinity,
                listeners:{
                    blur:function(com,The,eOpts){
                ...
                itemId:'NumberMax',value:Infinity,
                listeners:{
                    blur:function(com,The,eOpts){
                ...
                itemId:'NumberIncremental',value:1,
                listeners:{
                    blur:function(com,The,eOpts){
    ...
    GetRadiogroupItems:function(myUrl,groupName,valueField,displayField,defaultValue2){
        ...
        var localData=[];
        if(myUrl==""||myUrl==null){
            alertError('没有配置数据服务地址或者配置失败！');
            return null;
        }else{
        var mychecked=false;var arrStr=[];
        arrStr=defaultValue2;
        myUrl=myUrl+ "?isPlanish=true&fields="+valueField.trim()+","+displayField.trim();
        Ext.Ajax.request({
            async:false,//非异步
            url:myUrl,
            method:'GET',
    ...
    GetComboboxItems:function(url,value2,text2){
        ...
        var myUrl=url+ "?isPlanish=true&fields="+value2.trim()+","+text2.trim();
        Ext.Ajax.request({
            async:false,//非异步
            url:myUrl,
            method:'GET',
    ...
    setBasicParamsPanelValues:function(componentItemId,record){
        ...
        //填写数据
        basic.getComponent("NotNull").setValue(record.get('NotNull'));
        basic.getComponent("defaultValue").setValue(record.get('defaultValue'));
        basic.getComponent("EmptyText").setValue(record.get('EmptyText'));
        basic.getComponent("Where").setValue(record.get('Where'));
    },
    /**
     * 属性面板基础数据X,Y赋值
     * @private
...
//            },
    ...
    getSouthStoreFields:function(){
        ...
        var fields = [
            ...
            {name:'NumberMin',type:'int',useNull:true},//数字最小值
            {name:'NumberMax',type:'int',useNull:true},//数字最大值
            ...
            {name:'NumberIncremental',type:'int',useNull:true},//数字增量
            ...
            {name:'NotNull',type:'bool'},//必填
            {name:'EmptyText',type:'string'},//空值提示
            {name:'Where',type:'string'}//条件
        ];
...
//                    "}" +
   ...
   /**
    ...
    insertButtonParamsPanel:function(obj){
        ...
        for(var i in fields){
    		if(fields[i].xtype == 'fieldcontainer'){
    			for(var j in fields[i].items.items){
    				fields[i].items.items[j].on({
    					change:function(com,value){
    						if(com.xtype == 'radiogroup'){
    							me.setSouth2RecordByKeyValue(obj.buttonItemId,com.name,value.openWinType);
    						}else{
	 ...
	 * @param {} form
	 */
	htmlImgChange:function(form){
		var me = this;
		var bottomtoolbar = form.getComponent('bottomtoolbar');
		var save = bottomtoolbar.getComponent('save');
		save.disable();//禁用
		var callback = function(filepath){
			form.oldSrc = filepath;
			var image = form.getComponent('image');
			var src = getHtmlBackgroundPictureRootPath() + "/" + filepath;
			image.setSrc(src);
			save.enable();//启用
		};
		//提交背景文件
		var url = me.saveBackgroundPictureServerUrl;
		form.getForm().submit({
			url:url,
			params: {
		        relativePath:getHtmlBackgroundPictureRootPath(true).replace(/\//g,"\\"),
		        fileType:null,
		        isGUIDFileName:'true'
		    },
	 ...
	 * @private
	 */
	changeComponentInfo:function(){
		var me = this,
			params = me.getPanelParams(),
        	openLayoutType = params.openLayoutType;//是否开启排列方式
