 ...
 * @version 2015-07-02
 */
Ext.define('Shell.class.pki.StepPriceGrid',{
    extend:'Shell.ux.grid.Panel',
    title:'经销商阶梯价格列表',
    
    /**获取数据服务路径*/
    selectUrl:'/BaseService.svc/ST_UDTO_SearchDStepPriceByHQL?isPlanish=true',
    /**修改服务地址*/
    editUrl:'/BaseService.svc/ST_UDTO_UpdateDStepPriceByField',
    /**删除数据服务路径*/
	delUrl:'/BaseService.svc/ST_UDTO_DelDStepPrice',
    /**默认加载*/
	defaultLoad:false,
	/**后台排序*/
	remoteSort:true,
	/**排序字段*/
	defaultOrderBy:[
		{property:'DStepPrice_BeginDate',direction:'DESC'},
		{property:'DStepPrice_SampleCount',direction:'DESC'}
	],
	/**带分页栏*/
	...
	initComponent:function(){
		...
		//数据列
		me.columns = [{
			dataIndex:'DStepPrice_BeginDate',text:'开始日期',width:80,isDate:true,sortable:false
		},{
			dataIndex:'DStepPrice_EndDate',text:'截止日期',width:80,isDate:true,sortable:false
		},{
			dataIndex:'DStepPrice_SampleCount',text:'<b style="color:blue;">数量要求</b>',width:100,
			editor:{xtype:'numberfield'},sortable:false
		},{
			dataIndex:'DStepPrice_StepPrice',text:'<b style="color:blue;">价格</b>',width:100,
			editor:{xtype:'numberfield'},sortable:false
		},{
			dataIndex:'DStepPrice_ContractNo',text:'合同号',width:100,defaultRenderer:true,sortable:false
		},{
	...
	openStepPriceForm:function(id){
		...
		JShell.Win.open('Shell.class.pki.StepPriceForm',config).show();
	},
	/**点击复制按钮处理*/
	onCopyClick:function(){
		...
		JShell.Win.open('Shell.class.pki.StepPriceFormCopy',{
			ContractNo:records[0].get('DStepPrice_ContractNo'),
			listeners:{save:function(win,data){me.saveCopyIfo(win,data)}}
		}).show();
	},
	/**保存拷贝信息*/
	saveCopyIfo:function(win,data){
		...
		for(var i=0;i<len;i++){
			...
			me.saveOneByEntity(win,rec.get(me.PKField),{
				...
				StepPrice:rec.get('DStepPrice_StepPrice'),
				SampleCount:rec.get('DStepPrice_SampleCount'),
	...
	onSaveClick:function(){
		...
		for(var i=0;i<len;i++){
			...
			var count = rec.get('DStepPrice_SampleCount');
			var price = rec.get('DStepPrice_StepPrice');
	...
	updateOneBySampleCountAndPrice:function(id,count,price){
		...
		var params = Ext.JSON.encode({
			entity:{Id:id,SampleCount:count,StepPrice:price},
			fields:'Id,SampleCount,StepPrice'
		});
		JShell.Server.post(url,params,function(data){
			var record = me.store.findRecord(me.PKField,id);
			if(data.success){
	...
	loadByDealerIdAndItemId:function(dealerId,itemId){
		...
		me.ItemId = itemId;
		me.defaultWhere = 'dstepprice.BDealer.Id=' + dealerId + 
			' and dstepprice.BTestItem.Id=' + itemId;
		me.onSearch();
	}
});