$(function() {
	...
	//机构类型
	var CENORGTYPE = Shell.util.getRequestParams(true).CENORGTYPE;
	//当前选中的类型
	...
	function createRow(value){
		...
		html.push('<div class="list-group-item-text">订货单号：' + (value.BmsCenOrderDoc_OrderDocNo || '') + '</div>');
		
		if(value.BmsCenOrderDoc_Status == '0'){
			html.push('<div class="list-group-item-text">新增时间：' + 
				(Shell.util.Date.toString(value.BmsCenOrderDoc_DataAddTime) || '') + '</div>');
		}else{
			html.push('<div class="list-group-item-text">提交时间：' + 
				(Shell.util.Date.toString(value.BmsCenOrderDoc_OperDate) || '') + '</div>');
		}
		...
		if(value.BmsCenOrderDoc_Status == "0"){//临时
			...
			html.push('<div data="back" class="block60" style="float:right;margin-top:-65px;">撤回</div>');
		}else if(value.BmsCenOrderDoc_Status == "2"){//已确认
			
		}else if(value.BmsCenOrderDoc_Status == "3"){//已出货
			html.push('<div data="accept" class="block60" style="float:right;margin-top:-65px;">验收</div>');
