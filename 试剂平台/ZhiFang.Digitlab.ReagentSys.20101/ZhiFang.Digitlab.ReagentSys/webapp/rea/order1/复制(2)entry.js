...
//			}
	...
	$("#checked-goods").on(Shell.util.Event.click,function (e) {
		...
		$("#checked-goods-list-div").html(html);
		initCheckedListener();
		$("#checked-goods-div").show();
	});
	$("#mask-div").on(Shell.util.Event.click,function (e) {
		e.preventDefault();
		if(!Shell.util.Event.isClick()) return;
		$("#checked-goods-div").hide();
	});
	$("#checked-goods-list-div").on("touchmove",function (e) {
		e.preventDefault();
		var y = Shell.util.Event.touch.move_y();
		if(y > 50){
			$("#checked-goods-list-div").css("WebkitTransform","translateY(0px)");
		}else{
			$("#checked-goods-list-div").css("WebkitTransform","translateY(" + y + "px)");
		}
	});
	$("#checked-goods-list-div").on("touchend",function (e) {
		e.preventDefault();
	});
	$("#submit div").on(Shell.util.Event.click,function (e) {
		e.preventDefault();
		if(!Shell.util.Event.isClick()) return;
...
//		if(CHECKED_TYPE_DOM){CHECKED_TYPE_DOM.removeClass("active");}
...
//		});
		...
		initOrderData(function(){
			...
			refreshTypeInfo();
			//默认选中类型,刷新数据
			$("#type-ul li[index='" + (TYPE ? TYPE : "1") + "']").click();
		});
	},500);
});