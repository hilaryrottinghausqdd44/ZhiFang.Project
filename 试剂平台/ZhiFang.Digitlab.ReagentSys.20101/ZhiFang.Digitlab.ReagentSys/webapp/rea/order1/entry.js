$(function() {
	...
	function addDoc(type){
		...
		addDocData(Shell.util.JSON.encode({entity:entity}),function(){
	...
	//刷新待选产品列表数据
	function refreshUncheckedGoodsList(){
		var type = CHECKED_TYPE_DOM.attr('data'),
			list = filter(type),
			html = createGoodsList(list);
		$("#unchecked-goods-div").html(html);
		initListeners();
	}
	//初始化产品所有过滤条件
	function initAllGoodsType(){
		initOneGoodsType("ProdList","1");//品牌
		initOneGoodsType("GoodsClassList","2");//一级
		initOneGoodsType("GoodsClassTypeList","3");//二级
		
		$("#unchecked-params-div div").on(Shell.util.Event.click,function () {
			var data = $(this).attr("data");
			
			refreshChildrenParams(data);
		});
	}
	//创建产品类型内容
	function createOneGoodsType(name){
		var list = LOCAL_DATA[name],
			len = list.length,
			all = '<div class="div-button-2" data="all">全部</div>',
			other = '<div class="div-button-2" data="other">其他</div>';
			
		var html = [];
		html.push(all);
		for(var i=0;i<len;i++){
			html.push('<div class="div-button-2" data="' + list[i] + 
				'">' + list[i] + '</div>');
		}
		html.push(other);
	...
	function refreshChildrenParams(type){
		var dom = $("#unchecked-params-div");
		...
		var obj = {
			"1":"ProdList",
			"2":"GoodsClassList",
			"3":"GoodsClassTypeList"
		};
		
		var html = createOneGoodsType(obj[type]);
	...
	//初始化已选产品列表
	function initCheckedGoodsList(){
		var list = getCheckedGoods(LOCAL_DATA.GoodsList || []),
			len = list.length,
			html = [];
			
		if(len == 0){
			html.push('<div class="no-data-div">没有选中的产品!</div>');//没有数据
			return;
		}
		...
		for(var i=0;i<len;i++){
			...
			var row0 = '<span style="color:#d9534f;font-weight:bold;">规格：( ' + data.Goods_UnitMemo + ' )';
			var row1 = '<span style="color:#666666;">品牌：' + data.Goods_Prod_CName + '</span>';
			var row2 = data.Goods_GoodsClass + ' - ' + data.Goods_GoodsClassType;
			var row3 = "单价：" + data.Goods_Price + "元";
			
			html.push('<div class="list-group-item" style="padding:0;" data="' + data.Goods_Id + '">');
			
			html.push('<div class="list-group-item-content">');
			html.push('<h4 class="list-group-item-heading">' + data.Goods_CName + '</h4>');
			html.push('<div class="list-group-item-text">' + row0 + '</div>');
			html.push('<div class="list-group-item-text touch">' + row1 + '</div>');
			html.push('<div class="list-group-item-text">' + row2 + '</div>');
			html.push('<div class="list-group-item-text">' + row3 + '</div>');
			...
			html.push('总价：<span style="color:#d9534f;font-weight:bold;">' + 
				parseFloat(data.TotalPrice || data.Goods_Price).toFixed(2) + '</span>元');
			html.push('</div>');
	...
	//刷新待选产品列表
	function refreshUnckeckedGoodsList(){
	...
	function initCheckedListener(){
		...
		//数量加减监听
		$(".number-spinner button").on(Shell.util.Event.click,function(){
	...
	//选择产品
	$("#check-goods").on(Shell.util.Event.click,function () {
		$("#unchecked-div").show();
	});
	$("#unchecked-mask").on(Shell.util.Event.click,function () {
		$("#unchecked-div").hide();
	});
	$("#mask-window-params-div div").on(Shell.util.Event.click,function () {
		var data = $(this).attr("data");
		if(CHECKED_TYPE_DOM){CHECKED_TYPE_DOM.removeClass("div-button-active");}
		CHECKED_TYPE_DOM = $(this);
		CHECKED_TYPE_DOM.addClass("div-button-active");
		TYPE = CHECKED_TYPE_DOM.attr("data");
		//刷新子选项内容
		refreshChildrenParams(data);
		//刷新待选产品列表数据
		//refreshUncheckedGoodsList();
	});
