    ...
    public class ImageHelp
    {
        ...
        public static void ZoomAuto(System.IO.Stream fromFile, string savePath, System.Double targetWidth, System.Double targetHeight, string watermarkText, string watermarkImage)
        {
            ...
            if (initImage.Width <= targetWidth && initImage.Height <= targetHeight)
            {
                ...
                //保存
                EncoderParameters ep = new EncoderParameters(1);
                ep.Param[0] = new EncoderParameter(System.Drawing.Imaging.Encoder.Quality, 1000);
                ImageCodecInfo[] icis = ImageCodecInfo.GetImageEncoders();
                ImageCodecInfo ici = null;
                foreach (ImageCodecInfo i in icis)
                {
                ...
                initImage.Save("@"+savePath, ici, ep);
            }
            else
            {
                //缩略图宽、高计算
        ...
        /// <param name="strNewPic">缩小后保存为文件名(包括路径)</param>
        /// <param name="intWidth">缩小至宽度</param>
        public void SmallPicWidth(string strOldPic, string strNewPic, int intWidth)
        {
        ...
        /// <param name="strNewPic">缩小后保存为文件名(包括路径)</param>
        /// <param name="intHeight">缩小至高度</param>
        public void SmallPicHeight(string strOldPic, string strNewPic, int intHeight)
        {
        ...
        public static bool IconCheck(Stream imagestream,int width,int height)
        {
            try
            {
                System.Drawing.Image tmpimage = System.Drawing.Image.FromStream(imagestream, true);
