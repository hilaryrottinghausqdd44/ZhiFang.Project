...
Ext.define('Ext.zhifangux.CipherForm', {
    ...
    //登录验证
    verificationUrl:getRootPath() + '/RBACService.svc/RBAC_BA_Login',
    //获取用户帐号
    Url:getRootPath()+'/RBACService.svc/RBAC_UDTO_SearchRBACUserById?isPlanish=true&fields=RBACUser_Account,RBACUser_EnMPwd,RBACUser_AccBeginTime,RBACUser_AccEndTime,RBACUser_Id&id=',
    //获取用户员工信息
    getAccountUrl:getRootPath() + '/' + 'RBACService.svc/RBAC_UDTO_GetHREmployeeBySessionHREmpID',
    //验证输入旧密码服务
    CheckUserPWDUrl:getRootPath()+'/'+'RBACService.svc/AutoCreateUserAccount',
     ...
     * @type String
     */
    files:'RBACUser_Account,RBACUser_PWD,RBACUser_EnMPwd,RBACUser_AccLock,RBACUser_Id,RBACUser_DataTimeStamp,RBACUser_HREmployee_Id,RBACUser_HREmployee_DataTimeStamp',
    bodyCls:'',
    layout:'absolute',
    ...
    initComponent:function() {
        ...
        me.load = function(id) {
            Ext.Ajax.request({
                async:false,
                url:getRootPath() + '/RBACService.svc/RBAC_UDTO_SearchRBACUserById?isPlanish=true&fields=RBACUser_Account,RBACUser_PWD,RBACUser_EnMPwd,RBACUser_PwdExprd&id=' + (id ? id :-1),
                method:'GET',
                timeout:5000,
                success:function(response, opts) {
                    var result = Ext.JSON.decode(response.responseText);
        ...
        me.items = [ {
            ...
            listeners:{
                ...
                {    
	                 ...
	                 //新密码验证
	                 var txtnewPWD = me.getComponent('RBACUser_PWD');
	                 //帐户
	                 var txtAccount=me.getComponent('RBACUser_Account');
	                 accountValue=txtAccount.getValue();
	                 //确定按钮OK
	                 ...
	                 //验证输入的旧密码正确才允许修改新的密码
	                 var updatePWDUrl=me.verificationUrl+'?strUserAccount='+accountValue+'&strPassWord='+odlpwdvalue+'&isValidate=true';
	                 var callback=function(text){ 
	                     var value = Ext.JSON.decode(text);                                     
		                 if(value)
		                 {                    
		                    setnewPWD.setReadOnly(false); 
		                    txtnewPWD.setReadOnly(false);                    
		                    butOK.setVisible(true);
		                 }else{
		                    setnewPWD.setReadOnly(true); 
		                    txtnewPWD.setReadOnly(true); 
		                    butOK.setVisible(false);                    
		                    //Ext.Msg.alert('提示', '您输入的密码不正确！');
		                 } 
	                 };
	                getToServer(updatePWDUrl,callback,false); 
                }
            },
            hasReadOnly:false,
            labelAlign:'right'
            ...
            listeners:{
                ...
                blur:function(com,The,eOpts){
                    //验证新设置长度是否在4-12位
                    var txtValue=com.getValue();
                    //alert('新密码长度为：'+txtValue.length);
                    if(txtValue.length<4&&txtValue.length>12){
                        com.focus(false, 100); 
        ...
        },
        {
            ...
            listeners:{
                click:function(but,e,eOpts){
                    ...
                    var newPwdValue=newpassword.getValue();
                    //旧密码
                    var txtOldpassword=me.getComponent('RBACUser_EnMPwd'); 
                    
                    var olduserid=me.userInfo['RBACUser_Id'];
                    var userDataTimeStamp=me.userInfo['RBACUser_DataTimeStamp'];
                    var objArr = [];
                    if (userDataTimeStamp && userDataTimeStamp != undefined) {
                        objArr = userDataTimeStamp.split(",");
                    }
                    var objdata={RBACUser_Id:olduserid,RBACUser_PWD:newPwdValue};
                    var objstr={Id:olduserid,  //SampleDeliveryID
                    ...
                    //var editerJSON=Ext.JSON.decode(objdata); 
	                 ...
	                 var txtnewPWD = me.getComponent('RBACUser_PWD');
                     //确定按钮OK
                     var butSave=me.getComponent('OK');
	                 if(newPwdValue==txtnewPWD.getValue()){
	                    //me.updateUserPassword(objdata); 
	                    //me.updateUserPasswordNew();
                        if(newPwdValue.length>3&&newPwdValue.length<13)
                        {
                            var objUpdate={entity:objstr,
	                        fields:'Id,PWD'};
	                        var params = Ext.JSON.encode(objUpdate);
	                        var defaultPostHeader = "application/json";
	                        var async=false;
	                        var myupdateUrl=getRootPath()+'/'+me.editDataServerUrl;
	                        ...
	                        postToServer(myupdateUrl, params, callbackUpate, defaultPostHeader,async);                            
                        }else{
                            alertInfo('新密码字符长度不符合,请输入4-12位新密码！');
                        }
    ...
    afterRender:function() {
        ...
        var userId=Ext.util.Cookies.get('000300');//员工账户ID
        me.accountId=userId!=""?userId:-1;
        //me.accountId=userId!=""?userId:'5746770623843575925';
        //me.accountId='5746770623843575925';
        /*if(employeeID!=""){
            me.accountId=employeeID;
        }*/
        if (me.type == 'add') {
            me.isAdd();
        } else if (me.type == 'edit') {
    ...
    getAccountInfo:function(){
            ...
            Ext.Ajax.request({
                async:false,//非异步
                //url:me.getAccountUrl,//+'?fields='+me.files+'&isPlanish=true&where=rbacuser.HREmployee.Id='+me.accountId,
                url:getRootPath()+'/RBACService.svc/RBAC_UDTO_SearchRBACUserById?isPlanish=true&fields=RBACUser_Account,RBACUser_EnMPwd,RBACUser_AccBeginTime,RBACUser_AccEndTime,RBACUser_PWD,RBACUser_DataTimeStamp,RBACUser_Id&id='+(me.accountId?me.accountId:-1),
                method:'GET',
                timeout:2000,
                success:function(response,opts){
                    var result = Ext.JSON.decode(response.responseText);
                    if(result.success){
                        if(result.ResultDataValue && result.ResultDataValue != ""){
                            ...
                            var addData = {
                                //RBACUser_Account:ResultDataValue.List[0]['Account'],
                                //RBACUser_PWD:ResultDataValue.List[0]['PWD'],  //RBACUserList
                                //RBACUser_EnMPwd:ResultDataValue.List[0]['EnMPwd'],
                                //RBACUser_AccBeginTime:ResultDataValue.List[0]['AccBeginTime'],
                                //RBACUser_AccEndTime:ResultDataValue.List[0]['AccEndTime'],
                                //RBACUser_Id:ResultDataValue.List[0]['Id'],
                                //RBACUser_DataTimeStamp:ResultDataValue.List[0]['DataTimeStamp']
                                
                                //===============
                                RBACUser_Account:ResultDataValue['RBACUser_Account'],
	                            RBACUser_PWD:ResultDataValue['RBACUser_PWD'],
	                            RBACUser_EnMPwd:ResultDataValue['RBACUser_EnMPwd'],
	                            RBACUser_AccBeginTime:ResultDataValue['RBACUser_AccBeginTime'],
	                            RBACUser_AccEndTime:ResultDataValue['RBACUser_AccEndTime'],
	                            RBACUser_DataTimeStamp:ResultDataValue['RBACUser_DataTimeStamp'],
	                            RBACUser_Id:ResultDataValue['RBACUser_Id']                                
                        };
                        //取表单的itemId
    ...
    //=====修改用户密码方法
    updateUserPasswordNew:function(objstr){
