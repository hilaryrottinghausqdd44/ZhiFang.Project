...
Ext.define('Shell.class.rea.order.lab.audit.DocGrid', {
	...
	createGridColumns:function(){
		...
		columns.splice(0,0,{
			dataIndex: 'BmsCenOrderDoc_IsThirdFlag',
			text: '上传结果',
			align:'center',
			width: 60,
			renderer: function(value, meta) {
				...
				if(color){meta.style = ';color:' + color;}
	...
	onCheckUpdate:function(records){
		...
		me.CheckUpdateErrors = [];//清空审核错误
		
		for(var i=0;i<len;i++){
			var rec = records[i];
			//var id = rec.get(me.PKField);
			me.onCheckBmsCenOrderDocById(rec);
		}
	},
	/**审核一个订货单*/
	onCheckBmsCenOrderDocById:function(rec){
		var me = this,
		...
		JShell.Server.post(url,Ext.JSON.encode({id:id}),function(data){
			...
			if(data.success){
				...
				me.CheckUpdateErrors.push('供货单号：' + rec.get('BmsCenOrderDoc_OrderDocNo') + ' 错误信息：' + data.msg);//记录审核错误
				me.saveErrorCount++;
				if(record){
			...
			if(me.saveCount + me.saveErrorCount == me.saveLength){
				...
				if(me.saveErrorCount == 0){
					...
					JShell.Msg.error(me.CheckUpdateErrors.join("<br>"));
				}
			}
		});
	},
