 ...
 * @version 2017-07-17
 */
Ext.define('Shell.class.rea.sale.manage.DocGrid', {
	...
	createGridColumns:function(){
		...
		var columns = [{
			dataIndex: 'BmsCenSaleDoc_DeleteFlag',
			text: '删除标志',
			align:'center',
			width: 60,
	...
	getInternalWhere:function(){
		...
		if(CompID){
			...
			if(value){
				where.push('bmscensaledoc.Comp.Id=' + value);
			}
		}
		if(LabID){
			var value = LabID.getValue();
			if(value){
				where.push('bmscensaledoc.Lab.Id=' + value);
			}
		}
		if(status){
		...
		if(DeleteFlag){
			...
			if(value == '0'){
				...
				where.push('bmscensaledoc.DeleteFlag=1');
			}
		}
		if(search){
