...
Ext.define('Ext.iqc.month.InfoList',{
	...
	serverUrl:{
		/**质控失控判断*/
		judgeLoseControlValue:getRootPath()+'/QCService.svc/QC_RJ_JudgeLoseControlValueNew',
		/**获取列表*/
		search:[
			getRootPath()+'/QCService.svc/QC_BA_GetMonthQCDValueByQCItemIDAndDate',//仪器-质控物-质控项目
			getRootPath()+'/QCService.svc/QC_BA_GetMonthQCDValueByItemIDAndDate'//仪器-检验项目-质控物
		],
		/**新增质控数据*/
	...
	/**仪器ID*/
	pid:'',
	/**服务类型,1:仪器-质控物-质控项目;2:仪器-检验项目-质控物*/
	urlType:1,
	/**数据列信息*/
	columnFields:[
		{dataIndex:'ReceiveTime',text:'日期',width:90},
		{dataIndex:'QCDataLotNo',text:'日序号',width:50},
		{dataIndex:'ReportValue',text:'质控结果',width:80},
		{dataIndex:'Target',text:'靶值',width:60},
		{dataIndex:'SD',text:'标准差',width:60},
		{dataIndex:'CVValue',text:'变异系数',width:80},
		{dataIndex:'IsControl',text:'失控',width:50},
		{dataIndex:'QCControlInfo',text:'失控规则',width:80},
		{dataIndex:'Operator',text:'检验人',width:80},
		{dataIndex:'ReceiveTime',text:'质控时间',width:80,type:'time'},
		{dataIndex:'Manu',text:'厂家',width:60},
	...
	createColumns:function(){
		...
		for(var i in list){
			...
			if(con.dataIndex == 'ReportValue'){//质控结果
				...
				if(con.type == 'time'){
					con.renderer = function(v,m){
						...
						var value  = Ext.util.Format.date(v,'H:i:s');
			...
			}else if(con.dataIndex == 'IsControl'){//是否在控
				con.renderer = function(v,m){
					m.style='background:#f0f0f0;';
					
					var value = me.Enum.getQCValueIsControlInfoByKey(v);
					
					value = value == null ? v : value.value;
		...
		columns = [{xtype:'rownumberer',text:'序号',width:35,align:'center'}].concat(columns);
	...
	createDockedItems:function(){
		...
		var dockedItems = [{
			...
			items:[{
				xtype:'iqcyearmonthcomponent',itemId:'yearmonth',dateRange:100,
				listeners:{
					afterrender:function(com){
						var date = com.getNowDate();
						if(date){
							var d = date.getDate();
							var num = d > 8 ? 0 : 1;
							var yearmonth = com.getComponent('yearmonth');
							var record = yearmonth.store.getAt(num);
							yearmonth.setValue(record);
							com.lastValidYearMonth = record;
						}
					}
				}
			}]
	...
	getUrl:function(start,end){
		...
		//QC_BA_GetMonthQCDValueByQCItemIDAndDate(long QCItemID, string QCStartDate, string QCEndDate, 
			//bool NotShowEmptyData, string fields, bool isPlanish);
		//QC_BA_GetMonthQCDValueByItemIDAndDate(long QCMatID,long ItemID, string QCStartDate, string QCEndDate, 
			//bool NotShowEmptyData, string fields, bool isPlanish);
		
		var params = [];
		
		if(me.urlType == 1){
			params.push('QCItemID=' + me.tid);
		}else if(me.urlType == 2){
			params.push('ItemID=' + me.pid);
			params.push('QCMatID=' + me.tid);
		}
		
		var sDate = getDateString(start);
		var eDate = getDateString(getNextDate(end));
	...
	initListeners:function(){
		...
		var toptoolbar = me.getComponent('toptoolbar'),
			date = toptoolbar.getComponent('yearmonth');
			...
			cellAvailable:function(editor,e){
				...
				if(e.value != value && e.field == 'ReportValue' && 
					...
					if(value && value != '' && !isNaN(value)){
						//变异系数处理(定值)
						if(QCItemValueType == me.Enum.QCValueType[0].value){
							me.changeCVValue(record,value);
						}
						//质控失控判定
						bo = me.getJudgeLoseControlValue(record,value);
					}
				}else{
					bo = true;
				}
	...
	clickAddButton:function(){
		...
		if(records.length != 1){
			...
			for(var i in data){
				...
				}else if(i == 'ReceiveTime'){//时间
					var systime = getSystemTime();
					config[i] = data[i].slice(0,10) + ' ' + getDateString(systime,true).slice(-8);
	...
	clickSaveButton:function(){
		...
		if(records.length == 0){
			...
			me.resultCount += me.updateQCValue.length;
			...
			//保存缓存中的数据
			for(var i in me.updateQCValue){
				me.updateOne(me.updateQCValue[i]);
			}
		}
	},
	/**
	...
	saveOne:function(record){
		...
		var info = "日期:[" + record.get('ReceiveTime').slice(0,10) + "]";
		if(me.urlType == 2){
			info = "日序号:[" + record.get('QCDataLotNo') + "]";
		}
		...
		//交互对象
		var params = {
			fields:'QCDValue_Id,QCDValue_CVValue',
			isPlanish:true
		};
		
		var IsControl = me.Enum.getQCValueIsControlInfoByKey(record.get('IsControl'),'value');
		IsControl = IsControl == null ? record.get('IsControl') : IsControl.key;
		...
		if(bo){//修改
			...
			params.entity = {
				...
				IsControl:IsControl,//在控
				QCControlInfo:record.get('QCControlInfo')//失控规则
			};
			//需要更新的数据
			params.updateField = 'Id,QCDataLotNo,ReportValue,CVValue,IsControl,QCControlInfo';
		}else{//新增
			info += " 新增出错";
			
			var systime = getSystemTime();
			systime = getDateString(systime,true);
			
			var ReceiveTime = record.get('ReceiveTime').slice(0,10);
			ReceiveTime += " " + systime.slice(-8);
			...
			params.entity = {
				...
				ReportValue:record.get('ReportValue'),//质控结果
				CVValue:record.get('CVValue'),//变异系数
				ReceiveTime:convertJSONDateToJSDateObject(ReceiveTime),//质控时间
				IsControl:IsControl,//在控
				QCControlInfo:record.get('QCControlInfo'),//失控规则
//				Operator:{
...
//				},//员工
			...
			record.set('ReceiveTime',ReceiveTime);
		}
	...
	updateOne:function(obj){
		...
		var params = {
			...
			entity:{
				Id:obj.Id,//主键ID
				IsControl:obj.IsControl,//在控
				QCControlInfo:obj.QCControlInfo//失控规则
			}
		};
		//需要更新的数据
		params.updateField = 'Id,IsControl,QCControlInfo';
		//与后台交互
		postToServer(url,Ext.JSON.encode(params),function(text){
			me.showResult(text,info,successInfo);
		},null,false);
	...
	outOfControl:function(bo){
		...
		if(bo){
			var info = "<a style='color:red;'>" + emptyText + "【质控数据失控!】</a>";
			title += info;
		}
	 ...
	 * @param {} value
	 */
	getJudgeLoseControlValue:function(record,value){
		var me = this,
		...
		//QC_RJ_JudgeLoseControlValue(QCDValue entity, string fields, bool isPlanish)
		var dateStr = record.get('ReceiveTime').slice(0,10);
		var params = {
			entity:{
				QCDataLotNo:record.get('QCDataLotNo'),
				ReceiveTime:convertJSONDateToJSDateObject(dateStr),
				ReportValue:value,
				QCItem:{
					Id:record.get('QCItemID')
				}
			},
			backupValueFrom:me.urlType
		};
		...
		var callback = function(text){
	        ...
	        if(result.success){
	            if(result.ResultDataValue && result.ResultDataValue != ""){
		            ...
		            var IsControl = me.Enum.getQCValueIsControlInfoByKey(data.entity.IsControl);
		            //更改当前行质控结果
		            record.set({
			            IsControl:IsControl.value,
			            QCControlInfo:data.entity.QCControlInfo
		            });
		            //更改缓存的数据
		           	me.changeUpdateQCValue(data.list);
	            }
	        }
		};
	 ...
	 * @param {} list
	 */
	changeUpdateQCValue:function(list){
		var me = this;
			
		if(!list) return ;
		
		for(var i in list){
			if(list[i].QCControlInfo == '-1'){
				me.deleteUpdateQCValue(list[i].Id);
			}else{
				me.addUpdateQCValue(list[i]);
			}
		}
	},
	/**
	 ...
	 * @param {} obj
	 */
	addUpdateQCValue:function(obj){
		var me = this,
			updateQCValue = me.updateQCValue,
			hasObj = false;
			
		for(var i in updateQCValue){
			if(updateQCValue[i].Id == obj.Id){
				updateQCValue[i] = obj;
				hasObj = true;
				break;
			}
	 ...
	 * @param {} id
	 */
	deleteUpdateQCValue:function(id){
		var me = this,
			updateQCValue = me.updateQCValue;
			
		for(var i in updateQCValue){
			if(updateQCValue[i].Id == id){
				delete updateQCValue[i];
				break;
			}
		}
	},
	...
	getDate:function(){
		var me = this,
			date = me.getComponent('toptoolbar').getComponent('yearmonth').getValue();
		return date;
	},
	/**
	 * 加载数据
	...
	load:function(obj,type,isPrivate){
		...
		me.store.proxy.url = me.getUrl(dateStart,dateEnd);
		me.store.load();
	}
});