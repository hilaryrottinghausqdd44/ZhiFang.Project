	...
	createSouth:function(){
		...
		var com = {
			...
			columns:[//列模式的集合
				...
				{text:'显示名称',dataIndex:'DisplayName',editor:{allowBlank:true}},
...
//	            itemId:'hasColumnHead',name:'hasColumnHead'
	...
	browse:function(){
		...
		if(params.titleText == ""){
			Ext.Msg.alert("提示","【<b style='color:red'>列表标题不能为空！</b>】");
			return;
		}
	...
	save:function(bo){
		...
		if(params.titleText == ""){
			message += "【<b style='color:red'>列表标题不能为空！</b>】\n";
			isOk = false;
		}
...
//		me.DataUpdateTime = appInfo.BTDAppComponents_DataUpdateTime;
	...
	saveOperate:function(){
		...
		var url = getRootPath()+"/ConstructionService.svc/ST_UDTO_AddBTDAppComponentsOperate";
		var callback = function(text){
			me.disablePanel(false);//启用面板
			var result = Ext.JSON.decode(text);
			if(!result.success){
