...
Ext.define('Ext.build.AppListPanel',{
	 ...
	 */
	appTypeList:[
		{text:'列表',appType:1,className:'Ext.build.BasicListPanel',ParserUrl:'Ext.build.ParserList102'},
		{text:'表单',appType:2,className:'Ext.build.BasicFormPanel',ParserUrl:'Ext.build.ParserForm'},
		{text:'应用',appType:3,className:'Ext.build.BuildAppPanel',ParserUrl:'Ext.build.ParserApp'},
    ...
    getInfoByIdFormServer:function(id,callback){
   		var me = this;
   		var url = me.getAppInfoServerUrl + '?isPlanish=true&id=' + id + "&fields=" + me.fieldsObj.ClassCode;
   		Ext.Ajax.defaultPostHeader = 'application/json';
   		Ext.Ajax.request({
			async:false,//非异步
			url:url,
...
//  	win.show(null,callback);
    ...
    openAppEditWin:function(appType,id){
    	...
    	var ParserUrl = appTypeInfo.ParserUrl;
...
//			{name:me.fieldsObj.LabID,type:'string'},//实验室ID
    ...
    getLoadUrl:function(){
		...
		return url + "&where=" + encodeString(w);
    },
    //=====================对外公开方法=======================
    /**
     * 刷新列表
