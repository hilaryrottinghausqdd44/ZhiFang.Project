/**
 * 高级(一般)查询构建工具 包含一般查询和高级查询 1.可以添加关系运算符, 2.公开属性或方法设置全与或者全或的关系 【可配参数】
 * 
 * 服务地址配置 objectUrl：获取数据对象列表的服务地址 objectPropertyUrl：获取数据对象内容的服务地址
 * objectGetDataServerUrl：获取获取服务列表的服务地址 objectSaveDataServerUrl：获取保存服务列表的服务地址
 * dictionaryListServerUrl：查询对象属性所属字典服务列表的服务地址 addServerUrl：保存新增信息的后台服务地址
 * editServerUrl：保存修改信息的后台服务地址 getAppParamsServerUrl：获取参数的后台服务地址
 * 
 * buildTitle：构建名称 appId：列表元应用的ID，默认是-1，即新增，大于0的为修改
 * 
 * 
 */
Ext.ns('Ext.build');
Ext.define('Ext.build.BasicSearchPanel', {
	 ...
	 * @type
	 */
	btnTypeList : [['select', '查询按钮'], ['reset', '重置按钮'], ['close', '关闭按钮']],
	// 数据对象配置private
	 ...
	 * @type
	 */
	objectFields : ['ClassName', 'CName', 'EName', 'SysDic', 'Description',
			'ShortCode'],
	/**
	 * 获取数据对象列表的服务地址
	 * 
	 ...
	 * @type
	 */
	objectPropertyFields : ['text', 'InteractionField', 'RightID', 'leaf',
			'icon', 'Tree', 'tid', 'checked', 'FieldClass'],
	/**
	 * 获取数据对象内容时后台接收的参数名称
	 * 
	 * @type String
	 ...
	 * @type
	 */
	objectPropertyUrl : getRootPath()
			+ '/ConstructionService.svc/CS_BA_GetEntityFrameTree',
	/**
	 * 返回数据对象内容的值属性 例如：
	 * 返回的json对象：{"ErrorInfo":"","success":true,"ResultDataFormatType":"JSON","ResultDataValue":"{count:1,list:[{a:1}]}"}
	 ...
	 * @type
	 */
	objectGetDataServerUrl : getRootPath()
			+ '/ConstructionService.svc/CS_BA_SearchReturnEntityServiceListByEntityName',
	/**
	 * 获取保存服务列表的服务地址
	 * 
	 * @type
	 */
	objectSaveDataServerUrl : getRootPath()
			+ '/ConstructionService.svc/CS_BA_SearchParaEntityServiceListByEntityName',
	 ...
	 * @type
	 */
	dictionaryListServerUrl : getRootPath()
			+ '/ConstructionService.svc/CS_BA_SearchReturnEntityDictionaryServiceListByEntityPropertynName',
	/**
	 ...
	 * @type
	 */
	addServerUrl : getRootPath()
			+ '/ConstructionService.svc/CS_UDTO_AddBTDAppComponents',
	/**
	 * 修改保存的后台服务地址
	 * 
	 * @type String
	 */
	editServerUrl : getRootPath()
			+ '/ConstructionService.svc/CS_UDTO_UpdateBTDAppComponents',
	/**
	 * 获取应用信息的后台服务地址
	 * 
	 * @type String
	...
	getAppInfoServerUrl : getRootPath()
			+ '/ConstructionService.svc/CS_UDTO_SearchBTDAppComponentsById',
	 ...
	 * @type
	 */
	comTypeList : [['combobox', '下拉框'], ['textfield', '文本框'],
			['textareafield', '文本域'], ['numberfield', '数字框'],
			['daterange', '日期区间新'], ['dateintervals', '日期区间旧'],

			['numbersintervals', '数字区间'], ['timeintervals', '时间区间'],
			['datefield', '日期框'], ['timefield', '时间框'],
			['datetimenew', '日期时间'],

			['checkboxgroup', '复选框'], ['radiogroup', '单选框'],
			['datacombobox', '定值下拉框'], ['datacheckboxgroup', '定值复选组'],
			['dataradiogroup', '定值单选组'], ['label', '纯文本'],
			['button', '按钮']],
	/**
	 * 面板样式
	 * 
	 * @type
	 */
	panelStyleList : [['', '默认'], ['red', '喜庆红'], ['blue', '金典蓝'],
			['pink', '温馨粉']],
	/**
	 * 属性面板itemId后缀
	 * 
	...
	createNorth : function() {
		...
		var com = {
			xtype : 'toolbar',
			border : false,
			items : ['<b>' + me.buildTitle + '</b>', '->',

			{
						xtype : 'button',
						text : '更新表单数据',
						itemId : 'loadFormValues',
	 ...
	 * 
	 * @param {}
	 *            InteractionField:交互字段,某一控件的itemId
	 * @param {}
	 *            newValue:修改的值
	 */
	...
	setComponentXY : function(InteractionField, x, y) {
		var me = this;
		var tempItem = me.getCenterCom().getComponent(InteractionField);
		if (tempItem != undefined && tempItem != 'undefined') {
	 ...
	 *            InteractionField:交互字段,某一控件的itemId
	 * @param {}
	 *            newValue:修改的值;record
	 */
	setchangeComponent : function(InteractionField, record, labelStyle,
			labelWidthNew, labelWidthOld, x, y) {
		var me = this;
		...
		// 重新生成新的控件
		var com = me.newfromItem(record, labelStyle, labelWidthNew,
				labelWidthOld, x, y);
		center.add(com);
		...
		if (type == 'checkboxgroup' || type == 'radiogroup') {
			...
			if (myUrl == '' || myUrl == null) {
				...
				var radioItem = me.getCenterCom()
						.getComponent(InteractionField);
				if (value == '' || value == null || value == undefined
					...
					if (defaultValue == '' || defaultValue == null
						...
						defaultValue = null;
						var data2 = me.GetRadiogroupItems(url, groudName,
	 ...
	 * @return {}
	 */
	newfromItem : function(record, labelStyle, labelWidthNew, labelWidthOld, x,
			y) {
		...
		if (com.xtype != 'daterange' && com.xtype != 'dateintervals'
				&& com.xtype != 'timeintervals'
				&& com.xtype != 'numbersintervals') {
			com.style = {
				borderColor : 'red',
				borderStyle : 'dashed'
			};
			...
			columns : [// 列模式的集合
			{
						...
						editor : {
							...
							listeners : {
								change : function(com, newValue) {
									...
									var InteractionField = record
											.get('InteractionField');
									record.set('DisplayName', newValue);
									record.commit();
									if (record.get('Type') == 'button') {
										me.setComponentFieldText(
												InteractionField, newValue);
									} else {
										me.setComponentFieldLabel(
												InteractionField, newValue);
									}
									me.setBasicParamsForInteractionField(
											InteractionField, 'name', newValue);
								}
							}
						}
					}, {
						...
						editor : {
							...
							listeners : {
								change : function(com, newValue, oldValue,
									...
									var InteractionField = record
											.get('InteractionField');
									me.setSouthRecordForNumberfield(
											InteractionField, 'sortNum',
											newValue);
									me.setBasicParamsForInteractionField(
											InteractionField, 'sortNum',
											newValue);
							...
							listeners : {
								change : function(com, newValue, oldValue,
									...
									var InteractionField = record
											.get('InteractionField');
									var y = record.get('Y');
									me.setSouthRecordByKeyValue(
											InteractionField, 'Y', y);
									me.setSouthRecordByKeyValue(
											InteractionField, 'X', newValue);
									me.setComponentXY(InteractionField,
											newValue, y);
						...
						editor : {
							...
							listeners : {
								change : function(com, newValue, oldValue,
									...
									var x = record.get('X');
									me.setSouthRecordByKeyValue(
											InteractionField, 'Y', newValue);
									me.setSouthRecordByKeyValue(
											InteractionField, 'X', x);
									me.setComponentXY(InteractionField, x,
											newValue);
								}
							}
						}
							...
							listeners : {
								change : function(com, newValue, oldValue,
									...
									var InteractionField = record
											.get('InteractionField');
									me
											.setSouthRecordForNumberfield(
													InteractionField, 'Width',
													newValue);
									// me.setSouthRecordByKeyValue(InteractionField,'Width',newValue);
									me.setComponentWidth(InteractionField,
											newValue);
								}
							}
						}
							...
							listeners : {
								change : function(com, newValue, oldValue,
									...
									var InteractionField = record
											.get('InteractionField');
									me.setSouthRecordForNumberfield(
											InteractionField, 'Height',
											newValue);
									// me.setSouthRecordByKeyValue(InteractionField,'Height',newValue);
									me.setComponentHeight(InteractionField,
											newValue);
								}
							}
						}
						...
						editor : {
							...
							listeners : {
								change : function(com, newValue, oldValue,
									...
									var y = record.get('Y');
									me.setSouthRecordForNumberfield(
											InteractionField, 'labelWidth',
											newValue);
									// me.setSouthRecordByKeyValue(InteractionField,'labelWidth',newValue);
									me.setchangeComponent(InteractionField,
											record, null, newValue, oldValue,
											x, y);
						...
						items : [{
							...
							handler : function(grid, rowIndex, colIndex) {
								...
								me.setSouthRecordByKeyValue(InteractionField,
										'LabFont', record.get('LabFont'));
							}
						}]
					}, {
						text : '是否有运算关系',
...

					{
						...
						text : '日期区间分隔符一',
						dataIndex : 'LabelSeparatorOne',
						hidden : true
					}, {
						text : '日期区间分隔符二',
						dataIndex : 'LabelSeparatorTwo',
						hidden : true
					}, {
						text : '显示名称二',
						dataIndex : 'fieldLabelTwo',
						hidden : true
					}, {
						text : '行列方式',
						dataIndex : 'RawOrCol',
					...
					},// raw:行;col:列
					{
						...
						text : '数字增量',
						dataIndex : 'NumberIncremental',
						hidden : true
					}, {
						text : '显示格式',
						dataIndex : 'ShowFomart',
						hidden : true
					}, {
						text : '日期控件允许手输',
						dataIndex : 'CanEdit',
	 ...
	 * 设置表单配置参数
	 * 
	 * @private obj={titleStyle:"fdsafd"}
	 * @return {}
	 */
	setFormValues : function(obj) {
		var me = this;
		var formParamsPanel = me.getComponent('east').getComponent('center'
				+ me.ParamsPanelItemIdSuffix);
			...
			myxtype = Ext.create('Ext.zhifangux.FontStyleSet', {
						...
						listeners : {
							onOKCilck : function(o) {
								...
								var record = me.getSouthRecordByKeyValue(
										me.columnParamsField.InteractionField,
										componentItemId);
								// 重新生成展示区域的某一个控件
								me.setchangeComponent(componentItemId, record,
										lastValue, 0, 0, 0, 0);
			...
			items : [{
				...
				listeners : {
					change : function(owner, newValue, oldValue, eOpts) {
						var index = owner.store.find(me.objectValueField,
								newValue);// 是否存在这条记录
				...
				listeners : {
					...
					beforeload : function(store) {
						if (this.nodeClassName != "") {
							store.proxy.url = me.objectPropertyUrl + "?"
									+ me.objectPropertyParam + "="
									+ this.nodeClassName;
						}
					},
					...
					proxy : {
						...
						extractResponseData : function(response) {
							...
							if (data.ResultDataValue
								...
								var objectPropertyTree = dataObject
										.getComponent('objectPropertyTree');
				...
				store : new Ext.data.Store({
							...
							proxy : {
								...
								reader : {
									type : 'json',
									root : me.objectServerRoot
								},
								extractResponseData : me.changeStoreData
							},
							listeners : {
								beforeload : function(store, operation, eOpts) {
									var formParamsPanel = me
											.getComponent('east')
											.getComponent('center'
													+ me.ParamsPanelItemIdSuffix);
									var dataObject = formParamsPanel
											.getComponent('dataObject');
									var objectName = dataObject
											.getComponent('objectName');
									store.proxy.url = me.objectGetDataServerUrl
											+ "?" + me.objectServerParam
	...
	createOther : function() {
		...
		var com = {
			...
			items : [{
						...
						xtype : 'numberfield',
						itemId : 'allLabelWidth',
						name : 'allLabelWidth',
						labelWidth : 130,
						emptyText : '默认',
						fieldLabel : '显示名称宽度【所有】',
						listeners : {
						...
						xtype : 'checkbox',
						itemId : 'hasSelectButton',
						name : 'hasSelectButton',
						boxLabel : '查询按钮===='// ,checked:true
						,
						listeners : {
							change : function(com, newValue, oldValue, eOpts) {
								...
								if (newValue == true) {// 页面已经打开并是勾选时才处理
									...
									if (count > 0) {
										...
										var itemCom = centerCom
												.getComponent(selectItemId);
						...
						xtype : 'checkbox',
						itemId : 'hasResetButton',
						name : 'hasResetButton',
						boxLabel : '重置按钮===='// ,checked:true
						,
						listeners : {
							change : function(com, newValue, oldValue, eOpts) {
								...
								if (newValue == true) {// 页面已经打开并是勾选时才处理
									...
									if (store <= 0) {
										...
										var store = me.getComponent('south').store;
										var record = store
												.findRecord('InteractionField',
														resetItemId);// 查询按钮是否已经存在
										if (record == null) {// 查询按钮不存在时才添加
											var obj = {
												itemId : resetItemId,
												btnName : '重置',
						...
						xtype : 'checkbox',
						itemId : 'hasColseButton',
						name : 'hasColseButton',
						boxLabel : '关闭按钮===='// ,checked:true
						,
						listeners : {
							change : function(com, newValue, oldValue, eOpts) {
	...
	/**
	 * 更改表单内组件的显示名称 true：开启显示名称； false：关闭显示名称；
	 * 
	 * @private
	 * @param {}
	 *            bo
	...
	changeFieldLabel : function(bo) {
		...
		if (bo) {
			for (var i in items) {
				var record = store.findRecord('InteractionField',
						items[i].itemId);
	...
	/**
	 * 更改表单内组件的边框 true：显示组件边框； false：隐藏组件边框；
	 * 
	 * @private
	 * @param {}
	 *            bo
	...
	save : function(bo) {
		...
		if (!isOk) {
			...
			me.appId = id;
			// 从后台获取应用信息
			var saveCallback = function(appInfo) {
				me.DataTimeStamp = appInfo[me.fieldsObj.DataTimeStamp];
			}
			me.getAppInfoFromServer(saveCallback);
		}
	},
	/**
	 * 另存按钮事件处理
	...
	objectChange : function(owner, newValue) {
		...
		objectPropertyTree.store.proxy.url = me.objectPropertyUrl + "?"
				+ me.objectPropertyParam + "=" + newValue;
		objectPropertyTree.store.load();

		// 获取获取数据服务列表
		var getDataServerUrl = dataObject.getComponent('getDataServerUrl');
		getDataServerUrl.store.proxy.url = me.objectGetDataServerUrl + "?"
				+ me.objectServerParam + "=List" + newValue;
	...
	objectPropertyOKClick : function() {
		var me = this;
		var dataObject = me.getComponent('east').getComponent('center'
				+ me.ParamsPanelItemIdSuffix).getComponent('dataObject');
					...
					if (record.get('leaf')) {
						var index = store
								.findExact(
										'InteractionField',
										record
												.get(me.columnParamsField.InteractionField));
						...
						if (index === -1) {// 新建不存在的对象
							var rec = ('Ext.data.Model', {
								...
								InteractionField : record
										.get(me.columnParamsField.InteractionField),
								LabelWidth : 55,// 显示名称宽度
								LabFont : '',// 显示名称字体内容
								Type : record.get(me.columnParamsField.Type)
										|| 'textfield',// 数据项类型
								X : 0,// 位置X
								Y : 0,// 位置X
		...
		store.each(function(record) {
					if (record && record.get('InteractionField') != null
							&& record.get('InteractionField') != "") {
						bo = false;
	...
	createForm : function() {
		...
		form.listeners = {
			...
			resize : function(com, width, height, oldWidth, oldHeight, eOpts) {
				var formParamsPanel = me.getComponent('east')
						.getComponent('center' + me.ParamsPanelItemIdSuffix);
				// 表单宽度和高度赋值
				formParamsPanel.getForm().setValues({
							Width : width,
							Height : height
						});
			}
	...
	createComponents : function() {
		...
		for (var i in coms) {
			...
			// 边框属性设置
			com.border = 0;
			if (com.xtype != 'dateintervals' && com.xtype != 'timeintervals'
					&& com.xtype != 'numbersintervals') {
				com.style = {
					borderColor : 'red',
					borderStyle : 'dashed'
			...
			if (com.type == 'image' || com.type == 'htmleditor'
					|| com.type == 'textareafield' || com.type == 'filefield') {
				// 图片、超文本、文本域
		...
		for (var i = 0; i < comArr.length; i++) {
			...
			for (var i = 0; i < coms.length; i++) {
				...
				// 更新属性列表中的光标顺序数据
				me.setSouthRecordByKeyValue(coms[i].itemId, "sortNum",
						coms[i].sortNum);
			}
			coms.push(com);
		}
	...
	createTimeintervals : function(record) {
		...
		if (rawOrCol == '') {
			me.setSouthRecordByKeyValue(record.get('InteractionField'),
					'RawOrCol', 'hbox');
			rawOrCol = 'hbox';
		}
		...
		if (showFomart == '' || showFomart == "" || showFomart == null) {
			showFomart = 'h:i';
			me.setColumnParamsRecord(record.get('InteractionField'),
					'ShowFomart', showFomart);
		}
	...
	createDataCheckboxfield : function(record) {
		...
		var com = {
			...
			columns : columns,
			vertical : false,
			items : record.get('combodata')
					? eval(record.get('combodata'))
					: []
		};
		return com;
	},
	...
	createDataCombox : function(record) {
		...
		if (height == '' || height == 0 || height == null) {
			height = 22;
			me.setColumnParamsRecord(record.get('InteractionField'), 'Height',
					height);
		}
		var com = {
			xtype : 'combobox',
			...
			store : new Ext.data.SimpleStore({
						fields : ['value', 'text'],
						data : record.get('combodata') ? eval(record
								.get('combodata')) : []
					})
		};
		return com;
	...
	createComboboxfield : function(record) {
		...
		var editable=true;
		var multiSelectstr=false;
		if(multiSelect=='true'){
			editable=false;
			multiSelectstr=true;
		}else{
			multiSelectstr=false;
		}
		var com = {
			xtype : 'combobox',
			// 自定义属性,每个个控件都有
		...
		/*
		 * var listeners = {}; listeners.beforequery = function(e){ var combo =
		 * e.combo; if(!e.forceAll){ var value = e.query;
		 * combo.store.filterBy(function(record,id){ var text =
		 * record.get(combo.displayField); return (text.indexOf(value) != -1);
		 * }); combo.expand(); return false; } }; com.listeners=listeners;
		 */
		return com;
	},
	...
	createDatetimefield : function(record) {
		...
		if (width == '' || width == 0 || width == null || width < 205) {
			width = 210;
			me.setColumnParamsRecord(record.get('InteractionField'), 'Width',
					width);
		}
		var height = record.get('Height');
		if (height == 0 || height == '') {
	...
	createCheckboxfield : function(record) {
		...
		if (myUrl == '' || myUrl == null) {
			...
			if (value == '' || value == null || value == undefined
				...
				// 单/复选组没有默认值时的处理
				defaultValue = null;
				var data2 = me.GetRadiogroupItems(url, groudName, value, text,
						defaultValue);
				if (data2 && data2 != null) {
					com.items = data2;
				}
	...
	removeComponent : function(componentItemId) {
		...
		// 删除数据项属性面板
		me.getComponent('east').remove(componentItemId
				+ me.ParamsPanelItemIdSuffix);
		// 删除数据项属性列表中的当前数据项数据
	...
	removeOperationCom : function(componentItemId, operationItemId) {
		...
		// 删除数据项属性列表中的当前数据项数据
		me.setBasicParamsForInteractionField(componentItemId, 'isOperation',
				false);
		...
		var record = me.getSouthRecordByKeyValue('InteractionField',
				componentItemId);
		var labelWidth = record.get("LabelWidth");
		var bools = (labelWidth > 134);
		if (bools) {
			var newLabelWidth = labelWidth - 80;
			me.setSouthRecordByKeyValue(componentItemId, "LabelWidth",
					newLabelWidth);
			me.setchangeComponent(componentItemId, record, null, newLabelWidth,
					labelWidth, record.get("X"), record.get("Y"));
		}
	},
	// =====================组件属性面板的创建与删除=======================
	/**
	...
	switchParamsPanel : function(componentItemId) {
		...
		if (panel && componentItemId != me.OpenedParamsPanel) {
			var OpenedParamsPanel = east.getComponent(me.OpenedParamsPanel
					+ me.ParamsPanelItemIdSuffix);
	...
	changeParamsPanel : function() {
		...
		for (var i in southRecords) {
			...
			// 添加组件属性面板
			me
					.addParamsPanel(record.get('Type'), record
									.get('InteractionField'), record
									.get('DisplayName'));
		}
	},
	/**
	...
	createParamsPanel : function(type, componentItemId, title) {
		...
		if (type == 'combobox') {
			...
			otherItems = me.createtxtfieldItems(componentItemId);
		} else if (type == 'textareafield') {// 文本域
		...
		var com = {
			...
			listeners : {
				blur : function(com, The, eOpts) {
					...
					me.setColumnParamsRecord(componentItemId, 'DisplayName',
							this.value);
				}
			}
		};
	...
	createbaseziti : function(componentItemId) {
		...
		var com = {
			...
			items : [{
						...
						hidden : true,
						value : '',
						itemId : 'basicItemStyle',
						name : 'basicItemStyle'
					}, {
						xtype : 'image',
						itemId : 'basicConfiguration',
						imgCls : 'build-img-font-configuration hand',
						width : 16,
						height : 16,
						margin : '2 0 2 5',
		...
		var com = {
			...
			listeners : {
				change : function(com, newValue, oldValue, eOpts) {
					...
					var store = me.getComponent('south').store;
					var index = store
							.findExact('InteractionField', operationId);
					var record = me.getSouthRecordByKeyValue(
							'InteractionField', componentItemId);

					if (index === -1 && newValue == true) {
						me.setColumnParamsRecord(componentItemId,
								"isOperation", newValue);
						var labelWidth = record.get("LabelWidth");
						...
						if (bools) {
							...
							me.setchangeComponent(componentItemId, record,
									null, newLabelWidth, labelWidth, record
											.get("X"), record.get("Y"));
						}
						var x = record.get("X") + 51;
						var y = record.get("Y");
						store.add(me.operationCombobox(componentItemId, x, y));
						var newRecord = me.getSouthRecordByKeyValue(
								'InteractionField', operationId);
						// 添加组件属性面板
						me.addParamsPanel(newRecord.get('Type'), newRecord
										.get('InteractionField'), newRecord
										.get('DisplayName'));
						me.setAllComXY(newRecord);
	...
	createBasicItems : function(componentItemId) {
		...
		var center = me.getComponent('east').getComponent('center'
				+ me.ParamsPanelItemIdSuffix);
		var tree = center.getComponent('dataObject')
				.getComponent('objectPropertyTree');
		...
		var defaultValue = (data.length > 0)
				? data[0][me.objectPropertyValueField]
				: "";
		var defaultValueArr = defaultValue.split("_");
		var text = defaultValueArr[defaultValueArr.length - 2] + "_"
				+ defaultValueArr[defaultValueArr.length - 1];
		var value = defaultValueArr[defaultValueArr.length - 2] + "_"
				+ defaultValueArr[defaultValueArr.length - 1];
	...
	createDataRadiogroupItems : function(componentItemId) {
		...
		var items = [{
			...
			items : [{
				...
				listeners : {
					blur : function(com, The, eOpts) {
						...
						var record = me.getSouthRecordByKeyValue(
								me.columnParamsField.InteractionField,
								componentItemId);
						me.setchangeComponent(componentItemId, record, null, 0,
								0, 0, 0);
					}
				}
			}, {
				...
				listeners : {
					blur : function(com, The, eOpts) {
						me.setColumnParamsRecord(componentItemId, 'Columns',
								this.value);
						// 更新设置展示区域的单选框的列宽
						var record = me.getSouthRecordByKeyValue(
								me.columnParamsField.InteractionField,
				...
				anchor : '100%',
				height : 80,
				itemId : 'datacomboValue',
				name : 'datacomboValue',
				listeners : {
					blur : function(com, The, eOpts) {
						if (this.value != null && this.value != '') {
							var combodata = me.transformItems(this.value,
							...
							if (combodata && combodata.length > 0) {
								...
								if (this.value.length > 2 && this.value != "[]") {
									var com = me.createDataDefaultValue(
											componentItemId, this.value);
									var otherParams = me
											.getOtherParams(componentItemId);
									var oldCom = otherParams
	...
	setDataradiogroupParamsPanelValue : function(componentItemId, record) {
		...
		if (datacomboValue.getValue().length > 2
				&& datacomboValue.getValue() != "[]") {
			var com = me.createDataDefaultValue(componentItemId, datacomboValue
							.getValue());
				...
				change : function(combo, newValue, oldValue, eOpts) {
					...
					if (newValue && newValue != "") {
						...
						if (record.get('Type') == 'dataradiogroup') {
							var combodata = me.updateTransformItems(record
											.get('combodata'), componentItemId,
									newValue);
							me.setColumnParamsRecord(componentItemId,
									'combodata', combodata);
							var newRecord = me.getSouthRecordByKeyValue(
									'InteractionField', componentItemId);
							me.setchangeComponent(componentItemId, newRecord,
									null, 0, 0, 0, 0);
						}
					}
				}
		...
		var items = [{
			...
			items : [{
				...
				listeners : {
					blur : function(com, The, eOpts) {
						me.setColumnParamsRecord(componentItemId,
								'fieldLabelTwo', this.value);
						var record = me.getSouthRecordByKeyValue(
								me.columnParamsField.InteractionField,
								componentItemId);
						me.setchangeComponent(componentItemId, record, record
										.get('LabFont'), 0, 0, record.get('X'),
								record.get('Y'));
					}
				}
			}, {
				...
				listeners : {
					change : function(com, newValue, oldValue, eOpts) {
						me.setColumnParamsRecord(componentItemId, 'RawOrCol',
								newValue.RawOrCol);
						var record = me.getSouthRecordByKeyValue(
								me.columnParamsField.InteractionField,
								componentItemId);
	...
	createTimeintervalsItems : function(componentItemId) {
		...
		var items = [{
			...
			items : [{
				...
				listeners : {
					blur : function(com, The, eOpts) {
						me.setColumnParamsRecord(componentItemId, 'ShowFomart',
								this.value);
					}
				}
			}, {
				...
				listeners : {
					blur : function(com, The, eOpts) {
						me.setColumnParamsRecord(componentItemId, 'CanEdit',
								this.value);
					}
				}
			}, {
	 ...
	 * @return {}
	 */
	createtxtfieldItems : function(componentItemId) {
		var me = this;
		var items = [{
			xtype : 'fieldset',
			title : '文本框特有属性',
			...
			collapsible : true,
			defaultType : 'textfield',
			itemId : 'othertxtParams',
			items : [{
	        	xtype:'combobox',fieldLabel:'匹配模式',value:'1',
	            labelWidth:60,mode:'local',editable:false,
	            itemId:'txtType',name:'txtType',
				displayField:'text',valueField:'value',
				store:new Ext.data.SimpleStore({
				    fields:['value','text'], 
				    data:[['rightmatches','右匹配'],['lefttmatches','左匹配'],['fuzzyMatching','模糊匹配']]
				}),
				listeners : {
...
//					change:function(owner,newValue,oldValue,eOpts){
//						alert(newValue);
//						me.setColumnParamsRecord(componentItemId,owner,
//								newValue);
//					},
//					
//				}
		...
		var items = [{
			...
			items : [{
				...
				listeners : {
					blur : function(com, The, eOpts) {
						me.setColumnParamsRecord(componentItemId,
								'LabelSeparatorOne', this.value);
						// var
						// record=me.getSouthRecordByKeyValue(me.columnParamsField.InteractionField,componentItemId);
				...
				listeners : {
					blur : function(com, The, eOpts) {
						me.setColumnParamsRecord(componentItemId, 'LabelWidth',
								this.value);
						// 更新设置展示区域的单选框的列宽
						// var
						// record=me.getSouthRecordByKeyValue(me.columnParamsField.InteractionField,componentItemId);
			...
			items : [{
				...
				listeners : {
					select : function(combo, records, eOpts) {
						...
						me.setColumnParamsRecord(componentItemId, 'btnType',
								newValue);
					}
				}
			}]
	...
	createComboboxfieldItems : function(componentItemId) {
        ...
        //defaultValueArr[defaultValueArr.length - 2] + "_"+ defaultValueArr[defaultValueArr.length - 1];
		...
		var items = [{
			...
			items : [{
				...
				store : new Ext.data.Store({
					...
					proxy : {
						type : 'ajax',
						url : me.dictionaryListServerUrl + "?"
								+ me.dictionaryListServerParam + "="
								+ componentItemId,
						reader : {
							type : 'json',
							root : me.objectRoot
					...
					listeners : {
						load : function(store, records, successful, eOpts) {
							if (records != null) {
								record = me.getSouthRecordByKeyValue('InteractionField', componentItemId);
								if (record.get('isOperation') == true) {
									...
									var panel = east.getComponent(panelItemId);
									var serverUrl = panel
											.getComponent('otherParams')
											.getComponent('serverUrl');
									var defaultValue = panel
											.getComponent('otherParams')
											.getComponent('defaultValue');
									var myUrl = '';
									if (records.length > 1) {
										myUrl = records[1]
												.get(me.objectServerValueField);
									} else {
										myUrl = records[0]
												.get(me.objectServerValueField);
									}
									...
									defaultValue.store.proxy.url = getRootPath()
											+ "/"
											+ myUrl.split("?")[0]
											+ "?isPlanish=true&where=";
				...
				listeners : {
					focus : function(owner, The, eOpts) {
						...
						me.setColumnParamsRecord(componentItemId, 'ServerUrl',
								serverUrl);
					},
					select : function(combo, records, eOpts) {
						// 给组件的服务地址赋值
						...
						var east = me.getComponent('east');
						var panel = east.getComponent(panelItemId);
						var defaultValue = panel.getComponent('otherParams')
								.getComponent('defaultValue');
				...
				store : new Ext.data.Store({
					...
					proxy : {
						...
						extractResponseData : function(response) {
                            ...
                            if(valuefield==""){
                                textfield=defaultValueArr[defaultValueArr.length - 2] + "_"+ defaultValueArr[defaultValueArr.length - 1];
                                valuefield=textfield;
                            }
							for (var i in data.ResultDataValue) {
								data.ResultDataValue[i].value = data.ResultDataValue[i][valuefield];
                                
								data.ResultDataValue[i].text = data.ResultDataValue[i][textfield];
								
							}

							// 下拉框组件data赋值
							var com = me.getCenterCom()
									.getComponent(componentItemId);
...
//							var tempValue = owner.getValue();
	        	...
	        	xtype:'combobox',fieldLabel:'选择模式',value:'false',
	            mode:'local',
	            labelWidth : 55,
				anchor : '100%',
				editable : true,
				typeAhead : true,
				forceSelection : true,
	            itemId:'multiSelect',name:'multiSelect',
				displayField:'text',valueField:'value',

				store:new Ext.data.SimpleStore({
				    fields:['value','text'], 
				    data:[['false','单选'],['true','多选']]
				}),
				listeners : {
					focus : function(owner, The, eOpts) {
						// 值字段赋值
	...
	changeComboParamsPanelDefaultValue : function(componentItemId) {
		...
		var defaultValue = panel.getComponent('otherParams')
				.getComponent('defaultValue');
		var valueField = panel.getComponent('otherParams')
				.getComponent('valueField');
		var displayField = panel.getComponent('otherParams')
				.getComponent('displayField');
		var serverUrl = panel.getComponent('otherParams')
				.getComponent('serverUrl');
	
		var valueFieldValue = valueField.getValue();
		var displayFieldValue = displayField.getValue();
		var url = serverUrl.getValue();
		if (valueFieldValue && valueFieldValue != "" && displayFieldValue
				&& displayFieldValue != "" && url && url != "") {
	...
	createCheckboxfieldItems : function(componentItemId) {
		...
		var items = [{
			...
			items : [{
				...
				listeners : {
					blur : function(com, The, eOpts) {
						me.setColumnParamsRecord(componentItemId, 'Height',
								this.value);
						// 更新设置展示区域的复选框的高度
						var radioItem = me.getCenterCom()
								.getComponent(componentItemId);
				...
				listeners : {
					blur : function(com, The, eOpts) {
						me.setColumnParamsRecord(componentItemId, 'Width',
								this.value);
						// 更新设置展示区域的复选框的宽度
						var radioItem = me.getCenterCom()
								.getComponent(componentItemId);
	...
	createNumberfieldItems : function(componentItemId) {
		...
		var items = [{
			...
			items : [{
				...
				listeners : {
					blur : function(com, The, eOpts) {
						me.setColumnParamsRecord(componentItemId, 'NumberMin',
								this.value);
					},
					select : function(combo, records, eOpts) {
				...
				listeners : {
					blur : function(com, The, eOpts) {
						me.setColumnParamsRecord(componentItemId, 'NumberMax',
								this.value);
					},
					select : function(combo, records, eOpts) {
						var value = combo.getValue();
				...
				listeners : {
					blur : function(com, The, eOpts) {
						me.setColumnParamsRecord(componentItemId,
								'NumberIncremental', this.value);
					},
					select : function(combo, records, eOpts) {
						var value = combo.getValue();
						me.setColumnParamsRecord(componentItemId,
								'NumberIncremental', value);
					}
				}
			}]
		}];
	...
	updateTransformItems : function(arrTemp, groupName, defaultValue) {
		...
		for (var i = 0; i < count; i++) {
			...
			tempItem = "{" + checkedStr + "name:'" + groupName + "',"
					+ "inputValue:'" + value + "'," + "boxLabel:'" + text + "'"
					+ "}";
	 ...
	 * @return {}
	 */
	GetRadiogroupItems : function(url, groupName, valueField, displayField,
			defaultValue2) {
		var me = this;
		var myUrl = '';
		if (url == "" || url == null) {
			...
			myUrl = url
					+ ("?isPlanish=true&start=0&limit=10000&fields="
						...
						if (result.success) {
							...
							for (var i = 0; i < count; i++) {
								...
								if (arrStr !== null && arrStr.length > 0) {
									mychecked = Ext.Array.contains(arrStr,
											DeptID);
								}
								var tempItem = {
									checked : mychecked,
	...
	GetComboboxItems : function(url, value2, text2) {
		...
		if (url == "" || url == null) {
			...
			myUrl = url
					+ ("?isPlanish=true&start=0&limit=10000&fields=" + value2
							+ "," + text2);
		}
		var localData = [];
	...
	/**
	 * 获取构建表单面板配置参数集合 如需要取表单类型的选择结果值时,就可以这样取 var
	 * params=getPanelParams();params.formType
	 * 
	 * @private
	 * @return {}
	 */
	 ...
	 * 
	 * @param {}
	 *            componentItemId:交互字段,某一控件的itemId
	 * @param {}
	 *            newValue:修改的值
	 */
	setBasicParamsForInteractionField : function(componentItemId, keys,
			newValue) {
		var me = this;
		var record = me.getSouthRecordByKeyValue('InteractionField',
				componentItemId);
	 ...
	 *            newValue:修改的值
	 */
	setOtherParamsForInteractionField : function(InteractionField, itemId,
			newValue) {
		var me = this;
		var record = me.getSouthRecordByKeyValue('InteractionField',
				InteractionField);
	...
	setParamsPanelValues : function(componentItemId) {
		...
		if (componentItemId != "center") {
			...
			me.setBasicParamsForInteractionField(InteractionField, 'name',
			...
			if (record.get('Type') != 'button') {
				me.setBasicParamsForInteractionField(InteractionField,
						'isOperation', record.get('isOperation'));
			}
			// 属性面板特有数据赋值
	...
	setComboboxValue : function(record, InteractionField) {
		...
		me.setOtherParamsForInteractionField(InteractionField, 'valueField',
				valueField);
		me.setOtherParamsForInteractionField(InteractionField, 'displayField',
				displayField);
		...
		if (serverUrl != "" && serverUrl != null) {
			...
			me.setOtherParamsForInteractionField(InteractionField, 'serverUrl',
					serverUrl);
		}
		...
		var panel = me.getComponent('east').getComponent(panelItemId);
		var other = panel.getComponent("otherParams");
		var multiSelectCom = other.getComponent('multiSelect');
		var multiSelect = record.get('multiSelect');
		
		if (multiSelect == '' || multiSelect == "" || multiSelect == null) {
			me.setColumnParamsRecord(InteractionField, 'multiSelect', multiSelect);
		}
		multiSelectCom.setValue(multiSelect);
	...
	setButtonParamsPanelValue : function(record, InteractionField) {
		...
		me.setOtherParamsForInteractionField(InteractionField,
				'InteractionField', interactionField);
		me.setOtherParamsForInteractionField(InteractionField, 'btnType',
				btnType);
	},
	/***************************************************************************
	 * 给单选组,复选组特有属性设置默认值
	...
	setCheckboxgroupValue : function(record, InteractionField) {
		...
		me
				.setOtherParamsForInteractionField(InteractionField, 'Height',
						height);
		me.setOtherParamsForInteractionField(InteractionField, 'Width', width);
		me.setOtherParamsForInteractionField(InteractionField, 'ColumnWidth',
				columnWidth);
		me.setOtherParamsForInteractionField(InteractionField, 'Columns',
				columns);
	...
	setParamsPanelValuesByType : function(componentItemId, record) {
		...
		// 文本框特有属性设值
			...
			me.setTxtParamsPanelValue(record, componentItemId);
		}
	},
	/**
	 * 文本框特有属性设值
	 ...
	 *            record
	 */
	setTxtParamsPanelValue : function(record, componentItemId) {
		var me = this;
		// 属性面板ItemId
		var panelItemId = componentItemId + me.ParamsPanelItemIdSuffix;
		// 组件属性面板
		var panel = me.getComponent('east').getComponent(panelItemId);
		var other = panel.getComponent("othertxtParams");
		var txtTypeCom = other.getComponent('txtType');
//		txtItemsParams.setValue(txtItemsParams.getValue());

//		alert(txtItemsParams.getValue());
//		var canEditCom = other.getComponent('CanEdit');

		var txtType = record.get('txtType');
		

		if (txtType == '' || txtType == "" || txtType == null) {
			me.setColumnParamsRecord(componentItemId, 'txtType', txtType);
		}
//		fieldLabelTwoCom.setValue(record.get('fieldLabelTwo'));
		txtTypeCom.setValue(txtType);
...
//		txtItemsParams.setValue(txtItemsParams.getValue());
	...
	setComListeners : function(com, arr) {
		...
		com.listeners = {// move和resize有冲突
								...
								items : [{
									...
									handler : function() {
										...
										if (com.itemId.indexOf("operation") != -1) {
											...
											oldItemId = oldItemId.substring(0,
													oldItemId.length - 10);// 去掉"_operation"
											me.removeOperationCom(oldItemId,
													com.itemId);
										} else {
											if (com.isOperation == true) {// 删除表单中的组件中存在关系运算组件
												var componentItemId = com.itemId;
												var operationItemId = com.itemId
														+ "_operation";
												me.removeOperationCom(
														componentItemId,
														operationItemId);
											}
			...
			specialkey : function(field, e) {
				...
				if (num > 0
						&& (e.getKey() == Ext.EventObject.ENTER || e.getKey() == Ext.EventObject.TAB)) {
					if (!e.shiftKey) {// 同时按下shift键
						num = (num + iNum > max) ? num + iNum - max : num
								+ iNum;
					} else {
			...
			beforequery : function(e) {
				if (!e.forceAll&&e.combo.xtype == "combobox") {
					try{    
	            		var value = e.query;
	            		e.combo.lastQuery = value; 
	            		e.combo.store.filterBy(function(record,id){   
	            			var text = record.get(e.combo.displayField); 
	            			return (text.indexOf(value)!=-1);   
	            		});
	            		e.combo.expand();
	            		return false;
	            	}catch(e){
	            		e.combo.selectedIndex = -1;
	            		e.combo.store.clearFilter();
	            		return false;  
	            	}
				}
			}
	...
	getSouthStoreFields : function() {
		var me = this;
		var fields = [{
					name : me.columnParamsField.DisplayName,
					type : 'string'
				},// 显示名称
				{
					name : me.columnParamsField.InteractionField,
					type : 'string'
				},// 交互字段
				{
					name : 'LabelWidth',
	...
	setAppParams : function() {
		var me = this;
		var paramsPanel = me.getComponent('east').getComponent('center'
				+ me.ParamsPanelItemIdSuffix);
		...
		var callback = function(appInfo) {
			...
			objectPropertyTree.store.on({
						load : function(store, node, records, successful, e) {
							if (me.appId != -1 && me.isJustOpen
									&& node == objectPropertyTree.getRootNode()) {
								// 对象内容勾选
			...
			// 渲染效果
			setTimeout(function(southParams) {
						me.browse();
					});
		};
		// 从后台获取应用信息
	...
	createAppClass : function() {
		...
		var GetComboboxItems = "function(url2,valueField,displayField,groupName,defaultValue){"
				+ "var myUrl=url2;"
				+ "if(myUrl==''||myUrl==null){"
					...
					if (item != null) {
						...
						if (type == 'checkboxgroup' || type == 'radiogroup') {
							...
							if (isdataValue == false) {// 不是定值的单/复选组才添加
								...
								strtemp = strtemp + "var array" + index
										+ "=me.GetGroupItems(" + "'"
										+ item.tempUrl + "'," + "'"
										+ item.tempValue + "'," + "'"
										+ item.tempText + "'," + "'"
										+ item.tempGroupName + "'," + "'"
										+ arrStr + "');" + "var item" + index
										+ "=me.getComponent('" + item.itemId
										+ "');" + "item" + index
		...
		var appClass = "Ext.define('" + params.appCode + "',{"
				+ "extend:'Ext.form.Panel'," + "alias:'widget."
				+ params.appCode + "'," + "title:'" + params.titleText + "',"
				+ "width:" + params.Width + "," + "height:" + params.Height
				+ "," + "autoScroll:true," + "type:'add'," + // 显示方式add（新增）、edit（修改）、show（查看）
				"layout:'absolute'," +
		...
		appClass = appClass + "initComponent:function(){" + "var me=this;";
		// 注册事件
		...
		appClass = appClass
				...
				+
				// 内部是否只读方法
				"me.setReadOnly=function(bo){" + "var items = me.items;"
				+ "for(var i in items){" + "items[i].readOnly=bo;" + "}" + "};"
				+
				// 内部组件
				"me.items=" + items + ";";

		if (html != "") {
			appClass = appClass + "me.html='" + html.replace(/\"/g, "\\\"")
					+ "';";
		}
		appClass = appClass + "me.callParent(arguments);" + "}," +

		"afterRender:function(){" + "var me=this;" +

		"if(me.type == 'edit'){" + "me.load();"
				+ "}else if(me.type == 'show'){" + "me.load();"
				+ "me.setReadOnly(true);" + "}" + strtemp
				+ "me.callParent(arguments);"
	...
	createNumbersintervalsStr : function(record) {
		...
		var labelStyle = (record.get('LabFont') != "")
				? (record.get('LabFont'))
				: ('font-style:normal;');
		var width1 = record.get('Width');
		var height1 = record.get('Height');
		...
		var com = "{" + "xtype:'numbersintervals'" + "," + "itemId:'"
				+ record.get('InteractionField') + "'," + "x:"
				+ record.get('X') + "," + "y:"
				+ record.get('Y')
				+ ","
				+ "hidden:"
				...
				// 自定义属性,每个个控件都有
				"isOperation:"
				+ record.get('isOperation')
				+ ","
				+ // 是否绑定带有运算关系符
				"fieldLabel:'" + fieldLabel + "'," + "labelWidth:"
				+ record.get('LabelWidth') + "," + "labelStyle:'" + labelStyle
				+ "'," + "width:" + width1 + "," + "height:" + height1 + ","
				+ "hidden:" + record.get('IsHidden') + "," + "fieldLabelTwo:'"
	...
	createTimeintervalsStr : function(record) {
		...
		var fieldLabelTwo = '' + record.get('fieldLabelTwo');//
		var com = // Ext.create('Ext.zhifangux.TimeIntervals', {
		"{" + "xtype:'timeintervals'" + "," + "x:" + record.get('X') + ","
				+ "y:"
				+ record.get('Y')
				+ ","
				+ "itemId:'"
				...
				+ "'," + "width:" + width1 + "," + "height:" + height1 + ","
				+ "hidden:" + record.get('IsHidden') + "," + "fieldLabelTwo:'"
				+ fieldLabelTwo + "'," + "layoutType:'" + rawOrCol
				+ "',"
				+ // 控件布局设置,默认为横布局(hbox),竖布局为(vbox)
				"labelAlign:'left'" + "," + "value:new Date(),"
				+ "valueTwo:new Date()," + "editable:" + canEdit + ","
				+ "dateFormat:" + "'" + showFomart + "'" +
	...
	createDataCheckboxStr : function(record) {
		...
		var columns = (record.get('Columns') != "")
				? (record.get('Columns'))
				: ('2');
		var columnWidth = (record.get('ColumnWidth').lenght > 0) ? (record
				.get('ColumnWidth')) : ('100');
		...
		var com = "{"
				...
				"fieldLabel:'" + fieldLabel + "'," + "labelWidth:"
				+ record.get('LabelWidth') + "," + "labelStyle:'" + labelStyle
				+ "'," + "width:" + record.get('Width') + "," + "height:"
				+ record.get('Height') + "," + "columnWidth:" + columnWidth
				+ "," + "columns:" + columns + "," + "itemId:'"
				+ record.get('InteractionField') + "'," + "x:"
				+ record.get('X') + "," + "y:" + record.get('Y') + ","
				+ "hidden:" + record.get('IsHidden') + "," + "readOnly:"
				+ record.get('IsReadOnly') + "," + "vertical: true" + ","
				+ "padding:2" + "," + "autoScroll:true," +

				"isdataValue:true" + ","
				+ // 是否是定值单/复选组,单/复选组特有自定义添加的属性
				"items:" + combodata + "," + "tempGroupName:'" + groupName
				+ "'" + "}";
		return com;
	},
	/**
	...
	createDataRadioStr : function(record) {
		...
		var com = "{"
				...
				+ record.get('X') + "," + "y:" + record.get('Y') + ","
				+ "readOnly:" + record.get('IsReadOnly') + ","
				+ "vertical: true" + "," + "padding:2" + ","
				+ "autoScroll:true" + "," +

				"hidden:" + record.get('IsHidden') + "," + "isdataValue:true"
				+ ","
				+ // 是否是定值单/复选组,单/复选组特有自定义添加的属性
				"items:" + combodata + "," + "tempGroupName:'" + groupName
				+ "'" + "}";
	...
	createListenersStr : function(records) {
		...
		var com = ",listeners:{"
				...
				+ "}" + "for(var i in items){" + "if(items[i].sortNum == num){"
	...
	createDataComboxStr : function(record) {
		...
		if (combodata.length > 0) {
			...
			for (var i = 0; i < arr.length; i++) {
				...
				// myValue="'"+tempValue+"'";
				// }
				data = data + "{'value':" + "'" + tempValue + "'," + "'text':"
						+ "'" + tempText + "'" + "},";
			}
		...
		if (data.length > 0) {// 截掉字符串最后一个字符","
			...
			myValue = "'" + record.get('defaultValue') + "'";// 设置默认值
		} else {
			data = "{'value':'=', 'text':'等于'},"
					+ "{'value':'!=','text':'不等于'},"
					+ "{'value':'>','text':'大于'},"
		...
		var com = "{"
				...
				// 自定义属性,定值运算关系下拉框特有
				"boundField:'" + record.get('BoundField') + "'," + "name:'"
				+ record.get('InteractionField') + "'," + "fieldLabel:'"
				+ fieldLabel + "'," + "labelWidth:" + record.get('LabelWidth')
				+ "," + "labelStyle:'" + labelStyle + "'," + "width:"
				+ record.get('Width') + "," + "height:" + record.get('Height')
				+ "," + "itemId:'" + record.get('InteractionField') + "',"
				+ "value:" + myValue + "," + "allowBlank: true," + "x:"
				+ record.get('X') + "," + "y:" + record.get('Y') + ","
				+ "displayField:'text'," + "valueField:'value'," + "readOnly:"
				+ record.get('IsReadOnly') + "," + "hidden:"
				+ record.get('IsHidden') + "," + "mode:'local'," +
				// "editable:false," +
				"store:" + myStore + "" + "}";
		return com;
	},
	...
	createTextStr : function(record) {
		...
		var com = "{"
				...
				+ record.get('LabelWidth') + "," + "labelStyle:'" + labelStyle
				+ "'," + "width:" + record.get('Width') + "," + "height:"
				+ record.get('Height') + "," + "itemId:'"
				+ record.get('InteractionField') + "'," + "x:"
				+ record.get('X') + "," + "y:" + record.get('Y') + ","
				+ "readOnly:" + record.get('IsReadOnly') + "," + "hidden:"
				+ record.get('IsHidden')+ ","  + "txtType:'"
				+ record.get('txtType')+ "'"  + "}";
		return com;
	},
	/**
	...
	createDateIntervalsStr : function(record) {
		...
		var com = "{"
				...
				+ record.get('InteractionField') + "'," + "itemId:'"
				+ record.get('InteractionField') + "'," + "fieldLabel:'"
				+ fieldLabel + "'," + "fieldLabelTwo:'" + fieldLabelTwo + "',"
				+ "labelWidth:" + record.get('LabelWidth') + "," + "width:"
				+ record.get('Width') + "," + "height:" + record.get('Height')
				+ "," + "layoutType:'" + rawOrCol + "'," + "labelAlign:'left'"
				+ "," + "value:''" + "," + "valueTwo:''" + "," + "height:"
				+ record.get('Height') + "," + "x:" + record.get('X') + ","
				+ "y:" + record.get('Y') + "," + "readOnly:"
				+ record.get('IsReadOnly') + "," + "labelStyle:'" + labelStyle
	...
	createDaterangeStr : function(record) {
		...
		var com = "{"
				...
				"editable:" + canEdit + "," + "name:'"
				+ record.get('InteractionField') + "'," + "itemId:'"
				+ record.get('InteractionField') + "'," + "fieldLabelOne:'"
				+ fieldLabel + "'," + "fieldLabelTwo:'" + fieldLabelTwo + "',"
				+ "labelWidthOne:" + record.get('LabelWidth') + ","
				+ "labelWidthTwo:" + record.get('LabelWidthTwo') + ","
				+ "labelSeparatorOne:'" + record.get('LabelSeparatorOne')
				+ "'," + "labelSeparatorTwo:'"
				+ record.get('LabelSeparatorTwo') + "'," + "width:"
				+ record.get('Width') + "," + "height:" + record.get('Height')
				+ "," + "layoutType:'" + rawOrCol + "'," + "labelAlign:'left'"
				+ "," + "valueOne:''" + "," + "valueTwo:''" + "," + "height:"
				+ record.get('Height') + "," + "x:" + record.get('X') + ","
				+ "y:" + record.get('Y') + "," + "readOnly:"
				+ record.get('IsReadOnly') + "," + "labelStyle:'" + labelStyle
				+ "'," + "hidden:" + record.get('IsHidden') + "," +
	...
	/**
	 * 创建下拉框Str getRootPath是不是需要修改成在类代码还原后再取
	 * 
	 * @private
	 * @param {}
	 *            record
	...
	createComboboxStr : function(record) {
		...
		if(multiSelect=='true'){
			editable=false;
			multiSelectstr="multiSelect:" + record.get('multiSelect') + "," ;
		}else{
			multiSelectstr='';
		}
		if ((valueField == null || valueField == "")
				...
				|| (url == null || url == "")) {
			Ext.Msg.alert('提示', '错误信息【<b style="color:red">'
							+ '请先配置下拉框的数据对象或数据服务' + "</b>】");
			return false;
		} else {
			url = url + "?isPlanish=true&fields=" + valueField + ","
					+ textField;
		}
		var com = "{"
				+ "xtype:'combobox'"
				...
				+ record.get('X') + "," + "y:" + record.get('Y') + ","
				+ "readOnly:" + record.get('IsReadOnly') + "," + "hidden:"
				+ record.get('IsHidden') + "," + "value:'"
				+ record.get('defaultValue') + "'," + "mode:'local',"+
				multiSelectstr+
//				 "multiSelect:" + record.get('multiSelect') + "," +
				 "allowBlank: true," +
				"editable:"+editable+"," +
				"displayField:'" + textField + "'," + "valueField:'"
				+ valueField + "'," + "store:new Ext.data.Store({"
				+ "fields:['" + textField + "','" + valueField + "']" + ","
				+ "pageSize:1000," + "proxy:{" + "type:'ajax',"
				+ "async:false," + "url:getRootPath()+" + "'/'+" + "'"
				+ url
				+ "',"
				+ // 修改getRootPath()的路径问题
				"reader:{type:'json',root:'" + me.objectRootTwo + "'}" + ","
				+ "extractResponseData:me.changeStoreData" + "},autoLoad:true"
				+ "})," + 
				"listeners:{"+
			      "blur:function(field, e) {"+
                      "if (field.xtype == 'combobox' && field.multiSelect==false ) {"+
                          "var index = field.store.findRecord(field.valueField, field.getValue());"+
                          "if (index == null) {"+
                              "field.reset();"+
                          "}"+
                      "}"+
	...
	createNumberStr : function(record) {
		...
		var com = "{"
				...
				"minValue:" + record.get('NumberMin') + "," + "maxValue:"
				+ record.get('NumberMax') + "," + "step:"
				+ record.get('NumberIncremental') + "}";
		return com;
	},
	/**
	 * 创建日期框Str
	...
	createTimeStr : function(record) {
		...
		var com = "{"
				...
				+ record.get('X') + "," + "y:" + record.get('Y') + ","
				+ "readOnly:" + record.get('IsReadOnly') + "," + "hidden:"
				+ record.get('IsHidden') + "," + "selectOnFocus:true";
		if (showFomart == '' || showFomart == "" || showFomart == null) {
	...
	createDateTimeStr : function(record) {
		...
		var com = "{"
				...
				+ ","
				+ // 是否绑定带有运算关系符
				"height:" + record.get('Height') + "," + "itemId:'"
				+ record.get('InteractionField') + "'," + "x:"
				+ record.get('X') + "," + "y:" + record.get('Y') + ","
				+ "xtype:'datetimenew'" + "," + "editable:" + canEdit + ","
				+ "name:'" + record.get('InteractionField') + "',"
				+ "fieldLabel:'" + fieldLabel + "'," + "labelWidth:"
				+ record.get('LabelWidth') + "," + "labelStyle:'" + labelStyle
				+ "'," + "width:" + record.get('Width') + "," + "x:"
				+ record.get('X') + "," + "y:" + record.get('Y') + ","
				+ "readOnly:" + record.get('IsReadOnly') + "," + "hidden:"
				+ record.get('IsHidden') + "," + "labelAlign:'left'" + "," +
				// "value:new Date()"+ ","+
				"setType:'datetime'" + "," + // 控件类型:datetime(日期时间),date(日期),time(时间),
				"selectOnFocus:true";
		if (showFomart == '' || showFomart == "" || showFomart == null) {
	...
	createCheckboxStr : function(record) {
		...
		var com = "{"
				...
				+ "readOnly:" + record.get('IsReadOnly') + ","
				+ "vertical: true" + "," + "padding:2" + ","
				+ "autoScroll:true," + "hidden:" + record.get('IsHidden') + ","
				+ "tempUrl:'" + "/" + url + "'" + "," + "tempValue:'"
				+ record.get('valueField') + "'" + "," + "tempText:'"
				+ record.get('textField') + "'" + "," + "isdataValue:false"
				+ ","
				+ // 是否是定值单/复选组,单/复选组特有自定义添加的属性
	...
	createLabelStr : function(record) {
		var com = "{"
				...
				"name:'" + record.get('InteractionField') + "'," + "width:"
				+ record.get('Width') + "," + "height:" + record.get('Height')
				+ "," + "text:'" + record.get('DisplayName') + "',"
				+ "itemId:'" + record.get('InteractionField') + "'," + "x:"
				+ record.get('X') + "," + "y:" + record.get('Y') + ","
				+ "hidden:" + record.get('IsHidden') + "}";
		return com;
	},
	...
	createImageStr : function(record) {
		var me = this;
		var fieldLabel = me.hasLab ? record.get('DisplayName') : "";
		var src = (record.get('ServerUrl') && record.get('ServerUrl') != "")
				? record.get('ServerUrl')
				: "getRootPath()+'/ui/css/images/default/defaultImg.png'"
		var labelStyle = (record.get('LabFont').lenght > 0) ? (record
				.get('LabFont')) : ('font-style:normal;');
		var com = "{"
				+ "xtype:'image',"
				+ "isOperation:"
				+ record.get('isOperation')
	...
	createFileStr : function(record) {
		var me = this;
		var fieldLabel = me.hasLab ? record.get('DisplayName') : "";
		var buttonText = (record.get('SelectFileText') && record
				.get('SelectFileText') != "")
				? record.get('SelectFileText')
				: "选择";
		var labelStyle = (record.get('LabFont') != "")
				? (record.get('LabFont'))
				: ('font-style:normal;');
		var com = "{"
				...
				"selectOnFocus:false" + "," + "buttonOnly:false" + ","
	...
	createButtonStr : function(record) {
		...
		var com = "{"
				...
				+ record.get('Width') + "," + "height:" + record.get('Height')
				+ "," + "text:'" + record.get('DisplayName') + "',"
				+ "btnType:'" + record.get('btnType') + "',"
				+ // 自定义属性,按钮类型:查询或重置,关闭
				"itemId:'" + record.get('InteractionField') + "'," + "x:"
				+ record.get('X') + "," + "y:" + record.get('Y') + ","

		if (btnType == "select") {// 查询按钮
			com = com + "hidden:me.isHiddenbtnSelect,"
					+ "iconCls:'bsearch-img-16',";

		} else if (btnType == "reset") {// 重置按钮
			com = com + "hidden:me.isHiddenbtnReset,"
					+ "iconCls:'build-button-refresh',";
		} else if (btnType == "colse") {// 关闭按钮
			com = com + "hidden:me.isHiddenbtnClose,"
					+ "iconCls:'imgdiv-close-show-16',";
		} else {
			com = com + "hidden:" + record.get('IsHidden') + ",";
		}

		com = com + "listeners: {" + "click: function() {";
		if (btnType == "select") {// 查询按钮
			com = com + "me.fireEvent('selectClick');";
			com = com + "var lastValue=me.getValue();";

		} else if (btnType == "reset") {// 重置按钮
			com = com + "me.getForm().reset();" + "me.fireEvent('resetClick');";
		} else if (btnType == "close") {// 关闭按钮
			com = com + "me.close();" + "me.fireEvent('closeClick');";
		}
		com = com + "}" + " }" +
	...
	createSubmitSelectStr : function() {
		var me = this;
		var fun = "function(){" + "var form = this;"
				+ "var logicalType= form.logicalType;"
				+ "if(logicalType === ''||logicalType ==undefined){"
				+ "logicalType=' and ';" + "}" + "var items=form.items.items;"
				+ "var length=form.items.length;"
				+ "var lastValue='';"
				+ // myValue:单个组件的结果值;lastValue:最后where串
				"var operation=' like ';"
				+ // 关系运算符,默认为like
				// 保证有值
	...
	getAppInfoFromServer : function(callback) {
		...
		if (me.appId != -1 && me.appId != "undefined") {
			...
			Ext.Ajax.request({
						...
						success : function(response, opts) {
							...
							if (result.success) {
								...
								Ext.Msg.alert('提示',
										'获取应用组件信息失败！错误信息【<b style="color:red">'
												+ result.errorInfo + "</b>】");
							}
						},
						failure : function(response, options) {
		...
		Ext.Ajax.request({
					...
					success : function(response, opts) {
						...
						if (result.success) {
							...
							Ext.Msg.alert('提示',
									'保存应用组件失败！错误信息【<b style="color:red">'
											+ result.ErrorInfo + "</b>】");
						}
					},
					failure : function(response, options) {
