$(function() {
	...
	function loadInfoData(callback){
		...
		var fields = [
			...
			'BmsCenOrderDoc_ReqDeliveryTime','BmsCenOrderDoc_LabMemo','BmsCenOrderDoc_CompMemo',
			'BmsCenOrderDoc_TotalPrice','BmsCenOrderDoc_OperDate'
		];
	...
	function loadDtlData(callback){
		...
		var fields = [
			...
			'BmsCenOrderDtl_GoodsUnit','BmsCenOrderDtl_UnitMemo','BmsCenOrderDtl_Memo',
	...
	function createInfo(data){
		...
		html.push('<div class="list-group-item-text" style="font-size:13px;">订货单号: ' + data.BmsCenOrderDoc_OrderDocNo + '</div>');
		...
		html.push('<div class="list-group-item-text" style="font-size:13px;">供货方备注: <span style="color:#d9534f;font-weight:bold;">' + (data.BmsCenOrderDoc_CompMemo || '') + '</span></div>');
		html.push('<div class="list-group-item-text" style="font-size:13px;">订货方备注: <span style="color:#d9534f;font-weight:bold;">' + (data.BmsCenOrderDoc_LabMemo || '') + '</span></div>');
	...
	function createRow(value){
		...
		//明细备注
		if(value.BmsCenOrderDtl_Memo){
			html.push('<div class="list-group-item-text">' +
				'<span style="color:#d9534f;font-weight:bold;">备注: ' + (value.BmsCenOrderDtl_Memo || '') + 
				'</span></div>');
		}
