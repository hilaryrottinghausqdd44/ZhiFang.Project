    ...
    public class WeiXinAppService : IWeiXinAppService
    {
        ...
        public BaseResultDataValue ST_UDTO_AddBWeiXinEmpLinkByUserAccount(string strUserAccount, string strPassWord, bool isValidate)
        {
            ...
            try
            {
                ...
                string WeiXinOpenID = ZhiFang.Common.Public.Cookie.CookieHelper.Read(ZhiFang.Digitlab.Entity.DicCookieSession.WeiXinOpenID);
                if (WeiXinOpenID != null && WeiXinOpenID.Trim() != "")
                {
                    ...
                    if (emp != null)
                    {
                        Cookie.CookieHelper.Write(DicCookieSession.LabID, "");
                        //Cookie.CookieHelper.Write(DicCookieSession.IsLabFlag, "");

                        SessionHelper.SetSessionValue(DicCookieSession.LabID, emp.LabID.ToString());//实验室ID
                        ...
                        Cookie.CookieHelper.Write(DicCookieSession.LabID, emp.LabID.ToString());//实验室ID
                        //if (emp.LabID > 0)
        ...
        public BaseResultDataValue PushConfirmBmsCenOrderDoc(long Id)
        {
            BaseResultDataValue tempBaseResultDataValue = new BaseResultDataValue();
            try
            {
                tempBaseResultDataValue = IBBmsCenOrderDoc.BmsCenOrderDocConfirmAndPush((SysWeiXinTemplate.PushWeiXinMessage)BasePage.PushWeiXinMessageAction, Id);
