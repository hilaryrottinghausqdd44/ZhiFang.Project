...
namespace ZhiFang.Digitlab.BLL.Business
{
	...
	///
	/// </summary>
    public class BBWeiXinAccount : BaseBLL<BWeiXinAccount>, ZhiFang.Digitlab.IBLL.Business.IBBWeiXinAccount
	{
        ...
        public void PushWeiXinMessage(SysWeiXinTemplate.PushWeiXinMessage pushWeiXinMessageAction, List<long> receiveidlist, Dictionary<string, TemplateDataObject> pdata, string syscode, string url)
        {
            if (receiveidlist.Count > 0)
            {
                ...
                if (bwxell != null && bwxell.Count > 0)
                {
                    ...
                    {
                        case "addbmscenorderdoc": Shortcode = SysWeiXinTemplate.新订单提醒; break;
                        case "bmscenstatuschange": Shortcode = SysWeiXinTemplate.订单状态通知; break;
                        case "addbmscenorderdoctocomp": Shortcode = SysWeiXinTemplate.新订单提醒公司; break;
                    }

                    IList<ZhiFang.Digitlab.Entity.BWeiXinPushMessageTemplate> bwxpmtl = IDBWeiXinPushMessageTemplateDao.GetListByHQL(" Shortcode='" + Shortcode + "' ");
