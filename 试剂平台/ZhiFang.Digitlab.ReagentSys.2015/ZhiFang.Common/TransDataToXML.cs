...
namespace ZhiFang.Common.Public
{
    public class TransDataToXML
    {
        ...
        /// <param name="xmlFilePath">XML文件路径</param>
        /// <returns>bool值</returns>
        public static bool CDataToXmlFile(DataTable dt, string xmlFilePath, string encodeName)
        {
            if ((dt != null) && (!string.IsNullOrEmpty(xmlFilePath)))
            {
                string path = xmlFilePath;
                ...
                try
                {
                    ms = new MemoryStream();
                    //根据ms实例化XmlWt
                    XmlWt = new XmlTextWriter(ms, Encoding.GetEncoding(encodeName));
                    XmlWt.Formatting = Formatting.Indented;
                    XmlWt.Indentation = 1;
                    XmlWt.IndentChar = '\t';
                    //获取ds中的数据
                    ...
                    //写文件
                    StreamWriter sw = new StreamWriter(path);
                    sw.WriteLine("<?xml version=\"1.0\" encoding=\""+ encodeName + "\"?>");
                    sw.WriteLine(Encoding.GetEncoding(encodeName).GetString(temp).Trim());
                    sw.Close();
                    return true;
                }
                catch (System.Exception ex)
        ...
        /// <param name="xmlFilePath">xml文件路径</param>
        /// <returns>bool]值</returns>
        public static bool CDataToXmlFile(DataSet ds, int tableIndex, string xmlFilePath, string encodeName)
        {
            if (tableIndex != -1)
            {
                return CDataToXmlFile(ds.Tables[tableIndex], xmlFilePath, encodeName);
            }
            else
            {
                return CDataToXmlFile(ds.Tables[0], xmlFilePath, encodeName);
            }
        }

        public static void DataTableToXML(DataTable dt, string fileName, string encodeName)
        {
            try
            {
                XmlDocument document = new XmlDocument();
                string xmlStr = "<?xml version=\"1.0\" encoding=\"" + encodeName + "\"?>";
                xmlStr += "<NewDataSet>";
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    string rowStr = "<TableName>";
                    foreach (DataColumn dc in dt.Columns)
                    {
                        rowStr += "<" + dc.ColumnName + ">";
                        if (dt.Rows[i][dc.ColumnName] != null)
                            rowStr += dt.Rows[i][dc.ColumnName].ToString();
                        rowStr += "</" + dc.ColumnName + ">";
