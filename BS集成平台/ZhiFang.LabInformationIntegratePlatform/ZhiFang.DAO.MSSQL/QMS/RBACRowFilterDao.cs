...
{
     public class RBACRowFilterDao : IDRBACRowFilter
    {
        ...
        public DataSet GetAllList()
        {
            ...
            strSql.Append("select * from RBAC_RowFilter");
            return DbHelperSQL.ExecuteDataSet(strSql.ToString());
        }
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(*) FROM RBAC_RowFilter");
        ...
        public List<ZhiFang.Entity.RBAC.RBACRowFilter> GetAllObjectList()
        {
            DataSet ds = new DataSet();
            ds = GetAllList();
            IList<ZhiFang.Entity.RBAC.RBACRowFilter> deptlist = new List<Entity.RBAC.RBACRowFilter>();
            if (ds == null || ds.Tables == null || ds.Tables.Count <= 0 || ds.Tables[0].Rows.Count <= 0)
            {
                return new List<Entity.RBAC.RBACRowFilter>();
            }
            deptlist = DataSetToEntityList<Entity.RBAC.RBACRowFilter>(ds, 0);
            return deptlist.ToList<ZhiFang.Entity.RBAC.RBACRowFilter>();
        }
        ...
        public static IList<T> DataSetToEntityList<T>(DataSet p_DataSet, int p_TableIndex)
        {
            ...
            for (int j = 0; j < p_Data.Rows.Count; j++)
            {
                ...
                {
                    if (pi.Name == "Id" && p_Data.Rows[j]["RowFilterID"] != DBNull.Value)
                    {
                        pi.SetValue(_t, p_Data.Rows[j]["RowFilterID"], null);
                    }
