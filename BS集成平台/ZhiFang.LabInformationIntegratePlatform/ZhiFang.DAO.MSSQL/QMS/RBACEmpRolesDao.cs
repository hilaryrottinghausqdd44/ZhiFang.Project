...
{
     public class RBACEmpRolesDao : IDRBACEmpRoles
    {
        ...
        public DataSet GetAllList()
        {
            ...
            strSql.Append("select * from RBAC_EmpRoles");
            return DbHelperSQL.ExecuteDataSet(strSql.ToString());
        }
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(*) FROM RBAC_EmpRoles");
        ...
        public List<ZhiFang.Entity.RBAC.RBACEmpRoles> GetAllObjectList()
        {
            DataSet ds = new DataSet();
            ds = GetAllList();
            IList<ZhiFang.Entity.RBAC.RBACEmpRoles> deptlist = new List<Entity.RBAC.RBACEmpRoles>();
            if (ds == null || ds.Tables == null || ds.Tables.Count <= 0 || ds.Tables[0].Rows.Count <= 0)
            {
                return new List<Entity.RBAC.RBACEmpRoles>();
            }
            deptlist = DataSetToEntityList<Entity.RBAC.RBACEmpRoles>(ds, 0);
            return deptlist.ToList<ZhiFang.Entity.RBAC.RBACEmpRoles>();
        }
        ...
        public static IList<T> DataSetToEntityList<T>(DataSet p_DataSet, int p_TableIndex)
        {
            ...
            for (int j = 0; j < p_Data.Rows.Count; j++)
            {
                ...
                {
                    if (pi.Name == "Id" && p_Data.Rows[j]["EmpRoleID"] != DBNull.Value)
                    {
                        pi.SetValue(_t, p_Data.Rows[j]["EmpRoleID"], null);
                    }
