...
{
     public class RBACEmpOptionsDao : IDRBACEmpOptions
    {
        ...
        public DataSet GetAllList()
        {
            ...
            strSql.Append("select * from RBAC_EmpOptions");
            return DbHelperSQL.ExecuteDataSet(strSql.ToString());
        }
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(*) FROM RBAC_EmpOptions");
        ...
        public List<ZhiFang.Entity.RBAC.RBACEmpOptions> GetAllObjectList()
        {
            DataSet ds = new DataSet();
            ds = GetAllList();
            IList<ZhiFang.Entity.RBAC.RBACEmpOptions> deptlist = new List<Entity.RBAC.RBACEmpOptions>();
            if (ds == null || ds.Tables == null || ds.Tables.Count <= 0 || ds.Tables[0].Rows.Count <= 0)
            {
                return new List<Entity.RBAC.RBACEmpOptions>();
            }
            deptlist = DataSetToEntityList<Entity.RBAC.RBACEmpOptions>(ds, 0);
            return deptlist.ToList<ZhiFang.Entity.RBAC.RBACEmpOptions>();
        }
        ...
        public static IList<T> DataSetToEntityList<T>(DataSet p_DataSet, int p_TableIndex)
        {
            ...
            for (int j = 0; j < p_Data.Rows.Count; j++)
            {
                ...
                {
                    if (pi.Name == "Id" && p_Data.Rows[j]["EmpOptionsID"] != DBNull.Value)
                    {
                        pi.SetValue(_t, p_Data.Rows[j]["EmpOptionsID"], null);
                    }
                    ...
                    else if (pi.Name == "Default" && p_Data.Rows[j]["DefaultModuleID"] != DBNull.Value)
                    {
                        var rBACModule = new Entity.RBAC.RBACModule();
                        rBACModule.Id = long.Parse(p_Data.Rows[j]["DefaultModuleID"].ToString());
