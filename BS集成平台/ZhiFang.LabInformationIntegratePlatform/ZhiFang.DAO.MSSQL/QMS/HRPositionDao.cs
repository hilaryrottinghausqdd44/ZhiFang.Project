...
{
     public class HRPositionDao : IDHRPosition
    {
        ...
        public DataSet GetAllList()
        {
            ...
            strSql.Append("select * from HR_Position");
            return DbHelperSQL.ExecuteDataSet(strSql.ToString());
        }
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(*) FROM HR_Position");
        ...
        public List<ZhiFang.Entity.RBAC.HRPosition> GetAllObjectList()
        {
            DataSet ds = new DataSet();
            ds = GetAllList();
            IList<ZhiFang.Entity.RBAC.HRPosition> deptlist = new List<Entity.RBAC.HRPosition>();
            if (ds == null || ds.Tables == null || ds.Tables.Count <= 0 || ds.Tables[0].Rows.Count <= 0)
            {
                return new List<Entity.RBAC.HRPosition>();
            }
            deptlist = DataSetToEntityList<Entity.RBAC.HRPosition>(ds, 0);
            return deptlist.ToList<ZhiFang.Entity.RBAC.HRPosition>();
        }
        ...
        public static IList<T> DataSetToEntityList<T>(DataSet p_DataSet, int p_TableIndex)
        {
            ...
            for (int j = 0; j < p_Data.Rows.Count; j++)
            {
                ...
                {
                    if (pi.Name == "Id" && p_Data.Rows[j]["PositionID"] != DBNull.Value)
                    {
                        pi.SetValue(_t, p_Data.Rows[j]["PositionID"], null);
                    }
