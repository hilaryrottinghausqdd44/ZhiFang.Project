...
{
     public class HRDeptIdentityDao : IDHRDeptIdentity
    {
        ...
        public DataSet GetAllList()
        {
            ...
            strSql.Append("select * from HR_DeptIdentity");
            return DbHelperSQL.ExecuteDataSet(strSql.ToString());
        }
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(*) FROM HR_DeptIdentity");
        ...
        public List<ZhiFang.Entity.RBAC.HRDeptIdentity> GetAllObjectList()
        {
            DataSet ds = new DataSet();
            ds = GetAllList();
            IList<ZhiFang.Entity.RBAC.HRDeptIdentity> deptlist = new List<Entity.RBAC.HRDeptIdentity>();
            if (ds == null || ds.Tables == null || ds.Tables.Count <= 0 || ds.Tables[0].Rows.Count <= 0)
            {
                return new List<Entity.RBAC.HRDeptIdentity>();
            }
            deptlist = DataSetToEntityList<Entity.RBAC.HRDeptIdentity>(ds, 0);
            return deptlist.ToList<ZhiFang.Entity.RBAC.HRDeptIdentity>();
        }
        ...
        public static IList<T> DataSetToEntityList<T>(DataSet p_DataSet, int p_TableIndex)
        {
            ...
            for (int j = 0; j < p_Data.Rows.Count; j++)
            {
                ...
                {
                    if (pi.Name == "Id" && p_Data.Rows[j]["DeptIdenID"] != DBNull.Value)
                    {
                        pi.SetValue(_t, p_Data.Rows[j]["DeptIdenID"], null);
                    }
