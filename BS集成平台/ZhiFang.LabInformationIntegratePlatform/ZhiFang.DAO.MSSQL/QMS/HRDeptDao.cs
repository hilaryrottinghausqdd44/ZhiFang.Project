...
{
     public class HRDeptDao: IDHRDept
    {
        ...
        public DataSet GetAllList()
        {
            ...
            strSql.Append("select * from HR_Dept");
            return DbHelperSQL.ExecuteDataSet(strSql.ToString());
        }
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(*) FROM HR_Dept");
        ...
        public List<ZhiFang.Entity.RBAC.HRDept> GetAllObjectList()
        {
            ...
            ds = GetAllList();
            IList<ZhiFang.Entity.RBAC.HRDept> deptlist = new List<Entity.RBAC.HRDept>();
            if (ds == null || ds.Tables == null || ds.Tables.Count <= 0 || ds.Tables[0].Rows.Count <= 0)
            {
                return new List<Entity.RBAC.HRDept>();
            }
            deptlist = DataSetToEntityList<Entity.RBAC.HRDept>(ds, 0);
            return deptlist.ToList<ZhiFang.Entity.RBAC.HRDept>();
        }
        ...
        public static IList<T> DataSetToEntityList<T>(DataSet p_DataSet, int p_TableIndex)
        {
            ...
            for (int j = 0; j < p_Data.Rows.Count; j++)
            {
                ...
                {
                    if (pi.Name == "Id" && p_Data.Rows[j]["DeptID"] != DBNull.Value)
                    {
                        pi.SetValue(_t, p_Data.Rows[j]["DeptID"], null);
                    }
