...
{
     public class RBACUserDao : IDRBACUser
    {
        ...
        public DataSet GetAllList()
        {
            ...
            strSql.Append("select * from RBAC_User");
            return DbHelperSQL.ExecuteDataSet(strSql.ToString());
        }
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(*) FROM RBAC_User");
        ...
        public List<ZhiFang.Entity.RBAC.RBACUser> GetAllObjectList()
        {
            DataSet ds = new DataSet();
            ds = GetAllList();
            IList<ZhiFang.Entity.RBAC.RBACUser> deptlist = new List<Entity.RBAC.RBACUser>();
            if (ds == null || ds.Tables == null || ds.Tables.Count <= 0 || ds.Tables[0].Rows.Count <= 0)
            {
                return new List<Entity.RBAC.RBACUser>();
            }
            deptlist = DataSetToEntityList<Entity.RBAC.RBACUser>(ds, 0);
            return deptlist.ToList<ZhiFang.Entity.RBAC.RBACUser>();
        }
        ...
        public static IList<T> DataSetToEntityList<T>(DataSet p_DataSet, int p_TableIndex)
        {
            ...
            for (int j = 0; j < p_Data.Rows.Count; j++)
            {
                ...
                {
                    if (pi.Name == "Id" && p_Data.Rows[j]["UserID"] != DBNull.Value)
                    {
                        pi.SetValue(_t, p_Data.Rows[j]["UserID"], null);
                    }
