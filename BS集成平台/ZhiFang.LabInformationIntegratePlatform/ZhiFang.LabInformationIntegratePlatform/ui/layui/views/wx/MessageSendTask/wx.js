...
layui.extend({
		...
		tableObj.table.render({
			...
			cols: [
				[{ type: 'checkbox' },{
					...
					{ title: '操作', width: 160, align: 'center', toolbar: '#tableOperation' }
				]
			],
			page: true,
			limit: 50,
	...
	tableObj.addClick = function () {
		...
		layer.open({
			...
			yes: function (index, layero) {
				...
				entity["AttachmentName"] = tableObj.uploadFileName;
				entity["AttachmentURL"] = tableObj.uploadFileURL;
				entity["AttachmentSize"] = tableObj.uploadFileSize;
	...
	tableObj.onSaveClick = function (data, type, index) {
		...
		uxutil.server.ajax(configs, function (data) {
			...
			tableObj.uploadFileName = "";
			tableObj.uploadFileURL = "";
			tableObj.uploadFileSize = "";
		});
	};
	//指定允许上传的文件类型
	upload.render({
		...
		, done: function (res) {
			if (res.success) {
				...
				tableObj.uploadFileName = fileinfo[0];
				tableObj.uploadFileURL = fileinfo[1];
				tableObj.uploadFileSize = fileinfo[2];
				$("#uploadname").text(tableObj.uploadFileName);
			} else {
	...
	tableObj.editClick = function (data) {
		...
		layer.open({
			...
			yes: function (index, layero) {
				var entity = {};
				if (tableObj.uploadFileName) {
					entity["AttachmentName"] = tableObj.uploadFileName;
				}
				if (tableObj.uploadFileURL) {
					entity["AttachmentURL"] = tableObj.uploadFileURL;
				}
				if (tableObj.uploadFileSize) {
					entity["AttachmentSize"] = tableObj.uploadFileSize;
				}
				entity["TaskName"] = $("#TaskName").val();
				entity["HospitalID"] = $("#BHospital").val() ? $("#BHospital").val() : 0;
