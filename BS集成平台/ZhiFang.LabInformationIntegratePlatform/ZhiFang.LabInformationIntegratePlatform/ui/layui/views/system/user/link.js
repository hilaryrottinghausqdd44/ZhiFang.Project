layui.extend({
	...
	//删除关系
	var DEL_LINK_URL = uxutil.path.ROOT + '/ServerWCF/RBACService.svc/RBAC_UDTO_DelHREmpIdentity';
	
	//人员枚举码
	var EMP_ENUM_CODE = 'EmpSystemType';
	...
	var table2_Ind = uxtable.render({
		...
		cols: [[
			...
			{field:'HREmpIdentity_Id',width:180,title:'ID',hide:true},
			{field:'HREmpIdentity_HREmployee_CName',width:150,title:'人员名称'},
			{field:'HREmpIdentity_HREmployee_StandCode',width:150,title:'标准代码'},
			{field:'HREmpIdentity_HREmployee_DeveCode',width:150,title:'开发商代码'}
		]],
		loading:false,
		limit:1000,
		done:function(res,curr,count){
	...
	//新增关系
	function onAddClick(){
		var url = 'checkgrid.html?t=' + new Date().getTime() +
	...
	function onDelClick(){
		...
		if(len > 0){
			layer.confirm('确定需要删除吗？', function(index){
				var ids = [];
				for(var i=0;i<len;i++){
					ids.push(data[i].HREmpIdentity_Id);
				}
				DelErrorList = [];
				delOneInfo(ids,0);
			});
	...
	function onLoadTypeList(callback){
		...
		url += '?classnamespace=ZhiFang.Entity.' + enumName + '&classname=' + EMP_ENUM_CODE;
	...
	function onLoadLinkList(callback){
		...
		url += "?isPlanish=true&fields=HREmpIdentity_Id,HREmpIdentity_HREmployee_CName,HREmpIdentity_HREmployee_StandCode,HREmpIdentity_HREmployee_DeveCode" +
		"&where=hrempidentity.TSysCode='" + CHECKED_TSYS_CODE + "'" +
		" and hrempidentity.SystemCode='" + CHECKED_SYSTEM_CODE + "'";
