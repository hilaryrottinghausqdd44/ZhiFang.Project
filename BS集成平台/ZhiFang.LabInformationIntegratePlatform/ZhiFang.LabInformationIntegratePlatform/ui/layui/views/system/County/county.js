...
layui.extend({
	...
	var tableObj = {
        ...
        current: null,
        delIndex: null,
        addUrl: uxutil.path.ROOT +'/ServerWCF/SingleTableService.svc/ST_UDTO_AddBCounty',
        selectUrl: uxutil.path.ROOT + '/ServerWCF/SingleTableService.svc/ST_UDTO_SearchBCountyByHQL?isPlanish=true&fields=BCounty_BCity_Name,' + getStoreFields(),
        updateUrl: uxutil.path.ROOT +'/ServerWCF/SingleTableService.svc/ST_UDTO_UpdateBCountyByField',
        delUrl: uxutil.path.ROOT +'/ServerWCF/SingleTableService.svc/ST_UDTO_DelBCounty',
        checkRowData: [], //选中数据
        refresh: function () {
        ...
        reset: function () { //重置
            if (formObj.type == 'edit') {
                if (tableObj.checkRowData.length > 0) {
                    ...
                    console.log(tableObj.checkRowData[tableObj.checkRowData.length - 1]);
                    if (tableObj.checkRowData[tableObj.checkRowData.length - 1].BCounty_IsUse == "true") {
    ...
    function tableRender() {
        tableObj.table.render({
            ...
            cols: [
                ...
                {
                    field: 'BCounty_Shortcode',
                    title: '快捷码',
                    minWidth: 130,
                    //sort: true
                },
                {
                    field: 'BCounty_PinYinZiTou',
                    title: '汉字拼音字头',
                    minWidth: 130,
                    //sort: true
                },
                {
                    field: 'BCounty_BCity_Name',
                    title: '城市名称',
                    //sort: true,
                    minWidth: 100
                },
                {
                    field: 'BCounty_Comment',
                    title: '备注',
                    //sort: true,
                    minWidth: 70
                },
                {
                    field: 'BCounty_BCity_Id',
                    title: '城市ID',
                    minWidth: 100,
                    //sort: true,
                    hide:true
    ...
    tableObj.table.on('sort(table)', function (obj) {
        ...
        if (val != "") {
            if (url.indexOf("where") != -1) {
                ...
                url = encodeURI(url + "&where=Name like '%" + val + "%' or SName like '%" + val  + "%' or Shortcode like '%" + val + "%' or PinYinZiTou like '%" + val + "%'");
            }
        }
        tableObj.table.reload('table', {
            initSort: obj, //记录初始排序，如果不设的话，将无法标记表头的排序状态
    ...
    function Citylist() {
        ...
        var url =  uxutil.path.ROOT + '/ServerWCF/SingleTableService.svc/ST_UDTO_SearchBCityByHQL?isPlanish=true' + '&where=IsUse=1' +
            '&fields=BCity_Name,BCity_Id,DataTimeStamp';
        uxutil.server.ajax({
            url: url
        }, function (data) {
            if (data) {
