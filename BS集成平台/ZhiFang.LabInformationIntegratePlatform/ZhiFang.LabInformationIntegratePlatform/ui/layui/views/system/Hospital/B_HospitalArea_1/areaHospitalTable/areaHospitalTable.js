...
layui.extend({
    ...
    areaHospitalTable.render = function (options,where) {
        var me = this;
        areaHospitalTable.AreaId=where;
        var inst = new Class(options);
        if (areaHospitalTable.AreaId) {
            inst.config.defaultWhere = "AreaId='" + areaHospitalTable.AreaId + "'";
    ...
    Class.pt.iniListeners = function () {
		...
		table.on('toolbar(areaHospitalTable)', function(obj) {
            if (!areaHospitalTable.AreaId) {
                layer.msg("请保存区域信息！", { icon: 5, anim: 6 });
                return;
            }
			switch(obj.event) {
                case 'add':
		            ...
		            parent.layer.open({
		                ...
		                maxmin: false,
		                title: '区域新增医院',
                        content: uxutil.path.ROOT + '/ui/layui/views/system/Hospital/B_HospitalArea/areaHospitalForm/areaHospitalForm.html?AreaId=' + areaHospitalTable.AreaId +"&formType=add",
		                cancel: function (index, layero) {
		                    flag = true;
		                },
		                success: function (layero, index) {
		                    var body = parent.layer.getChildFrame('body', index);//这里是获取打开的窗口元素
                            body.find('#AreaID').val(areaHospitalTable.AreaId);
		                },
		                end: function () {
                            if (flag) return;
                            if (areaHospitalTable.AreaId) {
                                me.config.defaultWhere = "AreaId='" + areaHospitalTable.AreaId + "'";
                            }
		 	...
		 	if(data.elem.checked){
			 	 var loadIndex = layer.load();//开启加载层
			 		$.ajax({
						type:'get',
						dataType:'json',
                            url: uxutil.path.ROOT + '/ServerWCF/LIIPCommonService.svc/ST_UDTO_SearchBHospitalAreaSonByID?id=' + areaHospitalTable.AreaId,
		                success: function (res) {
		                    layer.close(loadIndex);//关闭加载层
							if(res.success){
