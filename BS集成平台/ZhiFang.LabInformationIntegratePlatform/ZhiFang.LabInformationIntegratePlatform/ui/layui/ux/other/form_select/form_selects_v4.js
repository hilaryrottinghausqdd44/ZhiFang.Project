...
(function(layui, window, factory) {
	...
	//支持拼音字头过滤 Jcall 2021-11-05
	var uxpinyin = null;
	layui.extend({
		uxpinyin:'ux/pinyin'
	}).define(['uxpinyin'], function(exports) {
		...
		FormSelects = function(options) {
			this.config = {
				...
				filter: (id, inputVal, val, isDisabled) => {
					...
					if(
						//拼音从头到尾地匹配，全拼写法
						uxpinyin.ConvertPinyin(text).substring(0, len) === inputVal || 
						uxpinyin.ConvertPinyin(text).substring(0, len) === inputVal.toLowerCase() || 
						//首字母从头到尾匹配，拼音字头
						uxpinyin.makePy(text)[0].toLowerCase().substring(0, len) === inputVal || 
						uxpinyin.makePy(text)[0].toLowerCase().substring(0, len) === inputVal.toLowerCase() || 
						//内容模糊匹配
						text.toLowerCase().indexOf(inputVal) > -1
					){
						return false;
					}
					return true;
