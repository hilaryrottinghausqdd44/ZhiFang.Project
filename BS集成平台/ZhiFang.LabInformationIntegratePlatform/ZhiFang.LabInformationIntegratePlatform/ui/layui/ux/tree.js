...
layui.define('form', function(exports){
  ...
  Class.prototype.config = {
    ...
    ,checkParents: true //是否开始父节点勾选模式
    ,checkChildren: true //是否开启子节点勾选模式
  };
  ...
  Class.prototype.setCheckbox = function(elem, item, elemCheckbox){
    ...
    //开启子节点勾选模式&&同步子节点选中状态
    if(options.checkChildren && (typeof item.children === 'object' || elem.find('.'+ELEM_PACK)[0])){
    ...
    //开始父节点勾选模式
    options.checkParents && setParentsChecked(elem);
  ...
  Class.prototype.getChecked = function(){
    ...
    //遍历节点
    var eachNodes = function(checkId, data, checkNode){
	    layui.each(data, function(index, item){
	      if(item.id == checkId){
	        ...
	        if(options.checkParents && item.children){
	          cloneItem.children = [];
	          eachNodes(item.children, cloneItem.children);
	        }
	        return true
	      }else{
	      	if(!options.checkParents && !options.checkChildren && item.children && item.children.length > 0){
	          return eachNodes(checkId, item.children, checkNode);
	        }
	      }
	    });
    };
	
    layui.each(checkIds, function(index, item){
      eachNodes(item, $.extend({}, options.data), checkData);
    });
    return checkData;
  };
