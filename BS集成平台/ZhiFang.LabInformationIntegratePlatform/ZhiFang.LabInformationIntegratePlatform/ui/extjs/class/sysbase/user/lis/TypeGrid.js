 ...
 * @version 2019-10-15
 */
Ext.define('Shell.class.sysbase.user.lis.TypeGrid', {
	...
	getSystemListUrl:'/ServerWCF/LIIPService.svc/ST_UDTO_SearchIntergrateSystemSetByHQL',
  	/**获取数据服务路径*/
	selectUrl:'/ServerWCF/RBACService.svc/RBAC_UDTO_SearchHREmpIdentityByHQL',
	/**新增服务地址*/
	addUrl:'/ServerWCF/RBACService.svc/RBAC_UDTO_AddHREmpIdentity',
	/**删除数据服务路径*/
	delUrl:'/ServerWCF/RBACService.svc/RBAC_UDTO_DelHREmpIdentity',
	...
	createGridColumns:function(){
		...
		var columns = [{
			...
			text:'员工身份ID',dataIndex:'TSysID',width:150,
			hidden:true,hideable:false
		},{
			text:'员工身份名称',dataIndex:'TSysName',width:150,
			sortable:false,menuDisabled:true,defaultRenderer:true
		},{
			text:'身份类型编码',dataIndex:'TSysCode',width:100,
			sortable:false,menuDisabled:true,defaultRenderer:true
		},{
	...
	onLoadTypeList:function(callback){
		...
		url += '?classnamespace=&classname=' + me.EMP_ENUM_CODE;
		...
		JShell.Server.get(url,function(data){
			if(data.success){
				...
				//JShell.Msg.error(data.msg);
				var error = me.errorFormat.replace(/{msg}/,'本系统没有配员工身份枚举！');
				me.getView().update(error);
			}
		});
	},
	...
	onLoadLinkList:function(callback){
		...
		url += "?fields=HREmpIdentity_Id,HREmpIdentity_TSysCode" +
		"&where=hrempidentity.HREmployee.Id=" + me.EmpId +
		" and hrempidentity.SystemCode='" + me.SYSTEM_CODE + "'";
	...
	onAddOne:function(record){
		...
		var entity = {
			IsUse:true,
			TSysID:record.get('TSysID'),
			TSysCode:record.get('TSysCode'),
			TSysName:record.get('TSysName'),
			SystemID:me.SYSTEM_ID,
			SystemCode:me.SYSTEM_CODE,
			SystemName:me.SYSTEM_NAME,
			HREmployee:{Id:me.EmpId,DataTimeStamp:[0,0,0,0,0,0,0,0]}
		};
