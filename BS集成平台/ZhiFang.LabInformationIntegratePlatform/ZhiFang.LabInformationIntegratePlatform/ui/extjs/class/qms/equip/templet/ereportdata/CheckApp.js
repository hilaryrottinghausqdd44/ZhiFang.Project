 ...
 * @version 2016-08-24
 */
Ext.define('Shell.class.qms.equip.templet.ereportdata.CheckApp', {
	...
	getPdfWhereParams: function(list) {
		var me = this,
			whereParams = "",
			params = [];
		var CheckType  =  list.EReportData_ETemplet_CheckType;
		var Sysdate = list.EReportData_ReportDate;
	    var ReportDate = list.ReportDate;

		//按天审核
		if(CheckType=='1' && ReportDate){
			var RDate =JcallShell.Date.toString(ReportDate,true);
			if(!RDate)return;
			params.push("&endDate=" + RDate);
			params.push("&beginDate=" + RDate);
		}else {
			var b = Sysdate.split("/");
			var curDate = b[0] + '-' + b[1] + '-01';
			var endDate = JcallShell.Date.getMonthLastDate(b[0], b[1], true);
			params.push("&endDate=" + endDate);
			params.push("&beginDate=" + curDate);
		}
		if(params.length > 0) {
	...
	SaveClick: function() {
		...
		var endDate = JcallShell.Date.getMonthLastDate(b[0], b[1], true);
		me.getCheckReport(TempletID, curDate, endDate);
	},
	/**审核质量记录*/
