 ...
 * @version 2016-11-22
 */
Ext.define('Shell.class.qms.file.help.show.PanelForId', {
	...
	autoScroll: true,
	/**获取数据服务路径*/
	selectUrl: '/ServerWCF/CommonService.svc/QMS_UDTO_SearchFFileByHQL?isPlanish=true',
	/**服务附件服务路径*/
	...
	loadData: function(callback) {
		...
		var fields = "FFile_Id,FFile_Title,FFile_Memo,FFile_Content,FFile_Keyword,FFile_No,FFile_Summary,FFile_DataAddTime";
		url += "&start=0&page=1&limit=50&fields=" + fields + '&sort=[{"property": "FFile_DataAddTime","direction": "DESC"}]';
		var where = "";
		if(me.isSearchForId == true) {
			where = "ffile.IsUse=1 and ffile.Id=" + me.PK;
		} else {
			var no =me.ModuleId;
			//帮助文档的模块子序号
			...
			where = "ffile.IsUse=1 and ffile.No='" + no + "'";
		}
		if(where != "")
			where = where + " and ffile.Type=" + me.FTYPE;
		url += '&where=' + where;
		JShell.Server.get(url, function(data) {
			...
			if(data.success) {
				...
				if(info != null) {
					...
					me.ContentHtml = "<div class='alert alert-warning' style='margin:40px 20px;text-align:center;padding-top:40px;padding-bottom:40px;'><h4>文档编码: " + me.getModuleNo() + "</h4><br>该功能没有维护帮助文档，请联系管理人员添加!</div>";

				}
				if(ffileId != null && ffileId != undefined) {
				...
				me.ContentHtml = "<div class='alert alert-warning' style='margin:40px 20px;text-align:center;padding-top:40px;padding-bottom:40px;'><h4>文档编码: " + me.getModuleNo() + "</h4><br>该功能获取帮助文档信息出错!<br>" + data.msg + '<br>请联系管理人员</div>';
				callback(callbackData);
			}
		}, false);
	},
	...
	loadAttachmentData: function(ffileId, callback) {
		...
		var where = 'ffileattachment.IsUse=1 and ffileattachment.FFile.Id=' + ffileId;
		url += '&where=' + where;
		JShell.Server.get(url, function(data) {
			if(data.success) {
				...
				var errorInfo = '<b style="color:red">获取附件信息出错:<br>' + data.msg + '</b>'
