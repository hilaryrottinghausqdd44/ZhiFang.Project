	...
	createcombo: function() {
		...
		switch(values.FFileCopyUser_Type) {
			...
			case '4': //按人员
				...
				break;
			case '5': //按区域
				com = 'Shell.class.qms.file.copyuser.AreaCheckRoleGrid';
				//'Shell.class.qms.file.copyuser.CheckEmployeeRoleGrid';
				break;
			default:
				break;
...
//										}
									...
									case '4':
										...
										break;
									case '5':
										index = p.store.findExact('BHospitalArea_Id', value);
										if (index != -1) {
											p.getSelectionModel().select(index, true, false)
										}
										break;
	...
	onParentModuleAccept: function(record) {
		...
		var HospitalAreaID = me.getComponent('FFileCopyUser_BHospitalArea_Id');
		var Type = me.getComponent('FFileCopyUser_Type');
		me.IdStrValue = '';
		me.DisplayValue = '';
		me.FileCopyUserArr = {};
...
//						Id: record[i].get('HRDept_Id')
				...
				case '4': //员工
					...
					break;
				case '5': //员工
					me.IdStrValue += ',' + record[i].get('BHospitalArea_Id');
					me.DisplayValue += ',' + record[i].get('BHospitalArea_Name');
					//RBACEmpRoles_HREmployee_Id,RBACEmpRoles_HREmployee_CName
					tempModel = {
						Id: record[i].get('BHospitalArea_Id')
					}
					tempObj = {
						"BHospitalArea": tempModel
					};
					me.valueArr.push(tempObj);
					break;
				default:
		...
		FileCopy.setValue(me.DisplayValue);
		HospitalAreaID.setValue(me.IdStrValue);
		FileCopy.setValue(me.DisplayValue);
	...
	createHideItems: function() {
		...
		items.push({
			...
			hidden: true,
			xtype: 'textfield',
			name: 'FFileCopyUser_BHospitalArea_Id',
			itemId: 'FFileCopyUser_BHospitalArea_Id'
		});
		items.push({
			fieldLabel: '区域',
			hidden: true,
			xtype: 'textfield',
			name: 'FFileCopyUser_BHospitalArea_Name',
			itemId: 'FFileCopyUser_BHospitalArea_Name'
		});
		var names = [
			'FFileCopyUser_HRDept', 'FFileCopyUser_RBACRole', 'FFileCopyUser_HREmployee','FFileCopyUser_BHospitalArea'
		];
		for(var i in names) {
				...
				if(value != '' && value != null) {
					...
					for(var i = 0; i < value.length; i++) {
						switch(me.defaultTypeValue) {
							...
							case '5': //按人员
								FileCopy.setVisible(true);
								me.IdStrValue += ',' + value[i][me.objectEName + '_BHospitalArea_Id'];
								me.DisplayValue += ',' + value[i][me.objectEName + '_BHospitalArea_Name'];
								tempModel = {
									Id: value[i][me.objectEName + '_BHospitalArea_Id']
								};
								tempObj = {
									"BHospitalArea": tempModel
								};
