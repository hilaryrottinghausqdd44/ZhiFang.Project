...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util'], function(exports){
  ...
  Class.prototype.reload = function(options){
    ...
    that.config = $.extend({}, that.config, options);
    that.render();
    //Jcall添加-数据重载监听事件-20190716
    if(that.config.listeners && that.config.listeners.reload && typeof that.config.listeners.reload == 'function'){
    	that.config.listeners.reload(that);
    }
  };
