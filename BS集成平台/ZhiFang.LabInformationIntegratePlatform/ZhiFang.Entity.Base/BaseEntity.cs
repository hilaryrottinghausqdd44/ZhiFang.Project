                        ...
                        try
                        {
                            ...
                            if (IsLabFlag != null && IsLabFlag.Trim() == "1")
                            {
                                ...
                                else
                                {
                                    ...
                                    throw new Exception("未找到cookie里的LabID（通常是调用登录服务前还未写cookie）.数据库中取出的LabID");
                                    return _labID;//未找到cookie里的LabID;
                                }
                            }
                            else
                            {
                                ZhiFang.Common.Log.Log.Debug("未找到cookie里的Lab标记");
                                throw new Exception("未找到cookie里的Lab标记");
                                return _labID;
                            }
