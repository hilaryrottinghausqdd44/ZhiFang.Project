    ...
    public abstract class BaseDaoNHB<T, T1> : HibernateDaoSupport, IDBaseDao<T, T1> where T : BaseEntity
    {
        ...
        public void GetDataRowRoleHQLString()
        {
            ...
            if (!string.IsNullOrEmpty(ZhiFang.Common.Public.ConfigHelper.GetConfigString("IsMultiLabFlag")) && Common.Public.ConfigHelper.GetConfigString("IsMultiLabFlag").Trim() == "1")
            {
                ...
                {
                    if (typeof(T).ToString().IndexOf("ZhiFang.Entity.RBAC.HR") == 0 || typeof(T).ToString().IndexOf("ZhiFang.Entity.RBAC.RBAC") == 0)//判断组织机构、权限系统的实体类
                    {
                        if (!string.IsNullOrEmpty(ZhiFang.Common.Public.ConfigHelper.GetConfigString("RBACIsMultiLabFlag")) && Common.Public.ConfigHelper.GetConfigString("RBACIsMultiLabFlag").Trim() == "0")//不进行组织机构、权限系统的隔离，所有人公用权限系统，一般由最高级机构的管理员进行管理。
                        {
                            return;
                        }
                    }
