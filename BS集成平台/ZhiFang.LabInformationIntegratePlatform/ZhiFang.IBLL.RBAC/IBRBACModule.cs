...
{	
	public interface IBRBACModule : ZhiFang.IBLL.Base.IBGenericManager<ZhiFang.Entity.RBAC.RBACModule>
	{
        ...
        BaseResultTree<RBACModule> SearchRBACModuleToTree(string where, bool IsListTree);
        bool BModuleGetAndAdd(long bModuleId, string bModule, ref long id);

        BaseResultTree SearchModuleTreeTwoStageByHREmpID(long longHREmpID, long ModuleID);

        BaseResultTree SearchModuleTreeTwoStageByHREmpID(long longHREmpID, string ModuleCode);
        List<Ttree<RBACModule>> SearchRBACModule_IncludePModule(string where, bool IsListTree);
    } 
}
