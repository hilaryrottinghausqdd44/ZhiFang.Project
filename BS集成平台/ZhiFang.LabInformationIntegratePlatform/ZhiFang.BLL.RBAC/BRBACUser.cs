...
namespace ZhiFang.BLL.RBAC
{
    ...
    public class BRBACUser : Base.BaseBLL<RBACUser>, IBRBACUser
    {
        ...
        public string RJ_ResetAccountPassword(long id)
        {
            ...
            entity.PWD = LIIP.Common.SecurityHelp.MD5Encrypt(RJ_GetRandomNumber(6), LIIP.Common.SecurityHelp.PWDMD5Key);
            this.Entity = entity;
            this.Edit();
        ...
        public bool SetPWDbyAccount(string account, string PWD)
        {
            string pwd = LIIP.Common.SecurityHelp.MD5Encrypt(PWD, LIIP.Common.SecurityHelp.PWDMD5Key);
            return DBDao.UpdateByHql(" update RBACUser set PWD='" + pwd + "' where  Account='" + account + "'") > 0;
        }

        public BaseResultDataValue RestRBACUserPWD()
        {
            BaseResultDataValue brdv = new BaseResultDataValue();
            var rbacuserlist=DBDao.GetListByHQL(" 1=1 ");
            if (rbacuserlist != null)
            {
                foreach (var a in rbacuserlist)
                {
                    if (a.PWD == null || a.PWD.Trim() == "")
                    {
                        string pwd = LIIP.Common.SecurityHelp.MD5Encrypt(a.Account, LIIP.Common.SecurityHelp.PWDMD5Key);
                        DBDao.UpdateByHql(" update RBACUser set PWD='" + pwd + "' where  Id=" + a.Id);
                    }
                }
            }
        ...
        {
            BaseResultDataValue brdv = new BaseResultDataValue();
            var rbacuserlist = DBDao.GetListByHQL(" Account='"+account+"' ");
            if (rbacuserlist != null && rbacuserlist.Count > 0)
            {
                brdv.success = true;
                brdv.ResultDataValue = rbacuserlist[0].HREmployee.Id.ToString()+','+ rbacuserlist[0].HREmployee.CName;
            }
            else
            {
                brdv.success = false;
