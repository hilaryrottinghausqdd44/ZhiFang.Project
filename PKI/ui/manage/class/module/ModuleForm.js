Ext.ns('Ext.manage');
Ext.define('Ext.manage.module.ModuleForm',{
	...
	createItems:function(type){
		...
		var hideFields = [
			...
			{xtype:'textfield',name:'BTDAppComponents_Id',fieldLabel:'对应应用ID',hidden:true},
			{xtype:'textfield',name:'BTDAppComponents_DataTimeStamp',fieldLabel:'对应应用时间戳',hidden:true}
		];
		...
		var showFields = [
			...
			{xtype:'textfield',name:'Url',fieldLabel:'入口地址',x:10,y:105,width:450,labelWidth:55,readOnly:false},
	...
	addModule:function(saveType){
		...
		if(id != '-1'){
			...
			//需要更新的字段
			obj.fields = "Id,ParentID,DataTimeStamp,PicFile,CName,EName,PinYinZiTou,Shortcode,SName,DispOrder,ModuleType,UseCode,IsUse,StandCode,DeveCode,Url,Para,Comment";
			if(params.BTDAppComponents_Id && params.BTDAppComponents_Id != ''){obj.fields += ",BTDAppComponents_Id";}
		}
		//回调函数
	...
	ModuleTypeChange:function(newValue){
		...
		var BTDAppComponents_Id = me.getForm().findField('BTDAppComponents_Id');
		var BTDAppComponents_DataTimeStamp = me.getForm().findField('BTDAppComponents_DataTimeStamp');
	...
	updateFileToServer:function(callback){
		...
		form.getForm().submit({
			...
			failure:function(form,action){
				if(action.result){
					alertError('处理错误！错误信息:' + action.result.errorInfo);
				}else{
					alertError('上传图片文件失败！错误信息:'+ action.result.ErrorInfo);
				}
	...
	getModuleInfoFromServer:function(id,callback){
		var me = this;
		var str = "Id,LabID,ParentID,LevelNum,TreeCatalog,Owner,DataTimeStamp,PrimaryKey,PicFile,BTDAppComponents_Id," + 
	...
	load:function(obj){
		...
		if(typeof obj == 'number' || typeof obj == 'string'){
			...
			alertError("参数格式错误!正确的参数格式为:1或者{Id:1}");
		}
