 ...
 */
Ext.ns('Ext.build');
Ext.define('Ext.build.ParserPanel',{
	extend:'Ext.grid.Panel',
	alias:'widget.parserpanel',
	title:'应用列表',
	/**
	 * 批量更新的类型
	 * @type String
	...
	parserObj:{
		app:{
			AppType:'3',
			parserList:[
				//{text:'应用101版本',value:'Ext.build.ParserApp101'},
            	{text:'应用100版本',value:'Ext.build.ParserApp'}
            ]
		},
		form:{
			AppType:'2',
			parserList:[
            	{text:'表单100版本',value:'Ext.build.ParserForm'},
            	{text:'表单101版本',value:'Ext.build.ParserForm101'}
            ]
		},
		list:{
			AppType:'1',
			parserList:[
            	{text:'列表100版本',value:'Ext.build.ParserList'},
            	{text:'列表101版本',value:'Ext.build.ParserList101'},
            	{text:'列表102版本',value:'Ext.build.ParserList102'},
            	{text:'列表103版本[RABC]',value:'Ext.build.ParserList103'}
            ]
		}
	},
	/**
	...
	initComponent:function(){
		var me = this;
		if(me.type && me.type != ""){
    		me.internalWhere= "btdappcomponents.BuildType=1 and btdappcomponents.AppType=" + me.parserObj[me.type]['AppType'];
    	}
		//初始化视图
		me.initView();
		//监听
	...
	createList:function(){
		...
		me.columns = [{
			...
			dataIndex:me.fieldsObj.EName,text:'版本信息',width:150
		},{
	...
	createDockedItems:function(){
		...
		toolbar.items.push({
			xtype:'button',text:'批量更新',itemId:'UpdateListClassCodeButton',
			iconCls:'build-button-save',disbaled:!(me.type && me.type != ''),
			handler:function(button){
				var records = me.getSelectionModel().getSelection();
				if(records.length > 0){
					var parser = button.ownerCt.getComponent('ParserList').getValue();
					var parserList = Ext.create(parser);
					...
					var createCallback = function(id){
						var f = function(text){
							...
							var num = getCount();
							if(result.success){
								var value = num + '应用【' + id + '】保存成功';
								win.add({xtype:'label',text:value,style:'font-weight:bold;color:green;'});
							}else{
								var value = num + '应用【' + id + '】保存失败！失败原因：【' + result.ErrorInfo + '】';
								win.add({xtype:'label',text:value,style:'font-weight:bold;color:red;'});
							}
							if(num < length){
								var id1 = records[num].get(me.fieldsObj.AppComID);
								var callback = createCallback(id1);
								parserList.updateAppInfoById(id1,callback);
							}
						};
						return f;
					};
					var id1 = records[0].get(me.fieldsObj.AppComID);
					var callback = createCallback(id1);
					parserList.updateAppInfoById(id1,callback);
				}
			}
		...
		toolbar.items.push({
			xtype:'combobox',itemId:'ParserList',
        	width:110,value:me.parserObj[me.type]['parserList'][0]['value'] || '',
            mode:'local',
            editable:false, 
            displayField:'text',
            valueField:'value',
            store:new Ext.data.Store({
            	fields:['text','value'],
            	data:me.parserObj[me.type]['parserList']
            })
		});
        //查询
        toolbar.items.push('->');
    ...
    getInfoByIdFormServer:function(id,callback){
   		var me = this;
   		var url = me.getAppInfoServerUrl + '?isPlanish=false&id=' + id + "&fields=" + me.fieldsObj.ClassCode;
   		Ext.Ajax.defaultPostHeader = 'application/json';
   		Ext.Ajax.request({
			url:url,
			method:'GET',
	...
	openAppShowWin:function(title,ClassCode,id){
		...
		if(win.html && win.html != ""){
			win.html = win.html.replace(/@@/g,"'");
		}
    	//解决chrome浏览器的滚动条问题
...
//			{name:me.fieldsObj.LabID,type:'string'},//实验室ID
    ...
    search:function(){
		...
		if(value && value != ""){
			...
			where += "btdappcomponents.CName like '%" + value + "%' or btdappcomponents.ModuleOperCode like '%" + value + "%'";
			where += ")";
		}
