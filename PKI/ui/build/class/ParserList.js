...
Ext.define('Ext.build.ParserList',{
	...
	getClassCode:function(){
		...
		"Ext.define('" + panelParams.appCode + "',{" + 
			...
			"defaultWhere:'" + (panelParams.defaultParams || "").replace(/\%/g,"%25").replace(/\'/g,"%27").replace(/@@/g,"%25").replace(/##/g,"%27") + "',";//默认hql,@@=%;##='
			//开启复选
	...
	createColumnsStr:function(){
		...
		for(var i in columnParams){
                    ...
                    colEditer="editor:new Ext.zhifangux.colorscombobox({mode:'local'}),"
               		...
               		if(columnType == "number"){
                  		...
                  		colEditer="editor:{allowBlank:true,xtype:'numberfield'" + format + "},";
               			...
               			var format = columnParams[i].format && columnParams[i].format != "" ? ",format:'" + columnParams[i].format + "'" : ",format:'Y-m-d'";
                  		colEditer="editor:Ext.create('Ext.zhifangux.DateField',{allowBlank:true" + format + "}),";
                    }else{
			...
			var col = 
			"{" + 
				"HeadFont:'" + columnParams[i].HeadFont + "'," + 
	            ...
	            if(columnParams[i].format && columnParams[i].format != ""){
	            	...
	            	if(columnParams[i].ColumnType == "date"){
		            	col += "format:'Y-m-d',";
		            }
	            }
	            ...
	            if(columnParams[i].Width){col += "width:" + columnParams[i].Width + ",";}
	            if(columnParams[i].IsLocked){col += "locked:" + columnParams[i].IsLocked + ",";}
	            if(!columnParams[i].CanSort){col += "sortable:false,";}
	            if(columnParams[i].IsHidden || columnParams[i].CannotSee){col += "hidden:true,";}
	            if(columnParams[i].CannotSee){col += "hideable:false,";}
	            if(colEditer != ""){col += colEditer;}//是否可以修改行或单元格
	            
				col += "align:'" + columnParams[i].AlignType + "'" + 
			"}";
	...
	createFunButStr:function(){
        ...
        if(panelParams['panelEditor1'] == 'column'){
			...
			"{" + 
	            ...
	            "handler:function(but){" + 
                    ...
                    "var editFields=[" + me.getEditFields() + "];" + //获取需要修改的字段                 
                    "editFields.push(editer_id);" + 
                    "var records=me.store.getModifiedRecords();" + //获取修改后的行记录    
                     	...
                     	"if(button=='yes'){" + 
                     		...
                     		"for(var i=0;i<records.length;i++){" + 
                            	...
                            	"for(var j=0;j<editFields.length;j++){" + 
                            		//"obj[editFields[j]]=records[i].get(editFields[j]);" + 
                            		"obj=records[i].getChanges();" +
                            		"obj[editer_id]=records[i].get(editer_id);" +
                            	"}" + 
                                "me.saveToTable(obj,callback);" + 
                            "}"+  
		...
		//5个自定义按钮
		if(panelParams['toolbar-default1-checkbox']){
			var value = 
			"{" + 
				"itemId:'default1'," + 
				"text:'" + panelParams['toolbar-default1-text'] + "'," + 
				"iconCls:'operateImg16'," + 
				"handler:function(but,e){" + 
					"but.ownerCt.ownerCt.fireEvent('toolbarDefault1Click',but);" + 
				"}" + 
			"}";
			arr.push({
				key:panelParams['toolbar-default1-number'],
				value:value
			});
		}
		if(panelParams['toolbar-default2-checkbox']){
			var value = 
			"{" + 
				"itemId:'default2'," + 
				"text:'" + panelParams['toolbar-default2-text'] + "'," + 
				"iconCls:'operateImg16'," + 
				"handler:function(but,e){" + 
					"but.ownerCt.ownerCt.fireEvent('toolbarDefault2Click',but);" + 
				"}" + 
			"}";
			arr.push({
				key:panelParams['toolbar-default2-number'],
				value:value
			});
		}
		if(panelParams['toolbar-default3-checkbox']){
			var value = 
			"{" + 
				"itemId:'default3'," + 
				"text:'" + panelParams['toolbar-default3-text'] + "'," + 
				"iconCls:'operateImg16'," + 
				"handler:function(but,e){" + 
					"but.ownerCt.ownerCt.fireEvent('toolbarDefault3Click',but);" + 
				"}" + 
			"}";
			arr.push({
				key:panelParams['toolbar-default3-number'],
				value:value
			});
		}
		if(panelParams['toolbar-default4-checkbox']){
			var value = 
			"{" + 
				"itemId:'default4'," + 
				"text:'" + panelParams['toolbar-default4-text'] + "'," + 
				"iconCls:'operateImg16'," + 
				"handler:function(but,e){" + 
					"but.ownerCt.ownerCt.fireEvent('toolbarDefault4Click',but);" + 
				"}" + 
			"}";
			arr.push({
				key:panelParams['toolbar-default4-number'],
				value:value
			});
		}
		if(panelParams['toolbar-default5-checkbox']){
			var value = 
			"{" + 
				"itemId:'default5'," + 
				"text:'" + panelParams['toolbar-default5-text'] + "'," + 
				"iconCls:'operateImg16'," + 
				"handler:function(but,e){" + 
					"but.ownerCt.ownerCt.fireEvent('toolbarDefault5Click',but);" + 
				"}" + 
			"}";
			arr.push({
				key:panelParams['toolbar-default5-number'],
				value:value
			});
		}
