 ...
 * @version 2016-05-13
 */
Ext.define('Shell.class.pki.dealer.contractprice.StepPriceGrid', {
	...
	onAddClick: function() {
		...
		var IsStepPrice = records[0].get("DContractPrice_IsStepPrice");
		switch (IsStepPrice) {
			case "是":
				IsStepPrice = "1";
				break;
		...
		if (IsStepPrice==="1") {
			...
			JShell.Msg.alert("只能选择是否阶梯价值为【是】的行新增");
		}
	...
	onDelClick: function() {
		...
		for (var i in records) {
			var IsStepPrice = records[i].get("DContractPrice_IsStepPrice");
			switch (IsStepPrice) {
				case "是":
		...
		if (delRecords.length == 0) {
			JShell.Msg.error("只能选择是否阶梯价值为【是】的行删除");
			return;
		}
		JShell.Msg.del(function(but) {
			...
			me.delErrorCount = 0;
			me.delCount = 0;
			me.delLength = delRecords.length;

			me.showMask(me.delText); //显示遮罩层
			for (var i in delRecords) {
				var id = delRecords[i].get(me.PKField);
				me.delOneById(i, id);
			}
		});
	},
	...
	openStepPriceForm: function(id) {
		...
		var config = {
			...
			formtype: 'add',
			title: me.titleForm,
			//			ContractPriceType:me.ContractPriceType,
			listeners: {
				save: function(win) {
					me.onSearch();
					win.close();
		...
		if (id) {
			...
			config.PK = id;
			config.editIsStepPrice=records[0].get('DContractPrice_IsStepPrice')== '1' ? false : true;
		} else {
		...
		JShell.Win.open('Shell.class.pki.dealer.contractprice.Form', config).show();
	...
	onSaveClick: function() {
		...
		for (var i = 0; i < len; i++) {
			var rec = records[i];
			me.updateOneBySampleCountAndPrice(rec);
		}
	},
	/**修改数量和价格*/
	updateOneBySampleCountAndPrice: function(record) {
		var me = this;
		var id = record.get("DContractPrice_Id");
		var count = record.get('DContractPrice_SampleCount');
		var price = record.get('DContractPrice_StepPrice');
		//		var isStepPrice = record.get('DContractPrice_IsStepPrice');
		//		switch (isStepPrice.toString()) {
		//			case "1":
		...
		var params = Ext.JSON.encode({
			...
			fields: 'Id,SampleCount,StepPrice,StepPriceMemo' //StepPriceMemo,ConfirmUser,ConfirmTime'
		});
		JShell.Server.post(url, params, function(data) {
			var record = me.store.findRecord("DContractPrice_Id", id);
			if (data.success) {
