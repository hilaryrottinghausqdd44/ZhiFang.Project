...
Ext.define('Ext.zhifangux.SortList', {
    extend:'Ext.panel.Panel',
    alias:'widget.sortlist',
    layout:'border',
    frame:true,
    title:'',
    //标题,默认值为''
    ...
    load:function(where) {
        ...
        if (leftGrid) {
            leftGrid.store.proxy.url = me.dataServerUrl + '&where=' + w;
            leftGrid.store.load();
        }
    },
    /**
    ...
    setmyGridField:function() {
        var me = this;
        if (me.modelField.length == 0 && me.valueField.length > 0) {
            me.modelField = [];
            for (var i = 0; i < me.valueField.length; i++) {
                var text = me.valueField[i].dataIndex;
                var objJson = {
                    name:text,
                    type:'auto'
                };
                me.modelField.push(objJson);
            }
        }
    },
    /**
   ...
   */
    ...
    */
    load:function(where) {
        ...
        grid.store.proxy.url = me.dataServerUrl + '&where=' + w;
        grid.store.load();
        grid.getStore().sort(me.listorder, 'asc');
    },
   /**
