...
{
    /// <summary>
    /// 质量指标分类类型-统计纬度按样本类型送检人分组
    /// </summary>
    class OfSampleTypeAndInspectorGroupBy : DimensionStrategy
    {
        public override IList<SPSAQualityIndicatorType> GetSPSAQualityIndicatorTypeOfGroupBy(IList<SPSAQualityIndicatorType> nfList, string qIndicatorType)
        {
            ...
            foreach (var group in groupBy)
            {
                ...
                entity.NurseSendCarrier = group.ElementAt(0).NurseSendCarrier;

                var tempList = nfList.Where(p => p.SampleTypeNo == group.Key.SampleTypeNo && p.NurseSendCarrier == group.Key.NurseSendCarrier);
