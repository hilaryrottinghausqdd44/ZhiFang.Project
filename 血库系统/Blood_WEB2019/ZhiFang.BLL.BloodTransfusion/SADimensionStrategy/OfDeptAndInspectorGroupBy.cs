...
{
    /// <summary>
    /// 质量指标分类类型-统计纬度按科室送检人分组
    /// </summary>
    class OfDeptAndInspectorGroupBy : DimensionStrategy
    {
        public override IList<SPSAQualityIndicatorType> GetSPSAQualityIndicatorTypeOfGroupBy(IList<SPSAQualityIndicatorType> nfList, string qIndicatorType)
        {
            ...
            foreach (var group in groupBy)
            {
                ...
                var tempList = nfList.Where(p => p.DeptNo == group.Key.DeptNo&& p.NurseSendCarrier == group.Key.NurseSendCarrier);
