...
{
    /// <summary>
    /// 质量指标分类类型图表数据处理-标本类型错误率
    /// </summary>
    class OfSampleTypeSADimension : QITypeOfEChartStrategy
    {
        public override BaseResultDataValue GetSAQualityIndicatorTypeOfEChart(IList<SPSAQualityIndicatorType> saList, string qIndicatorType, string sadimension)
        {
            ...
            {
                if (sadimension == SampleTypeSADimension.就诊类型.Key)
                {
                    axisDataValue = vo.SickTypeCName;
                }
                else if (sadimension == SampleTypeSADimension.就诊类型科室.Key)
                {
                    axisDataValue = vo.SickTypeCName + "_" + vo.DeptCName;
                }
                else if (sadimension == SampleTypeSADimension.就诊类型医生.Key)
                {
                    axisDataValue = vo.SickTypeCName + "_" + vo.DoctorCName;
                }
                else if (sadimension == SampleTypeSADimension.科室.Key)
                {
                    axisDataValue = vo.DeptCName;
                }
                else if (sadimension == SampleTypeSADimension.科室医生.Key)
                {
                    axisDataValue = vo.DeptCName + "_" + vo.DoctorCName;
                }
                else if (sadimension == SampleTypeSADimension.按年份.Key)
                {
                    axisDataValue = vo.Year;
                }
                else if (sadimension == SampleTypeSADimension.按季度.Key)
                {
                    axisDataValue = vo.Quarter;
                }
                else if (sadimension == SampleTypeSADimension.按月份.Key)
                {
        ...
        public override IList<SPSAQualityIndicatorType> GetSAQualityIndicatorTypeOfList(IList<SPSAQualityIndicatorType> nfList, string qitype, string sadimension, string startDate, string endDate)
        {
            ...
            {
                dimensionStrategy = new OfSickTypeAndDoctorGroupBy();
            }
            else if (sadimension == SampleTypeSADimension.科室医生.Key)
            {
                dimensionStrategy = new OfDeptAndDoctorGroupBy();
            }
            else if (sadimension == SampleTypeSADimension.按年份.Key)
            {
