...
{
    /// <summary>
    /// 质量指标分类类型图表数据处理-标本运输时间过长
    /// </summary>
    class OfSpecimenTransportTimeIsLong : QITypeOfEChartStrategy
    {
        public override BaseResultDataValue GetSAQualityIndicatorTypeOfEChart(IList<SPSAQualityIndicatorType> saList, string qIndicatorType, string sadimension)
        {
            ...
            {
                if (sadimension == SpecimenTransportTimeIsLong.就诊类型.Key)
                {
                    axisDataValue = vo.SickTypeCName;
                }
                else if (sadimension == SpecimenTransportTimeIsLong.就诊类型送检人.Key)
                {
                    axisDataValue = vo.SickTypeCName + "_" + vo.NurseSendCarrier;
                }
                else if (sadimension == SpecimenTransportTimeIsLong.就诊类型样本类型.Key)
                {
                    axisDataValue = vo.SickTypeCName + "_" + vo.SampleTypeCName;
                }
                else if (sadimension == SpecimenTransportTimeIsLong.就诊类型科室.Key)
                {
                    axisDataValue = vo.SickTypeCName + "_" + vo.DeptCName;
                }

                else if (sadimension == SpecimenTransportTimeIsLong.样本类型.Key)
                {
                    axisDataValue = vo.SampleTypeCName;
                }
                else if (sadimension == SpecimenTransportTimeIsLong.样本类型科室.Key)
                {
                    axisDataValue = vo.SampleTypeCName + "_" + vo.DeptCName;
                }
                else if (sadimension == SpecimenTransportTimeIsLong.样本类型送检人.Key)
                {
                    axisDataValue = vo.SampleTypeCName + "_" + vo.NurseSendCarrier;
                }

                else if (sadimension == SpecimenTransportTimeIsLong.科室.Key)
                {
                    axisDataValue = vo.DeptCName;
                }
                else if (sadimension == SpecimenTransportTimeIsLong.科室样本类型.Key)
                {
                    axisDataValue = vo.DeptCName + "_" + vo.SampleTypeCName;
                }
                else if (sadimension == SpecimenTransportTimeIsLong.科室送检人.Key)
                {
                    axisDataValue = vo.DeptCName + "_" + vo.NurseSendCarrier;
                }
                else if (sadimension == SpecimenTransportTimeIsLong.采样人.Key)
                {
                ...
                else if (sadimension == SpecimenTransportTimeIsLong.按年份.Key)
                {
                    axisDataValue = vo.Year;
                }
                else if (sadimension == SpecimenTransportTimeIsLong.按季度.Key)
                {
                    axisDataValue = vo.Quarter;
                }
                else if (sadimension == SpecimenTransportTimeIsLong.按月份.Key)
                {
        ...
        public override IList<SPSAQualityIndicatorType> GetSAQualityIndicatorTypeOfList(IList<SPSAQualityIndicatorType> nfList, string qitype, string sadimension, string startDate, string endDate)
        {
            ...
            else if (sadimension == SpecimenTransportTimeIsLong.送检人.Key)
            {
