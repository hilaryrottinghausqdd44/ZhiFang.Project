...
{
    /// <summary>
    /// 质量指标分类类型图表数据处理-抗凝标本凝集
    /// </summary>
    class OfASpecimenAgglutinationSADimension : QITypeOfEChartStrategy
    {
        public override BaseResultDataValue GetSAQualityIndicatorTypeOfEChart(IList<SPSAQualityIndicatorType> saList, string qIndicatorType, string sadimension)
        {
            ...
            {
                if (sadimension == ASpecimenAgglutinationSADimension.就诊类型.Key)
                {
                    axisDataValue = vo.SickTypeCName;
                }
                else if (sadimension == ASpecimenAgglutinationSADimension.就诊类型科室.Key)
                {
                    axisDataValue = vo.SickTypeCName + "_" + vo.DeptCName;
                }
                else if (sadimension == ASpecimenAgglutinationSADimension.就诊类型采样人.Key)
                {
                    axisDataValue = vo.SickTypeCName + "_" + vo.Collecter;
                }

                if (sadimension == ASpecimenAgglutinationSADimension.科室.Key)
                {
                    axisDataValue = vo.DeptCName;
                }
                else if (sadimension == ASpecimenAgglutinationSADimension.科室采样人.Key)
                {
                    axisDataValue = vo.DeptCName + "_" + vo.Collecter;
                }

                else if (sadimension == ASpecimenAgglutinationSADimension.按年份.Key)
                {
                    axisDataValue = vo.Year;
                }
                else if (sadimension == ASpecimenAgglutinationSADimension.按季度.Key)
                {
                    axisDataValue = vo.Quarter;
                }
                else if (sadimension == ASpecimenAgglutinationSADimension.按月份.Key)
                {
        ...
        public override IList<SPSAQualityIndicatorType> GetSAQualityIndicatorTypeOfList(IList<SPSAQualityIndicatorType> nfList, string qitype, string sadimension, string startDate, string endDate)
        {
            ...
            else if (sadimension == ASpecimenAgglutinationSADimension.科室.Key)
            {
