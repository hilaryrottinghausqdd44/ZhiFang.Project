...
{
    /// <summary>
    /// 质量指标分类类型图表数据处理-标本运输途中损坏
    /// </summary>
    class OfSTransportationDamageDuring : QITypeOfEChartStrategy
    {
        public override BaseResultDataValue GetSAQualityIndicatorTypeOfEChart(IList<SPSAQualityIndicatorType> saList, string qIndicatorType, string sadimension)
        {
            ...
            {
                if (sadimension == STransportationDamageDuring.就诊类型.Key)
                {
                    axisDataValue = vo.SickTypeCName;
                }
                else if (sadimension == STransportationDamageDuring.就诊类型送检人.Key)
                {
                    axisDataValue = vo.SickTypeCName + "_" + vo.NurseSendCarrier;
                }
                else if (sadimension == STransportationDamageDuring.就诊类型样本类型.Key)
                {
                    axisDataValue = vo.SickTypeCName + "_" + vo.SampleTypeCName;
                }
                else if (sadimension == STransportationDamageDuring.就诊类型科室.Key)
                {
                    axisDataValue = vo.SickTypeCName + "_" + vo.DeptCName;
                }

                else if (sadimension == STransportationDamageDuring.样本类型.Key)
                {
                    axisDataValue = vo.SampleTypeCName;
                }
                else if (sadimension == STransportationDamageDuring.样本类型科室.Key)
                {
                    axisDataValue = vo.SampleTypeCName + "_" + vo.DeptCName;
                }
                else if (sadimension == STransportationDamageDuring.样本类型送检人.Key)
                {
                    axisDataValue = vo.SampleTypeCName + "_" + vo.NurseSendCarrier;
                }

                else if (sadimension == STransportationDamageDuring.科室.Key)
                {
                    axisDataValue = vo.DeptCName;
                }
                else if (sadimension == STransportationDamageDuring.科室样本类型.Key)
                {
                    axisDataValue = vo.DeptCName + "_" + vo.SampleTypeCName;
                }
                else if (sadimension == STransportationDamageDuring.科室送检人.Key)
                {
                    axisDataValue = vo.DeptCName + "_" + vo.NurseSendCarrier;
                }
                else if (sadimension == STransportationDamageDuring.送检人.Key)
                {
                    axisDataValue = vo.NurseSendCarrier;
                }
                else if (sadimension == STransportationDamageDuring.采样人.Key)
                {
                    axisDataValue = vo.Collecter;
                }
                else if (sadimension == STransportationDamageDuring.按年份.Key)
                {
                    axisDataValue = vo.Year;
                }
                else if (sadimension == STransportationDamageDuring.按季度.Key)
                {
                    axisDataValue = vo.Quarter;
                }
                else if (sadimension == STransportationDamageDuring.按月份.Key)
                {
