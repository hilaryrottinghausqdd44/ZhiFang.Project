...
namespace ZhiFang.BLL.BloodTransfusion
{
    ...
    public class BBloodBReqForm : BaseBLL<BloodBReqForm, string>, ZhiFang.IBLL.BloodTransfusion.IBBloodBReqForm
    {
        ...
        #region 医嘱申请
        public BaseResultDataValue AddBloodBReqFormAndDtl(BloodBReqForm entity, BloodsConfig bloodsConfigVO, SysCurUserInfo curDoctor, IList<BloodBReqItem> addBreqItemList, IList<BloodBReqFormResult> addResultList, long empID, string empName)
        {
            ...
            if (useItemList != null && useItemList.Count > 0)
            {
                ...
                string reviewTips = "";
                if (bloodsConfigVO.Common.ConfirmedIsAutoCompleted == true)
                {
                    reviewTips = _getReviewTips2(entity);
                }
                else
                {
                    reviewTips = _getReviewTips(entity);
                }
        ...
        private string _getReviewTips(BloodBReqForm entity)
        {
            ...
            if (entity.BreqStatusID.Value == long.Parse(BreqFormStatus.提交申请.Key))
            {
                ...
                {
                    reviewTips = string.Format("24小时累计血量为:{0}ml,需要{1}进行审批!请提醒相关人员进行审批!", entity.ReqTotal, "[上级医生]");
                }
                else if (entity.ReqTotal >= 800 && entity.ReqTotal < 1600)
                {
                    reviewTips = string.Format("24小时累计血量为:{0}ml,需要{1}进行审批!请提醒相关人员进行审批!", entity.ReqTotal, "[上级医生][科主任]");
                }
                else if (entity.ReqTotal >= 1600)
                {
                    reviewTips = string.Format("24小时累计血量为:{0}ml,需要{1}进行审批!请提醒相关人员进行审批!", entity.ReqTotal, "[上级医生][科主任][医务科]");
                }
                else
                {
                    reviewTips = "";
            ...
            if (!string.IsNullOrEmpty(reviewTips))
                ZhiFang.Common.Log.Log.Info("申请单号为:" + entity.Id + ",大量用血审核提示信息:" + reviewTips);
            return reviewTips;
        }
        /// <summary>
        /// 走用血申请确认后自动完成用血审批流程时的提示
        /// 医生确认提交后提示需要哪些角色进行审批操作
        /// </summary>
        /// <param name="entity"></param>
        /// <returns></returns>
        private string _getReviewTips2(BloodBReqForm entity)
        {
            string reviewTips = "";
            if (entity.BreqStatusID.Value != long.Parse(BreqFormStatus.医嘱暂存.Key) && entity.BreqStatusID.Value != long.Parse(BreqFormStatus.医嘱作废.Key))
            {
                if (entity.ReqTotal < 800)
                {
                    reviewTips = string.Format("24小时累计血量为:{0}ml", entity.ReqTotal);
                    //reviewTips = string.Format("24小时累计血量为:{0}ml,需要{1}进行审批!请提醒相关人员进行审批!", entity.ReqTotal, "[上级医生]");
                }
                else if (entity.ReqTotal >= 800 && entity.ReqTotal < 1600)
                {
        ...
        /// <returns></returns>
        public BaseResultBool EditBloodBReqFormAndDtlByField(ref BloodBReqForm entity, BloodsConfig bloodsConfigVO, SysCurUserInfo curDoctor, string[] tempArray, IList<BloodBReqItem> addBreqItemList, IList<BloodBReqItem> editBreqItemList, IList<BloodBReqFormResult> addResultList, IList<BloodBReqFormResult> editResultList, long empID, string empName)
        {
        ...
        /// <param name="empName"></param>
        /// <returns></returns>
        public BaseResultBool EditBloodBReqFormOfConfirmApplyByReqFormId(BloodBReqForm entity, BloodsConfig bloodsConfigVO, string[] tempArray, long empID, string empName)
        {
            BaseResultBool brdv = new BaseResultBool();
