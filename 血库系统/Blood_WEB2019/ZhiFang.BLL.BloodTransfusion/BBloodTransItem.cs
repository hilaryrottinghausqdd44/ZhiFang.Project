    ...
    public class BBloodTransItem : BaseBLL<BloodTransItem>, ZhiFang.IBLL.BloodTransfusion.IBBloodTransItem
    {
        ...
        public void AddTransfusionAntriesList(ref BaseResultDataValue brdv, BloodTransForm transForm, IList<BloodTransItem> transfusionAntriesList, long empID, string empName)
        {
            for (int i = 0; i < transfusionAntriesList.Count; i++)
            {
                ...
                entity.ContentTypeID = int.Parse(ClassificationOfTransfusionContent.病人体征记录项.Key);
        ...
        public bool AddBloodTransItem(BloodTransForm transForm, BloodTransItem transfusionAntries, long empID, string empName)
        {
            ...
            if (entity.ContentTypeID != int.Parse(ClassificationOfTransfusionContent.病人体征记录项.Key) && entity.ContentTypeID != int.Parse(ClassificationOfTransfusionContent.临床处理结果描述.Key))
            {
                if (entity.BloodTransRecordTypeItem != null)
                {
            ...
            //病人体征的数字结果处理
            if (entity.ContentTypeID == int.Parse(ClassificationOfTransfusionContent.病人体征记录项.Key) && !entity.NumberItemResult.HasValue && !string.IsNullOrEmpty(entity.TransItemResult))
            {
            ...
            else if (entity.ContentTypeID != int.Parse(ClassificationOfTransfusionContent.病人体征记录项.Key))
            {
                entity.NumberItemResult = null;
            }
        ...
        public EntityList<BloodTransItem> SearchPatientSignsByOutDtlIdStr(string outDtlIdStr)
        {
            ...
            strb.Append(" 1=1 ");
            strb.Append(" and bloodtransitem.ContentTypeID =" + ClassificationOfTransfusionContent.病人体征记录项.Key);
            ...
            IList<BloodTransRecordTypeItem> recordTypeItemList = GetRecordTypeItemList(ClassificationOfTransfusionContent.病人体征记录项.Key);
            //按发血血袋明细分组
        ...
        private IList<BloodTransItem> GetPatientSigns2(string[] outDtlArr, IList<BloodTransItem> transItemList)
        {
            ...
            foreach (var groupBy2 in groupByList2)
            {
                ...
                transItem.ContentTypeID = int.Parse(ClassificationOfTransfusionContent.病人体征记录项.Key);
        ...
        public IList<BloodTransRecordTypeItem> GetRecordTypeItemList(string contentTypeID)
        {
        ...
        public BaseResultDataValue EditBatchTransfusionAntriesByOutDtlList(BloodTransForm transForm, IList<BloodTransItem> transfusionAntriesList, long empID, string empName, string transAddType)
        {
            ...
            for (int i = 0; i < transfusionAntriesList.Count; i++)
            {
                ...
                if (tempList != null && tempList.Count() > 0)
                {
                    ...
                    bool isUpdate = false;
                    //输血结束提交需要直接覆盖更新
                    if (transAddType == BloodTransAddType.输血结束.Key && editValue != serverValue)
                    {
                        isUpdate = true;
                    }
                    //原来的批量修改处理方式
                    else if (!string.IsNullOrEmpty(editValue) && editValue != serverValue)
                    {
                        isUpdate = true;
                    }
        ...
        public BaseResultDataValue EditBatchClinicalResultsByOutDtlListr(BloodTransForm transForm, BloodTransItem clinicalResults, long empID, string empName, string transAddType)
        {
            BaseResultDataValue brdv = new BaseResultDataValue();
        ...
        public BaseResultDataValue EditBatchClinicalResultsDescByOutDtlList(BloodTransForm transForm, BloodTransItem clinicalResultsDesc, long empID, string empName, string transAddType)
        {
            BaseResultDataValue brdv = new BaseResultDataValue();
