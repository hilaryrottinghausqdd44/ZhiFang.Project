...
namespace ZhiFang.BloodTransfusion.hisinterface
{
    ...
    public static class BReqToHisHelp
    {
        ...
        {
            if (!BReqToHisHelp.ToHisCurIdList.Contains(entity.Id))
                BReqToHisHelp.ToHisCurIdList.Add(entity.Id);//添加正在上传的业务单Id
            BaseResult baseresultdata = new BaseResult();
            ZhiFang.Common.Log.Log.Debug("用血申请单号为:" + entity.Id + ",上传HIS调用开始!");
            string url = bloodsConfigVO.CSServer.CS_BASE_URL + bloodsConfigVO.CSServer.CS_TOHISDATA_URL + "?sBreqFormID=" + entity.Id;
            ...
            //移除正在上传的业务单Id
            if (BReqToHisHelp.ToHisCurIdList.Contains(entity.Id))
                BReqToHisHelp.ToHisCurIdList.Remove(entity.Id);
            return baseresultdata;
        }
        /// <summary>
        ...
        public static BaseResult ObsoleteReqDataToHis(BloodBReqForm entity, BloodsConfig bloodsConfigVO, SysCurUserInfo curDoctor)
        {
            ...
            string url = bloodsConfigVO.CSServer.CS_BASE_URL + bloodsConfigVO.CSServer.CS_TOHISOBSOLETE_URL + "?sBreqFormID=" + entity.Id;
