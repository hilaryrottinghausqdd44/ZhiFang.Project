layui.extend({
    ...
    'bagretrievetable', 'bagretrieveform', 'usercheck'], function(){
	"use strict";
	var $ = layui.$;
	var form = layui.form;
	...
	function CheckOprateUser(callback){
		usercheck.openCheckUser(function(data){
			...
			if (!UserNo) {
				...
				bagretrieveform.setSelect("#carrier_select", UserNo);				
			}
			typeof callback === 'function' && callback(UserNo);  
		})		
	}
	//禁用ui元素
	...
	function onSave(){
        ...
        if (!bagretrieveform.CarrierID || bagretrieveform.CarrierID == ""){
        	CheckOprateUser(function(UserNo){
        	   if (UserNo){
        	   	    bagretrieveform.setSelect("#carrier_select", UserNo);
        	   	    onSave();
        	   }else{
