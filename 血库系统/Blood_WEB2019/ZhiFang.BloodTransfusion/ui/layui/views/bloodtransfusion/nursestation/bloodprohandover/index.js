layui.extend({
	...
	usercheck: 'views/bloodtransfusion/usercheck/usercheck',
    ...
    'bloodsconfig', 'bagoperationfrom','cachedata', 'nursesconfig', 'usercheck'], function(){
	"use strict";
	var $ = layui.jquery; //
	var form = layui.form;
    ...
    //禁用ui元素
	function DisabledEles(){
		//禁用护士信息 
		$("#carrier_select").attr('disabled', true);
	}
	//获取护工
	function getCarrierData(){
        var ele = $('#carrier_select');
        ...
        if (CarrierName.length > 1) {
        	...
        	ShortCode = CarrierName[1];
        };
        return {"UserNo": CarrierID, "UserName":Carrier, "ShortCode": ShortCode};
	}
    //设置确认框数据
    function setConfigData(layero, BagOperResultID){
        ...
        data["bagOpDtl"] = opdtl; 
        //设置是运送人
        var CarrierData = getCarrierData();
        var CarrierID = CarrierData["UserNo"];
        var Carrier = CarrierData["UserName"];
    ...
    //选择操作用户
	function CheckOprateUser(callback){
		var UserNo = "";
		usercheck.openCheckUser(function(data){
		    UserNo = data["UserNo"];
			if (!UserNo) {
			    $("#carrier_select").val("");
			    form.render('select');
			} else{
				bagoperationfrom.setSelect("#carrier_select", UserNo);				
			}
			callback && typeof callback === 'function' && callback(UserNo);
		});
	}
