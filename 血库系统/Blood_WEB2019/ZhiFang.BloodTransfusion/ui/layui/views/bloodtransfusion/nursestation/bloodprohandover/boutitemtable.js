layui.extend({
...
}).define(['form', 'table', 'uxutil', 'dataadapter', 'laytpl', 'layer', 'util', 'nursesconfig'], function(exports){
	"use strict";
	var $ = layui.jquery;
	var form = layui.form;
	var util = layui.util;
	...
	var bagItems = [
	    ...
	    '<div name="b3code"><span>{{d.BloodBOutItem_B3Code}}</span><span></div>',
	...
	var bagRegisterbtn = [
	    ...
	    '<div name="Invaliddate"><span>{{"有效期：" + d.BloodBOutItem_InvalidDate}}</span></div>',
	    '</div>'];
...
//		    };
	...
	boutitemtableProxy.SetBloodbagItems = function(data, callback){
		...
		for(var i = 0; i < data.length; i++) {
			Ldata = data[i] || {};
			//转换有效期格式
			InvalidDate = Ldata['BloodBOutItem_InvalidDate'];
			if (InvalidDate) {
				Ldata['BloodBOutItem_InvalidDate']= util.toDateString(InvalidDate, 'yyyy-MM-dd');
			} 
