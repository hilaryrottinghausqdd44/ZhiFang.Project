...
layui.extend({
...
}).define(['layer', 'table', 'uxutil', 'dataadapter','cachedata','bloodsconfig'], function(exports) {
	"use strict";
	...
	//显示关闭的提示信息
	breqItemEditTable.showBloodStyle = function(){
		var data = [];
		data.push('<p>1、红细胞类: 血常规检测。</p>');
		data.push('<p>2、血小板类: 血常规+血栓弹力图检测。</p>');
		data.push('<p>3、血浆类:</p>');
		data.push('<P style="text-indent:20px">a.血栓弹力图+凝血常规(白天)</P>');
		data.push('<P style="text-indent:20px">b.凝血常规(夜晚)</p>');
		data.push('<p>4、冷沉淀类:凝血常规</p>');
		data.push('<p>5、联合使用以上血液制品，检查项目应累加以上项目。</p>');
		data.push('<p style="text-indent:20px">a.血常规+血栓弹力图+凝血常规(白天)</p>');
		data.push('<p style="text-indent:20px">b.血常规+凝血常规(白天)检测</p>');
		
		layer.alert(data.join(''), {
					title: "提示信息",
					btn: ['关闭'],
					icon: 5
				});
	...
	breqItemEditTable.onChooseBloodstyle = function(callback) {
		...
		var dataTable = []; //table.cache["LAY-app-table-BloodBreqItem"] || [];2010-10-10 by xiehz modify
		//先删除已经选择的血制品
		var addList = [];
