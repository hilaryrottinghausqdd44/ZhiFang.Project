...
layui.extend({
...
}).define(['util', 'layer', 'table', 'uxutil', 'dataadapter', 'formSelects', "form", "cachedata", "runParams",'bloodsconfig', "csserver","soulTable"
], function(exports) {
	"use strict";
	...
	var breqFormTable = {
		...
		config: {
			...
			/**删除数据服务路径*/
			delUrl: uxutil.path.ROOT + "/BloodTransfusionManageService.svc/BT_UDTO_DelBloodBReqForm",
			selectUrlById: uxutil.path.ROOT +
				"/BloodTransfusionManageService.svc/BT_UDTO_SearchBloodBReqFormById?isPlanish=true",
			updPrtTotalUrl: uxutil.path.ROOT + "/BloodTransfusionManageService.svc/BT_UDTO_UpdateBloodBReqFormPrintTotalById",
			/**table查询服务URL*/
			url: "",
			where: "",
			defaultOrderBy: [{
			...
			cols: [
				[{
					...
					field: 'BloodBReqForm_ConfirmType',
					sort: true,
					width: 145,
					title: '钉钉审批'
				},{
					...
					templet: function(data) {
						...
						if (value == "0") {
							...
							value = '<span style = "color:red">受理不通过</span>';
						} else {
							value = "未受理";
						}
						return value;
	...
	/**
	 * @description 用血申请作废(BS调用CS服务,CS服务调用HIS作废接口作废)
	 * @param {Object} curRow
	 * @param {Object} callback
	 */
	...
	breqFormTable.onObsoleteToHisOfBS = function(curRow, callback) {
		...
		var url = uxutil.path.ROOT + "/BloodTransfusionManageService.svc/BT_UDTO_UpdateReqDataObsoleteToHis";
		var config = {
			type: "POST",
			url: url,
			timeout: uxutil.BS_TIME_OUT,
	...
	breqFormTable.onDelete = function(curRow, callback) {
		...
		var url = uxutil.path.ROOT + "/BloodTransfusionManageService.svc/BT_UDTO_DelBloodBReqForm?id=" + reqFormId;
		//IE浏览器查询时,需要带上个额外的时间戳参数,防止获取到的查询结果是缓存信息
	...
	breqFormTable.onReview = function(entity, callback) {
		...
		var url = uxutil.path.ROOT + "/BloodTransfusionManageService.svc/BT_UDTO_UpdateBloodBReqFormOfReviewByReqForm";
		var config = {
			type: "POST",
			url: url,
			timeout: uxutil.BS_TIME_OUT,
	...
	/**
	 * @description 手工调用BS服务返回医嘱信息给HIS
	 * @param {Object} curRow
	 * @param {Object} callback
	 */
	breqFormTable.onToHisDataOfBS = function(curRow, callback) {
		...
		var url = uxutil.path.ROOT + "/BloodTransfusionManageService.svc/BT_UDTO_UpdateReqDataUploadToHis";
		var config = {
			type: "POST",
			url: url,
			timeout: uxutil.BS_TIME_OUT,
	...
	/**
	 * @description 手工调用CS服务返回医嘱信息给HIS
	 * @param {Object} curRow
	 * @param {Object} callback
	 */
	breqFormTable.onToHisDataOfCS = function(curRow, callback) {
		...
		var url = bloodsconfig.getBreqToHisUrl() + "?sBreqFormID=" + reqFormId;
		//IE浏览器查询时,需要带上个额外的时间戳参数,防止获取到的查询结果是缓存信息
		url = url + "&t=" + new Date().getTime();
		var config = {
			type: "GET",
	 ...
	 */
	breqFormTable.renderBloodsconfig = function() {
		var me = this;
