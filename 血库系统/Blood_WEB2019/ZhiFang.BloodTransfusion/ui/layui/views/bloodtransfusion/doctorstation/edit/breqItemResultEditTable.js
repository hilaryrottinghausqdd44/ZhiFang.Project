...
layui.extend({
	...
	var breqItemResultEditTable = {
		...
		config: {
			...
			selectUrl: uxutil.path.ROOT + "/BloodTransfusionManageService.svc/BT_UDTO_GetBloodBReqItemResultListByVLisResultHql?isPlanish=true",
			/**删除数据服务路径*/
	...
	breqItemResultEditTable.getVLisresultHql = function() {
		...
		if(arr.length > 0) {
			...
			if(edate) sdate = util.toDateString(dateutil.getNextDate(edate, -day3), "yyyy-MM-dd");
			if(sdate.length > 0)
	...
	//获取几天内的LIS检验结果
	breqItemResultEditTable.gtLisResultDays=function(){
		var me = this;
