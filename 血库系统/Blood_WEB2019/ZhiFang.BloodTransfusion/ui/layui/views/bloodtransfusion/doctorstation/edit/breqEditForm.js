...
layui.extend({
...
}).define(['util', 'uxutil', 'dataadapter', 'laydate', 'form', "layer", "cachedata", "runParams", 'bloodsconfig',
	'formSelects', "bloodSelectData", "bloodFormSelects", "insertSelect"
], function(exports) {
	"use strict";
    ...
    var USETYPEID = '5'; //紧急用血的ID值，定制给鄞州人民医院使用
	var breqEditForm = {
		//新增时外部传入的表单项默认值
		defaultValues: "",
		config: {
			...
			selectUrl: uxutil.path.ROOT + "/BloodTransfusionManageService.svc/BT_UDTO_SearchBloodBreqFormById?isPlanish=true",
			/**新增服务地址 BT_UDTO_AddBloodBReqFormAndDtl */
			addUrl: uxutil.path.ROOT + "/BloodTransfusionManageService.svc/BT_UDTO_AddBloodBReqFormAndDtl",
			/**修改服务地址*/
			editUrl: uxutil.path.ROOT + "/BloodTransfusionManageService.svc/BT_UDTO_UpdateBloodBReqFormAndDtlByField"
		}
	};
	//构造器
	...
	//清空申请类型
	breqEditForm.clearUseType = function(){
		var me = this;
		var elem = $('select[name="BloodBReqForm_UseTypeID"]');
		elem.val('');
		form.render('select');		
	}
	
	//取消紧急用血
	breqEditForm.cancelUseTimeType = function(){
		var me = this;
		me.clearUseType();
		me.setReqFormBUseTimeTypeChecked(false);
	}
	//根据申请类型设置紧急用血状态
	breqEditForm.setReqFormBUseTimeTypeChecked = function(check){
		var me = this;
		var elem = $('input[type=checkbox][name="BloodBReqForm_BUseTimeTypeID"]');
		elem.prop('checked', check);
		form.render('checkbox');
		//模拟触发事件，尽量不修改之前的代码
		elem.checked = check; 
		layui.event.call(elem, 'form', 'checkbox(BloodBReqForm_BUseTimeTypeID)', {
            	elem: elem
               ,value: check
               ,othis: null
            })
	}
	//根据选择 申请类型 判断做紧急用血标识选择
	breqEditForm.setReqFormBUseTimeTypeID = function(elem){
		var me = this;
		var ele = $(elem);
        var UseTypeValue = ele.val(); 
        var showInfo = ['<P style="text-indent:30px;">1、仅供抢救病人时使用；</P>'];
        showInfo.push('<P style="text-indent:30px;">2、请电话告知输血科准备</P>');
        me.setReqFormBUseTimeTypeChecked(false);
        if (USETYPEID === UseTypeValue){
        	layer.open({
				type:1,
				title: "用血申请选择",
				area: ['300px', '150px'],
				content: showInfo.join(''),
				btn: ["确定继续", "取消返回"],
				yes: function(index, layero){
					layer.closeAll();
					me.setReqFormBUseTimeTypeChecked(true);
				},
				btn2:function(index, layero){
                    me.cancelUseTimeType();
				},
				end: function() {},
				cancel: function() {
                    me.cancelUseTimeType();
	...
	breqEditForm.onSave = function(editForm, submitData, callback) {
		...
		if ((status == 2 && params.entity.BUseTimeTypeID == 1 && isU == true) || (status == 2 && runPVal4 == true)) {
			status = 9; //审批完成
		...
		uxutil.server.ajax(config, function(result) {
			...
			if (result.success) {
				...
				id += '';
				//确认提交后的审批提示信息
				var reqStatusID = "" + params.entity.BreqStatusID;
				if (reqStatusID == "2" || (runPVal4 == true && reqStatusID == "9")) {
				...
				//result["id"] = id;
				result["entity"] = params.entity; //提交的实体信息
	...
	//刷新同步Bloodsconfig
	breqEditForm.renderBloodsconfig = function() {
		var me = this;
