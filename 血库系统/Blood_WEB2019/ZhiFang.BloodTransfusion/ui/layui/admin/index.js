layui.config({
	...
	//var GET_MODULE_TREE_URL = uxutil.path.RBAC_ROOT + '/RBACService.svc/RBAC_UDTO_SearchRBACModuleToTree';
	var GET_MODULE_TREE_URL = uxutil.path.ROOT + '/RBACService.svc/RBAC_UDTO_SearchModuleTreeBySessionHREmpID';
	...
	function getModuleTree() {
		...
		uxutil.server.ajax({
			...
			if((!data.value || data.value == null)&&uxutil.ISHASLIIP==false) {
				data.success = true;
				data.value = defaultmodule.loadDefaultModule();
			}
			if(data.success) {
