 ...
 * @version 2020-02-11
 */
Ext.define('Shell.class.sysbase.transrecordtype.Grid', {
	...
	/**删除数据服务路径*/
	delUrl: '/BloodTransfusionManageService.svc/BT_UDTO_DelBloodTransRecordType',
	...
	createGridColumns: function() {
		...
		var columns = [{
			...
			dataIndex: 'BloodTransRecordType_TransTypeId',
			text: '输血记录类型',
			width: 120,
			renderer: function(value, meta) {
				var v = "";
			...
			xtype: 'checkcolumn',
			text: '使用',
			dataIndex: 'BloodTransRecordType_IsVisible',
			width: 40,
			align: 'center',
			sortable: false,
			menuDisabled: true,
			...
			text: '创建时间',
			dataIndex: 'BloodTransRecordType_DataAddTime',
			width: 130,
			hidden: true,
			isDate: true,
			hasTime: true
			...
			text: '备注',
			dataIndex: 'BloodTransRecordType_Memo',
			width: 200,
			sortable: false,
			menuDisabled: true,
			defaultRenderer: true
			...
			text: '次序',
			dataIndex: 'BloodTransRecordType_DispOrder',
			width: 100,
			defaultRenderer: true,
			align: 'center',
			type: 'int'
	...
	onSaveClick: function() {
		...
		for (var i = 0; i < len; i++) {
			...
			var IsVisible = rec.get('BloodTransRecordType_IsVisible');
	...
	changeResult: function(data) {
		...
		for (var i = 0; i < len; i++) {
			var isVisible = data.list[i].BloodTransRecordType_IsVisible;
			...
			data.list[i].BloodTransRecordType_IsVisible = isVisible;
		}
		return data;
	}
});
