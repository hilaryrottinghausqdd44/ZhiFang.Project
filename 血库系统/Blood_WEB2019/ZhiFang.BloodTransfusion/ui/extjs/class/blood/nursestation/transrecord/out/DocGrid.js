...
Ext.define('Shell.class.blood.nursestation.transrecord.out.DocGrid', {
	...
	initComponent: function() {
		var me = this;
		me.addEvents('onAddTrans', 'onBatchRegister');
		//数据列
	...
	createSearchButtonsToolbar: function(items) {
		...
		items.push({
			...
			xtype: 'button',
			iconCls: 'button-add',
			text: '登记', //输血登记方式一(包含输血结束前及输血结束两部分)
			hidden: true,
			tooltip: '对选中的第血袋进行登记',
			listeners: {
				click: function(but) {
			...
			xtype: 'button',
			iconCls: 'button-add',
			text: '输血结束登记', //输血登记方式二(区分为输血结束前及输血结束两部分)
			tooltip: '对选中的第血袋进行输血结束登记',
			hidden:true,
			itemId: "btnBloodEnd",
			listeners: {
				click: function(but) {
	...
	onBatchRegister: function() {
		var me = this;
		me.fireEvent('onBatchRegister', me);
	},
	/**@description 是否显示输血结束登记按钮*/
	setBloodEndVisible: function(visible) {
		var me = this;
		var buttonsToolbar = me.getComponent('buttonsToolbar');
		if(!buttonsToolbar)return;
		
		var btnBloodEnd = buttonsToolbar.getComponent('btnBloodEnd');
		if(btnBloodEnd){
			btnBloodEnd.setVisible(visible);
		}
	},
