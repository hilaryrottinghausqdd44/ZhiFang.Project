...
Ext.define('Shell.class.blood.nursestation.transrecord.out.OutPanel', {
	...
	onListeners: function() {
		...
		me.DocGrid.on({
			...
			onBatchRegister: function(p) {
				//transTypeId==2时,为输血结束，按全部发血血袋进行操作
				me.DtlGrid.isSelectAll=false;
				me.DtlGrid.getSelectionModel().selectAll();				
	...
	addTrans: function() {
		...
		//判断是批量新增还是批量修改或单个修改
		var formtype = "add";
		var outDocId = ""; //发血主单Id,只有输血结束时才传值
	 ...
	 * 区分输血结束前及输血结束--输血结束前;	
	 */
	batchRegister:function(transTypeId){
		var me=this;
		...
		//判断是批量新增还是批量修改或单个修改
		var formtype = "add";
		var outDocIdStr = ""; //发血主单Id,只有输血结束时才传值
		var outDtlIdStr = ""; //批量登记的发血明细Id
		var transId = null; //单个修改的输血过程记录主单Id
		var isBatchEdit = false; //是否批量修改
		if (records.length > 1) {
		...
		//批量新增或单个修改
		var app = "Shell.class.blood.nursestation.transrecord.batchregister.App";
		if (isBatchEdit == true) {
			transId = "";
		}
		if (transTypeId=="2") {//发血主单Id,只有输血结束时才传值
			outDocIdStr = me.outDocRecord.get(me.DocGrid.PKField); 
		}
		var maxWidth = document.body.clientWidth - 10; //* 0.98;
		var height1 = document.body.clientHeight * 0.96;
		JShell.Win.open(app, {
	...
	/**
	 * @description 发血明细数据加载完成后处理
	 * @description 如果发血单中只要有任何一袋血制品没有进行过输血过程登记（即没有过输血结束前登记）,，
	 * @description 不允许进行输血结束登记操作；
	 * @param {Object} data
	 */
	dtlChangeResult:function(data){
		var me=this;
		var isShowBtn=true;//是否显示输血结束登记按钮
		var list=[];
		if(data&&data.list && data.list.length > 0)list=data.list;
		
		for (var i = 0; i < list.length; i++) {
			var courseCompletion=""+list[i]["BloodBOutItem_CourseCompletion"];
			if(!courseCompletion||courseCompletion=="0"||courseCompletion=="1"){
				isShowBtn=false;
				break;
			}
		}
		//处理输血结束登记按钮是否显示
		me.DocGrid.setBloodEndVisible(isShowBtn);
	}
});
