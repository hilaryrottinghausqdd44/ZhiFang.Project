...
Ext.define('Shell.class.blood.nursestation.transrecord.transdtl.DtlForm', {
	...
	/**获取数据服务路径*/
	selectUrl: '/BloodTransfusionManageService.svc/BT_UDTO_SearchBloodTransItemListByTransFormId?isPlanish=true',
	...
	createItems: function() {
		var me = this,
			items = [];
		me.itemObjList = [];
		me.getFieldsetHeight();
		var fwidth=me.getFieldsetWidth();
		//输血记录项分类
	...
	getFieldsetWidth: function() {
		...
		if (fwidth < me.defaults.width) {
			fwidth = me.defaults.width + 5;
		} else if (fwidth > 380) {
			fwidth = fwidth - 10;
		}
		return fwidth;
	...
	getFieldsetHeight: function() {
		...
		var counts = me.transRecordTypeItemList.length;
		if(counts<4){
			me.fheight = parseInt((me.height -15));
		}
	},
	//创建fieldset
