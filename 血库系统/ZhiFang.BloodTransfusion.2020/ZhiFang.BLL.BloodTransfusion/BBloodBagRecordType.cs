...
namespace ZhiFang.BLL.BloodTransfusion
{
	...
	///
	/// </summary>
	public  class BBloodBagRecordType : BaseBLL<BloodBagRecordType>, ZhiFang.IBLL.BloodTransfusion.IBBloodBagRecordType
	{
        IBSCOperation IBSCOperation { get; set; }

        #region 修改信息记录
        public void AddSCOperation(BloodBagRecordType serverEntity, string[] arrFields, long empID, string empName)
        {
            StringBuilder strbMemo = new StringBuilder();
            EditGetUpdateMemoHelp.EditGetUpdateMemo<BloodBagRecordType>(serverEntity, this.Entity, this.Entity.GetType(), arrFields, ref strbMemo);
            if (strbMemo.Length > 0)
            {
                ...
                sco.BusinessModuleCode = "BloodBagRecordType";
                ...
                sco.IsUse = true;
                sco.Type = long.Parse(UpdateOperationType.血袋记录类型.Key);
