...
namespace ZhiFang.BLL.BloodTransfusion
{
	...
	///
	/// </summary>
	public  class BBloodChargeItem : BaseBLL<BloodChargeItem>, ZhiFang.IBLL.BloodTransfusion.IBBloodChargeItem
	{
        IBSCOperation IBSCOperation { get; set; }

        #region 修改信息记录
        public void AddSCOperation(BloodChargeItem serverEntity, string[] arrFields, long empID, string empName)
        {
            StringBuilder strbMemo = new StringBuilder();
            EditGetUpdateMemoHelp.EditGetUpdateMemo<BloodChargeItem>(serverEntity, this.Entity, this.Entity.GetType(), arrFields, ref strbMemo);
            if (strbMemo.Length > 0)
            {
                ...
                sco.BusinessModuleCode = "BloodChargeItem";
                ...
                sco.IsUse = true;
                sco.Type = long.Parse(UpdateOperationType.费用项目记录.Key);
