...
namespace ZhiFang.BLL.BloodTransfusion
{
	...
	///
	/// </summary>
	public  class BBloodBTestItem : BaseBLL<BloodBTestItem>, ZhiFang.IBLL.BloodTransfusion.IBBloodBTestItem
	{
        IBSCOperation IBSCOperation { get; set; }

        #region 修改信息记录
        public void AddSCOperation(BloodBTestItem serverEntity, string[] arrFields, long empID, string empName)
        {
            StringBuilder strbMemo = new StringBuilder();
            EditGetUpdateMemoHelp.EditGetUpdateMemo<BloodBTestItem>(serverEntity, this.Entity, this.Entity.GetType(), arrFields, ref strbMemo);
            if (strbMemo.Length > 0)
            {
                ...
                sco.BusinessModuleCode = "BloodBTestItem";
                ...
                sco.IsUse = true;
                sco.Type = long.Parse(UpdateOperationType.检验项目记录.Key);
