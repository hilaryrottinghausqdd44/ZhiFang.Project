...
namespace ZhiFang.BLL.BloodTransfusion
{
    ...
    public  class BBDict : BaseBLL<BDict>, IBBDict
	{
        ...
        #region 修改信息记录
        public void AddSCOperation(BDict serverEntity, string[] arrFields, long empID, string empName)
        {
            StringBuilder strbMemo = new StringBuilder();
            EditGetUpdateMemoHelp.EditGetUpdateMemo<BDict>(serverEntity, this.Entity, this.Entity.GetType(), arrFields, ref strbMemo);
            if (strbMemo.Length > 0)
            {
                ...
                sco.IsUse = true;
                sco.Type = long.Parse(UpdateOperationType.公共字典记录.Key);
