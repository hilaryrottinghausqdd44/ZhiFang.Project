...
namespace ZhiFang.BLL.BloodTransfusion
{
	...
	///
	/// </summary>
	public  class BBloodChargeItemType : BaseBLL<BloodChargeItemType>, ZhiFang.IBLL.BloodTransfusion.IBBloodChargeItemType
	{
        IBSCOperation IBSCOperation { get; set; }

        #region 修改信息记录
        public void AddSCOperation(BloodChargeItemType serverEntity, string[] arrFields, long empID, string empName)
        {
            StringBuilder strbMemo = new StringBuilder();
            EditGetUpdateMemoHelp.EditGetUpdateMemo<BloodChargeItemType>(serverEntity, this.Entity, this.Entity.GetType(), arrFields, ref strbMemo);
            if (strbMemo.Length > 0)
            {
                ...
                sco.BusinessModuleCode = "BloodChargeItemType";
                ...
                sco.IsUse = true;
                sco.Type = long.Parse(UpdateOperationType.费用项目类型描述.Key);
