...
namespace ZhiFang.BLL.BloodTransfusion
{
	...
	///
	/// </summary>
	public  class BBloodBagProcessTypeQry : BaseBLL<BloodBagProcessTypeQry>, ZhiFang.IBLL.BloodTransfusion.IBBloodBagProcessTypeQry
	{
        IBSCOperation IBSCOperation { get; set; }

        #region 修改信息记录
        public void AddSCOperation(BloodBagProcessTypeQry serverEntity, string[] arrFields, long empID, string empName)
        {
            StringBuilder strbMemo = new StringBuilder();
            EditGetUpdateMemoHelp.EditGetUpdateMemo<BloodBagProcessTypeQry>(serverEntity, this.Entity, this.Entity.GetType(), arrFields, ref strbMemo);
            if (strbMemo.Length > 0)
            {
                ...
                sco.BusinessModuleCode = "BloodBagProcessTypeQry";
                ...
                sco.IsUse = true;
                sco.Type = long.Parse(UpdateOperationType.血制品加工类型关系.Key);
