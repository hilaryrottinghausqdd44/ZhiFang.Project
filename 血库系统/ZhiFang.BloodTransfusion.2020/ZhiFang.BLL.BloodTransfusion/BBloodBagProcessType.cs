...
namespace ZhiFang.BLL.BloodTransfusion
{
	...
	///
	/// </summary>
	public  class BBloodBagProcessType : BaseBLL<BloodBagProcessType>, ZhiFang.IBLL.BloodTransfusion.IBBloodBagProcessType
	{
        IBSCOperation IBSCOperation { get; set; }

        #region 修改信息记录
        public void AddSCOperation(BloodBagProcessType serverEntity, string[] arrFields, long empID, string empName)
        {
            StringBuilder strbMemo = new StringBuilder();
            EditGetUpdateMemoHelp.EditGetUpdateMemo<BloodBagProcessType>(serverEntity, this.Entity, this.Entity.GetType(), arrFields, ref strbMemo);
            if (strbMemo.Length > 0)
            {
                ...
                sco.BusinessModuleCode = "BloodBagProcessType";
                ...
                sco.IsUse = true;
                sco.Type = long.Parse(UpdateOperationType.加工类型记录.Key);
