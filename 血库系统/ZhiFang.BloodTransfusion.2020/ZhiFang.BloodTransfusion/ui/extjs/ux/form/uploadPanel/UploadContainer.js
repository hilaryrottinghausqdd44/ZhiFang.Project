...
Ext.define('Shell.ux.form.uploadPanel.UploadContainer', {
	...
	initaddEvents: function() {
		...
		//me.addEvents('selectedfilerender', 'beginupload', 'handleUploadstart', 'handleUploadProgress','handleUploadfailure','handleUploadsuccess', 'uploadcomplete','save');
	...
	onFileSelection: function(file, fileList) {
		...
		if(isValidator == true) {
			...
			Ext.each(fileList, function(f) {
				...
				obj[BusinessModuleCode] = me.BusinessModuleCode;
				obj[DispOrder] = me.fileItemGird.getStore().getCount() + 1;
	...
	save: function() {
		...
		for(var i = 0; i < me.fileItemGird.getStore().getCount(); i++) {
			...
			//新增上传文件
			if(id && id == "-1") {
				if(!record.get(DispOrder)) {
					record.set(DispOrder, i + 1);
				}
			...
			} else if(id && id != "-1" && record.dirty == true) {
		...
		if(editRecords.length > 0) {
			...
			for(var i = 0; i < editRecords.length; i++) {
				me.updateOne(i, editRecords[i],addRecords.length);
			}
		}
	...
	/**依文档附件ID的自定义名称编辑保存*/
	updateOne: function(index, record,addCounts) {
		var me = this;
		setTimeout(function() {
			var id = "" + record.get(me.PKField);
			var newFileName = record.get(me.objectEName + '_NewFileName');
			var DispOrder = record.get(me.objectEName + '_DispOrder');
			var obj = {
				entity: {
					Id: id,
			...
			if(me.objectEName != "PProjectAttachment") {
				obj.entity.DispOrder = DispOrder;
				obj.fields += ",DispOrder";
			}
			...
			JShell.Server.post(me.editUrl, params, function(data) {
				...
				if(me.saveCount + me.saveErrorCount == me.saveLength&&addCounts== 0) {
					var obj = {
						success: me.saveErrorCount == 0 ? true : false,
						msg: me.progressMsg
					};
					//me.isIE10Less() == false是IE10及以上版本和其他浏览器
	...
	handleUploadfailure: function(event, msg, record) {
		...
		if(!msg) msg = "";
		if(msg.length > 0) msg = '<span style="color:red;">' + msg + "</sapn>";
		me.progressMsg = me.progressMsg + "【" + fileName + "】上传失败!" + msg + "<br />";
