...
Ext.define('Shell.class.blood.nursestation.transrecord.out.DocGrid', {
	...
	/**获取数据服务路径*/
	selectUrl: '/ServerWCF/BloodTransfusionManageService.svc/BT_UDTO_SearchBloodBOutFormOfBloodTransByHQL?isPlanish=true',
	/**只能获取到可配置的系统参数*/
	defaultWhere: "",
	/**默认每页数量*/
	defaultPageSize: 100,
		...
		var columns = [{
			...
			items: [{
				...
				handler: function(grid, rowIndex, colIndex) {
					...
					me.onUpdateCourseCompletion(rec);
				}
			}]
		},{
			text: '病历号',
	...
	getInternalWhere: function() {
		...
		if (handoverCompletion) {
			...
			if (value) {
				if (value == "1") {//未接收
					params.push("(bloodboutform.HandoverCompletion=0 or bloodboutform.HandoverCompletion=1 or bloodboutform.HandoverCompletion is null)");
				} else {
	...
	onPrintClick: function() {
		...
		var url = JShell.System.Path.getRootUrl("/ServerWCF/BloodTransfusionManageService.svc/RS_UDTO_SearchBusinessReportOfPdfById");
		var params = [];
	...
	onDownLoadExcel: function() {
		...
		var url = JShell.System.Path.getRootUrl("/ServerWCF/BloodTransfusionManageService.svc/RS_UDTO_SearchBusinessReportOfExcelById");
		var params = [];
		params.push("operateType=" + operateType);
		params.push("id=" + id);
		params.push("breportType=" + me.breportType);
	 ...
	 * @param {Object} rec
	 */
	onUpdateCourseCompletion: function(rec) {
		var me = this;
		...
		params.push("id=" + outId);
		params.push("updateValue=" + updateValue);
		params.push("empID=" + empID);
		if (empName) {
			params.push("empName=" + JShell.String.encode(empName));
		}
		var url =JShell.System.Path.ROOT + "/ServerWCF/BloodTransfusionManageService.svc/BT_UDTO_UpdateBOutCourseCompletionByOutId";		
		url += "?" + params.join("&");
