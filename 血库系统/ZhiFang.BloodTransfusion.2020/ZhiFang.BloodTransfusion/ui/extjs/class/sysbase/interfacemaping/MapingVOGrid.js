...
Ext.define('Shell.class.sysbase.interfacemaping.MapingVOGrid', {
	...
	/**获取数据服务路径*/
	selectUrl: '/ServerWCF/BloodTransfusionManageService.svc/BT_UDTO_SearchBDictMapingVOByHQL?isPlanish=true',
	/**修改服务地址*/
	editUrl: '/ServerWCF/BloodTransfusionManageService.svc/BT_UDTO_UpdateBloodInterfaceMapingByField',
	/**删除数据服务路径*/
	delUrl: '/ServerWCF/BloodTransfusionManageService.svc/BT_UDTO_DelBloodInterfaceMaping',
	...
	createGridColumns: function() {
		...
		var columns = [{
			text: '对照关系Id',
			dataIndex: 'BDictMapingVO_BloodInterfaceMaping_Id',
			width: 165,
			isKey: true,
			hidden: true
		}, {
			text: '字典编码',
			dataIndex: 'BDictMapingVO_BloodInterfaceMaping_BobjectID',
			width: 165,
			doSort: function(state) {
				var field = "BloodInterfaceMaping_BobjectID";
				me.store.sort({
					property: field,
					direction: state
				});
			...
			dataIndex: 'BDictMapingVO_BloodInterfaceMaping_MapingCode',
			width: 140,
			doSort: function(state) {
				var field = "BloodInterfaceMaping_MapingCode";
				me.store.sort({
					property: field,
					direction: state
				});
			...
			dataIndex: 'BDictMapingVO_BloodInterfaceMaping_DispOrder',
			width: 65,
			defaultRenderer: true,
			align: 'center',
			type: 'int',
			doSort: function(state) {
				var field = "BloodInterfaceMaping_DispOrder";
				me.store.sort({
					property: field,
					direction: state
				});
	...
	getLoadUrl: function() {
		...
		if (me.objectTypeId) {
			...
			url += "&mapingWhere=bloodinterfacemaping.BobjectType.Id=" + me.objectTypeId;
		}
		return url;
	},
	/**保存按钮处理*/
	...
	updateOne: function(index, rec) {
		...
		var dispOrder = rec.get('BDictMapingVO_BloodInterfaceMaping_DispOrder');
		var mapingCode = rec.get('BDictMapingVO_BloodInterfaceMaping_MapingCode');
	...
	onShowOperation: function(record) {
		...
		var config = {
			...
			title: '操作记录',
			defaultWhere: "scoperation.BusinessModuleCode='BloodInterfaceMaping'"
		};
