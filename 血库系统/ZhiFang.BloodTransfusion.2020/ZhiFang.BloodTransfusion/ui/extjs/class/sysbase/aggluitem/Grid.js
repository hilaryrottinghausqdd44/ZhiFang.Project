 ...
 * @version 2020-04-08
 */
Ext.define('Shell.class.sysbase.aggluitem.Grid', {
	...
	/**删除数据服务路径*/
	delUrl: '/ServerWCF/BloodTransfusionManageService.svc/BT_UDTO_DelBloodAggluItem ',
	...
	/**用户UI配置Key*/
	userUIKey: 'BloodAggluItem.Grid',
	/**用户UI配置Name*/
	userUIName: "凝集规则明细字典列表",
	...
	initComponent: function() {
		...
		me.searchInfo = {
			...
			emptyText: '名称/简称',
			isLike: true,
			fields: ['bloodaggluitem.CName', 'bloodaggluitem.ShortCode']
		};
	...
	createGridColumns: function() {
		...
		var columns = [{
			text: '血液分类',
			dataIndex: 'Bloodstyle_BloodClass_CName',
			width: 90,
			defaultRenderer: true
		}, {
			text: '凝集字典',
			dataIndex: 'Bloodstyle_BloodAgglu_CName',
			width: 90,
			defaultRenderer: true
		}, {
			text: '编号',
			...
			text: 'ABO简称',
			dataIndex: 'BloodAggluItem_SName',
			width: 90,
			defaultRenderer: true
		}, {
			text: '拼音字头',
			dataIndex: 'BloodAggluItem_PinYinZiTou',
			width: 90,
			defaultRenderer: true
		}, {
			text: 'Rh英文名',
			dataIndex: 'BloodAggluItem_RhEName',
			width: 90,
			defaultRenderer: true
		}, {
			text: '可配RH小因子',
			dataIndex: 'BloodAggluItem_AggluItemName',
			width: 90,
			defaultRenderer: true
		}, {
			text: '优先级',
			dataIndex: 'BloodAggluItem_RhPriority',
			width: 90,
			defaultRenderer: true
		}, {
			text: '<b style="color:blue;">使用</b>',
			xtype: 'checkcolumn',
			dataIndex: 'BloodAggluItem_IsUse',
			width: 40,
			align: 'center',
			sortable: false,
			menuDisabled: true,
			...
			dataIndex: 'BloodAggluItem_DispOrder',
			width: 60,
			defaultRenderer: true,
			type: 'int',
			editor: {
	...
	onSaveClick: function() {
		...
		for (var i = 0; i < len; i++) {
			var rec = records[i];
			me.updateOneByVisible(i, rec);
		}
	},
	updateOneByVisible: function(index, rec) {
		var me = this;
		var url = (me.editUrl.slice(0, 4) == 'http' ? '' : JShell.System.Path.ROOT) + me.editUrl;
		var id = rec.get(me.PKField);
		var Visible = rec.get('BloodAggluItem_IsUse');
		var DispOrder = rec.get('BloodAggluItem_DispOrder');
