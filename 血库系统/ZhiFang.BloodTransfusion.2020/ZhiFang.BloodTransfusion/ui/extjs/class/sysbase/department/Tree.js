 ...
 * @version 2020-03-26
 */
Ext.define('Shell.class.sysbase.department.Tree',{
	...
	/**获取数据服务路径*/
	//selectUrl:'/RBACService.svc/RBAC_RJ_GetDepartmentFrameListTree?fields=Department_Id,Department_Code1',
	/**获取数据服务路径*/
	selectUrl:'/ServerWCF/RBACService.svc/RBAC_RJ_GetDepartmentFrameTree',
	/**默认加载数据*/
	defaultLoad:true,
	/**根节点*/
	root:{
	...
	changeData:function(data){
    	...
    	var changeNode = function(node){
    		//图片地址处理
    		if(node.value && node.value.ShortCode){
    			node.text = '[' + node.value.ShortCode + '] ' + node.text;
    		}
