layui.extend({
...
}).use(['uxutil', 'layer', 'bloodsconfig', 'uxdata', 'cachedata', 'runParams'], function() {
	"use strict";
	...
	var nurseStationStrategy = function() {
		var strategy = {
			bloodprohandover: function() { //血袋接收
				var href = "/extjs/interface/one/index.html?className=Shell.class.blood.nursestation.bloodprohandover.App";
				return href;
			},
			transrecord: function() { //输血过程记录
				var href = "/extjs/interface/one/index.html?className=Shell.class.blood.nursestation.transrecord.App";
				return href;
			},
	...
	var statisticsStrategy = function() {
		var strategy = {
			reqntegrated: function() { //输血综合查询
				var href = "/extjs/interface/one/index.html?className=Shell.class.blood.statistics.reqntegrated.App";
				return href;
			}
		};
		//调用接口
	...
	function onLoginByHisCode(callback) {
		...
		var url = url = uxutil.path.ROOT + bloodsconfig.Common.LOGINOFPUSERBYHISCODE_URL;
