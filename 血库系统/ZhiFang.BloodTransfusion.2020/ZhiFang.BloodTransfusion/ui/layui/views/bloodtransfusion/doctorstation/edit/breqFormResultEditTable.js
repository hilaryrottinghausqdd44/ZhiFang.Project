...
layui.extend({
	...
	var breqFormResultEditTable = {
		...
		config: {
			...
			selectUrl: uxutil.path.ROOT +
				"/ServerWCF/BloodTransfusionManageService.svc/BT_UDTO_SelectBloodBReqFormResultListByVLisResultHql?isPlanish=true",
			/**删除数据服务路径*/
	...
	breqFormResultEditTable.changeResult = function(data) {
		...
		for (var i = 0; i < data.data.length; i++) {
			...
			if (!itemResult) itemResult = bloodsconfig.Common.LIS_DEFAULT_ITEMSRESULT;
	...
	breqFormResultEditTable.getVLisresultHql = function() {
		...
		if (arr.length > 0) {
			...
			sdate = util.toDateString(dateutil.getNextDate(edate, -bloodsconfig.Common.GET_LISRESULT_DAYS), "yyyy-MM-dd");
			if (sdate.length > 0)
