...
layui.extend({
...
}).use(['uxutil', 'dataadapter', 'bloodsconfig', 'csserver', 'layer', 'form', 'bloodBreqSearchForm', 'table',
	'breqFormApplyReviewTable', 'breqItemTable', 'breqFormResultTable', 'uxdata', "cachedata", 'bloodSelectData',
	'clodopprint', 'formSelects',
	"bloodClassDict"
], function() {
	"use strict";
	...
	function getBloodUseDesc(callback) {
		...
		var url = uxutil.path.ROOT +
			"/ServerWCF/BloodTransfusionManageService.svc/BT_UDTO_SearchBloodUseDescById?isPlanish=true&id=100";
		...
		if(result.reviewTips && result.reviewTips.length > 0) {
			layer.alert(result.reviewTips, {
				...
				icon: 5,
					...
					var id = result.id;
					if(id) {
						breqFormTable1.LoadById(id, function(curRow) {
							if(curRow) {
								onSenior(curRow);
							} else {
								onRefreshBreqFormTable();
	...
	function getPDFJS(id, callback) {
		var url = uxutil.path.ROOT +
			"/ServerWCF/BloodTransfusionManageService.svc/BT_UDTO_SearchBusinessReportOfPDFJSById?id=" +
	...
	function openPreviewPDF(data) {
		var id = data["BloodBReqForm_Id"];
		var url = uxutil.path.ROOT + "/ServerWCF/BloodTransfusionManageService.svc/BT_UDTO_SearchBusinessReportOfPdfById";
		var arr = [];
		arr.push("id=" + id);
	...
	/***
	 * 上级审核通过或科主任审核通过后,需要判断用血申请单是否审批完成,审批完成后,需要处理医嘱申请返回给HIS
	 * @param {Object} curRow
	...
	/***
	 * 新增医嘱申请时,按患者就诊号或病历号调用CS服务获取HIS病人信息
	 * HIS调用时,判断患者信息是否存在用血同意书,只有存在用血同意书,才能新增医嘱申请
	 * @param {Object} callback
	 */
	function onGetPatInfo(callback) {
		...
		var url = bloodsconfig.CSServer.CS_GETPATINFO_URL + "?sPatNo=" + sPatNo;
		url += '&t=' + new Date().getTime();
		...
		csserver.ajax({
			...
			if(result && result.length > 0) {
				...
				if(isAgree && (isAgree == "1" || isAgree == "有" || isAgree == "true")) {
					result1.isAgree = true;
				} else {
					result1.isAgree = false;
					result1.msg = '获取病人信息没有用血同意书信息!';
				}
			} else {
