...
layui.extend({
	...
	var bdictForm = {
		config: {
			...
			selectFields: "",
			lastData: "",
			selectUrl: uxutil.path.ROOT + "/ServerWCF/SingleTableService.svc/ST_UDTO_SearchBDictById?isPlanish=true",
			/**新增服务地址 BT_UDTO_AddBDictAndDtl */
			addUrl: uxutil.path.ROOT + "/ServerWCF/SingleTableService.svc/ST_UDTO_AddBDict",
			/**修改服务地址*/
			editUrl: uxutil.path.ROOT + "/ServerWCF/SingleTableService.svc/ST_UDTO_UpdateBDictByField"
		}
	};
	//构造器
	var Class = function(options) {
		var me = this;
		me.config = $.extend({}, me.config, bdictForm.config, options);
		var inst = $.extend(true, {}, bdictForm, me);
		return inst;
	};
	Class.pt = Class.prototype;
	//获提交的Fields
	bdictForm.getFields = function(entity, isString) {
		var me = this;
		var fields = [];
	...
	bdictForm.load = function(id, callback) {
		var me = this;
	...
	//对表单赋值处理
	bdictForm.setValues = function(result) {
		var me = this;
		form.val(me.config.formfilter, result);
	};
	//重新处理表单获取的数据
	bdictForm.setNullValues = function(result) {
		var me = this;
		return result;
	};
	//重新处理表单获取的数据
	bdictForm.changeData = function(result) {
		var me = this;
		return result;
	};
	//表单新增初始化
	bdictForm.isAdd = function() {
		var me = this;
		me.config.formtype = "add";
		var bdictTypeId = me.config.bdictTypeId;
		if(!bdictTypeId) bdictTypeId = "";
		var result = {
			"BDict_Id": "",
			"BDict_IsUse": true,
			"BDict_SName": "",
			...
			"BDict_DispOrder": "",
			"BDict_Memo": "",
			"BDict_BDictType_Id": bdictTypeId
		};
		form.val(me.config.formfilter, result);
	};
	//表单编辑初始化
	bdictForm.isEdit = function(id) {
		var me = this;
		me.config.PK = id;
	...
	//表单项是否只读处理
	bdictForm.setReadOnly = function(id) {
		var me = this;
	};
	/**初始化信息数据*/
	bdictForm.initInfo = function() {
		var me = this,
	...
	/**@overwrite 获取新增的数据*/
	bdictForm.getAddParams = function(data) {
		var me = this;
		var entity = JSON.stringify(data).replace(/BDict_/g, "");
		if(entity) entity = JSON.parse(entity);
		//字典所属字典类型处理
		entity.BDictType = {
			Id: entity["BDictType_Id"],
			DataTimeStamp: [0, 0, 0, 0, 0, 0, 0, 0]
		};
		delete entity["BDictType_Id"];
	...
	/**@overwrite 获取修改的数据*/
	bdictForm.getEditParams = function(data) {
		var me = this;
		var entity = me.getAddParams(data);
		entity.fields = me.getFields(entity.entity);
		if(data["BDict_Id"])
			entity.entity.Id = data["BDict_Id"];
		return entity;
	};
	//表单保存处理
	bdictForm.onSave = function(editForm, submitData, callback) {
	...
	/**初始化表单信息*/
	bdictForm.initForm = function() {
		var me = this;
	};
	bdictForm.getSelectFields = function() {
		var me = this;
		var field = [];
	...
	//核心入口
	bdictForm.render = function(options) {
		var me = this;
