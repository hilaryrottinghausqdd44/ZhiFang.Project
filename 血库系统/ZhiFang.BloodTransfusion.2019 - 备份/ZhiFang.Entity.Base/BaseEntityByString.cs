...
{
    [DataContract]
    public class BaseEntityByString
    {
        ...
        [DataDesc(CName = "主键ID", ShortCode = "Id", Desc = "主键ID", ContextType = SysDic.Number, Length = 50)]
        public virtual string Id
        {
            get
            {
                if (string.IsNullOrEmpty(_id) || _id == "-1")
                    _id = ZhiFang.Common.Public.GUIDHelp.GetGUIDLong().ToString();
                return _id;
            }
            set { _id = value; }
        }
        ...
        public virtual long LabID
        {
            ...
            {
                string IsLabFlag = "";
                if (!SysPublicSet.IsSetLicense)
                {
                    IsLabFlag = Cookie.CookieHelper.Read(ZhiFang.Entity.Base.SysPublicSet.SysDicCookieSession.IsLabFlag);
                }
                if (IsLabFlag != null && IsLabFlag.Trim() == "1")
                {
                    string labid = Cookie.CookieHelper.Read(ZhiFang.Entity.Base.SysPublicSet.SysDicCookieSession.LabID);
