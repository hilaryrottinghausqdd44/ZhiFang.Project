...
{
    /// <summary>
    /// 质量指标分类类型-统计纬度按样本类型科室分组
    /// </summary>
    class OfSampleTypeAndDeptGroupBy : DimensionStrategy
    {
        public override IList<SPSAQualityIndicatorType> GetSPSAQualityIndicatorTypeOfGroupBy(IList<SPSAQualityIndicatorType> nfList, string qIndicatorType)
        {
            ...
            foreach (var group in groupBy)
            {
                ...
                var tempList = nfList.Where(p => p.SampleTypeNo == group.Key.SampleTypeNo && p.DeptNo == group.Key.DeptNo);
