...
{
    /// <summary>
    /// 质量指标分类类型-统计纬度按就诊类型+医生分组
    /// </summary>
    class OfSickTypeAndDoctorGroupBy : DimensionStrategy
    {
        public override IList<SPSAQualityIndicatorType> GetSPSAQualityIndicatorTypeOfGroupBy(IList<SPSAQualityIndicatorType> nfList, string qIndicatorType)
        {
            ...
            foreach (var group in groupBy)
            {
                ...
                entity.DoctorNo = group.Key.DoctorNo;
                entity.DoctorCName = group.ElementAt(0).DoctorCName;

                var tempList = nfList.Where(p => p.SickTypeNo == group.Key.SickTypeNo && p.DoctorNo == group.Key.DoctorNo);
