...
{
    /// <summary>
    /// 质量指标分类类型-统计纬度按科室+医生分组
    /// </summary>
    class OfDeptAndDoctorGroupBy : DimensionStrategy
    {
        public override IList<SPSAQualityIndicatorType> GetSPSAQualityIndicatorTypeOfGroupBy(IList<SPSAQualityIndicatorType> nfList, string qIndicatorType)
        {
            ...
            foreach (var group in groupBy)
            {
                ...
                var tempList = nfList.Where(p => p.DeptNo == group.Key.DeptNo && p.DoctorNo == group.Key.DoctorNo);
