    ...
    public class BPUser : BaseBLL<PUser, int>, IBPUser
    {
        ...
        public BaseResultDataValue GetUserLoginByHisCode(string hisWardCode, string hisDeptCode, string hisDoctorCode, bool isAutoAddDepartmentUser)
        {
            ...
            if (userList != null && userList.Count > 0)
            {
                ...
                if (puser.Doctor != null && puser.Doctor.GradeNo <= 0)
                {
                    ZhiFang.Common.Log.Log.Error("传入的HIS医生编码为:" + hisDoctorCode + ",在LIS获取PUser的编码为:" + puser.Id + "!重新获取医生等级信息");
        ...
        private void GetCurUserInfo(string hisWardId, string hisDeptCode, string hisDoctorCode, PUser puser, ref BaseResultDataValue brdv)
        {
            ...
            if (puser.Doctor != null)
            {
                ...
                if (puser.Doctor.GradeNo>0)
                {
                    BlooddocGrade docGrade = IDBlooddocGradeDao.Get(puser.Doctor.GradeNo);
                    if (docGrade != null)
                    {
                    ...
                    {
                        ZhiFang.Common.Log.Log.Debug("HIS传入的医生编码为:" + hisDoctorCode + ",在LIS未设置医生的所属等级信息.GetCurUserInfo.puser.Doctor");
                    }
                }
                else
                {
                    ZhiFang.Common.Log.Log.Debug("HIS传入的医生编码为:" + hisDoctorCode + ",在LIS未设置医生的所属等级信息.GetCurUserInfo.Doctor3");
                }
            }
        ...
        public void AddSCOperation(PUser serverEntity, string[] arrFields, long empID, string empName)
        {
            ...
            EditGetUpdateMemo<PUser>(serverEntity, this.Entity, this.Entity.GetType(), arrFields, ref strbMemo);
