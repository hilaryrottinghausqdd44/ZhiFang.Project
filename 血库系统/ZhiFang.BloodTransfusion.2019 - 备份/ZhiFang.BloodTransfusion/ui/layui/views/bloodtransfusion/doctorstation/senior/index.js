...
layui.extend({
	...
	breqFormSeniorTable: '/views/bloodtransfusion/doctorstation/senior/breqFormSeniorTable',
...
}).use(['uxutil', 'dataadapter', 'layer', 'form', 'table', 'cachedata', 'runParams', 'bloodsconfig', 'bloodBreqSearchForm','breqFormSeniorTable', 'breqItemTable', 'breqFormResultTable'
], function() {
	"use strict";
	...
	var breqFormTable = layui.breqFormSeniorTable;
	...
	/***
	 * 医生审核等级验证处理 上级审核的用血申请量范围为(0-800)
	 * @param {Object} curRow
	...
	function onPass(curRow) {
		...
		onReview(curRow, 3, function(result) {
	...
	function onReturn(curRow) {
		...
		onReview(curRow, 4, function(result) {
	...
	function getBreqFormTableConfig() {
		...
		//同一患者一天内(上级审核通过)申请备血量少于800毫升的
		//defaultWhere.push("bloodbreqform.ReqTotal<800");
	...
	function initVerification(callback) {
		...
		if (!sysCurUserInfo) {
			...
			lowLimit = parseFloat(lowLimit);
			if (upperLimit < seniorGrade.UpperLimit) { //seniorGrade.LowLimit != lowLimit && seniorGrade.UpperLimit != upperLimit				
				info.push('当前医生审核的用血申请量范围为:' + lowLimit + '~' + upperLimit + ',不能进行上级审核操作!');
			}
		}
		if (info.length > 0) {
