 ...
 * @version 2020-03-13
 */
Ext.define('Shell.class.blood.nursestation.bloodbagretrieve.App', {
	...
	createItems: function() {
		...
		//发血主单列表
		me.OutDocGrid= Ext.create('Shell.class.blood.nursestation.bloodbagretrieve.out.DocGrid', {
			region: 'west',
			width: width1,
			header: true,
			itemId: 'OutDocGrid',
		...
		//右侧区域信息
		me.ShowPanel = Ext.create("Shell.class.blood.nursestation.bloodbagretrieve.ShowPanel", {
			region: 'center',
			header: true,
			width: width2,
			itemId: 'ShowPanel'
	...
	addBloodBagRetrieve: function() {
		...
		if (maxWidth > 1366) {
			maxWidth = document.body.clientWidth * 0.76;
		}else{
			maxWidth = document.body.clientWidth * 0.92;
		}
		var height1 = document.body.clientHeight;
		if (height1 > 768) {
			height1 = document.body.clientHeight * 0.82;
		}else{
			height1 = document.body.clientHeight * 0.95;
		}
		var src = JShell.System.Path.ROOT + "/ui/layui/views/bloodtransfusion/nursestation/bloodbagretrieve/index.html";
		if(me.defaultParams){
			var arr = [];
