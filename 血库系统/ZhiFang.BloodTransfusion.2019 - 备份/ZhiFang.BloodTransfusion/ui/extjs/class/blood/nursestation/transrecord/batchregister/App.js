/**
 * @description 输血过程记录批量修改入口
 * @description 分步批量登记(分为输血结束前登记及输血结束登记)
 * @author longfc
 * @version 2020-03-23
 */
Ext.define('Shell.class.blood.nursestation.transrecord.batchregister.App', {
	...
	/**修改服务地址*/
	editUrl: '/BloodTransfusionManageService.svc/BT_UDTO_UpdateBatchTransFormAndDtlList',
	...
	createItems: function() {
		var me = this;
		if (!me.outDtlIdStr) me.getOutDtlIdStr();
		...
		//输血过程记录项信息
		me.TransPanel = Ext.create('Shell.class.blood.nursestation.transrecord.batchregister.TransPanel', {
			region: 'center',
			header: false,
			split: true,
			itemId: 'TransPanel',
		...
		//不良反应相关信息
		me.AdverseReactionsPanel = Ext.create(
			"Shell.class.blood.nursestation.transrecord.batchregister.AdverseReactionsPanel", {
				region: 'east',
				width: width2,
				header: false,
				split: true,
	...
	isAdd: function() {
		...
		me.formtype = "add";
		me.outDtlIdStr = null;
		me.TransPanel.outDocId = me.outDocId;
	...
	isEdit: function(outDtlIdStr) {
		...
		me.formtype = "edit";
		me.TransPanel.outDocId = me.outDocId;
		me.TransPanel.outDtlIdStr = outDtlIdStr;
		me.TransPanel.outDtlRrecords = me.outDtlRrecords;
		me.TransPanel.isEdit(outDtlIdStr);

		me.AdverseReactionsPanel.PK = outDtlIdStr;
		me.AdverseReactionsPanel.outDtlRrecords = me.outDtlRrecords;
		me.AdverseReactionsPanel.isEdit(outDtlIdStr);
	},
	/**
	 * 保存按钮点击处理方法
	 * @param {Object} p
	 * @param {Object} docForm 输血过程记录主单表单
	 * @param {Object} dtlForm输血过程病人体征记录项表单
	 */
	onSave: function(p, docForm, dtlForm) {
		var me = this;
		//验证通过后处理
		if (!docForm.getForm().isValid()) {
			JShell.Msg.error("输血过程记录录入信息有误,请核对后再提交!");
			return;
		}
		//验证录入的核对人信息是否正确
		var result3=docForm.validityCheckInfo();
		if(result3.isCheck==false){
			JShell.Msg.error(result3.info);
			return;
		}
		...
		if (docInfo.entity.BeforeCheck1 && docInfo.entity.BeforeCheck2) {
			if (docInfo.entity.BeforeCheck1 === docInfo.entity.BeforeCheck2) {
				JShell.Msg.error("输血前核对人1及输血前核对人2,不能是同一个人!");
		...
		if (docInfo.entity.TransCheck1 && docInfo.entity.TransCheck2) {
			if (docInfo.entity.TransCheck1 === docInfo.entity.TransCheck2) {
				JShell.Msg.error("输血时核对人1及输血时核对人2,不能是同一个人!");
		...
		//输血登记方式:1:不区分输血结束前及输血结束的批量登记;2:输血结束前批量登记;3:输血结束批量登记(记录项结果值按本次提交进行覆盖更新)
		saveInfo.addType = "2";
		if (me.transTypeId == "2") {
			saveInfo.addType = "3";
		}
		...
		//当前批量选择的发血血袋信息
		saveInfo.outDtlList = me.getOutDtlList();
	...
	getOutDtlList: function() {
		...
		for (var i = 0; i < me.outDtlRrecords.length; i++) {
			...
			var reqFormId = outDtl.get("BloodBOutItem_BloodBReqForm_Id");
			if (reqFormId) {
			...
			var outFormId = outDtl.get("BloodBOutItem_BloodBOutForm_Id");
			if (outFormId) {
			...
			var bloodstyleId = outDtl.get("BloodBOutItem_Bloodstyle_Id");
			if (bloodstyleId) {
				entity.Bloodstyle = {
					Id: bloodstyleId,
