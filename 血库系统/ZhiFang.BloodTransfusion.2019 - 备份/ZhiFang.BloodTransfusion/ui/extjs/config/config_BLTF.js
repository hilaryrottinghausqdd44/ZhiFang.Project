...
if (!JcallShell.BLTF.RunParams) {
	JcallShell.BLTF.RunParams = {
		...
		initRunParams: function(callback) {
			...
			//用血申请传入的患者参数非空字段
			JcallShell.BLTF.RunParams.getRunParamsValue("NotNullParamField", false, function(data) {
				count++;
				JcallShell.BLTF.RunParams.getRunParamsAfter(callback, count);
			});
			//系统运行参数"列表默认分页记录数"
			...
			//获取几天内的LIS检验结果
			JcallShell.BLTF.RunParams.getRunParamsValue("GetLisResultDays", false, function(data) {
				count++;
				JcallShell.BLTF.RunParams.getRunParamsAfter(callback, count);
			});
			//结果为空时默认值
			JcallShell.BLTF.RunParams.getRunParamsValue("LisDefaulltItemsResult", false, function(data) {
				count++;
				JcallShell.BLTF.RunParams.getRunParamsAfter(callback, count);
			});
			//紧急用血是否在用血申请确认提交时上传HIS
			JcallShell.BLTF.RunParams.getRunParamsValue("IsBUseTimeTypeIDAUToUHis", false, function(data) {
				count++;
				JcallShell.BLTF.RunParams.getRunParamsAfter(callback, count);
			});
			//用血申请审核完成后是否返回给HIS
			JcallShell.BLTF.RunParams.getRunParamsValue("ReviewCompletedIsToHIS", false, function(data) {
				count++;
				JcallShell.BLTF.RunParams.getRunParamsAfter(callback, count);
			});
		},
