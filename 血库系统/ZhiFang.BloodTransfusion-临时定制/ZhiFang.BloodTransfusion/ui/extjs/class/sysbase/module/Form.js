...
Ext.define('Shell.class.sysbase.module.Form', {
	...
	/**获取数据服务路径*/
	selectUrl: '/RBACService.svc/RBAC_UDTO_SearchRBACModuleById?isPlanish=true',
	/**新增服务地址*/
	addUrl: '/RBACService.svc/RBAC_UDTO_AddRBACModule',
	/**修改服务地址*/
	editUrl: '/RBACService.svc/RBAC_UDTO_UpdateRBACModuleByField',
	/**上传图片文件服务地址*/
	uploadFileUrl: '/ConstructionService.svc/ReceiveModuleIconService',
	...
	createItems: function() {
		...
		items.push( {
			...
			y: 10,
			fieldLabel: '中文名称',
			name: 'RBACModule_CName',
			itemId: 'RBACModule_CName',
			emptyText: '必填项',
			allowBlank: false
		}, {
			x: 10,
			y: 35,
			fieldLabel: '英文名称',
			name: 'RBACModule_EName'
		}, {
			x: 10,
			y: 60,
			fieldLabel: '拼音字头',
			name: 'RBACModule_PinYinZiTou'
		});
		...
		items.push({
			...
			y: 10,
			fieldLabel: '模块简称',
			name: 'RBACModule_SName'
		}, {
			x: 190,
			y: 35,
			fieldLabel: '显示次序',
			name: 'RBACModule_DispOrder',
			xtype: 'numberfield',
			value: 0,
			allowBlank: false
		}, {
		...
		items.push({
				...
				hidden: true,
				value: me.ParentID,
				name: 'RBACModule_ParentID',
				itemId: 'RBACModule_ParentID'
			},
		...
		items.push( {
			fieldLabel: '时间戳',
			name: 'RBACModule_DataTimeStamp',
			hidden: true
		}, {
			fieldLabel: '图片名称',
			name: 'RBACModule_PicFile',
			itemId: 'RBACModule_PicFile',
			hidden: true,
			value: me.ModuleIconList[0][1]
		}, {
			fieldLabel: '机构ID',
			name: 'RBACModule_LabID',
			itemId: 'RBACModule_LabID',
			hidden: true,
			value: me.LabId
		}, {
			fieldLabel: '拥有者',
			name: 'RBACModule_Owner',
			itemId: 'RBACModule_Owner',
			hidden: true,
			value: me.Owner
		});
	...
	isAdd: function() {
		var me = this;
		me.callParent(arguments);
		me.getComponent('RBACModule_Id').setReadOnly(false);
	},
	isEdit: function(id) {
		var me = this;
		me.callParent(arguments);
		me.getComponent('RBACModule_Id').setReadOnly(true);
	},
	isShow: function(id) {
		var me = this;
		me.callParent(arguments);
	...
	getAddParams: function() {
		...
		var entity = {
			...
			ParentID: values.RBACModule_ParentID,
		...
		if (values.RBACModule_Id) entity.Id = values.RBACModule_Id;
		return {
			entity: entity
		};
	},
