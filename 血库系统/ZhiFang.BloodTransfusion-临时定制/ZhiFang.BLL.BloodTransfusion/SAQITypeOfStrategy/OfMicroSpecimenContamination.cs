...
{
    /// <summary>
    /// 质量指标分类类型图表数据处理-微生物标本污染
    /// </summary>
    class OfMicroSpecimenContamination : QITypeOfEChartStrategy
    {
        public override BaseResultDataValue GetSAQualityIndicatorTypeOfEChart(IList<SPSAQualityIndicatorType> saList, string qIndicatorType, string sadimension)
        {
            ...
            {
                if (sadimension == MicroSpecimenContamination.就诊类型.Key)
                {
                    axisDataValue = vo.SickTypeCName;
                }
                else if (sadimension == MicroSpecimenContamination.就诊类型采样人.Key)
                {
                    axisDataValue = vo.SickTypeCName + "_" + vo.Collecter;
                }
                else if (sadimension == MicroSpecimenContamination.就诊类型样本类型.Key)
                {
                ...
                else if (sadimension == MicroSpecimenContamination.样本类型.Key)
                {
                    axisDataValue = vo.SampleTypeCName;
                }
                else if (sadimension == MicroSpecimenContamination.样本类型科室.Key)
                {
                    axisDataValue = vo.SampleTypeCName + "_" + vo.DeptCName;
                }
                else if (sadimension == MicroSpecimenContamination.样本类型采样人.Key)
                {
                    axisDataValue = vo.SampleTypeCName + "_" + vo.Collecter;
                }

                else if (sadimension == MicroSpecimenContamination.科室.Key)
                {
                    axisDataValue = vo.DeptCName;
                }
                else if (sadimension == MicroSpecimenContamination.科室样本类型.Key)
                {
                    axisDataValue = vo.DeptCName + "_" + vo.SampleTypeCName;
                }
                else if (sadimension == MicroSpecimenContamination.科室采样人.Key)
                {
                    axisDataValue = vo.DeptCName + "_" + vo.Collecter;
                }

                else if (sadimension == MicroSpecimenContamination.采样人.Key)
                {
                    axisDataValue = vo.Collecter;
                }
                else if (sadimension == MicroSpecimenContamination.按年份.Key)
                {
                    axisDataValue = vo.Year;
                }
                else if (sadimension == MicroSpecimenContamination.按季度.Key)
                {
                    axisDataValue = vo.Quarter;
                }
                else if (sadimension == MicroSpecimenContamination.按月份.Key)
                {
        ...
        public override IList<SPSAQualityIndicatorType> GetSAQualityIndicatorTypeOfList(IList<SPSAQualityIndicatorType> nfList, string qitype, string sadimension, string startDate, string endDate)
        {
            ...
            else if (sadimension == MicroSpecimenContamination.就诊类型科室.Key)
            {
