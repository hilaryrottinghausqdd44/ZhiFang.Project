...
<%@ Page language="c#" AutoEventWireup="True" Inherits="OA.RBAC.Organizations.Deptlisthr" Codebehind="Deptlisthr.aspx.cs" %>
...
<HTML>
  <HEAD>
		...
		<script language="javascript" for="Toolbar1" event="onbuttonclick">
			...
			{
				case 'new':
					window.open('personaddhr.aspx?Id=0&RBACModuleID=<%=RBACModuleID%>&DeptId=<%=id%>','','width=590px,height=530px,resizable=yes,left=' + (screen.availWidth-740)/2 + ',top=' + (screen.availHeight-540)/2 );	
					break;
				...
				case 'Remove':
					...
					break;
				case 'newdept':
					window.open('Deptadd.aspx?Id=<%=id%>&RBACModuleID=<%=RBACModuleID%>','','width=450,height=260,top=200,left=200');
					break;
				case 'viewdept':
					window.open('Deptinfo.aspx?id=<%=Request.QueryString["id"] + ""%>&RBACModuleID=<%=RBACModuleID%>','','width=450,height=260,top=200,left=200');
					break;
				case 'deldept':
				...
				case 'viewrole':
					...
					{						
						window.open ('EmployeeRole.aspx?Id=' + SelEmpl,'','width=500px,height=430px,scrollbars=0,top=200,left=300');
					}
					break;
				case 'delempl':
					if (SelEmpl == '')
		...
		var account='';
			...
			{%>
				if(eid!='')
					window.open('personaddhr.aspx?Id=' + eid + '&RBACModuleID=<%=RBACModuleID%>','','width=700px,height=540px,resizable=yes,left=' + (screen.availWidth-740)/2 + ',top=' + (screen.availHeight-540)/2 );
			<%}%>
			}
			function ShowTree()
			{
