...
function getText() {
    var text;
    text = eWebEditor.document.body.innerText;
    return text;
}
...
function ExpandClose() {
    if (!isTableSelected()) {
        ShowDialog('dialog/ExpandClose.htm', 450, 385, true);
    }
    else {
        alert('请将折叠或展开的区域放入一个表格并选定');
    }
}

//<div class="Btn" title="插入新闻栏目" onclick="InsertNewsCatagory()"><img class="Ico" src="buttonimage/standard/III.gif"></div>
//插入新闻栏目
function InsertNewsCatagory() {
    ShowDialog('/OA/News/Browse/SelectNewsCatagoryForm.aspx', 450, 385, true);
}

//<div class="Btn" title="插入系统数据" onclick="InsertAppSystemData()"><img class="Ico" src="buttonimage/standard/DDD.gif"></div>
//插入单表系统数据
function InsertAppSystemData() {
    ShowDialog('/OA/News/Browse/SelectAppSystemForm.aspx', 450, 385, true);
}

//插入单表系统配置项OBJECT classid=\"clsid:1C6AAEBC-24A5-40CD-A653-3774CF44B234\"
function InsertAppSystemConfig() {
    //var paramXML = "<CUSTOMPARAM><F1>1111_edit</F1><F2>2222_edit</F2><F3>3333333_edit</F3></CUSTOMPARAM>";
    var url = "/OA/News/Browse/SetObjectParamForm.aspx"; //?param=" + paramXML;
    ShowDialog(url, 750, 600, true);
}


//控件的属性页
function SetObjectAttribute() {
    //var selection = eWebEditor.document.selection;
    //var cr = selection.createRange();
    //var objectXml = cr.item(0).outerHTML;
    //var paramXML = "<CUSTOMPARAM><F1>1111_edit</F1><F2>2222_edit</F2><F3>3333333_edit</F3></CUSTOMPARAM>";
    var url = "/OA/News/Browse/SetObjectParamForm.aspx"; //?param=" + paramXML;
    var ret = window.showModalDialog(url, '', 'dialogWidth:588px;dialogHeight:618px;help:no;scroll:no;status:no');
    if (ret != null)
        insertHTML(ret);
