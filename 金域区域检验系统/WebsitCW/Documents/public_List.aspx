<%@ Page language="c#" Codebehind="public_List.aspx.cs" AutoEventWireup="True" Inherits="OA.Documents._public_List" %>
...
<link rel="stylesheet" type="text/css" href="../explorer.css">
	...
	<A class="code" onclick="if(window.dd &amp;&amp; dd.elements &amp;&amp; dd.elements.fateba_se.children.length > 0) dd.elements.fateba_se.detachChild('ko5_st');return false;"
		...
		<TABLE id="TableTilte" style="display:none;BORDER-TOP-WIDTH: 1px; BORDER-LEFT-WIDTH: 1px; Z-INDEX: 101; BORDER-LEFT-COLOR: #0066ff; BORDER-BOTTOM-WIDTH: 1px; BORDER-BOTTOM-COLOR: #0066ff; WIDTH: 752px; BORDER-TOP-COLOR: #0066ff; HEIGHT: 22px; BORDER-RIGHT-WIDTH: 1px; BORDER-RIGHT-COLOR: #0066ff"
			cellSpacing=0 cellPadding=0 width="248" align="center" border="0">
			<tr>
				<td>
				
				<h4>位置: <font color=#cc6633><%=ShareEmpName%></font><font color=#7d7d7c size=3 face="宋体"><input type=hidden value='<%=filePath.Replace("\\\\","\\")%>' id="idfolder"/></font></h4>
				
				</td>
			</tr>
			<tr>
				<td>
				<a href="#" onclick="openwin(1)">图标</a>   <a href="#" onclick="openwin(2)">详细信息</a>
				</td>
				...
				<TD align="center">
					...
					<div id="Div<%=i.ToString()%>a" title="<%=dr[0].ToString()%>"
						...
						%> 
							...
							ondblclick="OpenDir('<%=strfiletype%>','<%=dr[0].ToString()%>','<%=folder%>','<%=x%>','<%=secID%>','<%=SendShareName%>','<%=SendShareNamelen%>','<%=thrID%>');" 
		...
		<script language="javascript">
					...
					function OpenDir(filetype,Type,folder,q,secID,sharename,SendShareNamelen,thrID)
					{
						if(Type=="...")
						{	
							q--;
						...
						//window.open('?folder='+folder+'&x='+q+'&secID='+secID+'&SendShareName='+sharename+'&SendShareNamelen='+SendShareNamelen+'&thrID='+thrID,'_self')
					
					window.open('?filetype='+filetype+'&&folder='+escape(folder),'_self')
					}
					</script>
		...
		<form id="frmContent" name="frmContent" method="post" encType="multipart/form-data" runat="server">
				...
				<TABLE id="addfile" border=0 cellPadding=0 align=center style="HEIGHT: 12px; DISPLAY:none " cellspacing=0>
					<TBODY>
