<%@ Page language="c#" Codebehind="NewsList.aspx.cs" AutoEventWireup="True" Inherits="OA.Documents.NewsList" %>
        ...
        string id = dtOANews.Rows[i]["id"].ToString();												
						%>
							<tr>
								...
								<td nowrap><div style="cursor:hand" onclick="javascript:openWinPositionSize('../news/browse/eachnews.aspx?id='+<%=id%>,'<%=strWinPositionSize%>','<%=id%>' );" 
								class="style-text" title="<%=dtOANews.Rows[i]["title"].ToString()%>"><%=StringLength(dtOANews.Rows[i]["title"].ToString(), CharacterNum)%></div>
								</td>
							</tr>
						<%}}	%>
						</table>
