<%@ Page language="c#" AutoEventWireup="True" Inherits="OA.DBQuery.DataRight.FieldPermissionSelect" Codebehind="FieldPermissionSelect.aspx.cs" %>
<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.0 Transitional//EN" >
<HTML>
	<HEAD>
		<title>FieldPermissionSelect</title>
					...
					case "btnDepartment":
						...
						if (result != 'undefined' && typeof(result)!='undefined')
						{
							...
							{	//PostModules.txtRoleTypeName.value='按部门分配';
								...
								window.parent.frames["FieldSetting"].location = "FieldAuth.aspx?<%=Request.ServerVariables["Query_String"]%>&RoleID=" + txtID.value + "&RoleType=department/"+txtName.value;
								return true;
							}
							else
							{
					...
					case "btnPost"://岗位
						...
						if (result != 'undefined' && typeof(result)!='undefined')
						{
							...
							{	//PostModules.txtRoleTypeName.value='按岗位分配';
								...
								window.parent.frames["FieldSetting"].location = "FieldAuth.aspx?<%=Request.ServerVariables["Query_String"]%>&RoleID=" + txtID.value + "&RoleType=post/"+txtName.value;
								return true;
							}
							else
							{
					...
					case "btnJob"://职位
						...
						if (result != 'undefined' && typeof(result)!='undefined')
						{
							...
							{	//PostModules.txtRoleTypeName.value='按职位分配';
								...
								window.parent.frames["FieldSetting"].location = "FieldAuth.aspx?<%=Request.ServerVariables["Query_String"]%>&RoleID=" + txtID.value + "&RoleType=duty/"+txtName.value;
								return true;
							}
							else
							{
					...
					case "btnEmployee":
						...
						if (result != 'undefined' && typeof(result)!='undefined')
						{
							...
							{	//PostModules.txtRoleTypeName.value='按员工分配';
								...
								window.parent.frames["FieldSetting"].location = "FieldAuth.aspx?<%=Request.ServerVariables["Query_String"]%>&RoleID=" + txtID.value + "&RoleType=employee/"+txtName.value;
								return true;
							}
							else
							{
