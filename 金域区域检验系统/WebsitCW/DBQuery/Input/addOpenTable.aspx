...
function FireQuery(obj)
{
	switch(obj.id)
	{
	    case 'BSave':
	        ...
	        //ConfirmNotAllowNull(this.frames["Right"].Form1.document.all['TableData'].childNodes);   //验证所有表单不为空
	        ...
	        if (Bconfirm) {
	            ...
	            var filename = OA.DBQuery.Input.addOpenTable.CreateFileName();           //获取服务器端的GUID文件名
	            //alert(filename.value)
	            if (window.frames["Right"].document.getElementById("$$Desig_" + modName))           //查看模板中是否有$$Desig_模块名控件 xiaod edit  07/07/04
	            {
	                window.frames["Right"].document.getElementById("$$Desig_" + modName).value = filename.value;
	            }
	            else {
	                alert('系统找不到$$Desig控件，请联系管理员！');
	                return;
	            }
	            ...
	            //alert(myDataRun);
	            //数据存入frames["Right"].Form1.document.all['hQueryCollection']中
	            this.frames["Right"].Form1.submit();
