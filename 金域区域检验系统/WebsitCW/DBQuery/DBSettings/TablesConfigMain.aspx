...
<%@ Page language="c#" AutoEventWireup="True" Inherits="Zhifang.Utilities.Query.DBSetting.TablesConfigMain" Codebehind="TablesConfigMain.aspx.cs" %>
...
<HTML>
	<HEAD>
		...
		<script language="javascript" type="text/javascript">
			//判断输入的字段是否合法：必须输入内容，不能重名（不区分大小写）【黄中福加】
			function IsSameName()
			{
			    //先判断有没有定义主键
				...
				for(var i=1; i<rowsNum; i++)
				{					
					var fieldName = document.Form1.all["txtBoxEN" + i].value;
					fieldName = fieldName.replace(/ /g,'');//fieldName = fieldName.Trim();
									
					//判断有没有输入字段名称和字段描述
					if(fieldName == "")
					{
					    alert("请输入字段英文名！");
					    document.Form1.all["txtBoxEN" + i].value = fieldName;
					    document.getElementById("txtBoxEN" + i).focus();
					    return false;
					}
					var fieldDesc = document.Form1.all["txtBoxCN" + i].value;
					fieldDesc = fieldDesc.replace(/ /g,'');
					if(fieldDesc == "")
					{
					    alert("请输入字段中文名！");
					    document.Form1.all["txtBoxCN" + i].value = fieldDesc;
					    document.getElementById("txtBoxCN" + i).focus();
					    return false;
					}
					//判断输入字段名称和字段描述是否重名
					fieldName = "||" + fieldName.toLowerCase() + "||";
					if(allFieldName.indexOf(fieldName) >= 0)
					{
					    alert("该字段英文名已经存在，请修改！");
					    document.getElementById("txtBoxEN" + i).focus();
					    return false;
					}
					else
					...
					if(allFieldDesc.indexOf(fieldDesc) >= 0)
					{
					    alert("该字段中文名已经存在，请修改！");
					    document.getElementById("txtBoxCN" + i).focus();
					    return false;
					}
					else
					{
					    allFieldDesc += fieldDesc;
					}
				}
			    return true;
			}
...
//****************************改变某类型的值，触发的事件****************************************
			...
			function superEdit()
			{
				document.Form1.all["btnSave"].disabled=true;
				;
				con = window.confirm("如果更改没有保存,进入高级代码设置将丢失,确认先保存了更改。确定要继续吗？");
				if(con == false)
					return false;
				
				var r = window.open("../Components/xmlEdit.aspx?FilePath=<%=fileCodeEditPath.Replace("\\","\\\\")%>", "", "fullscreen = yes;toolbar=no;statusbar=no ");
				//document.location = document.location;
			...
			function SetDefaultValue(obj)
			{
				...
				switch(objSelectValue)
				{
					...
					case "4":
						...
						var DlgRtnValue = window.showModalDialog("../Admin/PromptSelectDialog.aspx?WinOpenDefaultValueTemplet.aspx?DefaultValue="+ escape(objValue), "", "status:yes;resizable:yes;dialogHeight:560px;dialogWidth:780px;center:yes");
						if(DlgRtnValue != void 0)
						{
							obj.value = DlgRtnValue;
						}
			...
			function selectDB()
			{
			     var url = "../../DataInput/SelectFieldFromDatabaseByDatabaseName.aspx" +
                "?systemName=" +'<%=Request.QueryString["db"] %>' +
                "&dbName=" + '<%=Request.QueryString["db"] %>' +
                "&tableName=" + '<%=Request.QueryString["TableEName"] %>';
                window.open(url,'_blank','width=800,height=600,resizable=yes,scrollbars=yes,titlebar=no');
			}
			//========================End======================
			function ColumnEdit(name, username, password,database,TableName)
			{
			    ...
			    alert('请小心操作，只修改字段类型，长度，说明等，不要更改数据库的关键名称，位置等信息'
	...
	<body>
		<form id="Form1" method="post" runat="server">
			<!-- span 中的id不能改和删除 -->
			<P><FONT face="宋体"><IMG src="../images/icons/0041_a.gif">标题:一个</FONT><FONT face="黑体" size="5"><span id="tableName"><%Response.Write(Request.QueryString["TableName"]);%></span></FONT></P>
			<P>&nbsp;<input id="newField" type="button" value="新增字段" name="newField" runat="server" onserverclick="newField_ServerClick">&nbsp;
				...
				<asp:button id="btnSaveToDB" runat="server" Text="保存到数据库" onclick="btnSaveToDB_Click"/>
				
                <input type="button" id="btnSelectDB" value="从数据库添加字段" onclick="selectDB()" <%=strDisabled %>/> 
				<INPUT id="buttSuperEdit" type="button" value="高级设置" onclick="superEdit()">
			...
			<p>
				<table id="Table2" cellSpacing="1" cellPadding="0" width="50%" bgColor="#ffffff" border="1">
					...
					<tr>
						...
						<td><asp:dropdownlist id="dropListDate" Runat="server" Width="175">
