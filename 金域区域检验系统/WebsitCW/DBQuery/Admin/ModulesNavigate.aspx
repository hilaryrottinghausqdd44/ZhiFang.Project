		...
		<form id="FormDelmoban" onsubmit="return VerifyFormEmployee()" method="post" runat="server">
			<TABLE id="Table1" style=""
				cellSpacing="1" cellPadding="1" width="406" border="1">
					...
					<TR  height="30">
					<TD align="center"><B>导航配置</B>
					</TD>
					<TD>
						<asp:checkbox id="chkopen" runat="server"  Text="是否启用导航"></asp:checkbox> &nbsp;&nbsp;<asp:checkbox id="NavigateOpen" runat="server"  Text="弹出"></asp:checkbox>&nbsp;&nbsp;&nbsp;&nbsp;<asp:button id="btnSave" Runat="server" Text="保存" onclick="btnSave_Click"></asp:button>
					</TD>
				</TR>
					...
					if(CurrentNodeChilds!=null)
					{
						...
						%>
							<tr bgcolor=#ccccff><td colspan=3><%=node1.Attributes["Text"].Value%></td>
								...
								<% if (chilNode!=null)
								{ %>
								<td nowrap><%if(chilNode.ChildNodes.Count == 0){%><a href="#" onclick="javascript:DelGroup('<%=node1.Attributes.GetNamedItem("Text").InnerXml%>')">删除分类</a><%}%>&nbsp;</td>
								<%
								}
								%>
							...
							onmouseout="this.bgColor=''" title="<%=node.Attributes.GetNamedItem("Text").InnerXml%>">
								...
								<td><a href="#" onclick="javascript:Editmoban('<%=node1.Attributes.GetNamedItem("Text").InnerXml%>','<%=node.Attributes.GetNamedItem("Text").InnerXml%>','<%=node.Attributes.GetNamedItem("NavigateUrl").InnerXml.Replace("\\","\\\\")%>','<%=node.Attributes.GetNamedItem("ImageUrl").InnerXml%>')">修改</a></td>
