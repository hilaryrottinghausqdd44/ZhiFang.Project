...
var mpopTimer = 0;
mmc='#AFD2FA';mfc='MenuText';mmoc='#ffffff';mmic='#ffffff';mmobc='#AFD2FA';mmibc='#AFD2FA';mmioc='#000000';mmiic='#799310';movc='#004080';mie='0';msubedge='1';muw=54;miw=160;mmh=28;mmw='100%';mmenuadjust=0;mmenuadjustV=0;mfonts='font-family: ËÎÌו; font-size: 9pt; color: MenuText; ';mcursor='hand';
var swipeSteps = 4;
var swipemsec = 25;
...
function repeat(dir, index, steps, msec) {
	...
	if (el.swc == 0) {
		...
		switch (dir) {
			case 2:
				el.style.clip = "rect(" + height*el.swc/steps + "," + width + "," + height + "," + 0 + ")";
				el.style.top  = top - height*el.swc/steps;
				break;
			case 8:
				el.style.clip = "rect(" + 0 + "," + width + "," + height*(steps-el.swc)/steps + "," + 0 + ")";
				el.style.top  = top + height*el.swc/steps;
				break;
			case 6:
				el.style.clip = "rect(" + 0 + "," + width + "," + height + "," + width*(el.swc)/steps + ")";
				el.style.left  = left - width*el.swc/steps;
				break;
			case 4:
				el.style.clip = "rect(" + 0 + "," + width*(swipeSteps - el.swc)/steps + "," + height + "," + 0 + ")";
				el.style.left  = left + width*el.swc/steps;
				break;
		}
...
src.style.borderLeftColor=mmobc;
src.style.borderRightColor=mmibc;
src.style.borderTopColor=mmobc;
src.style.borderBottomColor=mmibc;
src.style.backgroundColor=mmoc;
src.style.color=movc;
}
function mtoin(src){

src.style.borderLeftColor=mmibc;
src.style.borderRightColor=mmobc;
src.style.borderTopColor=mmibc;
src.style.borderBottomColor=mmobc;
src.style.backgroundColor=mmic;
src.style.color=movc;
}
function mnochange(src){
src.style.borderLeftColor=mmc;
src.style.borderRightColor=mmc;
src.style.borderTopColor=mmc;
src.style.borderBottomColor=mmc;
...
function mallhide(){
	for(var nummenu=0;nummenu<mmenus.length;nummenu++){
        ...
        for(nummenu=1;nummenu<=mnumberofsub;nummenu++){  
        	...
        	document.all["mitem"+nummenu].style.color=mfc;
        	}
}
...
menuid.style.filter='Alpha(Opacity=95)';
...
if(miw+parseInt(menuid.style.left)>document.body.clientWidth+document.body.scrollLeft)
menuid.style.left=document.body.clientWidth+document.body.scrollLeft-miw;
...
if(item)document.all["mitem"+item].style.color=movc;
if(misdown||item){
	mtoin(srcel);
}
else{
...
function mwritetodocument(){
      var mwb=1;
                     var stringx='<div id="mposflag" style="position:absolute;"></div><table  id=mmenutable border=0 cellpadding=0 cellspacing=0 width='+mmw+' height='+mmh+' bgcolor='+mmc+
                     ' onselectstart="event.returnValue=false"'+
                     ' style="filter:Alpha(Opacity=95);cursor:'+mcursor+';'+mfonts+
                     ' border-left: '+mwb+'px solid '+mmobc+';'+
                     ' border-right: '+mwb+'px solid '+mmibc+'; '+
                     'border-top: '+mwb+'px solid '+mmobc+'; '+
                     'border-bottom: '+mwb+'px solid '+mmibc+'; padding:0px"><tr>'
                     for(var x=0;x<mmenus.length;x++){
                     	...
                     	stringx += "<td class=coolButton width="+thismenu.caption.length*6+"px id=mMenu"+x+" style='border: 1px solid "+mmc+
                     ...
                     for(var x=0;x<mmenus.length;x++){
                        ...
                        ' style="filter:Alpha(Opacity=95);cursor:'+mcursor+';position:absolute;'+
                        'width:'+miw+'px; z-index:'+(x+100);
                        if(mmibc!=mmobc&&msubedge=="0"){
                        stringx+=';border-left: 1px solid '+mmioc+
                        ';border-top: 1px solid '+mmioc;}
                        stringx+=';border-right: 1px solid '+mmibc+
                        ';border-bottom: 1px solid '+mmibc+';visibility:hidden" onselectstart="event.returnValue=false">\n'+
                     	'<table  width="100%" border="0" height="100%" align="center" cellpadding="0" cellspacing="2" '+
                     	'style="'+mfonts+' border-left: 1px solid '+mmobc;
                     	if(mmibc!=mmobc&&msubedge=="0"){
                     	stringx+=';border-right: 1px solid '+mmiic+
                     	';border-bottom: 1px solid '+mmiic;}
                     	stringx+=';border-top: 1px solid '+mmobc+
                     	';padding: 4px" bgcolor='+mmc+'>\n'
                     	for(var i=0;i<thismenu.items.length;i++){
                     		...
                     		stringx += "<tr><td id=mp"+thismenuitem.hasc+" class=coolButton style='border: "+mie+"px solid "+mmc+
                     		...
                     		stringx += "<tr><td class=coolButton style='border: "+mie+"px solid "+mmc+
...
stringx+='<div id=msubmenudiv'+j+
                        ' style="filter:Alpha(Opacity=95);tag:'+thisitem.level+';cursor:'+mcursor+';position:absolute;'+
                        'width:'+miw+'px; z-index:'+(j+200);
                        if(mmibc!=mmobc&&msubedge=="0"){
                        stringx+=';border-left: 1px solid '+mmioc+
                        ';border-top: 1px solid '+mmioc;}
                        stringx+=';border-right: 1px solid '+mmibc+
