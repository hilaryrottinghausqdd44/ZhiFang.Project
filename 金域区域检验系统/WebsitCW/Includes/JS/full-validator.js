...
(function(ns){
	...
	var $properties = "accept,action,as,code,depend,elements,empty,expression,for,format,level,max,maxElement,min,minElement,operator,options,pass,pattern,prop,regex,require,rule,tips,to,toElement,trim,warn".split(',');
	...
	ns.SummaryMessenger = new Class({
		...
		showPass : function(o){
			...
			else $(this.options.id).setStyle('display', '');
		},
		process : function(o){
			if(o.length){
	...
	ns.JsonReader = new Class({
		...
		initialize : function(form, options){
			this.options.url = this.options.url.replace('/@', location.pathname.replace(/\.[^\.]*$/, ''));
			this.form = form;
			this.setOptions(options);
			if (this.options.initialize) this.options.initialize.call(this);
			if(this.options.wait) window.addEvent('domready', this.read.bind(this));
		...
		read : function(){
			...
			var request = new Request[this.options.as]({url: this.options.url, onSuccess : function(data){
				if(Browser.loaded) self.wait(data);
				else window.addEvent('domready', self.wait.bind(self, data));
				request = null;
			}}).get();
		},
		wait : function(json){
	...
	var Validator = ns.Validator = {
		...
		toString : function(){return ['Version: 4.0.8 beta', 'Author: \u6211\u4f5b\u5c71\u4eba', 'Email: wfsr-at-msn.com'].join('\n');}
	};
	...
	ns.Form = new Class({
		...
		validate : function(){
			this.invalids.empty();
			this.batch = true;
			this.elements[this.options.step ? 'every' : 'each'](function(element){
		...
		loadConfigs : function(){
			...
			this.options.configs.split(util.comma).each(function(config, i){
				...
				if(config == 'xml' && form.options.xml.xpath) ns.XmlReader.prototype.options.xpath = {name : form.options.form, path : location.pathname};
				ns.Factory.Reader.build(config, form, form.options[config] || {});
				delete form.options[config];
			});
			...
			if(!this.form){
				...
				if(ns.contains.call(this.options.triggers, $triggers.submit)){
					...
					this.form.addEvent('submit', function(){
						return this.validate() && (oldEvent && oldEvent() || !oldEvent);
					}.bind(this));
				}
			}
			return this.form;
		...
		addElement : function(options){
			var ops = this.options;
			var element = options;
			this.elements.set(element.element.name, element);
		...
		remove : function(){
			var self = this;
			Array.flatten(arguments).each(function(name){
				var element = self.elements.get(name);
				element && self.elements.erase(name) && element.removeEvents() && element.detach();
			});
			return this;
		},
	...
	var AbstractElement = new Class({
		...
		getMessage : function(){
			return (this.$json ? this.$json.message : this.options[this.isValid ? 'pass' : (this.value === '' ? 'empty' : 'warn')]) || '\u672a\u914d\u7f6e\u63d0\u793a\u4fe1\u606f';
		},
		trim : function(text){
		...
		validate : function(){
			...
			if(ops.force || !(this.$value != undefined && this.value == this.$value)){
	...
	ns.GroupElement = new Class({
		...
		attach : function(validateOnBlur){
			...
			this._attach(validateOnBlur);
		},
		_attach  : function(validateOnBlur){
			var ops = this.options;
			if(ops.elements) {
			...
			if(this.element.get('tag') != 'input')return;
