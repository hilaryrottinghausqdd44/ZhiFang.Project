...
;!function(win){
	...
	var CLodop = {
		...
		//CLodop安装文件地址
		LodopFiLeUrl: '/ui/src/zhifang/print/Lodop6.226_Clodop4.142/CLodop_Setup_for_Win32NT_4.142CN_Extend.exe',//'/ui/src/zhifang/print/Lodop6.226_Clodop4.113/CLodop_Setup_for_Win32NT_4.113Extend.exe',
		//获取本文件的文件夹地址
	...
	//CLodop安装文件地址
	CLodop.LodopFiLeUrl = CLodop._getLocal() + CLodop.LodopFiLeUrl;
	...
	var ZFPrint = {
		...
		//pdf打印
		pdf:{
			//直接打印:urls(pdf地址数组),title(打印任务名称,可选),callback(回调函数),isEncode(url是否转码,默认true)，beforecallback:打印前回调函数
			print: function (urls, title, callback, isEncode, beforecallback){
			...
			//浏览打印:urls(pdf地址数组),title(打印任务名称,可选),callback(回调函数),isEncode(url是否转码,默认true)，beforecallback:打印前回调函数
			preview: function (urls, title, callback, isEncode, beforecallback){
				this._todo(urls, title, function () {
					beforecallback && beforecallback();
					LODOP.PREVIEW();
					//callback && callback();
		...
		//模型打印
		model:{
			//直接打印:contentList(lodop内容字符串数组),title(打印任务名称,可选),callback(回调函数),beforecallback(回调函数)
			print:function(contentList,title,callback,beforecallback){
			...
			_todo:function(contentList,title,callback){
				...
				ZFPrint._initLicenses(function(){
					...
					for(var i in list){
						LODOP.NEWPAGEA();
						eval(me._changeModelContent(list[i]));
					}
					callback();
				});
			},
			...
			_changeModelContent:function(content){
				var arr = content.split('LODOP.'),
					str = 'LODOP.PRINT_INIT',
					list = [];
				
				for(var i in arr){
					if(arr[i]){
						list.push('LODOP.' + arr[i]);
					}
				}
				for(var i in list){
