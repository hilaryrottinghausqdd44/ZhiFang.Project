layui.define(['table', 'jquery', 'form'], function (exports) {
    ...
    */
    tableSelect.prototype.render = function (opt) {
        ...
        opt.isHasCloseBtn = opt.isHasCloseBtn || false;//是否存在关闭按钮 -- zhangda-2021-04-23
        opt.isHideClearBtn = opt.isHideClearBtn || false;//是否隐藏清空按钮 -- zhangda-2021-06-15
        ...
        elem.off('click').on('click', function(e) {
            ...
            var tableBox = '<div class="tableSelect layui-anim layui-anim-upbit" style="left:'+l+';top:'+t+';border: 1px solid #d2d2d2;background-color: #fff;box-shadow: 0 2px 4px rgba(0,0,0,.12);padding:10px 10px 0 10px;position: absolute;z-index:66666666;margin: 5px 0;border-radius: 2px;min-width:400px;">';
                ...
                tableBox += '</form>';
            if (opt.isHasCloseBtn) tableBox += '<button style="float:right;margin-left:5px;" class="layui-btn layui-btn-sm layui-btn-primary tableSelect_btn_close">关闭</button>';
            if (!opt.isHideClearBtn) tableBox += '<button style="float:right;margin-left: 5px;" class="layui-btn layui-btn-sm layui-btn-primary tableSelect_btn_clear">清空<span></span></button>';
            ...
            var overWidth = (elem.offset().left + tableBox.outerWidth()) > $(window).width();
                //底部距离不够时 修改--zhangda--2021-04-23
                overHeight && tableBox.css({ 'top': (elem.offset().top - + tableBox.outerHeight() - $(window).scrollTop()) + "px" });
                //overHeight && tableBox.css({'top':'58px','bottom':'0px'});
               // overWidth && tableBox.css({ 'left': '30px', 'right': '5px' });
                //2021-11-19 修改left为auto  
                overWidth && tableBox.css({ 'left': 'auto', 'right': '5px' });
			
            //关键词搜索--2019-12-06 修改源码
            form.on('submit(tableSelect_btn_search)', function (data) {
                window.event.preventDefault();
                var name = "";
                var url = tableSelect_table.config.url;
                ...
                if (data.field[name] != "") {
                    ...
                    if (name.indexOf(",") != -1) {
                        ...
                        $.each(arr, function (a, item) {
                            if(str != "") str += " or "
                            str += item + " like '%" + data.field[name] + "%'"
                        });
                    } else {
                        str = name + " like '%" + data.field[name] + "%'"
                    }
                    if (url.indexOf("&where") == -1) {
                        url = encodeURI(url + "&where=" + str);
                    } else {
                        url = encodeURI(url.replace("&where=", '&where=(' + str + ") and "));
                    }
                }
            ...
            $(document).keydown(function (event) {
                switch (event.keyCode) {
                    case 13://回车
                        //判断焦点是否在该输入框
                        if (document.activeElement == document.getElementsByClassName("tableSelect_btn_search")) $(this).click();//查询
                        break;
                }
            });
            //双击行选中
