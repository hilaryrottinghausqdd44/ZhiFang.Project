...
layui.extend({
	...
	var sortlist = {
		...
		config:{
			...
			cols:[[
                ...
                { field:'LBEquipItemVO_LBEquipItem_DispOrderComm',width: 120,title: '双向发送次序',sort: true, edit: 'text',event: "setDispOrder"},
			...
			parseData:function(res){//res即为原始返回的数据
				...
				if(data && data.list){
					for(var i=0;i<data.list.length;i++){
                        ...
                        data.list[i].Temporary_DispOrder = data.list[i].LBEquipItemVO_LBEquipItem_DispOrderComm;
    ...
    Class.pt.getModifiedRecords =  function(){
	    ...
	    for(var i = 0;i<tableCache.length;i++){
	    	//找到修改过数据的行
            if (tableCache[i].Temporary_DispOrder != tableCache[i].LBEquipItemVO_LBEquipItem_DispOrderComm){
    ...
    Class.pt.updateOne =  function(index,obj){
   		...
   		setTimeout(function() {
            ...
            var DispOrderComm = obj.LBEquipItemVO_LBEquipItem_DispOrderComm;
            var entity ={
            	Id:id,
                DispOrderComm: DispOrderComm
            };
            var fields ="Id,DispOrderComm";
