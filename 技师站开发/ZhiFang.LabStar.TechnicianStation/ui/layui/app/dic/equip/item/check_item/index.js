layui.extend({
    ...
    app.url = {
        //获得左侧列表数据
        getLeftTableDataUrl: uxutil.path.ROOT + '/ServerWCF/LabStarBaseTableService.svc/LS_UDTO_SearchLBEquipItemVOByHQL?isPlanish=true&sort=[{"property": "LBItem_DispOrder","direction": "ASC"}]',
        //获取右侧列表数据
        getRightTableDataUrl: uxutil.path.ROOT + '/ServerWCF/LabStarBaseTableService.svc/LS_UDTO_SearchLBSectionItemVOByHQL?isPlanish=true&sort=[{property:"LBItem_DispOrder",direction:"ASC"}]',
        //保存服务
        saveUrl: uxutil.path.ROOT + '/ServerWCF/LabStarBaseTableService.svc/LS_UDTO_AddDelLBEquipItem',
    };
    //是否执行左侧列表中的done中部分内容
    ...
    app.fields = {
        ...
        LeftTableId: 'LBEquipItemVO_LBItem_Id',
        RightTableId: 'LBSectionItemVO_LBItem_Id'
    };
    //左侧列表查询列
    app.leftTableColumn = [
        'LBEquipItemVO_LBEquipItem_Id',
        'LBEquipItemVO_LBItem_Id',
        'LBEquipItemVO_LBItem_CName',
        'LBEquipItemVO_LBItem_SName',
        'LBEquipItemVO_LBItem_UseCode'
    ];
    //右侧列表查询列
    app.rightTableColumn = [
        'LBSectionItemVO_LBItem_Id',
        'LBSectionItemVO_LBItem_CName',
        'LBSectionItemVO_LBItem_SName',
        'LBSectionItemVO_LBItem_UseCode'
    ];
    //左侧列表初始数据
    ...
    app.init = function () {
        ...
        me.getParams();
        $("#CName").html(me.paramsObj.equipCName);
        me.initLeftTable();
        me.listeners();
    };
    //获得参数
    ...
    app.initLeftTable = function() {
        var me = this,
            url = me.url.getLeftTableDataUrl + "&fields=" + me.leftTableColumn.join() + "&where=lbequip.Id=" + me.paramsObj.equipID + "&t=" + new Date().getTime();
        ...
        table.render({
            ...
            cols: [
                [{
                    ...
                    field: 'LBEquipItemVO_LBItem_CName',
                    title: '项目名称',
                    minWidth: 130,
                    sort: false
                }, {
                    field: 'LBEquipItemVO_LBItem_SName',
                    title: '项目简称',
                    width: 130,
                    sort: false
                }, {
                    field: 'LBEquipItemVO_LBItem_UseCode',
                    title: '用户编码',
                    width: 100,
                    sort: false
                }
    ...
    app.initRightTable = function (url,options) {
        var me = this,
            ...
            url = url || me.url.getRightTableDataUrl + "&fields=" + me.rightTableColumn.join() + "&where=lbitem.GroupType=0 and lbitem.IsCalcItem <> 1 and lbsection.Id =" + me.paramsObj.sectionID + (me.leftTableData.length > 0 ? " and lbitem.Id not in(" + me.leftTableData.join() + ")" : "") + "&t=" + new Date().getTime();
        var config = {
            elem: '#rightTable',
            height: 'full-110',
            defaultToolbar: ['filter'],
            ...
            cols: [
                [{
                    ...
                    field: 'LBSectionItemVO_LBItem_CName',
                    title: '项目名称',
                    minWidth: 130,
                    sort: false
                }, {
                    ...
                    field: 'LBSectionItemVO_LBItem_UseCode',
                    title: '用户编码',
                    width: 100,
                    sort: false
                }]
    ...
    app.listeners = function () {
        ...
        form.on('submit(search)', function (data) {
            ...
            var where = "lbitem.GroupType = 0 and lbitem.IsCalcItem <> 1 and lbsection.Id =" + me.paramsObj.sectionID + (me.leftTableData.length > 0 ? " and lbitem.Id not in(" + me.leftTableData.join() + ")" : "");
            if (data.field.searchText != "") {//模糊查询
                var str = data.field.searchText;
                where += " and (lbitem.CName like '%" + str + "%' or lbitem.EName like '%" + str + "%' or lbitem.SName like '%" + str + "%' or lbitem.Shortcode like '%" + str + "%' or lbitem.UseCode like '%" + str + "%')";
            }
        ...
        table.on('sort(rightTable)', function (obj) {
            ...
            url += "&where=lbitem.GroupType = 0 and lbitem.IsCalcItem <> 1 and lbsection.Id = " + me.paramsObj.sectionID + (me.leftTableData.length > 0 ? " and lbitem.Id not in (" + me.leftTableData.join() + ")" : "");
            if (type == null) return;
            if (searchText != "") {//模糊查询
                var str = searchText;
    ...
    app.onAddClick = function (checkData) {
        ...
        for (var i = 0; i < checkData.length; i++) {
            ...
            if (flag) {
                var Obj = {
                    'LBEquipItemVO_LBItem_Id': checkData[i][app.fields.RightTableId],
                    'LBEquipItemVO_LBItem_CName': checkData[i].LBSectionItemVO_LBItem_CName,
                    'LBEquipItemVO_LBItem_SName': checkData[i].LBSectionItemVO_LBItem_SName,
                    'LBEquipItemVO_LBItem_UseCode': checkData[i].LBSectionItemVO_LBItem_UseCode,
                    'LBEquipItemVO_LBEquipItem_Id': checkData[i].LBEquipItemVO_LBEquipItem_Id
                };
    ...
    app.onSaveClick = function () {
        ...
        for (var i = 0; i < newData.length; i++) {
            ...
            if (addFlag) {
                ...
                addEntityList.push({
                    LBEquip: { Id: me.paramsObj.equipID, DataTimeStamp: DataTimeStamp },
