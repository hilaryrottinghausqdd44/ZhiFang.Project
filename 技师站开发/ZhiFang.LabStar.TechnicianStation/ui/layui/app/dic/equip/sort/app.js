...
layui.extend({
	uxutil: 'ux/util',
	sortlist:'app/dic/equip/sort/list'
    ...
    //获得传递参数
    function getParams() {
        var params = decodeURIComponent(location.search.split("?")[1]);
        //参数赋值
        if (params != "undefined") {
            paramsObj.searchWhere = "CName like '%" + params +
                "%' or Computer like '%" + params + "%' or SName like '%" +
                params + "%' or ComProgram like '%" +
                params + "%'";
        } else {
            paramsObj.searchWhere = "1=1";
        }
    ...
    var options = {
        ...
        height: 'full-50',
        size:'sm',
        where: { where: paramsObj.searchWhere },//需要修改部分
