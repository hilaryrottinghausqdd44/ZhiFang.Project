...
layui.extend({
	...
	//修改部分
	//获取列表服务地址
    var GET_LIST_URL = uxutil.path.ROOT + '/ServerWCF/LabStarBaseTableService.svc/LS_UDTO_SearchLBEquipByHQL?isPlanish=true&sort=[{property:"LBEquip_DispOrder",direction:"ASC"}]';
    //修改服务地址
    var GET_EDIT_URL = uxutil.path.ROOT + '/ServerWCF/LabStarBaseTableService.svc/LS_UDTO_UpdateLBEquipByField';
	...
	var sortlist = {
		...
		config:{
            ...
            //修改部分
                ...
                { field: 'LBEquip_Id', width: 150, title: '主键', sort: true, hide: true },
                { field:'LBEquip_DispOrder',width: 100,title: '显示次序',sort: true, edit: 'text',event: "setDispOrder"},
                { field:'LBEquip_CName', minWidth:150,flex:1, title: '名称', sort: true},
                { field:'LBEquip_EName', width:100,title: '英文名称', sort: true},
                { field:'LBEquip_SName', width:100,title: '简称', sort: true},
			...
			parseData:function(res){//res即为原始返回的数据
				...
				if(data && data.list){
					for(var i=0;i<data.list.length;i++){
                        ...
                        data.list[i].Temporary_DispOrder = data.list[i].LBEquip_DispOrder;//修改部分--当前文档直接替换LBEquip_DispOrder
    ...
    Class.pt.getModifiedRecords =  function(){
	    ...
	    for(var i = 0;i<tableCache.length;i++){
	    	//找到修改过数据的行
            if (tableCache[i].Temporary_DispOrder != tableCache[i].LBEquip_DispOrder){
	    		list.push(tableCache[i]);
	    	}
	    }
	    return list;
    };
    Class.pt.updateOne =  function(index,obj){//修改部分
   		var me = this;
   		setTimeout(function() {
   	        var id = obj.LBEquip_Id;
            var DispOrder = obj.LBEquip_DispOrder;
            var entity ={
            	Id:id,
                DispOrder: DispOrder
            };
