...
layui.extend({
	...
	//从已进行组合项目拆分的关系表获取组合项目信息
	var SELECT_URL = uxutil.path.ROOT +
		'/ServerWCF/LabStarBaseTableService.svc/LS_UDTO_SearchAlreadyLBParItemSplitPLBItemListByHQL';

	var lbItemTable = {
		searchInfo: {
			isLike: true,
			fields: ['lbitem.CName', 'lbitem.SName']
		},
		//参数配置
		config: {
			/**
			...
			/**是否自动选中行*/
			autoSelect: true,
			/**默认数据条件:GroupType:0=单项  1=组合 2=组套*/
			defaultWhere: 'lbitem.GroupType=1',
			/**内部数据条件*/
			internalWhere: '',
			/**外部数据条件*/
			externalWhere: '',
	...
	lbItemTable.getTableWhere = function() {
		...
		if (me.searchInfo) {
			var searchV = "";
			var search = $('#table_testitem_like_search');
