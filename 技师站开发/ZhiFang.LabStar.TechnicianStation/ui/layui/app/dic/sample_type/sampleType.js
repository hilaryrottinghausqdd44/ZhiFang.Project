...
layui.extend({
	...
	var tableObj = {
		table: layui.table,
		form: layui.form,
		idField: "LBSampleType_Id",
		fields:{
			Id:"LBSampleType_Id",
			IsUse:"LBSampleType_IsUse"
        },
        current: null,
        delIndex: null,
        addUrl: uxutil.path.ROOT +'/ServerWCF/LabStarBaseTableService.svc/LS_UDTO_AddLBSampleType',
        selectUrl: uxutil.path.ROOT + '/ServerWCF/LabStarBaseTableService.svc/LS_UDTO_SearchLBSampleTypeByHQL?isPlanish=true&sort=[{property:"LBSampleType_DispOrder",direction:"ASC"}]',
        updateUrl: uxutil.path.ROOT +'/ServerWCF/LabStarBaseTableService.svc/LS_UDTO_UpdateLBSampleTypeByField',
        delUrl: uxutil.path.ROOT +'/ServerWCF/LabStarBaseTableService.svc/LS_UDTO_DelLBSampleType',
        ...
        reset: function () { //重置
            if (formObj.type == 'edit' || formObj.type == 'show') {
                if (tableObj.checkRowData.length > 0) {
                    ...
                    if (tableObj.checkRowData[tableObj.checkRowData.length - 1].LBSampleType_IsUse == "true") {
    ...
    function tableRender() {
	    ...
	    tableObj.table.render({
		    ...
		    cols: [
                    ...
                    {
                        field: 'LBSampleType_SampleTypeNo',
                        title: '编码',
                        width: 80,
                        sort: true
                    },
				    ...
				    {
					    field: 'LBSampleType_EName',
					    title: '英文名',
                        minWidth: 130,
                        sort: true
                    },
                    {
                        field: 'LBSampleType_Shortcode',
                        title: '快捷码',
                        minWidth: 130,
                        sort: true
                    },
                    {
                        field: 'LBSampleType_PinYinZiTou',
                        title: '拼音字头',
                        minWidth: 130,
                        sort: true
                    },
		...
		switch(obj.event) {
            ...
            case 'import':
                layer.open({
                    ...
                    title: '选择导入文件',
                    area: ['400px', '160px'],
                    content: uxutil.path.LAYUI + '/views/system/comm/import/file/index.html?ENTITYNAME=LBSampleType',
    ...
    function getDispOrder() {
        uxutil.server.ajax({
            url: tableObj.getMaxNoByEntityFieldUrl + '?entityName=LBSampleType&entityField=DispOrder'
        }, function (res) {
            if (res.success) {
