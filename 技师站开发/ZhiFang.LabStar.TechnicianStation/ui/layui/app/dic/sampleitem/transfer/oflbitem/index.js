...
layui.extend({
	uxutil: '/ux/util',
	uxtable: '/ux/table',
	lefttable: 'app/dic/sampleitem/transfer/oflbitem/leftlist',
	righttable: 'app/dic/sampleitem/transfer/oflbitem/rightlist',
	...
	function getLeftTableConfig() {
		var where = "lbsampleitem.LBSampleType.Id=" + defaultParams.id;
		var leftobj = {
			elem: '#lefttable',
			title: '项目样本类型信息',
			height: 'full-85',
	...
	function getDefaultWhere(data) {
		...
		for (var i = 0; i < data.length; i++) {
			params.push(data[i]["LBSampleItem_LBItem_Id"]);
		}
	...
	function onRefreshRightTable(list) {
		...
		if (where) where = "lbitem.Id not in(" + where + ")";
	...
	function addLeft(curObj) {
		...
		for (var i = 0; i < addArr.length; i++) {
			var obj = addArr[i];
			var obj2 = {
				'LBSampleItem_LBItem_Id': obj.LBItem_Id,
				'LBSampleItem_LBItem_CName': obj.LBItem_CName,
				'LBSampleItem_LBItem_EName': obj.LBItem_EName,
				'LBSampleItem_LBItem_UseCode': obj.LBItem_UseCode,
				'LBSampleItem_LBSampleType_Id': defaultParams.id,
				'LBSampleItem_LBSampleType_CName': defaultParams.cname,
...
//		});
	...
	function delLeftRows(allData, delArr) {
		for (var i = 0; i < delArr.length; i++) {
			var obj1 = delArr[i]["LBSampleItem_LBItem_Id"];
			for (var j = 0; j < allData.length; j++) {
				var obj2 = allData[j]["LBSampleItem_LBItem_Id"];
				if (obj1 == obj2) {
					allData.splice(j, 1);
					break;
				}
