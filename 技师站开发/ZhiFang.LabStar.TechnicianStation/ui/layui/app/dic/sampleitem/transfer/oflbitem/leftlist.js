...
layui.extend({}).define(['uxutil', 'uxtable', 'table'], function(exports) {
		...
		config: {
			...
			cols: [
				[{
					...
					{
						field: 'LBSampleItem_LBItem_EName',
						title: '英文名称',
						width: 120,
						sort: true
					},
					{
						field: 'LBSampleItem_LBItem_UseCode',
						title: '用户编码',
						hide: true,
						width: 100,
						sort: true
	...
	var Class = function(setings) {
		...
		}, uxtable.config, lefttable.config, setings);
	...
	lefttable.getEntityList = function(defaultParams) {
		...
		for (var i = 0; i < leftData.length; i++) {
			...
			var entity2 = {
				Id: id,
				LBSampleType: {
					Id: leftData[i]["LBSampleItem_LBSampleType_Id"] || defaultParams.id,
					CName: leftData[i]["LBSampleItem_LBSampleType_CName"] || defaultParams.cname,
					DataTimeStamp: strDataTimeStamp
				},
				LBItem: {
					Id: leftData[i]["LBSampleItem_LBItem_Id"],
					CName: leftData[i]["LBSampleItem_LBItem_CName"],
					DataTimeStamp: strDataTimeStamp
				}
			}
