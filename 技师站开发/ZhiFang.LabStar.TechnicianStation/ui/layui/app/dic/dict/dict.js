...
layui.extend({
    uxutil: 'ux/util'
}).use(['uxutil', 'table', 'form', 'element', 'laydate','colorpicker'], function () {
    var $ = layui.jquery,
        layer = layui.layer,
        uxutil = layui.uxutil,
    ...
    var tableObj = {
        ...
        current: null,
        delIndex: null,
        addUrl: uxutil.path.ROOT + '/ServerWCF/LabStarBaseTableService.svc/LS_UDTO_AddLBDict',
        selectUrl: uxutil.path.ROOT + '/ServerWCF/LabStarBaseTableService.svc/LS_UDTO_SearchLBDictByHQL?isPlanish=true&sort=[{property:"LBDict_DispOrder",direction:"ASC"}]&fields=LBDict_Id,LBDict_DictType,LBDict_DictTypeName,LBDict_CName,LBDict_DictCode,LBDict_EName,LBDict_SName,LBDict_Shortcode,LBDict_PinYinZiTou,LBDict_DictInfo,LBDict_Ilevel,LBDict_ColorValue,LBDict_ColorDefault,LBDict_DictValue,LBDict_DictValueDefault,LBDict_Comment,LBDict_IsUse,LBDict_IsDefault,LBDict_DispOrder',
        updateUrl: uxutil.path.ROOT + '/ServerWCF/LabStarBaseTableService.svc/LS_UDTO_UpdateLBDictByField',
        delUrl: uxutil.path.ROOT + '/ServerWCF/LabStarBaseTableService.svc/LS_UDTO_DelLBDict',
        ...
        //getDictTypeUrl: uxutil.path.ROOT + '/ServerWCF/LabStarBaseTableService.svc/LS_UDTO_SearchLBDictTypeByHQL?isPlanish=true',//获得字典类型
        ...
        reset: function () { //重置
            if (formObj.type == 'edit' || formObj.type == 'show') {
                if (tableObj.checkRowData.length > 0) {
                    if (tableObj.checkRowData[tableObj.checkRowData.length - 1].LBDict_IsDefault == "true") {
                        //默认的话 不允许修改code
                        if (!$("#DictCode").hasClass("layui-disabled")) {
                            $("#DictCode").addClass("layui-disabled");
                            $("#DictCode").prop("disabled", "disabled");
                        }
                        if (!$("#DictType").hasClass("layui-disabled")) {
                            $("#DictType").addClass("layui-disabled");
                            $("#DictType").prop("disabled", "disabled");
                        }
                    } else {
                        if ($("#DictCode").hasClass("layui-disabled")) {
                            $("#DictCode").removeClass("layui-disabled");
                            $("#DictCode").prop("disabled", false);
                        }
                        if ($("#DictType").hasClass("layui-disabled")) {
                            $("#DictType").removeClass("layui-disabled");
                            $("#DictType").prop("disabled", false);
                        }
                    }
                    formObj.form.val("form", tableObj.checkRowData[tableObj.checkRowData.length - 1]);
                    colorpickerFun("ColorValue", tableObj.checkRowData[tableObj.checkRowData.length - 1].LBDict_ColorValue);
                    colorpickerFun("ColorDefault", tableObj.checkRowData[tableObj.checkRowData.length - 1].LBDict_ColorDefault);
                    if (tableObj.checkRowData[tableObj.checkRowData.length - 1].LBDict_IsUse == "true") {
                ...
                SetDisabled(false);
                //颜色
                colorpickerFun("ColorValue");
                //默认颜色
                colorpickerFun("ColorDefault");
                if (!$("#isUse").next('.layui-form-switch').hasClass('layui-form-onswitch')) {
                    $("#isUse").next('.layui-form-switch').addClass('layui-form-onswitch');
                    $("#isUse").next('.layui-form-switch').children("em").html("启用");
                    $("#isUse")[0].checked = true;
    ...
    function getServerData() {
        //获得字典类型枚举
        uxutil.server.ajax({
            url: tableObj.getEnumTypeUrl + '?classname=BaseDataType&classnamespace=ZhiFang.Entity.LabStar'
        }, function (res) {
            if (res.success) {
                if (res.ResultDataValue) {
                    ...
                    for (var i in data) {
                        ...
                        if (color != "") {
                            html += '<option value="' + data[i].Code + '" style="color:' + color + '">' + data[i].Name + '</option>';
                        } else {
                            html += '<option value="' + data[i].Code + '">' + data[i].Name + '</option>';
                        }
                    }
                    $("#DictType").html(html);
                    formObj.form.render("select");
                }
            } else {
                layer.msg("字典类型查询失败！", { icon: 5, anim: 6 });
            }
        });
    }
    //初始化表格
    function tableRender() {
        tableObj.table.render({
            ...
            cols: [
                ...
                {
                    field: 'LBDict_Shortcode',
                    title: '快捷码',
                    minWidth: 130,
                    sort: true
                },
                {
                    field: 'LBDict_PinYinZiTou',
                    title: '拼音字头',
                    minWidth: 130,
                    sort: true
                },
                {
                    field: 'LBDict_IsDefault',
                    title: '是否默认',
                    minWidth: 100,
                    sort: true,
                    templet: function (data) {
                        var str = "";
                        if (data.LBDict_IsDefault.toString() == "true") {
                            str = "<span style='color:red'>是</span>"
                        } else if (data.LBDict_IsDefault.toString() == "false") {
                            str = "<span>否</span>"
                        }
                        return str;
                    }
        ...
        switch (obj.event) {
            ...
            case 'del':
                if (tableObj.checkRowData.length === 0) {
                    ...
                    //一条数据 没有考虑多选 如需考虑 加循环
                    if (tableObj.checkRowData[0].LBDict_IsDefault.toString() == 'true') {//默认不允许删除
                        layer.msg("默认为是的不允许删除!");
                        return;
                    }
    ...
    formObj.form.on('submit(save)', function (data) {
        ...
        for (k in data.field) {
            if (k == "LBDict_DictType") {
                postData.DictType = data.field[k];
                postData.DictTypeName = $("#DictType").val() ? $("#DictType option:selected").text() : "";
                fields += "DictType,DictTypeName,";
                continue;
            }
            if (k.split("_")[1] == "ilevel") {
                if (data.field[k] == "") {
                    continue;
                } else {
                    if (isNaN(data.field[k])) {
                        layer.msg("等级是数值型！", { icon: 5, anim: 6 });
                        return;
                    }
                }
            }
    ...
    function getDispOrder() {
        uxutil.server.ajax({
            url: tableObj.getMaxNoByEntityFieldUrl + '?entityName=LBDict&entityField=DispOrder'
        }, function (res) {
            if (res.success) {
                if (res.ResultDataValue) {
    ...
    //清空/赋值颜色选择器
    function colorpickerFun(Id, colorVal) {
        if (!Id) return;
