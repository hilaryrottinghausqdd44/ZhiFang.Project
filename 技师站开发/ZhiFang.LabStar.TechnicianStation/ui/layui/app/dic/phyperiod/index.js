...
layui.extend({
	...
	var tableObj = {
		...
		fields:{
			Id:"LBPhyPeriod_Id",
			IsUse:'LBPhyPeriod_IsUse'
        },
        current: null,
        delIndex: null,
        addUrl: uxutil.path.ROOT +'/ServerWCF/LabStarBaseTableService.svc/LS_UDTO_AddLBPhyPeriod',
        selectUrl: uxutil.path.ROOT + '/ServerWCF/LabStarBaseTableService.svc/LS_UDTO_SearchLBPhyPeriodByHQL?isPlanish=true&sort=[{property:"LBPhyPeriod_DispOrder",direction:"ASC"}]',
        updateUrl: uxutil.path.ROOT +'/ServerWCF/LabStarBaseTableService.svc/LS_UDTO_UpdateLBPhyPeriodByField',
        delUrl: uxutil.path.ROOT +'/ServerWCF/LabStarBaseTableService.svc/LS_UDTO_DelLBPhyPeriod',
        ...
        reset: function () { //重置
            if (formObj.type == 'edit' || formObj.type == 'show') {
                if (tableObj.checkRowData.length > 0) {
                    ...
                    if (tableObj.checkRowData[tableObj.checkRowData.length - 1].LBPhyPeriod_IsUse == "true") {
    ...
    function tableRender() {
        tableObj.table.render({
            ...
            cols: [
                ...
                {
                    field: 'LBPhyPeriod_CName',
                    title: '名称',
                    minWidth: 130,
                    sort: true
                },
                {
                    field: 'LBPhyPeriod_SName',
                    title: '简称',
                    minWidth: 130,
                    sort: true
                },
                {
                    field: 'LBPhyPeriod_EName',
                    title: '英文名',
                    minWidth: 130,
                    sort: true
                },
                {
                    field: 'LBPhyPeriod_Shortcode',
                    title: '快捷码',
                    minWidth: 130,
                    sort: true
                },
                {
                    field: 'LBPhyPeriod_PinYinZiTou',
                    title: '拼音字头',
                    minWidth: 130,
                    sort: true
                },
                {
                    field: 'LBPhyPeriod_UseCode',
                    title: '用户编码',
                    sort: true,
                    hide: true,
                    minWidth: 70
                ...
                {
                    field: 'LBPhyPeriod_StandCode',
                    title: '标准编码',
                    minWidth: 100,
                    hide: true,
                    sort: true
                }, {
                    field: 'LBPhyPeriod_DispOrder',
                    title: '排列次序',
                    minWidth: 100,
                    sort: true
                },
    ...
    function getDispOrder() {
        $.ajax({
            type: "get",
            url: tableObj.getMaxNoByEntityFieldUrl + '?entityName=LBPhyPeriod&entityField=DispOrder',
            cache: false,
            dataType: 'json',
            success: function (res) {
                if (res.success) {
