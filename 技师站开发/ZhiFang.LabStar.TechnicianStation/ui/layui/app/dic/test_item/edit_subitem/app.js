layui.extend({
    ...
    app.url = {
        //获得左侧列表数据
        getLeftTableDataUrl: uxutil.path.ROOT + '/ServerWCF/LabStarBaseTableService.svc/LS_UDTO_QueryLBItemGroupByHQL?isPlanish=true&sort=[{property:"LBItemGroup_LBItem_DispOrder",direction:"ASC"}]',
        //获取右侧列表数据
        ...
        //保存服务
        saveUrl: uxutil.path.ROOT + '/ServerWCF/LabStarBaseTableService.svc/LS_UDTO_AddDelLBItemGroup'
    };
    //是否执行左侧列表中的done中部分内容
    ...
    app.fields = {
        Id: "LBItemGroup_Id",
        LeftTableId: 'LBItemGroup_LBItem_Id',
        RightTableId: 'LBItem_Id'
    };
    //左侧列表查询列
    app.leftTableColumn = [
        'LBItemGroup_Id',//组合项目表主键Id
        'LBItemGroup_DataAddTime',//组合项目添加该子项时间
        'LBItemGroup_LBGroup_Id',//组合项目Id
        'LBItemGroup_LBItem_Id',//子项目Id
        'LBItemGroup_LBItem_CName',//子项目名称
        'LBItemGroup_LBItem_EName',//子项目英文名
        'LBItemGroup_LBItem_SName',//子项目简称
        'LBItemGroup_LBItem_GroupType',//子项目组合类型
        'LBItemGroup_LBItem_ItemCharge',//子项目价格
        'LBItemGroup_LBItem_DispOrder'//子项目显示次序
    ];
    //右侧列表查询列
    ...
    app.paramsObj = {
        ...
        GroupType: null,
        CName: null,
        GroupTypeWhere: null//在init中主动赋值
    };
    //初始化
    app.init = function () {
        var me = this;
        ...
        if (me.paramsObj.GroupType == 1) {//组合项目
            me.paramsObj.GroupTypeWhere = "(0)";
        } else if (me.paramsObj.GroupType == 2) {//套餐项目
            me.paramsObj.GroupTypeWhere = "(0,1)";
        }
        me.getServerData();
        me.initLeftTable();
        me.listeners();
    ...
    app.initLeftTable = function() {
        var me = this,
            url = me.url.getLeftTableDataUrl + "&fields=" + me.leftTableColumn.join() + "&where=lbgroup.Id=" + me.paramsObj.groupItemID + "&t=" + new Date().getTime();
    ...
    app.initRightTable = function (url,options) {
        var me = this,
            ...
            url = url || me.url.getRightTableDataUrl + "&fields=" + me.rightTableColumn.join() + "&where=IsUse=1" + (me.paramsObj.GroupTypeWhere ? " and GroupType in " + me.paramsObj.GroupTypeWhere : "") + (me.leftTableData.length > 0 ? " and Id not in(" + me.leftTableData.join() + ")" : "") + "&t=" + new Date().getTime();
        var config = {
            elem: '#rightTable',
            height: 'full-110',
            defaultToolbar: ['filter'],
    ...
    app.listeners = function () {
        ...
        form.on('submit(search)', function (data) {
            ...
            var where = "IsUse=1" + (me.paramsObj.GroupTypeWhere ? " and GroupType in " + me.paramsObj.GroupTypeWhere : "") + (me.leftTableData.length > 0 ? " and Id not in(" + me.leftTableData.join() + ")" : "");
        ...
        table.on('sort(rightTable)', function (obj) {
            ...
            url += "&where=IsUse=1" + (me.paramsObj.GroupTypeWhere ? " and GroupType in " + me.paramsObj.GroupTypeWhere : "") + (me.leftTableData.length > 0 ? " and Id not in(" + me.leftTableData.join() + ")" : "");
    ...
    app.onAddClick = function (checkData) {
        ...
        for (var i = 0; i < checkData.length; i++) {
            ...
            if (flag) {
                var Obj = {
                    'LBItemGroup_LBGroup_Id': me.paramsObj.groupItemID,
                    'LBItemGroup_LBItem_Id': checkData[i][me.fields.RightTableId],
                    'LBItemGroup_LBItem_CName': checkData[i].LBItem_CName,
                    'LBItemGroup_LBItem_EName': checkData[i].LBItem_EName,
                    'LBItemGroup_LBItem_SName': checkData[i].LBItem_SName,
                    'LBItemGroup_LBItem_GroupType': checkData[i].LBItem_GroupType,
                    'LBItemGroup_LBItem_ItemCharge': checkData[i].LBItem_ItemCharge
                };
    ...
    app.onSaveClick = function () {
        ...
        for (var i = 0; i < newData.length; i++) {
            ...
            if (addFlag) {
                ...
                addEntityList.push({
                    LBGroup: { Id: me.paramsObj.groupItemID, DataTimeStamp: DataTimeStamp },
