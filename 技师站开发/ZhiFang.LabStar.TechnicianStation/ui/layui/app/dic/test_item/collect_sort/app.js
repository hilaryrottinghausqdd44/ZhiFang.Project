...
layui.extend({
	uxutil: 'ux/util',
	sortlist:'app/dic/test_item/collect_sort/list'
    ...
    var fields = {
        Id: "LBItem_Id",
        DispOrder: "LBItem_CollectSort"
    };
    var paramsObj = {
        url: uxutil.path.ROOT + '/ServerWCF/LabStarBaseTableService.svc/LS_UDTO_SearchLBItemListByHQL?isPlanish=true&sort=[{property:"LBItem_CollectSort",direction:"ASC"}]',
        searchWhere: null
    };
    getParams();
    //获得传递参数
    ...
    var options={
        ...
        height: 'full-50',
        size:'sm',
        url: paramsObj.url + paramsObj.searchWhere,
    	//where:{where:'1=1'},
  ...
  $('#adjustmentfinish').on('click',function(obj){
    ...
    var reorder = function(){
    	...
    	if(!DispOrder){
    		layer.msg("请输入采样次序！", { icon: 5, anim: 6 });
    		return;
        }
        var interval = document.getElementById('interval').value;
        if (!interval) {
            layer.msg("请输入采样次序间隔！", { icon: 5, anim: 6 });
            return;
        }
    	...
    	for(var i=0;i<tbData.length;i++){
    		var LBItemId = tbData[i][fields.Id];
    		for(var j=0;j<checkDatas.length;j++){
                if (LBItemId ==checkDatas[j][fields.Id] ){
...
//  	    tbData[i].LBItem_DisOrder =JSON.stringify(DispOrder);
    	}
