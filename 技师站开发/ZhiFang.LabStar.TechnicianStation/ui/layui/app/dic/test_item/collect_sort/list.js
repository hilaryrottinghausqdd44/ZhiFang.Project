...
layui.extend({
	...
	//获取项目列表服务地址
	var GET_LIST_URL = uxutil.path.ROOT + '/ServerWCF/LabStarBaseTableService.svc/LS_UDTO_SearchLBItemListByHQL?isPlanish=true&sort=[{property:"LBItem_CollectSort",direction:"ASC"}]';
    //修改项目服务地址
    var GET_EDIT_URL = uxutil.path.ROOT + '/ServerWCF/LabStarBaseTableService.svc/LS_UDTO_UpdateLBItemByField';
	...
	var sortlist = {
		...
		config:{
			...
			cols:[[
                ...
                { field:'LBItem_Id',width: 150,title: '主键',sort: true,hide:true},
				{ field:'LBItem_CollectSort',width: 100,title: '采样次序',sort: true, edit: 'text',event: "setDispOrder"},
                {field:'LBItem_CName', minWidth:150,flex:1, title: '项目名称', sort: true},
				{field:'LBItem_EName', width:100,title: '英文名称', sort: true},
				{field:'LBItem_SName', width:100,title: '简称', sort: true},
			...
			parseData:function(res){//res即为原始返回的数据
				...
				if(data && data.list){
					for(var i=0;i<data.list.length;i++){
						...
						data.list[i].Temporary_DispOrder = data.list[i].LBItem_CollectSort;
	...
	//数据加载
    Class.pt.loadData = function (url){
		var me = this,
    ...
    Class.pt.getModifiedRecords =  function(){
	    ...
	    for(var i = 0;i<tableCache.length;i++){
	    	//找到修改过数据的行
			if (tableCache[i].Temporary_DispOrder != tableCache[i].LBItem_CollectSort){
    ...
    Class.pt.updateOne =  function(index,obj){
   		var me = this;
   		setTimeout(function() {
   	        var  id = obj.LBItem_Id;
            var DispOrder = obj.LBItem_CollectSort;
            var entity ={
            	Id:id,
				CollectSort:DispOrder
            };
            var fields ="Id,CollectSort";
