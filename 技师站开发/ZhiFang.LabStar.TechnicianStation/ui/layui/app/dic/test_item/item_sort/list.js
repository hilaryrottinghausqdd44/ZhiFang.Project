...
layui.extend({
	...
	//获取项目列表服务地址
    var GET_LIST_URL = uxutil.path.ROOT + '/ServerWCF/LabStarBaseTableService.svc/LS_UDTO_SearchLBItemListByHQL?isPlanish=true&sort=[{property:"LBItem_DispOrder",direction:"ASC"}]';
    //修改项目服务地址
	...
	var sortlist = {
		...
		config:{
			...
			cols:[[
                ...
                { field:'LBItem_DispOrder',width: 100,title: '显示次序',sort: true, edit: 'text',event: "setDispOrder"},
			...
			parseData:function(res){//res即为原始返回的数据
				...
				if(data && data.list){
					for(var i=0;i<data.list.length;i++){
                        ...
                        data.list[i].Temporary_DispOrder = data.list[i].LBItem_DispOrder;
    ...
    Class.pt.getModifiedRecords =  function(){
	    ...
	    for(var i = 0;i<tableCache.length;i++){
	    	//找到修改过数据的行
            if (tableCache[i].Temporary_DispOrder != tableCache[i].LBItem_DispOrder){
    ...
    Class.pt.updateOne =  function(index,obj){
   		...
   		setTimeout(function() {
            ...
            var DispOrder = obj.LBItem_DispOrder;
            var entity ={
            	Id:id,
            	DispOrder:DispOrder
            };
