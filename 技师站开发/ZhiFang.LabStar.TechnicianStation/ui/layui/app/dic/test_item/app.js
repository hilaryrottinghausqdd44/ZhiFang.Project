...
$(function () {
    ...
    testItem.tableObj = {
        ...
        current: null,
        addUrl: Shell.util.Path.ROOT + '/ServerWCF/LabStarBaseTableService.svc/LS_UDTO_AddLBItem',
        //返回页码和该页数据  --//根据id，每页条数，排序获得数据所在页码与该页数据
        selectUrl: Shell.util.Path.ROOT + '/ServerWCF/LabStarBaseTableService.svc/LS_UDTO_SearchLBItemListByHQL?isPlanish=true&sort=[{property:"LBItem_DispOrder",direction:"ASC"}]',
        updateUrl: Shell.util.Path.ROOT + '/ServerWCF/LabStarBaseTableService.svc/LS_UDTO_UpdateLBItemByField',
        delUrl: Shell.util.Path.ROOT + '/ServerWCF/LabStarBaseTableService.svc/LS_UDTO_DelLBItem',
        getSectionUrl: Shell.util.Path.ROOT + '/ServerWCF/LabStarBaseTableService.svc/LS_UDTO_SearchLBSectionByHQL?isPlanish=true&where=IsUse=true',//获得小组
        getSpecialtyUrl: Shell.util.Path.ROOT + '/ServerWCF/LabStarBaseTableService.svc/LS_UDTO_SearchLBSpecialtyByHQL?isPlanish=true&sort=[{property:"LBSpecialty_DispOrder",direction:"ASC"}]',//获得专业
        getEquipUrl: Shell.util.Path.ROOT + '/ServerWCF/LabStarBaseTableService.svc/LS_UDTO_SearchLBEquipByHQL?isPlanish=true&fields=LBEquip_Id,LBEquip_CName&where=IsUse=true',
        getEnumTypeUrl: Shell.util.Path.ROOT + '/ServerWCF/CommonService.svc/GetClassDic',//获得枚举 传递枚举类型名 
        getPinYinZiTouUrl: Shell.util.Path.ROOT + '/ServerWCF/LabStarCommonService.svc/GetPinYinZiTou?chinese=',//获得拼音字头
        getMaxNoByEntityFieldUrl: Shell.util.Path.ROOT + '/ServerWCF/LabStarCommonService.svc/GetMaxNoByEntityField',//获取指定实体字段的最大号
        //组合项目
        delSubItemUrl: Shell.util.Path.ROOT + '/ServerWCF/LabStarBaseTableService.svc/LS_UDTO_DelLBItemGroup',//删除组合项目中的子项
        selectSubItemUrl: Shell.util.Path.ROOT + '/ServerWCF/LabStarBaseTableService.svc/LS_UDTO_QueryLBItemGroupByHQL?isPlanish=true&sort=[{property:"LBItem_DispOrder",direction:"ASC"}]&fields=LBItemGroup_LBItemGroup_Id,LBItemGroup_LBItemGroup_DataAddTime,LBItemGroup_LBGroup_Id,LBItemGroup_LBItem_Id,LBItemGroup_LBItem_CName,LBItemGroup_LBItem_EName,LBItemGroup_LBItem_SName,LBItemGroup_LBItem_GroupType,LBItemGroup_LBItem_ItemCharge,LBItemGroup_LBItem_DispOrder',
        copyComboItemUrl: Shell.util.Path.ROOT + '/ServerWCF/LabStarBaseTableService.svc/LS_UDTO_AddCopyLBItemGroup',//复制组合项目子项
    ...
    var tab = {
        index: 0,//当前页签
        allTabIndex: [0, 1, 2, 3, 4, 5, 6]//全部页签
    };
    //表单
    var formObj = {
        type: 'edit',
    ...
    function init() {
        ...
        $(".fixedBtnBox").css("width", $("#card").css("width"));
        tableRender();
        getServerData();
    }
    //发送服务获得基础数据
    function getServerData() {
        ...
        $.ajax({
            type: "get",
            url: tableObj.getEnumTypeUrl + '?classname=SectionType&classnamespace=ZhiFang.Entity.LabStar',
            cache: false,
            dataType: 'json',
            success: function (res) {
                if (res.success) {
                    if (res.ResultDataValue) {
                        ...
                        $("#SectionFun option:first-child").attr("selected", 'selected');
                        form.render('select');
                        //}
                    }
                } else {
            ...
            success: function (res) {
                if (res.success) {
                    if (res.ResultDataValue) {
                        ...
                        $("#ValueType").html(html);
                        $("#ValueType option:nth(2)-child").attr("selected", 'selected');
                        form.render('select');
                        //}
                    }
                } else {
    ...
    function getDispOrder() {
        $.ajax({
            type: "get",
            url: tableObj.getMaxNoByEntityFieldUrl + '?entityName=LBItem&entityField=DispOrder',
            cache: false,
            dataType: 'json',
            success: function (res) {
                if (res.success) {
    ...
    $("#sort").click(function () {
        ...
        parent.layer.open({
            ...
            title: ['项目排序调整'],
            area: ['1200px', '620px'],
            content: Shell.util.Path.ROOT + '/ui/layui/app/dic/test_item/item_sort/app.html?' + searchWhere,
    ...
    function clearFormSelects() {
        $("#formInfo select").each(function () {
            if ($(this).attr("id") == "SectionFun") {
                ...
                $(this).children('option').eq(1).attr("select", "select");
            } else {
                $(this).val('');
            }
        });
    ...
    function comboItemTableRender(comBoItemId) {
        var tableHeight = $(window).height() - $("#comboItemTable").offset().top - 30;
        table.render({
            elem: '#comboItemTable',
            height: tableHeight > 375 ? tableHeight : 375,
            defaultToolbar: ['filter'],
            size: 'sm', //小尺寸的表格
            toolbar: '#toolbarComboItem',
            url: tableObj.selectSubItemUrl + "&where=GroupItemID=" + comBoItemId,
            cols: [
                [{
                    type: 'checkbox',
                    fixed: 'left'
        ...
        switch (obj.event) {
            case 'edit':
                ...
                parent.layer.open({
                    ...
                    area: ['1200px', '600px'],
                    content: Shell.util.Path.ROOT + '/ui/layui/app/dic/test_item/edit_subitem/editSubItem.html?groupItemID=' + data[tableObj.fields.Id] + '&CName=' + data.LBItem_CName + '&GroupType=' + data.LBItem_GroupType,
                    cancel: function (index, layero) {
                        flag = true;
                    },
                    end: function () {
