...
layui.extend({
	...
	//小组名称 
	var SECTIONCNAME = PARAMS.SECTIONCNAME;
	//获取项目列表数据
	...
	//保存服务
	var SAVE_URL = uxutil.path.ROOT  +  '/ServerWCF/LabStarBaseTableService.svc/LS_UDTO_AddDelLBSectionItem';
    //当前左列表选择行
	...
	var table_Ind0 = table.render({
		...
		cols: [[
			...
			{field:'LBSectionItemVO_LBSectionItem_Id',title:'ID',width:150,sort:true,hide:true},
			{field:'LBSectionItemVO_LBItem_Id',title:'项目编号',width:100,sort:false,hide:true},
			{field:'LBSectionItemVO_LBItem_CName',title:'项目名称',flex:1,sort:false},
			{field:'LBSectionItemVO_LBItem_EName',title:'英文名称',width:100,sort:false},
			{field:'LBSectionItemVO_LBItem_UseCode',title:'用户编码',width:100,sort:false},
			{field:'LBSectionItemVO_LBItem_DispOrder',title:'次序',width:100,sort:false,hide:true},
	...
	var table_Ind1 = table.render({
		...
		cols: [[
            ...
            {field:'LBItem_CName', minWidth:150,flex:1, title: '项目名称'},
		    {field:'LBItem_EName', width:150, title: '英文名称'},
            {field:'LBItem_UseCode', width:150, title: '用户编码'},
            {field:'LBSectionItemVO_LBSectionItem_Id', width:150, title: '采样组ID', hide: true},
            {field:'LBItem_Tab', width:150, title: '已选', hide: true}
		]],
		limit: 50,
		loading:true,
		page: true,
		...
		text: {none: '暂无相关数据' },
			...
			res.data.forEach(function (item, index) {
				if (item.LBItem_Tab == '1'){
					//背景色-绿色
					$('div[lay-id="righttable"]').
					find('tr[data-index="' + index + '"]').
					find('td[data-field="LBItem_CName"]').
	...
	function resultData(data){
		...
		for(var i=0;i<data.length;i++){
    		...
    		for(var j=0;j<leftData.length;j++){
    		    if(data[i].LBItem_Id == leftData[j].LBSectionItemVO_LBItem_Id){
                    data[i].LBItem_Tab ='1';
    		    	break;
    		    }
    	   }
   ...
   //向左列表添加数据
        ...
        for (var i = 0; i < rightData.length; i++) {
            ...
            if(rightData[i].LAY_CHECKED && rightData[i].LAY_CHECKED===true){
                ...
                rightData[i].LAY_TABLE_INDEX && delete rightData[i].LAY_TABLE_INDEX
                 var obj = {
                    'LBSectionItemVO_LBItem_Id':  rightData[i].LBItem_Id,
                    'LBSectionItemVO_LBItem_CName':  rightData[i].LBItem_CName,
                    'LBSectionItemVO_LBItem_EName':  rightData[i].LBItem_EName,
                    'LBSectionItemVO_LBItem_UseCode':  rightData[i].LBItem_UseCode,
                    'LBSectionItemVO_LBSectionItem_Id':  rightData[i].LBSectionItemVO_LBSectionItem_Id,
                    'isAdd':'1'
                };
                for(var j=0;j<leftData.length;j++){
                	if(leftData[j].LBSectionItemVO_LBItem_Id == rightData[i].LBItem_Id){
                		isAdd =false;
                		break;
                	}
                }
                if(isAdd)leftData.push(obj);
            }else{
        ...
        for (var i = 0; i < leftData.length; i++) {
            if(leftData[i].LAY_CHECKED && leftData[i].LAY_CHECKED===true){
                ...
                leftData[i].LAY_TABLE_INDEX && delete leftData[i].LAY_TABLE_INDEX
                 var obj = {
                    'LBItem_Id': leftData[i].LBSectionItemVO_LBItem_Id,
                    'LBItem_CName': leftData[i].LBSectionItemVO_LBItem_CName,
                    'LBItem_EName': leftData[i].LBSectionItemVO_LBItem_EName,
                    'LBItem_UseCode': leftData[i].LBSectionItemVO_LBItem_UseCode,
                    'LBSectionItemVO_LBSectionItem_Id' : leftData[i].LBSectionItemVO_LBSectionItem_Id
                };
                rightData.push(obj)
            }else{
	...
	function loadData1(){
		...
		var where ="";
		if(searchval) where = "(lbitem.CName like '%"+searchval+"%')"; 
        if(where)where = encodeURI(where);
		var url = GET_ITEM_LIST_URL;
		if($('#LBSection_ID').val())url+='&sectionID='+$('#LBSection_ID').val();
...
//				page:CURR_PAGE
	...
	function getEntity(){
    	...
    	for (var i = 0; i < leftData.length; i++) {
             if (leftData[i].isAdd =='1') {
                ...
                addList.push({
                    ...
                    LBItem: { Id: leftData[i].LBSectionItemVO_LBItem_Id, DataTimeStamp: DataTimeStamp }
                });
            }
        }
    	//删除 
    	for (var i = 0; i < OLD_LEFT_DATA.length; i++) {
            ...
            for (var j = 0; j < leftData.length; j++) {
                if (OLD_LEFT_DATA[i].LBSectionItemVO_LBItem_Id == leftData[j].LBSectionItemVO_LBItem_Id) {
                    delFlag = false;//存在 不是删除的
                    break;
                }
            }
            if (delFlag)delIDList.push(OLD_LEFT_DATA[i].LBSectionItemVO_LBSectionItem_Id);
        }
	...
	function loadSectionItem(callback){
		var cols = table_Ind0.config.cols[0],
			fields = [];
		for(var i in cols){
			if(cols[i].field)fields.push(cols[i].field);
		}
		var url = GET_SECTION_ITEM_LIST_URL+'&fields='+fields.join(',');
		if(SECTIONID)url+='&where=lbsection.Id='+SECTIONID;
		url+='&sort='+JSON.stringify(table_Ind0.config.defaultOrderBy);
