...
layui.extend({
	...
	var tableObj = {
		...
		fields:{
			Id:"LBAgeUnit_Id",
			IsUse:'LBAgeUnit_IsUse'
        },
        current: null,
        delIndex: null,
        addUrl: uxutil.path.ROOT +'/ServerWCF/LabStarBaseTableService.svc/LS_UDTO_AddLBAgeUnit',
        selectUrl: uxutil.path.ROOT + '/ServerWCF/LabStarBaseTableService.svc/LS_UDTO_SearchLBAgeUnitByHQL?isPlanish=true&sort=[{property:"LBAgeUnit_DispOrder",direction:"ASC"}]',
        updateUrl: uxutil.path.ROOT +'/ServerWCF/LabStarBaseTableService.svc/LS_UDTO_UpdateLBAgeUnitByField',
        delUrl: uxutil.path.ROOT +'/ServerWCF/LabStarBaseTableService.svc/LS_UDTO_DelLBAgeUnit',
		...
		reset: function() { //重置
            if (formObj.type == 'edit' || formObj.type == 'show') {
				if(tableObj.checkRowData.length > 0) {
                    ...
                    if (tableObj.checkRowData[tableObj.checkRowData.length - 1].LBAgeUnit_IsUse == "true") {
    ...
    function tableRender() {
        tableObj.table.render({
            ...
            cols: [
                ...
                {
                    field: 'LBAgeUnit_CName',
                    title: '名称',
                    minWidth: 130,
                    sort: true
                },
                {
                    field: 'LBAgeUnit_SName',
                    title: '简称',
                    minWidth: 130,
                    sort: true
                },
                {
                    field: 'LBAgeUnit_EName',
                    title: '英文名',
                    minWidth: 130,
                    sort: true
                },
                {
                    field: 'LBAgeUnit_Shortcode',
                    title: '快捷码',
                    minWidth: 130,
                    sort: true
                },
                {
                    field: 'LBAgeUnit_PinYinZiTou',
                    title: '拼音字头',
                    minWidth: 130,
                    sort: true
                },
                {
                    field: 'LBAgeUnit_UseCode',
                    title: '用户编码',
                    sort: true,
                    hide: true,
                    minWidth: 70
    ...
    function getDispOrder() {
        uxutil.server.ajax({
            url: tableObj.getMaxNoByEntityFieldUrl + '?entityName=LBAgeUnit&entityField=DispOrder'
        }, function (res) {
            if (res.success) {
