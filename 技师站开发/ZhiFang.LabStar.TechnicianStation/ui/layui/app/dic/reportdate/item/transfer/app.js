...
layui.extend({
	...
	//取单时间分类名称 
	var REPORTDATECNAME = PARAMS.REPORTDATECNAME;
	//获取项目列表数据
	...
	//取单分类项目列表数据
	var GET_SAMPLINGITEM_LIST_URL = uxutil.path.ROOT + '/ServerWCF/LabStarBaseTableService.svc/LS_UDTO_SearchLBReportDateItemByHQL?isPlanish=true';
	//保存服务
	var SAVE_URL = uxutil.path.ROOT  + '/ServerWCF/LabStarPreService.svc/LS_UDTO_AddDelLBReportDateItem';
    //当前左列表选择行
	...
	var table_Ind0 = table.render({
		...
		cols: [[
			...
			{field:'LBReportDateItem_LBItem_Id',title:'项目编号',width:100,sort:true,hide:true},
			{field:'LBReportDateItem_LBItem_CName',title:'项目名称',flex:1,sort:true},
			{field:'LBReportDateItem_LBItem_EName',title:'英文名称',width:100,sort:true},
			{field:'LBReportDateItem_LBItem_UseCode',title:'用户编码',width:100,sort:true},
			{field:'LBReportDateItem_LBItem_DispOrder',title:'次序',width:100,sort:true,hide:true},
			{field:'isAdd', width:60, title: 'isAdd', hide: true}
		]],
		defaultOrderBy: JSON.stringify([{property: 'LBReportDateItem_LBItem_DispOrder',direction: 'ASC'}]),
		text: {none: '暂无相关数据' },
		loading:true,
		page: false,
	...
	var table_Ind1 = table.render({
		...
		cols: [[
            ...
            {field:'LBReportDateItem_Id0', width:150, title: '采样组ID', hide: true}
    ...
    function resultData(data){
		...
		for(var i=0;i<data.length;i++){
    		...
    		for(var j=0;j<leftData.length;j++){
    		    if(data[i].LBItem_Id == leftData[j].LBReportDateItem_LBItem_Id){
    		    	isExec = false;
    		    	break;
    		    }
    	    }
...
//  	loadData1();
   ...
   //向左列表添加数据
        ...
        for (var i = 0; i < rightData.length; i++) {
            if(rightData[i].LAY_CHECKED && rightData[i].LAY_CHECKED===true){
                ...
                rightData[i].LAY_TABLE_INDEX && delete rightData[i].LAY_TABLE_INDEX
                 var obj = {
                    'LBReportDateItem_LBItem_Id':  rightData[i].LBItem_Id,
                    'LBReportDateItem_LBItem_CName':  rightData[i].LBItem_CName,
                    'LBReportDateItem_LBItem_EName':  rightData[i].LBItem_EName,
                    'LBReportDateItem_LBItem_UseCode':  rightData[i].LBItem_UseCode,
                    'LBReportDateItem_Id':  rightData[i].LBReportDateItem_Id0,
                    'isAdd':'1'
                };
                leftData.push(obj)
            }else{
        ...
        for (var i = 0; i < leftData.length; i++) {
            if(leftData[i].LAY_CHECKED && leftData[i].LAY_CHECKED===true){
                ...
                leftData[i].LAY_TABLE_INDEX && delete leftData[i].LAY_TABLE_INDEX
                 var obj = {
                    'LBItem_Id': leftData[i].LBReportDateItem_LBItem_Id,
                    'LBItem_CName': leftData[i].LBReportDateItem_LBItem_CName,
                    'LBItem_EName': leftData[i].LBReportDateItem_LBItem_EName,
                    'LBItem_UseCode': leftData[i].LBReportDateItem_LBItem_UseCode,
                    'LBReportDateItem_Id0' : leftData[i].LBReportDateItem_Id
                };
                rightData.push(obj)
            }else{
	...
	function loadData0(){
		...
		var whereObj = {'where':'lbreportdateitem.LBReportDate.Id='+REPORTDATEID};
		IS_LOAD=true;
		table_Ind0.reload({
			url:url,
...
//		var wherestr = "(lbitem.CName like '%"+searchval+"%' or lbitem.EName like '%"+searchval+"%' or lbitem.UseCode like '%"+searchval+"%')";
//		if(searchval)whereObj={'where': encodeURI(wherestr)}; 
//		
		...
		var url = GET_ITEM_LIST_URL+'&ReportDateID='+REPORTDATEID;
	...
	function getEntity(){
    	...
    	for (var i = 0; i < leftData.length; i++) {
             if (leftData[i].isAdd =='1') {
                ...
                addList.push({
                    LBReportDate: { Id: REPORTDATEID, DataTimeStamp: DataTimeStamp },
                    LBItem: { Id: leftData[i].LBReportDateItem_LBItem_Id, DataTimeStamp: DataTimeStamp }
                });
            }
        }
    	//删除 
    	for (var i = 0; i < OLD_LEFT_DATA.length; i++) {
            ...
            for (var j = 0; j < leftData.length; j++) {
                if (OLD_LEFT_DATA[i].LBReportDateItem_LBItem_Id == leftData[j].LBReportDateItem_LBItem_Id) {
                    delFlag = false;//存在 不是删除的
                    break;
                }
            }
            if (delFlag)delIDList.push(OLD_LEFT_DATA[i].LBReportDateItem_Id);
        }
	...
	//初始化
	function init(){
		$('#sectionCName').text(REPORTDATECNAME);
		//下拉框数据加载
