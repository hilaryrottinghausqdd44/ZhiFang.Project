...
layui.extend({
	...
	//获取列表服务
	var GET_LIST_URL = uxutil.path.ROOT + '/ServerWCF/LabStarBaseTableService.svc/LS_UDTO_SearchBHostTypeByHQL?isPlanish=true';
	//删除数据服务
   ...
   //获取指定实体字段的最大号
	...
	var config = {
		...
		cols: [[
			...
			{field:'BHostType_Id',width:180,title:'ID',hide:true},
			{field:'BHostType_CName',width:150,title:'名称'},
			{field:'BHostType_SName',width:150,title:'简称'},
			{field:'BHostType_PinYinZiTou',width:100,title:'拼音字头'},
			{field:'BHostType_IsUse',width:60,title:'使用',templet:function(d){
				var arr = [
					'<div style="color:#FF5722;text-align:center;">否</div>',
					'<div style="color:#009688;text-align:center;">是</div>'
				];
				var result = d.BHostType_IsUse == 'true' ? arr[1] : arr[0];
				return result;
			}},
			{field:'BHostType_DispOrder',width:80,title:'次序',sort:true},
			{field:'BHostType_HostTypeDesc',minWidth:150,title:'备注'}
		]],
		loading:true,
		page: true,
		...
		text: {none: '暂无相关数据' },
			...
			if(count>0){
	            ...
	            for (var i = 0; i < res.data.length; i++) {
	                if (res.data[i].BHostType_Id == PK) {
	              	    rowIndex=i;
	              	    break;
	                }
	            }
	            //默认选择行	
	            var tableDiv = $("#bhosttype-table+div .layui-table-body.layui-table-body.layui-table-main");
	...
	//监听行单击事件
	table.on('row(bhosttype-table)', function(obj){
		//标注选中样式
        obj.tr.addClass('layui-table-click').siblings().removeClass('layui-table-click');
	   isEdit(obj.data.BHostType_Id);
	});
    //列表查询
	...
	//名称填写完赋值拼音字头和快捷码
    $("input[name='BHostType_CName']").blur(function () {
        getPinYinZiTou($(this).val(), function (str) {
            $("input[name='BHostType_Shortcode']").val(str);
            $("input[name='BHostType_PinYinZiTou']").val(str);
        });
    });
	//初始化
	function init(){
	...
	function onSearch(){
		...
		if(searchValue){
			...
			for(var i=0;i<len;i++){
				searchWhere.push("bhosttype." + searchFields[i] + " like '%" + searchValue + "%'");
			}
	...
	function changeResult(data){
		...
		if(list.BHostType_IsUse=="false")list.BHostType_IsUse="";
	...
	function getEditParams(data) {
		...
		entity.fields = 'Id,CName,SName,Shortcode,PinYinZiTou,DispOrder,IsUse,HostTypeDesc';
	...
	//删除方法 
	function onDelClick(){
		var id = document.getElementById("BHostType_Id").value;    
        if(!id)return;
    ...
    function onResetClick() {
        var me = this;
        if(formtype=='add'){
			$("#BHostType").find('input[type=text],select,textarea,input[type=hidden],input[type=number]').each(function() {
	           $(this).val('');
	        });
//	        document.getElementById("BHostType_IsUse").value='true';
	        getMaxNo(function (val) {
	            document.getElementById('BHostType_DispOrder').value = val;
	        });
	        if (!$("#BHostType_IsUse").next('.layui-form-switch').hasClass('layui-form-onswitch')) {
                $("#BHostType_IsUse").next('.layui-form-switch').addClass('layui-form-onswitch');
                $("#BHostType_IsUse").next('.layui-form-switch').children("em").html("是");
                $("#BHostType_IsUse")[0].checked = true;
            }
		}else{
			form.val('BHostType',DATA_LIST);
		}
