...
layui.define('jquery', function (exports) {
    ...
    var uxutil = {
        ...
        path: {
            ...
            REPORTFORMQUERYPRINTROOTPATH: '',//报告打印程序地址
            init: function () {
                ...
                this.getLIIPRootPath(function (p) { uxutil.path.LIIP_ROOT = p; });
                this.getReportFormQueryPrintRootPath(function (p) { uxutil.path.REPORTFORMQUERYPRINTROOTPATH = p; });
            },
            getLocal: function () {
            ...
            getLIIPRootPath: function (callback) {
                var url = this.ROOT + '/ServerWCF/LabStarPreService.svc/GetConfigLIIPUrl',
                    p = this.LOCAL + '/ZhiFang.LabInformationIntegratePlatform';
                uxutil.server.ajax({ url: url, async: false }, function (res) {
                    if (res.success) {
                        if (res.ResultDataValue)
                            p = res.ResultDataValue;
                    }
                    callback && callback(p);
                });
            },
            getReportFormQueryPrintRootPath: function (callback) {
                var url = this.ROOT + '/ServerWCF/LabStarPreService.svc/GetConfigReportFormQueryPrintUrl',
                    p = this.LOCAL + '/ZhiFang.ReportFormQueryPrint';
                uxutil.server.ajax({ url: url, async: false }, function (res) {
                    if (res.success) {
                        if (res.ResultDataValue)
                            p = res.ResultDataValue;
        ...
        cookie: {
            ...
            //验证cookie中部分信息是否存在，不存在则返回登录界面
            verify: function (valuelist,jumpurl) {
                var values = valuelist || ['000200', '000201', '000400'],
                    jumpurl = jumpurl || uxutil.path.LIIP_ROOT + "/ui/layui/admin/user/login.html";
                for (var i = 0; i < values.length; i++) {
                    if (!this.get(values[i])) {
                        location.href = jumpurl+'?t=' + new Date().getTime();//登录界面
                        break;
                    }
                }
            }
        ...
        localStorage: {
            ...
            get: function (name, isDecode) {
                ...
                if (isDecode) { data = JSON.parse(data); }
                return data;
            },
        ...
        server: {
			...
			//树结构处理
			changeTreeData: function(list){
				var me = this;
				var tree = [];
				for(var i in list){
					var info = typeof(list[i].Para) == 'string' ? JSON.parse(list[i].Para) : list[i].Para;
					tree.push({
						title:list[i].text,
						id:list[i].tid,
						//checked:true,
						spread:list[i].expanded,
						children:me.changeTreeData(list[i].Tree),
						info:info
					});
				}
