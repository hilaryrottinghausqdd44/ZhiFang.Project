...
layui.extend({
    uxutil: 'ux/util',
    mainTable: 'views/system/setting/tableColumn/main/mainTable'
    ...
    var config = {
        params: {
            toolbar: '#tableToolbar', defaultToolbar: 'filter', page: true, limit: 50, externalWhere: '', isHideColumnToolbar: false, isInitLaboratory: false, isInitComponentType: true, isSearch: false, isSaveSuccessLoadTable:true,
        ...
        uxutil.server.ajax({
            ...
            if (data && data.ResultDataValue) {
                var list = JSON.parse(data.ResultDataValue),
                    str = '';
                if (list.length > 0) {
                    $.each(list[0]["TableControl_Config"], function (i, itemI) {
