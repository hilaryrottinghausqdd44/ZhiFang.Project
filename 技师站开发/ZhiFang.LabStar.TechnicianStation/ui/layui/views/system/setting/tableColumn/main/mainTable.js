...
layui.extend({
    ...
    var mainTable = {
        ...
        config: {
            ...
            /**列表当前排序*/
            sort: [{
                "property": 'BModuleGridControlList_DispOrder',
                "direction": 'ASC'
            }],
            selectUrl:  '/ServerWCF/ModuleConfigService.svc/ST_UDTO_SearchBModuleGridControlListByHQL?isPlanish=true',
            addUrl:  '/ServerWCF/ModuleConfigService.svc/ST_UDTO_AddBModuleGridControlList',
            editUrl: '/ServerWCF/ModuleConfigService.svc/ST_UDTO_UpdateBModuleGridControlListByField',
            delUrl: '/ServerWCF/ModuleConfigService.svc/ST_UDTO_DelBModuleGridControlList',
            where: "",
            ...
            cols: [[
                ...
                { field: 'BModuleGridControlList_Id', width: 80, title: '主键ID', sort: false, hide: true },
                { field: 'BModuleGridControlList_LabID', width: 80, title: '实验室ID', sort: false, hide: true },
                { field: 'BModuleGridControlList_LabNo', title: '实验室No', width: 120, sort: true, hide: true },
                { field: 'BModuleGridControlList_GridID', title: '列表集合主键', width: 120, sort: true, hide:true },
                { field: 'BModuleGridControlList_GridCode', title: '列表代码', width: 120, sort: true },
                { field: 'BModuleGridControlList_CName', title: '列表名称', width: 120, sort: true },
                { field: 'BModuleGridControlList_TypeID', title: '控件类型Id', width: 120, sort: true, hide: true },
                { field: 'BModuleGridControlList_TypeName', title: '控件类型', width: 120, sort: true },
                { field: 'BModuleGridControlList_MapField', title: '对应属性', width: 120, sort: true },
                { field: 'BModuleGridControlList_ColName', title: '列头名称', width: 120, sort: true },
                { field: 'BModuleGridControlList_StyleContent', title: '控件样式', width: 120, sort: true },
                { field: 'BModuleGridControlList_ColData', title: '数据扩展', width: 120, sort: false },
                { field: 'BModuleGridControlList_Width', title: '宽度', width: 120, sort: true },
                { field: 'BModuleGridControlList_MinWidth', title: '最小宽度', width: 120, sort: true },
                { field: 'BModuleGridControlList_Edit', title: '编辑类型', width: 120, sort: true },
                { field: 'BModuleGridControlList_Toolbar', title: '工具条模板', width: 120, sort: true },
                { field: 'BModuleGridControlList_Align', title: '排序方式', width: 120, sort: true },
                { field: 'BModuleGridControlList_Fixed', title: '固定列', width: 120, sort: true },
                {
                    field: 'BModuleGridControlList_IsOrder', title: '是否排序', width: 120, sort: true,
                    templet: function (data) {
                        var str = "";
                        if (String(data.BModuleGridControlList_IsOrder) == "true")
                ...
                { field: 'BModuleGridControlList_OrderType', title: '排序类型', width: 80, sort: true },
                {
                    field: 'BModuleGridControlList_IsUse', title: '是否使用', width: 120, sort: true,
                    templet: function (data) {
                        var str = "";
                        if (String(data.BModuleGridControlList_IsUse) == "true")
                            str = "<span style='color:green;'>是</span>";
                        else
                            str = "<span style='color:red;'>否</span>";
                        return str;
                ...
                {
                    field: 'BModuleGridControlList_IsHide', title: '是否显示', width: 120, sort: true,
                    templet: function (data) {
                        var str = "";
                        if (String(data.BModuleGridControlList_IsHide) == "false")
                            str = "<span style='color:green;'>是</span>";
                        else
                            str = "<span style='color:red;'>否</span>";
                        return str;
                ...
                { field: 'BModuleGridControlList_DispOrder', title: '显示次序', width: 120, sort: true }
            ]],
            text: { none: '暂无相关数据' },
            response: function () {
                return {
            ...
            parseData: function (res) {//res即为原始返回的数据
                ...
                for (var i = 0; i < res.data.length; i++) {
                    if (res.data[i].BModuleGridControlList_Id == mainTable.config.PK) {
...
//                      $("#mainTable+div .layui-table-body table.layui-table tbody tr:nth-child(" + rowIndex + ")")[0].click();
    ...
    Class.pt.addClick = function () {
        ...
        layer.open({
            ...
            yes: function (index, layero) {
                ...
                entity["IsHide"] = $("#IsHide").prop("checked") ? 0 : 1;
    ...
    Class.pt.editClick = function (data) {
        var me = this,
            list = JSON.parse(JSON.stringify(data).replace(/BModuleGridControlList_/g, ""));
        list.IsUse = String(list.IsUse) == "true" ? true : false;
        list.IsOrder = String(list.IsOrder) == "true" ? true : false;
        list.IsHide = String(list.IsHide) == "false" ? true : false;
    ...
    Class.pt.delClick = function (data) {
        var me = this,
            id = data.BModuleGridControlList_Id;
        if (!id) return;
