layui.extend({
	...
	//危急值确认服务地址
	var CONFIRM_URL = uxutil.path.LIIP_ROOT + "/ServerWCF/IMService.svc/ST_UDTO_SCMsgByConfirm";
	//获取当前科室医生列表
	...
	var ID = PARAMS.ID;
	//新增时间
	var DATAADDTIME = PARAMS.DATAADDTIME;
	//接收科室ID
	...
	function onComfirm(empInfo){
		var entity = {
			...
			"ConfirmNotifyDoctorName":$("#ConfirmNotifyDoctorName").val(),
			"ConfirmerCodeHIS":"",
			"ConfirmIPAddress":"",
			"ConfirmMemo":$("#Memo").val() || '',
			"RecDeptID":RECDEPTID,
			"DataAddTime":uxutil.date.toServerDate(DATAADDTIME)
		};
