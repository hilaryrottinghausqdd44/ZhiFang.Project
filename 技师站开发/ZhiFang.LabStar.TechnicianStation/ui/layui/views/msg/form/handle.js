layui.extend({
	...
	var IS_URGENT = true;
	//不紧急的默认处理意见
	var DEFAULT_HANDLE_DESC = '继续观察，无需特殊处理。';
	
	$('#USERNAME').html('当前登录账户：' + uxutil.cookie.get(uxutil.cookie.map.USERNAME));
	
	form.on('checkbox(HandleType)', function(data){
		IS_URGENT = data.elem.checked;//是否被选中，true或者false
		var HandleDesc = IS_URGENT ? "" : DEFAULT_HANDLE_DESC;
		$("#HandleDesc").next().first().find('input').val(HandleDesc);
	});
	...
	function onHandle(empInfo){
		var entity = {
			...
			"HandleNodeIPAddress":"",
			"HandlerPWD":$("#pwd").val(),
			"HandleTypeID":IS_URGENT ? "2" : "1",
			"HandleTypeName":IS_URGENT ? "紧急" : "非紧急"
		};
		var loadIndex = layer.load();//开启加载层
		uxutil.server.ajax({
			url:HANDLE_URL,
