layui.extend({
	...
	var config = {
		...
		cols:[[
			...
			{field:'TimeOutCallFlag',width:90,title:'超时提醒',templet:function(d){
				var result = '';
				if(d.HandleFlag == '0' && d.isTimeout){
					if(d.TimeOutCallFlag == '1'){
						result = '<span style="color:#009688;">已提醒</span>';
					}else{
						result = '<span style="color:#FF5722;">未提醒</span>';
					}
				}
				return result;
			}},
			{field:'BUTTON',width:90,align:'center',toolbar:'#table-operate-bar'},
			{field:'TimeOutCallDateTime',width:160,title:'提醒时间',sort:true},
			...
			{field:'RecDeptID',width:180,title:'机构ID',hide:true}
		]],
	...
	tableInd.table.on('tool(table)', function(obj){
		if(obj.event === 'timeout'){
			openTimeoutWin(obj.data);
		}
	});
	//监听行双击事件
