layui.extend({
	...
	//危急值超时处理服务地址
	var TIMEOUT_URL = uxutil.path.LIIP_ROOT + "/ServerWCF/IMService.svc/ST_UDTO_SCMsgByTimeOutReSend";
	//获取机构信息
	...
	//危急值上报
	function onTimeoutClick(empInfo){
		...
		onUnCovertPasswordCheck(pwd,function(value){
			...
			isUserValid(account,pwd,function(){
				...
				getEmpInfoByAccount(account,function(info){
					//ID、超时未处理消息时提示标志、超时未处理消息时提示人ID、超时未处理消息时提示人名称、
					//超时未处理消息时提示接收人ID、超时未处理消息时提示接收人姓名、超时未处理消息时提示时间
					var fields = [
						'Id','TimeOutCallFlag','TimeOutCallUserID','TimeOutCallUserName',
						'TimeOutCallRecUserID','TimeOutCallRecUserName','TimeOutCallDateTime'
					];
					var params = {
						entity:{
							"Id":ID,
							"TimeOutCallFlag":"1",
							"TimeOutCallUserID":empInfo.RBACUser_HREmployee_HRDept_Id,
							"TimeOutCallUserName":empInfo.RBACUser_HREmployee_CName,
							"TimeOutCallRecUserID":info.RBACUser_HREmployee_Id,
							"TimeOutCallRecUserName":info.RBACUser_HREmployee_CName,
							"TimeOutCallDateTime":uxutil.date.toServerDate(uxutil.server.date.getDate())
						},
						fields:fields.join(',')
					};
					onTimeout(params);
