...
layui.extend({
...
}).define(['uxutil','uxtable','table','form','basicStatus'],function(exports){
	"use strict";
	...
	//获取样本单列表服务
	var GET_LIST_URL = uxutil.path.ROOT + '/ServerWCF/LabStarService.svc/LS_UDTO_QueryWillConfirmLisTestForm?isPlanish=true';
	//默认查询条件
	...
	Class.pt.getUrl = function(SERACHOBJ,url){
		...
		url+='&itemResultFlag='+SERACHOBJ.itemResultFlag;
    ...
    Class.pt.getWhere =function(obj){
		...
		if(obj.SickTypeID){//就诊类型
		 ...
		 //全部—智能审核成功样本
		if(obj.ZFSysCheckStatus)params.push("listestform.ZFSysCheckStatus=1");
		//全部—检验完成样本
        if(obj.TestStatus)params.push("(listestform.TestAllStatus=1 and listestform.FormInfoStatus=1)");
