	...
	app.initListeners = function () {
		...
		element.on('tab(Tab)', function (data) {
			if (data.index == 0) {//未确认条码
				var url = uxutil.path.ROOT + '/ui/layui/views/pre/sample/barcode/model1/confirmBarCode.html';
				me.openTabsPage(url);
			} else if (data.index == 1) {//已确认条码
				var url = uxutil.path.ROOT + '/ui/layui/views/pre/sample/barcode/model1/notConfirmBarCode.html';
				me.openTabsPage(url);
			} 
		});
	};
