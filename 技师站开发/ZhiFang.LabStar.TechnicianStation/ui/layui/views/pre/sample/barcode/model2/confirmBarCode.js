...
layui.extend({
	...
	PreSampleBarcodeMenzhenIndexConfirm:'modules/pre/sample/barcode/model2/confirmBarCode',
	CommonHostType:'modules/common/hosttype'
}).use(['uxutil', 'uxtoolbar','PreSampleBarcodeMenzhenIndexConfirm','CommonHostType'],function(){
	var $ = layui.$,
		uxutil = layui.uxutil,
		uxtoolbar = layui.uxtoolbar,
		PreSampleBarcodeMenzhenIndex = layui.PreSampleBarcodeMenzhenIndexConfirm,
	...
	function initToolbar(){
		...
		uxtoolbar.render({
			domId:'toolbar',
			buttons:[
				{type:'hisRefresh',text:'HIS获取',iconCls:'layui-icon-download-circle',buttonCls:''},
				{type:'lisRefresh',text:'LIS获取(F3)',iconCls:'layui-icon-refresh',buttonCls:''},
				{type:'checkSimple',text:'确认(F6)',iconCls:'layui-icon-ok-circle',buttonCls:''},
...
//				{type:'barcodeRead',text:'读卡(F2)',iconCls:'layui-icon-print',buttonCls:''},
				{type:'barcodeCancel',text:'条码作废',iconCls:'layui-icon-refresh',buttonCls:''},
//				{type:'cancel',text:'撤销确认',iconCls:'layui-icon-delete',buttonCls:''},
			...
			event:{
				hisRefresh:function(){PreSampleBarcodeMenzhenIndexInstance.getHisBarcode();},
				lisRefresh:function(){PreSampleBarcodeMenzhenIndexInstance.getLisBarcode();},
				checkSimple:function(){PreSampleBarcodeMenzhenIndexInstance.onCheckSimple();},
