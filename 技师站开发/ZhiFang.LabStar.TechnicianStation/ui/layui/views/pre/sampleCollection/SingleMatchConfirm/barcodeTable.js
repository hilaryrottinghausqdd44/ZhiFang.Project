...
layui.extend({
    ...
    var barcodeTable = {
        ...
        config: {
            ...
            parseData: function (res) {//res即为原始返回的数据
                ...
                if (count == 0) return;
                //触发点击事件
                $("#barcodeTable+div .layui-table-body table.layui-table tbody tr:first-child")[0].click();
    ...
    var Class = function (options) {
        var me = this;
        me.config = $.extend({}, barcodeTable.config, me.config, options);
        if (me.config.defaultLoad) me.config.url = me.getLoadUrl();
        me = $.extend(true, {}, barcodeTable, Class.pt, me);
        return me;
    };
    ...
    //核心入口
    barcodeTable.render = function (options) {
        var me = this;
    ...
    //对外公开-数据加载
    barcodeTable.onSearch = function (mytable, options) {
        var me = this;
        if (options) config = $.extend({}, config, options);
        var inst = new Class(me);
        config.loadIndex = layer.load();
        barcodeTable.url = inst.getLoadUrl();
        barcodeTable.config.elem = "#" + mytable;
        barcodeTable.config.id = mytable;
    ...
    Class.pt.iniListeners = function () {
        ...
        //监听排序事件
        table.on('sort(barcodeTable)', function (obj) {
    ...
    //暴露接口
    exports('barcodeTable', barcodeTable);
});
