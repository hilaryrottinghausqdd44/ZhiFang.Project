/**
 * @name：modules/pre/sample/gether/form 撤销采集
 * @author：liangyl
 * @version 2021-09-09
 */
layui.extend({
	uxutil:'ux/util'
}).define(['uxutil','form','layer'],function(exports){
	"use strict";
	...
	//扫码获取信息
	var BARCODE_LIST_URL = uxutil.path.ROOT + "/ServerWCF/LabStarPreService.svc/LS_UDTO_PreSampleRevocationGatherDataVerifyByBarCode";
	//撤销采集确认服务
	var SAVE_REVOCATION_URL = uxutil.path.ROOT + "/ServerWCF/LabStarPreService.svc/LS_UDTO_PreSampleRevocationGatherByBarCode";
		//样本单信息字段
	var FIELDS = [
	...
	Class.prototype.onBarCode = function(barcode,callback){
		...
		uxutil.server.ajax(config,function(data){
			...
			if(data.success){
			    ...
			    var ErrorInfo = data.value ? data.value : data.ErrorInfo;
				layer.msg(ErrorInfo,{ icon: 5, anim: 6 });
				me.clearData();
			}
		});
	};
		...
		if(!isTrue){
			...
			uxutil.server.ajax(config,function(data){
				...
				if(data.success){
					...
					//返回撤销成功的条码号
			        parent.afterUpateRevocation(data.value, barCode);
				}else{
