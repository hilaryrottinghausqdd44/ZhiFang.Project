...
layui.extend({
...
}).define(['uxutil','form','PreSampleContentIndex','PreSampleGetherBasicParams'],function(exports){
	"use strict";
	    ...
	    $("#"+me.config.domId+"-barCode").on('keydown', function (event) {
	        if (event.keyCode == 13) {
                ...
                if(barcode){
		            ...
		            var isConstraintUpdate = false;//强制更新
                    var isUpdate = false;//更新
                    $("#"+me.config.domId+"-barCode").blur();
	        ...
	        if (event.keyCode == 13) {
	        	...
	        	if(value){
	        	    PreSampleContentIndexInstance.onReceive(value,receiveType,function(list){
	        	    	...
	        	    	PreSampleContentIndexInstance.bLoadData(list);
	        	    });
	        	}else{
    ...
    Class.prototype.clearData = function(){
       ...
       //确认采集
    Class.prototype.updateSampleGatherState= function(){
    	var me = this;
    	...
    	PreSampleContentIndexInstance.onAccept();
    };
	//核心入口
