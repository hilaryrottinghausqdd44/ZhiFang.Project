	...
	var MOD_DOM = [
		'<div class="layui-form {domId}-grid-div" lay-filter="{domId}-form">',
			'<div class="layui-form-item" style="margin-bottom:0;">',
				...
				'<div class="layui-input-inline" id="check_all_div" style="width:105;float:right;">',
					'<input type="checkbox" name="" id="{domId}_check_all" title="全部勾选卡片" lay-filter="{domId}_check_all" lay-skin="primary">',
				'</div>',
...
//					'<div>审核时间：{checkTime}</div>',
					'<div>项目：{LisBarCodeFormVo_LisBarCodeForm_ParItemCName}</div>',
				'</div>',
			'</div>',
		'</div>'
	].join('');
		...
		$(document).keydown(function (event) {
			switch (event.keyCode) {
				case 13://回车 -- 样本号
					...
					if (document.activeElement == document.getElementById(me.config.domId + "-barcode")) {
						var ishis = PreSampleBarcodeBasicParamsInstance.get('Pre_OrderBarCode_DefaultPara_0060');
						if (ishis != null && ishis != "") {
							me.getHisBarcode();
						} else {
							me.getLisBarcode();
...
//			data[i].SexName = data[i].SexName == '男' ? '<span class="layui-badge">男</span>' : '<span class="layui-badge layui-bg-green">女</span>';
	...
	Class.prototype.onShowTable = function(){
		...
		if(me.table){
			...
			//else if (arr[0] == "LisBarCodeFormVo_LisBarCodeForm_ColorValue") {
			cols.push({
...
//				]],
	...
	Class.prototype.afterDataLoad = function(data){
		...
		if (error.length > 0) {
			...
			//}); 
			//layer.alert(error.join('<BR>'),{title:"以下项目无采样组或无默认采样组"});
		}
		
		me.config.LIST_DATA = result;
		me.config.Original_LIST_DATA = JSON.parse(JSON.stringify(result));
		//显示条码信息后打印条码
		var isautoprint = PreSampleBarcodeBasicParamsInstance.get('Pre_OrderBarCode_DefaultPara_0053');
		//界面是否控制自动打印
		var ZDDY = PreSampleBarcodeBasicParamsInstance.get('Pre_OrderBarCode_DefaultPara_0054');
		if (me.config.LIST_DATA.length > 0 && (ZDDY == "1" && autoPrintButtonStatus) || (ZDDY != "1" && isautoprint == "1"))
		{
			var barcodes = [];
			for (var i = 0; i < me.config.LIST_DATA.length; i++) {
				barcodes.push(me.config.LIST_DATA[i].LisBarCodeFormVo_LisBarCodeForm_BarCode);
			}
			me.onBarcodePrint(barcodes);
		}
	};
	//LIS获取条码
