...
}).define(['uxutil','uxbase','table','form','laydate','PreSampleBarcodeBasicParams','CommonSelectDept','CommonPrint'],function(exports){
	"use strict";
	...
	var MOD_DOM = [
		...
		'<div class="layui-row layui-col-space10 layui-form" lay-filter="{domId}-form-cards" style="background-color:#f2f2f2;padding:10px;margin:0;">',
			'<div class="layui-col-xs12 layui-col-sm12 layui-col-md12 layui-col-lg12">',
				...
				'<div id="{domId}-table" class="layui-hide"></div>',
			'</div>',
		'</div>',
		'<style>',
...
//			'.layui-form-onswitch-red{border-color:#FF5722;background-color:#FF5722;}',
	...
	Class.prototype.onSearch = function(){
		...
		uxutil.server.ajax({
			...
			if(data.success){
				me.config.LIST_DATA = (data.value ||{}).list || [];
				me.onShowData();
			}else{
	...
	function afterUpateRevocation(data, barCode) {		
		...
		if (data && data[0]) {
			...
			if (obj[barCode] == 'true') {
				...
				uxbase.MSG.onConfirm(BarCodeConfirmError, { title: '以下条码撤销失败，请检查样本状态', enter: true },
					function (index) {
						layer.close(index);//执行完后关闭
					},
					function (index) {
