/**
 * @name：modules/pre/sample/sign/content/list 签收列表
 * @author：zhangda
 * @version 2021-07-28
 */
layui.extend({
	...
	{  
		...
		//配置列
		me.tableConfig.cols = me.initCols(me.config.colsStr, me.config.sortStr);
	...
	//转换配置列
	Class.prototype.initCols = function (colsStr, sortStr) {
		var me = this,
			colsStr = colsStr || '',
			sortStr = sortStr || '',
		...
		$.each(colsStrArr, function (i, item) {
			var obj = {};
			...
			//存在排序
			if (sortStr.indexOf(obj["field"]) != -1) {
				obj["sort"] = true;
			}
			cols.push(obj);
		});
