/**
 * @name：modules/common/select/user 员工身份下拉框
 * @author：Jcall
 * @version 2020-06-30
 */
layui.extend({
	...
	var $ = layui.$,
		form = layui.form,
		uxutil = layui.uxutil,
		MOD_NAME = 'CommonSelectUser';
	
	//获取机构身份列表服务地址
	var GET_LIST_URL = uxutil.path.LIIP_ROOT + "/ServerWCF/RBACService.svc/RBAC_UDTO_SearchHREmpIdentityByHQL";
	//前处理系统编码
	var SYSTEM_CODE = "ZF_LAB_START";
	
	//员工身份下拉框
	var CommonSelectUser = {
		//对外参数
		config:{
			domId:null,
			code:null,//员工身份编码
	...
	var Class = function(setings){  
		var me = this;
		me.config = $.extend({},me.config,CommonSelectUser.config,setings);
	};
	//初始化HTML
	...
	Class.prototype.loadData = function(callback){
		...
		uxutil.server.ajax({
			...
			data:{
				isPlanish:true,
				fields:'HREmpIdentity_HREmployee_Id,HREmpIdentity_HREmployee_CName',
				where:"hrempidentity.TSysCode='" + me.config.code + "' and hrempidentity.SystemCode='" + SYSTEM_CODE + "'"
			}
		},function(data){
			if(data.success){
	...
	//核心入口
	CommonSelectUser.render = function(options){
	...
	//暴露接口
	exports(MOD_NAME,CommonSelectUser);
});