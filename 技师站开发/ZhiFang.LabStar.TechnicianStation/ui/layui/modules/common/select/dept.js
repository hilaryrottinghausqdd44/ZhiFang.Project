/**
 * @name：modules/common/select/dept 机构身份下拉框
 * @author：Jcall
 * @version 2020-06-30
 */
layui.extend({
	...
	var $ = layui.$,
		form = layui.form,
		uxutil = layui.uxutil,
		MOD_NAME = 'CommonSelectDept';
	
	//获取机构身份列表服务地址
	var GET_LIST_URL = uxutil.path.LIIP_ROOT + "/ServerWCF/RBACService.svc/RBAC_UDTO_SearchHRDeptIdentityByHQL";
	//前处理系统编码
	var SYSTEM_CODE = "ZF_LAB_START";
	
	//机构身份下拉框
	var CommonSelectDept = {
		//对外参数
		config:{
			domId:null,
			code:null,//机构身份编码
	...
	var Class = function(setings){  
		var me = this;
		me.config = $.extend({},me.config,CommonSelectDept.config,setings);
	};
	//初始化HTML
	Class.prototype.initHtml = function(list){
		...
		for(var i=0;i<len;i++){
			htmls.push('<option value="' + list[i].HRDeptIdentity_HRDept_Id + '">' + list[i].HRDeptIdentity_HRDept_CName + '</option>');
		}
	...
	Class.prototype.loadData = function(callback){
		...
		uxutil.server.ajax({
			...
			data:{
				...
				where:"hrdeptidentity.TSysCode='" + me.config.code + "' and hrdeptidentity.SystemCode='" + SYSTEM_CODE + "'"
			}
		},function(data){
			if(data.success){
	...
	//核心入口
	CommonSelectDept.render = function(options){
	...
	//暴露接口
	exports(MOD_NAME,CommonSelectDept);
});