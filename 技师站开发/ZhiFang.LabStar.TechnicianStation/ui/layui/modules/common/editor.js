/**
    @name：layui.ux.editorp 对layedit文本编辑器扩展
    @author：Jcall
    @version 2019-03-26
    @name: tools 可传入、  layedit-event='text' 则将该html直接加入、修改工具选中样式、取消右键 【用于项目快捷模板】
    @author：zhangda
    @version 2021-11-20
 */
/**
...
layui.extend({
    uxutil: 'ux/util',
    uxeditortemplet: 'ux/editortemplet'
}).define(['layer', 'form', 'code', 'uxutil', 'uxeditortemplet'], function (exports) {
    "use strict";
    ...
    var Edit = function () {
        ...
        that.config = {
            ...
            tools: {
                ...
                ,
                //添加扩展功能-标题
                title: '<i class="layui-icon layedit-tool-title" title="标题" layedit-event="title">标题</i>'
                ,
                content: '<i class="layui-icon layedit-tool-content" title="正文" layedit-event="content">正文</i>'
                ,
                holiday: '<i class="layui-icon layedit-tool-holiday" style="width:48px;" title="节假日" layedit-event="holiday">节假日</i>'
                ,
                mianban: '<i class="layui-icon layedit-tool-holiday" style="width:48px;" title="面板" layedit-event="">面板</i>'
                ,
                liebiao: '<i class="layui-icon layedit-tool-holiday" style="width:48px;" title="列表" layedit-event="">列表</i>'
            },
            //默认工具bar
            tool: [
    ...
    Edit.prototype.build = function (id, settings) {
        ...
        var tool = function () {
            ...
            layui.each(set.tool, function (_, item) {
                if (config.tools[item] && !hideTools[item]) {
                    node.push(config.tools[item]);
                }
            });
            return node.join('');
        }();
        ...
        var editor = $([
            '<div class="' + ELEM + '">',
            '<div class="layui-unselect layui-layedit-tool' + (config.tool.length == 0 ? "layui-hide" : "") + '">' +
            tool.replace("layBkColor_Index", "layBkColor_" +
                that.index).replace("layFontColor_Index", "layFontColor_" + that.index) + '</div>',
            '<div class="layui-layedit-iframe">',
            '<iframe id="' + name + '" name="' + name + '" textarea="' + id + '" frameborder="0"></iframe>',
            '</div>',
            '</div>',
            '<style>',
            '.layui-layedit-tool .layedit-tool-active{ background-color:transparent!important;color:#009688!important; }',
            '.layui-layedit-tool .layui-icon { width: auto!important;margin:3px 8px!important; }',
            '.layui-layedit-tool{ background-color: #eee; }',
            '</style>'
        ].join(''));
        ...
        iframe.css({
            ...
            var conts = iframe.contents(),
                ...
                style = $([
                    ...
                    'td{border: 1px solid #DDD;width:80px;padding:9px 15px;}',
    ...
    //快捷键处理
    var hotkey = function (iframeWin, iframe, textArea, set) {
    ...
    //在选区插入内联元素
    var insertInline = function (tagName, attr, range) {
    ...
    //工具选中
    var toolCheck = function (tools, othis) {
        var iframeDOM = this.document
        ...
        if (othis && othis.attr("layedit-event") != 'text') othis[othis.hasClass(CHECK) ? 'removeClass' : 'addClass'](CHECK);
        if (othis) othis.siblings('i').removeClass(CHECK);
    ...
    //触发工具
    var toolActive = function (iframeWin, editor, set) {
        ...
        //清除超链接
        toolEvent.unlink = function (range) {
            iframeDOM.execCommand('unlink');
        };
        //表情
        toolEvent.face = function (range) {
        ...
        //图片
        toolEvent.image = function (range) {
            var that = this;
        ...
        /*#Extens#*/
        //多图上传
        toolEvent.images = function (range) {
            var that = this;
            layer.open({
                type: 1
        ...
        //图片2
        toolEvent.image_alt = function (range) {
            var that = this;
            layer.open({
                type: 1
                , id: 'fly-jie-image-upload'
        ...
        //插入视频
        toolEvent.video = function (range) {
            var body = this
        ...
        //上传附件
        toolEvent.attachment = function (range) {
            var that = this;
            layer.open({
                type: 1
                , id: 'fly-jie-image-upload'
        ...
        //全屏
        toolEvent.fullScreen = function (range) {
        ...
        toolEvent.preview = function (range) {
            var currStyle = [];
            layui.each(set.quote.style, function (index, item) {
            ...
            docs = '<div style="padding:10px;">' + docs + '</div>';
            layer.open({
                type: 1
                , id: 'layui-kz-preview'
                , title: '预览'
        ...
        toolEvent.removeformat = function (range) {
            //if (device.ie)
        ...
        toolEvent.fontFomatt = function (range) {
        ...
        toolEvent.fontfamily = function (range) {
        ...
        toolEvent.fontSize = function (range) {
        ...
        toolEvent.customlink = function (range) {
            var container = getContainer(range)
                , parentNode = $(container).parent();
        ...
        toolEvent.anchors = function (range) {
        ...
        toolEvent.table = function (range) {
        ...
        toolEvent.addhr = function (range) {
            insertInline.call(iframeWin, 'hr', {}, range);
        };
        /*End*/
        //帮助
        toolEvent.help = function () {
            layer.open({
                type: 2
                , title: '帮助'
                , area: ['600px', '380px']
        ...
        var tools = editor.find('.layui-layedit-tool');
        ...
        var click = function () {
            ...
            //layedit-event=text 则直接将html加入body
            if (events == 'text') {
                body.append(othis.html());
        ...
        //添加扩展功能-开始
        //标题
        toolEvent.title = function (range) {
            var list = layui.uxeditortemplet.title,
                html = [];

            for (var i in list) {
                html.push('<div style="margin:5px;padding:5px;border:1px dashed red;">' + list[i].content + '</div>');
            }
            ...
            $('.editortitle').each(function (ind, elem) {
                $(this).on('click', function () {
                    var div = insertInline.call(iframeWin, 'div', {
                        text: $(this).html()
                    }, range);
                    $(div).after('<p></br></p>');
                    layer.close(index);
        ...
        //正文
        toolEvent.content = function (range) {
            var list = layui.uxeditortemplet.content,
                html = [];
            ...
            $('.editorcontent').each(function (ind, elem) {
                $(this).on('click', function () {
                    var div = insertInline.call(iframeWin, 'div', {
                        text: $(this).html()
                    }, range);
        ...
        //节假日
        toolEvent.holiday = function (range) {
            var list = layui.uxeditortemplet.holiday,
                html = [];
            ...
            $('.editorholiday').each(function (ind, elem) {
                $(this).on('click', function () {
                    var div = insertInline.call(iframeWin, 'div', {
                        text: $(this).html()
                    }, range);
        ...
        toolEvent.help = function () {
            layer.open({
                ...
                , offset: '100px'
                , skin: 'layui-layer-msg'
                , content: layui.uxutil.path.LAYUI + '/help/editor.html'
                , success: function (layero, index) {
                    //layer.full(index);
                }
            });
    ...
    //超链接面板
    var link = function (options, callback) {
    ...
    var customlink = function (options, callback) {
        var body = this, index = layer.open({
            type: 1
    ...
    var anchors = function (options, callback) {
        var body = this, index = layer.open({
            type: 1
    ...
    //表情面板
    var face = function (options, callback) {
        //表情库
        var faces = function () {
    ...
    var fontFomatt = function (options, callback) {
    ...
    var fontfamily = function (options, callback) {
    ...
    var fontSize = function (options, callback) {
    ...
    //插入代码面板
    var code = function (options, callback) {
    ...
    $.extend(edit.config, {
        tool: [
            ...
            //'|','html','preview','title','content','holiday','|','help','mianban','liebiao'
        ]
    });
    //暴露接口
    exports('uxeditor', edit);
    //添加扩展功能-结束
});
