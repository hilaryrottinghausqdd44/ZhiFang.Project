...
namespace ZhiFang.DAO.NHB.Base
{
    public abstract class BaseDaoNHB<T, T1> : HibernateDaoSupport, IDBaseDao<T, T1> where T : BaseEntity
    {
        ...
        public EntityList<T> GetListByHQL(string strHqlWhere, int start, int count, string strHQL, string strHQLCount)
        {
            EntityList<T> entityList = new EntityList<T>();
            if (!String.IsNullOrEmpty(strHQL))
            {
        ...
        public IList<T> GetListByHQL(string strHqlWhere, string strHQL)
        {
        ...
        public int GetCurPageByHQL(long id, string strHqlWhere, string Order, int count)
        {
            ...
            NHibernateHelper.GetTableAndMainFieldName<T>(this.SessionFactory, ref tableName, ref mainFieldName);
            string strSQL = NHibernateHelper.GetSqlByHql<T>(this.Session, this.SessionFactory, strHqlWhere);
            if (strSQL == "")
            {
                ZhiFang.Common.Log.Log.Info("无法获取HQL转换的SQL！");
                return -1;
            }
            if (tableName != "" && mainFieldName != "")
            {
                string strSort = " 0 as SortNumber, ";
                if (Order != null && Order.Trim().Length > 0)
                {
                    strSort = " ROW_NUMBER() OVER(ORDER BY " + Order + ") as SortNumber, ";
                }
                string querySQL = "select " + strSort + " " + mainFieldName + " from (" + strSQL + ") " + typeof(T).Name.ToLower();
                querySQL = "Select SortNumber from (" + querySQL + ") MM where " + mainFieldName + "=" + id + " order by SortNumber  ";
                string querySQLCount = "select count(*) from (" + strSQL + ") NN ";

                ZhiFang.Common.Log.Log.Info("分页定位SQLCount：" + querySQLCount);
                ZhiFang.Common.Log.Log.Info("分页定位SQL：" + querySQL);
                IList<int> tempCountList = this.Session.CreateSQLQuery(querySQLCount).List<int>();
                if (tempCountList != null && tempCountList.Count > 0)
                {
                    int allCount = tempCountList[0];
                    if (allCount > count)
                    {
                        IList<object> tempList = this.Session.CreateSQLQuery(querySQL).List<object>();
        ...
        public EntityPageList<T> GetListByHQL(long id, string strHqlWhere, string Order, int start, int count)
        {
            ...
            if (curPage > 0)
                start = curPage;
            EntityList<T> tempList = GetListByHQL(strHqlWhere, Order, start, count);
