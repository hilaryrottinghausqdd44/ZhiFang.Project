...
namespace ZhiFang.LabStar.Common
{
    public class MyNPOIHelper
    {
        ...
        public static MemoryStream ExportDT(DataTable dtSource)
        {
            ...
            HSSFDataFormat format = workbook.CreateDataFormat() as HSSFDataFormat;
            dateStyle.DataFormat = format.GetFormat("yyyy-mm-dd");
            int rowIndex = ExportDTByDataTable(dtSource, workbook, sheet, dateStyle, 0);
            using (MemoryStream ms = new MemoryStream())
            {
                workbook.Write(ms);
                ms.Flush();
        ...
        private static int ExportDTByDataTable(DataTable dtSource, HSSFWorkbook workbook, HSSFSheet sheet, HSSFCellStyle dateStyle, int firstRow)
        {
            //取得列宽
            int[] arrColWidth = new int[dtSource.Columns.Count];
            foreach (DataColumn item in dtSource.Columns)
        ...
        public static bool ExportDTtoExcel(DataTable dtSource, string strFileName)
        {
            try
            {
                string[] temp = strFileName.Split('.');
                ...
                {
                    ZhiFang.Common.Log.Log.Debug("xls格式的excel生成失败！");
                }
            }
            catch (Exception ex)
            {
