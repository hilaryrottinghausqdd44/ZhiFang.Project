    ...
    public class ParseObjectProperty : ZhiFang.Common.Public.JsonSerializerClass
    {
        ...
        public string GetObjectListPlanish<T>(EntityList<T> ObjectList)
        {
            ...
            if (ObjectList != null && ObjectList.count > 0)
            {
                ...
                {
                    if (ObjectList is EntityPageList<T>)
                        ResultStr = "{\"count\":" + ObjectList.count.ToString() + ",\"page\":" + (ObjectList as EntityPageList<T>).page.ToString() + ",\"list\":[" + tempStringBuilder.ToString().Remove(0, 1) + "]" + "}";
                    else
