<%@ Page language="c#" AutoEventWireup="True" Inherits="OA.RBAC.Organizations.OrgTreeStaff" Codebehind="OrgTreeStaff.aspx.cs" %>
...
function locateHref(node)
{
    if (node != null) {
        ...
        if (nodeData != "0" && nodeData.indexOf("_")<0) {
            var OrgStaff = OA.RBAC.Organizations.OrgTreeStaff.GetOrgStaff(nodeData).value;
            //alert(OrgStaff);
            window.status = OrgStaff;
            //node.nodeSrc = OrgStaff;

            node.setAttribute("treeNodeSrc", OrgStaff);
            node.databind();
...
//-->
	...
	language=javascript onload="return window_onload()" bgcolor="#d1d1d1" topmargin=0 rightmargin=0 leftmargin=0 bottommargin=0>
	...
	<table border="0" width="100%" align="center" cellspacing="0" cellpadding="0" style="COLOR: white;BORDER-COLLAPSE: collapse"
		...
		<tvns:treeview id="Treeview1" treenodesrc="<%=strOrgNodes.Replace("\"","\'") %>" target="MainList" 
		HelperID="__ModuleTree_State__" systemImagesPath="/webctrl_client/1_0/treeimages/" 
		selectExpands="false" 
