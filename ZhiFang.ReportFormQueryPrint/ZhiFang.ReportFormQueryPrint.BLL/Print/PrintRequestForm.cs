...
namespace ZhiFang.ReportFormQueryPrint.BLL.Print
{
    public class PrintRequestForm
    {
        ...
        private List<ReportFormFilesVO> TemplateDataBindAndCreatRequestForm(DataRow drreportform, DataTable dtri, DataTable dtrg, ReportFormTitle rft, string templatefullpath)
        {
            List<ReportFormFilesVO> reportformfileslist = new List<ReportFormFilesVO>();
            PrintRequestFormCommon prfc = new PrintRequestFormCommon();
            ...
            {
                case ".FRX":
                    reportformfileslist = prfc.CreatRequestFormFilesByFRX(drreportform, dtri, dtrg, templatefullpath, rft);
                    break;
                default: break;
            }
        ...
        private void SetReportItem(string reportformid, ref DataTable dtri)
        {
            ...
            DataSet dsri = rifb.GetRequestItemList_DataTable(reportformid).DataSet;
        ...
        private void SetReportMicro(string reportformid, ref DataTable dtrmicro)
        {
            ...
            DataTable dsri = rmfb.GetRequestMicroList(reportformid);
        ...
        private void SetReportMarrow(string reportformid, ref DataTable dtrmarrow)
        {
            ...
            DataTable dsri = rmarrowfb.GetRequestMarrowItemList(reportformid);
        ...
        public List<ReportFormFilesVO> CreatRequestFormFiles(List<string> reportformidlist, ReportFormTitle reportFormTitle, ReportFormFileType reportFormFileType, string st, int flag = 1,int pow=-1)
        {
            ZhiFang.ReportFormQueryPrint.BLL.BRequestForm brf = new BRequestForm();
            Model.RequestForm brf_m = new Model.RequestForm();
            List<ReportFormFilesVO> reportformfileslist = new List<ReportFormFilesVO>();
            BRequestMicro rmfb = new BRequestMicro();
            ...
            foreach (var reportformid in reportformidlist)
            {
                ...
                if (dtrf != null && dtrf.Rows.Count > 0)
                {
                    ...
                    if (flag == 1 || !prfc.ExistsReportForm_PDF(dtrf, ref tmpreportformfilesvo))
                    {
                            ...
                            case ZhiFang.ReportFormQueryPrint.Common.SectionType.TestGroupMicroSmear:
                                {
                                    ...
                                    try
                                    {
                                        ...
                                        dtri = rmfb.GetRequestMicroGroupList(reportformid);
                                    }
                                    catch (Exception ex)
                                    {
                        ...
                        #region 数据绑定生成报告
                        List<ReportFormFilesVO> reportpath = this.TemplateDataBindAndCreatRequestForm(dtrf.Rows[0], dtri, dtrg, reportFormTitle, TemplateFullPath);
        ...
        public List<ReportFormFilesVO> CreatRequestFormFilesBYSpid(List<string> reportformidlist, ReportFormTitle reportFormTitle, ReportFormFileType reportFormFileType, string st, int flag,string Template, int pow = -1)
        {
            ZhiFang.ReportFormQueryPrint.BLL.BRequestForm brf = new BRequestForm();
            Model.RequestForm brf_m = new Model.RequestForm();
            List<ReportFormFilesVO> reportformfileslist = new List<ReportFormFilesVO>();
            ...
            foreach (var reportformid in reportformidlist)
            {
                ...
                if (dtrf != null && dtrf.Rows.Count > 0)
                {
                    ...
                    if (flag == 1 || !prfc.ExistsReportForm_PDF(dtrf, ref tmpreportformfilesvo))
                    {
                        ...
                        ZhiFang.Common.Log.Log.Debug(TemplateFullPath);
                        #endregion
