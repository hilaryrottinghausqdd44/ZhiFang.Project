...
namespace ZhiFang.ReportFormQueryPrint.BLL.Print
{
    public class BHistoryShowFrom
    {
        ...
        public SortedList ShowFormTypeList(int SectionType, string PageName)
        {
            ...
            ZhiFang.Common.Log.Log.Debug("BHistoryShowFrom.ShowFormTypeList:SectionType=" + ((Common.SectionType)SectionType).ToString().Trim() + ";PageName=" + PageName);
            if (ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
            {
                for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                {
                    ...
                    {
                        ZhiFang.Common.Log.Log.Debug("BHistoryShowFrom.ShowFormTypeList字符串转枚举异常：" + e.ToString());
                    }
                    //ZhiFang.Common.Log.Log.Debug("BHistoryShowFrom.ShowFormTypeList.for:SectionTypem=" + ((Common.SectionType)SectionType).ToString().Trim() + ";SectionTypet=" + ds.Tables[0].Rows[i]["XSLName"].ToString().Trim() + ";PageName=" + ds.Tables[0].Rows[i]["PageName"].ToString().Trim());
                    //ZhiFang.Common.Log.Log.Debug("BHistoryShowFrom.ShowFormTypeList.for:tmpi=" + tmpi + ";SectionTypei=" + SectionType + ";PageName=" + ds.Tables[0].Rows[i]["PageName"].ToString().Trim());
        ...
        /// <returns></returns>
        public string ShowReportFormResult(string ReportFormNo, int SectionNo, string PageName, int ShowType,int SectionType)
        {
            try
            {
                ZhiFang.Common.Log.Log.Debug("ShowReportFormResult");
                ...
                ZhiFang.Common.Log.Log.Debug("BHistoryShowFrom.ShowReportFormResult:dtrf="+dtrf.Rows.Count + "dtri=" + dtri.Rows.Count);
                string modelPath = this.GetTemplatePath(dtrf, dtri, PageName, ShowType, SectionType);
                ZhiFang.Common.Log.Log.Debug("BHistoryShowFrom.ShowReportFormResult:modelPath=" + modelPath);
            ...
            {
                ZhiFang.Common.Log.Log.Debug("BHistoryShowFrom.ShowReportFormResult异常信息:"+ex.ToString() );
                return "";
            }
        }
        /// <summary>
        ...
        {
            ZhiFang.Common.Log.Log.Debug("BHistoryShowFrom.GetTemplatePath:pageName=" + pageName+ ";showType=" + showType + ";sectiontype=" + sectiontype);
            ...
            ZhiFang.Common.Log.Log.Debug("BHistoryShowFrom.GetTemplatePath:showmodel=" + showmodel);
        ...
        {
            BHistoryReportForm rffb = new BHistoryReportForm();
            BHistoryReportItem rifb = new BHistoryReportItem();
            BHistoryReportMicro rmfb = new BHistoryReportMicro();
            BHistoryReportMarrow rmarrowfb = new BHistoryReportMarrow();

            Model.HistoryReportForm rff_m = new Model.HistoryReportForm();
            Model.HistoryReportItem rif_m = new Model.HistoryReportItem();
            Model.HistoryReportMicro rmf_m = new Model.HistoryReportMicro();
            Model.HistoryReportMarrow rmarrowf_m = new Model.HistoryReportMarrow();
                    ...
                    case ZhiFang.ReportFormQueryPrint.Common.SectionType.Normal:
                        {
                            ...
                            try
                            {
                                dtri = rifb.GetReportItemList_DataTable(FormNo);
                            }
                            catch (Exception ex)
                            {
                            ...
                            try
                            {
                                ...
                                if (dtrf.Columns.Contains("STestType"))
                                {
                                    if (dtrf.Rows[0]["STestType"] != null && dtrf.Rows[0]["STestType"].ToString().Trim() != "")
                                    {
                                        if (dtrf.Rows[0]["STestType"].ToString().Trim() == "20")
                                        {
                                            dtri = rmfb.GetReportMicroGroupListForSTestType(FormNo);
                                        }
                                        if (dtrf.Rows[0]["STestType"].ToString().Trim() == "21")
                                        {
                                            dtri = rmfb.GetReportMicroGroupListForSTestType(FormNo);
                                    ...
                                    {
                                        dtri = rmfb.GetReportMicroGroupList(FormNo);
                                    }
                                }
                                else
                                {
                    ...
                    case ZhiFang.ReportFormQueryPrint.Common.SectionType.CellMorphology:
                        {
                            ...
                            try
                            {
                            dtri = rmarrowfb.GetReportItemList_DataTable(FormNo);
                            }
                            catch (Exception ex)
                            {
        ...
        public string GetResult(DataTable dtrf, DataTable dtri, string templatePath)
        {
            ...
            PrintHistoryReportFormCommon prfc = new PrintHistoryReportFormCommon();
            if (System.IO.Path.GetExtension(templateName).ToUpper() == ".FRX")
            {
                ZhiFang.Common.Log.Log.Debug("ZhiFang.ReportFormQueryPrint.BLL.Print.BHistoryShowFrom.GetResult.frx");
                tempHtml = prfc.CreatShowContextByFRX(dtrf, dtri, templatePath);
            }
            else if (System.IO.Path.GetExtension(templateName).ToUpper() == ".XSLT")
            {
                ZhiFang.Common.Log.Log.Debug("ZhiFang.ReportFormQueryPrint.BLL.Print.BHistoryShowFrom.GetResult.XSLT");
        ...
        /// <param name="sectiontype">小组类型</param>
        /// <returns></returns>
        public string ShowReportFormResultByReportTemp(string ReportFormNo, int SectionNo, string PageName, int ShowType, int SectionType)
        {
            try
            {
                ZhiFang.Common.Log.Log.Debug("ShowReportFormResult");
                    ...
                    case ZhiFang.ReportFormQueryPrint.Common.SectionType.Normal:
                        {
                            ...
                            try
                            {
                                dtri = rifb.GetReportItemList_DataTableByReportTemp(FormNo);
                            }
                            catch (Exception ex)
                            {
                                ZhiFang.Common.Log.Log.Debug("GetReportFormAndItemData.Normal异常信息:" + ex.ToString());
