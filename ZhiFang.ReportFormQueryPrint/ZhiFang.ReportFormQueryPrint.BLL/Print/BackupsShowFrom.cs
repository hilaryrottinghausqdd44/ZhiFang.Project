...
namespace ZhiFang.ReportFormQueryPrint.BLL.Print
{
    public class BBackupsShowFrom
    {
        ...
        public SortedList ShowFormTypeList(int SectionType, string PageName)
        {
            ...
            ZhiFang.Common.Log.Log.Debug("BBackupsShowFrom.ShowFormTypeList:SectionType=" + ((Common.SectionType)SectionType).ToString().Trim() + ";PageName=" + PageName);
            if (ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
            {
                for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                {
                    ...
                    {
                        ZhiFang.Common.Log.Log.Debug("BBackupsShowFrom.ShowFormTypeList字符串转枚举异常：" + e.ToString());
                    }
                    //ZhiFang.Common.Log.Log.Debug("BBackupsShowFrom.ShowFormTypeList.for:SectionTypem=" + ((Common.SectionType)SectionType).ToString().Trim() + ";SectionTypet=" + ds.Tables[0].Rows[i]["XSLName"].ToString().Trim() + ";PageName=" + ds.Tables[0].Rows[i]["PageName"].ToString().Trim());
                    //ZhiFang.Common.Log.Log.Debug("BBackupsShowFrom.ShowFormTypeList.for:tmpi=" + tmpi + ";SectionTypei=" + SectionType + ";PageName=" + ds.Tables[0].Rows[i]["PageName"].ToString().Trim());
        ...
        public string ShowReportFormResult(string ReportFormNo, int SectionNo, string PageName, int ShowType,int SectionType)
        {
            try
            {
                ...
                ZhiFang.Common.Log.Log.Debug("BBackupsShowFrom.ShowReportFormResult:dtrf="+dtrf.Rows.Count + "dtri=" + dtri.Rows.Count);
                string modelPath = this.GetTemplatePath(dtrf, dtri, PageName, ShowType, SectionType);
                ZhiFang.Common.Log.Log.Debug("BBackupsShowFrom.ShowReportFormResult:modelPath=" + modelPath);
            ...
            {
                ZhiFang.Common.Log.Log.Debug("BBackupsShowFrom.ShowReportFormResult异常信息:"+ex.ToString() );
                return "";
            }
        }
        /// <summary>
        ...
        {
            ZhiFang.Common.Log.Log.Debug("BBackupsShowFrom.GetTemplatePath:pageName=" + pageName+ ";showType=" + showType + ";sectiontype=" + sectiontype);
            ...
            ZhiFang.Common.Log.Log.Debug("BBackupsShowFrom.GetTemplatePath:showmodel=" + showmodel);
        ...
        {
            BBackupsReportForm rffb = new BBackupsReportForm();
            ...
            Model.BackupsReportForm rff_m = new Model.BackupsReportForm();
            Model.BackupsReportItem rif_m = new Model.BackupsReportItem();
            Model.BackupsReportMicro rmf_m = new Model.BackupsReportMicro();
            Model.BackupsReportMarrow rmarrowf_m = new Model.BackupsReportMarrow();
        ...
        public string GetResult(DataTable dtrf, DataTable dtri, string templatePath)
        {
            ...
            {
                ZhiFang.Common.Log.Log.Debug("ZhiFang.ReportFormQueryPrint.BLL.Print.BBackupsShowFrom.GetResult.frx");
                tempHtml = prfc.CreatShowContextByFRX(dtrf, dtri, templatePath);
            }
            else if (System.IO.Path.GetExtension(templateName).ToUpper() == ".XSLT")
            {
                ZhiFang.Common.Log.Log.Debug("ZhiFang.ReportFormQueryPrint.BLL.Print.BBackupsShowFrom.GetResult.XSLT");
