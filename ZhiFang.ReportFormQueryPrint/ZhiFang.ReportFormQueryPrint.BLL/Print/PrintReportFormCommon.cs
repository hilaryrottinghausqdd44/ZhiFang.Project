...
namespace ZhiFang.ReportFormQueryPrint.BLL.Print
{
    public class PrintReportFormCommon
    {
        ...
        {
            ZhiFang.Common.Log.Log.Debug("ZhiFang.ReportFormQueryPrint.BLL.Print.PrintReportFormCommon.CreatHtmlContextXslt");
            try
            {
                if (reportform.Rows.Count > 0)
                {
                    ...
                    //    }
                    //}
                    ZhiFang.Common.Log.Log.Debug("ZhiFang.ReportFormQueryPrint.BLL.Print.PrintReportFormCommon.CreatHtmlContextXslt.reportform.Rows.Count:" + reportform.Rows.Count);
            ...
            {
                ZhiFang.Common.Log.Log.Debug("hiFang.ReportFormQueryPrint.BLL.Print.PrintReportFormCommon.CreatHtmlContextXslt.异常：:" + e);
                return null;
            }
        }
        /// <summary>
        ...
        {
            ZhiFang.Common.Log.Log.Debug("CreatReportFormFilesByFRX.ReportFormID:"+ reportform["ReportFormId"].ToString());
            ...
            TextObject TxtIsSubreport = (TextObject)report.FindObject("Is_Need_Subreport");
            if (((TxtIsSubreport != null) && Validate.IsInt(TxtIsSubreport.Text.Trim())) && (Convert.ToInt32(TxtIsSubreport.Text.Trim()) == 1))
            {
                ZhiFang.Common.Log.Log.Debug("CreatReportFormFilesByFRX.按项目号赋值模板注入数据");
                RegeditSingleDataFRX(reportform, reportitem, ref report, templatefullpath, rft);
            }
            else{ 
            ...
            if (ZhiFang.ReportFormQueryPrint.Common.FilesHelper.CheckAndCreatDir(savepath))
            {
                switch (reportformfiletype)
                {
                    ...
                    case "PDF":
                        ...
                        tmpvo.PDFPath = tmppath + GetReportFormFileName_PDF(reportform);
        ...
        /// <param name="rft">抬头类型</param>
        /// <returns></returns>
        public string FindTemplate(DataRow drreportform, DataTable dtreportitem, ZhiFang.ReportFormQueryPrint.Common.ReportFormTitle rft,out string log, out string IsRFGraphdataPDf)
        {
            return FindTemplate(drreportform, dtreportitem, null, rft,out log,out IsRFGraphdataPDf,-1);
        }
        /// <summary>
        /// 查找模版
        /// </summary>
        ...
        /// <param name="rft">抬头类型</param>
        /// <returns></returns>
        public string FindTemplate(DataRow drreportform, DataTable dtreportitem, DataTable dtreportitem1, ZhiFang.ReportFormQueryPrint.Common.ReportFormTitle rft,out string whereLog,out string IsRFGraphdataPDf,long LabId)
        {
            ZhiFang.Common.Log.Log.Debug("FindTemplate.ReportFormId"+ drreportform["ReportFormId"].ToString());
            whereLog = "模板查询条件：";
            ...
            try
            {
                ...
                try
                {
                    sectionprint.SectionNo = long.Parse(drreportform["SectionNo"].ToString());
                    if (LabId>0)
                    {
                        sectionprint.LabID = LabId;
                    }
                }
                catch
                {
                ...
                {
                    Log.Debug("FindModel.SectionPrint.ReportFormID：" + drreportform["ReportFormID"].ToString() + "的模版.");
                    bool fitem = false;
                    ...
                    //判断就诊类型
                    if (listsectionprint.Where(a => a != null && a.sicktypeno != 0 && a.sicktypeno.ToString().Trim() != "" && a.sicktypeno.ToString().Trim() == drreportform["SickTypeNo"].ToString()).Count() > 0)
                    {
                        ZhiFang.Common.Log.Log.Debug("FindModel.SectionPrint.SickTypeNo：" + drreportform["SickTypeNo"].ToString());
                        listsectionprint = listsectionprint.Where(a => (a.sicktypeno.ToString() == drreportform["SickTypeNo"].ToString())).ToList();
                    }                  
                    
                    //判断客户
                    if (listsectionprint.Where(a => a != null && a.clientno != 0 && a.clientno.ToString().Trim() != "" && a.clientno.ToString() == drreportform["ClientNo"].ToString()).Count() > 0)
                    {
                        ZhiFang.Common.Log.Log.Debug("FindModel.SectionPrint.clientno：" + drreportform["ClientNo"].ToString());
                        listsectionprint = listsectionprint.Where(a => (a.clientno.ToString() == drreportform["ClientNo"].ToString())).ToList();
                    }
                    //判断样本类型
                    if (listsectionprint.Where(a => a != null && a.SampleTypeNo != 0 && a.SampleTypeNo.ToString().Trim() != "" && a.SampleTypeNo.ToString().Trim() == drreportform["SampleTypeNo"].ToString()).Count() > 0)
                    {
                        ZhiFang.Common.Log.Log.Debug("FindModel.SectionPrint.SampleTypeNo：" + drreportform["SampleTypeNo"].ToString());
                        listsectionprint = listsectionprint.Where(a  => (a.SampleTypeNo.ToString() == drreportform["SampleTypeNo"].ToString())).ToList();
                    }
                    ...
                    if (dtreportitem.Columns.Contains("MicroNo"))
                    {
                        ...
                        fMicro = false;
                        Log.Debug("PrintReportFormCommon.FindTemplate: MicroNo没有此列,不判断阴阳性");
                    }
                    ...
                    {
                        tmpsectionprint = listsectionprint.Where(a => (a.ItemCountMin <= itemcount && a.ItemCountMax >= itemcount));
                         whereLog += "不按照特殊项目匹配，阴性模板，有最大值最小值。itemcount:" + itemcount;
                    ...
                    if (tmpsectionprint != null && tmpsectionprint.Count() > 0)
                    {
                        ...
                        ZhiFang.Common.Log.Log.Debug("是否是外送报告："+tmpsectionprint.ElementAt(0).IsRFGraphdataPDf.ToString());
                        if (tmpsectionprint.ElementAt(0).IsRFGraphdataPDf.ToString() == "True") {
                            IsRFGraphdataPDf =  "True";
                        }
                        //外送报告有的项目能在数据库存储完整的项目数据，含有某些特殊项目时只存储了项目的关键字段，此时要使用第三方提供的pdf文件，但他们都在一个组不能单纯以IsRFGraphdataPDf字段区分
                        //要配置某些特殊项目，出现某些项目则直接用第三方的pdf
                        string specialSectionNo = ConfigHelper.GetConfigString("SpecialSectionNo");
                        string specialItemNo=ConfigHelper.GetConfigString("SpecialItemNo");
                        //先判断是否是配置的特定小组
                        if (!string.IsNullOrWhiteSpace(specialSectionNo) && specialSectionNo.Contains(drreportform["SectionNo"].ToString()))
                        {
                            ZhiFang.Common.Log.Log.Debug("指定小组号SpecialSectionNo：" + specialSectionNo);
                            //判断项目号
                            if (!string.IsNullOrWhiteSpace(specialItemNo))
                            {
                                if (dtreportitem != null && dtreportitem.Rows.Count > 0)
                                {
                                    for (int num = 0; num < dtreportitem.Rows.Count; num++)
                                    {
                                        //如果出现配置的特定项目号，则按外送报告调用pdf
                                        if (specialItemNo.Contains(dtreportitem.Rows[num]["ItemNo"].ToString()))
                                        {
                                            ZhiFang.Common.Log.Log.Debug("匹配到的特殊项目号：" + dtreportitem.Rows[num]["ItemNo"].ToString());
                                            IsRFGraphdataPDf = "True";
                                            break;
                                        }
                    ...
                    ZhiFang.Common.Log.Log.Debug("FindTemplate.whereLog:" + whereLog);
        ...
        public string PageTypeCheck(float height, float width)
        {
            ...
            if (Convert.ToInt32(height) == 297 && Convert.ToInt32(width) == 210)
                pagetype = "A4";
            if (Convert.ToInt32(height) == 270 && Convert.ToInt32(width) == 195)
                pagetype = "A4";
        ...
        public bool ExistsReportForm_PDF(DataTable brf, ref ReportFormFilesVO reportformfilesvo)
        {
            if (brf != null && brf.Rows.Count > 0)
            {
                ...
                string isUseFrxGeneratePDF = ConfigHelper.GetConfigString("IsUseFrxGeneratePDF");
                string reportformfileallpath = GetReportFormRelativePath(brf.Rows[0]) + GetReportFormFileName_PDF(brf.Rows[0]);
                string basepath = System.AppDomain.CurrentDomain.BaseDirectory;
                ZhiFang.Common.Log.Log.Debug(reportformfileallpath);
                ZhiFang.Common.Log.Log.Debug(basepath+reportformfileallpath);
                if (!String.IsNullOrEmpty(isUseFrxGeneratePDF) && isUseFrxGeneratePDF == "0")
                {
                    ZhiFang.Common.Log.Log.Debug("ZhiFang.ReportFormQueryPrint.BLL.Print.PrintReportFormCommon.ExistsReportForm_PDF:不通过frx模板生成PDF，直接使用生成好的PDF");
                ...
                if (File.Exists(basepath+reportformfileallpath))
                {
                    reportformfilesvo.PageCount = brf.Rows[0]["PageCount"].ToString();
                    reportformfilesvo.PageName = brf.Rows[0]["PageName"].ToString();
                    reportformfilesvo.PDFPath = reportformfileallpath;
                ...
                {
                    ZhiFang.Common.Log.Log.Debug("ZhiFang.ReportFormQueryPrint.BLL.Print.PrintReportFormCommon.ExistsReportForm_PDF:无生成好的文件");
                    return false;
                }
            }
            return false;
        ...
        /**public string GetReportFormFullPath(DataRow drrf)
        {
            ...
            reportformfilesname = drrf["ReportFormID"].ToString().Replace(" 00:00:00", "").Replace(",", "&") + ".PDF";
        ...
        public string GetReportFormFullPath(DataRow drrf)
        {
            ...
            ReportFormPath = GetReportFormPath(drrf);
            //reportformfilesname = drrf["ReportFormID"].ToString().Replace(" 00:00:00", "").Replace(",", "&") + ".PDF";
            DateTime checktime = DateTime.Parse(drrf["CheckTime"].ToString().Trim());
            reportformfilesname = drrf["ReportFormID"].ToString().Replace(" 00:00:00", "").Replace(",", "&").Replace(@"\", "") + "_" + checktime.ToLongTimeString().ToString().Replace(":", "-") + ".PDF";
            string reportformfullpath = ReportFormPath + reportformfilesname;
            return reportformfullpath;
        }
        ...
        public string GetReportFormRelativePath(DataRow drrf)
        {
            ...
            //DateTime receivedate = DateTime.Parse(drrf["Receivedate"].ToString().Trim());
            //ReportFormRelativePath = SysContractPara.ReportFormFilePath + receivedate.ToString("yyyy-MM-dd") + @"\";
            DateTime checkdate = DateTime.Parse(drrf["CheckDate"].ToString().Trim());
            ReportFormRelativePath = SysContractPara.ReportFormFilePath + checkdate.ToString("yyyy-MM-dd") + @"\";
            return ReportFormRelativePath;
        }
        ...
        {
            DateTime checktime = DateTime.Parse(drrf["CheckTime"].ToString().Trim());
            string reportformfilesname = drrf["ReportFormID"].ToString().Replace(" 00:00:00", "").Replace(",", "&").Replace(@"\","") + "_" + checktime.ToLongTimeString().ToString().Replace(":", "-") + ".PDF";
            return reportformfilesname;
        }
        ...
        public List<ReportFormFilesVO> CreatReportFormFilesBybyte(DataRow reportform, DataTable reportitem, DataTable reportgraph, string templatefullpath, ReportFormTitle rft)
        {
            ZhiFang.Common.Log.Log.Debug("CreatReportFormFilesBybyte.生成第三方外送报告开始.ReportFormID:" + reportform["ReportFormId"].ToString());
            List<ReportFormFilesVO> reportformfileslist = new List<ReportFormFilesVO>();
            string savepath = GetReportFormPath(reportform);//完整的存放PDF的文件夹路径
            string tmppath = GetReportFormRelativePath(reportform);//程序目录下存放PDF的文件夹的路径
            string[] pdfList = new string[reportgraph.Rows.Count];//用来存生成的pdf文件完整路径集合，为合并方法的传参
            string[] typeList = new string[reportgraph.Rows.Count];//为合并方法的传参
            ReportFormFilesVO tmpvo = new ReportFormFilesVO();
            #region 旧代码
            /* if (ZhiFang.ReportFormQueryPrint.Common.FilesHelper.CheckAndCreatDir(savepath))
            {
                if (reportgraph != null)
                {
                    for (var i = 0; i < reportgraph.Rows.Count; i++)
                    {
                        if (reportgraph.Columns.Contains("PointType") && reportgraph.Rows[i]["PointType"].ToString() == "8" && reportgraph.Columns.Contains("Graphjpg") && reportgraph.Rows[i]["Graphjpg"] != DBNull.Value)
                        {
                            var imagebytes = (byte[])reportgraph.Rows[i]["Graphjpg"];
                            if (ZhiFang.ReportFormQueryPrint.Common.FilesHelper.CreatDirFile(savepath, GetReportFormFileName_PDF(reportform), imagebytes))
                            {
                                tmpvo.PDFPath = tmppath + GetReportFormFileName_PDF(reportform);
                                tmpvo.PageCount = "1";
                                tmpvo.PageName = "A4";
                            ...
                            {
                                throw new Exception("生成第三方报告失败请查看数据是否正确，reportformid:" + reportform["ReportFormID"].ToString());
                            }
                        }
            ...
            try
            {
                if (ZhiFang.ReportFormQueryPrint.Common.FilesHelper.CheckAndCreatDir(savepath))
                {
                    if (reportgraph != null && reportgraph.Rows.Count > 0)
                    {
                        ...
                        for (var i = 0; i < reportgraph.Rows.Count; i++)
                        {
                            ...
                            {
                                var imagebytes = (byte[])reportgraph.Rows[i]["Graphjpg"];
                                string fileName = i + GetReportFormFileName_PDF(reportform);//要生成的pdf名字
                                if (ZhiFang.ReportFormQueryPrint.Common.FilesHelper.CreatDirFile(savepath, fileName, imagebytes))
                                {
                                    pdfList[i] = savepath + fileName;
                                    if (reportform["PageName"] != null && reportform["PageName"].ToString().Trim() != "")
                                    {
                                        typeList[i] = reportform["PageName"].ToString();
                                    }
                                    else
                                    {
                                        typeList[i] = "A4";
                                ...
                                {
                                    ZhiFang.Common.Log.Log.Debug("ZhiFang.ReportFormQueryPrint.BLL.Print.PrintReportFormCommon.CreatReportFormFilesBybyte:生成第三方报告失败请查看数据是否正确，reportformid:" + reportform["ReportFormID"].ToString());
                                    throw new Exception("ZhiFang.ReportFormQueryPrint.BLL.Print.PrintReportFormCommon.CreatReportFormFilesBybyte:生成第三方报告失败请查看数据是否正确，reportformid:" + reportform["ReportFormID"].ToString());
                                }

                            }
                            //报告库有的存的pdf文件，filepath是对应存放路径
                            else if (reportgraph.Columns.Contains("PointType") && reportgraph.Rows[i]["PointType"].ToString() == "8" && reportgraph.Columns.Contains("FilePath") && reportgraph.Rows[i]["FilePath"] != DBNull.Value)
                            {
                                isDeleteFile = false;
                                string FilePath = reportgraph.Rows[i]["FilePath"].ToString();
                        ...
                        //将所有生成的pdf合成一个
                        ZhiFang.Common.Log.Log.Debug("ZhiFang.ReportFormQueryPrint.BLL.Print.PrintReportFormCommon.CreatReportFormFilesBybyte:开始合成所有pdf");
                        bool isBlendSuccess = false;
                        string fullPath = savepath + GetReportFormFileName_PDF(reportform);
                        
                        if (reportform["PageName"] != null && reportform["PageName"].ToString().Trim().Equals("A5"))
                        {
                            //mergePdfFiles方法没有判断纸张类型直接合并提供的pdf集合
                            isBlendSuccess = PDFMergeHelp.mergePdfFiles(pdfList, fullPath, isDeleteFile);
                        }
                        else
                        {
                            //按比例缩放合并到A4纸上
                            isBlendSuccess = PDFMergeHelp.mergePdfFilesZoomToA4(pdfList, fullPath, isDeleteFile);
                        }
                        if (!isBlendSuccess)
                        {
                            ZhiFang.Common.Log.Log.Debug("ZhiFang.ReportFormQueryPrint.BLL.Print.PrintReportFormCommon.CreatReportFormFilesBybyte:合成失败！");
                        }
                        ZhiFang.Common.Log.Log.Debug("ZhiFang.ReportFormQueryPrint.BLL.Print.PrintReportFormCommon.CreatReportFormFilesBybyte:所有pdf合成结束");

                        tmpvo.PDFPath = tmppath + GetReportFormFileName_PDF(reportform);
                        if (reportform["PageCount"] != null && reportform["PageCount"].ToString().Trim() != "")
                        {
                            tmpvo.PageCount = reportform["PageCount"].ToString();
                        }
                        else
                        {
                            tmpvo.PageCount = "1";
                        ...
                        if (reportform["PageName"] != null && reportform["PageName"].ToString().Trim() != "")
                        {
                            tmpvo.PageName = reportform["PageName"].ToString();
                        }
                        else
                        {
                            tmpvo.PageName = "A4";
                        ...
                        ZhiFang.Common.Log.Log.Debug("ZhiFang.ReportFormQueryPrint.BLL.Print.PrintReportFormCommon.CreatReportFormFilesBybyte:未查询到第三方外送报告");
                    }
                    

                }
                ZhiFang.Common.Log.Log.Debug("ZhiFang.ReportFormQueryPrint.BLL.Print.PrintReportFormCommon.CreatReportFormFilesBybyte:生成第三方外送报告结束.ReportFormID:" + reportform["ReportFormId"].ToString());
            }
            catch (Exception e)
            {
                ZhiFang.Common.Log.Log.Debug("ZhiFang.ReportFormQueryPrint.BLL.Print.PrintReportFormCommon.CreatReportFormFilesBybyte:生成第三方报告失败" + e.Message);

            }
            return reportformfileslist;
        }

        public void RegeditSingleDataFRX(DataRow drreportform, DataTable dtreportitem, ref FastReport.Report report, string TemplateFullPath, ReportFormTitle rft)
        {
            try
            {
                #region 按单个项目赋值的模板数据处理
                for (int i = 0; i < dtreportitem.Rows.Count; i++)
                {
                    TextObject text = (TextObject)report.FindObject("TextRV" + dtreportitem.Rows[i]["ItemNo"].ToString());
                    if (text != null)
                    {
                        if (dtreportitem.Columns.Contains("ReportValue"))
                        {
                    ...
                    TextObject TextIV = (TextObject)report.FindObject("TextIV" + dtreportitem.Rows[i]["ItemNo"].ToString());
                    if (TextIV != null)
                    {
                        if (dtreportitem.Columns.Contains("ItemValue"))
                        {
                            TextIV.Text += dtreportitem.Rows[i]["ItemValue"].ToString();
                        }
                    }
                    TextObject textRR = (TextObject)report.FindObject("TextRR" + dtreportitem.Rows[i]["ItemNo"].ToString());
                    if (textRR != null)
                    ...
                    TextObject TextIU = (TextObject)report.FindObject("TextIU" + dtreportitem.Rows[i]["ItemNo"].ToString());
                    if (TextIU != null)
                    {
                        if (dtreportitem.Columns.Contains("ItemUnit"))
                        {
                            textRR.Text += dtreportitem.Rows[i]["ItemUnit"].ToString();
                        }
                    }
                }
                #endregion              
            ...
            {
                ZhiFang.Common.Log.Log.Debug("RegeditSingleDataFRX.异常:" + ex.StackTrace.ToString() + "---------" + DateTime.Now.ToString("yyMMDD hhmmss"));
            }
        }
    }
