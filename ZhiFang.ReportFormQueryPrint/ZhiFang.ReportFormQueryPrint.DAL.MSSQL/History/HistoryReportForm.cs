...
*/
                ...
                #region 执行视图
                ZhiFang.Common.Log.Log.Debug("MSSQL.History.GetReportFormFullList:" + FormNo);
                string[] p = FormNo.Split(';');
                ...
                ZhiFang.Common.Log.Log.Debug("MSSQL.History.GetReportFormFullList:sql=" + sql);
                if (ds.Tables.Count > 0)
                {
                    for (int i = 0; i < ds.Tables[0].Columns.Count; i++)
                    {
            ...
            {
                ZhiFang.Common.Log.Log.Debug("MSSQL.History.GetReportFormFullList:" + e);
                return new DataTable();
            }
        }
