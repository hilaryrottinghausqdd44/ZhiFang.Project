...
namespace ZhiFang.ReportFormQueryPrint.DAL.MSSQL.MSSQL66
{
    ...
    /// 数据访问类ReportForm。
    /// </summary>
    public class RequestForm : IDRequestForm
    {
        ...
        //}
        //public RequestForm(string ConnStr)
        //{
        //    DbHelperSQL = new DbHelperSQLObj(ConnStr);
        //}

        public int Add(Model.RequestForm t)
        {
            throw new NotImplementedException();
        }
        public DataSet GetList(Model.RequestForm t)
        {
            throw new NotImplementedException();
        }
        ...
        public int Update(Model.RequestForm t)
        {
            throw new NotImplementedException();
        }
        /// <summary>
        ...
        public int GetCountFormFull(string strWhere)
        {
            ...
            strSql.Append(" FROM RequestFormQueryDataSource ");
            if (strWhere.Trim() != "")
            {
        ...
        public DataTable GetRequestFormFullList(string FormNo)
        {
...
*/
                ...
                #region 执行视图
                ZhiFang.Common.Log.Log.Debug("MSSSQL6.6.GetRequestFormFullList:" + FormNo);
                string[] p = FormNo.Split(';');
                string receivedate = p[0];
                string sectionno = p[1];
                string testtypeno = p[2];
                string sampleno = p[3];
                var sql = "select * from RequestFormQueryDataSource where ReceiveDate='" + receivedate + "' and SectionNo=" + sectionno + " and TestTypeNo=" + testtypeno + " and SampleNo='" + sampleno + "' ";
                DataSet ds = DbHelperSQL.Query(sql);
                ZhiFang.Common.Log.Log.Debug("MSSSQL6.6.GetRequestFormFullList:sql=" + sql);
                if (ds.Tables.Count > 0)
                {
                    for (int i = 0; i < ds.Tables[0].Columns.Count; i++)
                    {
            ...
            {
                ZhiFang.Common.Log.Log.Debug("MSSSQL6.6.GetRequestFormFullList:" + e);
                return new DataTable();
            }
        }
        public bool UpdatePageInfo(string reportformlist, string pageCount, string pageName)
        {
            ...
            {
                StringBuilder strSql = new StringBuilder();
                strSql.Append(" update RequestForm set PageCount='" + pageCount + "' , pageName='" + pageName + "'   where  ");
        ...
        public bool UpdatePrintTimes(string[] reportformlist)
        {
            ...
            {
                StringBuilder strSql = new StringBuilder();
                strSql.Append(" update RequestForm set printtimes=printtimes+1 , PRINTDATETIME='" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "'         where  ");
        ...
        public bool UpdateClientPrintTimes(string[] reportformlist)
        {
            ...
            {
                StringBuilder strSql = new StringBuilder();
                strSql.Append(" update RequestForm set ClientPrint=isnull(ClientPrint, 0) +1 , PRINTDATETIME='" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "'         where  ");
                foreach (string a in reportformlist)
                {
                    string[] tmp = a.Split(';');
        ...
        public DataSet ResultMhistory(string ReportFormID, string PatNo, string Where)
        {
            ...
            string testtypeno = p[2];
            string sampleno = p[3];
            strSql.Append(" from RequestFormQueryDataSource where ReceiveDate='" + receivedate + "' and SectionNo=" + sectionno + " and TestTypeNo=" + testtypeno + " and SampleNo='" + sampleno + "' ");
            //strSql.Append(" FROM RequestFormQueryDataSource Where 1=1 ");
        ...
        public DataSet GetRequestFromByReportFormID(List<string> idList, string fields, string strWhere)
        {
            StringBuilder strSql = new StringBuilder();
            ...
            ZhiFang.Common.Log.Log.Debug("GetRequestFromByReportFormID:"+strSql.ToString());
            ZhiFang.Common.Log.Log.Debug("GetRequestFromByReportFormID.idList:" + idList + ";fields:" + fields + ";strWhere:" + strWhere);
            return DbHelperSQL.Query(strSql.ToString());
        }
    }
}
