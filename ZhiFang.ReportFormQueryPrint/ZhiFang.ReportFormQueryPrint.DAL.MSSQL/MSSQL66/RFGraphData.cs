	...
	public class RFGraphData:IDRFGraphData
	{
        ...
        public DataSet GetListByReportFormId(string ReportFormId)
        {
            ...
            if (p.Length >= 4)
            {
                ...
                {
                    ZhiFang.Common.Log.Log.Error("MSSQL66.GetListByReportFormId,异常：" + e.ToString());
                    return null;
                }
            }
            else
            {
                ZhiFang.Common.Log.Log.Error("MSSQL66.GetListByReportFormId,参数错误ReportFormId：" + ReportFormId);
                return null;
            }
        }
                        ...
                        for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                        {
                            ...
                            if (ds.Tables[0].Rows[i]["Graphjpg"] != DBNull.Value)
                            {
                                ...
                                if (ds.Tables[0].Rows[i]["pointtype"] != null && ds.Tables[0].Rows[i]["pointtype"].ToString().Trim() == "8")
                                {
                                    ...
                                    string filename = ReportPublicationID + "_" + ds.Tables[0].Rows[i]["ReceiveDate"].ToString() + "_" + ds.Tables[0].Rows[i]["GraphName"].ToString().Replace("/","") + "_" + ds.Tables[0].Rows[i]["GraphNo"].ToString() + "_" + ds.Tables[0].Rows[i]["PointType"].ToString() + ".pdf";
                                ...
                                else
                                {
                                    ...
                                    string path = System.AppDomain.CurrentDomain.BaseDirectory + ZhiFang.ReportFormQueryPrint.Common.ConfigHelper.GetConfigString("ReportFormImagesURL") + "RFGraphData\\" + Convert.ToDateTime(ds.Tables[0].Rows[i]["ReceiveDate"].ToString()).ToString("yyyy-MM-dd") + "\\";
                                    string filename = ReportPublicationID + "_" + ds.Tables[0].Rows[i]["ReceiveDate"].ToString() + "_" + ds.Tables[0].Rows[i]["GraphName"].ToString().Replace("/", "") + "_" + ds.Tables[0].Rows[i]["GraphNo"].ToString() + "_" + ds.Tables[0].Rows[i]["PointType"].ToString() + ".jpg";
                                    //if(!File.Exists(path+ filename))原有是否覆盖
                                    if (ZhiFang.ReportFormQueryPrint.Common.FilesHelper.CreatDirFile(path, filename, imagebytes))
                                    {
        ...
        public DataSet GetListByReportPublicationIDAndPointType(string ReportPublicationID, string PointType)
        {
			...
			if (p.Length >= 4)
			{
				...
				{
					DataSet ds = null;
					string param = " ReceiveDate='" + p[0] + "' and SectionNo=" + p[1] + " and TestTypeNo=" + p[2] + " and SampleNo='" + p[3] + "'";
					if (!string.IsNullOrEmpty(PointType.Trim()))
					{
						param += " and PointType not in ("+PointType+")";
					}
					ds = this.GetList(param);
					if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
					{
						ds.Tables[0].Columns.Add("FilePath");
						ds.Tables[0].Columns.Add("UrlPath");
						for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
						{
							byte[] imagebytes = null;
							if (ds.Tables[0].Rows[i]["Graphjpg"] != DBNull.Value)
							{
								...
								if (ds.Tables[0].Rows[i]["pointtype"] != null && ds.Tables[0].Rows[i]["pointtype"].ToString().Trim() == "8")
								{
									...
									{
										ds.Tables[0].Rows[i]["FilePath"] = ZhiFang.ReportFormQueryPrint.Common.ConfigHelper.GetConfigString("ReportFormImagesURL") + "RFGraphData\\" + Convert.ToDateTime(ds.Tables[0].Rows[i]["ReceiveDate"].ToString()).ToString("yyyy-MM-dd") + "\\" + filename;
										ds.Tables[0].Rows[i]["UrlPath"] = ZhiFang.ReportFormQueryPrint.Common.ConfigHelper.GetConfigString("ReportFormImagesURL") + "RFGraphData\\" + Convert.ToDateTime(ds.Tables[0].Rows[i]["ReceiveDate"].ToString()).ToString("yyyy-MM-dd") + "\\" + filename;
									}
									else
									{
										ZhiFang.Common.Log.Log.Debug(path + "" + filename + "保存失败！");
				...
				{
					ZhiFang.Common.Log.Log.Error("MSSQL66.GetListByReportPublicationIDAndPointType,异常：" + e.ToString());
					return null;
				}
			}
			else
			{
				ZhiFang.Common.Log.Log.Error("MSSQL66.GetListByReportPublicationIDAndPointType,参数错误ReportPublicationID：" + ReportPublicationID);
				return null;
			}
		}
    }
