...
namespace ZhiFang.ReportFormQueryPrint.Common
{
    public class ReportTemplateFrx
    {
        ...
        public static string CreatReport(DataSet ds, string templatecode, ReportFileType rft)
        {
            ...
            string url = "";
            string strModelPath = ConfigHelper.GetConfigString("ReportTemplateDir");
            string strBasePath=GetFilePath.GetPhysicsFilePath(strModelPath + "/");
            report.Clear();
            report.Load(strBasePath + templatecode + ConfigHelper.GetConfigString("ReportTemplateFileExtension"));
