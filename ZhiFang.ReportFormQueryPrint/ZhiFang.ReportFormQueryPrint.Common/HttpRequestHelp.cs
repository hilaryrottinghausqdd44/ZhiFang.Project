...
namespace ZhiFang.ReportFormQueryPrint.Common
{
    public class HttpRequestHelp
    {
        ...
        public static string Post(string PostData, string DataType,string url, bool isUseCert, int timeout, string SSLCERT_PATH, string SSLCERT_PASSWORD, bool IsVerification = false)
        {
            System.GC.Collect();//垃圾回收，回收没有正常关闭的http连接
        ...
        public static string Post(string PostData, string DataType, string url, int timeout)
        {
            ...
            try
            {
                ...
                * ************************************************************/
                ...
                if (DataType.Trim().ToUpper() == "OTHER")
                {
        ...
        public static string Get(string url,bool IsVerification=false)
        {
            ...
            {
                ZhiFang.Common.Log.Log.Error("HttpRequestHelp.Get.HttpService.Thread - caught ThreadAbortException - resetting.");
                ZhiFang.Common.Log.Log.Error("HttpRequestHelp.Get.Exception message: " + e.ToString());
                System.Threading.Thread.ResetAbort();
            }
            catch (WebException e)
            {
        ...
        {
            ZhiFang.Common.Log.Log.Debug("MD5Sign.str:" + str);
            var md5 = MD5.Create();
