 ...
 * @version 2018-09-20
 */
Ext.define('Shell.class.debug.app.PrintContent', {
    ...
    initComponent: function () {
		...
		var grid = Ext.create('Shell.class.debug.app.PrintContentTest',{
			itemId:'PrintContentTest',
			
		});
		
		me.items=[grid];
        var radioDataArr = [];
        if (me.hasReportPage) radioDataArr.push({ text: '报告', value: 1, checked: me.defaultCheckedPage == 1 ? true : false });
        if (me.hasResultPage) radioDataArr.push({ text: '结果', value: 2, checked: me.defaultCheckedPage == 2 ? true : false });
        if (me.hasTestPage) radioDataArr.push({ text: '测试', value: 3, checked: me.defaultCheckedPage == 3 ? true : false });
        ...
        me.toolbars = [{
            dock: 'top', itemId: 'toptoolbar', buttons: ['->', {
                ...
                listeners: { change: function () { 
                	if(me.getComponent('toptoolbar').getComponent('type').getValue(true) == 3){
