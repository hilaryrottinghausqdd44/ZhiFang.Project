 ...
 * @version 2018-09-20
 */
Ext.define('Shell.class.jianyanzhixingdiaoyong.queryRequestForm.PdfWins', {
	...
	//每次生成的数量
	createCount:5,
    selectUrl: Shell.util.Path.rootPath + '/ServiceWCF/ReportFormService.svc/GetRequestFromByReportFormID',
    
    ReportFormIdList:'',
    ReportFormPrintResult:[],
	...
	test:function(){
		var me = this;
		setTimeout(function(){
			if(me.ReportFormIdList && me.ReportFormIdList != ""){
				me.GetReportFromByReportFormID(me.ReportFormIdList);
			}else{
				var linshiReportFormIdlist = $("#ReportFormIdList").val();
				if(linshiReportFormIdlist && linshiReportFormIdlist!=""){
					$("#ReportFormIdList").val("");
					me.ReportFormIdList = linshiReportFormIdlist;
				}
			}
			me.test();
		},1000);
	},
	GetReportFromByReportFormID:function(idList){
		var me = this;
		var config = [];
		var isprint=false;
        var data = { idList: idList, fields: "FormNo,RECEIVEDATE,SECTIONNO,SectionType,CNAME,SAMPLENO,PageName,PageCount", strWhere:""};
		    ...
		    Ext.Ajax.request({
		        ...
		        success: function (response, options) {
		            ...
		            if(rs.ResultDataValue ==null || rs.ResultDataValue ==""){
		            	//控件返回值
		            	
		            	var ReportFormPrintResult = $("#ReportFormPrintResult").val();
		            	if(ReportFormPrintResult == "" || ReportFormPrintResult == null){
		            		var idlists = idList.split(",");
		            		for(var i= 0;i<idlists.length;i++){
		            			if(me.ReportFormPrintResult.length < 1){
		            				me.ReportFormPrintResult[i]='{ReportFormId:"'+idlists[i]+'",status:"0",error:"未查到报告单"}';
		            			}else{
		            				me.ReportFormPrintResult[me.ReportFormPrintResult.length++]='{ReportFormId:"'+idlists[i]+'",status:"0",error:"未查到报告单"}';
		            			}
		            		}
		            		$("#ReportFormPrintResult").val("["+me.ReportFormPrintResult+"]");
		            		me.ReportFormIdList="";
		            	}else{
		            		me.ReportFormPrintResult = ReportFormPrintResult;
		            		var idlists = idList.split(",");
		            		for(var i= 0;i<idlists.length;i++){
		            			me.ReportFormPrintResult[me.ReportFormPrintResult.length++]='{ReportFormId:"'+idlists[i]+'",status:"0",error:"未查到报告单"}';
		            		}
		            	...
		            	rsr = JSON.parse(rs.ResultDataValue).rows;
	                    total = JSON.parse(rs.ResultDataValue).total;
			            if(total!=0){
	                            for (var a = 0; a < rsr.length;a++) {
	                                config.push({
	                                    ReportFormID: rsr[a].FormNo,
	                                    DATE: rsr[a].RECEIVEDATE.split(" ")[0],
	                                    SectionNo: rsr[a].SECTIONNO,
	                                    SectionType: rsr[a].SectionType,
	                                    CNAME: rsr[a].CNAME,
	                                    SAMPLENO: rsr[a].SAMPLENO,
	                                    PageName: rsr[a].PageName,
	                                    PageCount: rsr[a].PageCount,
	                                    url: null
	                                });
	                            }
	                            var strPageName = "双A5";
	                            Ext.Ajax.defaultPostHeader = 'application/json';
	                            Ext.Ajax.request({
	                                url: Shell.util.Path.rootPath + '/ServiceWCF/ReportFormService.svc/GetSectionPrintStrPageNameBySectionNo',
	                                async: false,
	                                method: 'get',
	                                params: { SectionNo: rsr[0].SECTIONNO },
	                                success: function (response) {
	                                    rs = Ext.JSON.decode(response.responseText);
	                                    if(rs.success){
	                                    	strPageName = rs.ResultDataValue.replace('"','').replace('"','');
	                                    }
	                                }
	                            })
	                            me.ReportFormIdList="";
								me.print(config,strPageName);
	 ...
	 */
	/**@public 打印*/
	print: function (config,strPageName, isView, pdfPrinter) {
		var me = this;
        ...
        me.A4Type = 1;
        me.strPageName = strPageName;
        me.reportList = config;
	...
	isPrinting: function(index, callback) {
		...
		var pdfurl = Shell.util.Path.rootPath +"/"+obj.url;
		me.getToServer(pdfurl, function(v) {
		...
		for(var i = 0; i < len; i++) {
			...
			var ReportFormPrintResult = $("#ReportFormPrintResult").val();
        	if(ReportFormPrintResult == "" || ReportFormPrintResult == null){
        		if(me.ReportFormPrintResult.length == 0){
        			me.ReportFormPrintResult[i]='{ReportFormId:"'+row.ReportFormID+'",status:"1",error:""}';
        		}else{
        			me.ReportFormPrintResult[me.ReportFormPrintResult.length++]='{ReportFormId:"'+row.ReportFormID+'",status:"1",error:""}';
        		}
        		
        	}else{
        		me.ReportFormPrintResult = ReportFormPrintResult;
        		for(var i=0;i<me.ReportFormPrintResult.length;i++){
        			if(me.ReportFormPrintResult[i].id != row.ReportFormID){
        				me.ReportFormPrintResult[me.ReportFormPrintResult.length++]='{ReportFormId:"'+row.ReportFormID+'",status:"1",error:""}';
        			}
        		}
	...
	onServerCreateReportOne:function(index,callback){
        ...
        me.getToServer(url, function(v) {
			...
			if(result.success) {
				...
				//控件返回值
            	if(me.ReportFormPrintResult == "" || me.ReportFormPrintResult == null){
            		if(me.ReportFormPrintResult.length < 1){
            			me.ReportFormPrintResult[0]='{ReportFormId:"'+data.ReportFormID+'",status:"0",error:"生成失败"}';
            		}else{
            			me.ReportFormPrintResult[me.ReportFormPrintResult.length++]='{ReportFormId:"'+data.ReportFormID+'",status:"0",error:"生成失败"}';
            		}
            	}else{
            		me.ReportFormPrintResult[me.ReportFormPrintResult.length++]='{ReportFormId:"'+data.ReportFormID+'",status:"0",error:"生成失败"}';
            	}
