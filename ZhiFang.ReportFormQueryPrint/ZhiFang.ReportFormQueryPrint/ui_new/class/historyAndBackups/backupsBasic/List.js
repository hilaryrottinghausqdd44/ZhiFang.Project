 ...
 * @version 2020-01-08
 */
Ext.define('Shell.class.historyAndBackups.backupsBasic.List',{
	...
	/**获取数据服务路径*/
	selectUrl: '/ServiceWCF/ReportFormBackupsService.svc/SelectReportSort',
    /**当前打印ID*/
	printId:null,
	/**打印错误信息数组*/
	errorInfo:[],
	...
	HistoryCompare:function(store,record){
        ...
        if(ReportFormID.length>0){//是否选择数据
			...
			Ext.Ajax.request({
				method: 'POST',
				async: false,
			    url: Shell.util.Path.rootPath +'/ServiceWCF/ReportFormBackupsService.svc/ResultMhistory',
			    params:Ext.JSON.encode(data),
				success: function(response){
				    flag=response;
				},
	...
	createDockedItems: function () {
	    ...
	    dockedItems.push(Ext.create('Shell.class.historyAndBackups.backupsBasic.PdfWinS', {
	        itemId: 'PdfWin',
	        header: false,
	        height: 0,
	        width: 0,
