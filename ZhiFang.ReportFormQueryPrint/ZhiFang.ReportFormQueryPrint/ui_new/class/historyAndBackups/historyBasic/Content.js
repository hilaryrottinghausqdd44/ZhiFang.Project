 ...
 * @version 2020-01-08
 */
Ext.define('Shell.class.historyAndBackups.historyBasic.Content', {
    ...
    /**获取数据服务路径*/
    getImgeSrcUrl: '/ServiceWCF/ReportFormHistoryService.svc/PreviewReport',
    /**返回数据类型,1:报告;2:结果*/
    resultType: 1,
    ...
    initComponent: function () {
        ...
        var radioDataArr = [];
        if (me.hasReportPage) radioDataArr.push({ text: '预览',itemId:'yulan', value: 1, checked: me.defaultCheckedPage == 1 ? true : false });
        if (me.hasResultPage) radioDataArr.push({ text: '结果',itemId:'jieguo', value: 2, checked: me.defaultCheckedPage == 2 ? true : false });

        me.toolbars = [{
            dock: 'top', itemId: 'toptoolbar', buttons: ['->', {
                xtype: 'uxradiogroup', itemId: 'type2', defaultSelect: 1, margin: '0 10 0 0',
                data: radioDataArr,
    ...
    getResultTypeValue: function () {
        var me = this,
			type = me.getComponent('toptoolbar').getComponent('type2'),
			value = type.getValue(true);
        return value;
    },

    /**@public 更改内容*/
    changeContent: function (params, isPrivate,isenableControl) {
        var me = this,
        ...
        if (!isPrivate && (!params || !params.ReportFormID || !params.SectionNo || !params.SectionType)) {
            var errorInfo = [];
            if (!params) {
                me.showError("Shell.class.basic.Content的changeContent方法没有接收到参数对象!");
                return;
            }
            errorInfo.push("Shell.class.basic.Content的changeContent方法接收的参数对象有错!");
    ...
    getContent: function (params, callback) {
        ...
        if (ModelType == "report") {
		    ...
		    var url = "/ServiceWCF/ReportFormHistoryService.svc/GetReportFormPDFByReportFormID";
		    if(me.IsQueryRequest+"" == "true"){
		    	url = "/ServiceWCF/ReportFormService.svc/GetRequestFormPDFByReportFormID";
		    }else{
		    	url = "/ServiceWCF/ReportFormHistoryService.svc/GetReportFormPDFByReportFormID";
		    }
        ...
        if(me.IsQueryRequest+"" == "true"){
        	...
        	me.getImgeSrcUrl = "/ServiceWCF/ReportFormHistoryService.svc/PreviewReport";
        }
