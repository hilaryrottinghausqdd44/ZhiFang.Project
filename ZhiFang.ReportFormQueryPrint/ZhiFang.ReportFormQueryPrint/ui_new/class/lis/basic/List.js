 ...
 * @version 2018-09-20
 */
Ext.define('Shell.class.lis.basic.List',{
	    ...
	    if (me.hasPrint) {
			...
			if(me.IsUseClodopPrint == 'true'){
				buttons.push({
					...
					handler: function () {
						me.fireEvent('nodata',this);
						
						var PdfWin = me.getComponent('PdfWin');
						PdfWin.clodopPreviewPrint=true;
						me.onPrintClick(this,false);
					}
				});
			}
			buttons.push('-');
			...
			buttons.push({
	            ...
	            //         }
                //    }else{
                //     	Shell.util.Msg.showInfo("超过下载数量限制，最多下载"+me.MaxDownLoadNum+"份！");
                //    }
                }
            });
	...
	HistoryCompare:function(store,record){
        ...
        if(ReportFormID.length>0){//是否选择数据
        	...
        	if(me.QueryType == 'REQUEST'){
        		resultUrl = '/ServiceWCF/ReportFormService.svc/RequestResultMhistory';
        	}else{
        		resultUrl = '/ServiceWCF/ReportFormService.svc/ResultMhistory';
        	}
        	var PatNO = record[0].data.PatNo;
        	var data = {ReportFormID:ReportFormID,PatNO:PatNO,SectionType:SectionType};
			Ext.Ajax.defaultPostHeader = 'application/json';
			Ext.Ajax.request({
				method: 'POST',
				async: false,
			    url: Shell.util.Path.rootPath + resultUrl,
			    params:Ext.JSON.encode(data),
				success: function(response){
				    flag=response;
				},
	...
	Mhistor:function(store,record,response){
		...
		Shell.util.Win.open('Shell.class.lis.basic.ProjectContrast', {   
	...
	createGridColumns: function () {
        ...
        //数据列信息
 		if(me.PreView){
 			columns.push({dataIndex: 'PatNo', text: '病历号', width: 100});
	   		columns.push({dataIndex: 'CNAME', text: '姓名', width: 50});
	   		columns.push({dataIndex: 'CHECKDATE', text: '审核日期', width: 100,
			...
			return value;
	   		...
	   		columns.push({dataIndex: 'CHECKTIME', text: '审核时间', width: 100,
			renderer:function (v, meta, record, index) {
				//显示审核时间
	   		...
	   		columns.push({dataIndex: 'PRINTTIMES', text: '打印次数', width: 60,
	    ...
	    me.columns.push({dataIndex: 'CLIENTNO', text: '客户', width: 40, hideable: true, hidden: true});
	    me.columns.push({dataIndex: 'DistrictNo', text: '病区-病房', width: 40, hideable: true, hidden: true});
	    me.columns.push({dataIndex: 'SampleTypeNo', text: '样本类型', width: 40, hideable: true, hidden: true});
	    me.columns.push({dataIndex: 'SECTIONNO', text: '检验小组', width: 40, hideable: true, hidden: true});
	    me.columns.push({dataIndex: 'DeptNo', text: '科室', width: 40, hideable: true, hidden: true});
	    me.columns.push({dataIndex: 'SickTypeNo', text: '就诊类型', width: 40, hideable: true, hidden: true});
	},
    /**创建数据集属性*/
	...
	getStoreFields: function () {
        ...
        for (var i in fields) {
            ...
            if ('CHECKDATE' == fields[i]) { boRDT = true; }
			...
			if('CNAME'== fields[i]){bo = true;}
			if('PRINTTIMES'== fields[i]){boRDT = true;}
	    ...
	    fields.push('PatNo','CLIENTNO','DistrictNo','SampleTypeNo','DeptNo','SickTypeNo');
	    return fields;
	},
    /**创建挂靠*/
	createDockedItems: function () {
	    ...
	    dockedItems.push(Ext.create('Shell.class.lis.basic.PdfWinS', {
	        itemId: 'PdfWin',
	        header: false,
	        height: 0,
	        width: 0,
			...
			QueryType:me.QueryType,
			NewWindowLoadIframeToPrint:me.NewWindowLoadIframeToPrint,
	    ...
	    dockedItems.push(Ext.create('Shell.class.basic.PdfDownload', {
		    itemId: 'PdfDownload',
		    header: false,
		    height: 0,
		    width: 0,
	...
	onPrintClick:function(but){
		...
		if(!me.PreView){
			me.winpanel = Shell.util.Win.open('Ext.panel.Panel', {   
			    title: '打印选项',
		        ...
		        items:[
	                ...
	                },
	            	{
						...
						store:Ext.create('Ext.data.Store', {
			                    fields: ['text', 'value'],
			                    data: [
		                            { text: '当前', value: 'dangqian' },
		                            { text: '客户', value: 'CLIENTNO' },
		                            { text: '病区', value: 'DistrictNo' },
		                            { text: '样本类型', value: 'SampleTypeNo' },
		                            { text: '检验小组', value: 'SECTIONNO' },
		                            { text: '科室', value:'DeptNo'}
			                    ]
			                }),
			            value: ['dangqian']
	                },
	                {
		                ...
		                itemId: 'spcname',
		                disabled:false,
		                className: 'Shell.class.lis.basic.sectionprint',
		                classConfig:{sectionNo:records[0].data.SECTIONNO},
		                listeners: {
		                    check: function (p, record) {
			                    me.winpanel.getComponent('spcname').setValue(record.get("PrintProgram"));
				                p.close();
		                    }
		                }
	                },
		        ...
		        dockedItems: [{
				    ...
				    items: [
						...
						iconCls:'button-print',
						handler:function(){
							var poValue = me.winpanel.getComponent('PrintOrder').value;
							var reportV = me.winpanel.getComponent('reportV').value;
							var spcname = me.winpanel.getComponent('spcname').value;
							if(reportV){
								for(i=0;i<records.length;i++){
						        	var data = {ReportFormID:records[i].data.ReportFormID,SectionType:records[i].data.SectionType,QueryType:me.QueryType};
									Ext.Ajax.defaultPostHeader = 'application/json';
									Ext.Ajax.request({
										method: 'POST',
										async: false,
									    url: Shell.util.Path.rootPath +'/ServiceWCF/ReportFormService.svc/RequestIsPrintNullValues',
									    params:Ext.JSON.encode(data),
										success: function(response){
										     var result = Ext.JSON.decode(response.responseText);
										     if(!result.success){
										     	records[i].remove(records[i]);
										     }
										},
										error : function(response){
											Shell.util.Msg.showInfo("执行错误！");
							...
							if(poValue=='dangqian'){
							    ...
							    for(i=0;i<records.length-1;i++){
							        for(j=0;j<records.length-1-i;j++){
							            if(records[j].get(poValue)>records[j+1].get(poValue)){
							                var temp=records[j];
	                    	...
	                    	var isurl = false;
	                    	var urlls = [];
	                    	if(spcname != null && spcname != ""){
								for(i=0;i<records.length;i++){
						        	var data = {ReportFormID:records[i].data.ReportFormID,SectionNo:records[i].data.SECTIONNO,SectionType:records[i].data.SectionType,flag:1,Template:spcname,QueryType:me.QueryType};
									Ext.Ajax.defaultPostHeader = 'application/json';
									Ext.Ajax.request({
										method: 'POST',
										async: false,
									    url: Shell.util.Path.rootPath +'/ServiceWCF/ReportFormService.svc/GetReportFormPDFByReportFormIDANDTemplate',
									    params:Ext.JSON.encode(data),
										success: function(response){
										    var result = Ext.JSON.decode(response.responseText);
										    var ResultDataValue = JSON.parse(result.ResultDataValue);
										    if(result.success){
										    	isurl=true;
										    	urlls.push(ResultDataValue.PDFPath);
										    }else{
										    	isurl=false;
										    	urlls.push('');
										    }
							...
							if(iscall){
								me.printAll(records,isurl,urlls);
							}
	                    	me.winpanel.close();
						}
					}]
			...
			if(records.length >= 2){
				for(var i=0;i<records.length;i++){
					if(records[i].get("SECTIONNO") != null && records[i].get("SECTIONNO") != ""){
						if(records[0].get("SECTIONNO")!=records[i].get("SECTIONNO")){
							me.winpanel.getComponent('spcname').setDisabled(true);
							return;
						}
					}else{
						me.winpanel.getComponent('spcname').setDisabled(true);
	...
	/**打印勾选的报告*/
	printAll:function(records,isurl,urlls){
		var me = this,
		...
		if (PdfWin) {
			if(me.hasPdfPrinter){
				...
				PdfWin.print(me.getReportPrintInfo(records,isurl,urlls),null,printer);
			}else{
				PdfWin.print(me.getReportPrintInfo(records,isurl,urlls));
			}
		} else {
	...
	/**获取勾选的数据*/
	getReportPrintInfo:function(records,isurl,urlls){
	    var me = this,
			toptoolbar = me.getComponent('toptoolbar'),
			strPageName = toptoolbar.getComponent('strPageName').getValue(),
			hasFilter = me.getComponent('toptoolbar').getComponent('hasFilter'),
		...
		for(var i=0;i<count;i++){
			...
			if(isurl){
				...
				urll = Shell.util.Path.reportPath + "/" + fileName;
			}
			var obj = {
	...
	DownloadPDF:function(){
		var me = this,
		    records = me.getSelectionModel().getSelection(),
			len = records.length;;
		if(records.length<me.MaxDownLoadNum){	
			for(var i = 0; i<records.length;i++){
				if(records[i].data.bTempReport && records[i].data.bTempReport==1){
					Shell.util.Msg.showError("部分审核的报告不允许打印!");
					return;
				}
				if(records[i].data.ReportType && records[i].data.ReportType == "中间报告"){
					Shell.util.Msg.showError("中间报告不允许打印!");
					return;
				}
			}	
			if(len == 0){
				Shell.util.Msg.showError("必须选择数据才能进行下载!",me);
				return;
			}
			
			var PdfDownload = me.getComponent('PdfDownload');
			if (PdfDownload) {
					PdfDownload.print(me.getReportPrintInfo(records));
			} else {
			    Shell.util.Msg.showError('下载组件加载失败！',me);
			}
		}else{
		 	Shell.util.Msg.showInfo("超过下载数量限制，最多下载"+me.MaxDownLoadNum+"份！");
		}
	...
	onBeforeLoad: function () {
	    ...
	    me.disableControl();//禁用 所有的操作功能
  		...
  		//服务路径
        if(me.QueryType=="REQUEST"){
			me.selectUrl=me.selectUrl.replace("SelectReport","SelectRequest");
		}else{
			me.selectUrl=me.selectUrl.replace("SelectRequest","SelectReport");
		}
