 ...
 * @version 2020-02-06
 */
Ext.define('Shell.class.basic.PdfWinS', {
	...
	isPrintEnd:false,//是否打印结束
	i:0,//循环体
	mapkey:1,//当前打印的数据的map key值
	printmaps:{},//传入参数数据集合
	/**
	 * @public 打印
	 * @example
    ...
    DelphiPrint:function(){
    	...
    	if(len <= 0){
			...
			var isclose = false;
			me.endPrintDPF(isclose);
		}else{
			me.showProgresstInfo('共 ' + len + ' 份打印文件需要打印');
		
			for(var i = 0 ;i < len;i++){
				var printnum = i+1;
				me.onPrintProgressbarChange(printnum, len);//打印进度条
				me.showProgresstInfo('正在打印第 ' + printnum + ' 份文件');
				if (reports[i].url.toLowerCase().indexOf(".pdf")<0) {
                    me.showProgresstInfo(reports[i].CNAME + " 打印失败", false, true);
                    continue;
                }
				var url = Shell.util.Path.rootPath + "/" +reports[i].url.replace(/\\/g, "/");
				var params = '{"list":[{"url":"'+url+'"}]}';
				var strResult = false;
					strResult = JS_DELPHI_BOM.onShellPrint(params, me.pdfPrinter);
				if(strResult){
					ids.push(reports[i].ReportFormID);
					var info = reports[i].DATE + ' ' + reports[i].CNAME + ' ' + reports[i].PageName + ' ' + reports[i].PageCount + '张';
					me.showProgresstInfo(info, false, false, {
						marginLeft: '20px'
					});
		...
		if(me.openAddPrintTimes) {
			me.delphiAddPrintTimes(ids.join(",")); //增加打印次数
		}
		me.endPrintDPF(true);
    },
    MergePrint: function (config, isView, pdfPrinter, isMerge) {
        var me = this;
        me.i = me.i+1;
		me.printmaps[me.i]=[{config:config,isView:isView,pdfPrinter:pdfPrinter,isMerge:isMerge}];
		
		if(me.i==1){
			me.MergePrintLogic(1);
		}else if(me.printmaps[me.i-1]==null &&  me.isPrintEnd && me.printmaps[me.i] != null){
			me.isPrintEnd = false;
			me.MergePrintLogic(me.i);
		}
    },
	MergePrintLogic:function(num){
		var me = this;
		var allParameter = me.printmaps[num][0];
    	var isView = allParameter.isView;
    	var config = allParameter.config;
    	var pdfPrinter = allParameter.pdfPrinter;
    	var isMerge = allParameter.isMerge;
    	me.mapkey = num;
	...
	isPrinting: function(index, callback) {
		...
		me.printNum += len;
		if (me.IsChrome && me.IsUseClodopPrint == 'false') {//加了clodop打印后的修改
	...
	endPrintDPF: function(isClose) {
		...
		}else if(me.isPrintEnd && me.mapkey<arr && me.printmaps[me.mapkey+1] != null  && me.printmaps[me.mapkey+1][0].isMerge == true){
			MergePrintLogic(me.mapkey+1);
		};
	...
	showProgresstInfo: function(info, isBold, isError, style) {
		...
		if(me.printmaps[me.mapkey][0].isMerge){
			...
			if(me.NewWindowLoadIframeToPrint=='true'){
				me["progressInfoWin"+me.mapkey].modal = false;
			}
		};		
	...
	GetPdf: function (index) {
		...
		//Shell.util.Win.open("Shell.class.basic.ChromeTwoPrintView");
		//针对谷歌2.0版本加载不出来pdf问题，要重新打开个pdf加载
		if(me.NewWindowLoadIframeToPrint=='true'){
	...
	/**壳打印增加打印次数*/
	delphiAddPrintTimes: function(ids) {
	    var me = this,
			url = me.addPrintTimesUrl + "?reportformidstr=" + ids;
		...
		Ext.Ajax.request({
			...
			success: function(response){
				var responsea = Ext.decode(response.responseText);
	...
	getPrintList: function() {
		...
		if(!me.ForcedPagingField) { //不强制切分
			...
			for(var i = 0; i < len; i++) {
				var obj = reportList[i];
				list[0].urlList.push(reportList[i].url);
				list[0].indexList.push(i);
			}
		} else { //强制切分
			var patient = {};
			for(var i = 0; i < len; i++) {
...
//				}
	...
	loadPdf:function(url){
		var pdfurl = Shell.util.Path.rootPath +"/"+url;
		this.getToServer(pdfurl, function(v) {});
    }
});