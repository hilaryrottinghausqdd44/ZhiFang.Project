 ...
 * @version 2018-09-20
 */
Ext.define('Shell.class.basic.List',{
	...
	/**获取数据服务路径*/
	selectUrl: '/ServiceWCF/ReportFormService.svc/SelectReportSort',
	//生成报告服务
...
/*					setTimeout(function(){
	    ...
	    if (me.hasPrint) {
			...
			buttons.push({
                ...
                listeners: {
                    click: function () {
                        ...
                        }else if(record.length ==1 && record[0].data.PatNo != null){
                        	me.HistoryCompare(store,record);                       	
                        }else{
                        	Shell.util.Msg.showInfo("仅含有病历号的数据可以进行对比，请您重新选择数据！");
                        	return;
			...
			buttons.push({
                ...
                handler: function () {
					...
					/* var record = me.getSelectionModel().getSelection();//获取选择的行数据
					 if(record.length<10){
					 	if(record.length > 0){
					     	var ReportFormID = "";
					     	var SectionType ="";
	...
	Mhistor:function(store,record,response){
		...
		Shell.util.Win.open('Shell.class.basic.ProjectContrast', {   
	    ...
	    me.columns.push({
                ...
                items: [{
	                ...
	                handler: function (grid, rowIndex, colIndex) {
	                    ...
	                    me.onPrintClick(this,false,rec);
	                }
	            }]
            });
	...
	getStoreFields: function () {
	    ...
	    var bo = false,bo2 = false,boRID=false,boSENO=false,boST=false,boRE=false,boRDT=false,isAddPatNo=true;
        for (var i in fields) {
			...
			if('PatNo'== fields[i]){isAddPatNo = false;}
        }
		...
		if(isAddPatNo){fields.push('PatNo')};
	...
	createDockedItems: function () {
	    ...
	    if(JS_DELPHI_API){
	    	me.hasPdfPrinter = true;
	    	IsDelphi = true;
	    }
	    
	    if(me.hasPdfPrinter){
	    	var list = me.pdfPrinterList || [],
					len = list.length,
					data = [],
					defaultPrinter = "";
	    	if(JS_DELPHI_API){
	    		var Printer = JS_DELPHI_API.onGetPrinterList();
		    	var  PrinterList= Ext.JSON.decode(Printer);
		        for(var i = 0;i<PrinterList.data.length;i++){
		        	data.push([PrinterList.data[i].PrintName,'【报告打印机】' + PrinterList.data[i].PrintName]);
		        }
	    	}else{
	    		//新建一个WScript.Shell对象     
				...
				defaultPrinter = me._defaultPrinter.split(',')[0];
		    ...
		    dockedItems.unshift({
		    	...
		    	items:[{
		            ...
		            mode:'local',editable:false,
		            displayField:'text',valueField:'value',
		            width:'50%',fieldLabel:'打印机',labelWidth:50,
	    ...
	    dockedItems.push(Ext.create('Shell.class.basic.PdfWinS', {
	        itemId: 'PdfWin',
	        header: false,
	        height: 0,
	        width: 0,
	...
	/**点击直接打印处理*/
    onPrintClick: function (but,isMerge,record){
		var me = this,
		...
		for(var i = 0; i<recordsisbTempReport.length;i++){
			...
			if(recordsisbTempReport[i].data.ReportType && recordsisbTempReport[i].data.ReportType=="中间报告"){
				Shell.util.Msg.showError("中间报告不允许打印!");
				return;
			}
		}
		/*var strResult = JS_DELPHI_BOM.onGetVersion();
		alert(strResult);*/
	    /**
		 * 批量打印功能只支持IE8以上版本
		 * @JcallShell
	    ...
	    if (IEVersion == -1 || IEVersion == 6 || IEVersion == 7 || IEVersion == 'edge') {
	    	...
	    	if(JS_DELPHI_API){
    			isdelphi = true;
    			me.getComponent("PdfWin").IsDelphi = true;
	    	}else{
	    		isdelphi = false;
	    	}
	    	//判断是否谷歌
    ...
    mergeGetReportPrintInfo: function () {
        ...
        for (var i = 0; i < count; i++) {
            ...
            //如果是普通生化的合并
            if (record.get('SectionType') == 1 || record.get('SectionType') == 3) {
                ReportFormID += ReportFormid + ",";
                IsMerge = true;
            } else {
