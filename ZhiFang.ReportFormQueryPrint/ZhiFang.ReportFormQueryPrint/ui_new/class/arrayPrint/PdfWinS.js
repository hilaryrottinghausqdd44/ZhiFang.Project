 ...
 * @version 2018-09-20
 */
Ext.define('Shell.class.arrayPrint.PdfWins', {
	...
	initComponent: function() {
		...
		//if (url.indexOf("?") != -1) {
		var url = $("#ReportFormIdList").val();
		var strs = url.split("&");
		me.QueryType = decodeURIComponent(strs[1].replace("QueryType=",""));//获取区分查询REPOER和REQUEST参数
        idList = decodeURIComponent(strs[0].replace("ReportFormID=",""));//获取url传过来的ReportFormID数组
        PreView = decodeURIComponent(strs[2].replace("PreView=", ""));//区分预览打印
        var PageType = null;
        if(strs[3]){
        	PageType = decodeURIComponent(strs[3].replace("PageType=", ""));//打印类型
        }

		if(me.QueryType=="REQUEST"){
            me.selectUrl = me.selectUrl.replace("GetReportFromByReportFormID","GetRequestFromByReportFormID");
		}else{
            me.selectUrl = me.selectUrl.replace("GetRequestFromByReportFormID","GetReportFromByReportFormID");
        }
		    ...
		    Ext.Ajax.request({
		        ...
		        success: function (response, options) {
		            ...
		            if(total!=0){
                                ...
                                Ext.Ajax.request({
                                    ...
                                    success: function (response) {
                                        ...
                                        if(rs.success){
                                        	...
                                        	Shell.util.Msg.showError('查询打印类型失败！');
		            						return;
                                        }
                                    }
                                })
                        ...
                        } else if (PreView == 1 || PreView == 2){
							window.location.href = Shell.util.Path.uiPath + '/class/lis/index.html?'+url;
			            }else{
			            	Shell.util.Msg.showError('PreView传入参数错误，请检查参数！');
		            		return;
			            }
                        
		            }else{
		            	Shell.util.Msg.showError('未查询到报告单，请检查传入参数并排查数据！');
		            	return;
		            }
    ...
    MergePrint: function (config, isView, pdfPrinter, isMerge) {
        var me = this;
        me.isView = (isView === true ? true : false);
        me.pdfPrinter = pdfPrinter;
        me.isMerge = isMerge;
        ...
        //先生成报告再打印
         me.onServerCreateReport(function () {
            ...
            me.showProgresstInfo('【开始打印报告】', true);
            for (var i = 0; i < me.reportList.length; i++) {
                if (me.reportList[i].url.toLowerCase().indexOf(".pdf")<0) {
                    me.showProgresstInfo(me.reportList[i].CNAME + " 打印失败", false, true);
                    continue;
                }
                var printUrl = { url: me.reportList[i].url,indexList:[i] };
                me.printList[i] = printUrl;
            }
             if (me.IsChrome) {
                    //单面打印
                    if(me.IsUseClodopPrint == 'true'){
