 ...
 * @version 2019-10-16
 */
Ext.define('Shell.class.opd.basic.List',{
	...
	createButtonsToolbar: function () {
	    ...
	    buttons.push({
            ...
            handler: function () {
                var record = me.getSelectionModel().getSelection();
                if(record.length>0){
                	var  ReportFormIdList = "";
                	var SectionType = "";
                	//获取选中的数据----------
                	for(var i= 0;i<record.length;i++){
                		if(i==0){
                			ReportFormIdList = record[i].data.ReportFormID;
                			SectionType = record[i].data.SectionType;
                		}else{
                			ReportFormIdList += ","+record[i].data.ReportFormID;
                			SectionType += ","+record[i].data.SectionType;
                	...
                	//向后台发送请求查询样本清单信息返回PDF路径并打印--------
                	var data = {ReportFormIdList:ReportFormIdList,SectionType:SectionType};
                	Ext.Ajax.defaultPostHeader = 'application/json';
					Ext.Ajax.request({
						method: 'POST',
						async: false,
					    url: Shell.util.Path.rootPath +'/ServiceWCF/ReportFormService.svc/GetSampleReportFromByReportFormIdList',
					    params:Ext.JSON.encode(data),
						success: function(response){
							var response = Ext.decode(response.responseText);
							var ResultDataValue = JSON.parse(response.ResultDataValue);
							if(response.success){
								if(Ext.isChrome){//判断是否是谷歌
									//谷歌打印
									var url = Shell.util.Path.rootPath +"/" +ResultDataValue.PDFPath
									var iframe = document.createElement('iframe');
								    iframe.appendChild(document.createElement('body'));
								    me.getEl().appendChild(iframe);
								    var doc = iframe.contentWindow.document;
								    doc.body.innerHTML = '<iframe id="sampleReport" src="' + url +
											        '" height="100%" width="100%" frameborder="0" ' +
											        'style="overflow:hidden;overflow-x:hidden;overflow-y:hidden;height:100%;width:100%;' +
											        'position:absolute;top:0px;left:0px;right:0px;bottom:0px;" ' +
										        '></iframe>';
								    var p = doc.getElementById("sampleReport");
								    p.contentWindow.print();
								}else{
									me.initIframeHtml(ResultDataValue.PDFPath);
								}
							}else{//IE打印
								Shell.util.Msg.showInfo(response.ErrorInfo);
							}							    
                	...
                	Shell.util.Msg.showInfo("请选择报告单！");
                }
           } 
        });
        buttons.push({
            ...
            handler: function () {
            	var record = me.getSelectionModel().getSelection();
            	if(record.length == 1 && record[0].data.SectionType !="" && record[0].data.SectionType != null){
            		//判断用户是否登录
					var UserNo = me.getCookie("UserNo");
					var UserCName = me.getCookie("UserCName");
					var ShortCode = me.getCookie("ShortCode");
				  	var cookie = {UserNo:UserNo,CName:UserCName,ShortCode:ShortCode}
					if (UserNo == null || UserNo == "") {
					     Shell.util.Win.open('Shell.class.opd.userSign.app.App', {
				            formtype: 'add',
				            //resizable: false,
				            ...
				            listeners: {
				                login: function (p) {
				                    p.close();
				                    me.win = Shell.util.Win.open('Shell.class.opd.edit.EditAPP',{
				                    	records:record[0].data,
				                    	cookie:cookie,
				                    	listeners:{
				                    		updatesave:function(a){
						        				me.win.close();
						        				me.fireEvent("itemclick",null,record[0],1);
						        			},																		
				                    		beforeshow: function () {
			                                    this.x =  2;
			                                    this.y = 1;
            		...
            		Shell.util.Msg.showInfo("请选择样本!");
            	}else if(record.length > 1){
            		Shell.util.Msg.showInfo("只能选择一个样本!");
            	}
            }
        });
	    buttons.push('->');
	...
	Mhistor:function(store,record,response){
		...
		Shell.util.Win.open('Shell.class.opd.basic.ProjectContrast', {   
	...
	createDockedItems: function () {
	    ...
	    dockedItems.push(Ext.create('Shell.class.opd.basic.PdfWinS', {
	        itemId: 'PdfWin',
	        header: false,
	        height: 0,
	        width: 0,
