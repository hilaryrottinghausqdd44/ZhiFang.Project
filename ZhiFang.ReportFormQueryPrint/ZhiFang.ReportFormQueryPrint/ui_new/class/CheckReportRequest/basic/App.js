 ...
 * @version 2020-09-04
 */
Ext.define('Shell.class.CheckReportRequest.basic.App', {
	...
	//列表格式
	ListClassName: 'Shell.class.CheckReportRequest.basic.List',
	/**获取定义参数*/
	...
	afterRender: function() {
		...
		var QueryPanel = me.getComponent('QueryPanel'),
			...
			patientMessage = List.getComponent('patientMessage');

		patientMessage && patientMessage.on({
			beforeSearch: function() {
				me.clearData();
			},
			itemclick: function(view, record) {
				//console.log(record);
				me.onSearchList(me.ListSelectWhere, record);
			}
		});
		//列表行选中监听
		ReportListcenter && ReportListcenter.on({
			...
			itemclick: function(view, record) {
				if (record.get("ReportStatus") == "检验完成") {
			...
			if(field != "") {
				...
				for(var i in params) {
					...
					if(i == me.DateField) { //报告时间
						...
						if(arr.length == 1) {
							field.setValue({
								start: Shell.util.Date.getDate(arr[0])
							});
	...
	createApps: function() {
		...
		me.IsUseClodopPrint+="";
		var apps = [{
			className: 'Shell.class.CheckReportRequest.basic.QueryPanel',
			itemId: 'QueryPanel',
			region: 'north',
			split: true,
			collapsible: true,
			...
			className: 'Shell.class.CheckReportRequest.basic.sampleStateList.sampleListconent',
			...
			className: 'Shell.class.CheckReportRequest.basic.Content',
			autoScroll: true,
			itemId: 'Content',
			header: false,
			region: 'center',
			...
			className: 'Shell.class.CheckReportRequest.basic.HistoryCompare',
	...
	onSearch: function(where, isPrivate) {
		...
		var isWhereValid = me.isWhereValid();
		if(isWhereValid) {
			patientMessage.internalWhere = patientMessage.getInternalWhere();
			me.ListSelectWhere = where;
			patientMessage.load(where, isPrivate);
		} else {
			Shell.util.Msg.showWarning('条件不完整（除日期外，需要起码一个其他条件才能查询）！');
		}
	},
	/**查询*/
	onSearchList: function(where, record, isPrivate) {
		debugger;
		var me = this,
			List = me.getComponent('List'),
			ReportListcenter = List.getComponent('ReportListcenter');
		...
		if(where == null || where == ""){
			if(me.externalWhere != null && me.externalWhere != ""){
				where = me.externalWhere;
			}else{
				where = " 1=1 ";
			}			
		}else{
			if(where.indexOf('DATE') < 0){
				//where += " and "+ me.externalWhere;	
			}
		}
		if(record.get("Bed") != null && record.get("Bed") != "") {
			//where += " and Bed='" + record.get("Bed") + "'";
		}
		if (record.get("CNAME") != null && record.get("CNAME") != "") {
			where += " and CName='" + record.get("CNAME") + "'";
		}
		if (record.get("PatNo") != null && record.get("PatNo") != "") {
			where += " and PatNo='" + record.get("PatNo") + "'";
		}
		if(record.get("DeptNo") != null && record.get("DeptNo") != "") {
			//where += " and DeptNo='" + record.get("DeptNo") + "'";
		}
		if(record.get("SickTypeNo") != null && record.get("SickTypeNo") != "") {
			//where += " and SickTypeNo='" + record.get("SickTypeNo") + "'";
		}
		//where条件是否符合规范
		var isWhereValid = me.isWhereValid();
		if(isWhereValid) {
		...
		for(var i in pars) {
			...
			if(i.indexOf('DATE') > 0) { //报告时间
				...
				if(arr.length == 1) {
					...
					if(arr[1]) {
						da.push(i + "<'" + Shell.util.Date.toString(Shell.util.Date.getNextDate(arr[1]), true) + "'");
					}
