 ...
 * @version 2018-09-03
 */
Ext.define('Shell.class.CheckReportRequest.basic.List',{
	...
	/**获取数据服务路径*/
	selectUrl: '/ServiceWCF/ReportFormService.svc/SelectRequestFormAndReportForm',
	//生成报告服务
							...
							Ext.Ajax.request({
								...
								success: function(response){
									...
									if(response.success){
										...
										if(JS_DELPHI_API){
											var urls = Shell.util.Path.rootPath +"/"+ ResultDataValue;
   											var strResult = JS_DELPHI_BOM.onDownloadfile(urls); 
										}else{
	...
	createGridColumns: function () {
	    ...
	    //me.columns.push({dataIndex: 'ReceiveDateTime', text: '核收时间', width: 120, hideable: true, hidden: false});
	    me.columns.push({dataIndex: 'ReportStatus', text: '检测状态', width: 100, hideable: true,renderer:function (v, meta, record) {
	                var result = '';
	                if(v == "检验完成"){
				...
				meta.style="background-color:#f5fa85";
	                	return v;
			}
	                return result;
	            }});
	    me.columns.push({dataIndex: 'Serialno', text: '条码号', width: 100, hideable: true, hidden: false});
	    me.columns.push({
                menuDisabled: true,
                sortable: false,
                xtype: 'actioncolumn',
	...
	getStoreFields: function () {
	    ...
	    fields.push('ReportStatus');
	    fields.push('Serialno');
	    //fields.push('ReceiveDateTime');
	    return fields;
	},
    /**创建挂靠*/
	createDockedItems: function () {
	    ...
	    dockedItems.push(Ext.create('Shell.class.CheckReportRequest.basic.PdfWinS', {
	        itemId: 'PdfWin',
	        header: false,
	        height: 0,
	        width: 0,
    ...
    onPrintClick: function (but,isMerge,record){
		...
		for(var i = 0; i<recordsisbTempReport.length;i++){
			...
			if(recordsisbTempReport[i].data.ReportStatus && recordsisbTempReport[i].data.ReportStatus!="检验完成"){
				Shell.util.Msg.showError("非检验完成报告不允许打印!");
				return;
			}
		}
