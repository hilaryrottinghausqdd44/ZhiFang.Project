...
layui.extend({
    ...
    var mainTable = {
        ...
        config: {
            ...
            selectUrl: uxutil.path.ROOT + '/ServiceWCF/DictionaryService.svc/GetBModuleGridList?isPlanish=true',
            addUrl: uxutil.path.ROOT + '/ServiceWCF/DictionaryService.svc/AddBModuleGridList',
            editUrl: uxutil.path.ROOT + '/ServiceWCF/DictionaryService.svc/UpdateBModuleGridList',
            delUrl: uxutil.path.ROOT + '/ServiceWCF/DictionaryService.svc/deleteBModuleGridList',
            where: "",
            toolbar: '#Toolbar',
    ...
    Class.pt.editClick = function (data) {
        ...
        layer.open({
            ...
            yes: function (index, layero) {
                var entity = {};
                entity["GridID"] = $("#GridID").val() ? $("#GridID").val() : 0;
                entity["LabID"] = $("#LabID").val() ? $("#LabID").val() : 0;
                entity["LabNo"] = $("#LabID").val() ? $("#LabID option:selected").attr("data-labno") : "";
                entity["GridCode"] = $("#GridCode").val();
                entity["CName"] = $("#CName").val();
        ...
        layer.confirm('确定删除选中项?', { icon: 3, title: '提示' }, function (index) {
            ...
            $.each(checkData, function (i, item) {
                ...
                uxutil.server.ajax({
                    type: "POST",
                    url: url,
                    data: JSON.stringify({GridListID:[item.GridID]}),
                }, function (data) {
    ...
    Class.pt.onSaveClick = function (data, type, index) {
        var me = this,
            ...
            params = data;
        if (!url) return;
        if (type == "add") delete params.entity.GridID;
        params = JSON.stringify({ModuleGridList:[params.entity]});
        //显示遮罩层
