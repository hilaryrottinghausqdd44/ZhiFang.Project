...
layui.extend({
    ...
    var mainTable = {
        ...
        config: {
            ...
            selectUrl: uxutil.path.ROOT + '/ServiceWCF/DictionaryService.svc/GetBModuleFormControlListByFormCode?isPlanish=true',
            addUrl: uxutil.path.ROOT + '/ServiceWCF/DictionaryService.svc/AddFormControlList',
            editUrl: uxutil.path.ROOT + '/ServiceWCF/DictionaryService.svc/UpdateFormControlList',
            delUrl: uxutil.path.ROOT + '/ServiceWCF/DictionaryService.svc/deleteFormControlList',
            where: "",
            toolbar: '#Toolbar',
            ...
            cols: [[
                ...
                { field: 'FormControlID', width: 80, title: '主键ID', sort: false, hide: true },
                ...
                { field: 'Cols', title: '所占栅格', width: 100, sort: true },
                { field: 'MapField', title: '对应属性', width: 120, sort: true },
                { field: 'Label', title: '显示名称', width: 120, sort: true },
                { field: 'DefaultValue', title: '默认值', width: 120, sort: true },
                { field: 'ValueField', title: '值属性', width: 120, sort: true },
                { field: 'TextField', title: '显示属性', width: 120, sort: true },
                { field: 'URL', title: '数据服务地址', width: 120, sort: true },
                { field: 'CallBackFunc', title: '回调函数', width: 120, sort: true },
                { field: 'ClassName', title: '控件Class名', width: 120, sort: true },
                { field: 'StyleContent', title: '控件样式', width: 120, sort: true },
                { field: 'DataJSON', title: '数据集', width: 120, sort: false },
                {
                ...
                {
                    field: 'IsHasNull', title: '是否存在空值', width: 120, sort: true,
                    templet: function (data) {
                        var str = "";
                        if (String(data.IsHasNull) == "true")
                            str = "<span style='color:green;'>是</span>";
                        else
                            str = "<span style='color:red;'>否</span>";
                        return str;
    ...
    Class.pt.addClick = function () {
        ...
        layer.open({
            ...
            yes: function (index, layero) {
                ...
                entity["FormControlID"] = config.FormID ? config.FormID : 0;
    ...
    Class.pt.editClick = function (data) {
        ...
        list.IsHasNull = String(list.IsHasNull) == "true" ? true : false;
    ...
    Class.pt.delClick = function (data) {
        ...
        layer.confirm('确定删除选中项?', { icon: 3, title: '提示' }, function (index) {
            ...
            uxutil.server.ajax({
                type: "POST",
                url: url,
                data: JSON.stringify({FormControlID:[id]})
            }, function (data) {
                layer.close(indexs);
    ...
    Class.pt.onSaveClick = function (data, type, index) {
        ...
        if (type == "add") delete params.entity.FormControlID;
        params = JSON.stringify({ModuleFormControlList:[params.entity]});
        //显示遮罩层
        var indexs = layer.load();
        var configs = {
            type: "POST",
