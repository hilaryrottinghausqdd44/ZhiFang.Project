...
layui.extend({
    ...
    var mainTable = {
        ...
        config: {
            ...
            selectUrl: uxutil.path.ROOT + '/ServiceWCF/DictionaryService.svc/GetBModuleFormControlSetByFormCode?isPlanish=true',
            addUrl: uxutil.path.ROOT + '/ServiceWCF/DictionaryService.svc/AddBModuleFormControlSet',
            editUrl: uxutil.path.ROOT + '/ServiceWCF/DictionaryService.svc/UpdateBModuleFormControlSet',
            delUrl: uxutil.path.ROOT + '/ServiceWCF/DictionaryService.svc/deleteBModuleFormControlSet',
            where: "",
            toolbar: '#Toolbar',
            ...
            cols: [[
                ...
                { field: 'FormControlSetID', width: 80, title: '主键ID', sort: false, hide: true },
                { field: 'FormControlID', width: 80, title: 'BModuleFormControlList表主键ID', sort: false, hide: true },
                { field: 'LabID', width: 80, title: '实验室ID', sort: false, hide: true },
                { field: 'LabNo', title: '实验室No', width: 120, sort: true, hide: true },
                { field: 'FormCode', title: '表单代码', width: 120, sort: true, hide: true },
                { field: 'CName', title: '名称', width: 160, sort: true, hide: true },
                { field: 'Label', title: '显示名称', width: 120, sort: true, edit: 'text' },
                { field: 'DefaultValue', title: '默认值', width: 120, sort: true, edit: 'text' },
                { field: 'DispOrder', title: '显示次序', width: 120, sort: true, edit: 'text' },
                { field: 'IsReadOnly', title: '是否只读', width: 120, sort: true, templet: '#IsReadOnlyTpl' },
                { field: 'IsDisplay', title: '是否显示', width: 120, sort: true, templet: '#IsDisplayTpl' },
            ...
            parseData: function (res) {//res即为原始返回的数据
                ...
                $.each(list, function (i, item) {
                    config.BModuleFormControlListIds.push(item.FormControlID);
                });
                return {
    ...
    Class.pt.iniListeners = function () {
        ...
        form.on('checkbox(IsReadOnly)', function (data) {
            ...
            if (rowObj) delete rowObj.IsReadOnly;
            if (!rowObj) rowObj = {};
            $.each(tableCache, function (index, value) {
                if (index == dataindex) {
                    value.Tab = true;
                    value.IsReadOnly = data.elem.checked;
                }
            });
        });
        //是否使用
        form.on('checkbox(IsDisplay)', function (data) {
            ...
            //改变后的数据
            var rowObj = tableCache[dataindex].Tab;
            if (rowObj) delete rowObj.IsDisplay;
            if (!rowObj) rowObj = {};
            $.each(tableCache, function (index, value) {
                if (index == dataindex) {
                    value.Tab = true;
                    value.IsDisplay = data.elem.checked;
                }
            });
        });
        //是否使用
    ...
    Class.pt.addClick = function () {
        ...
        layer.open({
            ...
            area: ['90%', '90%'],
            content: uxutil.path.ROOT + '/ui_new/layui/class/setting/public/formItem/main/app.html?toolbar=false&defaultToolbar=&externalWhere=' + where + '&isHideColumnToolbar=true&isInitLaboratory=false&isInitComponentType=false&isSearch=true',
    ...
    Class.pt.editClick = function () {
        ...
        $.each(arr, function (i, item) {
            var params = [
                 {
                    FormControlSetID: item.FormControlSetID,
                    DefaultValue: item.DefaultValue,
                    Label: item.Label,
                    DispOrder: item.DispOrder,
                    IsUse: item.IsUse,
                ...
                //, fields: 'FormControlSetID,DefaultValue,Label,DispOrder,IsReadOnly,IsDisplay,IsUse'
            ];
            var configs = {
                type: "POST",
                url: me.config.editUrl,
                data: JSON.stringify({ bModuleFormControlSetList:params })
            };
            uxutil.server.ajax(configs, function (data) {
                if (data.success) {
                    successCount++;
    ...
    Class.pt.delClick = function () {
        ...
        layer.confirm('确定删除选中项?', { icon: 3, title: '提示' }, function (index) {
            ...
            $.each(checkData, function (i, item) {
                var url = me.config.delUrl + '?FormControSetlID=' + item.FormControlSetID;
                uxutil.server.ajax({
                    url: url
                }, function (data) {
