Ext.define('Shell.ReportPrint2.class.PrintList',{
	...
	selectColumnsUrl: '/ServiceWCF/DictionaryService.svc/GetReprotPrintSetting',
	/**当前打印ID*/
	printId:null,
	/**打印错误信息数组*/
	errorInfo:[],
	...
	createButtonsToolbar: function () {
	    ...
	    if (me.hasPrint) {
	        ...
	        buttons.push('-', {
	            xtype: 'button', iconCls: 'button-print', text: '打印当前',
	            listeners: {
	                click: function() {
	                    me.onPrintClick(null,me.thisRecord);
	                }
	            }
	        });

	        //buttons.push('-', {
	        //    xtype: 'button', iconCls: 'button-print', text: '合并打印',
	        //    listeners: {
	        //        click: function () {
	        //            me.onPrintClick(null, me.thisRecord);
	        //        }
	...
	getColumnsSetting: function () {
	    ...
	    Ext.Ajax.request({
	        url: Shell.util.Path.rootPath + me.selectColumnsUrl,
	        async: false,
	        method: 'get',
	        params: {
	            Where: "paramCodeNo='" + me.paramCodeNo + "'"
	        },
	        success: function (response, options) {
	            var rs = Ext.JSON.decode(response.responseText);
	            if (rs.success) {
	                me.columnsRecords = Ext.JSON.decode(rs.ResultDataValue);
	            }
	        }
	    });
	},
	...
	createGridColumns: function () {
	    ...
	    var records = me.columnsRecords;
	    
	    var columnsData = Ext.create("Shell.reportSetting.columns");
	    var arrcolumns = columnsData.columns;
	    for (var i = 0; i < records.length; i++) {
	        for (var j = 0; j < arrcolumns.length; j++) {
	            if (records[i].CName == arrcolumns[j].CName) {
	                columns.push({dataIndex:arrcolumns[j].EName,text:arrcolumns[j].CName,width:arrcolumns[j].width,sortable:false});
	            }
	        }
	    }
	    ////数据列信息
	    ...
	    //columns.push({
	    //    dataIndex: me.DateField, text: '报告日期', width: 90, sortable: false, renderer: function (v, meta) {
	...
	/**点击直接打印处理*/
	onPrintClick:function(but,record){
		var me = this,
			...
			records = record ? [record] : me.getSelectionModel().getSelection(),
			len = records.length,
			filterLen = 0;
		if(len == 0){
				...
				for(var i=0;i<A4List.length;i++){
					var a4 = A4List[i],
						text = 
							...
							Shell.util.Date.toString(a4.get(me.DateField), true) + '</b>' +
							'<b style="color:red;">】</b>' +
		...
		if(me.maxPrintTimes){
			...
			for(var i=0;i<len;i++){
				...
				if(rec.get('PRINTTIMES') >= me.maxPrintTimes){
					var text = 
						'<b style="color:red;margin:0 5px;">报告日期【</b><b>' + 
						Shell.util.Date.toString(rec.get(me.DateField),true) + '</b>' +
						'<b style="color:red;">】</b>' +
						'<b style="color:red;margin:0 5px;">姓名【</b><b>' + 
						rec.get('CNAME') + 
						'<b style="color:red;">】</b>' +
	...
	printAll:function(records){
		...
		if (PrintPdfWin) {
			if(me.hasPdfPrinter){
				...
				PrintPdfWin.print(me.getReportPrintInfo(records), null, printer);
			}else{
			    PrintPdfWin.print(me.getReportPrintInfo(records));
			}
		} else {
		    Shell.util.Msg.showError('打印组件加载失败！');
		}
	},
	/**获取勾选的数据*/
	getReportPrintInfo: function (red) {
	    var me = this,
			...
			records = red? red:me.getSelectionModel().getSelection(),
			len = records.length,
			data = [];
		...
		return {
			...
			isDoublePrint:me.isDoublePrint,
			data:data
		};
	},
	/**收缩*/
