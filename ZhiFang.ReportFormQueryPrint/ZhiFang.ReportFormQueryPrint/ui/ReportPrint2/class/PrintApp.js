Ext.define('Shell.ReportPrint2.class.PrintApp',{
	extend:'Shell.ux.panel.AppPanel',
	
	title:'打印查询',
	layout:{type:'border',regionWeights:{north:3,west:2,south:1}},
    //参数分类
	columnsParamCodeNo: '1001',
    /**默认数据条件,过滤没有文件的结果*/
	...
	afterRender:function(){
		...
		PrintList && PrintList.on({
			...
			select:function(view,record){
                //打印预览
			    PrintList.onPrintClick(null, record);
			...
			itemclick: function (view, record) {
			    ...
			    url = Shell.util.Path.rootPath + url + "?ReportFormID=" + record.get('ReportFormID');
			    me.getToServer(url, function (v) {
			        
			        var rfid = record.get("ReportFormID");
			        if (PrintContent.getResultTypeValue()==1) {
			            var path = Ext.decode(Ext.decode(v).ResultDataValue).PDFPath;
			            rfid = path.substring(path.lastIndexOf("\\")+1, path.length - 4);
			        }
		...
		if(PrintSearch){
			PrintSearch.on({
			    search: function (com, where) {
			        me.onSearch(where);
			        if (!PrintSearch.items.items[3].getComponent("seachsavewhere").getValue()) {
			            PrintSearch.clearSearch();
			        }
				}
			});
	...
	createApps:function(){
		var me = this;
		var apps = [{
			className:'Shell.ReportPrint2.class.PrintSearch',
			itemId:'PrintSearch',region:'north',
			split: true, collapsible: true,
			headCollapsed: me.headCollapsed,
			DateField: me.DateField
		},{
		    className: 'Shell.ReportPrint2.class.PrintList',
		    paramCodeNo:me.columnsParamCodeNo,
			...
			defaultLoad: true,//me.externalWhere ? true : false,
			A4Type: me.A4Type,
			...
			className:'Shell.ReportPrint2.class.PrintContent',autoScroll:true,
			...
			className:'Shell.ReportPrint2.class.PrintChart',
