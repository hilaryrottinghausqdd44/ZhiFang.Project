...
Ext.define('Shell.print.Lodop',{
	...
	/**打印所有URL内容*/
	printAllByUrl:function(lodop,taskName,urls,E32,E64){
		if(!urls) return;
		if(urls.length == 0) return;
		
		lodop = lodop || this.getLodopObj(taskName,E32,E64),
			len = urls.length;
		...
		var infos = [];
		for(var i=0;i<len;i++){
			var info = this.printOneByUrl(null,urls[i],lodop);
	...
	/**打印一个URL内容*/
	printOneByUrl:function(taskName,url,lodop,E32,E64){
		try{
			var isOne = lodop ? false : true;
			lodop = lodop || this.getLodopObj(taskName,E32,E64);
			if(!lodop) return;
	...
	getLodop:function(object,embed,E32,E64){
		...
		try{		     
			...
			if((lodop == null) || (typeof(lodop.VERSION) == "undefined")){
				...
				if(errorInfo.length > 0){Shell.util.Msg.showError(errorInfo.join("</br>"));}
