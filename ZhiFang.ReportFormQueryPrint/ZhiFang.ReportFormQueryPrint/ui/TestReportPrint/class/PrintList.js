 ...
 * @version 2014-10-15
 */
Ext.define('Shell.TestReportPrint.class.PrintList', {
    ...
    getColumnsSetting:function () {
        ...
        Ext.Ajax.request({
            url: Shell.util.Path.rootPath + '/ServiceWCF/DictionaryService.svc/GetColumnsTemplateByAppType?AppType=' + me.appType,
            async: false,
            method: 'get',
            success: function (response, options) {
                ...
                if (reponse.success) {
                    ...
                    for (var i = 0; i < items.length; i++) {
                        ...
                        if (items[i].Render != null) {
                            hash["renderer"] = Ext.JSON.decode(items[i].Render);
                        }
	...
	createDockedItems: function () {
	    ...
	    dockedItems.push(Ext.create('Shell.TestReportPrint.class.PrintPdfWinS', {
	        itemId: 'PrintPdfWin',
	        header: false,
	        height: 0,
	        width: 0,
	...
	getReportPrintInfo:function(){
		...
		for(var i=0;i<count;i++){
			...
			var record = tempArr[i],
				...
				DATE = Shell.util.Date.toString(record.get(me.DateField),true),
