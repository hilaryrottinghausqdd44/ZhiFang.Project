...
xmlns:SharePoint="Microsoft.Sharepoint.WebControls">
	<xsl:output method="html" encoding="utf-8" doctype-public="-//W3C//DTD XHTML 1.0 Transitional//EN" doctype-system="http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd"/>
	 <!--字符串替换 old原字符串，new被替换字符  to替换为-->
  <!--使用方式
     <xsl:call-template name="dots">
        <xsl:with-param name="old" select="From"/>
        <xsl:with-param name="new" select="Source"/>
...
     </xsl:call-template>
  -->
	 <xsl:template name="dots">
	  ...
	  <xsl:value-of select="substring-before($old,$new)"/>
	  <!--默认使用换行替换字符串-->
	  <br/>
	  <!--<xsl:value-of select="$to"/>-->
	  <xsl:call-template name="dots">
	  <xsl:with-param name="old" select="substring-after($old,$new)"/>
	...
	<xsl:template match="/">
		...
		<html xmlns="http://www.w3.org/1999/xhtml">
			...
			<body onload="alert('aaa');">
						...
						<td>
							<table id="yishu" style="width:100%;">
								<tr>
									...
									<!--调用HIS按钮开始 -->
									<!--<td colspan='1' style="padding:10px 5px;font-weight:bold;font-size:13px;border-bottom:2px solid #e0e0e0;">
										<input  style="padding:2px" type="button" onclick="Call_HIS_Data()"  value="调用HIS"></input>
									</td>-->
									<!--调用HIS按钮结束 -->
								...
								<xsl:for-each select="WebReportFile/ReportForm">
									...
									<tr>
										...
										<td style="padding:5px;">
											录入者：
											<xsl:value-of select="TECHNICIANOPERATOR"/>
										</td>
									</tr>
									<tr>
										<td style="padding:5px;">
										...
										<td style="padding:5px;">
											...
											<xsl:value-of select="substring-after(TESTTIME,' ')"/>
										</td>
									</tr>
									<tr>
				...
				<div style="display:none;" id="Call_HIS_Data">
					{
					<xsl:for-each select="WebReportFile/ReportForm">
						"userGuid": "<xsl:value-of select="PATNO"/>",
						"serialNumber": "<xsl:value-of select="PATNO"/>",
						"patientName": "<xsl:value-of select="CNAME"/>",
						"doctorGuid": "<xsl:value-of select="DOCTORCODE1"/>",
						"doctorName": "<xsl:value-of select="DOCTOR"/>",
						"admissionTime": "<xsl:value-of select="RECEIVEDATE"/>",
						"inpatientDepartment": "<xsl:value-of select="DEPT"/>",
						"pageSource": "2",
						"patientInfo": {
							"pregnancyStatus": "0",
							"gender": "<xsl:value-of select="GENDERNO"/>",
							"birthDate": "<xsl:value-of select="BIRTHDAY"/>",
							"age": "<xsl:value-of select="AGE"/>",
							"ageType": "<xsl:value-of select="AGEUNIT"/>",
							"maritalStatus": "9"
						},		
						"labTestList":[{
									"labTestNumber": "<xsl:value-of select="REPORTFORMID"/>",
									"labTestName": "<xsl:value-of select="PARITENNAME"/>",
									"labTestSample": "<xsl:value-of select="SAMPLETYPE"/>",						
									"recordTime": "<xsl:value-of select="TESTDATE"/>",
									"labTestItems": [
					</xsl:for-each>					
						<xsl:for-each select="WebReportFile/ReportItem">
							<xsl:choose>
								<xsl:when test="position() = 1">
									{
									"labTestItemName": "<xsl:value-of select="ITEMCNAME"/>",
									"labTestResult": "<xsl:value-of select="ITEMVALUE"/>",
									"labTestValueUnit": "<xsl:value-of select="ITEMUNIT"/>",
									<xsl:choose>
										<xsl:when test="RESULTSTATUS=NULL">
											"labTestValueChange": "N",
										</xsl:when>
										<xsl:when test="RESULTSTATUS=''">
											"labTestValueChange": "N",
										</xsl:when>
										<xsl:otherwise>
											"labTestValueChange": "<xsl:value-of select="RESULTSTATUS"/>",											
										</xsl:otherwise>
									</xsl:choose>									
									"labTestMethod": "<xsl:value-of select="DIAGMETHOD"/>",
									"normalRange": "<xsl:value-of select="REFRANGE"/>"
									}									
								</xsl:when>
								<xsl:otherwise>
									,{
