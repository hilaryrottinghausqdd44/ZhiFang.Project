...
namespace ZhiFang.BLL.SA.SADimensionStrategy
{
    ...
    /// 质量指标分类类型-统计纬度按季度分组
    /// </summary>
    class OfQuarterGroupBy : DimensionStrategy
    {
        public override IList<SPSAQualityIndicatorType> GetSPSAQualityIndicatorTypeOfGroupBy(IList<SPSAQualityIndicatorType> nfList, string qIndicatorType)
        {
            ...
            for (int i = 0; i < nfList.Count; i++)
            {
                if (nfList[i].FlagDateDelete.HasValue)
                {
                    ...
                    nfList[i].Quarter = SACommon.SeasonHelp.GetSeason(nfList[i].FlagDateDelete.Value);
                }
            }
            ...
            foreach (var group in groupBy)
            {
                ...
                entity.Quarter = group.Key.Quarter;

                var tempList = nfList.Where(p => p.Quarter == group.Key.Quarter);
