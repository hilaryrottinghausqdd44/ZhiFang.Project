...
{
    /// <summary>
    /// 质量指标分类类型-统计纬度按就诊类型+科室分组
    /// </summary>
    class OfSickTypeAndDeptGroupBy : DimensionStrategy
    {
        public override IList<SPSAQualityIndicatorType> GetSPSAQualityIndicatorTypeOfGroupBy(IList<SPSAQualityIndicatorType> nfList, string qIndicatorType)
        {
            ...
            foreach (var group in groupBy)
            {
                ...
                var tempList = nfList.Where(p => p.SickTypeNo == group.Key.SickTypeNo && p.DeptNo == group.Key.DeptNo);
