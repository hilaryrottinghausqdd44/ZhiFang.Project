...
{
    /// <summary>
    /// 质量指标分类类型-统计纬度按月份(年份+月份)分组
    /// </summary>
    class OfMonthGroupBy : DimensionStrategy
    {
        public override IList<SPSAQualityIndicatorType> GetSPSAQualityIndicatorTypeOfGroupBy(IList<SPSAQualityIndicatorType> nfList, string qIndicatorType)
        {
            ...
            {
                if (nfList[i].FlagDateDelete.HasValue)
                {
                    nfList[i].Year = nfList[i].FlagDateDelete.Value.ToString("yyyy");
                    nfList[i].Month = nfList[i].FlagDateDelete.Value.ToString("yyyy-MM");
                }
            }
            ...
            foreach (var group in groupBy)
            {
                ...
                entity.Year = group.Key.Year;
                entity.Month = group.Key.Month;

                var tempList = nfList.Where(p => p.Year == group.Key.Year && p.Month == group.Key.Month);
