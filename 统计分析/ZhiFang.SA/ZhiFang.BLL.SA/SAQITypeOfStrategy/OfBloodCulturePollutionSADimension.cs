...
{
    /// <summary>
    /// 质量指标分类类型图表数据处理-血培养污染统计
    /// </summary>
    class OfBloodCulturePollutionSADimension : QITypeOfEChartStrategy
    {
        public override BaseResultDataValue GetSAQualityIndicatorTypeOfEChart(IList<SPSAQualityIndicatorType> saList, string qIndicatorType, string sadimension)
        {
            ...
            {
                if (sadimension == BloodCulturePollutionSADimension.就诊类型.Key)
                {
                    axisDataValue = vo.SickTypeCName;
                }
                else if (sadimension == BloodCulturePollutionSADimension.就诊类型科室.Key)
                {
                    axisDataValue = vo.SickTypeCName + "_" + vo.DeptCName;
                }
                else if (sadimension == BloodCulturePollutionSADimension.就诊类型采样人.Key)
                {
                    axisDataValue = vo.SickTypeCName + "_" + vo.Collecter;
                }

                else if (sadimension == BloodCulturePollutionSADimension.科室.Key)
                {
                    axisDataValue = vo.DeptCName;
                }
                else if (sadimension == BloodCulturePollutionSADimension.科室采样人.Key)
                {
                    axisDataValue = vo.DeptCName + "_" + vo.Collecter;
                }

                else if (sadimension == BloodCulturePollutionSADimension.按年份.Key)
                {
                    axisDataValue = vo.Year;
                }
                else if (sadimension == BloodCulturePollutionSADimension.按季度.Key)
                {
                    axisDataValue = vo.Quarter;
                }
                else if (sadimension == BloodCulturePollutionSADimension.按月份.Key)
                {
