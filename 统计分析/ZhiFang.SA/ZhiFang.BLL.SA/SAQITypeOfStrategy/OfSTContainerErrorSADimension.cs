...
{
    /// <summary>
    /// 质量指标分类类型图表数据处理-标本容器错误率
    /// </summary>
    class OfSTContainerErrorSADimension : QITypeOfEChartStrategy
    {
        public override BaseResultDataValue GetSAQualityIndicatorTypeOfEChart(IList<SPSAQualityIndicatorType> saList, string qitype, string sadimension)
        {
            ...
            {
                if (sadimension == STContainerErrorSADimension.就诊类型.Key)
                {
                    axisDataValue = vo.SickTypeCName;
                }
                else if (sadimension == STContainerErrorSADimension.就诊类型采样人.Key)
                {
                    axisDataValue = vo.SickTypeCName + "_" + vo.Collecter;
                }
                else if (sadimension == STContainerErrorSADimension.就诊类型样本类型.Key)
                {
                    axisDataValue = vo.SickTypeCName + "_" + vo.SampleTypeCName;
                }
                else if (sadimension == STContainerErrorSADimension.就诊类型科室.Key)
                {
                    axisDataValue = vo.SickTypeCName + "_" + vo.DeptCName;
                }

                else if (sadimension == STContainerErrorSADimension.样本类型.Key)
                {
                    axisDataValue = vo.SampleTypeCName;
                }
                else if (sadimension == STContainerErrorSADimension.样本类型科室.Key)
                {
                    axisDataValue = vo.SampleTypeCName + "_" + vo.DeptCName;
                }
                else if (sadimension == STContainerErrorSADimension.样本类型采样人.Key)
                {
                    axisDataValue = vo.SampleTypeCName + "_" + vo.Collecter;
                }

                else if (sadimension == STContainerErrorSADimension.科室.Key)
                {
                    axisDataValue = vo.DeptCName;
                }
                else if (sadimension == STContainerErrorSADimension.科室样本类型.Key)
                {
                    axisDataValue = vo.DeptCName + "_" + vo.SampleTypeCName;
                }
                else if (sadimension == STContainerErrorSADimension.科室采样人.Key)
                {
                    axisDataValue = vo.DeptCName + "_" + vo.Collecter;
                }

                else if (sadimension == STContainerErrorSADimension.采样人.Key)
                {
                    axisDataValue = vo.Collecter;
                }
                else if (sadimension == STContainerErrorSADimension.按年份.Key)
                {
                    axisDataValue = vo.Year;
                }
                else if (sadimension == STContainerErrorSADimension.按季度.Key)
                {
                    axisDataValue = vo.Quarter;
                }
                else if (sadimension == STContainerErrorSADimension.按月份.Key)
                {
