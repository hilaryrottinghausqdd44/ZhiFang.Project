...
namespace ZhiFang.DBUpdate.PM
{
        ...
        DBUpdate
    {
        ...
        static string MainAssemblyFile = "ZhiFang.SA.dll";//可以从配置文件获取
        ...
        public static bool DataBaseUpdate(string oldVersion)
        {
            ...
            if (IsUpdateDataBase(oldVersion, "1.0.0.2"))
            {
                ...
                #region SA_ConfigModule
                updateSql = "  if object_id('SA_ConfigModule') is  null CREATE TABLE [dbo].[SA_ConfigModule]( [LabID] [bigint] NULL, [ConfigId] [bigint] NOT NULL, [ModuleId] [bigint] NULL, [ModuleNo] [varchar](100) NULL, [ModuleUrl] [varchar](180) NULL, [ModuleParam] [varchar](180) NULL, [CName] [varchar](80) NULL, [SName] [varchar](80) NULL, [Shortcode] [varchar](40) NULL, [PinYinZiTou] [varchar](40) NULL, [Comment] [ntext] NULL, [FilePath] [varchar](200) NULL, [IsUse] [bit] NULL, [IsDefault] [bit] NULL, [DataAddTime] [datetime] NULL, [DataTimeStamp] [timestamp] NULL, CONSTRAINT [PK_SA_ConfigModule] PRIMARY KEY CLUSTERED( [ConfigId] ASC)WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON) ON [PRIMARY] ) ON [PRIMARY] TEXTIMAGE_ON [PRIMARY]; ";
                listSQL.Add(updateSql);

                #endregion

                #region PhrasesWatchClass&PhrasesWatchClassItem

                //删除PK_PhrasesWatchClassID
                updateSql = " IF EXISTS(SELECT * FROM sys.indexes WHERE object_id = OBJECT_ID(N'[dbo].[PhrasesWatchClass]') AND name = N'PK_PhrasesWatchClassID') ALTER TABLE [dbo].[PhrasesWatchClass] DROP CONSTRAINT [PK_PhrasesWatchClassID];";
                listSQL.Add(updateSql);

                //删除PK_PhrasesWatchClassItem
                updateSql = " IF EXISTS(SELECT * FROM sys.indexes WHERE object_id = OBJECT_ID(N'[dbo].[PhrasesWatchClassItem]') AND name = N'PK_PhrasesWatchClassItem') ALTER TABLE [dbo].[PhrasesWatchClassItem] DROP CONSTRAINT [PK_PhrasesWatchClassItem];";
                listSQL.Add(updateSql);

                //调整PhrasesWatchClass的PhrasesWatchClassID数据类型为bigint
                updateSql = " alter table PhrasesWatchClass alter column PhrasesWatchClassID bigint not null; ";
                listSQL.Add(updateSql);

                //调整PhrasesWatchClassItem的PhrasesWatchClassID数据类型为bigint
                updateSql = " alter table PhrasesWatchClassItem alter column PhrasesWatchClassID bigint not null; ";
                listSQL.Add(updateSql);

                //重新创建PK_PhrasesWatchClassID
                updateSql = " ALTER TABLE [dbo].[PhrasesWatchClass] ADD CONSTRAINT [PK_PhrasesWatchClassID] PRIMARY KEY CLUSTERED( [PhrasesWatchClassID] ASC)WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, SORT_IN_TEMPDB = OFF, IGNORE_DUP_KEY = OFF, ONLINE = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON) ON [PRIMARY]; ";
                listSQL.Add(updateSql);

                //重新创建PK_PhrasesWatchClassItem
                updateSql = " ALTER TABLE [dbo].[PhrasesWatchClassItem] ADD CONSTRAINT [PK_PhrasesWatchClassItem] PRIMARY KEY CLUSTERED( [PhrasesWatchClassID] ASC, [RefuseID] ASC)WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, SORT_IN_TEMPDB = OFF, IGNORE_DUP_KEY = OFF, ONLINE = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON) ON [PRIMARY]; ";
                listSQL.Add(updateSql);

                //添加质量指标类型的父级节点
                updateSql = " IF COL_LENGTH('PhrasesWatchClass', 'ParentID') IS NULL ALTER TABLE PhrasesWatchClass ADD ParentID bigint; ";
                listSQL.Add(updateSql);

                //质量指标分类类型Id
                updateSql = " IF COL_LENGTH('PhrasesWatchClass', 'QIndicatorTypeId') IS NULL ALTER TABLE PhrasesWatchClass ADD QIndicatorTypeId bigint; ";
                listSQL.Add(updateSql);

                updateSql = " IF COL_LENGTH('PhrasesWatchClass', 'QIndicatorTypeCName') IS NULL ALTER TABLE PhrasesWatchClass ADD QIndicatorTypeCName varchar(80); ";
                listSQL.Add(updateSql);

                updateSql = " update PhrasesWatchClass set ParentID=0 where (ParentID is null or ParentID=''); ";
                listSQL.Add(updateSql);
                ...
                #region LStat_Total
                //添加统计结果表
                updateSql = " if object_id('LStat_Total') is null CREATE TABLE [dbo].[LStat_Total]( [LabID] [bigint] NULL, [StatTotalID] [bigint] NOT NULL, [ClassificationId] [bigint] NOT NULL, [ClassificationName] [varchar](50) NULL, [StatType] [bigint] NOT NULL, [StatName] [nvarchar](50) NULL, [StatDateType] [bigint] NULL, [StatKey] [nchar](500) NULL, [StatDName] [nvarchar](50) NULL, [StatDValue] [bigint] NULL, [StatDValueDesc] [nvarchar](50) NULL, [StatDValue2] [bigint] NULL, [StatDValueDesc2] [nvarchar](50) NULL, [StatDateBegin] [datetime] NULL, [StatDateEnd] [datetime] NULL, [StatValue] [ntext] NULL, [DataAddTime] [datetime] NULL, [DataUpdateTime] [datetime] NULL, CONSTRAINT [PK_LSTAT_TOTAL] PRIMARY KEY CLUSTERED( [StatTotalID] ASC)WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON) ON [PRIMARY] ) ON [PRIMARY] TEXTIMAGE_ON [PRIMARY]; ";
                listSQL.Add(updateSql);
