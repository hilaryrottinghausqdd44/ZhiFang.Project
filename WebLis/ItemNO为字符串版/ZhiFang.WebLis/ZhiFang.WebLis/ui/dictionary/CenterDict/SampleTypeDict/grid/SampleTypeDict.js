...
$(function () {
    $("#dg").datagrid({
        ...
        rownumbers: true,
        collapsible: false,
        idField: 'SampleTypeNo',
        url: Shell.util.Path.rootPath + '/ServiceWCF/DictionaryService.svc/GetPubDict?tableName=' + "SampleType",
       // url: 'data.txt',
        method: 'get',
        striped: true,
        columns: [[
            { field: 'cb', checkbox: 'true' },
                { field: 'SampleTypeNo', title: '项目编码', width: '28%' },
    ...
    $('#txtSampleTypeNo').numberbox({
        
        validType: 'length[0,10]',
        onChange: function (newValue, oldValue) {
            var SampleTypeNo = $('#txtSampleTypeNo').numberbox('getValue');
        if (btnType == 'add') {
            if (SampleTypeNo.length > 10)
            {
                $.ajax({
                    success: function () {
                        $('#txtSampleTypeNo').numberbox('clear');
                ...
                $.ajax({
                    ...
                    data: { filerValue: newValue.trim(), tablename: "SampleType", precisequery: "SampleTypeNo" },
            ...
            if (r) {
                for (var i = 0; i < rows.length; i++) {
                    $.ajax({
                        type: 'get',
                        contentType: 'application/json',
                        url: Shell.util.Path.rootPath + '/ServiceWCF/DictionaryService.svc/DeleteSampleTypeModelByID?SampleTypeNo=' + rows[i].SampleTypeNo,
                        dataType: 'json',
                        async: false,
                        success: function (data) {
                            if (data.success == true) {
...
function add() {
    $('#txtSampleTypeNo').textbox('enable');
    btnType = 'add';
...
function edit(index, value) {
    ...
    $('#txtSampleTypeNo').textbox('disable');
...
function save() {
    if (btnType == 'edit') {
        ...
        var r = '';
        r += '{"jsonentity":{';
        var SampleTypeNo = $("#txtSampleTypeNo").val();
        if (SampleTypeNo == "") {
            errors += 1;
        }
        if (SampleTypeNo) {
            r += '"SampleTypeNo":' + SampleTypeNo + ',';
        }
        ...
        if (errors > 0) {
            ...
            $.ajax({
                type: 'post',
                contentType: 'application/json',
                url: Shell.util.Path.rootPath + '/ServiceWCF/DictionaryService.svc/UpdateSampleTypeModelByID',
                data: r,
                dataType: 'json',
                success: function (data) {
                    if (data.success == true) {
        ...
        if (errors > 0) {
            ...
            $.ajax({
                type: 'post',
                contentType: 'application/json',
                url: Shell.util.Path.rootPath + '/ServiceWCF/DictionaryService.svc/AddSampleTypeModel',
                data: r,
                dataType: "json",
                success: function (data) {
                    if (data.success == true) {
