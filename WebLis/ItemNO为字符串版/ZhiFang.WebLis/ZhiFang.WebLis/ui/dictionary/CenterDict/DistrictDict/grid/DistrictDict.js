...
$(function () {
    $("#dg").datagrid({
        ...
        collapsible: false,
        idField: 'DistrictNo',
        url: Shell.util.Path.rootPath + '/ServiceWCF/DictionaryService.svc/GetPubDict?tableName=' + "District",
      //  url: 'data.txt',
        method: 'get',
        striped: true,
        columns: [[
                { field: 'cb', checkbox: 'true' },
                { field: 'DistrictNo', title: '项目编码', width: '15%' },
    ...
    $('#txtDistrictNo').numberbox({
      
        validType: 'length[0,10]',
        onChange: function (newValue, oldValue) {
        var ItemNo = $('#txtDistrictNo').numberbox('getValue');
        if (btnType == 'add') {
            if (ItemNo.length > 10) {
                $.ajax({
                    success: function () {
                        $('#txtDistrictNo').numberbox('clear');
                ...
                $.ajax({
                    ...
                    data: { filerValue: newValue.trim(), tablename: "District", precisequery: "DistrictNo" },
            ...
            if (r) {
                for (var i = 0; i < rows.length; i++) {
                    $.ajax({
                        type: 'get',
                        contentType: 'application/json',
                        url: Shell.util.Path.rootPath + '/ServiceWCF/DictionaryService.svc/DeleteDistrictModelByID?DistrictNo=' + rows[i].DistrictNo,
                        dataType: 'json',
                        async: false,
                        success: function (data) {
                            if (data.success == true) {
...
function save() {
    ...
    if (btnType == 'edit') {
        ...
        var r = '';
        r += '{"jsonentity":{';
        var DistrictNo = $("#txtDistrictNo").val();
        if (DistrictNo == "") {
            errors += 1;
        }
        if (DistrictNo) {
            r += '"DistrictNo":' + DistrictNo + ',';
        }
        ...
        if (errors > 0) {
            ...
            $.ajax({
                type: 'post',
                contentType: 'application/json',
                url: Shell.util.Path.rootPath + '/ServiceWCF/DictionaryService.svc/UpdateDistrictModelByID',
                data: r,
       ...
       // var r = '{"jsonentity":{"DistrictNo":"' + DistrictNo + '","ShortName":"' + ShortName + '","CName":"' + CName + '","ShortCode":"' + ShortCode + '","DispOrder":"' + DispOrder + '","HisOrderCode":"' + HisOrderCode + '","Visible":"' + Visible + '"}}';
        ...
        if (errors > 0) {
            ...
            $.ajax({
                type: 'post',
                contentType: 'application/json',
                url: Shell.util.Path.rootPath + '/ServiceWCF/DictionaryService.svc/AddDistrictModel',
                data: r,
                dataType: "json",
                success: function (data) {
                    if (data.success == true) {
