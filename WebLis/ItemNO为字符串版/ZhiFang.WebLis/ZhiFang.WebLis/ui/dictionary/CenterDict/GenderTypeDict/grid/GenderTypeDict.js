...
$(function () {
    $("#dg").datagrid({
        ...
        collapsible: false,
        idField: 'GenderNo',
        url: Shell.util.Path.rootPath + '/ServiceWCF/DictionaryService.svc/GetPubDict?tableName=' + "GenderType",
       // url: 'data.txt',
        method: 'get',
        striped: true,
        columns: [[
            { field: 'cb', checkbox: 'true' },
                { field: 'GenderNo', title: '编码', width: '28%' },
    ...
    $('#txtGenderNo').numberbox({
     
        validType: 'length[0,10]',
        onChange: function (newValue, oldValue) {
        var GenderNo = $('#txtGenderNo').numberbox('getValue');
        if (btnType == 'add') {
            if (GenderNo.length > 10)
            {
                $.ajax({
                    success: function () {
                        $('#txtGenderNo').numberbox('clear');
                ...
                $.ajax({
                    ...
                    data: { filerValue: newValue.trim(), tablename: "GenderType", precisequery: "GenderNo" },
            ...
            if (r) {
                for (var i = 0; i < rows.length; i++) {
                    $.ajax({
                        type: 'get',
                        contentType: 'application/json',
                        url: Shell.util.Path.rootPath + '/ServiceWCF/DictionaryService.svc/DeleteGenderTypeModelByID?GenderNo=' + rows[i].GenderNo,
                        dataType: 'json',
                        async: false,
                        success: function (data) {
                            if (data.success == true) {
    ...
    if (btnType == 'edit') {
        ...
        if (errors > 0) {
            ...
            $.ajax({
                type: 'post',
                contentType: 'application/json',
                url: Shell.util.Path.rootPath + '/ServiceWCF/DictionaryService.svc/AddGenderTypeModel',
                data: r,
                dataType: "json",
                success: function (data) {
                    if (data.success == true) {
