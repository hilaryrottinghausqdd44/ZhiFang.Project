...
$(function(){
    ...
    $('#LabGenderNo').textbox({onChange:function(newValue,oldValue){
        if(btnType=='add' && newValue.length<=10)
            askService('validate',null,{precisequery:"LabGenderNo",filerValue:newValue,labcode:LabCode,tablename:"B_lab_GenderType"});//验证主键的唯一性
    }});
});
...
function createColumns() {
    var columns = [
        [
            ...
            {field: 'LabGenderNo', title: '编码', width: fixWidth(0.2) },
        ...
        success: function (data) {
            if (data.success) {
                switch (serviceType) {
                    ...
                    case'edit':
                    case'delete':
                        $('#tbGenderType').datagrid('load',{tablename:"B_lab_GenderType",filerValue:null,labcode:LabCode,time:new Date().getTime()});
                        $('#txtSearch').searchbox('setValue',null);//增删改成功后，重新加载数据请求
                        break;

                    case'search':
                        $('#tbGenderType').datagrid('loadData',data);
                        break;
                    case 'validate':
                ...
                (ErrorInfo.indexOf('此项已对照')>-1)?contrastList.push(where.LabGenderNo):null;
            }
        },
        error: function (data) {
...
function setService(serviceType, serviceParam, where) {
    ...
    switch (serviceType) {
        case'search':
            ...
            break;
        case'delete':
            serviceParam.serviceName = 'DeleteLabGenderTypeModelByID';//删除数据服务名
            serviceParam.type = 'GET';//数据请求方式GET
            serviceParam.data =  where;//发送到服务器的数据，根据ID号进行删除
            break;
        case'add':
            serviceParam.serviceName = '/AddLabGenderTypeModel';//增加数据服务名
            serviceParam.type = 'POST';//数据请求方式GET
            break;
        case'edit':
            serviceParam.serviceName = 'UpdateLabGenderTypeModelByID';//编辑数据服务名UpdateReportGroupModelByID
            serviceParam.type = 'POST';//数据请求方式GET
            break;
        case'validate':
...
function getEntity(getType) {
    ...
    var LabGenderNo=$('#LabGenderNo').numberbox('getValue');
    if(!LabGenderNo)
        return 1;
    result+='"LabGenderNo":'+LabGenderNo+',';
...
function addRow(){
    ...
    $('#LabGenderNo').textbox('enable');
    setCombobox([]);//设置下拉框
}

//修改
function editRow(index){
    var curData = $('#tbGenderType').datagrid('getData'),//返回当前页加载完毕的数据
...
//删除
function deleteRow(){
    var rows = $('#tbGenderType').datagrid('getSelections') ||[],
        ...
        $.messager.confirm('确认', '确认删除吗？', function (btn) {
            if (btn) {
                for (var i = 0; i < length; i++) {
                    askService('delete', null,{LabGenderNo:rows[i]['LabGenderNo'],labcode:LabCode});
                }
                $('#tbGenderType').datagrid('clearSelections');//因为getSelections会记忆选过的记录，所以要清空一下
        ...
        $.messager.confirm('确认', '确认禁用所选记录吗？', function (btn) {
            if (btn) {
                for (var i = 0; i < rows.length; i++) {
                    if(rows[i]['UseFlag']){
                        index=$('#tbGenderType').datagrid('getRowIndex',rows[i]);
...
function exportExcel(){
    $('#ifmExport').attr('src',Shell.util.Path.rootPath + '/ServiceWCF/DictionaryService.svc/DictionaryExportExcel?table=B_Lab_GenderType&labcode='+LabCode);
}
...
function disableOrEnable(index,UseFlag){
    ...
    result += '{"jsonentity":{';//构造符合格式的字符串
    result+='"LabGenderNo":'+curRow.LabGenderNo+',';
    result+='"LabCode":'+LabCode+',';
...
function search(value){
    $('#tbGenderType').datagrid('load',{tablename:"B_lab_GenderType",labcode:LabCode,filerValue:value,time:new Date().getTime()});
}
