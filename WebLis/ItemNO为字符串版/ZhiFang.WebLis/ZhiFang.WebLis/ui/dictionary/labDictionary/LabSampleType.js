...
$(function(){
    ...
    $('#LabSampleTypeNo').textbox({onChange:function(newValue,oldValue){
        if(btnType=='add' && newValue.length<=10)
            askService('validate',null,{precisequery:"LabSampleTypeNo",filerValue:newValue,labcode:LabCode,tablename:"B_Lab_SampleType"});//验证主键的唯一性
    }});
});
...
function createColumns() {
    var columns = [
        [
            ...
            {field: 'LabSampleTypeNo', title: '编码', width: fixWidth(0.2) },
        ...
        success: function (data) {
            if (data.success) {
                switch (serviceType) {
                    ...
                    case'edit':
                    case'delete':
                        $('#tbSampleType').datagrid('load',{tablename:"B_Lab_SampleType",filerValue:null,labcode:LabCode,time:new Date().getTime()});
                        $('#txtSearch').searchbox('setValue',null);//增删改成功后，重新加载数据请求
                        break;

                    case'search':
                        $('#tbSampleType').datagrid('loadData',data);
                        break;
                    case 'validate':
                ...
                (ErrorInfo.indexOf('此项已对照')>-1)?contrastList.push(where.LabSampleTypeNo):null;
            }
        },
        error: function (data) {
...
function setService(serviceType, serviceParam, where) {
    ...
    switch (serviceType) {
        case'search':
            ...
            break;
        case'delete':
            serviceParam.serviceName = 'DeleteLabSampleTypeModelByID';//删除数据服务名
            serviceParam.type = 'GET';//数据请求方式GET
            serviceParam.data =  where;//发送到服务器的数据，根据ID号进行删除
            break;
        case'add':
            serviceParam.serviceName = 'AddLabSampleTypeModel';//增加数据服务名
            serviceParam.type = 'POST';//数据请求方式GET
            break;
        case'edit':
            serviceParam.serviceName = 'UpdateLabSampleTypeModelByID';//编辑数据服务名UpdateReportGroupModelByID
            serviceParam.type = 'POST';//数据请求方式GET
            break;
        case'validate':
...
function getEntity(getType) {
    ...
    var LabSampleTypeNo=$('#LabSampleTypeNo').textbox('getValue');
    if(!LabSampleTypeNo)
        return 1;
    result+='"LabSampleTypeNo":"'+LabSampleTypeNo+'",';
    ...
    var code_1=$('#code_1').textbox('getValue');
    if(code_1)
    result+='"code_1":"'+code_1+'",';

    var code_2=$('#code_2').textbox('getValue');
    if(code_2)
    result+='"code_2":"'+code_2+'",';

    var code_3=$('#code_3').textbox('getValue');
    if(code_3)
...
function addRow(){
    ...
    $('#LabSampleTypeNo').textbox('enable');
    setCombobox([]);//设置下拉框
}

//修改
function editRow(index){
    var curData = $('#tbSampleType').datagrid('getData'),//返回当前页加载完毕的数据
    ...
    if(curRow){
        ...
        $('#LabSampleTypeNo').textbox('disable');
...
//删除
function deleteRow(){
    var rows = $('#tbSampleType').datagrid('getSelections') ||[],
        ...
        $.messager.confirm('确认', '确认删除吗？', function (btn) {
            if (btn) {
                for (var i = 0; i < length; i++) {
                    ...
                    askService('delete', null,{LabSampleTypeNo:rows[i]['LabSampleTypeNo'],labcode:LabCode});
                }
                $('#tbSampleType').datagrid('clearSelections');//因为getSelections会记忆选过的记录，所以要清空一下
        ...
        $.messager.confirm('确认', '确认禁用所选记录吗？', function (btn) {
            if (btn) {
                for (var i = 0; i < rows.length; i++) {
                    if(rows[i]['UseFlag']){
                        index=$('#tbSampleType').datagrid('getRowIndex',rows[i]);
...
function exportExcel(){
    $('#ifmExport').attr('src',Shell.util.Path.rootPath + '/ServiceWCF/DictionaryService.svc/DictionaryExportExcel?table=B_Lab_SampleType&labcode='+LabCode);
}
...
function disableOrEnable(index,UseFlag){
    ...
    result += '{"jsonentity":{';//构造符合格式的字符串
    result+='"LabSampleTypeNo":"'+curRow.LabSampleTypeNo+'",';
    result+='"LabCode":'+LabCode+',';
    ...
    if(!curRow.code_1 && curRow.code_1!==0){}else{
        result+='"code_1":"'+curRow.code_1+'",';
    }

    if(!curRow.code_2 && curRow.code_2!==0){}else{
        result+='"code_2":"'+curRow.code_2+'",';
    }

    if(!curRow.code_3 && curRow.code_3!==0){}else{
...
function search(value){
    $('#tbSampleType').datagrid('load',{tablename:"B_Lab_SampleType",labcode:LabCode,filerValue:value,time:new Date().getTime()});
}
