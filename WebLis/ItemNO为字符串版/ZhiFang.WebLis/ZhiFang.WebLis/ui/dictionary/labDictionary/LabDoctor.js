...
$(function(){
    ...
    $('#LabDoctorNo').textbox({onChange:function(newValue,oldValue){
        if(btnType=='add' && newValue.length<=10)
            askService('validate',null,{precisequery:"LabDoctorNo",filerValue:newValue,labcode:LabCode,tablename:"B_Lab_Doctor"});//验证主键的唯一性
    }});
});
...
function createColumns() {
    var columns = [
        [
            ...
            {field: 'LabDoctorNo', title: '编码', width: fixWidth(0.2) },
        ...
        success: function (data) {
            if (data.success) {
                switch (serviceType) {
                    ...
                    case'edit':
                    case'delete':
                        $('#tbDoctor').datagrid('load',{tablename:"B_Lab_Doctor",filerValue:null,labcode:LabCode,time:new Date().getTime()});
                        $('#txtSearch').searchbox('setValue',null);//增删改成功后，重新加载数据请求
                        break;

                    case'search':
                        $('#tbDoctor').datagrid('loadData',data);
                        break;
                    case 'validate':
                ...
                (ErrorInfo.indexOf('此项已对照')>-1)?contrastList.push(where.LabDoctorNo):null;
            }
        },
        error: function (data) {
...
function setService(serviceType, serviceParam, where) {
    ...
    switch (serviceType) {
        case'search':
            ...
            break;
        case'delete':
            serviceParam.serviceName = 'DeleteLabDoctorModelByID';//删除数据服务名
            serviceParam.type = 'GET';//数据请求方式GET
            serviceParam.data =  where;//发送到服务器的数据，根据ID号进行删除
            break;
        case'add':
            serviceParam.serviceName = 'AddLabDoctorModel';//增加数据服务名
            serviceParam.type = 'POST';//数据请求方式GET
            break;
        case'edit':
            serviceParam.serviceName = 'UpdateLabDoctorModelByID';//编辑数据服务名UpdateReportGroupModelByID
            serviceParam.type = 'POST';//数据请求方式GET
            break;
        case'validate':
...
function getEntity(getType) {
    ...
    var LabDoctorNo=$('#LabDoctorNo').numberbox('getValue');
    if(!LabDoctorNo)
        return 1;
    result+='"LabDoctorNo":'+LabDoctorNo+',';
...
function addRow(){
    ...
    $('#LabDoctorNo').textbox('enable');
    setCombobox([]);//设置下拉框
}

//修改
function editRow(index){
    var curData = $('#tbDoctor').datagrid('getData'),//返回当前页加载完毕的数据
...
//删除
function deleteRow(){
    var rows = $('#tbDoctor').datagrid('getSelections') ||[],
        ...
        $.messager.confirm('确认', '确认删除吗？', function (btn) {
            if (btn) {
                for (var i = 0; i < length; i++) {
                    askService('delete', null,{LabDoctorNo:rows[i]['LabDoctorNo'],labcode:LabCode});
                }
                $('#tbDoctor').datagrid('clearSelections');//因为getSelections会记忆选过的记录，所以要清空一下
        ...
        $.messager.confirm('确认', '确认禁用所选记录吗？', function (btn) {
            if (btn) {
                for (var i = 0; i < rows.length; i++) {
                    if(rows[i]['UseFlag']){
                        index=$('#tbDoctor').datagrid('getRowIndex',rows[i]);
...
function exportExcel(){
    $('#ifmExport').attr('src',Shell.util.Path.rootPath + '/ServiceWCF/DictionaryService.svc/DictionaryExportExcel?table=B_Lab_Doctor&labcode='+LabCode);
}
...
function disableOrEnable(index,UseFlag){
    ...
    result += '{"jsonentity":{';//构造符合格式的字符串
    result+='"LabDoctorNo":'+curRow.LabDoctorNo+',';
    result+='"LabCode":'+LabCode+',';
...
function search(value){
    $('#tbDoctor').datagrid('load',{tablename:"B_Lab_Doctor",labcode:LabCode,filerValue:value,time:new Date().getTime()});
}
