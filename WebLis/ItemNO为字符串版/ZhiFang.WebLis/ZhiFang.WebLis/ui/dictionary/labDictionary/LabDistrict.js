...
$(function () {
    ...
    $('#LabDistrictNo').textbox({
        onChange: function (newValue, oldValue) {
            if (btnType == 'add' && newValue.length <= 10)
                askService('validate', null, { precisequery: "LabDistrictNo", filerValue: newValue, labcode: LabCode, tablename: "B_Lab_District" });//验证主键的唯一性
        }
    });
});
...
function createColumns() {
    var columns = [
        [
            ...
            { field: 'LabDistrictNo', title: '编码', width: fixWidth(0.2) },
            { field: 'CName', title: '中文名称', width: fixWidth(0.2) },
            { field: 'ShortName', title: '简称', width: fixWidth(0.2) },
            ...
            if (data.success) {
                switch (serviceType) {
                    ...
                    case 'delete':
                        ...
                        $('#tbDistrict').datagrid('load', { tablename: "B_Lab_District", filerValue: null, labcode: LabCode, time: new Date().getTime() });//增删改成功后，重新加载数据请求
                        break;

                    case 'search':
                        $('#tbDistrict').datagrid('loadData', data);
                        break;
                    case 'validate':
                ...
                (ErrorInfo.indexOf('此项已对照') > -1) ? contrastList.push(where.LabDistrictNo) : null;
            }
        },
        error: function (data) {
...
function setService(serviceType, serviceParam, where) {
    ...
    switch (serviceType) {
        case 'search':
            ...
            break;
        case 'delete':
            serviceParam.serviceName = 'DeleteLabDistrictModelByID';//删除数据服务名
            serviceParam.type = 'GET';//数据请求方式GET
            serviceParam.data = where;//发送到服务器的数据，根据ID号进行删除
            break;
        case 'add':
            serviceParam.serviceName = 'AddLabDistrictModel';//增加数据服务名
            serviceParam.type = 'POST';//数据请求方式GET
            break;
        case 'edit':
            serviceParam.serviceName = 'UpdateLabDistrictModelByID';//编辑数据服务名UpdateReportGroupModelByID
            serviceParam.type = 'POST';//数据请求方式GET
            break;
        case 'validate':
...
function getEntity(getType) {
    ...
    var LabDistrictNo = $('#LabDistrictNo').numberbox('getValue');
    if (!LabDistrictNo)
        return 1;
    result += '"LabDistrictNo":' + LabDistrictNo + ',';
...
//刷新
function refresh() {
    $('#tbDistrict').datagrid('load', { tablename: "B_Lab_District", filerValue: null, labcode: LabCode, time: new Date().getTime() });
    $('#txtSearch').searchbox('setValue', null);
}
...
function addRow() {
    ...
    $('#LabDistrictNo').textbox('enable');
    setCombobox([]);//设置下拉框
}

//修改
function editRow(index) {
    var curData = $('#tbDistrict').datagrid('getData'),//返回当前页加载完毕的数据
...
//删除
function deleteRow() {
    var rows = $('#tbDistrict').datagrid('getSelections') || [],
        ...
        $.messager.confirm('确认', '确认删除吗？', function (btn) {
            if (btn) {
                for (var i = 0; i < length; i++) {
                    askService('delete', null, { LabDistrictNo: rows[i]['LabDistrictNo'], labcode: LabCode });
                }
                $('#tbDistrict').datagrid('clearSelections');//因为getSelections会记忆选过的记录，所以要清空一下
        ...
        $.messager.confirm('确认', '确认禁用所选记录吗？', function (btn) {
            if (btn) {
                for (var i = 0; i < rows.length; i++) {
                    if (rows[i]['UseFlag']) {
                        index = $('#tbDistrict').datagrid('getRowIndex', rows[i]);
...
function exportExcel() {
    $('#ifmExport').attr('src', Shell.util.Path.rootPath + '/ServiceWCF/DictionaryService.svc/DictionaryExportExcel?table=B_Lab_District&labcode=' + LabCode);
}
...
function disableOrEnable(index, UseFlag) {
    ...
    result += '{"jsonentity":{';//构造符合格式的字符串
    result += '"LabDistrictNo":' + curRow.LabDistrictNo + ',';
    result += '"LabCode":' + LabCode + ',';
...
function search(value) {
    $('#tbDistrict').datagrid('load', { tablename: "B_Lab_District", labcode: LabCode, filerValue: value, time: new Date().getTime() });
}
