$(function () {
    ...
    function initComponent(){
	    ...
	    $("#payCode").textbox('textbox').keydown(function (e) {
            if (e.keyCode == 13) {
            	...
            	//一秒后光标复位，防止快速回车
            	setTimeout(function(){
            		$("#payCode").textbox('textbox').focus();//光标复位
            	},1000);
    ...
    function getConsumeInfo(payCode,callback){
    	...
    	$.ajax({
            dataType: 'json',
            contentType: 'application/json',
            url: GET_CONSUME_URL + '?payCode=' + payCode,
            success: function (result) {
            	onMaskHide();//取消遮罩层  
            	Shell.util.Msg.showLog("获取消费单信息成功");
                callback(result);
            },
            error: function (request, strError) {
            	onMaskHide();//取消遮罩层  
                Shell.util.Msg.showError("获取消费单信息失败！错误信息：" + strError);
            }
        });
    }
    //获取消费单套餐明细信息
    ...
    function getNrequestForm(){
    	var form = {
            ...
            AreaNo: $("#form_AreaNo").combobox("getValue"),//区域
