...
namespace ZhiFang.BLL.Common
{
    public class TransDataToXML
    {
        ...
        /// 将DataRow转换成Xml文件
        /// </summary>
        /// <param name="dt">要转换的DataRow</param>
        /// <param name="recordsstr">记录节点</param>
        /// <returns></returns>
        public static XmlDocument TransformDRIntoXML(DataRow dr, string rootstr, string recordsstr)
        {
            try
            {
                XmlDocument xd = new XmlDocument();
                ...
                {
                    XmlElement xc = xd.CreateElement(dr.Table.Columns[j].ColumnName);
                    //xc.InnerText = HttpUtility.HtmlEncode(dt.Rows[i][j].ToString());
                    xc.InnerText = dr[j].ToString();
                    xrecordsstr.AppendChild(xc);
                }
                xroot.AppendChild(xrecordsstr);
                xd.AppendChild(xroot);
        ...
        /// <param name="rootstr">根节点</param>
        /// <param name="recordsstr">记录节点</param>
        /// <param name="xd">原始xml</param>
        /// <returns></returns>
        public static XmlDocument TransformDTInsertIntoXML(DataTable dt, string xpath, string recordsstr, XmlDocument xd)
        {
            try
            {
