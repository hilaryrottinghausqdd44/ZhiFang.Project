...
namespace ZhiFang.DAL.MsSql.Weblis
{
    ...
    public partial class B_Lab_GenderType : BaseDALLisDB, IDLab_GenderType
    {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  *,'('+convert(varchar(100),LabGenderNo)+')'+CName as LabGenderNoAndName  ");
            strSql.Append(" FROM b_lab_GenderType where 1=1 ");
            if (model.LabCode != null)
            {
                strSql.Append(" and LabCode='" + model.LabCode + "' ");
        ...
        public DataSet GetListByPage(ZhiFang.Model.Lab_GenderType model, int nowPageNum, int nowPageSize)
        {
            ...
            strSql.Append("select top " + nowPageSize + " * from b_lab_GenderType where 1=1  ");
            if (model.LabCode != null)
            {
                strSql.Append(" and LabCode='" + model.LabCode + "' ");
            }
            strSql.Append(" " + strLike + " and GenderID not in ");
            strSql.Append("(select top " + (nowPageSize * nowPageNum) + " GenderID from b_lab_GenderType where 1=1  ");
            if (model.LabCode != null)
            {
                strSql.Append(" and LabCode='" + model.LabCode + "' ");
            }
            strSql.Append(" " + strLike + " order by " + model.OrderField + ") order by " + model.OrderField + "  ");
            return DbHelperSQL.ExecuteDataSet(strSql.ToString());
        }
        ...
        public int AddByDataRow(DataRow dr)
        {
            ...
            {
                ZhiFang.Common.Log.Log.Error("ZhiFang.DAL.MsSql.Weblis.b_lab_GenderType.AddByDataRow 同步数据时异常：", ex);
                return 0;
            }
        }
        public int UpdateByDataRow(DataRow dr)
        {
            ...
            {
                ZhiFang.Common.Log.Log.Error("ZhiFang.DAL.MsSql.Weblis.b_lab_GenderType .UpdateByDataRow同步数据时异常：", ex);
                return 0;
            }
        }
        ...
        public DataSet GetLabCodeNo(string LabCode, List<string> LabCname)
        {
            ...
            try
            {
                ...
                ZhiFang.Common.Log.Log.Info("ZhiFang.DAL.MsSql.Weblis.b_lab_GenderType.GetLabCodeNo:" + strSql);
                ds = DbHelperSQL.ExecuteDataSet(strSql);
            }
            catch (Exception ex)
            {
                ZhiFang.Common.Log.Log.Error("ZhiFang.DAL.MsSql.Weblis.b_lab_GenderType.GetLabCodeNo异常->", ex);
            }
            return ds;
        }
    }
