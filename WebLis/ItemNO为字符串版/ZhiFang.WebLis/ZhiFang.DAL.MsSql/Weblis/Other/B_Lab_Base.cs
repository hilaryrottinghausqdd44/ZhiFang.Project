    ...
    public class B_Lab_Base : BaseDALLisDB, IDB_Lab_Base
    {
        ...
        public DataSet GetLabNo(string tableName, List<string> labCname, string SourceOrgID, string str)
        {
            ...
            try
            {
                ...
                ZhiFang.Common.Log.Log.Info("ZhiFang.DAL.MsSql.Weblis.Other.B_Lab_Base.GetLabNo:" + strSql);
                ds = DbHelperSQL.ExecuteDataSet(strSql);
            }
            catch (Exception ex)
            {
                ZhiFang.Common.Log.Log.Error("ZhiFang.DAL.MsSql.Weblis.Other.B_Lab_Base.GetLabNo异常->", ex);
            }
            return ds;
        }
        ...
        public DataSet GetCentNo(string tableName, List<string> labNo, string SourceOrgID, string str)
        {
            ...
            try
            {
                ...
                ZhiFang.Common.Log.Log.Info("ZhiFang.DAL.MsSql.Weblis.Other.B_Lab_Base.GetCentNo:" + strSql);
                ds = DbHelperSQL.ExecuteDataSet(strSql);
            }
            catch (Exception ex)
            {
                ZhiFang.Common.Log.Log.Error("ZhiFang.DAL.MsSql.Weblis.Other.B_Lab_Base.GetCentNo异常->", ex);
            }
            return ds;
        }
        ...
        public bool CheckCenterNo(string tableName, List<string> labNo, string SourceOrgID, string str)
        {
            ...
            try
            {
                ...
                string strSql = "select * from " + tableName + " where ControlLabNo='" + SourceOrgID.Trim() + "' and Control" + str + " in (" + strItemNos + ")";
                ZhiFang.Common.Log.Log.Info("ZhiFang.DAL.MsSql.Weblis.Other.B_Lab_Base.CheckCenterNo:" + strSql);
            ...
            {
                ZhiFang.Common.Log.Log.Error("ZhiFang.DAL.MsSql.Weblis.Other.B_Lab_Base.CheckCenterNo异常->", ex);
            }
            return b;
        }
        ...
        public DataSet GetLabControlNo(string tableName, List<string> CenterNo, string SourceOrgID, string str)
        {
            ...
            try
            {
                ...
                ZhiFang.Common.Log.Log.Info("ZhiFang.DAL.MsSql.Weblis.Other.B_Lab_Base.GetLabControlNo:" + strSql);
                ds = DbHelperSQL.ExecuteDataSet(strSql);
            }
            catch (Exception ex)
            {
                ZhiFang.Common.Log.Log.Error("ZhiFang.DAL.MsSql.Weblis.Other.B_Lab_Base.GetLabControlNo异常->", ex);
            }
            return ds;
        }
        ...
        public bool CheckLabNo(string tableName, List<string> CenterNo, string SourceOrgID, string str)
        {
            ...
            try
            {
                ...
                ZhiFang.Common.Log.Log.Info("ZhiFang.DAL.MsSql.Weblis.Other.B_Lab_Base.CheckLabNo:" + strSql);
                DataSet ds = DbHelperSQL.ExecuteDataSet(strSql);
                if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                {
                    if (ds.Tables[0].Rows.Count == count)
            ...
            {
                ZhiFang.Common.Log.Log.Error("ZhiFang.DAL.MsSql.Weblis.Other.B_Lab_Base.CheckLabNo异常->", ex);
            }
            return b;
        }
        ...
        public int GetTotalCenterCount(string tableName, string SourceOrgID, string LabNo, string str)
        {
            ...
            strSql.Append("select count(*) FROM " + tableName + " where ControlLabNo='" + SourceOrgID +  "' and Control" + str + " in (" + LabNo + ")");
            string strCount = DbHelperSQL.ExecuteScalar(strSql.ToString());
            if (strCount != null && strCount.Trim() != "")
            {
                return Convert.ToInt32(strCount.Trim());
