...
namespace ZhiFang.DAL.MsSql.Weblis
{
	...
	{	
        ...
        public DataSet GetListByPage(ZhiFang.Model.TestType model, int nowPageNum, int nowPageSize)
        {
            ...
            {
                strSql.Append(" select top " + nowPageSize + "  * from TestType left join TestTypeControl on TestType.TestTypeNo=TestTypeControl.TestTypeNo ");
                if (model.LabCode != null)
                {
                    strSql.Append(" and TestTypeControl.ControlLabNo='" + model.LabCode + "' ");
                }
                strSql.Append("where TestTypeNo not in ( ");
                strSql.Append("select top " + (nowPageSize * nowPageNum ) + " TestTypeNo from  TestType left join TestTypeControl on TestType.TestTypeNo=TestTypeControl.TestTypeNo ");
                if (model.LabCode != null)
                {
                    strSql.Append(" and TestTypeControl.ControlLabNo='" + model.LabCode + "' ");
                }
                strSql.Append("order by TestType.TestTypeNo desc  ) order by TestType.TestTypeNo  desc ");
                return DbHelperSQL.ExecuteDataSet(strSql.ToString());
            }
            else
            {
                strSql.Append("select top " + nowPageSize + "  * from TestType where TestTypeNo not in  ");
                strSql.Append("(select top " + (nowPageSize * nowPageNum) + " TestTypeNo from TestType order by TestTypeNo  desc ) order by TestTypeNo  desc  ");
                return DbHelperSQL.ExecuteDataSet(strSql.ToString());
            }
        }
