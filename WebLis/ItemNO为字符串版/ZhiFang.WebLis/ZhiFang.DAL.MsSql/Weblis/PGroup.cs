...
namespace ZhiFang.DAL.MsSql.Weblis
{
    ...
    public class PGroup : BaseDALLisDB, IDPGroup, IDBatchCopy
    {
        ...
        public int Add(List<ZhiFang.Model.PGroup> modelList)
        {
            string strsqls = "";
        ...
        public int Update(List<ZhiFang.Model.PGroup> modelList)
        {
            string strsqls = "";
            foreach (var model in modelList)
            {
        ...
        */
        ...
        public DataSet GetListByPage(ZhiFang.Model.PGroup model, int nowPageNum, int nowPageSize)
        {
            ...
            if (model != null && model.LabCode != null)
            {
                ...
                strSql.Append(" select top " + nowPageSize + "  * from PGroup left join B_PGroupControl on PGroup.SectionNo=B_PGroupControl.SectionNo ");
                if (model.LabCode != null)
                {
                    strSql.Append(" and B_PGroupControl.ControlLabNo='" + model.LabCode + "' ");
                }
                strSql.Append("where PGroup.SectionNo not in ( ");
                strSql.Append("select top " + (nowPageSize * nowPageNum) + " PGroup.SectionNo from  PGroup left join B_PGroupControl on PGroup.SectionNo=B_PGroupControl.SectionNo ");
                if (model.LabCode != null)
                {
                    strSql.Append(" and B_PGroupControl.ControlLabNo='" + model.LabCode + "' ");
                }
            ...
            else
            {
                ...
                strSql.Append("(select top " + (nowPageSize * nowPageNum) + " SectionNo from PGroup where 1=1 " + likesql + " order by " + model.OrderField + " desc ) " + likesql + " order by " + model.OrderField + "  desc  ");
        ...
        public bool CopyToLab(List<string> lst)
        {
            ...
            try
            {
                for (int i = 0; i < lst.Count; i++)
                {
                    ...
                    strSql.Append(" LabSectionNo , onlinetime , KeyDispOrder , dummygroup , uniontype , SectorTypeNo ,  SampleRule ,  LabSuperGroupNo , CName , ShortName , ShortCode , SectionDesc , SectionType , Visible , DispOrder ");
                    strSql.Append(") select '" + lst[i].Trim() + "' as LabCode, ");
                    strSql.Append("SectionNo,onlinetime,KeyDispOrder,dummygroup,uniontype,SectorTypeNo,SampleRule,SuperGroupNo,CName,ShortName,ShortCode,SectionDesc,SectionType,Visible,DispOrder");
                    strSql.Append(" from PGroup ");
                    if (str.Trim() != "")
                    ...
                    strSqlControl.Append("  '" + lst[i].Trim() + "'+'_'+convert(varchar(50),SectionNo)+'_'+convert(varchar(50), SectionNo) as SectionControlNo,SectionNo,'" + lst[i].Trim() + "' as ControlLabNo,SectionNo,1 as UseFlag ");
