...
namespace ZhiFang.DAL.MsSql.Weblis
{
	...
	{	
        ...
        public DataSet GetListByPage(ZhiFang.Model.BUSINESSMAN model, int nowPageNum, int nowPageSize)
        {
            ...
            {
                strSql.Append(" select top " + nowPageSize + "  * from BUSINESSMAN left join BUSINESSMANControl on BUSINESSMAN.BMANNO=BUSINESSMANControl.BMANNO ");
                if (model.LabCode != null)
                {
                    strSql.Append(" and BUSINESSMANControl.ControlLabNo='" + model.LabCode + "' ");
                }
                strSql.Append("where BMANNO not in ( ");
                strSql.Append("select top " + (nowPageSize * nowPageNum ) + " BMANNO from  BUSINESSMAN left join BUSINESSMANControl on BUSINESSMAN.BMANNO=BUSINESSMANControl.BMANNO ");
                if (model.LabCode != null)
                {
                    strSql.Append(" and BUSINESSMANControl.ControlLabNo='" + model.LabCode + "' ");
                }
                strSql.Append("order by BUSINESSMAN.BMANNO  desc ) order by BUSINESSMAN.BMANNO desc  ");
                return DbHelperSQL.ExecuteDataSet(strSql.ToString());
            }
            else
            {
                strSql.Append("select top " + nowPageSize + "  * from BUSINESSMAN where BMANNO not in  ");
                strSql.Append("(select top " + (nowPageSize * nowPageNum) + " BMANNO from BUSINESSMAN order by BMANNO desc ) order by BMANNO  desc  ");
                return DbHelperSQL.ExecuteDataSet(strSql.ToString());
            }
        }
        ...
        public int AddByDataRow(DataRow dr)
        {
            ...
            {
                ZhiFang.Common.Log.Log.Error("ZhiFang.DAL.MsSql.Weblis.BUSINESSMAN.AddByDataRow 同步数据时异常：", ex);
                return 0;
            }          
        }
        public int UpdateByDataRow(DataRow dr)
        {
            ...
            {
                ZhiFang.Common.Log.Log.Error("ZhiFang.DAL.MsSql.Weblis.BUSINESSMAN .UpdateByDataRow同步数据时异常：", ex);
                return 0;
            }
        }
        public int DeleteByDataRow(DataRow dr)
        {
            ...
            {
                ZhiFang.Common.Log.Log.Error("ZhiFang.DAL.MsSql.weblis.BUSINESSMAN.DeleteByDataRow同步数据时异常：", ex);
                return 0;
            }
        }
        ...
        public DataSet GetListByTimeStampe(string LabCode, int dTimeStampe)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select *,'" + LabCode + "' as LabCode,BMANNO as LabBMANNO from BUSINESSMAN where 1=1 ");
