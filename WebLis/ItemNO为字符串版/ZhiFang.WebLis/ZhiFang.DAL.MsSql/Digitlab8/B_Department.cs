...
namespace ZhiFang.DAL.MsSql.Digitlab8  
{
	...
	public partial class B_Department : IDDepartment,IDBatchCopy,IDGetListByTimeStampe
	{	
		DBUtility.IDBConnection idb;
        public B_Department(string dbsourceconn)
        {
		...
		public int Add(ZhiFang.Model.Department model)
		{
            ...
            strSql.Append("DeptNo,CName,ShortName,ShortCode,Visible,DispOrder,HisOrderCode,StandCode,ZFStandCode,UseFlag");
		...
		public int Delete(int DeptNo)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from B_Department ");
			strSql.Append(" where DeptNo=@DeptNo ");
						SqlParameter[] parameters = {
					new SqlParameter("@DeptNo", SqlDbType.Int,4)};
		...
		public ZhiFang.Model.Department GetModel(int DeptNo)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select DeptID, DeptNo, CName, ShortName, ShortCode, Visible, DispOrder, HisOrderCode, DTimeStampe, AddTime, StandCode, ZFStandCode, UseFlag  ");			
			...
			ZhiFang.Model.Department model=new ZhiFang.Model.Department();
        ...
        public DataSet GetList(ZhiFang.Model.Department model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select *,DeptNo as LabNo,CName+'('+convert(varchar(100),DeptNo)+')' as LabNoAndName ");
            strSql.Append(" FROM B_Department where 1=1 ");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(*) FROM B_Department ");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(*) FROM B_Department where 1=1 ");
        ...
        public DataSet GetListByPage(ZhiFang.Model.Department model, int nowPageNum, int nowPageSize)
        {
            ...
            {
                strSql.Append(" select top " + nowPageSize + "  * from B_Department left join B_DepartmentControl on B_Department.DeptNo=B_DepartmentControl.DeptNo ");
                if (model.LabCode != null)
                {
                    strSql.Append(" and B_DepartmentControl.ControlLabNo='" + model.LabCode + "' ");
                }
                strSql.Append("where DeptID not in ( ");
                strSql.Append("select top " + (nowPageSize * nowPageNum) + " DeptID from  B_Department left join B_DepartmentControl on B_Department.DeptNo=B_DepartmentControl.DeptNo ");
                if (model.LabCode != null)
                {
                    strSql.Append(" and B_DepartmentControl.ControlLabNo='" + model.LabCode + "' ");
                }
                strSql.Append("order by B_Department.DeptID ) order by B_Department.DeptID ");
                return idb.ExecuteDataSet(strSql.ToString());
            }
            else
            {
                strSql.Append("select top " + nowPageSize + "  * from B_Department where DeptID not in  ");
                strSql.Append("(select top " + (nowPageSize * nowPageNum) + " DeptID from B_Department order by DeptID) order by DeptID  ");
                return idb.ExecuteDataSet(strSql.ToString());
            }
        }
        ...
        {
            StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) from B_Department ");
			strSql.Append(" where DeptNo ='"+DeptNo+"'");
			string strCount = idb.ExecuteScalar(strSql.ToString());
		...
		public bool CopyToLab(List<string> lst)
		{
			...
			string LabTableName="B_Department";
            ...
            try
            {
	            for (int i = 0; i < lst.Count; i++)
	            {
	            		...
	            		strSql.Append(" LabDeptNo , CName , ShortName , ShortCode , Visible , DispOrder , HisOrderCode , StandCode , ZFStandCode , UseFlag ");
	            		...
	            		strSqlControl.Append(" from B_Department ");  
		...
		public int GetMaxId()
        {
            return idb.GetMaxID("DeptNo","B_Department");
        }

        public DataSet GetList(int Top, ZhiFang.Model.Department model, string filedOrder)
        {
            StringBuilder strSql=new StringBuilder();
			strSql.Append("select ");
			if(Top>0)
        ...
        public DataSet GetListByTimeStampe(string LabCode, int dTimeStampe)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select *,'" + LabCode + "' as LabCode,DeptNo as LabDeptNo from B_Department where 1=1 ");
            ...
            strSql2.Append("select *,LabDeptNo as DeptNo from B_Lab_Department where 1=1 ");
            if (LabCode.Trim() != "")
            {
            ...
            strSql3.Append("select * from B_DepartmentControl where 1=1 ");
            if (LabCode.Trim() != "")
            {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select *,DeptNo as LabNo,convert(varchar(100),DeptNo)+'_'+CName as LabNo_Value,CName+'('+convert(varchar(100),DeptNo)+')' as LabNoAndName_Text ");
            strSql.Append(" FROM B_Department  ");
        ...
        #region IDataBase<Department> 成员
        ...
        public int AddByDataRow(DataRow dr)
        {
            try
            {
                StringBuilder strSql = new StringBuilder();
                strSql.Append("insert into B_Department (");
                strSql.Append("DeptNo,CName,ShortName,ShortCode,Visible,DispOrder,HisOrderCode,AddTime,StandCode,ZFStandCode,UseFlag");
                strSql.Append(") values (");
                strSql.Append("'" + dr["DeptNo"].ToString().Trim() + "','" + dr["CName"].ToString().Trim() + "','" + dr["ShortName"].ToString().Trim() + "','" + dr["ShortCode"].ToString().Trim() + "','" + dr["Visible"].ToString().Trim() + "','" + dr["DispOrder"].ToString().Trim() + "','" + dr["HisOrderCode"].ToString().Trim() + "','" + DateTime.Parse(dr["AddTime"].ToString().Trim()) + "','" + dr["StandCode"].ToString().Trim() + "','" + dr["ZFStandCode"].ToString().Trim() + "','" + dr["UseFlag"].ToString().Trim() + "'");
                strSql.Append(") ");
                return idb.ExecuteNonQuery(strSql.ToString());
            }
            catch
