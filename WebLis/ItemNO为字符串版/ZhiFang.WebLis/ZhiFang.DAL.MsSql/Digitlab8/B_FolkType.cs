...
namespace ZhiFang.DAL.MsSql.Digitlab8  
{
	...
	public partial class B_FolkType : IDFolkType,IDBatchCopy,IDGetListByTimeStampe
	{	
		DBUtility.IDBConnection idb;
        public B_FolkType(string dbsourceconn)
        {
		...
		public int Add(ZhiFang.Model.FolkType model)
		{
			...
			strSql.Append("insert into B_FolkType(");			
            strSql.Append("FolkNo,CName,ShortCode,Visible,DispOrder,HisOrderCode,StandCode,ZFStandCode,UseFlag");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update B_FolkType set ");
		...
		public int Delete(int FolkNo)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from B_FolkType ");
			strSql.Append(" where FolkNo=@FolkNo ");
						SqlParameter[] parameters = {
					new SqlParameter("@FolkNo", SqlDbType.Int,4)};
		...
		public ZhiFang.Model.FolkType GetModel(int FolkNo)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select FolkID, FolkNo, CName, ShortCode, Visible, DispOrder, HisOrderCode, DTimeStampe, AddTime, StandCode, ZFStandCode, UseFlag  ");			
			strSql.Append("  from B_FolkType ");
			strSql.Append(" where FolkNo=@FolkNo ");
						SqlParameter[] parameters = {
					new SqlParameter("@FolkNo", SqlDbType.Int,4)};
			parameters[0].Value = FolkNo;
		...
		public DataSet GetList(ZhiFang.Model.FolkType model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select * ");
			strSql.Append(" FROM B_FolkType where 1=1 ");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(*) FROM B_FolkType ");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(*) FROM B_FolkType where 1=1 ");
        ...
        public DataSet GetListByPage(ZhiFang.Model.FolkType model, int nowPageNum, int nowPageSize)
        {
            ...
            {
                strSql.Append(" select top " + nowPageSize + "  * from B_FolkType left join B_FolkTypeControl on B_FolkType.FolkNo=B_FolkTypeControl.FolkNo ");
                if (model.LabCode != null)
                {
                    strSql.Append(" and B_FolkTypeControl.ControlLabNo='" + model.LabCode + "' ");
                }
                strSql.Append("where FolkID not in ( ");
                strSql.Append("select top " + (nowPageSize * nowPageNum) + " FolkID from  B_FolkType left join B_FolkTypeControl on B_FolkType.FolkNo=B_FolkTypeControl.FolkNo ");
                if (model.LabCode != null)
                {
                    strSql.Append(" and B_FolkTypeControl.ControlLabNo='" + model.LabCode + "' ");
                }
                strSql.Append("order by B_FolkType.FolkID ) order by B_FolkType.FolkID ");
                return idb.ExecuteDataSet(strSql.ToString());
            }
            else
            {
                strSql.Append("select top " + nowPageSize + "  * from B_FolkType where FolkID not in  ");
                strSql.Append("(select top " + (nowPageSize * nowPageNum) + " FolkID from B_FolkType order by FolkID) order by FolkID  ");
                return idb.ExecuteDataSet(strSql.ToString());
            }
        }
        ...
        {
            StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) from B_FolkType ");
			strSql.Append(" where FolkNo ='"+FolkNo+"'");
			string strCount = idb.ExecuteScalar(strSql.ToString());
		...
		public bool CopyToLab(List<string> lst)
		{
			...
			string LabTableName="B_FolkType";
            ...
            try
            {
	            for (int i = 0; i < lst.Count; i++)
	            {
	            		...
	            		strSql.Append(" LabFolkNo , CName , ShortCode , Visible , DispOrder , HisOrderCode , StandCode , ZFStandCode , UseFlag ");
	            		...
	            		strSqlControl.Append(" from B_FolkType ");  
		...
		public int GetMaxId()
        {
            return idb.GetMaxID("FolkNo","B_FolkType");
        }
        ...
        public DataSet GetListByTimeStampe(string LabCode, int dTimeStampe)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select *,'" + LabCode + "' as LabCode,FolkNo as LabFolkNo from B_FolkType where 1=1 ");
            ...
            strSql2.Append("select *,LabFolkNo as FolkNo from B_Lab_FolkType where 1=1 ");
            if (LabCode.Trim() != "")
            {
        ...
        public int AddByDataRow(DataRow dr)
        {
            try
            {
                StringBuilder strSql = new StringBuilder();
                strSql.Append("insert into B_FolkType (");
                strSql.Append("FolkNo,CName,ShortCode,Visible,DispOrder,HisOrderCode,AddTime,StandCode,ZFStandCode,UseFlag");
                strSql.Append(") values (");
                strSql.Append("'" + dr["FolkNo"].ToString().Trim() + "','" + dr["CName"].ToString().Trim() + "','" + dr["ShortCode"].ToString().Trim() + "','" + dr["Visible"].ToString().Trim() + "','" + dr["DispOrder"].ToString().Trim() + "','" + dr["HisOrderCode"].ToString().Trim() + "','" + DateTime.Parse(dr["AddTime"].ToString().Trim()) + "','" + dr["StandCode"].ToString().Trim() + "','" + dr["ZFStandCode"].ToString().Trim() + "','" + dr["UseFlag"].ToString().Trim() + "'");
                strSql.Append(") ");
                return idb.ExecuteNonQuery(strSql.ToString());
            }
            catch
