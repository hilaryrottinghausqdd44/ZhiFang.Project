...
namespace ZhiFang.DAL.MsSql.Digitlab8  
{
	...
	public partial class B_Doctor : IDDoctor,IDBatchCopy,IDGetListByTimeStampe
	{	
		DBUtility.IDBConnection idb;
        public B_Doctor(string dbsourceconn)
        {
		...
		public int Add(ZhiFang.Model.Doctor model)
		{
			...
			strSql.Append("insert into B_Doctor(");			
            strSql.Append("DoctorNo,CName,ShortCode,HisOrderCode,Visible,StandCode,ZFStandCode,UseFlag");
            ...
            parameters[0].Value = model.DoctorNo;                        
            parameters[1].Value = model.CName;                        
            parameters[2].Value = model.ShortCode;                        
            parameters[3].Value = model.HisOrderCode;                        
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update B_Doctor set ");
		...
		public int Delete(int DoctorNo)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from B_Doctor ");
			strSql.Append(" where DoctorNo=@DoctorNo ");
						SqlParameter[] parameters = {
					new SqlParameter("@DoctorNo", SqlDbType.Int,4)};
			parameters[0].Value = DoctorNo;
		...
		public ZhiFang.Model.Doctor GetModel(int DoctorNo)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select DoctorID, DoctorNo, CName, ShortCode, HisOrderCode, Visible, DTimeStampe, AddTime, StandCode, ZFStandCode, UseFlag  ");			
			strSql.Append("  from B_Doctor ");
			strSql.Append(" where DoctorNo=@DoctorNo ");
						SqlParameter[] parameters = {
					new SqlParameter("@DoctorNo", SqlDbType.Int,4)};
			parameters[0].Value = DoctorNo;
			...
			ZhiFang.Model.Doctor model=new ZhiFang.Model.Doctor();
		...
		public DataSet GetList(ZhiFang.Model.Doctor model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select * ");
			strSql.Append(" FROM B_Doctor where 1=1 ");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(*) FROM B_Doctor ");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(*) FROM B_Doctor where 1=1 ");
        ...
        public DataSet GetListByPage(ZhiFang.Model.Doctor model, int nowPageNum, int nowPageSize)
        {
            ...
            {
                strSql.Append(" select top " + nowPageSize + "  * from B_Doctor left join B_DoctorControl on B_Doctor.DoctorNo=B_DoctorControl.DoctorNo ");
                if (model.LabCode != null)
                {
                    strSql.Append(" and B_DoctorControl.ControlLabNo='" + model.LabCode + "' ");
                }
                strSql.Append("where DoctorID not in ( ");
                strSql.Append("select top " + (nowPageSize * nowPageNum) + " DoctorID from  B_Doctor left join B_DoctorControl on B_Doctor.DoctorNo=B_DoctorControl.DoctorNo ");
                if (model.LabCode != null)
                {
                    strSql.Append(" and B_DoctorControl.ControlLabNo='" + model.LabCode + "' ");
                }
                strSql.Append("order by B_Doctor.DoctorID ) order by B_Doctor.DoctorID ");
                return idb.ExecuteDataSet(strSql.ToString());
            }
            else
            {
                strSql.Append("select top " + nowPageSize + "  * from B_Doctor where DoctorID not in  ");
                strSql.Append("(select top " + (nowPageSize * nowPageNum) + " DoctorID from B_Doctor order by DoctorID) order by DoctorID  ");
                return idb.ExecuteDataSet(strSql.ToString());
            }
        }
        ...
        {
            StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) from B_Doctor ");
			strSql.Append(" where DoctorNo ='"+DoctorNo+"'");
			string strCount = idb.ExecuteScalar(strSql.ToString());
		...
		public bool CopyToLab(List<string> lst)
		{
            ...
            try
            {
	            for (int i = 0; i < lst.Count; i++)
	            {
	            		...
	            		strSql.Append(" LabDoctorNo , CName , ShortCode , HisOrderCode , Visible , StandCode , ZFStandCode , UseFlag ");
	            		...
	            		strSqlControl.Append(" from B_Doctor ");  
		...
		public int GetMaxId()
        {
            return idb.GetMaxID("DoctorNo","B_Doctor");
        }

        public DataSet GetList(int Top, ZhiFang.Model.Doctor model, string filedOrder)
        {
            StringBuilder strSql=new StringBuilder();
			strSql.Append("select ");
			if(Top>0)
        ...
        public DataSet GetListByTimeStampe(string LabCode, int dTimeStampe)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select *,'" + LabCode + "' as LabCode,DoctorNo as LabDoctorNo from B_Doctor where 1=1 ");
            ...
            strSql2.Append("select *,LabDoctorNo as DoctorNo from B_Lab_Doctor where 1=1 ");
            if (LabCode.Trim() != "")
            {
            ...
            strSql3.Append("select * from B_DoctorControl where 1=1 ");
            if (LabCode.Trim() != "")
            {
        ...
        public int AddByDataRow(DataRow dr)
        {
            try
            {
                StringBuilder strSql = new StringBuilder();
                strSql.Append("insert into B_Doctor (");
                strSql.Append("DoctorNo,CName,ShortCode,HisOrderCode,Visible,AddTime,StandCode,ZFStandCode,UseFlag");
                strSql.Append(") values (");
                strSql.Append("'" + dr["DoctorNo"].ToString().Trim() + "','" + dr["CName"].ToString().Trim() + "','" + dr["ShortCode"].ToString().Trim() + "','" + dr["HisOrderCode"].ToString().Trim() + "','" + dr["Visible"].ToString().Trim() + "','" + DateTime.Parse(dr["AddTime"].ToString().Trim()) + "','" + dr["StandCode"].ToString().Trim() + "','" + dr["ZFStandCode"].ToString().Trim() + "','" + dr["UseFlag"].ToString().Trim() + "'");
                strSql.Append(") ");
                return idb.ExecuteNonQuery(strSql.ToString());
            }
            catch
