...
namespace ZhiFang.DAL.Oracle.weblis
{
    ...
    public class PrintFormat : BaseDALLisDB, IDPrintFormat
    {
        ...
        public int Add(ZhiFang.Model.PrintFormat model)
        {
            ...
            object obj1 = DbHelperSQL.GetSingle(strSql.ToString());
            if (obj1 == null)
            {
                return 1;
            }
            object obj2 = DbHelperSQL.ExecuteScalar("SELECT S_PRINTFORMAT.currval  id FROM Dual");
            ZhiFang.Common.Log.Log.Info("obj2:" + obj2);
            if (obj2 == null)
            {
                return 1;
            }
        ...
        */
        ...
        public DataSet GetListByPage(ZhiFang.Model.PrintFormat model, int nowPageNum, int nowPageSize)
        {
            ...
            {
                strSql.Append(" select  * from PrintFormat left join PrintFormatControl on PrintFormat.Id=PrintFormatControl.Id ");
                if (model.LabCode != null)
                {
                    strSql.Append(" and PrintFormatControl.ControlLabNo='" + model.LabCode + "' ");
                }
                strSql.Append("where  ROWNUM <= '" + nowPageSize + "' and Id not in ( ");
                strSql.Append("select Id from  PrintFormat left join PrintFormatControl on PrintFormat.Id=PrintFormatControl.Id where 1=1 ");
                if (model.LabCode != null)
                {
                    strSql.Append(" and PrintFormatControl.ControlLabNo='" + model.LabCode + "' ");
                }
                strSql.Append(" and ROWNUM <= '" + (nowPageSize * nowPageNum) + "' ) order by PrintFormat.Id ");
                return DbHelperSQL.ExecuteDataSet(strSql.ToString());
            }
            else
            {
                strSql.Append("select * from PrintFormat where  ROWNUM <= '" + nowPageSize + "' and Id not in  ");
                strSql.Append("(select Id from PrintFormat where 1=1 and ROWNUM <= '" + (nowPageSize * nowPageNum) + "' ) order by Id  ");
                return DbHelperSQL.ExecuteDataSet(strSql.ToString());
            }
        }
