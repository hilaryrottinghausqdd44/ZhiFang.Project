...
namespace ZhiFang.DAL.Oracle.weblis
{
    ...
    public partial class B_Lab_Doctor : IDLab_Doctor
    {
        ...
        public int Add(ZhiFang.Model.Lab_Doctor model)
        {
            ...
            {
                strSql1.Append("LabDoctorNo,");
                strSql2.Append("" + model.LabDoctorNo + ",");
            }
            if (model.CName != null)
            {
        ...
        public int Update(ZhiFang.Model.Lab_Doctor model)
        {
            ...
            strSql.Remove(n, 1);
            strSql.Append(" where LabCode='" + model.LabCode + "' and LabDoctorNo=" + model.LabDoctorNo + "  ");
            if (idb.ExecuteNonQuery(strSql.ToString()) > 0)
            {
                return d_log.OperateLog("Doctor", "", "", DateTime.Now, 1);
            }
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select B_Lab_Doctor.* ,concat(concat('(',CONCAT(LabDoctorNo,')')),cname) as LabDoctorNoAndName ");
            strSql.Append(" FROM B_Lab_Doctor where 1=1 ");
            if (model.LabCode != null)
            {
                strSql.Append(" and LabCode='" + model.LabCode + "' ");
        ...
        public DataSet GetListByPage(ZhiFang.Model.Lab_Doctor model, int nowPageNum, int nowPageSize)
        {
            ...
            strSql.Append("select * from B_Lab_Doctor where 1=1  ");
            if (model.LabCode != null)
            {
                strSql.Append(" and LabCode='" + model.LabCode + "' ");
            }
            strSql.Append(" " + strLike + " and  ROWNUM <= '" + nowPageSize + "' and DoctorID not in ");
            strSql.Append("(select DoctorID from B_Lab_Doctor where 1=1 and ROWNUM <= '" + (nowPageSize * nowPageNum) + "' ");
            if (model.LabCode != null)
            {
                strSql.Append(" and LabCode='" + model.LabCode + "' ");
            }
