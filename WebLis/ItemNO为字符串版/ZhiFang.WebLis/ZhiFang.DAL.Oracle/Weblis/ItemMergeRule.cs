       ...
       public ItemMergeRule()
        ...
        public DataSet GetListItem(Model.ItemMergeRule model) 
        {
            try
            {
                ...
                //{
                //    strSql.Append(" and ItemMerge.ClientNo='" + model.ClientNo + "' ");
                //}
                //if (model.SuperGroupNo != null)
                //{
                //    strSql.Append(" and ItemMerge.SuperGroupNo=" + model.SuperGroupNo + " ");
                //}
                if (model.ItemNo != null)
                {
                    strSql.Append(" and ItemMerge.ItemNo in (" + model.ItemNo + ")");
                }
                ZhiFang.Common.Log.Log.Info(strSql.ToString());
                return DbHelperSQL.ExecuteDataSet(strSql.ToString());
            }
        ...
        public System.Data.DataSet GetListByPage(Model.ItemMergeRule model, int nowPageNum, int nowPageSize)
        {
            ...
            if (model != null)
            {
                ...
                //}
                //strSql.Append("select top " + nowPageSize + " CLIENTELE.* from  BusinessLogicClientControl INNER JOIN  CLIENTELE ON BusinessLogicClientControl.ClientNo = CLIENTELE.ClIENTNO where BusinessLogicClientControl.Account+'_'+BusinessLogicClientControl.ClientNo not in  ");
                //strSql.Append("(select top " + (nowPageSize * nowPageNum) + " BusinessLogicClientControl.Account+'_'+BusinessLogicClientControl.ClientNo from  BusinessLogicClientControl INNER JOIN   CLIENTELE ON BusinessLogicClientControl.ClientNo = CLIENTELE.ClIENTNO where 1=2 " + sql + "  order by BusinessLogicClientControl.Account,BusinessLogicClientControl.ClientNo)");
                //strSql.Append(sql);
            ...
            {
                //strSql.Append("select top " + nowPageSize + "  BusinessLogicClientControl.Account, CLIENTELE.*  from BusinessLogicClientControl INNER JOIN  CLIENTELE ON BusinessLogicClientControl.ClientNo = CLIENTELE.ClIENTNO where BusinessLogicClientControl.Account+'_'+BusinessLogicClientControl.ClientNo not in  ");
                //strSql.Append("(select top " + (nowPageSize * nowPageNum) + "BusinessLogicClientControl.Account+'_'+BusinessLogicClientControl.ClientNo from BusinessLogicClientControl INNER JOIN   CLIENTELE ON BusinessLogicClientControl.ClientNo = CLIENTELE.ClIENTNO order by BusinessLogicClientControl.Account,BusinessLogicClientControl.ClientNo) order by BusinessLogicClientControl.Account,BusinessLogicClientControl.ClientNo  ");
                //return DbHelperSQL.ExecuteDataSet(strSql.ToString());
            }
            return null;
        }
