...
namespace ZhiFang.DAL.Oracle.weblis
{
    ...
    public partial class B_Lab_SickType : BaseDALLisDB, IDLab_SickType
    {
        ...
        public int Add(ZhiFang.Model.Lab_SickType model)
        {
            ...
            {
                strSql1.Append("LabSickTypeNo,");
                strSql2.Append("" + model.LabSickTypeNo + ",");
            }
            if (model.CName != null)
            {
        ...
        public int Update(ZhiFang.Model.Lab_SickType model)
        {
            ...
            strSql.Remove(n, 1);
            strSql.Append(" where LabCode='" + model.LabCode + "' and LabSickTypeNo=" + model.LabSickTypeNo + "  ");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  B_Lab_SickType.*,concat(concat('(',CONCAT(LabSickTypeNo,')')),cname) as LabSickTypeNoAndName  ");
            strSql.Append(" FROM B_Lab_SickType where 1=1 ");
            if (model.LabCode != null)
            {
                strSql.Append(" and LabCode='" + model.LabCode + "' ");
        ...
        public DataSet GetListByPage(ZhiFang.Model.Lab_SickType model, int nowPageNum, int nowPageSize)
        {
            ...
            strSql.Append("select * from B_Lab_SickType where 1=1  ");
            if (model.LabCode != null)
            {
                strSql.Append(" and LabCode='" + model.LabCode + "' ");
            }
            strSql.Append(" " + strLike + " and  ROWNUM <= '" + nowPageSize + "' and SickTypeID not in ");
            strSql.Append("(select SickTypeID from B_Lab_SickType where 1=1 and ROWNUM <= '" + (nowPageSize * nowPageNum) + "' ");
            if (model.LabCode != null)
            {
                strSql.Append(" and LabCode='" + model.LabCode + "' ");
            }
        ...
        public int AddByDataRow(DataRow dr)
        {
            ...
            {
                ZhiFang.Common.Log.Log.Error("ZhiFang.DAL.Oracle.weblis.B_Lab_SickType.AddByDataRow 同步数据时异常：", ex);
                return 0;
            }
        }
        public int UpdateByDataRow(DataRow dr)
        {
            ...
            {
                ZhiFang.Common.Log.Log.Error("ZhiFang.DAL.Oracle.weblis.B_Lab_SickType .UpdateByDataRow同步数据时异常：", ex);
                return 0;
            }
        }
        ...
        public DataSet GetLabCodeNo(string LabCode, List<string> LabCnameList)
        {
            ...
            try
            {
                ...
                ZhiFang.Common.Log.Log.Info("ZhiFang.DAL.Oracle.weblis.LabSickTypeNo.GetLabCodeNo:" + strSql);
                ds = DbHelperSQL.ExecuteDataSet(strSql);
            }
            catch (Exception ex)
            {
                ZhiFang.Common.Log.Log.Error("ZhiFang.DAL.Oracle.weblis.LabSickTypeNo.GetLabCodeNo异常->", ex);
            }
            return ds;
        }
        /// <summary>
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select *   FROM B_Lab_SickType ");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
                if (Top > 0)
