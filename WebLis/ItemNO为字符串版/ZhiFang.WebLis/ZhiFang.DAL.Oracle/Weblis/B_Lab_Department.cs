...
namespace ZhiFang.DAL.Oracle.weblis
{
    ...
    public partial class B_Lab_Department : IDLab_Department
    {
        ...
        public int Update(ZhiFang.Model.Lab_Department model)
        {
            ...
            strSql.Remove(n, 1);
            strSql.Append(" where LabCode='" + model.LabCode + "' and LabDeptNo=" + model.LabDeptNo + "  ");
        ...
        public ZhiFang.Model.Lab_Department GetModel(string LabCode, int LabDeptNo)
        {
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                {
                    model.LabDeptNo = int.Parse(ds.Tables[0].Rows[0]["LabDeptNo"].ToString());
                }
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select B_Lab_Department.*,concat(concat('(',CONCAT(LabDeptNo,')')),cname) as LabDeptNoAndName ");
            strSql.Append(" FROM B_Lab_Department where 1=1 ");
            if (model.LabCode != null)
            {
                strSql.Append(" and LabCode='" + model.LabCode + "' ");
        ...
        public DataSet GetListByPage(ZhiFang.Model.Lab_Department model, int nowPageNum, int nowPageSize)
        {
            ...
            strSql.Append("select  * from B_Lab_Department where 1=1  ");
            if (model.LabCode != null)
            {
                strSql.Append(" and LabCode='" + model.LabCode + "' ");
            }
            strSql.Append(" " + strLike + " and  ROWNUM <= '" + nowPageSize + "' and DepartmentID not in ");
            strSql.Append("(select DepartmentID from B_Lab_Department where 1=1 and ROWNUM <= '" + (nowPageSize * nowPageNum) + "' ");
            if (model.LabCode != null)
            {
                strSql.Append(" and LabCode='" + model.LabCode + "' ");
            }
