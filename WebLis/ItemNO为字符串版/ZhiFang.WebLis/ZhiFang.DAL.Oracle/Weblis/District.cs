...
namespace ZhiFang.DAL.Oracle.weblis
{
    ...
    public class District : BaseDALLisDB, IDDistrict, IDBatchCopy
    {
        ...
        public ZhiFang.Model.District GetModel(int DistrictNo)
        {
            ...
            strSql.Append(" where ROWNUM <='1' and DistrictNo=" + DistrictNo + " ");
        ...
        */
        ...
        public DataSet GetListByPage(ZhiFang.Model.District model, int nowPageNum, int nowPageSize)
        {
            ...
            if (model != null && model.LabCode != null)
            {
                ...
                strSql.Append(" select  * from District left join B_DistrictControl on District.DistrictNo=B_DistrictControl.DistrictNo ");
                if (model.LabCode != null)
                {
                    strSql.Append(" and B_DistrictControl.ControlLabNo='" + model.LabCode + "' ");
                }
                strSql.Append("where  ROWNUM <= '" + nowPageSize + "' and District.DistrictNo not in ( ");
                strSql.Append("select District.DistrictNo from  District left join B_DistrictControl on District.DistrictNo=B_DistrictControl.DistrictNo ");
                if (model.LabCode != null)
                {
                    strSql.Append(" and B_DistrictControl.ControlLabNo='" + model.LabCode + "' ");
                }
            ...
            else
            {
                ...
                strSql.Append("select * from District where  ROWNUM <= '" + nowPageSize + "' and DistrictNo not in  ");
                strSql.Append("(select DistrictNo from District where 1=1 and ROWNUM <= '" + (nowPageSize * nowPageNum) + "' " + strLike + " ) " + strLike + " order by " + model.OrderField + "  ");
                return DbHelperSQL.ExecuteDataSet(strSql.ToString());
            }
        ...
        public bool CopyToLab(List<string> lst)
        {
            ...
            try
            {
                for (int i = 0; i < lst.Count; i++)
                {
                    ...
                    strSqlControl.Append("  concat(concat(concat(concat('" + lst[i].Trim() + "','_'),DistrictNo),'_'),DistrictNo) as DistrictControlNo,DistrictNo,'" + lst[i].Trim() + "' as ControlLabNo,DistrictNo,1 ");
        ...
        public int DeleteByDataRow(DataRow dr)
        {
            ...
            {
                ZhiFang.Common.Log.Log.Error("ZhiFang.DAL.Oracle.weblis.District .DeleteByDataRow同步数据时异常：", ex);
                return 0;
            }
        }
