...
namespace ZhiFang.DAL.Oracle.weblis
{
    ...
    public partial class ClientEleArea : IDClientEleArea, IDBatchCopy, IDGetListByTimeStampe
    {
        ...
        public int Add(ZhiFang.Model.ClientEleArea model)
        {
            ...
            {
                strSql1.Append("AreaCName,");
                strSql2.Append("'" + model.AreaCName + "',");
            }
            if (model.AreaShortName != null)
            {
                strSql1.Append("AreaShortName,");
                strSql2.Append("'" + model.AreaShortName + "',");
            }
        ...
        public int Update(ZhiFang.Model.ClientEleArea model)
        {
            ...
            if (model.AreaCName != null)
            {
                strSql.Append("AreaCName='" + model.AreaCName + "',");
            }
            if (model.AreaShortName != null)
            {
                strSql.Append("AreaShortName='" + model.AreaShortName + "',");
            }
            if (model.ClientNo != null)
            {
            ...
            strSql.Remove(n, 1);
            strSql.Append(" where AreaID=" + model.AreaID + " ");
            return idb.ExecuteNonQuery(strSql.ToString());
        ...
        public ZhiFang.Model.ClientEleArea GetModel(int AreaID)
        {
            ...
            strSql.Append(" AreaID,AreaCName,AreaShortName,ClientNo,AddTime ");
            strSql.Append(" from ClientEleArea ");
            strSql.Append(" where 1=1 and AreaID=" + AreaID + " ");
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                {
                    model.ClientNo = Convert.ToInt64(ds.Tables[0].Rows[0]["ClientNo"].ToString());
                }
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select ClientEleArea.*,AreaID as LabNo,concat(concat(AreaID,'_'),AreaCName) as LabNo_Value,concat(concat(concat(AreaCName,'('),AreaID),')') as LabNoAndName_Text ");
            strSql.Append(" FROM ClientEleArea  ");
            if (model.AreaCName != null)
            {
        ...
        public DataSet GetListByPage(ZhiFang.Model.ClientEleArea model, int nowPageNum, int nowPageSize)
        {
            ...
            try
            {
                ...
                strSql.Append("select * from ClientEleArea LEFT JOIN ClientEle on ClientEleArea.ClientNo=ClientEle.ClientNo where  ROWNUM <= '" + nowPageSize + "' and ClientEleArea.AreaID not in  ");
                strSql.Append("(select ClientEleArea.AreaID from ClientEleArea LEFT JOIN ClientEle on ClientEleArea.ClientNo=ClientEle.ClientNo where 1=1 and ROWNUM <= '" + (nowPageSize * nowPageNum) + "' " + strWhere + " ) " + strWhere + " order by  ClientEleArea." + model.OrderField + "  ");
                //ZhiFang.Common.Log.Log.Info(strSql.ToString());
                return idb.ExecuteDataSet(strSql.ToString());
            }
            catch (Exception ex)
            {
                ZhiFang.Common.Log.Log.Error("ZhiFang.DAL.Oracle.weblis.ClientEleArea.GetListByPage出错，sql=" + strSql.ToString(), ex);
                return null;
            }
        ...
        public DataSet GetListByTimeStampe(string LabCode, int dTimeStampe)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select ClientEleArea.*,'" + LabCode + "' as LabCode,AreaID as LabAreaID from ClientEleArea where 1=1 ");
            ...
            StringBuilder strSql2 = new StringBuilder();
            strSql2.Append("select B_Lab_ientEleArea.*,LabAreaID as AreaID from B_Lab_ientEleArea where 1=1 ");
            if (LabCode.Trim() != "")
            {
        ...
        public List<ZhiFang.Model.ClientEleArea> DataTableToList(DataTable dt)
        {
            ...
            if (rowsCount > 0)
            {
                ...
                for (int n = 0; n < rowsCount; n++)
                {
                    ...
                    if (dt.Columns.Contains("AreaID") && dt.Rows[0]["AreaID"].ToString() != "")
                    {
                        model.AreaID = int.Parse(dt.Rows[0]["AreaID"].ToString());
                    }
                    if (dt.Columns.Contains("AreaCName") && dt.Rows[0]["AreaCName"].ToString() != "")
                    {
                        model.AreaCName = dt.Rows[0]["AreaCName"].ToString();
                    }
                    if (dt.Columns.Contains("AreaShortName") && dt.Rows[0]["AreaShortName"].ToString() != "")
                    {
                        model.AreaShortName = dt.Rows[0]["AreaShortName"].ToString();
                    }
                    if (dt.Columns.Contains("ClientNo") && dt.Rows[0]["ClientNo"].ToString() != "")
                    {
                        model.ClientNo = Convert.ToInt64(dt.Rows[0]["ClientNo"].ToString());
                    }
                    if (dt.Columns.Contains("AddTime") && dt.Rows[0]["AddTime"].ToString() != "")
                    {
