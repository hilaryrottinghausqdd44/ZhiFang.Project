    ...
    public partial class S_Modules : IDModules
    {
        ...
        public int Add(ZhiFang.Model.Modules model)
        {
            ...
            else
            {
                ...
                subcount = int.Parse(idb.ExecuteScalar(" select count(*) from S_Modules where  SN LIKE '" + model.PSN + "%' and Len(SN)=" + (model.PSN.Length+2).ToString()));
            }
            if (subcount >= 9)
            {
        ...
        public DataSet GetListByPage(ZhiFang.Model.Modules model, int nowPageNum, int nowPageSize)
        {
            ...
            {
                strSql.Append(" select top " + nowPageSize + "  * from S_Modules left join S_ModulesControl on S_Modules.ID=S_ModulesControl.ID ");
                if (model.LabCode != null)
                {
                    strSql.Append(" and S_ModulesControl.ControlLabNo='" + model.LabCode + "' ");
                }
                strSql.Append("where ID not in ( ");
                strSql.Append("select top " + (nowPageSize * nowPageNum) + " ID from  S_Modules left join S_ModulesControl on S_Modules.ID=S_ModulesControl.ID ");
                if (model.LabCode != null)
                {
                    strSql.Append(" and S_ModulesControl.ControlLabNo='" + model.LabCode + "' ");
                }
                strSql.Append("order by S_Modules.ID ) order by S_Modules.ID ");
                return idb.ExecuteDataSet(strSql.ToString());
            }
            else
            {
                strSql.Append("select top " + nowPageSize + "  * from S_Modules where ID not in  ");
                strSql.Append("(select top " + (nowPageSize * nowPageNum) + " ID from S_Modules order by ID) order by ID  ");
                return idb.ExecuteDataSet(strSql.ToString());
            }
        }

        public DataSet GetListByPage(string where, int page, int limit, string Sort)
        {
            StringBuilder strSql = new StringBuilder();
            ...
            strSql.Append("select top " + limit + "  * from S_Modules where ID not in  ");
            strSql.Append("(select top " + (limit * (page - 1)) + " ID from S_Modules where 1=1 and " + where + " order by " + Sort + "  ) and " + where + " order by " + Sort + " ");
            ZhiFang.Common.Log.Log.Debug("ZhiFang.DAL.MsSql.Weblis.S_Modules.GetListByPage.sql:" + strSql.ToString());
        ...
        public DataSet GetListByRBACModulesList(List<string> rbac_moduleslist)
        {
            ...
            ZhiFang.Common.Log.Log.Info("rbac_moduleslist:" + rbac_moduleslist.Count);
            StringBuilder strSql = new StringBuilder();
            ...
            ZhiFang.Common.Log.Log.Info("GetListByRBACModulesList:" + strSql.ToString());
            try
            {
            ...
            {
                ZhiFang.Common.Log.Log.Error("ZhiFang.DAL.MsSql.Weblis.S_Modules.ConnectionString：" + idb.ConnectionString + "---sql：" + strSql.ToString() + "---异常信息：" + e.ToString());
                return null;
            }
        }

        public DataSet GetListByRBACModulesListAll()
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Insert(0, " select * from S_Modules where 1=1 ");
            strSql.Append(" order by SN ");
            ZhiFang.Common.Log.Log.Info("GetListByRBACModulesListAll:" + strSql.ToString());
            try
            {
                return idb.ExecuteDataSet(strSql.ToString());
            }
            ...
            {
                ZhiFang.Common.Log.Log.Error("ZhiFang.DAL.MsSql.Weblis.S_Modules.GetListByRBACModulesListAll.ConnectionString：" + idb.ConnectionString + "---sql：" + strSql.ToString() + "---异常信息：" + e.ToString());
                return null;
            }
        }
        ...
        public int GetRecordCount(string wherestr)
        {
            ...
            strSql.Append("select count(*) FROM S_Modules where  "+ wherestr);
            string strCount = idb.ExecuteScalar(strSql.ToString());
            if (strCount != null && strCount.Trim() != "")
            {
                return Convert.ToInt32(strCount.Trim());
