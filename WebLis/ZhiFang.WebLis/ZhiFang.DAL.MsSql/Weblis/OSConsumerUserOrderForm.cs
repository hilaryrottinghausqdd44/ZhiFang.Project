    ...
    public class OSConsumerUserOrderForm: BaseDALLisDB,IDOSConsumerUserOrderForm
    {
        ...
        public bool SaveOSUserConsumerForm(long nRequestFormNo, DataRow DrOSUserOrderForm, NrequestCombiItemBarCodeEntity jsonentity)
        {
            try
            {
                ...
                osucf.EmpAccount = jsonentity.NrequestForm.CollecterName;
                osucf.ConsumerAreaID = jsonentity.NrequestForm.AreaID;
		...
		public bool AddOSUserConsumerForm(Model.OSUserConsumerForm model)
        {
            ...
            if (model.EmpAccount != null)
            {
                strSql1.Append("EmpAccount,");
                strSql2.Append("'" + model.EmpAccount + "',");
            }
            if (model.ConsumerAreaID != null)
            {
                strSql1.Append("ConsumerAreaID,");
                strSql2.Append("" + model.ConsumerAreaID + ",");
            }
            if (model.Memo != null)
            {
                strSql1.Append("Memo,");
        ...
        public bool UpdateOSUserConsumerForm(Model.OSUserConsumerForm model)
        {
            ...
            if (model.EmpAccount != null)
            {
                strSql.Append("EmpAccount='" + model.EmpAccount + "',");
            }
            else
            {
                strSql.Append("EmpAccount= null ,");
            }
            if (model.ConsumerAreaID != null)
            {
                strSql.Append("ConsumerAreaID=" + model.ConsumerAreaID + ",");
            }
            else
            {
                strSql.Append("ConsumerAreaID= null ,");
            }
            if (model.Memo != null)
            {
        ...
        public Model.OSUserOrderForm DataRowToModelOSUserOrderForm(DataRow row)
        {
            ...
            if (row != null)
            {
                ...
                {
                    model.OrgID = long.Parse(row["OrgID"].ToString());
                }
                if (row["WeblisSourceOrgID"] != null && row["WeblisSourceOrgID"].ToString() != "")
                {
                    model.WeblisSourceOrgID = row["WeblisSourceOrgID"].ToString();
                }
                if (row["WeblisSourceOrgName"] != null && row["WeblisSourceOrgName"].ToString() != "")
                {
                    model.WeblisSourceOrgName = row["WeblisSourceOrgName"].ToString();
                }
                if (row["EmpID"] != null && row["EmpID"].ToString() != "")
                {
                    model.EmpID = long.Parse(row["EmpID"].ToString());
                }
                if (row["EmpAccount"] != null && row["EmpAccount"].ToString() != "")
                {
                    model.EmpAccount =row["EmpAccount"].ToString();
                }
                if (row["ConsumerAreaID"] != null && row["ConsumerAreaID"].ToString() != "")
                {
                    model.ConsumerAreaID = long.Parse(row["ConsumerAreaID"].ToString());
                }
                if (row["PayCode"] != null)
                {
        ...
        public int LockOSUserOrderFormByPayCode(string PayCode,string EmpAccount,string ZhiFangUserID,string WeblisSourceOrgID,string WeblisSourceOrgName, string ConsumerAreaID)
        {
            if (PayCode.ToLower().IndexOf("del") < 0 && PayCode.ToLower().IndexOf("drop") < 0 && PayCode.ToLower().IndexOf("update") < 0)
            {
                return DbHelperSQL.ExecuteNonQuery(" update OS_UserOrderForm set Status=" + ZhiFang.Model.WeiXinDic.UserOrderFormStatus.使用中.Key + ",ConsumerStartTime='" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss:fff") + "',WeblisSourceOrgID='" + WeblisSourceOrgID + "',WeblisSourceOrgName='" + WeblisSourceOrgName + "',EmpID=" + ZhiFangUserID + ",EmpAccount='" + EmpAccount + "',ConsumerAreaID=" + ConsumerAreaID + "  where payCode='" + PayCode + "' and Status=" + ZhiFang.Model.WeiXinDic.UserOrderFormStatus.已交费.Key + " and AreaID= "+ ConsumerAreaID);
            }
            return 0;        
        }
        ...
        {
            return DbHelperSQL.ExecuteNonQuery(" update OS_UserOrderForm set Status=" + ZhiFang.Model.WeiXinDic.UserOrderFormStatus.已交费.Key + ",ConsumerStartTime=null  where payCode='" + PayCode + "' and Status=" + ZhiFang.Model.WeiXinDic.UserOrderFormStatus.使用中.Key + " and WeblisSourceOrgID=" + WeblisSourceOrgID + " and WeblisSourceOrgName='" + WeblisSourceOrgName + "' and EmpID=" + ZhiFangUserID + " and EmpAccount='" + EmpAccount + "'");
        }

        public List<OSUserOrderForm> SearchUnConsumerUserOrderFormList(string PayCode, string EmpAccount, string ZhiFangUserID, string WeblisSourceOrgID, string WeblisSourceOrgName)
        {
            List<OSUserOrderForm> list = new List<OSUserOrderForm>();
            DataSet dsuof = new DataSet();
            if (PayCode.ToLower().IndexOf("del") < 0 && PayCode.ToLower().IndexOf("drop") < 0 && PayCode.ToLower().IndexOf("update") < 0)
            {
                if (PayCode != null && PayCode.Trim() != "")
                {
                    dsuof = DbHelperSQL.ExecuteDataSet(" select AreaID, HospitalID,PayCode, UOFID, UOFCode, DOFID, DoctorAccountID, OS_UserConsumerFormID,                 OS_UserConsumerFormCode, WeiXinUserID, DoctorOpenID, DoctorName, UserAccountID, UserWeiXinUserID,                UserName, UserOpenID, Status, Memo, DispOrder, IsUse, DataAddTime, DataUpdateTime, MarketPrice, GreatMasterPrice, DiscountPrice, Discount, Price, AdvicePrice, RefundPrice, PayTime, CancelApplyTime,                CancelFinishedTime, ConsumerStartTime, ConsumerFinishedTime,   OrgID, WeblisSourceOrgID, WeblisSourceOrgName, EmpID, EmpAccount, ConsumerAreaID from OS_UserOrderForm where payCode='" + PayCode + "' and Status=" + ZhiFang.Model.WeiXinDic.UserOrderFormStatus.使用中.Key + " and WeblisSourceOrgID=" + WeblisSourceOrgID + " and WeblisSourceOrgName='" + WeblisSourceOrgName + "' and EmpID=" + ZhiFangUserID + " and EmpAccount='" + EmpAccount + "'");
                }
                else
                {
                    dsuof = DbHelperSQL.ExecuteDataSet(" select AreaID, HospitalID,PayCode, UOFID, UOFCode, DOFID, DoctorAccountID, OS_UserConsumerFormID,                 OS_UserConsumerFormCode, WeiXinUserID, DoctorOpenID, DoctorName, UserAccountID, UserWeiXinUserID,                UserName, UserOpenID, Status, Memo, DispOrder, IsUse, DataAddTime, DataUpdateTime, MarketPrice, GreatMasterPrice, DiscountPrice, Discount, Price, AdvicePrice, RefundPrice, PayTime, CancelApplyTime,                CancelFinishedTime, ConsumerStartTime, ConsumerFinishedTime,   OrgID, WeblisSourceOrgID, WeblisSourceOrgName, EmpID, EmpAccount, ConsumerAreaID from OS_UserOrderForm where Status=" + ZhiFang.Model.WeiXinDic.UserOrderFormStatus.使用中.Key + " and WeblisSourceOrgID=" + WeblisSourceOrgID + " and WeblisSourceOrgName='" + WeblisSourceOrgName + "' and EmpID=" + ZhiFangUserID + " and EmpAccount='" + EmpAccount + "'");
                }
            }
            if (dsuof != null && dsuof.Tables.Count>0 && dsuof.Tables[0].Rows.Count > 0)
            {
                for (int i = 0; i < dsuof.Tables[0].Rows.Count; i++)
                {
                    OSUserOrderForm tmp = DataRowToModelOSUserOrderForm(dsuof.Tables[0].Rows[i]);
                    list.Add(tmp);
                }
            }
            return list;
