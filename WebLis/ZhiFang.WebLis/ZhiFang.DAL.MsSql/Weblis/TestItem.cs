...
namespace ZhiFang.DAL.MsSql.Weblis
{
    ...
    public partial class TestItem : BaseDALLisDB, IDTestItem, IDBatchCopy, IDGetListByTimeStampe
    {
        ...
        public int Add(ZhiFang.Model.TestItem model)
        {
            ...
            strSql.Append("ItemNo,CName,EName,ShortName,ShortCode,DiagMethod,Unit,IsCalc,Visible,DispOrder,Prec,IsProfile,OrderNo,StandardCode,ItemDesc,FWorkLoad,Secretgrade,Cuegrade,IsDoctorItem,IschargeItem,price,lowprice,isCombiItem,Color,SuperGroupNo");
            ...
            SqlParameter[] parameters = {
                        ...
                        new SqlParameter("@price", SqlDbType.Float,8) ,
                         new SqlParameter("@lowprice", SqlDbType.Float,8) ,
            ...
            parameters[21].Value = model.LowPrice;
            parameters[22].Value = model.IsCombiItem;
            parameters[23].Value = model.Color;
            parameters[24].Value = model.SuperGroupNo;
        ...
        public ZhiFang.Model.TestItem GetModel(string ItemNo)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select ItemNo, CName, EName, ShortName, ShortCode, DiagMethod, Unit, IsCalc, Visible, DispOrder, Prec, IsProfile, OrderNo, StandardCode, ItemDesc, FWorkLoad, Secretgrade, Cuegrade, IsDoctorItem, IschargeItem, price, LowPrice,isCombiItem, Color ,SuperGroupNo  ");
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                if (ds.Tables[0].Rows[0]["lowprice"].ToString() != "")
                {
                    model.LowPrice = decimal.Parse(ds.Tables[0].Rows[0]["lowprice"].ToString());
                }
                    ...
                    if (lst[0].Trim() == "CopyToLab_LabFirstSelect")
                    {
                        ...
                        for (int i = 1; i < lst.Count; i++)
                        {
                            ...
                            for (int j = 0; j < arryItemno.Length; j++)
                            {
                                ...
                                {
                                    strSql.Append("insert into " + LabTableName + "( LabCode,");
                                    strSql.Append(" Visible , DispOrder , Prec , IsProfile ,OrderNo ,StandardCode ,ItemDesc ,FWorkLoad ,Secretgrade ,Cuegrade ,ItemNo ,IsDoctorItem ,IschargeItem,Price,LowPrice,IsCombiItem ,CName ,EName , ShortName , ShortCode , DiagMethod , Unit , IsCalc,Color,UseFlag,AddTime ");
                                    strSql.Append(") select '" + lst[i].Trim().Split('|')[0].Trim() + "' as LabCode, ");
                                    //strSql.Append("Visible,DispOrder,Prec,IsProfile,OrderNo,StandardCode,ItemDesc,FWorkLoad,Secretgrade,Cuegrade,ItemNo,IsDoctorItem,IschargeItem,Price,LowPrice,IsCombiItem,CName,EName,ShortName,ShortCode,DiagMethod,Unit,IsCalc,(select top 1 ColorName from ItemColorDict where TestItem.Color=ItemColorDict.ColorID ) as Color ,1 as UseFlag,GETDATE() as AddTime");
                                    strSql.Append("Visible,DispOrder,Prec,IsProfile,OrderNo,StandardCode,ItemDesc,FWorkLoad,Secretgrade,Cuegrade,ItemNo,IsDoctorItem,IschargeItem,Price,LowPrice,IsCombiItem,CName,EName,ShortName,ShortCode,DiagMethod,Unit,IsCalc,TestItem.Color as Color ,1 as UseFlag,GETDATE() as AddTime");
                                    strSql.Append(" from TestItem where ItemNo in (" + strItemNos + ") ");
                                    strSql.Append(" and IsProfile is not null ");
                                    //arrySql.Add(strSql.ToString());
                    ...
                    else
                    {
                        ...
                        for (int i = 0; i < lst.Count; i++)
                        {
                            ...
                            for (int j = 0; j < arryLabNo.Length; j++)
                            {
                                ...
                                strSql.Append("insert into " + LabTableName + "( LabCode,");
                                strSql.Append("Visible , DispOrder , Prec , IsProfile ,OrderNo ,StandardCode ,ItemDesc ,FWorkLoad ,Secretgrade ,Cuegrade ,ItemNo ,IsDoctorItem ,IschargeItem,Price,LowPrice,IsCombiItem ,CName ,EName , ShortName , ShortCode , DiagMethod , Unit , IsCalc,Color ");
        ...
        public DataSet GetItemListByColor(string colorvalue,string where , string orderby, int startIndex, int endIndex)
        {
            
            string strWhere = (!string.IsNullOrEmpty(colorvalue)) ? (colorvalue.Trim().ToLower()=="all") ? " 1=1  " : (colorvalue.Trim().ToLower() == "allset") ? "  Color is not null  " : " Color='" + colorvalue + "' " : " Color is null";
            strWhere+= (!string.IsNullOrEmpty(where)) ? " and " + where + " " : " ";
            StringBuilder strSql = new StringBuilder();
            ...
            {
                strSql.Append("order by T.ItemNo ");
            }
            strSql.Append(")AS Row, T.*  from testitem T ");
        ...
        public int GetItemListByColorCount(string colorvalue, string where)
        {
            StringBuilder strSql = new StringBuilder();
            //string strWhere = (!string.IsNullOrEmpty(colorvalue)) ? " Color='" + colorvalue + "' " : " Color is null";
            //strWhere += (!string.IsNullOrEmpty(where)) ? " and " + where + " " : " ";
            ...
            return GetItemListByColorCount(strWhere);
        }
        public int GetItemListByColorCount( string where)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(*) FROM testitem ");
        ...
        public int SetItemColorByItemNoList(string colorvalue, List<string> itemnolist)
        {
            if (colorvalue.ToLower().Trim() != "null")
            {
                colorvalue = "'" + colorvalue + "'";
            }
            string sql = " update testitem set Color=" + colorvalue + " where itemno in (" + string.Join(",", itemnolist) + ") ";
            ZhiFang.Common.Log.Log.Debug("TestItem.SetItemColorByItemNoList.sql:" + sql);
