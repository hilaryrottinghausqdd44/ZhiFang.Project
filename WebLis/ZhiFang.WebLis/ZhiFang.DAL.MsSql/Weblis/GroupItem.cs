...
namespace ZhiFang.DAL.MsSql.Weblis
{
    ...
    public partial class GroupItem : IDGroupItem, IDBatchCopy, IDGetListByTimeStampe
    {
        ...
        public int Add(ZhiFang.Model.GroupItem model)
        {
            ...
            model.Id = ZhiFang.Tools.GUIDHelp.GetGUIDLong();
            if (model.Id>0 )
            {
                strSql1.Append("GroupItemID,");
        ...
        /// 根据实体获取DataSet
        /// </summary>
        public DataSet GetPitemList(ZhiFang.Model.GroupItem model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * ");
            strSql.Append(" FROM GroupItem  INNER JOIN   dbo.TestItem ON dbo.GroupItem.PItemNo =  dbo.TestItem.ItemNo   where 1=1 ");
