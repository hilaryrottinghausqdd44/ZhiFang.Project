...
namespace ZhiFang.DAL.MsSql.Weblis
{
    ...
    public partial class ReportMicroFull : BaseDALLisDB, IDReportMicroFull
    {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top 1 * from ReportMicroFull ");
            return DbHelperSQL.ExecuteDataSet(strSql.ToString());
        }
        ...
        */
        ...
        public DataSet GetReportMicroGroupList(string ReportFormID)
        {
            try
            {
                #region 执行存储过程
                //string[] p = ReportFormID.Split(';');
                if (ReportFormID.Length > 0)
                {
                    SqlParameter sp = new SqlParameter("@ReportFormID", SqlDbType.VarChar, 100);
                    sp.Value = ReportFormID;

                    DataSet ds = DbHelperSQL.ExecDataSetStoredProcedure("GetReportMicroGroupFullList", new SqlParameter[] { sp });
                    ZhiFang.Common.Log.Log.Info(ds.GetXml() + "@");
        ...
        public int BackUpReportMicroFullByWhere(string Strwhere)
        {
            ...
            strb.Append("  insert into dbo.ReportMicroFull_BackUp ");
            strb.Append("([ReportFormID]");
            strb.Append("  ,[ReportItemID]");
            strb.Append(" ,[ResultNo]");
            strb.Append(" ,[ItemNo]");
            strb.Append("  ,[ItemName]");
            strb.Append("  ,[DescNo]");
            strb.Append(" ,[DescName]");
            strb.Append("  ,[MicroNo]");
            strb.Append("  ,[MicroDesc]");
            strb.Append("  ,[MicroName]");
            strb.Append("  ,[AntiNo]");
            strb.Append(" ,[AntiName]");
            strb.Append("  ,[Suscept]");
            strb.Append("  ,[SusQuan]");
            strb.Append(" ,[RefRange]");
            strb.Append("   ,[SusDesc]");
            strb.Append("   ,[AntiUnit]");
            strb.Append("  ,[ItemDate]");
            strb.Append("  ,[ItemTime]");
            strb.Append("  ,[ItemDesc]");
            strb.Append("  ,[EquipNo]");
            ...
            strb.Append("  ,[FormNo]");
            strb.Append("   ,[isReceive]");
            strb.Append("   ,[microCountDesc]");
            strb.Append("  ,[mresulttType]");
            strb.Append(" ,[itemename]");
            strb.Append("  ,[Microename]");
            strb.Append("  ,[Antiename]");
            strb.Append("  ,[Antishortname]");
            strb.Append("  ,[antishortcode]");
            strb.Append("  ,[Barcode]");
            ...
            strb.Append(" ,[ReportMicroIndexID]");
            ...
            strb.Append(" from ReportMicroFull   where ");
