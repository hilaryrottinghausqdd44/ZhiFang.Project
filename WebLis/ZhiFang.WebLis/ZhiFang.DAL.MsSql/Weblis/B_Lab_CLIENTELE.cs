...
namespace ZhiFang.DAL.MsSql.Weblis  
{
	...
	public partial class B_Lab_CLIENTELE: BaseDALLisDB, IDLab_CLIENTELE	{	
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  *,'('+convert(varchar(100),ClIENTID)+')'+CName as ClIENTIDAndName  ");
			strSql.Append(" FROM B_Lab_CLIENTELE where 1=1 ");
			if (model.LabCode != null)
            {
                strSql.Append(" and LabCode='" + model.LabCode + "' ");
        ...
        public DataSet GetListByPage(ZhiFang.Model.Lab_CLIENTELE model, int nowPageNum, int nowPageSize)
        {
            ...
            if(model.LabClIENTNO !=0)
                        ...
                        strSql.Append(" and ZFStandCode='"+model.ZFStandCode+"' ");
                                                ...
                                                strSql.Append("and ClIENTID not in ");
            ...
            if(model.LabClIENTNO !=0)
                        ...
                        strSql.Append(" and ZFStandCode='"+model.ZFStandCode+"' ");
                                                ...
                                                strSql.Append(" order by ClIENTID desc) order by ClIENTID  desc ");
            return DbHelperSQL.ExecuteDataSet(strSql.ToString());
        } 
         ...
         public int AddByDataRow(DataRow dr)
        {
            ...
            {
                ZhiFang.Common.Log.Log.Error("ZhiFang.DAL.MsSql.Weblis.B_Lab_CLIENTELE.AddByDataRow 同步数据时异常：", ex);
                return 0;
            }          
        }
        public int UpdateByDataRow(DataRow dr)
        {
            ...
            {
                ZhiFang.Common.Log.Log.Error("ZhiFang.DAL.MsSql.Weblis.B_Lab_CLIENTELE .UpdateByDataRow同步数据时异常：", ex);
                return 0;
            }
        }
