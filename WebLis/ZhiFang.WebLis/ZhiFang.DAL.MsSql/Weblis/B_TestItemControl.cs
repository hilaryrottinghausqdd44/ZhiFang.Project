    ...
    public partial class B_TestItemControl : BaseDALLisDB, IDTestItemControl, IDBatchCopy
    {
        ...
        public int GetTotalCount(ZhiFang.Model.TestItemControl model)
        {
            try
            {
                ...
                Common.Log.Log.Info(strSql.ToString()+"@@@"+DbHelperSQL.ConnectionString);
                string strCount = DbHelperSQL.ExecuteScalar(strSql.ToString());
                Common.Log.Log.Info("strCount:"+ strCount);
        ...
        public bool CheckIncludeLabCode(List<string> l, string LabCode)
        {
            ...
            try
            {
                ...
                string strSql = "select * from B_TestItemControl where ControlLabNo='" + LabCode.Trim() + "' and ControlItemNo in (" + strItemNos + ")";
                try
                {
            ...
            {
                ZhiFang.Common.Log.Log.Error("ZhiFang.DAL.MsSql.Weblis.B_TestItemControl.CheckIncludeLabCode异常->", ex);
            }
            return b;
        }
        ...
        public bool CheckIncludeCenterCode(List<string> l, string LabCode)
        {
            ...
            try
            {
                ...
                string strSql = "select * from B_TestItemControl where ControlLabNo='" + LabCode.Trim() + "' and ItemNo in (" + strItemNos + ")";
                ZhiFang.Common.Log.Log.Error("ZhiFang.DAL.MsSql.Weblis.B_TestItemControl.CheckIncludeCenterCode.sql"+strSql);
                DataSet ds = DbHelperSQL.ExecuteDataSet(strSql);
                DbHelperSQL.Dispose();
                if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                {
            ...
            {
                ZhiFang.Common.Log.Log.Error("ZhiFang.DAL.MsSql.Weblis.B_TestItemControl.CheckIncludeCenterCode异常->", ex);
            }
            return b;
        }
        ...
        public DataSet GetLabCodeNo(string LabCode, List<string> CenterNoList)
        {
            ...
            try
            {
                if (CenterNoList == null || CenterNoList.Count <= 0)
                {
                    ZhiFang.Common.Log.Log.Error("ZhiFang.DAL.MsSql.Weblis.B_TestItemControl.GetLabCodeNo异常->CenterNoList.参数为空！");
                    return ds;
                }
                string strSql = "select dbo.B_Lab_TestItem.CName, dbo.B_TestItemControl.ControlLabNo, dbo.B_TestItemControl.ControlItemNo, dbo.B_Lab_TestItem.Color, dbo.B_TestItemControl.ItemNo FROM dbo.B_Lab_TestItem INNER JOIN dbo.B_TestItemControl ON dbo.B_Lab_TestItem.ItemNo = dbo.B_TestItemControl.ControlItemNo AND dbo.B_Lab_TestItem.LabCode = dbo.B_TestItemControl.ControlLabNo where B_TestItemControl.ControlLabNo='" + LabCode.Trim() + "' and dbo.B_TestItemControl.ItemNo in ('" + string.Join("','", CenterNoList) + "')";
            ...
            {
                ZhiFang.Common.Log.Log.Error("ZhiFang.DAL.MsSql.Weblis.B_TestItemControl.GetLabCodeNo异常->", ex);
            }
            return ds;
        }
        ...
        public DataSet GetCenterNo(string LabCode, List<string> LabPrimaryNoList)
        {
            ...
            {
                ZhiFang.Common.Log.Log.Error("ZhiFang.DAL.MsSql.Weblis.B_TestItemControl.GetCenterNo异常->", ex);
            }
            return ds;
        }
        ...
        public DataSet GetLabItemCodeMapListByNRequestLabCodeAndFormNo(string LabCode, string NRequestFormNo)
        {
            ...
            try
            {
                string strSql = "SELECT   dbo.B_Lab_TestItem.CName, dbo.B_TestItemControl.ControlLabNo, dbo.B_TestItemControl.ControlItemNo,dbo.B_Lab_TestItem.Color, dbo.B_TestItemControl.ItemNo, dbo.B_Lab_TestItem.price,dbo.ItemColorDict.ColorValue FROM   dbo.B_Lab_TestItem INNER JOIN dbo.B_TestItemControl ON dbo.B_Lab_TestItem.ItemNo = dbo.B_TestItemControl.ControlItemNo AND dbo.B_Lab_TestItem.LabCode = dbo.B_TestItemControl.ControlLabNo INNER JOIN dbo.ItemColorDict ON dbo.B_Lab_TestItem.Color = dbo.ItemColorDict.ColorName";
        ...
        //    }
        //    strSql.Append(" select top " + nowPageSize + "  * from B_Lab_TestItem left join B_TestItemControl on TestItem.ItemID=B_TestItemControl.ItemNo ");
        //    if ((nowPageSize + nowPageNum) != 0)
        //    {
        //        strSql.Append("where B_Lab_TestItem.ItemID not in ( ");
        //        strSql.Append("select top " + (nowPageSize * nowPageNum) + " B_Lab_TestItem.ItemID from  TestItem left join B_TestItemControl on B_Lab_TestItem.ItemID=B_TestItemControl.ItemNo ");
        //        if (model.LabCode != null)
        //        {
