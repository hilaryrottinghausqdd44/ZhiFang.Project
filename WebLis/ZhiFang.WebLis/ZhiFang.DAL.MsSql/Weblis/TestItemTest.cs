...
{
    public partial class TestItemTest : BaseDALLisDB, IDTestItemTest, IDBatchCopy, IDGetListByTimeStampe
    {

        public TestItemTest(string dbsourceconn)
		{
        ...
        public ZhiFang.Model.TestItemTest GetModel(string ItemNo)
        {
            ...
            ZhiFang.Model.TestItemTest model = new ZhiFang.Model.TestItemTest();
        ...
        public DataSet GetListLike(ZhiFang.Model.TestItemTest model)
        {
            StringBuilder strSql = new StringBuilder();
        ...
        public int GetTotalCount(ZhiFang.Model.TestItemTest model)
        {
            StringBuilder strSql = new StringBuilder();
            ...
            //        strSql.Append("select count(*) FROM TestItem where 1=1  and ( IschargeItem=1) and (isCombiItem=0) " + likesql); break;
        ...
        public DataSet GetListByPage(ZhiFang.Model.TestItemTest model, int nowPageNum, int nowPageSize)
        {
            ...
            if (model != null && model.LabCode != null)
            {
                ...
                strSql.Append(" where 1=1 " + strWhere.ToString() + " order by " + strOrderBy + " ) " + strWhere.ToString() + " order by " + strOrderBy + " ");
                return DbHelperSQL.ExecuteDataSet(strSql.ToString());
            }
            else
            {
                ...
                switch (model.TestItemSuperGroupClass)
                {
                    case Common.Dictionary.TestItemSuperGroupClass.ALL:
                        strSql.Append("select top " + nowPageSize + "  * from TestItem where ItemNo not in  ");
                        strSql.Append("(select top " + (nowPageSize * nowPageNum) + " ItemNo from TestItem where 1=1 " + likesql + " order by ItemNo) ");
                    ...
                    case Common.Dictionary.TestItemSuperGroupClass.COMBI:
                        strSql.Append("select top " + nowPageSize + "  * from TestItem where ItemNo not in  ");
                        strSql.Append("(select top " + (nowPageSize * nowPageNum) + " ItemNo from TestItem where 1=1  and (isCombiItem=1)  " + likesql + " order by ItemNo) ");
                        strSql.Append(" and (isCombiItem=1) ");
                        strSql.Append(likesql);
                        strSql.Append(" order by ItemNo  "); break;
                    case Common.Dictionary.TestItemSuperGroupClass.CHARGE:
                        strSql.Append("select top " + nowPageSize + "  * from TestItem where ItemNo not in  ");
                        strSql.Append("(select top " + (nowPageSize * nowPageNum) + " ItemNo from TestItem where 1=1  and ( IschargeItem=1) and (isCombiItem=0)  " + likesql + " order by ItemNo) ");
                    ...
                    case Common.Dictionary.TestItemSuperGroupClass.SUPERGROUP:
                        ...
                        strSql.Append("(select top " + (nowPageSize * nowPageNum) + " ItemNo from dbo.TestItem INNER JOIN    dbo.SuperGroup ON dbo.TestItem.SuperGroupNo = dbo.SuperGroup.SuperGroupNo where 1=1 and  SuperGroup.SuperGroupNo =" + model.SuperGroupNo + "  " + likesql + " order by TestItem.ItemNo) ");
                    ...
                    default: strSql.Append("select top " + nowPageSize + "  * from TestItem where 1=1 " + strWhere.ToString() + " and ItemNo not in  ");
                        strSql.Append("(select top " + (nowPageSize * nowPageNum) + " ItemNo from TestItem where 1=1 " + likesql + " order by " + model.OrderField + ") ");
                        strSql.Append(likesql);
        ...
        public string GetCombiItems(string strLabCode, string strCopyList)
        {
            ...
            try
            {
                ...
                {
                    Model.TestItemTest model = new Model.TestItemTest();
        ...
        public DataSet GetList(int Top, ZhiFang.Model.TestItemTest model, string filedOrder)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select ");
            if (Top > 0)
