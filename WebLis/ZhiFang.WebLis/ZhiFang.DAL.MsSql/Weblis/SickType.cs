...
namespace ZhiFang.DAL.MsSql.Weblis
{
    ...
    public partial class SickType : BaseDALLisDB, IDSickType, IDBatchCopy, IDGetListByTimeStampe
    {
        ...
        public int Add(List<ZhiFang.Model.SickType> modelList)
        {
            string strsqls = "";
        ...
        public int Update(List<ZhiFang.Model.SickType> modelList)
        {
            string strsqls = "";
            foreach (var model in modelList)
            {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select *,SickTypeNo as LabNo,convert(varchar(100),SickTypeNo)+'_'+CName as LabNo_Value,CName+'('+convert(varchar(100),SickTypeNo)+')' as LabNoAndName_Text ");
            strSql.Append(" FROM SickType  ");
            if (model.CName != null)
            {
        ...
        public DataSet GetListByPage(ZhiFang.Model.SickType model, int nowPageNum, int nowPageSize)
        {
            ...
            if (model != null && model.LabCode != null)
            {
                ...
                strSql.Append(" select top " + nowPageSize + "  * from SickType left join B_SickTypeControl on SickType.SickTypeNo=B_SickTypeControl.SickTypeNo ");
                if (model.LabCode != null)
                {
                    strSql.Append(" and B_SickTypeControl.ControlLabNo='" + model.LabCode + "' ");
                }
                strSql.Append("where SickType.SickTypeNo not in ( ");
                strSql.Append("select top " + (nowPageSize * nowPageNum) + " SickType.SickTypeNo from  SickType left join B_SickTypeControl on SickType.SickTypeNo=B_SickTypeControl.SickTypeNo ");
                if (model.LabCode != null)
                {
                    strSql.Append(" and B_SickTypeControl.ControlLabNo='" + model.LabCode + "' ");
                }
            ...
            else
            {
                ...
                strSql.Append("select top " + nowPageSize + "  * from SickType where SickTypeNo not in  ");
                strSql.Append("(select top " + (nowPageSize * nowPageNum) + " SickTypeNo from SickType where 1=1 " + strLike + " order by " + model.OrderField + " desc ) " + strLike + " order by " + model.OrderField + " desc ");
                return DbHelperSQL.ExecuteDataSet(strSql.ToString());
            }
        }
        ...
        public int AddByDataRow(DataRow dr)
        {
            ...
            {
                ZhiFang.Common.Log.Log.Error("ZhiFang.DAL.MsSql.Weblis.SickType.AddByDataRow 同步数据时异常：", ex);
                return 0;
            }
        }
        public int UpdateByDataRow(DataRow dr)
        {
            ...
            {
                ZhiFang.Common.Log.Log.Error("ZhiFang.DAL.MsSql.Weblis.SickType .UpdateByDataRow同步数据时异常：", ex);
                return 0;
            }
        }
        ...
        public DataSet GetListByTimeStampe(string LabCode, int dTimeStampe)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select *,'" + LabCode + "' as LabCode,SickTypeNo as LabSickTypeNo from SickType where 1=1 ");
