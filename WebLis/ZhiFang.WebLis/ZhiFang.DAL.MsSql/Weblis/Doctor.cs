...
namespace ZhiFang.DAL.MsSql.Weblis
{
    ...
    public class Doctor : BaseDALLisDB, IDDoctor, IDBatchCopy
    {
        ...
        public int Add(List<ZhiFang.Model.Doctor> modelList)
        {
            string strsqls = "";
        ...
        public int Update(List<ZhiFang.Model.Doctor> modelList)
        {
            string strsqls = "";
            foreach (var model in modelList)
            {
        ...
        */
        ...
        public DataSet GetListByPage(ZhiFang.Model.Doctor model, int nowPageNum, int nowPageSize)
        {
            ...
            if (model != null && model.LabCode != null)
            {
                ...
                strSql.Append(" select top " + nowPageSize + "  * from Doctor left join B_DoctorControl on Doctor.DoctorNo=B_DoctorControl.DoctorNo ");
                if (model.LabCode != null)
                {
                    strSql.Append(" and B_DoctorControl.ControlLabNo='" + model.LabCode + "' ");
                }
                strSql.Append("where Doctor.DoctorNo not in ( ");
                strSql.Append("select top " + (nowPageSize * nowPageNum) + "Doctor.DoctorNo from  Doctor left join B_DoctorControl on Doctor.DoctorNo=B_DoctorControl.DoctorNo");
                if (model.LabCode != null)
                {
                    strSql.Append(" and B_DoctorControl.ControlLabNo='" + model.LabCode + "' ");
                }
            ...
            else
            {
                ...
                strSql.Append("select top " + nowPageSize + "  * from Doctor where DoctorNo not in  ");
                strSql.Append("(select top " + (nowPageSize * nowPageNum) + " DoctorNo from Doctor where 1=1 " + strLike + " order by " + model.OrderField + " desc ) " + strLike + " order by " + model.OrderField + " desc ");
                return DbHelperSQL.ExecuteDataSet(strSql.ToString());
            }
        ...
        public bool CopyToLab(List<string> lst)
        {
            ...
            try
            {
                for (int i = 0; i < lst.Count; i++)
                {
                    ...
                    strSqlControl.Append("  '" + lst[i].Trim() + "'+'_'+convert(varchar(50),DoctorNo)+'_'+convert(varchar(50), DoctorNo) as DoctorControlNo,DoctorNo,'" + lst[i].Trim() + "' as ControlLabNo,DoctorNo,1 ");
        ...
        public int DeleteByDataRow(DataRow dr)
        {
            ...
            {
                ZhiFang.Common.Log.Log.Error("ZhiFang.DAL.MsSql.weblis.Doctor .DeleteByDataRow同步数据时异常：", ex);
                return 0;
            }
        }
