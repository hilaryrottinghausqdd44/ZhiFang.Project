	...
	{	
        ...
        public DataSet GetListByPage(ZhiFang.Model.FolkType model, int nowPageNum, int nowPageSize)
        {
            ...
            if (model != null && model.LabCode != null)
            {
                ...
                strSql.Append(" select top " + nowPageSize + "  * from FolkType left join B_FolkTypeControl on FolkType.FolkNo=B_FolkTypeControl.FolkNo ");
                if (model.LabCode != null)
                {
                    strSql.Append(" and B_FolkTypeControl.ControlLabNo='" + model.LabCode + "' ");
                }
                strSql.Append("where FolkType.FolkNo not in ( ");
                strSql.Append("select top " + (nowPageSize * nowPageNum) + " FolkType.FolkNo from  FolkType left join B_FolkTypeControl on FolkType.FolkNo=B_FolkTypeControl.FolkNo ");
                if (model.LabCode != null)
                {
                    strSql.Append(" and B_FolkTypeControl.ControlLabNo='" + model.LabCode + "' ");
                }
            ...
            else
            {
                ...
                strSql.Append("select top " + nowPageSize + "  * from FolkType where FolkNo not in  ");
                strSql.Append("(select top " + (nowPageSize * nowPageNum) + " FolkNo from FolkType where 1=1 " + strLike + " order by " + model.OrderField + " desc ) " + strLike + " order by " + model.OrderField + " desc ");
                return DbHelperSQL.ExecuteDataSet(strSql.ToString());
            }
        }
        ...
        public int DeleteByDataRow(DataRow dr)
        {
            ...
            {
                ZhiFang.Common.Log.Log.Error("ZhiFang.DAL.MsSql.weblis.FolkType .DeleteByDataRow同步数据时异常：", ex);
                return 0;
            }
        }
        ...
        public DataSet GetListByTimeStampe(string LabCode, int dTimeStampe)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select *,'" + LabCode + "' as LabCode,FolkNo as LabFolkNo from FolkType where 1=1 ");
