<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="SubscripList_dajia.aspx.cs" Inherits="ZhiFang.WebLis.ApplyInput.SubscripList_dajia" %>
<html>
<head id="Head1" runat="server">
    ...
    <script type="text/javascript">
        function OpenInsertPage(flag) {
            //flag == '0' 表示录入新病历单
            //flag == '1' 表示修改原病历单
            var url = "ApplyInput_Weblis_dajia.aspx?Flag=0&BarCodeInputFlag=" + getQueryString("BarCodeInputFlag");
        ...
        function ModifyTest(NRequestFormNo) {
            var url = "ApplyInput_Weblis_dajia.aspx?Flag=1&NRequestFormNo=" + NRequestFormNo + "&BarCodeInputFlag=" + getQueryString("BarCodeInputFlag");
            window.open(url, "申请单维护", "width=" + Math.floor(window.screen.width * 0.9) + ",height=" + Math.floor(window.screen.height * 0.8) + ",toolbar=no,menubar=no,scrollbars=yes,resizable=yes,location=no,status=no,top=" + Math.floor(window.screen.height * 0.1) + ",left=" + Math.floor(window.screen.width * 0.05));
        }
        function DeleteNRequestForm(NRequestFormNo) {
            event.returnValue = confirm("删除是不可恢复的，你确认要删除吗？");
            if (event.returnValue) {
                var i = ZhiFang.WebLis.ApplyInput.SubscripList_dajia.DeletRequest(NRequestFormNo);
                if (i = true) {
                    alert("删除成功");
    ...
    <script type="text/javascript">
        ...
        function CheckBarcodePrintFlag() {
            if (getQueryString('BarCodeInputFlag') == '1' || getQueryString('BarCodeInputFlag') == '1#') {
                //alert('a');
                ...
                //document.getElementById('td_printbarcode').style.display = 'block';
            }
        }
        function PrintNRequestFormList() {
            var ClientNo = document.getElementById('hiddenInputClientNo').value;
            var txtStartDate = document.getElementById('txtStartDate').value;
            var txtEndDate = document.getElementById('txtEndDate').value;
            var txtCollectStartDate = document.getElementById('txtCollectStartDate').value;
            var txtCollectEndDate = document.getElementById('txtCollectEndDate').value;
...
<body onload="document.getElementById('UpdatePanel1').style.position = 'absolute';document.getElementById('UpdatePanel1').style.index = '998';CheckBarcodePrintFlag();">
    ...
    <div class="Operate_Div">
        <table class="Operate_Table">
            <tr>
                <td>
                    <input type="button" name="Input" accesskey="I" onclick="OpenInsertPage();" value="录入[I]" /></td>
                    <td>
    ...
    <div class="Search_Div">
        <table width="98%" height="50" border="0" cellpadding="0" cellspacing="0" style="font-size:12px">
            <tr>
                <td>
                    送检单位：<input type="text" class="inputText" id="txtClientNo" name="txtClientNo"  runat="server" onfocus="flag=true;SelectInputItem(this,'hiddenClient','WebLisSourceOrgID','Client');" 
                                    onkeydown="SelectNextClient(this,'hiddenClient')" 
                                    onpropertychange="SelectInputItem(this,'hiddenClient','WebLisSourceOrgID','Client')" onblur="SelectValue(this,'hiddenClient','WebLisSourceOrgID','Client');DisplayDiv(true);" tabindex="0" /><input type="hidden" id="hiddenClient" runat="server" />
            ...
            <tr><td>
                    医生&nbsp;&nbsp;&nbsp;&nbsp;：<input type="text" id="SelectDoctor"  onfocus="flag=true;SelectInputItem(this,'hiddenDoctorNo','DoctorNo','Doctor')" onkeydown="SelectNextClient(this,'hiddenDoctorNo')"
                                    onpropertychange="SelectInputItem(this,'hiddenDoctorNo','DoctorNo','Doctor')" onblur="SelectValue(this,'hiddenDoctorNo','DoctorNo','Doctor');DisplayDiv(true);"
                                    class="inputText" runat="server" tabindex="14"  /><input type="hidden" id="hiddenDoctorNo" runat="server" />
                    ...
                    <asp:UpdatePanel ID="UpdatePanel2" runat="server">
                        <ContentTemplate>
                            <asp:Button TabIndex="4" ID="btnSearch" runat="server" Text="查询[S]" AccessKey="S"
                                OnClick="btnSearch_Click" />&nbsp;<input type="button" value="打印预览" onclick="PrintNRequestFormList();" />
                        </ContentTemplate>
                    </asp:UpdatePanel>
    ...
    <div id="UpdatePanel1">
            ...
            <div id="tableUserInputSJDWList"></div>
                <iframe id='listiframe' style="position:absolute;width:100%;height:100%;_filter:alpha(opacity=0);opacity=0;border-style:none; display:none"></iframe>
            </div>
    ...
    </asp:UpdatePanel><div style="font-size:12px;display:none"></div>
    </form>
</body>
</html>
