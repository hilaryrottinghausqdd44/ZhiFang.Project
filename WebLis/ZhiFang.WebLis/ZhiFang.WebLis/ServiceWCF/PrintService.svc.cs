    ...
    public class PrintService : ZhiFang.WebLis.ServerContract.IPrintService
    {
        ...
        */
        ...
        public BaseResultDataValue ReportPrint(string reportformId, string reportformtitle, string reportformfiletype, string printtype)
        {
            ...
            #region 验证
            if (string.IsNullOrEmpty(reportformId))
            {
                brdv.success = false;
                brdv.ErrorInfo = "参数错误1！";
                return brdv;
            }  
            if (printtype == null || printtype == "")
            {
                brdv.success = false;
                brdv.ErrorInfo = "参数错误2！";
                return brdv;
            }
            if (reportformtitle == null || reportformtitle == "")
            {
                brdv.success = false;
                brdv.ErrorInfo = "参数错误3！";
                return brdv;
            }
            #endregion
        ...
        public BaseResult UpdatePrintTimeByReportFormID(string ReportFormID)
        {
            ...
            try
            {
                ZhiFang.Common.Log.Log.Debug("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@:" + ReportFormID);
                IBReportFormFull ibrff = BLLFactory<IBReportFormFull>.GetBLL("ReportFormFull");
        ...
        /// <param name="reportformfilenameset">报告名称设置</param>
        /// <returns></returns>
        public BaseResultDataValue DownLoadReport(Model.DownloadReportParam jsonentity)
        {
            ...
            try
            {
                ...
                foreach (string reportformId in reportformIds.Split(';'))
                {
                    ...
                    string filenameset = (jsonentity.reportformfilenameset != null && jsonentity.reportformfilenameset.Trim() != "") ? jsonentity.reportformfilenameset : "CNAME,FORMNO,SERIALNO";
                    ZhiFang.Common.Log.Log.Debug("DownLoadReport.5。filenameset：" + filenameset);
                    //多个报告打包到Zip
                    foreach (string reportformContext in ReportFormContextListTemp)
                    {
                        ...
                        if (ZhiFang.Common.Public.FilesHelper.CheckAndCreatDir(tmppath))
                        {
                            ...
                            foreach (var a in filenameset.Split(','))
                            {
                                if (dsrf.Tables[0].Rows[0][a] != null)
                                    par.Add(dsrf.Tables[0].Rows[0][a].ToString());
                                else
                                    par.Add("");
                            }
                            newfilename = zipfilepath+ string.Join("-", par) + ".PDF";
                            ZhiFang.Common.Log.Log.Debug("DownLoadReport.newfilename=" + par);
        ...
        ///// <param name="reportformtitle">抬头</param>
        ///// <returns></returns>
        //public Stream DownLoadReportByIdsToStream(Model.DownloadReportParam jsonentity)
        //{            
        //    try
        //    {
