...
namespace ZhiFang.WebLis.ServiceWCF
{
    ...
    public class ReportFromService : ServerContract.IReportFromService
    {
        ...
        /// <param name="rows"></param>
        /// <returns></returns>
        public BaseResultDataValue SearchReportFormFull_ReportItem_WeiJiZhi(string wherestr, int page, int rows, string order)
        {

            BaseResultDataValue brdv = new BaseResultDataValue();
            //EntityListEasyUI<UiReportForm_N> easyui = new EntityListEasyUI<UiReportForm_N>();
            EntityListEasyUI<Model.ReportFormFull_ReportItem_WeiJiZhi> easyui = new EntityListEasyUI<Model.ReportFormFull_ReportItem_WeiJiZhi>();
            try
            {
                #region 验证
                if (wherestr == null)
                ...
                ZhiFang.Common.Log.Log.Debug("SearchReportFormFull_ReportItem_WeiJiZhi.wherestr=" + wherestr + "@order=" + order);
                DataSet ds = ibrff.SearchReportFormFull_ReportItem_WeiJiZhi(wherestr, order, (page - 1) * rows + 1, page * rows);
                if (ds == null || ds.Tables.Count <= 0 || ds.Tables[0].Rows.Count <= 0)
                {
                    brdv.success = false;
                    brdv.ErrorInfo = "未能查询到危急值！";
                    return brdv;
                }
                DataTable dt = ds.Tables[0];

                List<ReportFormFull_ReportItem_WeiJiZhi> Result = new List<ReportFormFull_ReportItem_WeiJiZhi>();
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ReportFormFull_ReportItem_WeiJiZhi RFFRIWJZ = new ReportFormFull_ReportItem_WeiJiZhi();
                    if (dt.Columns.Contains("AGE") && dt.Rows[i]["AGE"] != DBNull.Value)
                    {
                        RFFRIWJZ.AGE = dt.Rows[i]["AGE"].ToString().Trim();
                    }

                    if (dt.Columns.Contains("AGEUNITNAME") && dt.Rows[i]["AGEUNITNAME"] != DBNull.Value)
                    {
                        RFFRIWJZ.AGE += dt.Rows[i]["AGEUNITNAME"].ToString().Trim();
                    }

                    if (dt.Columns.Contains("CNAME") && dt.Rows[i]["CNAME"] != DBNull.Value)
                    {
                        RFFRIWJZ.CNAME = dt.Rows[i]["CNAME"].ToString().Trim();
                    }

                    if (dt.Columns.Contains("CLIENTNAME") && dt.Rows[i]["CLIENTNAME"] != DBNull.Value)
                    {
                        RFFRIWJZ.CLIENTNAME = dt.Rows[i]["CLIENTNAME"].ToString().Trim();
                    }

                    if (dt.Columns.Contains("CLIENTNO") && dt.Rows[i]["CLIENTNO"] != DBNull.Value)
                    {
                        RFFRIWJZ.CLIENTNO = dt.Rows[i]["CLIENTNO"].ToString().Trim();
                    }

                    if (dt.Columns.Contains("TESTDATE") && dt.Rows[i]["TESTDATE"] != DBNull.Value)
                    {
                        RFFRIWJZ.TESTDATE = DateTime.Parse(dt.Rows[i]["TESTDATE"].ToString().Trim());
                    }

                    if (dt.Columns.Contains("TESTITEMNAME") && dt.Rows[i]["TESTITEMNAME"] != DBNull.Value)
                    {
                        RFFRIWJZ.TESTITEMNAME = dt.Rows[i]["TESTITEMNAME"].ToString().Trim();
                    }

                    if (dt.Columns.Contains("REPORTVALUE") && dt.Rows[i]["REPORTVALUE"] != DBNull.Value)
                    {
                        RFFRIWJZ.REPORTVALUE = dt.Rows[i]["REPORTVALUE"].ToString().Trim();
                    }

                    if (dt.Columns.Contains("REPORTDESC") && dt.Rows[i]["REPORTDESC"] != DBNull.Value)
                    {
                        RFFRIWJZ.REPORTDESC = dt.Rows[i]["REPORTDESC"].ToString().Trim();
                    }

                    if (!string.IsNullOrEmpty(RFFRIWJZ.REPORTVALUE))
                    {
                        RFFRIWJZ.REPORTVALUEALL = RFFRIWJZ.REPORTVALUE;
                    }
                    else
                    {
                        if (!string.IsNullOrEmpty(RFFRIWJZ.REPORTDESC))
                        {
                            RFFRIWJZ.REPORTVALUEALL = RFFRIWJZ.REPORTDESC;
                        }
                    }

                    if (dt.Columns.Contains("UNIT") && dt.Rows[i]["UNIT"] != DBNull.Value)
                    {
                        RFFRIWJZ.UNIT = dt.Rows[i]["UNIT"].ToString().Trim();
                    }

                    if (dt.Columns.Contains("REFRANGE") && dt.Rows[i]["REFRANGE"] != DBNull.Value)
                    {
                        RFFRIWJZ.REFRANGE = dt.Rows[i]["REFRANGE"].ToString().Trim();
                    }

                    if (dt.Columns.Contains("ItemRESULTSTATUS") && dt.Rows[i]["ItemRESULTSTATUS"] != DBNull.Value)
                    {
                        RFFRIWJZ.ItemRESULTSTATUS = dt.Rows[i]["ItemRESULTSTATUS"].ToString().Trim();
                    }

                    if (dt.Columns.Contains("PARITEMNAME") && dt.Rows[i]["PARITEMNAME"] != DBNull.Value)
                    {
                        RFFRIWJZ.PARITEMNAME = dt.Rows[i]["PARITEMNAME"].ToString().Trim();
                    }
                    if (dt.Columns.Contains("GENDERNAME") && dt.Rows[i]["GENDERNAME"] != DBNull.Value)
                    {
                        RFFRIWJZ.GENDERNAME = dt.Rows[i]["GENDERNAME"].ToString().Trim();
                    }
                    if (dt.Columns.Contains("SAMPLETYPENAME") && dt.Rows[i]["SAMPLETYPENAME"] != DBNull.Value)
                    {
                        RFFRIWJZ.SAMPLETYPENAME = dt.Rows[i]["SAMPLETYPENAME"].ToString().Trim();
                    }
                    Result.Add(RFFRIWJZ);
                }

                DataTable tempDt = ds.Tables[0];
                int total = ibrff.SearchReportFormFull_ReportItem_WeiJiZhi_Count(wherestr);
                easyui.rows = Result;
                easyui.total = total;
        ...
        //{
        //    return SelectReportListByPara(Startdate, Enddate, CLIENTNO, SECTIONNO, CNAME, GENDERNAME, SAMPLENO, PATNO, SICKTYPENO, statues, ZDY10, PERSONID, LIKESEARCH, serialno, clientcode, collectStartdate, collectEnddate, noperdateStart, noperdateEnd, operdateStart, operdateEnd, checkdateStart, checkdateEnd, page, rows, sort, order, abnormalstatues, DeptNos);
        //}
        /// <summary>
        /// 获取报告查询列表
        /// </summary>
        ...
        public BaseResultDataValue SelectReportListByPara(string Startdate, string Enddate, string CLIENTNO, string SECTIONNO, string CNAME, string GENDERNAME, string SAMPLENO, string PATNO, string SICKTYPENO, string statues, string ZDY10, string PERSONID, string LIKESEARCH, string serialno, string clientcode, string collectStartdate, string collectEnddate, string noperdateStart, string noperdateEnd, string operdateStart, string operdateEnd, string checkdateStart, string checkdateEnd, int page, int rows, string sort, string order, string abnormalstatues, string DeptNos)
        {
            ...
            Model.ReportFormFull modelSearch = ConvertToReportFormFull(Startdate, Enddate, CLIENTNO, SECTIONNO, CNAME, GENDERNAME, SAMPLENO, PATNO, SICKTYPENO, statues, ZDY10, PERSONID, LIKESEARCH, serialno, clientcode, collectStartdate, collectEnddate, noperdateStart, noperdateEnd, operdateStart, operdateEnd, checkdateStart, checkdateEnd, DeptNos);
            ...
            {
                #region 验证
                if (Startdate == null && Enddate == null && collectStartdate == null && collectEnddate == null && noperdateStart == null && noperdateEnd == null && checkdateStart == null && checkdateEnd == null && operdateStart == null && operdateEnd == null)
                {
                    brdv.success = false;
        ...
        /// <param name="rows"></param>
        /// <returns></returns>
        public BaseResultDataValue SearchReportListByJsonStrPara(string jsonstr, int page, int rows, string sort, string order)
        {
            BaseResultDataValue brdv = new BaseResultDataValue();
            if (string.IsNullOrEmpty(jsonstr))
            {
                ZhiFang.Common.Log.Log.Error("SelectReportListByJsonStrPara.jsonstr为空！");
                brdv.ErrorInfo = "参数错误！";
                brdv.success = false;
                return brdv;
            }
            ZhiFang.Common.Log.Log.Debug("SelectReportListByJsonStrPara.jsonstr:" + jsonstr);

            try
            {
                SearchReportFromListParaVO svo = ZhiFang.Common.Public.JsonSerializer.JsonDotNetDeserializeObject<SearchReportFromListParaVO>(jsonstr);
                if (svo.ClientNo == null)
                {
                    List<Model.CLIENTELE> lc = new List<Model.CLIENTELE>();
                    ...
                    lc.ForEach(a => cnolist.Add(a.ClIENTNO));
                    svo.ClientNo = string.Join(",", cnolist);
                }
                Model.ReportFormFull modelSearch = ConvertToReportFormFull(svo);
                if (!string.IsNullOrEmpty(svo.AbnormalStatus))
                {
                    modelSearch.ResultStatus = int.Parse(svo.AbnormalStatus);
                }
                ...
                #region 验证
                if (svo.Startdate == null && svo.Enddate == null && svo.CollectDateStart == null && svo.CollectDateEnd == null && svo.NOperDateStart == null && svo.NOperDateEnd == null && svo.CheckDateStart == null && svo.CheckDateEnd == null && svo.OperDateStart == null && svo.OperDateEnd == null)
                {
                    brdv.success = false;
                    brdv.ErrorInfo = "查询日期不能为空";
                    return brdv;
                ...
                int count = ibrff.GetTotalCount(modelSearch);
                ZhiFang.Common.Log.Log.Debug("SelectReportListByJsonStrPara:count=" + count + "@ReportSelMaxNum=" + ReportSelMaxNum);
                if (count > ReportSelMaxNum)
                {
                    brdv.success = false;
                    brdv.ErrorInfo = "返回记录数大于配置的条数";
                ...
                ZhiFang.Common.Log.Log.Debug("SelectReportListByJsonStrPara.sqlsort=" + sqlsort);
                DataSet ds = ibrff.GetList(ReportSelMaxNum, modelSearch, sqlsort);
                if (ds == null)
                {
                    return null;
            ...
            {
                ZhiFang.Common.Log.Log.Error("SelectReportListByJsonStrPara异常：" + e.ToString());
                brdv.ErrorInfo = "SelectReportListByJsonStrPara异常";
                brdv.success = false;
                return brdv;
            }
            return brdv;
        ...
        {
            Model.ReportFormFull model1 = ConvertToReportFormFull(Startdate, Enddate, CLIENTNO, SECTIONNO, CNAME, GENDERNAME, SAMPLENO, PATNO, null, statues, ZDY10, PERSONID, LIKESEARCH, null, null, null, null, null, null, null, null, null, null,null);
            //Model.ReportFormFull model = new ReportFormFull();
            BaseResultDataValue brdv = new BaseResultDataValue();
        ...
        private Model.ReportFormFull ConvertToReportFormFull(string Startdate, string Enddate, string CLIENTNO, string SECTIONNO, string CNAME, string GENDERNAME, string SAMPLENO, string PATNO, string SICKTYPENO, string PRINTTIMES, string ZDY10, string PERSONID, string LIKESEARCH, string serialno, string clientcode, string collectStartdate, string collectEnddate, string noperdateStart, string noperdateEnd, string operdateStart, string operdateEnd, string checkdateStart, string checkdateEnd,string DeptNos)
        {
            ...
            if (operdateStart != null)
                tempModel.operdateStart = DateTime.Parse(operdateStart);
            if (operdateEnd != null)
                tempModel.operdateEnd = DateTime.Parse(operdateEnd);
            ...
            if (!string.IsNullOrEmpty(DeptNos)&&int.TryParse(DeptNos,out int dno))
                tempModel.DEPTNO = dno;

            return tempModel;
        }

        private Model.ReportFormFull ConvertToReportFormFull(SearchReportFromListParaVO srflpvo)
        {
            if (srflpvo == null)
                return null;
            ZhiFang.Common.Log.Log.Debug("ConvertToReportFormFull.srflpvo:" + ZhiFang.Common.Public.JsonSerializer.JsonDotNetSerializer(srflpvo));
            Model.ReportFormFull tempModel = new ReportFormFull();
            ...
            if (!string.IsNullOrEmpty(srflpvo.Startdate))
                tempModel.Startdate = DateTime.Parse(srflpvo.Startdate);
            if (!string.IsNullOrEmpty(srflpvo.Enddate))
                tempModel.Enddate = DateTime.Parse(srflpvo.Enddate);
            if (!string.IsNullOrEmpty(srflpvo.CollectDateStart))
                tempModel.collectStartdate = DateTime.Parse(srflpvo.CollectDateStart);
            if (!string.IsNullOrEmpty(srflpvo.CollectDateEnd))
                tempModel.collectEnddate = DateTime.Parse(srflpvo.CollectDateEnd);
            if (!string.IsNullOrEmpty(srflpvo.NOperDateStart))
                tempModel.noperdateStart = DateTime.Parse(srflpvo.NOperDateStart);
            if (!string.IsNullOrEmpty(srflpvo.NOperDateEnd))
                tempModel.noperdateEnd = DateTime.Parse(srflpvo.NOperDateEnd);
            if (!string.IsNullOrEmpty(srflpvo.OperDateStart))
                tempModel.operdateStart = DateTime.Parse(srflpvo.OperDateStart);
            if (!string.IsNullOrEmpty(srflpvo.OperDateEnd))
                tempModel.operdateEnd = DateTime.Parse(srflpvo.OperDateEnd);
            if (!string.IsNullOrEmpty(srflpvo.CheckDateStart))
                tempModel.CheckStartDate = DateTime.Parse(srflpvo.CheckDateStart);
            if (!string.IsNullOrEmpty(srflpvo.CheckDateEnd))
                tempModel.CheckEndDate = DateTime.Parse(srflpvo.CheckDateEnd);
            if (string.IsNullOrEmpty(srflpvo.ClientNo) || srflpvo.ClientNo.Contains(",") == false)
                tempModel.CLIENTNO = srflpvo.ClientNo;
            else
                tempModel.ClientList = srflpvo.ClientNo;
            if (!string.IsNullOrEmpty(srflpvo.SectionNo))
                tempModel.SECTIONNO = srflpvo.SectionNo;
            if (!string.IsNullOrEmpty(srflpvo.CName))
            {
                tempModel.CNAME = srflpvo.CName;
            }
            if (!string.IsNullOrEmpty(srflpvo.GenderName))
                tempModel.GENDERNAME = srflpvo.GenderName;
            if (!string.IsNullOrEmpty(srflpvo.SampleNo))
                tempModel.SAMPLENO = srflpvo.SampleNo;
            if (!string.IsNullOrEmpty(srflpvo.PatNo))
                tempModel.PATNO = srflpvo.PatNo;
            if (!string.IsNullOrEmpty(srflpvo.PrintStatus))
                tempModel.PRINTTIMES = int.Parse(srflpvo.PrintStatus);
            if (!string.IsNullOrEmpty(srflpvo.ZDY10))
                tempModel.ZDY10 = srflpvo.ZDY10;
            if (!string.IsNullOrEmpty(srflpvo.PersonId))
                tempModel.PERSONID = srflpvo.PersonId;
            if (!string.IsNullOrEmpty(srflpvo.LikeSearch))
                tempModel.LIKESEARCH = srflpvo.LikeSearch;
            if (!string.IsNullOrEmpty(srflpvo.SerialNo))
                tempModel.serialno = srflpvo.SerialNo;
            if (!string.IsNullOrEmpty(srflpvo.ClientCode))
                tempModel.clientcode = srflpvo.ClientCode;
            if (!string.IsNullOrEmpty(srflpvo.SickTypeNo))
                tempModel.SICKTYPENO = srflpvo.SickTypeNo;
            if (!string.IsNullOrEmpty(srflpvo.DeptNo))
                tempModel.DEPTNO = int.Parse(srflpvo.DeptNo);
        ...
        /// <param name="rows"></param>
        /// <returns></returns>
        public BaseResultDataValue SelectReportListByPerson_Barcode_Name_DataRoundType(string Barcode, string Name, int DataRoundType, int page, int rows, string sort, string order)
        {
            ...
            try
            {
                ...
                Name = ZhiFang.Tools.Validate.ReplaceWhereString(Name);
                wheresql = " SerialNo='" + Barcode.Trim() + "' and CName='" + Name.Trim() + "' ";
                switch (DataRoundType)
                {
                    case 1:
                        wheresql += " and CHECKDATE>='" + DateTime.Now.AddMonths(-1).ToString("yyyy-MM-dd HH:mm:ss") + "' ";
                        break;
                    case 2:
                        wheresql += " and CHECKDATE>='" + DateTime.Now.AddMonths(-6).ToString("yyyy-MM-dd HH:mm:ss") + "' ";
                        break;
                    case 3:
                        wheresql += " and CHECKDATE<='" + DateTime.Now.AddMonths(-6).ToString("yyyy-MM-dd HH:mm:ss") + "' ";
                        break;
                ...
                ZhiFang.Common.Log.Log.Debug("SelectReportListByPerson_Barcode_Name_DataRoundType.wherestr+sqlsort=" + wheresql + sqlsort);
                DataSet ds = ibrff.GetList(wheresql + sqlsort);
                if (ds == null)
                {
                    return null;
        ...
        /// <param name="rows"></param>
        /// <returns></returns>
        public BaseResultDataValue SelectReportListByPerson_Patno_Name_DataRoundType(string Patno, string Name, int DataRoundType, int page, int rows, string sort, string order)
        {
            ...
            try
            {
                ...
                if (Patno == null || Patno == string.Empty || Patno.Trim() == "")
                {
                    brdv.success = false;
                    brdv.ErrorInfo = "Barcode错误";
                    return brdv;
                }
                Patno = ZhiFang.Tools.Validate.ReplaceWhereString(Patno);
                ...
                Name = ZhiFang.Tools.Validate.ReplaceWhereString(Name);
                wheresql = " (Patno='" + Patno.Trim() + "' or SerialNo='" + Patno.Trim() + "'or ZDY5='" + Patno.Trim() + "')  and CName='" + Name.Trim() + "' ";
                switch (DataRoundType)
                {
                    case 1:
                        wheresql += " and CHECKDATE>='" + DateTime.Now.AddMonths(-1).ToString("yyyy-MM-dd HH:mm:ss") + "' ";
        ...
        public BaseResultDataValue SearchReportFormResultByPerson_Barcode_Name_ReportFormID(string Barcode, string Name, string ReportFormID)
        {

            BaseResultDataValue brdv = new BaseResultDataValue();
            Model.UiModel.WeiXin.ReportForm entity = new Model.UiModel.WeiXin.ReportForm();
            try
            {
                List<Model.UiModel.WeiXin.AppRFObject> LARFO = new List<Model.UiModel.WeiXin.AppRFObject>();
                entity = ibrff.SearchReportFormResultByReportFormID(ReportFormID);

                if (entity != null)
                {
                    LARFO.Add(Model.UiModel.WeiXin.AppRFObject.SetValue(entity));
                }
                brdv.success = true;
                brdv.ResultDataValue = JsonHelp.JsonDotNetSerializer(LARFO);
            }
            catch (Exception ex)
            {
                brdv.success = false;
                brdv.ErrorInfo = "序列化错误：" + ex.Message;
                ZhiFang.Common.Log.Log.Debug("SearchReportFormByPerson_Barcode_Name_ReportFormID:" + ex.ToString());
        ...
        //public BaseResultDataValue SelectReportListByBarcode_ClientNo_Date(string Barcode, string Name, int page, int rows, string sort, string order)
        //{
