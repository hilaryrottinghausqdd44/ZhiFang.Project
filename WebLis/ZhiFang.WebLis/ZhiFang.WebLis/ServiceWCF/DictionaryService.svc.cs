    ...
    public class DictionaryService : IDictionaryService
    {
        ...
        public List<Model.CLIENTELE> GetClientListByRBAC(int page, int rows, string fields, string where, string sort)
        {
            ...
            try
            {
                ...
                EntityList<Model.CLIENTELE> cl1 = blcc.GetBusinessLogicClientList(new Model.BusinessLogicClientControl { Account = user.Account.Trim(), SelectedFlag = true, Flag = 1 }, page, rows, fields, where, sort);
                if (cl1 != null&&cl1.list!=null &&cl1.list.Count>0)
                {
                    foreach (var c in cl.list)
                    {
                        if (cl1.list.Count(a => a.ClIENTNO == c.ClIENTNO) > 0)
                        {
                            c.BusinessLogicClientControlFlag = 1;
                        }
                    }
                }
                ...
                if (fields.IndexOf("SHORTCODE_CNAME") >= 0)
                {
                    foreach (var a in cl.list)
                    {
                        a.CNAME = a.SHORTCODE + "_" + a.CNAME;
                    }
                }
                lc = cl.list.OrderBy(a=>a.SHORTCODE).ToList();
                return lc;
            }
            catch (Exception e)
            {
        ...
        public BaseResultDataValue GetPubDict(string tablename, string fields, string filervalue, string itemno, string selectedflag, string labcode, int page, int rows, string precisequery, string sort, string order)
        {
                    ...
                    switch (flagstr.ToLower())
                    {
                        ...
                        case "default":
                            ...
                            if (tablenameParam == "TestItem" && fields == "ShortName")
                            {
                                ...
                                brds.rows = dv.ToTable(true, "ShortName");//shortName去重
            ...
            else
            {
                switch (tablename.ToLower())
                {
                    ...
                    #region 对照关系
                    case "testitemcontrol": return GetTestItemControlModelManage(filervalue, tablename, labcode, selectedflag, page, rows,sort,order);
                    case "b_lab_testitemcontrol": return GetTestItemControlModelManage(filervalue, tablename, labcode, selectedflag, page, rows, sort, order);
                    case "b_lab_resulttestitemcontrol": return GetTestItemControlModelManage(filervalue, tablename, labcode, selectedflag, page, rows, sort, order);
        ...
        public BaseResultDataValue GetSubTestItemByItemNo(string ItemNo)
        {
            BaseResultDataValue brdv = new BaseResultDataValue();
            List<TestItemDetail> ttdList = new List<TestItemDetail>();
            try
            {
                GetSubLabItem(ItemNo, "", ref ttdList);
                if (ttdList.Count > 0)
                {
                    brdv.success = true;
            ...
            catch (Exception e)
            {
                ...
                ZhiFang.Common.Log.Log.Error("GetSubTestItemByItemNo.异常" + e.ToString());
            }
            return brdv;
        }

        public BaseResultDataValue GetParTestItemByItemNo(string ItemNo)
        {
            BaseResultDataValue brdv = new BaseResultDataValue();
            List<TestItemDetail> ttdList = new List<TestItemDetail>();
            try
            {
                GetParLabItem(ItemNo, "",ref ttdList);
                if (ttdList.Count > 0)
                {
                    brdv.success = true;
            ...
            catch (Exception e)
            {
                ...
                brdv.ErrorInfo = "异常";// e.ToString();
                brdv.ResultDataValue = "";
                ZhiFang.Common.Log.Log.Error("GetParTestItemByItemNo.异常" + e.ToString());
            }
            return brdv;
        }

        public BaseResultDataValue GetItemListByColor(string colorvalue,string where, int page, int rows, string sort, string order)
        {
            BaseResultDataValue brdv = new BaseResultDataValue();
            EntityListEasyUI<TestItem> result = new EntityListEasyUI<TestItem>();
            try
            {
                if (page <= 0)
                {
                ...
                List<TestItem> list = CenterTestItem.GetItemListByColor(colorvalue, where, order, (page - 1) * rows + 1, page * rows);
                int iCount = CenterTestItem.GetItemListByColorCount(colorvalue, where);
                ...
                brdv.success = true;
                brdv.ResultDataValue = ZhiFang.Common.Public.JsonSerializer.JsonDotNetSerializer(result);
                // 在此处添加操作实现
                return brdv;
            }
            catch (Exception e)
            {
                ZhiFang.Common.Log.Log.Error("SearchReportFormFull.异常：" + e.ToString());
                brdv.success = false;
                brdv.ErrorInfo = "程序异常！";
                return brdv;
            }
        ...
        public BaseResultDataValue SetItemColorByItemNoList(string colorvalue, List<string> itemnolist)
        {
            BaseResultDataValue brdv = new BaseResultDataValue();
            EntityListEasyUI<TestItem> result = new EntityListEasyUI<TestItem>();
            try
            {
                ...
                brdv = CenterTestItem.SetItemColorByItemNoList(colorvalue, itemnolist);
                return brdv;
            }
            catch (Exception e)
            {
        ...
        public BaseResultDataValue GetLabTestItemByLabItemNo(string LabItemNo, string labcode)
        {
            IBLL.Common.BaseDictionary.IBLab_TestItem ibDictionary = BLLFactory<IBLab_TestItem>.GetBLL();
            BaseResultDataValue brdv = new BaseResultDataValue();
            EntityListEasyUI<Model.Lab_TestItem> EntityList = new EntityListEasyUI<Model.Lab_TestItem>();
            try
            {                

                DataSet ds = ibDictionary.GetList(new Lab_TestItem { LabCode = labcode, LabItemNo = LabItemNo, IfUseLike="0" });
                if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                {
                    EntityList.rows = ibDictionary.DataTableToList(ds.Tables[0]);
                    ////if (!string.IsNullOrEmpty(itemkey))
                    //EntityLis.total = ibDictionary.GetTotalCount(new Lab_TestItem { LabCode = labcode, TestItemLikeKey = itemkey });
                    ////else
                    ////    EntityLis.total = ibDictionary.GetTotalCount();
                    ...
                    brdv.ErrorInfo = "请求成功!";
                    brdv.ResultDataFormatType = "JSON";
                    brdv.ResultDataValue = ZhiFang.BLL.Common.JsonHelp.JsonDotNetSerializer(EntityList);
                }
                else
                {
                    ZhiFang.Common.Log.Log.Info("请求失败,字典无数据！");
        ...
        public BaseResultDataValue GetLabSubTestItemByItemNo(string ItemNo,string LabCode)
        {
            BaseResultDataValue brdv = new BaseResultDataValue();
            List<TestItemDetail> ttdList = new List<TestItemDetail>();
            try
            {
                GetSubLabItem(ItemNo, LabCode, ref ttdList);
                if (ttdList.Count > 0)
                {
                    brdv.success = true;
            ...
            catch (Exception e)
            {
                ...
                brdv.ErrorInfo = "异常";// e.ToString();
                brdv.ResultDataValue = "";
                ZhiFang.Common.Log.Log.Error("GetLabSubTestItemByItemNo.异常" + e.ToString());
            }
            return brdv;
        }

        public BaseResultDataValue GetLabParTestItemByItemNo(string ItemNo, string LabCode)
        {
            BaseResultDataValue brdv = new BaseResultDataValue();
            List<TestItemDetail> ttdList = new List<TestItemDetail>();
            try
            {
                GetParLabItem(ItemNo, LabCode, ref ttdList);
                if (ttdList.Count > 0)
                {
                    brdv.success = true;
            ...
            catch (Exception e)
            {
                ...
                brdv.ErrorInfo = "异常";// e.ToString();
                brdv.ResultDataValue = "";
                ZhiFang.Common.Log.Log.Error("GetLabParTestItemByItemNo.异常" + e.ToString());
            }
            return brdv;
        }
        #endregion
        ...
        public BaseResultDataValue GetCenterSickTypeListByLab_Area_Center(string itemkey, string labcode, int page, int rows)
        {
            IBLL.Common.BaseDictionary.IBLab_SickType ibDictionary = BLLFactory<IBLab_SickType>.GetBLL();
            BaseResultDataValue brdv = new BaseResultDataValue();
            EntityListEasyUI<Model.SickType> EntityLis = new EntityListEasyUI<Model.SickType>();
            try
            {
                string tmplabcode = labcode;
                if (page > 0)
                    page = page - 1;
                //读取实验室自己的字典
                DataSet ds = ibDictionary.GetListByPage(new Lab_SickType { LabCode = tmplabcode, SearchLikeKey = itemkey }, page, rows);
                if (ds == null || ds.Tables.Count <= 0 || ds.Tables[0].Rows.Count <= 0)                
                {
                    IBCLIENTELE ibc = BLLFactory<IBCLIENTELE>.GetBLL();
                    Model.CLIENTELE c=ibc.GetModel(long.Parse(tmplabcode));
                    if (c.AreaID.HasValue && c.AreaID.Value > 0)
                    {
                        IBClientEleArea ibca = BLLFactory<IBClientEleArea>.GetBLL();
                        var area=ibca.GetModel(c.AreaID.Value);
                        if (area.ClientNo.HasValue && area.ClientNo.Value > 0)
                        {
                            tmplabcode = area.ClientNo.Value.ToString();
                            //读取区域
                            ds = ibDictionary.GetListByPage(new Lab_SickType { LabCode = area.ClientNo.Value.ToString(), SearchLikeKey = itemkey }, page, rows);
                        }
                    }
                }
                ...
                if (ds == null || ds.Tables.Count <= 0 || ds.Tables[0].Rows.Count <= 0)
                {
                    ...
                    //读取中心
                    ds = ibst.GetListByPage(new SickType { SearchLikeKey = itemkey }, page, rows);
                    EntityLis.rows = new List<SickType>();
                    EntityLis.rows = ibst.DataTableToList(ds.Tables[0]);
                    EntityLis.total = ibst.GetTotalCount(new SickType {  SearchLikeKey = itemkey });
                }
                else
                {
                    EntityLis.rows = new List<SickType>();
                    for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                    {
                        SickType tmp = new SickType();
                        IBSickTypeControl controlBLL = BLLFactory<IBLL.Common.BaseDictionary.IBSickTypeControl>.GetBLL("BaseDictionary.SickTypeControl");
                        Model.SickTypeControl controlModel = new Model.SickTypeControl();
                        controlModel.ControlLabNo = tmplabcode;
                        controlModel.ControlSickTypeNo = int.Parse(ds.Tables[0].Rows[i]["LabSickTypeNo"].ToString().Trim());
                        controlModel.SickTypeNo = -1;
                        DataSet dsControl = controlBLL.GetList(controlModel);
                        if (dsControl != null && dsControl.Tables.Count > 0 && dsControl.Tables[0].Rows.Count > 0)
                        {
                            tmp.SickTypeNo = int.Parse(dsControl.Tables[0].Rows[0]["SickTypeNo"].ToString());
                            tmp.CName = ds.Tables[0].Rows[i]["CName"].ToString();
                            EntityLis.rows.Add(tmp);
                        }
                        
                    }
                    EntityLis.total = ibDictionary.GetTotalCount(new Lab_SickType { LabCode = tmplabcode, SearchLikeKey = itemkey });                   
                }

                EntityLis.rows=EntityLis.rows.OrderBy(a => a.SickTypeNo).ToList<SickType>();
                brdv.success = true;
                brdv.ErrorInfo = "请求成功!";
                brdv.ResultDataFormatType = "JSON";
                brdv.ResultDataValue = ZhiFang.BLL.Common.JsonHelp.JsonDotNetSerializer(EntityLis);
        ...
        public BaseResultDataValue GetLabDepartmentModelManage(string itemkey, string labcode, int page, int rows)
        {
            ...
            try
            {
                ...
                if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                    {
                        ds.Tables[0].Rows[i]["ControlStatus"] = "--";
        ...
        public DataTable ReturnDataTableTestItemControl(DataView dv)
        {
            int total = dv.Count;
            DataTable dt = new DataTable();
            dt.Columns.Add("LabItemNo", typeof(string));
            ...
            {
                DataRow row = dt.NewRow();
                row["LabItemNo"] = dv[i]["LabItemNo"];
                row["CName"] = dv[i]["CName"];
                row["ShortCode"] = dv[i]["ShortCode"];
                row["CenterItemNo"] = dv[i]["CenterItemNo"];
                row["CenterCName"] = dv[i]["CenterCName"];
                dt.Rows.Add(row);
            }
            return dt;
        }
        ...
        public BaseResultDataValue GetTestItemControlModelManage(string itemkey, string tablename, string labcode, string selectedflag, int page, int rows,string sort,string order)
        {
            ...
            try
            {
                ...
                if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    {
                        brdv.ResultDataFormatType = "JSON";
                        //brdv.ResultDataValue = DataSetToJson.ToJson(dv);
                        BaseResultDataSet brds = new BaseResultDataSet();
                        brds.total = ds.Tables[0].Rows.Count;

                        //brds.rows = ds.Tables[0];

                        brds.rows = ReturnDataTableTestItemControl(dv);
        ...
        public BaseResult UpdateOrAddTestItemControlModelByID(Model.TestItemControl jsonentity)
        {
            ...
            try
            {
                ...
                //ticontrolModel.Id = ZhiFang.Common.Public.GUIDHelp.GetGUIDLong();
        ...
        /// <param name="ItemKey"></param>
        /// <returns></returns>
        public BaseResult BatchCopySickTypeToLab(string ItemNos, string fromLabCodeNo, string LabCodeNo)
        {
            BaseResult br = new BaseResult();
            ...
            try
            {
                ...
                if (fromLabCodeNo != null)//实验室到实验室间的复制
                {
                    IBLL.Common.BaseDictionary.IBLab_SickType iblabs = BLLFactory<IBLab_SickType>.GetBLL();
                    br= iblabs.CopyToLab(ItemNos.Split(',').ToList(), LabCodeNo.Split(',').ToList(), fromLabCodeNo);
                }
                else//中心到实验室间的复制
                {
                    IBLL.Common.BaseDictionary.IBSickType ibs = BLLFactory<IBSickType>.GetBLL();
                    br = ibs.CopyToLabByItemNoList(ItemNos.Split(',').ToList(), LabCodeNo.Split(',').ToList());
                }
        ...
        public void GetSubLabItem(string ItemNo, string labcode, ref List<TestItemDetail> listtestitem)
        {
            try
            {
                ...
                if (labcode != null && labcode != "")
                {
                    dsitem = LabGroupItem.GetGroupItemList(ItemNo, labcode);
                }
                else
                    dsitem = CenterGroupItem.GetGroupItemList(ItemNo);
        ...
        public void GetParLabItem(string ItemNo, string labcode, ref List<TestItemDetail> listtestitem)
        {
            try
            {
                DataSet dsitem = null;
                if (labcode != null && labcode != "")
                {
                    dsitem = LabGroupItem.GetGroupItemListBySubItemNo(ItemNo, labcode);
                }
                else
                    dsitem = CenterGroupItem.GetGroupItemListBySubItemNo(ItemNo);

                if (dsitem != null && dsitem.Tables.Count > 0 && dsitem.Tables[0].Rows.Count > 0)
                {
                    for (int i = 0; i < dsitem.Tables[0].Rows.Count; i++)
                    {
                        GetParLabItem(dsitem.Tables[0].Rows[i]["PItemNo"].ToString(), labcode, ref listtestitem);
                        TestItemDetail ttd = new TestItemDetail();
                        ttd.CName = dsitem.Tables[0].Rows[i]["CName"].ToString();
                        ttd.ItemNo = dsitem.Tables[0].Rows[i]["ItemNo"].ToString();
                        ttd.EName = dsitem.Tables[0].Rows[i]["EName"].ToString();
        ...
        */
        ...
        public BaseResultDataValue CreatAESEncryptFile(UIAESEntity jsonentity)
        {
            BaseResultDataValue brdv = new BaseResultDataValue();
            brdv.success = true;
            string webLisServiceName = "WeblisService_yidongcaiyang";//ZhiFang.Common.Public.ConfigHelper.GetConfigString()
            StringBuilder errorInfo = new StringBuilder();
            if (string.IsNullOrEmpty(webLisServiceName))
            {
                errorInfo.Append("WebLisService的应用程序名的配置项为空!");
                brdv.success = false;
            }
            if (jsonentity == null)
            {
            ...
            if (!baseUrl.Contains("http://")) baseUrl = string.Format("http://{0}", baseUrl);
            baseUrl = baseUrl.TrimEnd('/');
            if (string.IsNullOrEmpty(jsonentity.Port))
                jsonentity.Port = HttpContext.Current.Request.Url.Port.ToString();
            if (!baseUrl.Contains(":" + jsonentity.Port)) baseUrl = string.Format("{0}:{1}", baseUrl, jsonentity.Port);
            jsonentity.BaseUrl = baseUrl + "/" + webLisServiceName;
            ZhiFang.Common.Log.Log.Debug("BaseUrl为:" + jsonentity.BaseUrl);
            //移动客户端下载字典名称
            jsonentity.DownloadDictName = "/TransferSamplingService.svc/DownloadDictionaryInfoByLabCode";
            //申请录入增加和修改服务名称
            jsonentity.NrequestFormAddOrUpdateName = "/RequestFormServiceRestfull.svc/NrequestFormAddOrUpdate";
            #endregion
