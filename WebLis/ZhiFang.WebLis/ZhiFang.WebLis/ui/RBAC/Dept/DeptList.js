...
var GetDeptTreeListUrl = Shell.util.Path.rootPath + "/ServiceWCF/RBACService.svc/RBAC_GetDeptTree",
    /**获取下属部门列表服务地址**/
    GetSubDeptListUrl = Shell.util.Path.rootPath + "/ServiceWCF/RBACService.svc/RBAC_GetDeptList",
    /**新增部门服务地址**/
    AddDeptUrl = Shell.util.Path.rootPath + "/ServiceWCF/RBACService.svc/RBAC_AddDept",
    /**删除部门服务地址**/
    DelDeptUrl = Shell.util.Path.rootPath + "/ServiceWCF/RBACService.svc/RBAC_DelDept",
    btnType = "add"
$(function () {
    $("#Tree").tree({
        lines: true,
    ...
    $("#PDept").combobox({
        url: GetSubDeptListUrl + '?guid=' + generateMixed(10) + '&page=1&rows=1000&sort=SN',
        method: 'GET',
        valueField: 'ID',
        textField: 'CName',
        loadFilter: function (data) {
...
function save() {
    ...
    if (btnType == "edit") {
        ...
        var pdept = $("#PDept").combobox('getValue');
        if (pdept == "") {
            errors += 1;
        }
        if (pdept) {
        ...
        $.messager.confirm('提示', '确定要删除?', function (r) {
            if (r) {
                $.ajax({
                    type: 'get',
                    contentType: 'application/json',
                    url: DelDeptUrl + '?DeptId=' + rows[0].ID,
                    dataType: 'json',
                    async: false,
                    success: function (data) {
                        if (data.success == true) {
