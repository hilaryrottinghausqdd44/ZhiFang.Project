...
$(function () {
    ...
    $('#tblBarcodePrice').datagrid({
        border: false,
        collapsible: false,
        idField: 'ClientNo',
        url: Shell.util.Path.rootPath + '/ServiceWCF/NRequestFromService.svc/GetBarcodePrice',
        queryParams: {
            StartDate: date_s,
            EndDate: date_e
        },
        ...
        onLoadSuccess: function (data) {
            ...
            if (data.rows.length) {
                $('#tblBarcodePrice').datagrid('appendRow', {
                    OperDate: '汇总',
                    Barcode: '<span class="subtotal">' + compute("Barcode") + '</span>',
    ...
    function createColumns() {
        var columns = [
            [
                { field: 'OperDate', title: '开单时间', width: fixWidth(0.15) },
                { field: 'ClientName', title: '客户', width: fixWidth(0.15) },
                { field: 'Barcode', title: '条码数', width: fixWidth(0.15) },
                { field: 'Price', title: '金额', width: fixWidth(0.15) }
...
function compute(colName) {
    var rows = $('#tblBarcodePrice').datagrid('getRows');
    var total = 0;
...
function onSearch() {
    ...
    if ($('#tblBarcodePrice').datagrid("options").url) {
        $('#tblBarcodePrice').datagrid('load', param);
    }
    else//初始化列表的情况
    {
        // $('#tblBarcodePrice').datagrid("options").url = serverUrl.SelectReportListUrl;
        $('#tblBarcodePrice').datagrid('load', param);
    }
    return n = 1;
}
...
//生成Excel
function sExcel() {
    $('#tblBarcodePrice').datagrid('options').loadMsg = '正在生成Excel...';
    $('#tblBarcodePrice').datagrid('loading');
    var param = getQueryConfig();

    $.ajax({
        type: 'get',
        contentType: 'application/json',
        url: Shell.util.Path.rootPath + '/ServiceWCF/ReportFromService.svc/GetBarcodePrice',
        dataType: 'json',
        data: param,
    ...
    $('#tblBarcodePrice').datagrid('options').loadMsg = '正在加载数据...';
}
