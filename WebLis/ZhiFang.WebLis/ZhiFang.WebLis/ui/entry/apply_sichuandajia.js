$(function () {
    ...
    //医生
    var DoctorConfig = getComboboxConfig("Doctor", "DoctorNo", "CName", 'DoctorNo,CName,ShortCode', true);
    ...
    function changeHasChildItemData(div, callback) {
        ...
        if (info) {
            ...
            Shell.util.Action.delay(function () {
                var labcode = $("#form_AreaNo").combobox("getValue");
    ...
    function TestItemLoaded(total) {
        ...
        for (var i = 0; i < length; i++) {
            $.ajax({
                url: encodeURI(GetTestItemUrl + "?supergroupno=COMBI&page=1&rows=1&itemkey=" + rows[i].CName + "&labcode=" + labcode),
                dataType: 'json',
