$(function () {
    ...
    /**重新获取就诊类型列表*/
    GetSickTypeUrl = Shell.util.Path.rootPath + "/ServiceWCF/DictionaryService.svc/GetCenterSickTypeListByLab_Area_Center";

    /*特殊项目检测，新冠病毒项目检测，如果是则必须输入身份证号*/
    CheckItemNo = '10020040';
    ...
    $('#form_PersonID').textbox(
        {
            onChange: function (newValue, oldValue) {
                ...
                if (newValue != "" && !reg.test(newValue)) {
                    alert("身份证输入不合法");
                    $('#form_PersonID').textbox("clear");
                    return false;
                }
                else {
                    var UUserCard = newValue;
                    //获取出生日期
                    ...
                    $('#form_GenderNo').combobox('setValue', genderno);
                    //获取年龄
                    var myDate = new Date();
    ...
    $('#form_jztype').combobox({
        ...
        onHidePanel: function () {
            var value = $('#form_jztype').combobox("getValue");
            if (!value || value == "0") {
                $('#form_jztype').combobox("clear");
            }
        }
    });
    //样本类型-下拉框
    ...
    function resetInfo() {
        ...
        var textboxList = ["form_NRequestFormNo", "form_CName", "form_PatNo", "form_WardNo", "form_Diag", "form_Operator", "form_Bed", "form_PersonID"],
			...
			comboboxList = ["form_AgeUnitNo", "form_DeptNo", "form_DistrictNo", "form_Doctor",
                "form_GenderNo", "form_FolkNo", "form_jztype", "form_TestTypeNo"];
        ...
        for (var i in comboboxList) {
            if (comboboxList[i] == "form_AgeUnitNo" || comboboxList[i] == "form_Doctor" || comboboxList[i] == "form_jztype") {
    ...
    function saveInfo() {
        ...
        if (CheckItemNo != "") {
            for (var i = 0; i < jsonentity.BarCodeList.length; i++) {
                for (var j = 0; j < jsonentity.BarCodeList[i].ItemList.length; j++) {
                    if (jsonentity.BarCodeList[i].ItemList[j] == CheckItemNo) {
                        if (!jsonentity.NrequestForm.PersonID || jsonentity.NrequestForm.PersonID == "") {
                            errorInfo.push("<b><span style='color:red;'>包含检测新冠病毒项目的申请单<span style='color:red;'>必须填写身份证号！</span></b>");
                        }
                    }
                }
            }
    ...
    function getNrequestForm() {
        var form = {
            ...
            PersonID: $("#form_PersonID").textbox("getValue"),//身份证号
    ...
    function setNRequestForm(values) {
        ...
        $("#form_PersonID").numberbox("setValue", values.PersonID || ""); //身份证号
    ...
    //重新获取就诊类型列表?labcode=' + $('#txtClientNo').combobox('getValue') + '&page=0&rows=1000'
    function onReloadSickType(callback) {
        var url = GetSickTypeUrl + "?labcode=" + $('#form_ClientNo').combobox('getValue') +"&page=0&rows=1000";
        $('#form_jztype').combobox('reload', url);
    }
    /**
     * 校验条码格式
     * @version 2017-07-19
