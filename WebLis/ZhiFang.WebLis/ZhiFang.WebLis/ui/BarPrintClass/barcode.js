...
Shell.barcode.Print = {
	...
	/**默认的条码模板文件路径*/
	DefaultModelUrl: Shell.util.Path.uiPath + "/BarPrintClass/model.txt",
	/**
	 * 默认模型
	 * 条码 管子颜色
	 * 送检单位
...
//		"LODOP.ADD_PRINT_TEXT(100,10,170,15,\"{CollectDate}\");",
	...
	barcode:function(info,preview,model){
		var lodop = Lodop.getLodopObj("打印条码");
	...
	getContent:function(info,model){
		...
		model = model.replace(/101140101001/g, info.BarCode);//临时办法，后期要解决
		model = model.replace(/\[送检单位\]/g, info.ClientName);
		model = model.replace(/\[科室\]/g, info.DeptNo);
		model = model.replace(/\[姓名\]/g, info.Name);
		model = model.replace(/\[性别\]/g, info.Sex);
		model = model.replace(/\[年龄\]/g, info.Age);
		model = model.replace(/\[年龄单位\]/g, info.AgeUnit);
		model = model.replace(/\[就诊类型\]/g, info.SickTypeName);
		model = model.replace(/\[病历号\]/g, info.PatNo);
		model = model.replace(/\[送检类型\]/g, info.TestTypeName);
		model = model.replace(/\[颜色\]/g, info.ColorName);
		model = model.replace(/\[所有项目\]/g, info.ItemList);
		model = model.replace(/\[采样时间\]/g, info.CollectDate);
		return model;
	},
	/**获取默认的面板文件内容*/
