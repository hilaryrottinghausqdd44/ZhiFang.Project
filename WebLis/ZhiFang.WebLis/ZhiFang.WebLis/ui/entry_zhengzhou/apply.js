$(function(){
	...
	//------------文字框-------------
	//申请号-只读
    $('#form_NRequestFormNo').numberbox({required:true});
	//姓名-必填
    $('#form_CName').textbox({required:true,missingMessage:"该输入项为必输项"});
    //病历号
    $('#form_PatNo').textbox({required:false,missingMessage:"该输入项为可选项"});
    //病房
	...
	function getNrequestForm(){
		var form = {
			NRequestFormNo:$("#form_NRequestFormNo").numberbox("getValue") ,//申请号
		...
		if(!form.NRequestFormNo) errorList.push("<b><span style='color:red;'>申请号</span>必须填写！</b>");
	...
	function setNRequestForm(values){
		$("#form_NRequestFormNo").numberbox("setValue",values.NRequestFormNo || "");//申请号
