...
$(function () {
    $("#dg").datagrid({
        ...
        collapsible: false,
        idField: 'SickTypeNo',
        url: Shell.util.Path.rootPath + '/ServiceWCF/DictionaryService.svc/GetPubDict?tableName=' + "SickType",
        // url: 'data.txt',
        method: 'get',
        striped: true,
        columns: [[
            { field: 'cb', checkbox: 'true' },
            { field: 'SickTypeNo', title: '编码', width: '28%' },
            { field: 'CName', title: '中文名称', width: '29%' },
            { field: 'ShortCode', title: '简码', width: '29%' },
            {
    ...
    $('#txtSickTypeNo').numberbox({

        validType: 'length[0,10]',
        onChange: function (newValue, oldValue) {
            var SickTypeNo = $('#txtSickTypeNo').numberbox('getValue');
            if (btnType == 'add') {
                if (SickTypeNo.length > 10) {
                    $.ajax({
                        success: function () {
                            $('#txtSickTypeNo').numberbox('clear');
                        }
                    })
                } else {
                    $.ajax({
                        ...
                        data: { filerValue: newValue.trim(), tablename: "SickType", precisequery: "SickTypeNo" },
                        dataType: "json",
...
function show_confirm() {
    ...
    var sicktyperows = $('#dg').datagrid('getSelections');
    var sicktypeno = [];
    for (var i = 0; i < sicktyperows.length; i++) {
        sicktypeno.push(sicktyperows[i].SickTypeNo);
    }
    if (r == true) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $.ajax({
                        type: 'post',
                        contentType: 'application/json',
                        url: Shell.util.Path.rootPath + '/ServiceWCF/DictionaryService.svc/BatchCopySickTypeToLab?ItemNos=' + sicktypeno.join(',') + '&LabCodeNo=' + LABCODENO,
                        async: false,
                        dataType: 'json',
                        success: function (data) {
                            if (data.success == true) {
...
function show_confirm1() {
    ...
    //        contentType: 'application/json',
    //        //url: Shell.util.Path.rootPath + '/ServiceWCF/DictionaryService.svc/CopyAllToLabs',
    //        url: Shell.util.Path.rootPath + '/ServiceWCF/DictionaryService.svc/BatchCopySickTypeToLab',
    //        async: false,
    //        dataType: 'json',
    //        //data: {
    //        //    DicTable: "SickType",
    ...
    $.ajax({
        ...
        contentType: 'application/json',
        //url: Shell.util.Path.rootPath + '/ServiceWCF/DictionaryService.svc/CopyAllToLabs',
        url: Shell.util.Path.rootPath + '/ServiceWCF/DictionaryService.svc/BatchCopySickTypeToLab?ItemNos=' + sicktypeno.join(',') + '&LabCodeNo=' + rLABCODENO,
        async: false,
        dataType: 'json',
        success: function (data) {
            if (data.success == true) {
            ...
            if (r) {
                for (var i = 0; i < rows.length; i++) {
                    $.ajax({
                        type: 'get',
                        contentType: 'application/json',
                        url: Shell.util.Path.rootPath + '/ServiceWCF/DictionaryService.svc/DeleteSickTypeModelByID?SickTypeNo=' + rows[i].SickTypeNo,
                        dataType: 'json',
                        async: false,
                        success: function (data) {
                            if (data.success == true) {
...
function edit(index, value) {
    ...
    // $('#txtSickTypeNo').textbox("readonly", true);
    $('#txtSickTypeNo').textbox('disable');
...
function save() {
    if (btnType == 'edit') {
        ...
        var r = '';
        r += '{"jsonentity":{';
        var SickTypeNo = $("#txtSickTypeNo").val();
        if (SickTypeNo == "") {
            errors += 1;
        }
        if (SickTypeNo) {
            r += '"SickTypeNo":' + SickTypeNo + ',';
        }
        ...
        // var r = '{"jsonentity":{"SampleTypeNo":"' + SampleTypeNo + '","CName":"' + CName + '","ShortCode":"' + ShortCode + '","HisOrderCode":"' + HisOrderCode + '","DispOrder":"' + DispOrder + '"}}'
        r += '}}'
        if (errors > 0) {
            ...
            $.ajax({
                type: 'post',
                contentType: 'application/json',
                url: Shell.util.Path.rootPath + '/ServiceWCF/DictionaryService.svc/UpdateSickTypeModelByID',
                data: r,
                dataType: 'json',
                success: function (data) {
                    if (data.success == true) {
        ...
        if (errors > 0) {
            ...
            $.ajax({
                type: 'post',
                contentType: 'application/json',
                url: Shell.util.Path.rootPath + '/ServiceWCF/DictionaryService.svc/AddSickTypeModel',
                data: r,
                dataType: "json",
                success: function (data) {
                    if (data.success == true) {
