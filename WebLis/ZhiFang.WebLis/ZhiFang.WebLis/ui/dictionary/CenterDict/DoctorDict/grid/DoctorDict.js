...
$(function () {
    $("#dg").datagrid({
        ...
        collapsible: false,
        idField: 'DoctorNo',
        url: Shell.util.Path.rootPath + '/ServiceWCF/DictionaryService.svc/GetPubDict?tableName=' + "Doctor",
       // url: 'data.txt',
        method: 'get',
        striped: true,
        columns: [[
                 { field: 'cb', checkbox: 'true' },
                { field: 'DoctorNo', title: '项目编码', width: '28%' },
    ...
    $('#txtDoctorNo').numberbox({
       
        validType: 'length[0,10]',
        onChange: function (newValue, oldValue) {
        var DoctorNo = $('#txtDoctorNo').numberbox('getValue');
        if (btnType == 'add') {
            if (DoctorNo.length > 10)
            {
                $.ajax({
                    success: function () {
                        $('#txtDoctorNo').numberbox('clear');
                ...
                $.ajax({
                    ...
                    data: { filerValue: newValue.trim(), tablename: "Doctor", precisequery: "DoctorNo" },
            ...
            if (r) {
                for (var i = 0; i < rows.length; i++) {
                    $.ajax({
                        type: 'get',
                        contentType: 'application/json',
                        url: Shell.util.Path.rootPath + '/ServiceWCF/DictionaryService.svc/DeleteDoctorModelByID?DoctorNo=' + rows[i].DoctorNo,
                        dataType: 'json',
                        async: false,
                        success: function (data) {
                            if (data.success == true) {
...
function save() {
    if (btnType == "edit") {
        ...
        // var r = '{"jsonentity":{"DoctorNo":"' + DoctorNo + '","HisOrderCode":"' + HisOrderCode + '","CName":"' + CName + '","Visible":"' + Visible + '","ShortCode":"' + ShortCode + '"}}';
        r += '}}'
        if (errors > 0) {
            ...
            $.ajax({
                type: 'post',
                contentType: 'application/json',
                url: Shell.util.Path.rootPath + '/ServiceWCF/DictionaryService.svc/UpdateDoctorModelByID',
                data: r,
                dataType: 'json',
                success: function (data) {
                    if (data.success == true) {
       ...
       // var r = '{"jsonentity":{"DoctorNo":"' + DoctorNo + '","HisOrderCode":"' + HisOrderCode + '","CName":"' + CName + '","Visible":"' + Visible + '","ShortCode":"' + ShortCode + '"}}';
        ...
        if (errors > 0) {
            ...
            $.ajax({
                ...
                url: Shell.util.Path.rootPath + '/ServiceWCF/DictionaryService.svc/AddDoctorModel',
                data: r,
                dataType: "json",
                success: function (data) {
                    if (data.success == true) {
