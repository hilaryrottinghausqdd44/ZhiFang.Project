...
$(function () {
    $("#dg").datagrid({
        ...
        collapsible: false,
        idField: 'FolkNo',
        url: Shell.util.Path.rootPath + '/ServiceWCF/DictionaryService.svc/GetPubDict?tableName=' + "FolkType",
       // url: 'data.txt',
        method: 'get',
        striped: true,
        columns: [[
                { field: 'cb', checkbox: 'true' },
                { field: 'FolkNo', title: '项目编码', width: '28%' },
    ...
    $('#txtFolkNo').numberbox({
        ...
        var FolkNo = $('#txtFolkNo').numberbox('getValue');
        if (btnType == 'add') {
            if (FolkNo.length > 10)
            {
                $.ajax({
                    success: function () {
                        $('#txtFolkNo').numberbox('clear');
                ...
                $.ajax({
                    ...
                    data: { filerValue: newValue.trim(), tablename: "FolkType", precisequery: "FolkNo" },
            ...
            if (r) {
                for (var i = 0; i < rows.length; i++) {
                    $.ajax({
                        type: 'get',
                        contentType: 'application/json',
                        url: Shell.util.Path.rootPath + '/ServiceWCF/DictionaryService.svc/DeleteFolkTypeModelByID?FolkNo=' + rows[i].FolkNo,
                        dataType: 'json',
                        async: false,
                        success: function (data) {
                            if (data.success == true) {
    ...
    if(btnType=='edit')
    {
        ...
        if (errors > 0) {
            ...
            $.ajax({
                type: 'post',
                contentType: 'application/json',
                url: Shell.util.Path.rootPath + '/ServiceWCF/DictionaryService.svc/UpdateFolkTypeModelByID',
                data: r,
                dataType: 'json',
                success: function (data) {
                    if (data.success == true) {
        ...
        if (errors > 0) {
            ...
            $.ajax({
                ...
                url: Shell.util.Path.rootPath + '/ServiceWCF/DictionaryService.svc/AddFolkTypeModel',
                data: r,
                dataType: "json",
                success: function (data) {
                    if (data.success == true) {
