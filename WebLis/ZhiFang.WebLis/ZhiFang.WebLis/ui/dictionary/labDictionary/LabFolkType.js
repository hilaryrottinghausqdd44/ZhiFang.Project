...
$(function(){
    ...
    $('#LabFolkNo').textbox({onChange:function(newValue,oldValue){
        if(btnType=='add' && newValue.length<=10)
            askService('validate',null,{precisequery:"LabFolkNo",filerValue:newValue,labcode:LabCode,tablename:"B_Lab_FolkType"});//验证主键的唯一性
    }});
});
...
function createColumns() {
    var columns = [
        [
            ...
            {field: 'LabFolkNo', title: '编码', width: fixWidth(0.2) },
        ...
        success: function (data) {
            if (data.success) {
                switch (serviceType) {
                    ...
                    case'edit':
                    case'delete':
                        $('#tbFolkType').datagrid('load',{tablename:"B_Lab_FolkType",filerValue:null,labcode:LabCode,time:new Date().getTime()});
                        $('#txtSearch').searchbox('setValue',null);//增删改成功后，重新加载数据请求
                        break;

                    case'search':
                        $('#tbFolkType').datagrid('loadData',data);
                        break;
                    case 'validate':
                ...
                (ErrorInfo.indexOf('此项已对照')>-1)?contrastList.push(where.labClientNo):null;
            }
        },
        error: function (data) {
...
function setService(serviceType, serviceParam, where) {
    ...
    switch (serviceType) {
        case'search':
            ...
            break;
        case'delete':
            serviceParam.serviceName = 'DeleteLabFolkTypeModelByID';//删除数据服务名
            serviceParam.type = 'GET';//数据请求方式GET
            serviceParam.data =  where;//发送到服务器的数据，根据ID号进行删除
            break;
        case'add':
            serviceParam.serviceName = 'AddLabFolkTypeModel';//增加数据服务名
            serviceParam.type = 'POST';//数据请求方式GET
            break;
        case'edit':
            serviceParam.serviceName = 'UpdateLabFolkTypeModelByID';//编辑数据服务名UpdateReportGroupModelByID
            serviceParam.type = 'POST';//数据请求方式GET
            break;
        case'validate':
...
function getEntity(getType) {
    ...
    var LabFolkNo=$('#LabFolkNo').numberbox('getValue');
    if(!LabFolkNo)
        return 1;
    result+='"LabFolkNo":'+LabFolkNo+',';
...
function addRow(){
    ...
    $('#LabFolkNo').textbox('enable');
    setCombobox([]);//设置下拉框
}

//修改
function editRow(index){
    var curData = $('#tbFolkType').datagrid('getData'),//返回当前页加载完毕的数据
...
//删除
function deleteRow(){
    var rows = $('#tbFolkType').datagrid('getSelections') ||[],
        ...
        $.messager.confirm('确认', '确认删除吗？', function (btn) {
            if (btn) {
                for (var i = 0; i < length; i++) {
                    ...
                    askService('delete', null,{labClientNo:rows[i]['LabFolkNo'],labcode:LabCode});
                }
                $('#tbFolkType').datagrid('clearSelections');//因为getSelections会记忆选过的记录，所以要清空一下
        ...
        $.messager.confirm('确认', '确认禁用所选记录吗？', function (btn) {
            if (btn) {
                for (var i = 0; i < rows.length; i++) {
                    if(rows[i]['UseFlag']){
                        index=$('#tbFolkType').datagrid('getRowIndex',rows[i]);
...
function exportExcel(){
    $('#ifmExport').attr('src',Shell.util.Path.rootPath + '/ServiceWCF/DictionaryService.svc/DictionaryExportExcel?table=B_Lab_FolkType&labcode='+LabCode);
}
...
function disableOrEnable(index,UseFlag){
    ...
    result += '{"jsonentity":{';//构造符合格式的字符串
    result+='"LabFolkNo":'+curRow.LabFolkNo+',';
    result+='"LabCode":'+LabCode+',';
...
function search(value){
    $('#tbFolkType').datagrid('load',{tablename:"B_Lab_FolkType",labcode:LabCode,filerValue:value,time:new Date().getTime()});
}
