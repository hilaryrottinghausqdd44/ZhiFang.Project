...
$(function(){
    ...
    $('#LabDeptNo').textbox({onChange:function(newValue,oldValue){
        if(btnType=='add' && newValue.length<=10)
            askService('validate',null,{precisequery:"LabDeptNo",filerValue:newValue,labcode:LabCode,tablename:"B_Lab_Department"});//验证主键的唯一性
    }});
});
...
function createColumns() {
    var columns = [
        [
            ...
            {field: 'LabDeptNo', title: '编码', width: fixWidth(0.2) },
        ...
        success: function (data) {
            if (data.success) {
                switch (serviceType) {
                    ...
                    case'edit':
                    case'delete':
                        $('#tbDepartment').datagrid('load',{tablename:"B_Lab_Department",filerValue:null,labcode:LabCode,time:new Date().getTime()});
                        $('#txtSearch').searchbox('setValue',null);//增删改成功后，重新加载数据请求
                        break;
                    case'search':
                        $('#tbDepartment').datagrid('loadData',data);
                        break;
                    case 'validate':
                ...
                var ErrorInfo=data.ErrorInfo;
                   (ErrorInfo.indexOf('此项已对照')>-1)?contrastList.push(where.LabDeptNo):null;
            }
        },
        error: function (data) {
...
function setService(serviceType, serviceParam, where) {
    ...
    switch (serviceType) {
        ...
        case'add':
            ...
            break;
        case'edit':
            serviceParam.serviceName = 'UpdateLabDepartmentModelByID';//编辑数据服务名UpdateReportGroupModelByID
            serviceParam.type = 'POST';//数据请求方式GET
            break;
        case'validate':
...
function getEntity(getType) {
    ...
    var LabDeptNo=$('#LabDeptNo').numberbox('getValue');
    if(!LabDeptNo)
        return 1;
    result+='"LabDeptNo":'+LabDeptNo+',';
...
function addRow(){
    ...
    $('#LabDeptNo').textbox('enable');
    setCombobox([]);//设置下拉框
}

//修改
function editRow(index){
    var curData = $('#tbDepartment').datagrid('getData'),//返回当前页加载完毕的数据
...
//删除
function deleteRow(){
    var rows = $('#tbDepartment').datagrid('getSelections') ||[],
        ...
        $.messager.confirm('确认', '确认删除吗？', function (btn) {
            if (btn) {
                for (var i = 0; i < length; i++) {
                    askService('delete', null,{LabDeptNo:rows[i]['LabDeptNo'],labcode:LabCode});
                }
                $('#tbDepartment').datagrid('clearSelections');//因为getSelections会记忆选过的记录，所以要清空一下
        ...
        $.messager.confirm('确认', '确认禁用所选记录吗？', function (btn) {
            if (btn) {
                for (var i = 0; i < rows.length; i++) {
                    if(rows[i]['UseFlag']){
                        index=$('#tbDepartment').datagrid('getRowIndex',rows[i]);
...
function exportExcel(){
    $('#ifmExport').attr('src',Shell.util.Path.rootPath + '/ServiceWCF/DictionaryService.svc/DictionaryExportExcel?table=B_Lab_Department&labcode='+LabCode);
}
...
function disableOrEnable(index,UseFlag){
    ...
    result += '{"jsonentity":{';//构造符合格式的字符串
    result+='"LabDeptNo":'+curRow.LabDeptNo+',';
    result+='"LabCode":'+LabCode+',';
...
function search(value){
    $('#tbDepartment').datagrid('load',{tablename:"B_Lab_Department",labcode:LabCode,filerValue:value,time:new Date().getTime()});
}
