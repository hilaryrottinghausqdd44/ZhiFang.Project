...
$(function () {
    ...
    $('#leftItem').datagrid({
        ...
        queryParams:{
            tablename:"B_Lab_TestItemControl",
...
function createColumns() {
    var columns = [
        [
            ...
            {field: 'LabItemNo', title: '实验室项目编号', width: fixWidth(0.2),sortable:true},
            { field: 'CName', title: '实验室项目名称', width: fixWidth(0.3), sortable: true },
            { field: 'ShortCode', title: '实验室项目简码', width: fixWidth(0.2), sortable: true },
...
function matchColumns() {
    var columns = [
        [
            { field: 'ItemNo', title: '中心项目编号', width: fixWidth(0.2), sortable: true },
            { field: 'CName', title: '中心项目名称', width: fixWidth(0.3), sortable: true },
            { field: 'ShortCode', title: '中心项目简码', width: fixWidth(0.2), sortable: true }
        ]
    ];
    return columns;
}
...
function setService(serviceType, serviceParam, entity) {
    ...
    switch (serviceType) {
        case'add':
            serviceParam.serviceName = 'UpdateOrAddTestItemControlModelByID';//增加数据服务名
            serviceParam.type = 'POST';//数据请求方式GET
            serviceParam.data = entity;//向服务器传递的数据
            break;
        case'edit':
            serviceParam.serviceName = 'UpdateOrAddTestItemControlModelByID';//编辑数据服务名
            serviceParam.type = 'POST';//数据请求方式POST
            serviceParam.data = entity;//向服务器传递的数据
            break;
        case'delete':
            serviceParam.serviceName = 'DelTestItemControlModelByID';//编辑数据服务名
...
function labSearch(value) {
    $('#leftItem').datagrid('load', {filervalue: value.trim(),labcode:LabCode,tablename:"B_Lab_TestItemControl",selectedflag:labContrast,time:new Date().getTime()});
    if(matchRow){
        matchRow.css("background-color",'');
    }
}
...
function matchSingle() {
    ...
    for(var i=0;i<rightLength;i++){
        if(leftRow.CenterItemNo==rightRows[i].ItemNo){
            ...
            //先解除左边的对照关系
            var delID =  'id='+LabCode+'_' +leftRow.CenterItemNo+'_'+ leftRow.LabItemNo;
            ...
            for(var j=0;j<leftLength;j++){
                if(leftRows[j].CenterItemNo===rightRow.ItemNo){
                    leftIndex= $('#leftItem').datagrid('getRowIndex', leftRows[j]);
                    //先解除右边的对照关系
                    var delID =  'id='+LabCode+'_' +leftRows[j].CenterItemNo+'_'+ leftRows[j].LabItemNo;
                    askService('delete', delID);//请求服务器

                    //动态改变原始表数据
                    for(var x=0;x<labRows.length;x++){
                        if(leftRows[j].LabItemNo===labRows[x]['LabItemNo']){
                            labRows[x]['CenterCName']=null;
                            labRows[x]['CenterItemNo']=null;
                            break;
                        }
            ...
            if(!leftExist && leftLength<labRows.length){
                for(var k=0;k<labRows.length;k++){
                    if(labRows[k].CenterItemNo===rightRow.ItemNo){
                        leftIndex= 0;
                        //先解除右边的对照关系
                        var delID =  'id='+LabCode+'_' +labRows[k].CenterItemNo+'_'+ labRows[k].LabItemNo;
                        askService('delete', delID);//请求服务器

                        //动态改变原始表数据
                        labRows[k]['CenterCName']=null;
                        labRows[k]['CenterItemNo']=null;
                        leftExist=true;
                        break;
                    }
                }
    ...
    if(!rightExit){
        ...
        for(var i=0;i<leftLength;i++){
            if(leftRows[i].CenterItemNo==rightRow.ItemNo){

                leftIndex= $('#leftItem').datagrid('getRowIndex', leftRows[i]);
                //先解除右边的对照关系
                var delID =  'id='+LabCode+'_' +leftRows[i].CenterItemNo+'_'+ leftRows[i].LabItemNo;
                askService('delete', delID);//请求服务器

                //动态改变原始表数据
                for(var x=0;x<labRows.length;x++){
                    if(leftRows[i].LabItemNo===labRows[x]['LabItemNo']){
                        labRows[x]['CenterCName']=null;
                        labRows[x]['CenterItemNo']=null;
                        break;
                    }
        ...
        if(!leftExist && leftLength<labRows.length){
            for(var j=0;j<labRows.length;j++){
                if(labRows[j].CenterItemNo==rightRow.ItemNo){

                    leftIndex= 0;
                    //先解除右边的对照关系
                    var delID =  'id='+LabCode+'_' +labRows[j].CenterItemNo+'_'+ labRows[j].LabItemNo;
                    askService('delete', delID);//请求服务器

                    //动态改变原始表数据
                    labRows[j]['CenterCName']=null;
                    labRows[j]['CenterItemNo']=null;
                    leftExist=true;
                    break;
                }
            }
