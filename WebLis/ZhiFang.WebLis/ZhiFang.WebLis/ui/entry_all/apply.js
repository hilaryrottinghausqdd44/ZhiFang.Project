...
//          $('#itemtype_grid').datagrid("reload");
    ...
    function createDiv(obj) {
    	...
    	if(obj.isCombiItem == "1"){
    		mask = '<span style="padding:1px;color:orange;border:1px solid orange;">组</span>';
    	}else{
    		mask = '<span style="padding:1px;color:green;border:1px solid green;">单</span>';
    	}
    	
        var div =
		"<div style='padding:1px;' mark='" + itemDiv + obj.ItemNo + "' " + "data='" + Shell.util.JSON.encodeValue(obj) + "'>" +
			mask + "<span style='padding:1px;'>" + obj.CName + "</span></br>" +
			"<span style='padding:1px;'>(" + obj.ItemNo + ")</span>" +
		"</div>";
        return div;
    }
    /**创建组合列表*/
    ...
    function changeHasChildItemData(div, callback) {
        ...
        var value = div.getAttribute('data'), //attributes[0].value,//兼容性bug
        	value = Shell.util.JSON.decode(value),
			itemno = value.ItemNo,
    ...
    function TestItemLoaded(total) {
        ...
        for (var i = 0; i < length; i++) {
            $.ajax({
                url: encodeURI(GetTestItemUrl + "?supergroupno=All&page=1&rows=1&itemkey=" + rows[i].CName + "&labcode=" + labcode),
                dataType: 'json',
    ...
    function uncheckgridLoad() {
        ...
        if (rowData) {
            ...
            GetTestItem("All", changeUncheckecGrid);
        }
    }
    ...
    function resetInfo() {
        ...
        var textboxList = ["form_NRequestFormNo", "form_CName", "form_PatNo", "form_WardNo", "form_Diag", "form_Operator", "form_Bed"],
...
//      });
    ...
    function getLabItem(labcode,callback){
        $.ajax({
            dataType: 'json',
            contentType: 'application/json',
            url: GetTestItemUrl + "?supergroupno=All&page=1&rows=1&labcode=" + labcode,
            success: function (result) {
                if (result.success) {
    ...
    function NrequestFormAddOrUpdate(entity, callback) {
    	$('#save_barcode_list').linkbutton('disable');//禁用按钮
        $.ajax({
            type: 'post',
            dataType: 'json',
            contentType: 'application/json',
            ...
            success: function (result) {
                if (result.success) {
                    ...
                    $('#save_barcode_list').linkbutton('enable');//启用按钮
                }
            },
            error: function (request, strError) {
    ...
    function openDoctorWin(){
    	//清空内容
    	$("#form_Doctor_Add_Name").val("");
    	$("#form_Doctor_Add_No").numberbox('clear');
    	$("#form_Doctor_Add_ShortCode").val("");
    	//显示窗口
