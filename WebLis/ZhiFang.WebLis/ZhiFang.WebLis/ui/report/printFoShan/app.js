$(function(){
	...
	var errorInfo = [],
		...
		serverUrl = {
			/**获取报告列表服务地址*/
			SelectReportListUrl:Shell.util.Path.rootPath + "/ServiceWCF/ReportFromService.svc/SelectReportListFoshan",
			/**获取报告服务地址*/
	...
	function setService(serviceType, serviceParam, where) {
		...
		switch (serviceType) {
			...
			case'print':
				...
				break;
			case'load':
				serviceParam.serviceName = 'ReportFromService.svc/SelectReportListFoshan';//SelectReportList2
	...
	function getWhere(){
		var date_s = $("#search-text-RECEIVEDATE-S").datebox("getValue"),
			...
			SectionNo = $("#search-text-PGroup").combobox("getValue"),
			personID = $("#search-text-personID").searchbox("getValue"),
	...
	//默认展示嵌套列表第一条记录
    function getSubGridFirstRow(CNAME,PERSONID){
		var Param={},
			date_s = $("#search-text-RECEIVEDATE-S").datebox("getValue"),
			date_e = $("#search-text-RECEIVEDATE-E").datebox("getValue");

		Param.Startdate=date_s?date_s:null;
		Param.Enddate=date_e?date_e:null;
		Param.CNAME=CNAME?CNAME:null;
		Param.PERSONID=PERSONID?PERSONID:null;
		Param.page=1;
		Param.rows=10;
		$.ajax({
			url: Shell.util.Path.rootPath + "/ServiceWCF/ReportFromService.svc/SelectReportList2",
			data: Param,
			dataType: 'json',
			type: 'GET',
			timeout: 10000,
	...
	//获取嵌套列表勾选数据
	function getSubGridCheckedRows(index){
			var rows=arrySubGrid[index].datagrid('getChecked') || [],
				length=rows.length || 0,
				person;

			for(var i=0;i<length;i++){
				person={};
				person.ReportFormID=rows[i]["ReportFormID"];
				person.CNAME=rows[i]["CNAME"];;
				person.RECEIVEDATE=rows[i]["RECEIVEDATE"];
				person.SAMPLENO=rows[i]["SAMPLENO"];
				checkedRows.push(person);
				allCheckedRows.push(person);
			}
	}

	//获取嵌套列表所有数据
	function getSubGridRows(CNAME,PERSONID){
			...
			var	date_s = $("#search-text-RECEIVEDATE-S").datebox("getValue"),
				...
				Param={},
				isLocal=false,
				length=allCheckedRows.length || [];

			/*for(var i=0;i<length;i++){
				if(allCheckedRows[i]['subIndex'].indexOf(index+'_')>-1){//原始表已存在嵌套表数据
						checkedRows.push(allCheckedRows[i]);
					isLocal=true;
				}
			}
			if(isLocal){
	...
	function printReport(preview){
		...
		var list = [],//$('#report_grid').datagrid("getChecked") ||
			rows=$('#report_grid').datagrid('getRows') || [],
			length=rows.length,
			len = list.length,
		...
		checkedRows=[];
		for(var i=0;i<length;i++){
		var className=$('#datagrid-row-r1-2-'+i).attr('class').toString();
			if(arrySubGrid[i].toString().indexOf('sub_')>-1){//嵌套列表关闭
				if(className.indexOf('checked')>-1){
					getSubGridRows(rows[i]['CNAME'],rows[i]['PERSONID']);
				}
			}else{//嵌套列表展开
				getSubGridCheckedRows(i);
			}
		}
	...
	function getReportCheckedData(){
		...
		var result={};
		result.total=checkedRows.length || 0;
		result.rows=checkedRows || [];
		return result;
	}
	...
	function winPdfPrint(){
		...
		var url = Shell.util.Path.uiPath + "/report/printFoShan/printPDF.html";
    ...
    $('#report_grid').datagrid({
		...
		remoteSort:false,
		columns:[[
            {field:'ReportFormID',title:'主键',checkbox:true,hidden:false},
            ...
            {field:'SAMPLENO',title:'样本号',sortable:true,width:100,hidden:true},
			{field:'PRINTTIMES',title:'打印次数',sortable:true,width:100,hidden:true},
			{field:'PERSONID',title:'身份证号',sortable:true,width:100},
			{field:'PATNO',title:'诊疗卡号',sortable:true,width:100,hidden:true},
			{field:'ZDY8',title:'编号',width:70,sortable:true,hidden:true},
		...
		onLoadSuccess:function(data){
			...
			if(data.total == 0){
				...
				$('#report_grid').datagrid('unselectAll');
				pageRows=data.rows.length;
				arrySubGrid=[];
				for(var i=1;i<=pageRows;i++){
					arrySubGrid.push('sub_'+i);
				}
				//$('#report_grid').datagrid("selectRow",0);
			}
		},
		onSelect:function(rowIndex,rowData){
			//getSubGridFirstRow(rowData.CNAME,rowData.PERSONID);
		},
		...
		onExpandRow:function(index,row){
			var ddv = $(this).datagrid('getRowDetail',index).find('table.ddv'),
				 ...
				 date_e = $("#search-text-RECEIVEDATE-E").datebox("getValue"),
				  Param={},
				subObject,
				rows=$('#report_grid').datagrid('getChecked') || [];

			Param.Startdate=date_s?date_s:null;
			Param.Enddate=date_e?date_e:null;
			Param.CNAME=row.CNAME?row.CNAME:null;
			Param.PERSONID=row.PERSONID?row.PERSONID:null;
			Param.page=1;
			Param.rows=90;
			subObject=ddv.datagrid({
				method:'GET',
				url:Shell.util.Path.rootPath + "/ServiceWCF/ReportFromService.svc/SelectReportList2",
				queryParams:Param,
				fitColumns:true,
				...
				columns:[[
					...
					{field:'PRINTTIMES',title:'打印次数',sortable:true,width:100},
					{field:'PERSONID',title:'身份证号',sortable:true,width:100},
					{field:'PATNO',title:'诊疗卡号',sortable:true,width:100},
					{field:'ZDY8',title:'编号',width:70,sortable:true,hidden:true}
				]],
				onResize:function(){
					$('#report_grid').datagrid('fixDetailRowHeight',index);
				},
				loadFilter:function(data){
					if(data.success){
						var data=eval('('+data.ResultDataValue+')');
			...
			$('#report_grid').datagrid('fixDetailRowHeight',index);
            arrySubGrid[index]=subObject;
		},
		onCollapseRow:function(index,row){
             arrySubGrid[index]='sub_'+(index+1);
		}
	});
	...
	//查询身份证
	$("#search-text-personID").searchbox({height:22,width:120,prompt:'请输入身份证号',searcher:onSearch});
	//查询诊疗卡号
	$("#search-text-patNO").searchbox({height:22,width:120,prompt:'请输入诊疗卡号',searcher:onSearch});
	//查询-检验小组
