$(function () {
    ...
    /*选择未打印、已打印报告单*/
    function SelectPrintStatus(flag) {
        if (flag) {
            //alert($("#UnPrint").is(':checked'));
            var a = $('#report_grid').datagrid("getRows");
            //alert(a.length);
            if ($("#UnPrint").is(':checked')) {
                for (var i = 0; i < a.length; i++) {
                    if (a[i].PRINTTIMES == 0) {
                        $('#report_grid').datagrid("checkRow", i);
                    }
                }
            }
            else {
                for (var i = 0; i < a.length; i++) {
                    if (a[i].PRINTTIMES == 0) {
                        $('#report_grid').datagrid("uncheckRow", i);
                    }
                }
            }
        }
    ...
    function getGroupWhere() {
        if ($("#search-button-today").linkbutton("options").selected) {
            ...
            return "RECEIVEDATE<'" + date_e + "' and RECEIVEDATE>='" + date_s + "'";
        }
    ...
    function getRECEIVEDATE() {
        ...
        var s = Shell.util.Date.getNextDate(date_s, 365).getTime(),
    		e = (date_e ? Shell.util.Date.getDate(date_e) : new Date()).getTime();
        if (s < e) return null;

        //生成时间条件
        if (date_s) { where.push("RECEIVEDATE>='" + Shell.util.Date.toString(date_s, true) + "'"); }
        if (date_e) { where.push("RECEIVEDATE<'" + Shell.util.Date.toString(Shell.util.Date.getNextDate(date_e), true) + "'"); }
    ...
    function getTextWhere() {
        ...
        if (ClientNo && ClientNo != "0" && ClientNo != "") {
            where.push("ClientNo='" + ClientNo.replace(/'/g, "") + "'");
        }
        else {
            var tmpclientnolist = $("#search-text-ClientNo").combobox("getData");
            var c = "";
            alert(tmpclientnolist[0].ClIENTNO);
            for (var i = 0; i < tmpclientnolist.length; i++) {
                if (i == tmpclientnolist.length - 1) {
                    c = c + tmpclientnolist[i].ClIENTNO.replace(/'/g, "");
                }
                else {
                    c = c + tmpclientnolist[i].ClIENTNO.replace(/'/g, "") + ",";
                }
            }
            where.push("ClientNo in (" + c + ") ");
        }
        ...
        if (BARCODE) { where.push("BARCODE='" + BARCODE.replace(/'/g, "") + "'"); }
    ...
    function printReport(preview) {
        ...
        if (fileType == "PDF") {
            ...
            ReportFormIDArr = [];
            for (var i = 0; i < len; i++) {
                GetReportPrintList(list[i].ReportFormID, reportformtitle, function (result, reportformId) {
                    if (result.success) {
                        var list = Shell.util.JSON.decode(result.ResultDataValue) || [];
    ...
    function winPdfPrint() {
        ...
        var url = Shell.util.Path.uiPath + "/report/print/printPDF.html?reportformtitle=" + reportformtitle; ;
    ...
    //选中未打印的报告单
    $("#UnPrint").bind('click', function () { SelectPrintStatus(true); });
    //选中已打印的报告单
    $("#Printed").bind('click', function () { SelectPrintStatus(false); });
    ...
    $('#report_grid').datagrid({
        ...
        onLoadSuccess: function (data) {
            ...
            $("#report_grid").parent().find("div .datagrid-header-check").children("input[type=\"checkbox\"]").eq(0).attr("style", "display:none;");
        },
    ...
    //$.ajax()请求服务 gwh add 2015-3-30
    function askService(where) {
        $.ajax({
            url: Shell.util.Path.rootPath + '/ServiceWCF/PrintService.svc/DownLoadReport',
            data: where,
            dataType: 'json',
            type: 'POST',
            timeout: 10000,
            ...
            success: function (data) {
                if (data.success) {
                    ...
                    $('#downLoadReportForm').attr('href', Shell.util.Path.rootPath + '/' + result);
    ...
    $.DownLoad = function (index) {
        ...
        //PKI默认抬头是CLINET
        askService('{"jsonentity":{"reportformIds":"' + ReportFormID + '","reportformtitle":"CLIENT"}}');
    }
    //批量下载
    ...
    $('#search-text-ClientNo').combobox({
        ...
        editable: true,
        method: 'GET',
        url: Shell.util.Path.rootPath + "/ServiceWCF/DictionaryService.svc/GetClientListByRBAC?page=1&rows=1000&fields=CLIENTELE.CNAME,CLIENTELE.ClIENTNO,SHORTCODE_CNAME",
        loadFilter: function (data) {
            data = data || [];
    ...
    //查询-样本号
    $("#search-text-SAMPLENO").searchbox({ height: 22, width: 100, prompt: '请输入样本号', searcher: onSearch });
    //查询-病历号
    $("#search-text-PATNO").searchbox({ height: 22, width: 100, prompt: '请输入病历号', searcher: onSearch });
    //查询-条码号
    $("#search-text-BARCODE").searchbox({ height: 22, width: 100, prompt: '请输入条码号', searcher: onSearch });
