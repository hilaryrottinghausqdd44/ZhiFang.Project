$(function () {
    ...
    var errorInfo = [],
        ...
        serverUrl = {
            /**获取报告列表服务地址*/
            SelectReportListUrl: Shell.util.Path.rootPath + "/ServiceWCF/ReportFromService.svc/SelectReportListByPara",
            //SelectReportListUrl: Shell.util.Path.rootPath + "/ServiceWCF/ReportFromService.svc/SearchReportListByJsonStrPara",
            /**获取报告服务地址*/
    ...
    function onSearch() {
        ...
        var newValue = $("#search-text-PGroup").combobox('getValues');
        if (newValue == 56) {
            $("#report_grid_toolbar-downLoadMost").show();
        }
        else {
            $("#report_grid_toolbar-downLoadMost").hide();
        }
    }
    /**获取查询条件*/
    function getWhere() {
        var date_s = $("#search-text-RECEIVEDATE-S").datebox("getValue"),
            ...
            DeptNos = $("#search-text-Dept").combobox("getValue"),
            param = {};
        ...
        if (DeptNos && DeptNos != '0')
            param.DeptNos = DeptNos;
        
        // var jsonstr=[];
        // jsonstr.push('"Startdate":"'+date_s+'"');
        // jsonstr.push('"Enddate":"'+date_e+'"');
        // jsonstr.push('"Startdate":"'+statues+'"');
    ...
    function winPdfPrint() {
        ...
        var url = Shell.util.Path.uiPath + "/report/Google/printPDF.html",
    ...
    //列表-批量下载
    $("#report_grid_toolbar-downLoadMost").bind('click', function () { downLoadMost(); }).hide();
    //删除报告单
    ...
    $('#report_grid').datagrid({
        ...
        columns: [[
            ...
            {
                field: 'PATNO', title: '病历号', sortable: true, width: 100, sorter: function (a, b) {
                    return;
                }
            },
            {
            ...
            {
                field: 'Isdown', title: '下载', sortable: true, width: 100, hidden: true,
            ...
            {
                field: 'opt', title: '操作', align: 'right', width: 100, align: 'center', hidden: true,
                formatter: function (value, row, index) {
    ...
    function downLoadMost() {
        ...
        param += '"reportformIds":"' + ReportFormIDs + '","reportformfilenameset":"CNAME,PATNO"}}';
    ...
    $('#search-text-ClientNo').combobox({
        ...
        onChange: function (newValue, oldValue) {
            $("#titleType").combobox({
                valueField: "TitleTypeNo",
                textField: "TitleTypeNoCName",
                editable: false,
                method: 'GET',
                url: Shell.util.Path.rootPath + "/ServiceWCF/DictionaryService.svc/SearchBBClientParaByParaNoAndLabIDAndLabName?ParaNo=ReportFormPrintDefaultTitle&LabID=" + newValue,
                loadFilter: function (data) {
                    if (data.success) {
                        var obj = Shell.util.JSON.decode(data.ResultDataValue) || {};
                        var list = obj.rows || [];
                        if (list.length > 0) {
                            if (list[0].ParaValue == "CLIENT")
                                list.unshift({ "TitleTypeNo": "CLIENT", "TitleTypeNoCName": "医院", "selected": true });
                            if (list[0].ParaValue == "CENTER")
                                list.unshift({ "TitleTypeNo": "CENTER", "TitleTypeNoCName": "中心", "selected": true });
                        }
                        return list;
                    } else {
                        return [
                            { "TitleTypeNo": "CENTER", "TitleTypeNoCName": "中心", "selected": true },
                            { "TitleTypeNo": "CLIENT", "TitleTypeNoCName": "医院" },
                            { "TitleTypeNo": "BATCH", "TitleTypeNoCName": "套打" },
                            { "TitleTypeNo": "MENZHEN", "TitleTypeNoCName": "门诊"},
                            { "TitleTypeNo": "ZHUYUAN", "TitleTypeNoCName": "住院" },
                            { "TitleTypeNo": "TIJIAN", "TitleTypeNoCName": "体检" }];
                    }
                }
            });
        }
    ...
    $("#search-text-Dept").combobox({
        ...
        panelMaxHeight: 180,
        method: 'GET',
        url: Shell.util.Path.rootPath + "/ServiceWCF/DictionaryService.svc/GetPubDict?tableName=department&fields=DeptNo,CName",
        loadFilter: function (data) {
            if (data.success) {
                var obj = Shell.util.JSON.decode(data.ResultDataValue) || {};
                var list = obj.rows || [];
                list.unshift({ "DeptNo": 0, "CName": "全部", "selected": true });
                return list;
            } else {
                return [{ "DeptNo": 0, "CName": "全部", "selected": true }];
            }
        }
    });
    //初始化结果图窗口
