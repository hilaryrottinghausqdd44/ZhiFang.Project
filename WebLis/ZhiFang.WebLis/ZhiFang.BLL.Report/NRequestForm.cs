...
namespace ZhiFang.BLL.Report
{
    ...
    /// NRequestForm
    /// </summary>
    public class NRequestForm : ZhiFang.IBLL.Report.IBNRequestForm
    {
        ...
        private readonly IDBusinessLogicClientControl DBusinessLogicClientControl = DalFactory<IDBusinessLogicClientControl>.GetDalByClassName("BusinessLogicClientControl");
        private readonly IDCLIENTELE IDCLIENTELE = DalFactory<IDCLIENTELE>.GetDalByClassName("CLIENTELE");
        public NRequestForm()
        {
        ...
        public List<Model.NRequestForm> DataTableToList(DataTable dt)
        {
            ...
            if (rowsCount > 0)
            {
                ...
                for (int n = 0; n < rowsCount; n++)
                {
                    ...
                    if (dt.Columns.Contains("Artificerorder") &&dt.Rows[n]["Artificerorder"] != null && dt.Rows[n]["Artificerorder"].ToString() != "")
                    {
                        model.Artificerorder = dt.Rows[n]["Artificerorder"].ToString();
                    }
                    if (dt.Columns.Contains("sickorder") && dt.Rows[n]["sickorder"] != null && dt.Rows[n]["sickorder"].ToString() != "")
                    {
                        model.sickorder = dt.Rows[n]["sickorder"].ToString();
                    }
                    if (dt.Columns.Contains("chargeflag") && dt.Rows[n]["chargeflag"] != null && dt.Rows[n]["chargeflag"].ToString() != "")
                    {
                    ...
                    if (dt.Columns.Contains("FormComment") && dt.Rows[n]["FormComment"] != null && dt.Rows[n]["FormComment"].ToString() != "")
                    {
                        model.FormComment = dt.Rows[n]["FormComment"].ToString();
                    }
                    if (dt.Columns.Contains("nurseflag") && dt.Rows[n]["nurseflag"] != null && dt.Rows[n]["nurseflag"].ToString() != "")
                    {
                    ...
                    if (dt.Columns.Contains("CaseNo") && dt.Rows[n]["CaseNo"] != null && dt.Rows[n]["CaseNo"].ToString() != "")
                    {
                        model.CaseNo = dt.Rows[n]["CaseNo"].ToString();
                    }
                    if (dt.Columns.Contains("refuseopinion") && dt.Rows[n]["refuseopinion"] != null && dt.Rows[n]["refuseopinion"].ToString() != "")
                    {
                        model.refuseopinion = dt.Rows[n]["refuseopinion"].ToString();
                    }
                    if (dt.Columns.Contains("refusereason") && dt.Rows[n]["refusereason"] != null && dt.Rows[n]["refusereason"].ToString() != "")
                    {
                        model.refusereason = dt.Rows[n]["refusereason"].ToString();
                    }
                    if (dt.Columns.Contains("signintime") && dt.Rows[n]["signintime"] != null && dt.Rows[n]["signintime"].ToString() != "")
                    {
                        model.signintime = DateTime.Parse(dt.Rows[n]["signintime"].ToString());
                    }
                    if (dt.Columns.Contains("signer") && dt.Rows[n]["signer"] != null && dt.Rows[n]["signer"].ToString() != "")
                    {
                        model.signer = dt.Rows[n]["signer"].ToString();
                    }
                    if (dt.Columns.Contains("signflag") && dt.Rows[n]["signflag"] != null && dt.Rows[n]["signflag"].ToString() != "")
                    {
                        model.signflag = int.Parse(dt.Rows[n]["signflag"].ToString());
                    }
                    if (dt.Columns.Contains("SamplingGroupNo") && dt.Rows[n]["SamplingGroupNo"] != null && dt.Rows[n]["SamplingGroupNo"].ToString() != "")
                    {
                        model.SamplingGroupNo = int.Parse(dt.Rows[n]["SamplingGroupNo"].ToString());
                    }
                    if (dt.Columns.Contains("PrintCount") && dt.Rows[n]["PrintCount"] != null && dt.Rows[n]["PrintCount"].ToString() != "")
                    {
                        model.PrintCount = int.Parse(dt.Rows[n]["PrintCount"].ToString());
                    }
                    if (dt.Columns.Contains("PrintInfo") && dt.Rows[n]["PrintInfo"] != null && dt.Rows[n]["PrintInfo"].ToString() != "")
                    {
                        model.PrintInfo = dt.Rows[n]["PrintInfo"].ToString();
                    }
                    if (dt.Columns.Contains("SampleCap") && dt.Rows[n]["SampleCap"] != null && dt.Rows[n]["SampleCap"].ToString() != "")
                    {
                        model.SampleCap = decimal.Parse(dt.Rows[n]["SampleCap"].ToString());
                    }
                    if (dt.Columns.Contains("IsPrep") && dt.Rows[n]["IsPrep"] != null && dt.Rows[n]["IsPrep"].ToString() != "")
                    {
                        model.IsPrep = int.Parse(dt.Rows[n]["IsPrep"].ToString());
                    }
                    if (dt.Columns.Contains("IsAffirm") && dt.Rows[n]["IsAffirm"] != null && dt.Rows[n]["IsAffirm"].ToString() != "")
                    {
                        model.IsAffirm = int.Parse(dt.Rows[n]["IsAffirm"].ToString());
                    }
                    if (dt.Columns.Contains("IsSampling") && dt.Rows[n]["IsSampling"] != null && dt.Rows[n]["IsSampling"].ToString() != "")
                    {
                        model.IsSampling = int.Parse(dt.Rows[n]["IsSampling"].ToString());
                    }
                    if (dt.Columns.Contains("IsSend") && dt.Rows[n]["IsSend"] != null && dt.Rows[n]["IsSend"].ToString() != "")
                    {
                        model.IsSend = int.Parse(dt.Rows[n]["IsSend"].ToString());
                    }
                    if (dt.Columns.Contains("incepter") && dt.Rows[n]["incepter"] != null && dt.Rows[n]["incepter"].ToString() != "")
                    {
                        model.incepter = dt.Rows[n]["incepter"].ToString();
                    }
                    if (dt.Columns.Contains("inceptTime") && dt.Rows[n]["inceptTime"] != null && dt.Rows[n]["inceptTime"].ToString() != "")
                    {
                        model.inceptTime = DateTime.Parse(dt.Rows[n]["inceptTime"].ToString());
                    }
                    if (dt.Columns.Contains("inceptDate") && dt.Rows[n]["inceptDate"] != null && dt.Rows[n]["inceptDate"].ToString() != "")
                    {
                        model.inceptDate = DateTime.Parse(dt.Rows[n]["inceptDate"].ToString());
                    }
                    if (dt.Columns.Contains("isByHand") && dt.Rows[n]["isByHand"] != null && dt.Rows[n]["isByHand"].ToString() != "")
                    {
                    ...
                    if (dt.Columns.Contains("OldSerialNo") && dt.Rows[n]["OldSerialNo"] != null && dt.Rows[n]["OldSerialNo"].ToString() != "")
                    {
                        model.OldSerialNo = dt.Rows[n]["OldSerialNo"].ToString();
                    }
                    if (dt.Columns.Contains("TestTypeNo") && dt.Rows[n]["TestTypeNo"] != null && dt.Rows[n]["TestTypeNo"].ToString() != "")
                    {
                        model.TestTypeNo = int.Parse(dt.Rows[n]["TestTypeNo"].ToString());
                    }
                    if (dt.Columns.Contains("DispenseFlag") && dt.Rows[n]["DispenseFlag"] != null && dt.Rows[n]["DispenseFlag"].ToString() != "")
                    {
                        model.DispenseFlag = int.Parse(dt.Rows[n]["DispenseFlag"].ToString());
                    }
                    if (dt.Columns.Contains("refuseUser") && dt.Rows[n]["refuseUser"] != null && dt.Rows[n]["refuseUser"].ToString() != "")
                    {
                        model.refuseUser = dt.Rows[n]["refuseUser"].ToString();
                    }
                    if (dt.Columns.Contains("refuseTime") && dt.Rows[n]["refuseTime"] != null && dt.Rows[n]["refuseTime"].ToString() != "")
                    {
                        model.refuseTime = DateTime.Parse(dt.Rows[n]["refuseTime"].ToString());
                    }
                    if (dt.Columns.Contains("jytype") && dt.Rows[n]["jytype"] != null && dt.Rows[n]["jytype"].ToString() != "")
                    {
                        model.jytype = dt.Rows[n]["jytype"].ToString();
                    }
                    if (dt.Columns.Contains("CasSerialScanTime_oldeNo") && dt.Rows[n]["SerialScanTime_old"] != null && dt.Rows[n]["SerialScanTime_old"].ToString() != "")
                    {
                        model.SerialScanTime_old = dt.Rows[n]["SerialScanTime_old"].ToString();
                    }
                    if (dt.Columns.Contains("IsCheckFee") && dt.Rows[n]["IsCheckFee"] != null && dt.Rows[n]["IsCheckFee"].ToString() != "")
                    {
                        model.IsCheckFee = int.Parse(dt.Rows[n]["IsCheckFee"].ToString());
                    }
                    if (dt.Columns.Contains("Dr2Flag") && dt.Rows[n]["Dr2Flag"] != null && dt.Rows[n]["Dr2Flag"].ToString() != "")
                    {
                        model.Dr2Flag = int.Parse(dt.Rows[n]["Dr2Flag"].ToString());
                    }
                    if (dt.Columns.Contains("CaseExecDeptNoNo") && dt.Rows[n]["ExecDeptNo"] != null && dt.Rows[n]["ExecDeptNo"].ToString() != "")
                    {
                        model.ExecDeptNo = int.Parse(dt.Rows[n]["ExecDeptNo"].ToString());
                    }
                    if (dt.Columns.Contains("CaseClientHostNo") && dt.Rows[n]["ClientHost"] != null && dt.Rows[n]["ClientHost"].ToString() != "")
                    {
                        model.ClientHost = dt.Rows[n]["ClientHost"].ToString();
                    }
                    if (dt.Columns.Contains("PreNumber") && dt.Rows[n]["PreNumber"] != null && dt.Rows[n]["PreNumber"].ToString() != "")
                    {
                        model.PreNumber = int.Parse(dt.Rows[n]["PreNumber"].ToString());
                    }
                    if (dt.Columns.Contains("UrgentState") && dt.Rows[n]["UrgentState"] != null && dt.Rows[n]["UrgentState"].ToString() != "")
                    {
                        model.UrgentState = dt.Rows[n]["UrgentState"].ToString();
                    }
                    if (dt.Columns.Contains("ZDY6") && dt.Rows[n]["ZDY6"] != null && dt.Rows[n]["ZDY6"].ToString() != "")
                    {
                        model.ZDY6 = dt.Rows[n]["ZDY6"].ToString();
                    }
                    if (dt.Columns.Contains("ZDY7") && dt.Rows[n]["ZDY7"] != null && dt.Rows[n]["ZDY7"].ToString() != "")
                    {
                        model.ZDY7 = dt.Rows[n]["ZDY7"].ToString();
                    }
                    if (dt.Columns.Contains("ZDY8") && dt.Rows[n]["ZDY8"] != null && dt.Rows[n]["ZDY8"].ToString() != "")
                    {
                        model.ZDY8 = dt.Rows[n]["ZDY8"].ToString();
                    }
                    if (dt.Columns.Contains("ZDY9") && dt.Rows[n]["ZDY9"] != null && dt.Rows[n]["ZDY9"].ToString() != "")
                    {
                        model.ZDY9 = dt.Rows[n]["ZDY9"].ToString();
                    }
                    if (dt.Columns.Contains("ZDY10") && dt.Rows[n]["ZDY10"] != null && dt.Rows[n]["ZDY10"].ToString() != "")
                    {
                        model.ZDY10 = dt.Rows[n]["ZDY10"].ToString();
                    }
                    if (dt.Columns.Contains("phoneCode") && dt.Rows[n]["phoneCode"] != null && dt.Rows[n]["phoneCode"].ToString() != "")
                    {
                        model.phoneCode = dt.Rows[n]["phoneCode"].ToString();
                    }
                    if (dt.Columns.Contains("IsNode") && dt.Rows[n]["IsNode"] != null && dt.Rows[n]["IsNode"].ToString() != "")
                    {
                        model.IsNode = int.Parse(dt.Rows[n]["IsNode"].ToString());
                    }
                    if (dt.Columns.Contains("PhoneNodeCount") && dt.Rows[n]["PhoneNodeCount"] != null && dt.Rows[n]["PhoneNodeCount"].ToString() != "")
                    {
                        model.PhoneNodeCount = int.Parse(dt.Rows[n]["PhoneNodeCount"].ToString());
                    }
                    if (dt.Columns.Contains("AutoNodeCount") && dt.Rows[n]["AutoNodeCount"] != null && dt.Rows[n]["AutoNodeCount"].ToString() != "")
                    {
                        model.AutoNodeCount = int.Parse(dt.Rows[n]["AutoNodeCount"].ToString());
                    }
                    if (dt.Columns.Contains("clientno") && dt.Rows[n]["clientno"] != null && dt.Rows[n]["clientno"].ToString() != "")
                    {
                        model.ClientNo = dt.Rows[n]["clientno"].ToString();
                    }
                    if (dt.Columns.Contains("SerialScanTime") && dt.Rows[n]["SerialScanTime"] != null && dt.Rows[n]["SerialScanTime"].ToString() != "")
                    {
                        model.SerialScanTime = DateTime.Parse(dt.Rows[n]["SerialScanTime"].ToString());
                    }
                    if (dt.Columns.Contains("CountNodesFormSource") && dt.Rows[n]["CountNodesFormSource"] != null && dt.Rows[n]["CountNodesFormSource"].ToString() != "")
                    {
                        model.CountNodesFormSource = dt.Rows[n]["CountNodesFormSource"].ToString();
                    }
                    if (dt.Columns.Contains("StateFlag") && dt.Rows[n]["StateFlag"] != null && dt.Rows[n]["StateFlag"].ToString() != "")
                    {
                        model.StateFlag = int.Parse(dt.Rows[n]["StateFlag"].ToString());
                    }
                    if (dt.Columns.Contains("AffirmTime") && dt.Rows[n]["AffirmTime"] != null && dt.Rows[n]["AffirmTime"].ToString() != "")
                    {
                        model.AffirmTime = DateTime.Parse(dt.Rows[n]["AffirmTime"].ToString());
                    }
                    if (dt.Columns.Contains("IsNurseDo") && dt.Rows[n]["IsNurseDo"] != null && dt.Rows[n]["IsNurseDo"].ToString() != "")
                    {
                        model.IsNurseDo = int.Parse(dt.Rows[n]["IsNurseDo"].ToString());
                    }
                    if (dt.Columns.Contains("NurseSender") && dt.Rows[n]["NurseSender"] != null && dt.Rows[n]["NurseSender"].ToString() != "")
                    {
                        model.NurseSender = dt.Rows[n]["NurseSender"].ToString();
                    }
                    if (dt.Columns.Contains("NurseSendTime") && dt.Rows[n]["NurseSendTime"] != null && dt.Rows[n]["NurseSendTime"].ToString() != "")
                    {
                        model.NurseSendTime = DateTime.Parse(dt.Rows[n]["NurseSendTime"].ToString());
                    }
                    if (dt.Columns.Contains("NurseSendCarrier") && dt.Rows[n]["NurseSendCarrier"] != null && dt.Rows[n]["NurseSendCarrier"].ToString() != "")
                    {
                        model.NurseSendCarrier = dt.Rows[n]["NurseSendCarrier"].ToString();
                    }
                    if (dt.Columns.Contains("CollectCount") && dt.Rows[n]["CollectCount"] != null && dt.Rows[n]["CollectCount"].ToString() != "")
                    {
                        model.CollectCount = int.Parse(dt.Rows[n]["CollectCount"].ToString());
                    }
                    if (dt.Columns.Contains("ForeignSendFlag") && dt.Rows[n]["ForeignSendFlag"] != null && dt.Rows[n]["ForeignSendFlag"].ToString() != "")
                    {
                        model.ForeignSendFlag = int.Parse(dt.Rows[n]["ForeignSendFlag"].ToString());
                    }
                    if (dt.Columns.Contains("HisAffirm") && dt.Rows[n]["HisAffirm"] != null && dt.Rows[n]["HisAffirm"].ToString() != "")
                    {
                        model.HisAffirm = int.Parse(dt.Rows[n]["HisAffirm"].ToString());
                    }
                    if (dt.Columns.Contains("PatPhoto") && dt.Rows[n]["PatPhoto"] != null && dt.Rows[n]["PatPhoto"].ToString() != "")
                    {
                        model.PatPhoto = (byte[])dt.Rows[n]["PatPhoto"];
                    }
                    if (dt.Columns.Contains("ChargeOrderNo") && dt.Rows[n]["ChargeOrderNo"] != null && dt.Rows[n]["ChargeOrderNo"].ToString() != "")
                    {
                        model.ChargeOrderNo = dt.Rows[n]["ChargeOrderNo"].ToString();
                    }
                    if (dt.Columns.Contains("ReportFlag") && dt.Rows[n]["ReportFlag"] != null && dt.Rows[n]["ReportFlag"].ToString() != "")
                    {
                        model.ReportFlag = int.Parse(dt.Rows[n]["ReportFlag"].ToString());
                    }                    
                    if (dt.Columns.Contains("ClientName") && dt.Rows[n]["ClientName"] != null && dt.Rows[n]["ClientName"].ToString() != "")
                    {
                        model.ClientName = dt.Rows[n]["ClientName"].ToString();
                    }
                    if (dt.Columns.Contains("WebLisSourceOrgID") && dt.Rows[n]["WebLisSourceOrgID"] != null && dt.Rows[n]["WebLisSourceOrgID"].ToString() != "")
                    {
                        model.WebLisSourceOrgID = dt.Rows[n]["WebLisSourceOrgID"].ToString();
                    }
                    if (dt.Columns.Contains("WebLisSourceOrgName") && dt.Rows[n]["WebLisSourceOrgName"] != null && dt.Rows[n]["WebLisSourceOrgName"].ToString() != "")
                    {
        ...
        {
            return dal.GetListByBarCodeList(BarCodeList);
        }
        ...
        public List<Model.BarCodeFormResult> GetBarCodeListByNRequestFormNo(string nrequestformno)
        {
            List<Model.BarCodeFormResult> barcodelist = new List<Model.BarCodeFormResult>();
            DataTable dt = dal.GetBarCodeByNRequestFormNo(nrequestformno);
            if (dt != null && dt.Rows.Count > 0)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ZhiFang.Model.BarCodeFormResult bf = new Model.BarCodeFormResult();
                    bf.BarCode = dt.Rows[i]["BarCode"].ToString().Trim();
                    bf.BarCodeFormNo = long.Parse(dt.Rows[i]["BarCodeFormNo"].ToString().Trim());
                    bf.Color = dt.Rows[i]["color"].ToString().Trim();
                    bf.ItemName = dt.Rows[i]["ItemName"].ToString().Trim();
                    bf.ItemNo = dt.Rows[i]["ItemNo"].ToString().Trim();
                    bf.ItemNo = dt.Rows[i]["ItemNo"].ToString().Trim();
                    bf.WebLisFlag = dt.Rows[i]["WebLisFlag"].ToString().Trim();
                    //bf.SampleTypeName = dt.Rows[i]["SampleTypeName"].ToString().Trim();
                    //bf.SampleTypeNo = dt.Rows[i]["SampleTypeName"].ToString().Trim();
                    barcodelist.Add(bf);
                }
                return barcodelist;
            }
        ...
        public  List<Model.StaticRecOrgSamplePrice> DataTableToStaticRecOrgSamplePriceList(DataTable dt)
        {
        ...
        public List<Model.StaticRecOrgSamplePrice> OperatorWorkDataTableToList(DataTable dt)
        {
            List<ZhiFang.Model.StaticRecOrgSamplePrice> modelList = new List<ZhiFang.Model.StaticRecOrgSamplePrice>();
            int rowsCount = dt.Rows.Count;
            if (rowsCount > 0)
        ...
        public DataTable GetNRequstFormList_CombiItemNo(Model.NRequestForm nrf_m, int StartPage, int PageSize, out int intPageCount, out int iCount)
        {
            iCount = dal.GetNRequstFormListTotalCount(nrf_m);
            intPageCount = int.Parse((iCount / PageSize).ToString()) + 1;
            return dal.GetNRequstFormListByPage_CombiItemNo(nrf_m, StartPage, PageSize).Tables[0];
        }

        public DataTable GetNRequstFormList_SampleSendNo(Model.NRequestForm nrf_m, int StartPage, int PageSize, out int intPageCount, out int iCount)
        {
            iCount = dal.GetNRequstFormListTotalCount(nrf_m);
            intPageCount = int.Parse((iCount / PageSize).ToString()) + 1;
            return dal.GetNRequstFormList_SampleSendNo(nrf_m, StartPage, PageSize).Tables[0];
        }

        public bool SendBarCodeFromByBarCodeList(string barCodeList, string userId, string employeeName)
        {
            dal.SendBarCodeFromByBarCodeList(barCodeList, userId, employeeName);
            DBarCodeForm.SendBarCodeFromByBarCodeList(barCodeList, userId, employeeName);
            return true;
        }

        public BaseResultBool DeliveryBarCodeFromByBarCodeList(string barCodeList, string userId, string employeeName, bool flag, string reason)
        {
            BaseResultBool brb = new BaseResultBool();
            if (!CheckStatus(barCodeList, "Delivery", out var error))
            {
                brb.ErrorInfo = error;
                brb.success = false;
                return brb;
            }
            dal.DeliveryBarCodeFromByBarCodeList(barCodeList, userId, employeeName, flag, reason);
            DBarCodeForm.DeliveryBarCodeFromByBarCodeList(barCodeList, userId, employeeName, flag, reason);
            brb.success = true;
            return brb;
        }

        public BaseResultBool ReceiveBarCodeFromByBarCodeList(string barCodeList, string userId, string employeeName, bool flag, string reason,string account)
        {
            BaseResultBool brb = new BaseResultBool();
            var blcclist=DBusinessLogicClientControl.GetBusinessLogicClientListByPage(new BusinessLogicClientControl() { Account = account, Flag = 1 },1,10,null,null,null);
            if (blcclist == null || blcclist.Tables.Count <= 0 || blcclist.Tables[0].Rows.Count <= 0)
            {
                ZhiFang.Common.Log.Log.Error("未能找到账户：" + account + ",所属的医疗机构！");
                brb.ErrorInfo = "未能找到账户：" + account + ",所属的医疗机构！";
                brb.success = false;
                return brb;
            }
            if (!CheckStatus(barCodeList, "Receive", out var error))
            {
                brb.ErrorInfo = error;
                brb.success = false;
                return brb;
            }
            string weblisorgid = blcclist.Tables[0].Rows[0]["ClientNo"].ToString();
            string weblisorgname = blcclist.Tables[0].Rows[0]["CNAME"].ToString();
            dal.ReceiveBarCodeFromByBarCodeList(barCodeList, userId, employeeName, flag, reason, weblisorgid, weblisorgname);
            DBarCodeForm.ReceiveBarCodeFromByBarCodeList(barCodeList, userId, employeeName, flag, reason, weblisorgid, weblisorgname);
            brb.success = true;
            return brb;
        }
        public bool CheckStatus(string barCodeList,string statusname, out string error)
        {
            DataSet ds = dal.GetListByBarCodeList(barCodeList.Split(',').ToList());
            if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
            {
                ...
                {                    
                    case "Delivery":
                        ...
                        {
                            if (ds.Tables[0].Rows[i]["WeblisFlag"] == DBNull.Value || ds.Tables[0].Rows[i]["WeblisFlag"].ToString().Trim() != "1")
                            {
                                error = "条码列表中包含非已打包的条码！";
                                ZhiFang.Common.Log.Log.Error("CheckStatus,条码列表(" + barCodeList + ")中包含非已打包的条码,BarCode:" + ds.Tables[0].Rows[i]["BarCode"].ToString().Trim());
                                return false;
                            }
                        }
                        break;
                    case "Receive":
                        for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                        {
                            if (ds.Tables[0].Rows[i]["WeblisFlag"] == DBNull.Value || ds.Tables[0].Rows[i]["WeblisFlag"].ToString().Trim() != "2")
                            {
                                error = "条码列表中包含非已外送的条码！";
                                ZhiFang.Common.Log.Log.Error("CheckStatus,条码列表(" + barCodeList + ")中包含非已外送的条码,BarCode:" + ds.Tables[0].Rows[i]["BarCode"].ToString().Trim());
                                return false;
                            }
                        }
                        break;
            ...
            {
                ZhiFang.Common.Log.Log.Debug("CheckStatus.未能找到相关条码的申请单!barCodeList='"+ barCodeList + "'@statusname="+ statusname);
                error = "未能找到相关条码的申请单！";
                return false;
            }
        }

        public List<StaticRecOrgSamplePrice> BarcodeDataTableToList(DataTable dt)
        {
            List<ZhiFang.Model.StaticRecOrgSamplePrice> modelList = new List<ZhiFang.Model.StaticRecOrgSamplePrice>();
            int rowsCount = dt.Rows.Count;
            if (rowsCount > 0)
