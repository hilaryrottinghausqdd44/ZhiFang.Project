        ...
        public List<ZhiFang.Model.CLIENTELE> DataTableToList(DataTable dt)
        {
            ...
            if (rowsCount > 0)
            {
                ...
                for (int n = 0; n < rowsCount; n++)
                {
                    ...
                    if (dt.Columns.Contains("BusinessLogicClientControlFlag") && dt.Rows[n]["BusinessLogicClientControlFlag"].ToString() != "")
                    {
                        model.BusinessLogicClientControlFlag = int.Parse(dt.Rows[n]["BusinessLogicClientControlFlag"].ToString());
                    }
                    modelList.Add(model);
                }
            }
