...
namespace ZhiFang.BLL.Common.BaseDictionary
{
    ...
    public partial class ResultTestItemControl : IBSynchData, IBResultTestItemControl, IBBatchCopy
    {
        IDAL.IDResultTestItemControl dal = DALFactory.DalFactory<IDAL.IDResultTestItemControl>.GetDal("B_ResultTestItemControl", ZhiFang.Common.Dictionary.DBSource.LisDB());
        IDAL.IDBatchCopy dalCopy = DALFactory.DalFactory<IDAL.IDBatchCopy>.GetDal("B_ResultTestItemControl", ZhiFang.Common.Dictionary.DBSource.LisDB());
        public ResultTestItemControl()
        { }
        ...
        /// 得到一个对象实体，从缓存中
        /// </summary>
        public ZhiFang.Model.ResultTestItemControl GetModelByCache(string ItemNo, string LabCode, string LabItemNo)
        {
            ...
            return (ZhiFang.Model.ResultTestItemControl)objModel;
        }
        ...
        /// 获得数据列表
        /// </summary>
        public List<ZhiFang.Model.ResultTestItemControl> GetModelList(string strWhere)
        {
        ...
        /// 获得数据列表
        /// </summary>
        public List<ZhiFang.Model.ResultTestItemControl> DataTableToList(DataTable dt)
        {
            List<ZhiFang.Model.ResultTestItemControl> modelList = new List<ZhiFang.Model.ResultTestItemControl>();
            int rowsCount = dt.Rows.Count;
            if (rowsCount > 0)
            {
                ZhiFang.Model.ResultTestItemControl model;
                for (int n = 0; n < rowsCount; n++)
                {
                    model = new ZhiFang.Model.ResultTestItemControl();
        ...
        #region IBBase<ResultTestItemControl> 成员
        ...
        {
            return dal.Exists(ItemNo, ControlLabNo);
        }
        ...
        {
            DataSet ds = dal.GetList(new Model.ResultTestItemControl { ControlLabNo = LabCode, ControlItemNo = LabItemNo });
        ...
        {
            DataSet ds = dal.GetList(new Model.ResultTestItemControl { ControlLabNo = LabCode, ItemNo = ItemNo });
            if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
            {
        ...
        public string GetLabCodeNo(string LabCode, string CenterNo)
        {
            ...
            try
            {
                Model.ResultTestItemControl model = new Model.ResultTestItemControl();
            ...
            {
                LabCodeNo = "";
                ZhiFang.Common.Log.Log.Error("ZhiFang.BLL.Common.BaseDictionary.ResultTestItemControl.GetLabCodeNo异常 ||| CenterNo=" + CenterNo, ex);
            }
            return LabCodeNo;
        }
        ...
        public string GetCenterNo(string LabCode, string LabPrimaryNo)
        {
            ...
            {
                CenterNo = "";
                ZhiFang.Common.Log.Log.Error("ZhiFang.BLL.Common.BaseDictionary.ResultTestItemControl.GetCenterNo异常 ||| LabCode=" + LabCode + " ||| LabPrimaryNo=" + LabPrimaryNo, ex);
            }
            return CenterNo;
        }
