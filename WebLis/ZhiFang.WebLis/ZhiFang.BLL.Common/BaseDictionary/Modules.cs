...
namespace ZhiFang.BLL.Common.BaseDictionary
{
    ...
    public partial class Modules :IBLL.Common.IBSynchData, IBModules
    {
        ...
        /// 删除一条数据
        /// </summary>
        public BaseResultDataValue Delete(int ID)
        {
            BaseResultDataValue brdv = new BaseResultDataValue();
            var entity = this.GetModel( ID);
            if (entity == null )
            {
                ZhiFang.Common.Log.Log.Debug("ZhiFang.BLL.Common.BaseDictionary.Modules.Delete.未能找到模块！ID："+ID);
                return new BaseResultDataValue() { ErrorInfo = "未能删除模块！", success = false };
            }
            //检查角色模块关系
            //检查模块父级关系
            var count = this.GetRecordCount(" SN like '"+ entity.SN+"%' and SN<>'" + entity.SN+"' ");
            if (count > 0)
            {
                ZhiFang.Common.Log.Log.Debug("ZhiFang.BLL.Common.BaseDictionary.Modules.Delete.因为此模块包含子模块！未能删除模块！ID：" + ID);
                return new BaseResultDataValue() { ErrorInfo = "因为此模块包含子模块！未能删除模块！", success = false };
            }
            brdv.success=dal.Delete(ID)>0;
            return brdv;
        }
        ...
        {
            return dal.GetListByRBACModulesListAll();
        }
        ...
        public DataSet GetListByPage(string where, int page, int limit, string Sort)
        {
            if (page >= 0 && limit > 0)
            {
                return dal.GetListByPage(where, page, limit, Sort);
            }
            else
                return null;
        }
        ...
        BaseResultDataValue IBModules.Addentity(Model.Modules model)
        {
            BaseResultDataValue brdv = new BaseResultDataValue();
            brdv.success = this.Add(model) > 0;
