...
namespace ZhiFang.BLL.Common.BaseDictionary
{
    ...
    public partial class TestItem : IBSynchData, IBTestItem, IBBatchCopy, IBDataDownload
    {
        ...
        public List<ZhiFang.Model.TestItem> DataTableToList(DataTable dt)
        {
            ...
            if (rowsCount > 0)
            {
                ...
                for (int n = 0; n < rowsCount; n++)
                {
                    ...
                    if (dt.Columns.Contains("lowprice") && dt.Rows[n]["lowprice"].ToString() != "")
                    {
                        model.LowPrice = decimal.Parse(dt.Rows[n]["lowprice"].ToString());
                    }
        ...
        public List<Model.TestItem> GetItemListByColor(string colorvalue, string where, string orderby, int startIndex, int endIndex)
        {
            List<Model.TestItem> list = new List<Model.TestItem>();
            DataSet ds= dal.GetItemListByColor(colorvalue, where, orderby, startIndex, endIndex);
            ...
            ItemColorDict BItemColorDict = new ItemColorDict();
            DataSet dscolor= BItemColorDict.GetAllList();
            ...
            list.ForEach(a => {
                ...
                {
                    if (!string.IsNullOrEmpty(a.Color) && a.Color.Trim() == dscolor.Tables[0].Rows[i]["ColorName"].ToString().Trim())
                    {
                        a.ColorValue = dscolor.Tables[0].Rows[i]["ColorValue"].ToString().Trim();
                        break;
                    }
                }
            });
        ...
        {
            return dal.GetItemListByColorCount(colorvalue, where);
        }

        public BaseResultDataValue SetItemColorByItemNoList(string colorvalue, List<string> itemnolist)
        {
            BaseResultDataValue brdv = new BaseResultDataValue();
            brdv.success = dal.SetItemColorByItemNoList(colorvalue, itemnolist) > 0;
            return brdv;
        }
    }
}
