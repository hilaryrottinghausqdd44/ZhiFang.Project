...
namespace ZhiFang.DAL.Oracle.weblis
{
	...
	public partial class GenderType : BaseDALLisDB, IDGenderType, IDBatchCopy, IDGetListByTimeStampe
	{
		...
		public int Update(ZhiFang.Model.GenderType model)
		{
            ...
            strSql.Remove(n, 1);
            strSql.Append(" where GenderNo=" + model.GenderNo + " ");
		...
		{
			StringBuilder strSql = new StringBuilder();
            strSql.Append("select GenderType.*,GenderNo as LabNo,concat(concat(GenderNo,'_'),CName) as LabNo_Value,concat(concat(concat(CName,'('),GenderNo),')') as LabNoAndName_Text ");
			strSql.Append(" FROM GenderType  ");
			if (model.CName != null)
			{
		...
		public DataSet GetListByPage(ZhiFang.Model.GenderType model, int nowPageNum, int nowPageSize)
		{
			...
			if (model != null && model.LabCode != null)
			{
				...
				strSql.Append(" select  * from GenderType left join B_GenderTypeControl on GenderType.GenderNo=B_GenderTypeControl.GenderNo ");
				if (model.LabCode != null)
				{
					strSql.Append(" and B_GenderTypeControl.ControlLabNo='" + model.LabCode + "' ");
				}
                strSql.Append("where  ROWNUM <= '" + nowPageSize + "' and GenderType.GenderNo not in ( ");
				strSql.Append("select GenderType.GenderNo from  GenderType left join B_GenderTypeControl on GenderType.GenderNo=B_GenderTypeControl.GenderNo ");
				if (model.LabCode != null)
				{
					strSql.Append(" and B_GenderTypeControl.ControlLabNo='" + model.LabCode + "' ");
				}
			...
			else
			{
                ...
                strSql.Append("select  * from GenderType where  ROWNUM <= '" + nowPageSize + "' and GenderNo not in  ");
                strSql.Append("(select GenderNo from GenderType where 1=1 and ROWNUM <= '" + (nowPageSize * nowPageNum) + "' " + strLike + " ) " + strLike + " order by " + model.OrderField + "  ");
				return DbHelperSQL.ExecuteDataSet(strSql.ToString());
			}
		}
		...
		public int AddByDataRow(DataRow dr)
		{
			...
			{
				ZhiFang.Common.Log.Log.Error("ZhiFang.DAL.Oracle.weblis.GenderType.AddByDataRow 同步数据时异常：", ex);
				return 0;
			}
		}
		public int UpdateByDataRow(DataRow dr)
		{
			...
			{
				ZhiFang.Common.Log.Log.Error("ZhiFang.DAL.Oracle.weblis.GenderType .UpdateByDataRow同步数据时异常：", ex);
				return 0;
			}
		}
		public int DeleteByDataRow(DataRow dr)
		{
			...
			{
				ZhiFang.Common.Log.Log.Error("ZhiFang.DAL.Oracle.weblis.GenderType .DeleteByDataRow同步数据时异常：", ex);
				return 0;
			}
		}
		...
		public DataSet GetListByTimeStampe(string LabCode, int dTimeStampe)
		{
			...
			StringBuilder strSql = new StringBuilder();
            strSql.Append("select GenderType.*,'" + LabCode + "' as LabCode,GenderNo as LabGenderNo from GenderType where 1=1 ");
			...
			StringBuilder strSql2 = new StringBuilder();
            strSql2.Append("select S_Dic_GenderType.*,LabGenderNo as GenderNo from S_Dic_GenderType where 1=1 ");
			if (LabCode.Trim() != "")
			{
