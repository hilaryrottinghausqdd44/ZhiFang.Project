...
namespace ZhiFang.DAL.Oracle.weblis
{
    ...
    public partial class B_Lab_SuperGroup : BaseDALLisDB, IDLab_SuperGroup
    {
        ...
        public int Update(ZhiFang.Model.Lab_SuperGroup model)
        {
            ...
            strSql.Remove(n, 1);
            strSql.Append(" where LabCode='" + model.LabCode + "' and LabSuperGroupNo=" + model.LabSuperGroupNo + "  ");
            if (DbHelperSQL.ExecuteNonQuery(strSql.ToString()) > 0)
            {
                return d_log.OperateLog("SuperGroup", "", "", DateTime.Now, 1);
            }
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select B_Lab_SuperGroup.* ,concat(concat('(',CONCAT(LabSuperGroupNo,')')),cname) as LabSuperGroupNoAndName ");
            strSql.Append(" FROM B_Lab_SuperGroup where 1=1 ");
            if (model.LabCode != null)
            {
                strSql.Append(" and LabCode='" + model.LabCode + "' ");
        ...
        public DataSet GetListByPage(ZhiFang.Model.Lab_SuperGroup model, int nowPageNum, int nowPageSize)
        {
            ...
            strSql.Append(strLike + " and  ROWNUM <= '" + nowPageSize + "' and SuperGroupID not in ");
            strSql.Append("(select SuperGroupID from B_Lab_SuperGroup where 1=1 and ROWNUM <= '" + (nowPageSize * nowPageNum) + "' " + strLike);
        ...
        public int AddByDataRow(DataRow dr)
        {
            ...
            {
                ZhiFang.Common.Log.Log.Error("ZhiFang.DAL.Oracle.weblis.B_Lab_SuperGroup.AddByDataRow 同步数据时异常：", ex);
                return 0;
            }
        }
        public int UpdateByDataRow(DataRow dr)
        {
            ...
            {
                ZhiFang.Common.Log.Log.Error("ZhiFang.DAL.Oracle.weblis.B_Lab_SuperGroup .UpdateByDataRow同步数据时异常：", ex);
                return 0;
            }
        }
        #endregion
        ...
        public DataSet GetLabCodeNo(string LabCode, List<string> LabCname)
        {
            ...
            try
            {
                ...
                ZhiFang.Common.Log.Log.Info("ZhiFang.DAL.Oracle.weblis.B_Lab_SuperGroup.GetLabCodeNo:" + strSql);
                ds = DbHelperSQL.ExecuteDataSet(strSql);
            }
            catch (Exception ex)
            {
                ZhiFang.Common.Log.Log.Error("ZhiFang.DAL.Oracle.weblis.B_Lab_SuperGroup.GetLabCodeNo异常->", ex);
            }
            return ds;
        }
