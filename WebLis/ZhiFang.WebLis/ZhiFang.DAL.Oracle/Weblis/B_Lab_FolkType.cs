...
namespace ZhiFang.DAL.Oracle.weblis
{
    ...
    public partial class B_Lab_FolkType : IDLab_FolkType
    {
        ...
        public int Add(ZhiFang.Model.Lab_FolkType model)
        {
            ...
            {
                strSql1.Append("LabFolkNo,");
                strSql2.Append("" + model.LabFolkNo + ",");
            }
            if (model.CName != null)
            {
        ...
        public int Update(ZhiFang.Model.Lab_FolkType model)
        {
            ...
            strSql.Remove(n, 1);
            strSql.Append(" where LabCode='" + model.LabCode + "' and LabFolkNo=" + model.LabFolkNo + " ");
            if (idb.ExecuteNonQuery(strSql.ToString()) > 0)
            {
                return d_log.OperateLog("FolkType", "", "", DateTime.Now, 1);
            }
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select B_Lab_FolkType.* ,concat(concat('(',CONCAT(LabFolkNo,')')),cname) as LabFolkNoAndName ");
            strSql.Append(" FROM B_Lab_FolkType where 1=1 ");
            if (model.LabCode != null)
            {
                strSql.Append(" and LabCode='" + model.LabCode + "' ");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * from B_Lab_FolkType where 1=1  ");
            ...
            strSql.Append("and  ROWNUM <= '" + nowPageSize + "' and FolkID not in ");
            strSql.Append("(select FolkID from B_Lab_FolkType where 1=1 and ROWNUM <= '" + (nowPageSize * nowPageNum) + "' " + strLike + " ");
            if (model.LabCode != null)
            {
                strSql.Append(" and LabCode='" + model.LabCode + "' ");
            }
            ...
            if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
            {
                try
                {
                    ...
                    for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                    {
                        ...
                        if (this.
                            Exists(ds.Tables[0].Rows[i]["LabCode"].ToString().Trim(), int.Parse(ds.Tables[0].Rows[i]["LabFolkNo"].ToString().Trim())))
                        {
