...
namespace ZhiFang.DAL.Oracle.weblis
{
    public class Department : BaseDALLisDB, IDDepartment, IDBatchCopy
    {
        ...
        public ZhiFang.Model.Department GetModel(int DeptNo)
        {
            ...
            strSql.Append(" where ROWNUM <='1' and DeptNo=" + DeptNo + " ");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select Department.*,DeptNo as LabNo,concat(concat(concat(CName,'('),DeptNo),')') as LabNoAndName ");
            strSql.Append(" FROM Department where 1=1 ");
        ...
        public int DeleteByDataRow(DataRow dr)
        {
            ...
            {
                ZhiFang.Common.Log.Log.Error("ZhiFang.DAL.Oracle.weblis.Department .DeleteByDataRow同步数据时异常：", ex);
                return 0;
            }
        }
        ...
        #region IDataPage<Department> 成员
        ...
        public DataSet GetListByPage(ZhiFang.Model.Department model, int nowPageNum, int nowPageSize)
        {
            ...
            if (model != null && model.LabCode != null)
            {
                ...
                strSql.Append(" select  * from Department left join B_DepartmentControl on Department.DeptNo=B_DepartmentControl.DeptNo ");
                if (model.LabCode != null)
                {
                    strSql.Append(" and B_DepartmentControl.ControlLabNo='" + model.LabCode + "' ");
                }
                strSql.Append("where  ROWNUM <= '" + nowPageSize + "' and Department.DeptNo not in ( ");
                strSql.Append("select Department.DeptNo from  Department left join B_DepartmentControl on Department.DeptNo=B_DepartmentControl.DeptNo ");
                if (model.LabCode != null)
                {
                    strSql.Append(" and B_DepartmentControl.ControlLabNo='" + model.LabCode + "' ");
                }
            ...
            else
            {
                ...
                strSql.Append("select  * from Department where  ROWNUM <= '" + nowPageSize + "' and DeptNo not in  ");
                // strSql.Append("(select top " + (nowPageSize * nowPageNum ) + " DeptNo from Department  where 1=1 " + likesql + "  order by DeptNo)");
                strSql.Append("(select DeptNo from Department where 1=1 and ROWNUM <= '" + (nowPageSize * nowPageNum) + "' " + likesql + " ) " + likesql + " order by " + model.OrderField + "  ");
        ...
        public bool CopyToLab(List<string> lst)
        {
            ...
            try
            {
                for (int i = 0; i < lst.Count; i++)
                {
                    ...
                    strSqlControl.Append("  concat(concat(concat(concat('" + lst[i].Trim() + "','_'),DeptNo),'_'),DeptNo) as DepartmentControlNo,DeptNo,'" + lst[i].Trim() + "' as ControlLabNo,DeptNo,1 ");
