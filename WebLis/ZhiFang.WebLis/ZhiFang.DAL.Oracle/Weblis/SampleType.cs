	...
	{	
        ...
        public DataSet GetListByPage(ZhiFang.Model.SampleType model, int nowPageNum, int nowPageSize)
        {
            ...
            if (model != null && model.LabCode != null)
            {
                ...
                strSql.Append(" select  * from SampleType left join B_SampleTypeControl on SampleType.SampleTypeNo=B_SampleTypeControl.SampleTypeNo ");
                if (model.LabCode != null)
                {
                    strSql.Append(" and B_SampleTypeControl.ControlLabNo='" + model.LabCode + "' ");
                }
                strSql.Append("where  ROWNUM <= '" + nowPageSize + "' and 1=1  " + likesql + " and SampleType.SampleTypeNo not in ( ");
                strSql.Append("select SampleType.SampleTypeNo from  SampleType left join B_SampleTypeControl on SampleType.SampleTypeNo=B_SampleTypeControl.SampleTypeNo ");
                if (model.LabCode != null)
                {
                    strSql.Append(" and B_SampleTypeControl.ControlLabNo='" + model.LabCode + "'");
                }
                strSql.Append(" where 1=1 and ROWNUM <= '" + (nowPageSize * nowPageNum) + "' " + likesql + "  ) order by " + strOrderBy + " ");
                return DbHelperSQL.ExecuteDataSet(strSql.ToString());
            }
            else
            {
                ...
                strSql.Append("select  * from SampleType where  ROWNUM <= '" + nowPageSize + "' and SampleTypeNo not in  ");
                strSql.Append("(select SampleTypeNo from SampleType  where 1=1 and ROWNUM <= '" + (nowPageSize * nowPageNum) + "' " + likesql + "  ) ");
                strSql.Append(likesql);
                strSql.Append(" order by " + model.OrderField + "  ");
                return DbHelperSQL.ExecuteDataSet(strSql.ToString());
            }
        }
        ...
        public int DeleteByDataRow(DataRow dr)
        {
            ...
            {
                ZhiFang.Common.Log.Log.Error("ZhiFang.DAL.Oracle.weblis.SampleType.DeleteByDataRow同步数据时异常：", ex);
                return 0;
            }
        }
        ...
        public DataSet GetListByTimeStampe(string LabCode, int dTimeStampe)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select SampleType.*,'" + LabCode + "' as LabCode,SampleTypeNo as LabSampleTypeNo from SampleType where 1=1 ");
            ...
            StringBuilder strSql2 = new StringBuilder();
            strSql2.Append("select B_Lab_mpleType.*,LabSampleTypeNo as SampleTypeNo from B_Lab_mpleType where 1=1 ");
            if (LabCode.Trim() != "")
            {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select SampleType.SampleTypeNo,CName,ColorName,ColorValue ");
            strSql.Append(" FROM SampleType INNER JOIN ");
            strSql.Append(" ItemColorAndSampleTypeDetail ON SampleType.SampleTypeNo = ItemColorAndSampleTypeDetail.SampleTypeNo ");
            strSql.Append(" INNER JOIN ItemColorDict ON ItemColorAndSampleTypeDetail.ColorId = ItemColorDict.ColorID ");
