...
namespace ZhiFang.DAL.Oracle.weblis
{
    ...
    public class PGroupPrint : BaseDALLisDB, IDPGroupPrint
    {
        ...
        public DataSet GetList_No_Name(ZhiFang.Model.PGroupPrint model)
        {
            ...
            strSql.Append(" order by Sort asc,PGroupPrint.Id desc ");
            return DbHelperSQL.ExecuteDataSet(strSql.ToString());
        }
        /// <summary>
        /// 获得前几行数据
        ...
        */
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(*) FROM PGroupPrint where 1=1 ");
            if (model.SectionNo != null)
            {
                strSql.Append(" and SectionNo=" + model.SectionNo + "");
            }
        ...
        public DataSet GetListByPage(ZhiFang.Model.PGroupPrint model, int nowPageNum, int nowPageSize)
        {
            ...
            {
                strSql.Append(" select  * from PGroupPrint left join PGroupPrintControl on PGroupPrint.Id=PGroupPrintControl.Id ");
                if (model.LabCode != null)
                {
                    strSql.Append(" and PGroupPrintControl.ControlLabNo='" + model.LabCode + "' ");
                }
                strSql.Append("where  ROWNUM <= '" + nowPageSize + "' and Id not in ( ");
                strSql.Append("select Id from  PGroupPrint left join PGroupPrintControl on PGroupPrint.Id=PGroupPrintControl.Id ");
                if (model.LabCode != null)
                {
                    strSql.Append(" and PGroupPrintControl.ControlLabNo='" + model.LabCode + "' ");
                }
                ...
                {
                    strSql.Append(" where 1=1 and ROWNUM <= '" + (nowPageSize * nowPageNum) + "' ");
                }
                strSql.Append(" ) order by PGroupPrint.Id ");
                return DbHelperSQL.ExecuteDataSet(strSql.ToString());
            }
            else
            {
                strSql.Append("select  * from PGroupPrint where  ROWNUM <= '" + nowPageSize + "' and Id not in  ");
                strSql.Append("(select Id from PGroupPrint where 1=1 and ROWNUM <= '" + (nowPageSize * nowPageNum) + "' ) order by Id  ");
                return DbHelperSQL.ExecuteDataSet(strSql.ToString());
            }
        }
        #endregion
