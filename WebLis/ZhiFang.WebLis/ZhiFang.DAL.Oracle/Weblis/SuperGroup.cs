...
namespace ZhiFang.DAL.Oracle.weblis
{
    ...
    public partial class SuperGroup : BaseDALLisDB, IDSuperGroup, IDBatchCopy, IDGetListByTimeStampe
    {
        ...
        public int Add(ZhiFang.Model.SuperGroup model)
        {
            ...
            {
                strSql1.Append("ParentNo,");
                strSql2.Append("" + model.ParentNo + ",");
            }
        ...
        public int Update(ZhiFang.Model.SuperGroup model)
        {
            ...
            if (model.ParentNo != null)
            {
                strSql.Append("ParentNo=" + model.ParentNo + ",");
            }
            else
            {
                strSql.Append("ParentNo= null ,");
            }
            int n = strSql.ToString().LastIndexOf(",");
            strSql.Remove(n, 1);
            strSql.Append(" where SuperGroupNo=" + model.SuperGroupNo + " ");
        ...
        public ZhiFang.Model.SuperGroup GetModel(int SuperGroupNo)
        {
            ...
            strSql.Append(" SuperGroupNo,CName,ShortName,ShortCode,Visible,DispOrder,ParentNo ");
            strSql.Append(" from SuperGroup ");
            strSql.Append(" where 1=1 and ROWNUM <= '1' and SuperGroupNo=" + SuperGroupNo + " ");
        ...
        public DataSet GetListByPage(ZhiFang.Model.SuperGroup model, int nowPageNum, int nowPageSize)
        {
            ...
            if (model != null && model.LabCode != null)
            {
                ...
                strSql.Append(" select * from SuperGroup left join B_SuperGroupControl on SuperGroup.SuperGroupNo=B_SuperGroupControl.SuperGroupNo ");
                if (model.LabCode != null)
                {
                    strSql.Append(" and B_SuperGroupControl.ControlLabNo='" + model.LabCode + "' ");
                }
                strSql.Append("where  ROWNUM <= '" + nowPageSize + "' and SuperGroup.SuperGroupNo not in ( ");
                strSql.Append("select SuperGroup.SuperGroupNo from  SuperGroup left join B_SuperGroupControl on SuperGroup.SuperGroupNo=B_SuperGroupControl.SuperGroupNo ");
                if (model.LabCode != null)
                {
                    strSql.Append(" and B_SuperGroupControl.ControlLabNo='" + model.LabCode + "' ");
                }
            ...
            else
            {
                ...
                strSql.Append("select  * from SuperGroup where  ROWNUM <= '" + nowPageSize + "' and SuperGroupNo not in  ");
                strSql.Append("(select SuperGroupNo from SuperGroup where 1=1 and ROWNUM <= '" + (nowPageSize * nowPageNum) + "' " + strLike + " ) " + strLike + " order by " + model.OrderField + "  ");
                return DbHelperSQL.ExecuteDataSet(strSql.ToString());
            }
        }
        ...
        public int DeleteByDataRow(DataRow dr)
        {
            ...
            {
                ZhiFang.Common.Log.Log.Error("ZhiFang.DAL.Oracle.weblis.SuperGroup.DeleteByDataRow同步数据时异常：", ex);
                return 0;
            }
        }
        ...
        public DataSet GetListByTimeStampe(string LabCode, int dTimeStampe)
        {
            DataSet dsAll = new DataSet();
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select SuperGroup.*,'" + LabCode + "' as LabCode,SuperGroupNo as LabSuperGroupNo from SuperGroup where 1=1 ");
            ...
            StringBuilder strSql2 = new StringBuilder();
            strSql2.Append("select B_Lab_perGroup.*,LabSuperGroupNo as SuperGroupNo from B_Lab_perGroup where 1=1 ");
            if (LabCode.Trim() != "")
            {
