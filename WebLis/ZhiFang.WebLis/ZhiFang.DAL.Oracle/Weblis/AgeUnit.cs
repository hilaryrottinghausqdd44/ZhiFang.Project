...
namespace ZhiFang.DAL.Oracle.weblis
{
	...
	{	
		...
		public int Update(ZhiFang.Model.AgeUnit model)
        {
            ...
            strSql.Remove(n, 1);
            strSql.Append(" where AgeUnitNo=" + model.AgeUnitNo + " ");
        ...
        public DataSet GetListByPage(ZhiFang.Model.AgeUnit model, int nowPageNum, int nowPageSize)
        {
            ...
            {
                strSql.Append(" select  * from AgeUnit left join AgeUnitControl on AgeUnit.AgeUnitNo=AgeUnitControl.AgeUnitNo ");
                if (model.LabCode != null)
                {
                    strSql.Append(" and AgeUnitControl.ControlLabNo='" + model.LabCode + "' ");
                }
                strSql.Append("where  ROWNUM <= '" + nowPageSize + "' and AgeUnitNo not in ( ");
                strSql.Append("select  AgeUnitNo from  AgeUnit left join AgeUnitControl on AgeUnit.AgeUnitNo=AgeUnitControl.AgeUnitNo where 1=1 ");
                if (model.LabCode != null)
                {
                    strSql.Append(" and AgeUnitControl.ControlLabNo='" + model.LabCode + "' ");
                }
                strSql.Append(" and ROWNUM <= '" + (nowPageSize * nowPageNum) + "' ) order by AgeUnit.AgeUnitNo ");
                return DbHelperSQL.ExecuteDataSet(strSql.ToString());
            }
            else
            {
                strSql.Append("select  * from AgeUnit where  ROWNUM <= '" + nowPageSize + "' and AgeUnitNo not in  ");
                strSql.Append("(select AgeUnitNo from AgeUnit where 1=1 and ROWNUM <= '" + (nowPageSize * nowPageNum) + "' ) order by AgeUnitNo  ");
                return DbHelperSQL.ExecuteDataSet(strSql.ToString());
            }
        }
		...
		public bool CopyToLab(List<string> lst)
		{
            ...
            try
            {
	            for (int i = 0; i < lst.Count; i++)
	            {
                        ...
                        strSqlControl.Append("  concat(concat(concat(concat(lis[i].Trim(),'_'),TableKey),'_'),TableKey) as " + TableKeySub + "ControlNo," + TableKey + ",'" + lst[i].Trim() + "' as ControlLabNo," + TableKey + ",UseFlag ");
