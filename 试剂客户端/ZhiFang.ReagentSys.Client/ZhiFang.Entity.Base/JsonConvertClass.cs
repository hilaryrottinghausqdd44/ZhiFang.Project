...
namespace ZhiFang.Entity.Base
{
    public class JsonConvertClass : JsonConverter
    {
        ...
        public override object ReadJson(JsonReader reader, Type objectType, object existingValue, JsonSerializer serializer)
        {
            ...
            else if (objectType == typeof(Int64) || objectType == typeof(Single) || objectType == typeof(Decimal) || objectType == typeof(Int64?))
            {
                returnObject = reader.Value;
                if (returnObject != null)
                {
            ...
            else if (objectType == typeof(Double) || objectType == typeof(Double?))//新加by douss 2021-07-23；否则double类型解析不了报异常
            {
                returnObject = reader.Value;
                if (returnObject != null)
                {
                    returnObject = double.Parse(returnObject.ToString());
                }
            }
            return returnObject;
        }
