...
namespace ZhiFang.Interceptor.ReagentSys.Client
{
    public class ModuleValidationAdvice : IMethodInterceptor
    {
        ...
        {
            //ZhiFang.Common.Log.Log.Debug("Invoke.method.Name:" + invocation.Method.Name);
    ...
    public class InterceptorAuthorityFilter : IMethodBeforeAdvice
    {
        ...
        {
            //ZhiFang.Common.Log.Log.Debug("Before.method.Name:" + method.Name);
            #region 记录用户操作日志
            IApplicationContext context = ContextRegistry.GetContext();
    ...
    public class AfterReturningAdvice : IAfterReturningAdvice
    {
        public void AfterReturning(object returnValue, MethodInfo method, object[] args, object target)
        {
            ...
            #endregion
            //ZhiFang.Common.Log.Log.Debug("AfterReturningAdvice.method.Name:" + method.Name);
        }
    }
    ...
    public class LoginValidationAdvice : IMethodInterceptor
    {
        public object Invoke(IMethodInvocation invocation)
        {
            //ZhiFang.Common.Log.Log.Debug("LoginValidationAdvice.Invoke.method.Name:" + invocation.Method.Name);
            string methodName = invocation.Method.Name;
    ...
    public class LoginAfterFilterAdvice : IAfterReturningAdvice
    {
        IBUpdateLabDataBase IBUpdateLabDataBase { get; set; }
        public void AfterReturning(object returnValue, MethodInfo method, object[] args, object target)
        {
            if (method.Name == "ZhiFang.ReagentSys.Client.ServerContract.IRBACService.RBAC_BA_Login")
            {
                string labIdStr = ZhiFang.Common.Public.Cookie.CookieHelper.Read(Entity.Base.SysPublicSet.SysDicCookieSession.LabID);
                long labID = -1;
                ...
                {
                    IApplicationContext context = ContextRegistry.GetContext();
                    IBUpdateLabDataBase = (IBUpdateLabDataBase)context.GetObject("BUpdateLabDataBase");
                    if (IBUpdateLabDataBase != null)
                    {
                        //ZhiFang.Common.Log.Log.Debug("帐号登录成功后,依帐号所属的LabID初始化或更新实验室在系统运行所需信息开始:");
                        bool result = IBUpdateLabDataBase.EditDataBaseByLabId(labID);
                        //ZhiFang.Common.Log.Log.Debug("帐号登录成功后,依帐号所属的LabID初始化或更新实验室在系统运行所需信息,结果为:" + result);
                    }
                }
            }
        }
