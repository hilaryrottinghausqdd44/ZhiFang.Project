...
namespace ZhiFang.ReagentSys.Client.Common
{
    public class MyNPOIHelper
    {
        ...
        public static bool FillExcelMoudleSheet(string strModuleFileName, string strFillFileName, Dictionary<string, string> dicCellValue)
        {
            return FillExcelMoudleSheet(strModuleFileName, strFillFileName, dicCellValue, "");
        }

        public static bool FillExcelMoudleSheet(string strModuleFileName, string strFillFileName, Dictionary<string, string> dicCellValue, string isFillEmptyCell)
        {
            ...
            try
            {
                using (FileStream fileStream = new FileStream(strModuleFileName, FileMode.Open, FileAccess.Read))
                {
                    wb = WorkbookFactory.Create(fileStream);
                    sheet = wb.GetSheetAt(0);
                    if (dicCellValue != null && dicCellValue.Count > 0)
                    {
                        ICellStyle cellStyle = sheet.Workbook.CreateCellStyle();
                        foreach (KeyValuePair<string, string> kv in dicCellValue)
                        {
                            string[] array = kv.Key.Split(',');
                            if (array.Length == 2)
                                MyNPOIHelper.FillExcelMoudleSheetCell(sheet, int.Parse(array[0]), int.Parse(array[1]), kv.Value, cellStyle, isFillEmptyCell);
                            else if (array.Length >= 3)
                            {
                                if (array[2].ToUpper() == "P")
                                    MyNPOIHelper.FillExcelMoudleSheetCellImage(sheet, int.Parse(array[0]), int.Parse(array[1]), kv.Value, array[3]);
                                else if (array[2].ToUpper() == "FC")
                                    FillExcelMoudleSheetCellFormula(wb, sheet, int.Parse(array[0]), int.Parse(array[1]), kv.Value, cellStyle, isFillEmptyCell);
                            }
                        }
                        //sheet.ForceFormulaRecalculation = true;
                    }
                }
                FileStream fileFileStream = new FileStream(strFillFileName, FileMode.Create);
                try
                {
                    int numSheet = wb.NumberOfSheets;
                    for (int i = 1; i < numSheet; i++)
                    {
                        wb.RemoveSheetAt(1);
                    }
                    wb.Write(fileFileStream);
                }
                finally
                {
            ...
            {
                boolResult = false;
                throw new Exception("填充Excel文件出错(" + strModuleFileName + ")：" + ex.Message);
            }
            return boolResult;
        }

        public static void FillExcelMoudleSheetCell(ISheet sheet, int row, int col, string value, ICellStyle cellStyle, string isFillEmptyCell)
        {
            IRow headerRow;
            ...
            if (!string.IsNullOrWhiteSpace(value))
            {
                ...
                string oldValue = cell.StringCellValue;
                if (oldValue == null)
                    oldValue = "";
                if (cellArray.Length >= 2)
                {
                    if (!string.IsNullOrWhiteSpace(cellArray[1]))
                    {
                        if (cellArray[0].ToUpper() == "I" || cellArray[0].ToUpper() == "L")
                            cell.SetCellValue(double.Parse(cellArray[1], NumberStyles.Integer));
                        else if (cellArray[0].ToUpper() == "F" || cellArray[0].ToUpper() == "R")
                            cell.SetCellValue(double.Parse(cellArray[1], NumberStyles.Float));
                        //else if (cellArray[0] == "C")
                        //    cell.SetCellValue(cellArray[1]);
                        //else if (cellArray[0] == "D")
                        //    cell.SetCellValue(cellArray[1]);
                        else
                            cell.SetCellValue(oldValue + cellArray[1]);
                    }
                    else//填充的值为空
                        cell.CellStyle = SetEmptyCellStyle(cell.CellStyle, cellStyle, isFillEmptyCell);
                }
                else
        ...
        public static void FillExcelMoudleSheetCellImage(ISheet sheet, int row, int col, string filePath, string empName)
        {
            if ((!string.IsNullOrEmpty(filePath)) && File.Exists(filePath))
            {
                ...
                IClientAnchor anchor = patriarch.CreateAnchor(0, 0, 0, 0, col, row, col + 1, row + 1);
                IPicture pict = patriarch.CreatePicture(anchor, pictureIdx);
            }
            else
            {
                FillExcelMoudleSheetCell(sheet, row, col, empName, null, "");
            }
        }

        public static void FillExcelMoudleSheetCellFormula(IWorkbook wb, ISheet sheet, int row, int col, string value, ICellStyle cellStyle, string isFillEmptyCell)
        {
            ...
            XSSFFormulaEvaluator feval = new XSSFFormulaEvaluator((XSSFWorkbook)wb);
            feval.EvaluateFormulaCell(cell);
        }
        private static ICellStyle SetEmptyCellStyle(ICellStyle cellStyle, ICellStyle emptyStyle, string isFillEmptyCell)
        {
            if (!string.IsNullOrEmpty(isFillEmptyCell) && cellStyle != null && emptyStyle != null)
            {
                if (isFillEmptyCell == "1")
                {
                    CopyCellStyle(cellStyle, emptyStyle);
                    emptyStyle.BorderDiagonalLineStyle = BorderStyle.Thin;
                    emptyStyle.BorderDiagonal = BorderDiagonal.Forward;//斜线 右上角-左下角
                    emptyStyle.BorderDiagonalColor = IndexedColors.Black.Index;
                    cellStyle = emptyStyle;
                }
                if (isFillEmptyCell == "2")
                {
                    CopyCellStyle(cellStyle, emptyStyle);
                    emptyStyle.BorderDiagonalLineStyle = BorderStyle.Thin;
                    emptyStyle.BorderDiagonal = BorderDiagonal.Backward;//斜线 左上角-右下角
                    emptyStyle.BorderDiagonalColor = IndexedColors.Black.Index;
                    cellStyle = emptyStyle;
                }
            }
        ...
        public static IList<string> ReadExcelMoudleFile(string strFileName)
        {
        ...
        private static IList<string> GetExcelMoudleFile(ISheet sheet, int HeaderRowIndex, bool needHeader)
        {
        ...
        private static string MyGetCellValue(ICell cell)
        {
            string strValue = "";
        ...
        private static int ExportDTByDataTable(DataTable dtSource, string strHeaderText, HSSFWorkbook workbook, HSSFSheet sheet, HSSFCellStyle dateStyle, int firstRow)
        {
            ...
            if (isHeader)
            {
                ...
                foreach (DataColumn column in dtSource.Columns)
                {
                    ...
                    //设置列宽
                    //宽度太长时生成文件出错;最长为40个字符--10240/256=40;
                    long width = (arrColWidth[column.Ordinal] + 1) * 256;
                    int width1 = 0;
                    if (width1 > 10240) width1 = 10240;
                    sheet.SetColumnWidth(column.Ordinal, width1);
        ...
        private static int ExportDTByDataTable(DataTable dtSource, string strHeaderText, XSSFWorkbook workbook, XSSFSheet sheet, XSSFCellStyle dateStyle, int firstRow)
        {
            ...
            if (isHeader)
            {
                ...
                foreach (DataColumn column in dtSource.Columns)
                {
                    ...
                    //设置列宽
                    //int.TryParse(width.ToString(), out width1);
                    //ZhiFang.Common.Log.Log.Error("TryParse:" + int.TryParse(width.ToString(), out width1)+ "GetColumnWidth:" + sheet.GetColumnWidth(column.Ordinal));
                    //ZhiFang.Common.Log.Log.Error("ColumnName:" +column.ColumnName + ",SetColumnWidth:" + width);
                    //宽度太长时生成文件出错;最长为40个字符--10240/256=40;
                    long width = (arrColWidth[column.Ordinal] + 1) * 256;
                    int width1 = 0;
                    if (width1 > 10240) width1 = 10240;
            ...
            foreach (DataRow row in dtSource.Rows)
            {
                ...
                foreach (DataColumn column in dtSource.Columns)
                {
                    ...
                    dataRow.GetCell(column.Ordinal).CellStyle = curCellStyle;
                    string drValue = row[column].ToString();
        ...
        public static bool ExportDTtoExcel(DataTable dtSource, string strHeaderText, string strFileName)
        {
            ...
            {
                ZhiFang.Common.Log.Log.Error("ExportDTtoExcel.生成Excel文件失败:" + ex.StackTrace);
                return false;
            }
            return true;
        }
