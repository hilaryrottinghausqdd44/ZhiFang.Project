        ...
        public static Stream SavePdfReport(DataSet dataSet, long labId, string pdfName, string publicTemplateDir, string reportSubDir, string frx, bool isClose)
        {
            ...
            if (dataSet.Tables.Count > 0 && dataSet.Tables[0].Rows.Count > 0)
            {
                ...
                try
                {
                    ...
                    //string connStr = ConfigHelper.GetConfigString("db.ReportConnString");
                    //if (!string.IsNullOrEmpty(connStr))
                    //    report.SetParameterValue(dataSet.DataSetName, connStr);
                ...
                {
                    ZhiFang.Common.Log.Log.Error("生成报表错误.Message" + ex.Message);
                    ZhiFang.Common.Log.Log.Error("生成报表错误.StackTrace" + ex.StackTrace);
                }
