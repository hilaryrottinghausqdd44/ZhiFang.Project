 ...
 * @version 2020-09-21
 */
Ext.define('Shell.interface3.Viewport', {
	...
	SYS_MAIN_INFO: {
		...
		url: '#Shell.class.sysbase.main.home.tab.App',
		closable: false
	},
	/**小图标根目录*/
	...
	/**平台统一登录定制接口*/
	InterfaceLoginUrl: '/ReaExternalInterface.svc/RBAC_BA_AnYangLiuYuanUnifiedLogin',
	...
	afterRender: function() {
		...
		me.ContentTab.on({
			tabchange: function(tabPanel, newCard, oldCard, eOpts) {
				var moduleId = newCard.itemId.split('_').slice(-1).join("");
		...
		//me.initInfo();
		me.InterfaceLogin(function(data){
			if(data.success){
				me.afterLogin();
	...
	createTopToolbar: function() {
		...
		var items = [{
			...
			text: '显示功能菜单',
			tooltip: '显示功能菜单',
			itemId: 'showFunctionTree',
			hidden: true,
			handler: function() {
				this.hide();
				me.FunctionTree.expand();
				this.ownerCt.getComponent('hideFunctionTree').show();
			}
		}, {
			iconCls: 'button-arrow-in',
			text: '隐藏功能菜单',
			tooltip: '隐藏功能菜单',
			itemId: 'hideFunctionTree',
			handler: function() {
				this.hide();
				me.FunctionTree.collapse();
				this.ownerCt.getComponent('showFunctionTree').show();
			}
		}, '-', {
			xtype: 'toolbar',
			border: false,
	...
	/**显示登录面板*/
	showLoginWin: function(noCloseBtn) {
		var me = this;
		JShell.Win.open('Shell.interface2.Login', {
			formtype: 'add',
			resizable: false,
	...
	initSystemInfo: function() {
		...
		//当前账户名
		me.account = JShell.System.Cookie.get(JShell.System.Cookie.map.ACCOUNTNAME);
	...
	onSystimesChange: function() {
		...
		var timeout = setTimeout(function() {
			...
			if(!isValid) {
				...
				var isServerError = JShell.System.Date.isError();
				if(isServerError) {
					clearTimeout(timeout);
					me.LockAccount(); //锁定账户
				} else {
	...
	onHelpClick: function() {
		...
		JShell.Win.open('Shell.class.sysbase.main.Help', {
			//resizable: false,
			title: '帮助信息',
			ModuleId: id,
			width: '90%',
	...
	InterfaceLogin : function(callback) {
		var me = this;
		var params = {
			"orgCode":me.orgCode,//医院代码
			"staffCode":me.staffCode,//员工工号
			"deptCode":me.deptCode,//科室/病区代码
			"timestamp":me.timestamp,//（时间戳）
			"sign":me.sign,//（签名值）
		};
		var url = JShell.System.Path.ROOT + me.InterfaceLoginUrl;
