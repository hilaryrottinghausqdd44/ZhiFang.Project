 ...
 */
layui.extend({
    tableChild:'ux/other/soulTable/tableChild',
    tableMerge:'ux/other/soulTable/tableMerge',
    tableFilter:'ux/other/soulTable/tableFilter'
  ...
  var tableFilter = layui.tableFilter,
    ...
    defaultConfig = { // 默认配置开关
      ...
      filter: false // 筛选及记忆相关(自动记忆)//    manualOpen: false // 如果此配置开启，则右键保存后才有记忆功能 
    },
    _BODY = $('body'),
    _DOC = $(document);
  ...
  var mod = {
    render: function (myTable) {
      ...
      if (curConfig.filter && curConfig.filter.cache) {
		...
		if (!originCols[myTable.id]) { // 只在第一次 render 时生效
		  ...
		  // start开启手动记忆功能
		 if(curConfig.filter.manualOpen){
		...
		}else if(curConfig.filter.manualOpen){ // 第二次（或者多次后）
			var curTableSession = localStorage.getItem("manualOpen" + storeKey);
			if (curTableSession) {
				this.updateCols(myTable, this.deepParse(curTableSession));
			} else {
				if(localStorage.getItem('origin' + storeKey)){
					this.updateCols(myTable, this.deepParse(localStorage.getItem('origin' + storeKey)));
				}else {
					localStorage.setItem('origin' + storeKey, colsStr);
					localStorage.removeItem(storeKey);
				}
    ...
    clearCache: function (myTable) {
      ...
      // 关于页面复用以及多列表复用一套地址，这个地方清除缓存需要改变
      localStorage.removeItem(location.pathname + location.hash + tableId)
	  localStorage.removeItem("manualOpen" +location.pathname + location.hash + tableId)
