 ...
 * @version 2018-01-02
 */
Ext.define('Shell.class.rea.order.comp.show.DocForm', {
    ...
    /**实验室消息推送服务*/
    onLabPushMessageUrl:'/WeiXinAppService.svc/PushConfirmBmsCenOrderDoc',
	/**内容周围距离*/
	...
	initComponent: function() {
		...
		me.buttonToolbarItems = [{
			...
			text:'<b style="color:#ffffff;">订单确认</b>',
			itemId:'save',
			tooltip:'<b>订单确认</b>',
			iconCls:'button-save',
			...
			handler:function(){
				JShell.Msg.confirm({
					msg:'订单确认后将自动产生供货单，是否确定对选中的订单进行确认？'
				},function(but){
					if (but != "ok") return;
					me.updateOrder();
				});
	...
	updateOrder:function(){
		...
		var params = {
			entity:{
				...
				Status:me.IsAutoCreateBmsCenSaleDoc,
				CompMemo:values.BmsCenOrderDoc_CompMemo
			},
			fields:'Id,Status,CompMemo'
		};
		...
		JShell.Server.post(url,params,function(data){
			...
			if(data.success){
				...
				me.onLabPushMessage(values.BmsCenOrderDoc_Id);
			}else{
	...
	//实验室消息推送
	onLabPushMessage:function(id){
		var me = this,
			url = JShell.System.Path.ROOT + me.onLabPushMessageUrl + '?Id=' + id;
			
		me.showMask("消息推送中...");//显示遮罩层
		JShell.Server.get(url,function(data){
			me.hideMask();//隐藏遮罩层
			if(data.success){
				if(me.showSuccessInfo) JShell.Msg.alert("给实验室推送信息成功！",null,me.hideTimes);
			}else{
				JShell.Msg.error(data.msg);
			}
		});
