...
Ext.define('Shell.class.rea.client.shelves.storage.Form', {
	...
	createItems: function() {
		...
		items.push({
			...
			fieldLabel: '代码',
			name: 'ReaStorage_ShortCode',
			emptyText: '必填项',
			allowBlank: false
		}, {
			name: 'ReaStorage_IsMainStorage',
			fieldLabel: '总库标志',
			...
			fieldLabel: '启用',
			name: 'ReaStorage_Visible',
			xtype: 'uxBoolComboBox',
			value: true,
			hasStyle: true
		}, {
			fieldLabel: '物资对照码',
			name: 'ReaStorage_MatchCode'
		}, {
			fieldLabel: '专项1',
			name: 'ReaStorage_ZX1',
			hidden: true
	...
	getAddParams: function() {
		...
		var entity = {
			...
			MatchCode: values.ReaStorage_MatchCode,
			Visible: values.ReaStorage_Visible ? 1 : 0,
			IsMainStorage: values.ReaStorage_IsMainStorage ? 1 : 0,
		};
	...
	getEditParams: function() {
		...
		var fields = [
			...
			'ZX2', 'ZX3', 'DispOrder', 'Memo', 'Visible,IsMainStorage,MatchCode'
		];
