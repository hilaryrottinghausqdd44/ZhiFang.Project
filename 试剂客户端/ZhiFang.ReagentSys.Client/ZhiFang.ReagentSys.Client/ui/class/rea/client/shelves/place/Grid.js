...
Ext.define('Shell.class.rea.client.shelves.place.Grid', {
	...
	IsValidate: function() {
		...
		//验证库房名称不能为空
		for(var i = 0; i < len; i++) {
			if(!changedRecords[i].get('ReaPlace_CName') || !changedRecords[i].get('ReaPlace_ShortCode')) {
				isExect = false;
			}
		}
		return isExect;
	...
	oneAdd: function(i, record) {
		...
		var entity = {
			...
			MatchCode: record.get('ReaPlace_MatchCode')
		};
	...
	updateOne: function(i, record) {
		...
		var params = Ext.JSON.encode({
			...
			fields: 'Id,CName,ShortCode,Visible,DispOrder,MatchCode,Memo'
		});
		JShell.Server.post(url, params, function(data) {
			if(data.success) {
				me.saveCount++;
