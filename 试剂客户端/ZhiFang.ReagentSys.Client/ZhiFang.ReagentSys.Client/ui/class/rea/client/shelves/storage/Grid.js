...
Ext.define('Shell.class.rea.client.shelves.storage.Grid', {
	...
	/**排序字段*/
	defaultOrderBy: [{
		property: 'ReaStorage_IsMainStorage',
		direction: 'DESC'
	}, {
	...
	/**用户UI配置Key*/
	userUIKey: 'shelves.storage.Grid',
	/**用户UI配置Name*/
	userUIName: "库房列表",
	...
	createGridColumns: function() {
		...
		var columns = [{
			...
			dataIndex: 'ReaStorage_IsMainStorage',
			text: '总库标志',
			width: 75,
			align: 'center',
			type: 'bool',
			...
			dataIndex: 'ReaStorage_MatchCode',
			text: '物资对照码',
			width: 80,
			editor: {},
			defaultRenderer: true
	...
	IsValidate: function() {
		...
		//验证库房名称不能为空
		for(var i = 0; i < len; i++) {
			if(!changedRecords[i].get('ReaStorage_CName') || !changedRecords[i].get('ReaStorage_ShortCode')) {
				isExect = false;
			}
		}
		return isExect;
	...
	oneAdd: function(i, record) {
		...
		var entity = {
			...
			MatchCode: record.get('ReaStorage_MatchCode')
		};
	...
	updateOne: function(i, record) {
		...
		var params = Ext.JSON.encode({
			...
			fields: 'Id,CName,ShortCode,Visible,DispOrder,Memo,IsMainStorage,MatchCode'
		});
		JShell.Server.post(url, params, function(data) {
			if(data.success) {
				me.saveCount++;
	...
	showMemoText: function(value, meta) {
		...
		var qtipValue = "<p border=0 style='vertical-align:top;font-size:12px; word-break:break-all;'>" + v + "</p>";
