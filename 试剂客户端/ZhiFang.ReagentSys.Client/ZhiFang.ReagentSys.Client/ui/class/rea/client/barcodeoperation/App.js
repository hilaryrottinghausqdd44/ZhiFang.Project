 ...
 * @version 2019-01-08
 */
Ext.define('Shell.class.rea.client.barcodeoperation.App', {
	...
	createItems: function() {
		...
		me.Grid = Ext.create('Shell.class.rea.client.barcodeoperation.Grid', {
			region: 'center',
			header: false,
			itemId: 'Grid'
		});
	...
	setExternalWhere: function(record) {
		...
		if(leaf == true) { //货架
			hql = 'reagoodsbarcodeoperation.PlaceID=' + id;
		} else { //库房
			if(id == 0 || id == "0") {
				if(me.isEmpPermission) {
					...
					hql = 'reagoodsbarcodeoperation.StorageID in (' + tidArr.join(',') + ")";
				}
			} else {
				hql = 'reagoodsbarcodeoperation.StorageID=' + id;
			}
		}
