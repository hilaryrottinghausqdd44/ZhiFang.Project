...
Ext.define('Shell.class.rea.client.goods2.AddPanel', {
	...
	createItems: function() {
		...
		me.BatchGrid = Ext.create('Shell.class.rea.client.goodslot.Grid', {
			title: '批号信息',
			itemId: 'BatchGrid',
			disabled: true,
			ReaGoodsNo: me.ReaGoodsNo,
			GoodsID: me.GoodsID,
			GoodsCName: me.GoodsCName,
			IsNeedPerformanceTest: me.IsNeedPerformanceTest,
			/**默认每页数量*/
			defaultPageSize: 100,
			/**带分页栏*/
	...
	onListeners: function() {
		...
		me.Form.on({
			save: function(p, id, bo) {
				if (!bo) return;
				var values = p.getForm().getValues();
				//货品名称
				var CName = values.ReaGoods_CName;
				var IsNeedPerformanceTest = values.ReaGoods_IsNeedPerformanceTest;
				/**货品ID*/
				me.GoodsID = id;
				/**货品名称*/
				me.GoodsCName = CName;
				...
				me.Form.PK = id;
				me.Form.isEdit(id);
				me.BatchGrid.ReaGoodsNo = values.ReaGoods_ReaGoodsNo;
				me.BatchGrid.GoodsID = id;
				me.BatchGrid.GoodsCName = CName;
				me.BatchGrid.IsNeedPerformanceTest = IsNeedPerformanceTest;
				...
				me.BatchGrid.setDisabled(false);
				//货品编码
				var ReaGoodsNo = values.ReaGoods_ReaGoodsNo;
				if (!ReaGoodsNo) me.RegisterGrid.setDisabled(true);
				if (ReaGoodsNo) {
					me.RegisterGrid.setDisabled(false);
				}
			}
		...
		me.on({
			...
			tabchange: function(tabPanel, newCard, oldCard, eOpts) {
				var me = this;
				var bo = false;
				if (newCard.itemId == 'Form') bo = true;
				me.isShowAddSave(bo);
			}
		});
	},
	isShowAddSave: function(bo) {
		var me = this;
		var buttonsToolbar = me.getComponent('buttonsToolbar');
		var btnAddSave = buttonsToolbar.getComponent('btnAddSave');
		btnAddSave.setVisible(bo)
	},
	//取消编辑
	cancelEdit: function() {
		var me = this;
	...
	isAdd: function() {
		...
		me.GoodsCName = '';
		me.ReaGoodsNo = '';
		me.BatchGrid.GoodsID = null;
		me.BatchGrid.GoodsCName = '';
		me.RegisterGrid.GoodsID = null;
		me.RegisterGrid.GoodsCName = '';
		me.Form.isAdd();
