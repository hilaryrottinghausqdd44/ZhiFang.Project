 ...
 * @version 2019-01-18
 */
Ext.define('Shell.class.rea.client.inventory.overage.DtlGrid', {
	...
	canEdit: true,
	/**用户UI配置Key*/
	userUIKey: 'inventory.overage.DtlGrid',
	/**用户UI配置Name*/
	userUIName: "盘盈入库明细列表",
	...
	createGridColumns: function() {
		...
		var columns = [{
			...
			dataIndex: 'ReaBmsInDtl_GoodsSName',
			text: '简称',
			width: 90,
			defaultRenderer: true,
			sortable: false
			...
			dataIndex: 'ReaBmsInDtl_GoodsLotID',
			text: '批号ID',
			hidden: true,
			defaultRenderer: true
		}];
	...
	getSaveDtlOne: function(record) {
		...
		var entity = {
			...
			UnitMemo: record.get("ReaBmsInDtl_UnitMemo"),
		...
		var GoodsLotID = record.get("ReaBmsInDtl_GoodsLotID");
		if(GoodsLotID){
			entity.GoodsLotID=GoodsLotID;			
		}
