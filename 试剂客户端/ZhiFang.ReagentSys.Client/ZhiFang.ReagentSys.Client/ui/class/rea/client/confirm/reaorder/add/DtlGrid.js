...
Ext.define('Shell.class.rea.client.confirm.reaorder.add.DtlGrid', {
	...
	/**获取数据服务路径*/
	selectUrl: '/ReaManageService.svc/ST_UDTO_SearchReaBmsCenSaleDtlConfirmVOOfConfirmTypeByHQL?isPlanish=true&confirmType=reaorder',
	...
	DocOrderId: null,
	/**选择好的订单的待验收订单明细集合*/
	ReaOrderDtlOfConfirmVOList: [],
	OTYPE: "reaorder",
	GoodsEnum: {},
	
	/**用户UI配置Key*/
	userUIKey: 'confirm.reaorder.add.DtlGrid',
	/**用户UI配置Name*/
	userUIName: "验货单明细列表",
	...
	afterRender: function() {
		...
		me.on({
			nodata: function(g) {
				...
				me.addRecordsOfOrderDtl(me.ReaOrderDtlOfConfirmVOList);
			}
		});
	...
	initComponent: function() {
		var me = this;
		me.addEvents('onSetConfirmInfo', 'changeSumTotal', 'choseOrderClick');
		//数据列
			...
			handler: function() {
				...
				if(me.store.getCount() > 0) {
					JShell.Msg.confirm({
						...
						me.DocOrderId = null;
						me.fireEvent('choseOrderClick', me);
						me.ReaOrderDtlOfConfirmVOList = [];
						me.store.removeAll();
						me.showChooseOrder();
					});
				} else {
		...
		var columns = [{
			...
			renderer: function(value, meta) {
				...
				if(v && ("" + v).indexOf(".") >= 0) {
					...
					meta.tdAttr = 'data-qtip="<b>购进数=接收数+拒收数</b>"';
				}
				return v;
			}
		}, {
			...
			dataIndex: 'ReaSaleDtlConfirmVO_ReaBmsCenSaleDtlConfirm_OrderDocID',
			sortable: false,
			text: '订单Id',
			hidden: true,
			defaultRenderer: true
		}, {
			dataIndex: 'ReaSaleDtlConfirmVO_ReaBmsCenSaleDtlConfirm_OrderDtlID',
			sortable: false,
			text: '订单明细Id',
			hidden: true,
			defaultRenderer: true
		}, {
			dataIndex: 'ReaSaleDtlConfirmVO_ReaBmsCenSaleDtlConfirm_OrderDocNo',
			sortable: false,
			text: '订单总单号',
			hidden: true,
			defaultRenderer: true
	...
	onRefreshClick: function() {
		var me = this;
		if((!me.ReaCompID || !me.DocOrderId) && !me.PK) {
			me.store.removeAll();
			var error = me.errorFormat.replace(/{msg}/, "请选择供货商或订单后再操作！");
			me.getView().update(error);
		} else if(me.PK || me.DocOrderId) {
			me.onSearch();
		}
	},
		...
		var config = {
			...
			listeners: {
				accept: function(p, record) {
					if(record) {
						var docOrderId = record.get("ReaBmsCenOrderDoc_Id");
						me.isConfirmOfDocOrderId(docOrderId, function(data) {
							if(data.success == false) {
								...
								me.getReaOrderDtlOfConfirmVOList(p);
								me.setButtonsDisabled();
								p.close();
							}
						});
	...
	isConfirmOfDocOrderId: function(orderId, callback) {
		var me = this;
		var selectUrl = '/ReaManageService.svc/ST_UDTO_SearchOrderIsConfirmOfByOrderId';
		var url = (selectUrl.slice(0, 4) == 'http' ? '' : JShell.System.Path.ROOT) + selectUrl;
		url = url + "?orderId=" + orderId;
		if(me.PK) {
			url = url + "&confirmId=" + confirmId;
		}
	...
	showChoseReaOrderDtl: function() {
		...
		me.store.each(function(record) {
			...
			if(record.get("ReaSaleDtlConfirmVO_ReaBmsCenSaleDtlConfirm_BarCodeType") == "1") {
				var dtlId = record.get("ReaSaleDtlConfirmVO_ReaBmsCenSaleDtlConfirm_OrderDtlID");
				arrId.push("reabmscenorderdtl.Id!=" + dtlId);
			}
		});
		...
		var win = JShell.Win.open('Shell.class.rea.client.confirm.choose.order.OrderDtlCheck', config);
		win.show();
		if(arrId.length > 0) {
			var dtlIdStr = "(" + arrId.join(" and ") + ")";
			where.push(dtlIdStr);
		}
		where.push("reabmscenorderdtl.OrderDocID=" + me.DocOrderId);
	...
	addRecordsOfOrderDtl: function(list) {
		...
		for(var i = 0; i < list.length; i++) {
			...
			var entity = {
				...
				"ReaSaleDtlConfirmVO_ReaBmsCenSaleDtlConfirm_OrderDocID": objDtl.ReaOrderDtlOfConfirmVO_OrderDocID,
				"ReaSaleDtlConfirmVO_ReaBmsCenSaleDtlConfirm_OrderDtlID": objDtl.ReaOrderDtlOfConfirmVO_Id,
				"ReaSaleDtlConfirmVO_ReaBmsCenSaleDtlConfirm_OrderDocNo": objDtl.ReaOrderDtlOfConfirmVO_OrderDocNo,
				"ReaSaleDtlConfirmVO_ReaBmsCenSaleDtlConfirm_BarCodeType": objDtl.ReaOrderDtlOfConfirmVO_BarCodeType,

				"ReaSaleDtlConfirmVO_ReaBmsCenSaleDtlConfirm_ReaGoodsName": objDtl.ReaOrderDtlOfConfirmVO_ReaGoodsName,
				"ReaSaleDtlConfirmVO_ReaBmsCenSaleDtlConfirm_ReaGoodsID": objDtl.ReaOrderDtlOfConfirmVO_ReaGoodsID,
				"ReaSaleDtlConfirmVO_ReaBmsCenSaleDtlConfirm_CompGoodsLinkID": objDtl.ReaOrderDtlOfConfirmVO_CompGoodsLinkID,
				"ReaSaleDtlConfirmVO_ReaBmsCenSaleDtlConfirm_LabcGoodsLinkID": "",
				"ReaSaleDtlConfirmVO_ReaBmsCenSaleDtlConfirm_GoodsUnit": objDtl.ReaOrderDtlOfConfirmVO_GoodsUnit,

				"ReaSaleDtlConfirmVO_ReaBmsCenSaleDtlConfirm_UnitMemo": objDtl.ReaOrderDtlOfConfirmVO_UnitMemo,
				"ReaSaleDtlConfirmVO_ReaBmsCenSaleDtlConfirm_BiddingNo": objDtl.ReaOrderDtlOfConfirmVO_BiddingNo,
				"ReaSaleDtlConfirmVO_ReaBmsCenSaleDtlConfirm_LotNo": objDtl.ReaOrderDtlOfConfirmVO_LotNo,
				"ReaSaleDtlConfirmVO_ReaBmsCenSaleDtlConfirm_ApproveDocNo": objDtl.ReaOrderDtlOfConfirmVO_ReaGoods_ApproveDocNo,
				"ReaSaleDtlConfirmVO_ReaBmsCenSaleDtlConfirm_RegisterNo": objDtl.ReaOrderDtlOfConfirmVO_ReaGoods_RegistNo,

				"ReaSaleDtlConfirmVO_ReaGoodsEName": objDtl.ReaOrderDtlOfConfirmVO_ReaGoods_EName,
				"ReaSaleDtlConfirmVO_ReaGoodsSName": objDtl.ReaOrderDtlOfConfirmVO_ReaGoods_SName,
				"ReaSaleDtlConfirmVO_ReaBmsCenSaleDtlConfirm_AcceptMemo": objDtl.ReaOrderDtlOfConfirmVO_AcceptMemo,
				...
				"ReaSaleDtlConfirmVO_ReaBmsCenSaleDtlConfirmLinkVOListStr": objDtl.ReaOrderDtlOfConfirmVO_ReaBmsCenSaleDtlConfirmLinkVOListStr,

				"ReaSaleDtlConfirmVO_ReaBmsCenSaleDtlConfirm_ReaGoodsNo": objDtl.ReaOrderDtlOfConfirmVO_ReaGoodsNo,
				"ReaSaleDtlConfirmVO_ReaBmsCenSaleDtlConfirm_ProdGoodsNo": objDtl.ReaOrderDtlOfConfirmVO_ProdGoodsNo,
				"ReaSaleDtlConfirmVO_ReaBmsCenSaleDtlConfirm_CenOrgGoodsNo": objDtl.ReaOrderDtlOfConfirmVO_CenOrgGoodsNo,
				"ReaSaleDtlConfirmVO_ReaBmsCenSaleDtlConfirm_GoodsNo": objDtl.ReaOrderDtlOfConfirmVO_GoodsNo,

				"ReaSaleDtlConfirmVO_ReaBmsCenSaleDtlConfirm_IsNeedPerformanceTest": objDtl.ReaOrderDtlOfConfirmVO_ReaGoods_IsNeedPerformanceTest
			};
			if(objDtl.ReaOrderDtlOfConfirmVO_ReaGoods_GoodsSort) {
				entity["ReaSaleDtlConfirmVO_ReaBmsCenSaleDtlConfirm_GoodsSort"] = objDtl.ReaOrderDtlOfConfirmVO_ReaGoods_GoodsSort;
			}
			if(objDtl.ReaOrderDtlOfConfirmVO_InvalidDate) entity.ReaSaleDtlConfirmVO_ReaBmsCenSaleDtlConfirm_InvalidDate = JShell.Date.toString(objDtl.ReaOrderDtlOfConfirmVO_InvalidDate, true);
			if(objDtl.ReaOrderDtlOfConfirmVO_ReaGoods_RegistNoInvalidDate) entity.ReaSaleDtlConfirmVO_ReaBmsCenSaleDtlConfirm_RegisterInvalidDate = JShell.Date.toString(objDtl.ReaOrderDtlOfConfirmVO_ReaGoods_RegistNoInvalidDate, true);

			if(objDtl.ReaOrderDtlOfConfirmVO_Price) {
				entity.ReaSaleDtlConfirmVO_ReaBmsCenSaleDtlConfirm_Price = parseFloat(objDtl.ReaOrderDtlOfConfirmVO_Price);
			} else {
				entity.ReaSaleDtlConfirmVO_ReaBmsCenSaleDtlConfirm_Price = 0;
			}
			//订单购进数
			if(objDtl.ReaOrderDtlOfConfirmVO_DtlGoodsQty) entity.ReaSaleDtlConfirmVO_DtlGoodsQty = parseFloat(objDtl.ReaOrderDtlOfConfirmVO_DtlGoodsQty);
			//订购数=原订单明细购进数
			if(!entity.ReaSaleDtlConfirmVO_DtlGoodsQty && objDtl.ReaOrderDtlOfConfirmVO_GoodsQty) entity.ReaSaleDtlConfirmVO_DtlGoodsQty = parseFloat(objDtl.ReaOrderDtlOfConfirmVO_GoodsQty);
			//已接收数
			if(objDtl.ReaOrderDtlOfConfirmVO_ReceivedCount) entity.ReaSaleDtlConfirmVO_ReceivedCount = parseFloat(objDtl.ReaOrderDtlOfConfirmVO_ReceivedCount);
			//已拒收数
			if(objDtl.ReaOrderDtlOfConfirmVO_RejectedCount) entity.ReaSaleDtlConfirmVO_RejectedCount = parseFloat(objDtl.ReaOrderDtlOfConfirmVO_RejectedCount);
			//可验收数
			if(objDtl.ReaOrderDtlOfConfirmVO_ConfirmCount) entity.ReaSaleDtlConfirmVO_ConfirmCount = parseFloat(objDtl.ReaOrderDtlOfConfirmVO_ConfirmCount);

			//供货订单导入处理
			if(objDtl.ReaOrderDtlOfConfirmVO_LotNo) {
				entity.ReaSaleDtlConfirmVO_ReaBmsCenSaleDtlConfirm_LotNo = objDtl.ReaOrderDtlOfConfirmVO_LotNo;
			}
			if(objDtl.ReaOrderDtlOfConfirmVO_ProdDate) {
				entity.ReaSaleDtlConfirmVO_ReaBmsCenSaleDtlConfirm_ProdDate = JShell.Date.toString(objDtl.ReaOrderDtlOfConfirmVO_ProdDate, true);
			}
			if(objDtl.ReaOrderDtlOfConfirmVO_InvalidDate) {
				entity.ReaSaleDtlConfirmVO_ReaBmsCenSaleDtlConfirm_InvalidDate = JShell.Date.toString(objDtl.ReaOrderDtlOfConfirmVO_InvalidDate, true);
			}
			if(objDtl.ReaOrderDtlOfConfirmVO_RegisterNo) {
				entity.ReaSaleDtlConfirmVO_ReaBmsCenSaleDtlConfirm_RegisterNo = objDtl.ReaOrderDtlOfConfirmVO_RegisterNo;
			}
			if(objDtl.ReaOrderDtlOfConfirmVO_StorageType) {
				entity.ReaSaleDtlConfirmVO_ReaBmsCenSaleDtlConfirm_StorageType = objDtl.ReaOrderDtlOfConfirmVO_StorageType;
			}
			//本次供货数
			if(objDtl.ReaOrderDtlOfConfirmVO_SaleGoodsQty) {
				entity.ReaSaleDtlConfirmVO_ReaBmsCenSaleDtlConfirm_AcceptCount = parseFloat(objDtl.ReaOrderDtlOfConfirmVO_SaleGoodsQty);

			}
			var AcceptCount = entity.ReaSaleDtlConfirmVO_ReaBmsCenSaleDtlConfirm_AcceptCount;
			var RefuseCount = entity.ReaSaleDtlConfirmVO_ReaBmsCenSaleDtlConfirm_RefuseCount;
			...
			if(!Price) Price = 0;
			entity.ReaSaleDtlConfirmVO_ReaBmsCenSaleDtlConfirm_GoodsQty = parseFloat(AcceptCount) + parseFloat(RefuseCount);
			entity.ReaSaleDtlConfirmVO_ReaBmsCenSaleDtlConfirm_SumTotal = parseFloat(AcceptCount) * parseFloat(Price);
			//默认冷链信息
	...
	onScanCodeUrl: function(barCode) {
		...
		var params = "?reaCompID=" + me.ReaCompID + "&serialNo=" + barCode2 + "&scanCodeType=reaorder&bobjectID=" + me.DocOrderId;
		//params=JShell.String.encode(params);
		url = url + params;
		JShell.Server.get(url, function(data) {
			if(data.success) {
				if(data.value) {
	...
	onScanCodeUrlAfterOfBoxAndExistDtl: function(record, reaBarCodeVO, barCode) {
		...
		if(record.get("ReaSaleDtlConfirmVO_ReaBmsCenSaleDtlConfirm_GoodsSort")) {
			reaBarCodeVO.GoodsSort = record.get("ReaSaleDtlConfirmVO_ReaBmsCenSaleDtlConfirm_GoodsSort");
		}
		...
		if(invalidDate == "" && reaBarCodeVO.InvalidDate) {
	...
	onScanCodeUrlAfterOfBoxAndNotExistDtl: function(reaBarCodeVO, barCode) {
		var me = this;
		var info = "条码为:" + barCode + ",货品名称为:" + reaBarCodeVO.CName + ",不存在当前订单的验收明细中!";
		JShell.Msg.error(info);
		me.gettxtScanCode().setValue("");
		me.gettxtScanCode().focus();
	},
	...
	validatorSave: function() {
		...
		me.store.each(function(record) {
			...
			var orderId = record.get("ReaSaleDtlConfirmVO_ReaBmsCenSaleDtlConfirm_OrderDocID");
			var orderDtId = record.get("ReaSaleDtlConfirmVO_ReaBmsCenSaleDtlConfirm_OrderDtlID");
			var InvalidDate = record.get("ReaSaleDtlConfirmVO_ReaBmsCenSaleDtlConfirm_InvalidDate");
			...
			if(BarCodeType == "1") {
				...
				me.store.queryBy(function(record2) {
					if(record2.get('ReaSaleDtlConfirmVO_ReaBmsCenSaleDtlConfirm_OrderDtlID') == dtlId) {
	...
	getSaveOneInfo: function(record) {
		...
		entity.OrderDocNo = record.get("ReaSaleDtlConfirmVO_ReaBmsCenSaleDtlConfirm_OrderDocNo");

		var orderId = record.get("ReaSaleDtlConfirmVO_ReaBmsCenSaleDtlConfirm_OrderDocID");
		var orderDtId = record.get("ReaSaleDtlConfirmVO_ReaBmsCenSaleDtlConfirm_OrderDtlID");
		var strDataTimeStamp2 = "1,2,3,4,5,6,7,8";
		if(orderId) entity.OrderDocID = orderId;
		if(orderDtId) entity.OrderDtlID = orderDtId;
		return entity;
	},
	/**@description 选择货品批号*/
