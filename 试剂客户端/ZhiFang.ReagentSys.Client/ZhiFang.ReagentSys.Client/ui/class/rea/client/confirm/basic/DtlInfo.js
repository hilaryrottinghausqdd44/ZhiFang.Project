 ...
 * @version 2017-12-01
 */
Ext.define('Shell.class.rea.client.confirm.basic.DtlInfo', {
	...
	initData:function(data){
		...
		html = html.replace(/{RefuseCount}/g,data.RefuseCount || "");
	...
	getHtmlTemplet:function(){
		...
		'<div style="padding:10px;font-weight:bold;font-size:20px;">' +
			...
			'<div style="padding:2px;">接收数:<span style="color:green;">{AcceptCount}</span> {Unit}</div>' +
			'<div style="padding:2px;">拒收数:<span style="color:green;">{RefuseCount}</span> {Unit}</div>' +
