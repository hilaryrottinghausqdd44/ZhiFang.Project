 ...
 * @version 2019-02-25
 */
Ext.define('Shell.class.rea.client.statistics.echart.out.equip.App', {
	...
	width: 1200,
	height: 800,
	/**统计类型:按库房:1;按供货商:2;按品牌:3;按货品一级分类:4;按货品二级分类:5;按仪器:6;按部门:7;*/
	statisticType: 6,
	...
	selectUrl: '/ReaStatisticalAnalysisService.svc/RS_UDTO_SearchOutEChartsVOListByHql?isPlanish=true',
	...
	afterRender: function() {
		...
		me.getComponent("buttonsToolbar1").getComponent("cboYAxis").setValue("sumtotal");
		//me.onLoadData();
	},
	...
	createItems: function() {
		...
		me.Grid = Ext.create('Shell.class.rea.client.statistics.echart.basic.equip.Grid', {
			region: 'west',
			header: false,
			width: 360,
			split: true,
		...
		me.BarChart = Ext.create('Shell.class.rea.client.statistics.echart.out.equip.BarChart', {
			region: 'center',
			header: false
		});
	...
	getParamsDocHql: function() {
		...
		if(dateArea && dateArea.getValue()) {
			...
			if(value.start) {
				...
				if(start) {
					docHql.push("reabmsoutdoc.DataAddTime>='" + start + " 00:00:00'");
				}
			}
			if(value.end) {
				var end = Ext.Date.format(value.end, "Y-m-d");
				if(end) {
					docHql.push("reabmsoutdoc.DataAddTime<='" + end + " 23:59:59'");
				}
			}
		}
	...
	onLoadData: function() {
		...
		var params = [];
		var cbsShowZero=me.getComponent('buttonsToolbar1').getComponent('cbsShowZero').getValue();
		params.push("statisticType=" + me.statisticType);
		params.push("showZero=" + cbsShowZero);
		params.push("fields=EChartsVO_TestEquipID,EChartsVO_TestEquipName,EChartsVO_AllSumTotal,EChartsVO_SumTotal,EChartsVO_SumTotalPercent");
	...
	afterDataLoad: function(list) {
		...
		if(list && list.length > 0) {
			for(var i = list.length - 1; i >= 0; i--) {
				...
				var record = me.Grid.store.findRecord("ReaTestEquipLab_Id", list[i]["EChartsVO_TestEquipID"]);
				if(record) {
