 ...
 * @version 2019-02-25
 */
Ext.define('Shell.class.rea.client.statistics.echart.out.storage.App', {
	...
	createItems: function() {
		...
		me.Grid = Ext.create('Shell.class.rea.client.statistics.echart.basic.storage.Grid', {
			region: 'west',
			header: false,
			width: 360,
			split: true,
		...
		me.BarChart = Ext.create('Shell.class.rea.client.statistics.echart.out.storage.BarChart', {
			region: 'center',
			header: false
		});
	...
	onLoadData: function() {
		...
		params.push("fields=EChartsVO_StorageID,EChartsVO_StorageName,EChartsVO_AllSumTotal,EChartsVO_SumTotal,EChartsVO_SumTotalPercent");
	...
	afterDataLoad: function(list) {
		...
		if(list && list.length > 0) {
			for(var i = list.length - 1; i >= 0; i--) {
				...
				var record = me.Grid.store.findRecord("ReaStorage_Id", list[i]["EChartsVO_StorageID"]);
				if(record) {
