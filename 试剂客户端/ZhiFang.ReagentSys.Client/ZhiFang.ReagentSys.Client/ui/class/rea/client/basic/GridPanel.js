 ...
 * @version 2018-12-12
 */
Ext.define('Shell.class.rea.client.basic.GridPanel', {
	...
	/**新增列表UI配置服务地址*/
	addUIUrl: '/ReaSysManageService.svc/ST_UDTO_AddBUserUIConfig',
	/**修改列表UI配置服务地址*/
	editUIUrl: '/ReaSysManageService.svc/ST_UDTO_UpdateBUserUIConfigByField',
	/**删除列表UI配置服务地址*/
	delUIUrl: '/ReaSysManageService.svc/ST_UDTO_DelBUserUIConfig',
	reaplay:[{
		"old":"ReaBmsReqDtl_GoodsSName",
		"new":"ReaGoods_SName"
	},{
		"old":"ReaBmsReqDtl_GoodsEName",
		"new":"ReaGoods_EName"
	},{
		"old":"ReaBmsCenSaleDtlConfirm_GoodsSName",
		"new":"ReaGoods_SName"
	},{
		"old":"ReaBmsCenOrderDtl_GoodSName",
		"new":"ReaGoods_SName"
	},{
		"old":"ReaBmsCenOrderDtl_GoodEName",
		"new":"ReaGoods_EName"
	},{
		"old":"ReaBmsOutDtl_SName",
		"new":"ReaGoods_SName"
	},{
		"old":"ReaBmsInDtl_GoodSName",
		"new":"ReaGoods_SName"
	},{
		"old":"ReaBmsQtyDtl_ReaGoods_DispOrder",
		"new":"ReaGoods_DispOrder"
	},{
		"old":"ReaBmsQtyDtl_ReaTestQty",
		"new":"ReaGoods_ReaTestQty"
	},{
		"old":"ReaBmsInDoc_RegisterNo",
		"new":"ReaGoods_RegistNo"
	},{
		"old":"ReaBmsInDoc_NetGoodsNo",
		"new":"ReaGoods_NetGoodsNo"
	},{
		"old":"ReaBmsCenOrderDtl_RegistNo",
		"new":"ReaGoods_RegistNo"
	},{
		"old":"ReaBmsCenOrderDtl_NetGoodsNo",
		"new":"ReaGoods_NetGoodsNo"
	},{
		"old":"ReaBmsInDtl_ProdOrgName",
		"new":"ReaGoods_ProdOrgName"
	}],
	...
	afterRender: function() {
		...
		me.on('sortchange', function(ct, column, direction, eOpts) {
			...
			for(var i=0;i<me.reaplay.length;i++){
				if(column.dataIndex==me.reaplay[i]["old"]){
					me.curOrderBy[0].property=me.reaplay[i]["new"];
				}
			}
		});
	},
	...
	initDefaultPageSize: function() {
		...
		var defaultPageSize = JcallShell.REA.RunParams.Lists.ReaUIDefaultPageSize.Value;
		//defaultPageSize=3000;
		if(!defaultPageSize) {
			//系统运行参数"列表默认分页记录数"
			JShell.REA.RunParams.getRunParamsValue("ReaUIDefaultPageSize", false, function(data) {
				var defaultPageSize = JcallShell.REA.RunParams.Lists.ReaUIDefaultPageSize.Value;
				if(!defaultPageSize) {
	...
	decreaseUserUI: function() {
		...
		//启用用户UI配置
		var enableUserUIConfig = ""+JcallShell.REA.RunParams.Lists.EnableUserUIConfig.Value;
		if(!enableUserUIConfig || enableUserUIConfig != "1") {
			me.initDefaultPageSize();
			return;
		}
		JcallShell.REA.BUserUIConfig.getUIConfigByKey(me.userUIKey, false, function(userUI) {
			if(!userUI) {
				me.initDefaultPageSize();
				return;
			}
	...
	getAddUIParams: function() {
		...
		if(defaultOrderBy.length==0 && me._defaultOrderBy && me._defaultOrderBy.length > 0){
			defaultOrderBy = me._defaultOrderBy;
		}
		var comment = {
			"columns": columns2,
	...
	onSaveUIClick: function() {
		...
		JShell.Server.post(url, params, function(data) {
			...
			if(data.success) {
				...
				var empID = JcallShell.System.Cookie.get(JcallShell.System.Cookie.map.USERID);
				var userUIKey1=empID+me.userUIKey;
				JcallShell.REA.BUserUIConfig.addListByKey(userUIKey1, params2);
		...
		setTimeout(function() {
			JShell.Server.get(url, function(data) {
				if(data.success) {
					...
					var empID = JcallShell.System.Cookie.get(JcallShell.System.Cookie.map.USERID);
					var userUIKey1=empID+me.userUIKey;
					JcallShell.REA.BUserUIConfig.removeByKey(userUIKey1);
