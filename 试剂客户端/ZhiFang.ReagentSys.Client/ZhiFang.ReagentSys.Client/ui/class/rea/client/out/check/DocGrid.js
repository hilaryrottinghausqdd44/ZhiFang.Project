 ...
 * @version 2019-03-18
 */
Ext.define('Shell.class.rea.client.out.check.DocGrid', {
	...
	onCheckClick: function() {
		...
		JShell.Msg.confirm({
			...
			me.onSaveOfCheck(records[0], "4", checkMemo);
		});
	},
	...
	onUnCheckClick: function() {
		...
		JShell.Msg.confirm({
			...
			me.onSaveOfCheck(records[0], "5", checkMemo);
		});
	},
	onSaveOfCheck: function(record, status, checkMemo) {
		...
		var outDoc = {
			...
			"CheckID": usernId,
			"CheckName": username,
			"CheckTime": JShell.Date.toServerDate(sysdate),
			"CheckMemo": checkMemo
		};
		var fields = ["Id", "Status"]; //, "IsHasCheck", "CheckID", "CheckName", "CheckTime", "CheckMemo"
		//当前审核类型为审核+,审核操作为"审核通过",审核通过时需要自动审批通过
		if(me.TYPE == "2" && status == "4") {
			outDoc.Status = "7";
			...
			outDoc.ApprovalMemo = checkMemo;
			//fields.push("IsHasApproval"); //IsHasApproval, "ApprovalId", "ApprovalTime", "ApprovalMemo"
		}
		var params = {
			"entity": outDoc,
			"fields": fields.join(",")
		...
		JShell.Server.post(url, Ext.JSON.encode(params), function(data) {
			if(data.success) {
				JShell.Msg.alert('出库申请审核操作保存成功', null, 2000);
				me.onSearch();
			} else {
				JShell.Msg.error('出库申请审核操作保存失败！' + data.msg);
			}
		});
	}
});