 ...
 * @version 2017-12-14
 */
Ext.define('Shell.class.rea.client.out.loss.DocGrid', {
	extend: 'Shell.class.rea.client.out.basic.DocGrid',
	/**报损出库审核是否需要确认(0:不启用;1:启用)*/
	ParaNo: 'C-RBCD-ISCH-0015',

	/**用户UI配置Key*/
	userUIKey: 'out.loss.DocGrid',
	/**用户UI配置Name*/
	userUIName: "出库总单列表",
	/**出库类型*/
	defaluteOutType: '3',
	...
	openAddPanel: function(TakerObj) {
		...
		//完成出库时是否需要出库确认
		JcallShell.REA.RunParams.getRunParamsValue("ReaBmsOutDocLossIsCheck", false, function(data) {
			var paraValue = "0";
			if(data.success) {
				var obj = data.value;
			...
			JShell.Win.open('Shell.class.rea.client.out.loss.AddPanel', config).show();
		});
	},
	//根据类型，赋值
	changeType: function() {
