 ...
 * @version 2018-03-23
 */
Ext.define('Shell.class.rea.client.qtywarning.storelower.QtyGrid', {
	...
	monthValue: 3,
	/**用户UI配置Key*/
	userUIKey: 'qtywarning.storelower.QtyGrid',
	/**用户UI配置Name*/
	userUIName: "低库存预警列表",
	AlertTypeList: [],
	...
	createDefaultButtonToolbarItems: function() {
		...
		var items = {
			...
			items: [{
				...
				itemId: "btnInfo",
				disabled: false,
				value: '说明:<b style="color:blue;">符合数据条件的计算公式:库存数量<库存下限值×查询输入项值/100;</b>'
			}]
		};
		return items;
	},
	...
	changeResult: function(data) {
		...
		if(data.list && data.list.length > 0) {
			...
			for(var i = 0; i < data.list.length; i++) {
				...
				var storeLower = data.list[i].ReaBmsQtyDtl_StoreLower;
				...
				/* if(comparisonValue!=0)calcComparisonValue=(comparisonValue-goodsQty)/comparisonValue*100;				
