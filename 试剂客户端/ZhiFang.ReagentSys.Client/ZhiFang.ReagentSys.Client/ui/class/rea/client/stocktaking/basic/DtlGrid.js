 ...
 * @version 2018-03-20
 */
Ext.define('Shell.class.rea.client.stocktaking.basic.DtlGrid', {
	...
	onCheckQtyChanged: function(record) {
		...
		var IsException = 0,
			IsHandleException = 1;
		if(CheckQty && CheckQty != GoodsQty) IsException = 1;
		if(IsException == 1) IsHandleException = 0;
		record.set('ReaBmsCheckDtl_IsException', IsException);
		//record.set('ReaBmsCheckDtl_IsHandleException', IsHandleException);
		record.commit();
	}
});