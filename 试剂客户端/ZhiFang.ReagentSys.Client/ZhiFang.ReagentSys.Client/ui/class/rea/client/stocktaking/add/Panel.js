 ...
 * @version 2018-03-20
 */
Ext.define('Shell.class.rea.client.stocktaking.add.Panel', {
	...
	createItems: function() {
		var me = this;
		me.DtlGrid = Ext.create('Shell.class.rea.client.stocktaking.add.DtlGrid', {
			header: false,
			itemId: 'DtlGrid',
			region: 'center',
			defaultLoad: false
		});
		me.DocForm = Ext.create('Shell.class.rea.client.stocktaking.add.DocForm', {
			header: false,
			itemId: 'DocForm',
			region: 'north',
			height: 225,
	...
	onConfirm: function(entity) {
		...
		me.DtlGrid.store.each(function(record) {
			...
			if(!checkQty && checkQty < 0) {
				isSave = false;
				info = "货品为:" + record.get("ReaBmsCheckDtl_GoodsName") + "的实盘数小于等于0,不能确认盘库!";
				return false;
			}
		});
		if(isSave == false) {
