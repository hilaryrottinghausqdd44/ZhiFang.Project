 ...
 * @version 2019-01-10
 */
Ext.define('Shell.class.rea.client.goodslot.Grid', {
	...
	createAddRec: function() {
		...
		if(!me.ReaGoodsNo) {
			isExect = false;
			JShell.Msg.error("货品编码不能为空!");
			return;
		}
		var obj = {
			ReaGoodsLot_ReaGoodsNo: me.ReaGoodsNo,
			ReaGoodsLot_LotNo: '',
		...
		if(me.GoodsID) {
			obj["ReaGoodsLot_GoodsID"] = me.GoodsID;
		}
		me.store.add(obj);
	},
	/**保存*/
	...
	updateOne: function(i, record) {
		...
		var params = {
			entity: {
				...
				ReaGoodsNo: record.get('ReaGoodsLot_ReaGoodsNo'),
				LotNo: record.get('ReaGoodsLot_LotNo'),
				Visible: record.get('ReaGoodsLot_Visible') ? 1 : 0,
				IsNeedPerformanceTest: me.IsNeedPerformanceTest ? 1 : 0
			}
		};
		...
		if(record.get('ReaGoodsLot_GoodsID')) {
			params.entity.GoodsID = record.get('ReaGoodsLot_GoodsID');
		}
		params.fields = 'Id,Visible,InvalidDate,ProdDate,ReaGoodsNo,IsNeedPerformanceTest';
	...
	addOne: function(record) {
		...
		if(me.GoodsID) {
			params.entity.GoodsID = me.GoodsID;
		}
		if(me.GoodsCName) {
	...
	isValid: function(records) {
		...
		for(var i = 0; i < records.length; i++) {
			//验证批号不能为空
			var ReaGoodsNo = records[i].get('ReaGoodsLot_ReaGoodsNo');
			...
			if(!LotNo || !ProdDate || !InvalidDate) {
				isExect = false;
				JShell.Msg.error("批号,生成日期,有效期都不能为空");
				break;
			}
	...
	loadDataById: function() {
		var me = this;
		if(me.ReaGoodsNo) {
			me.defaultWhere = "reagoodslot.ReaGoodsNo='" + me.ReaGoodsNo + "'";
		} else if(me.GoodsID) {
			me.defaultWhere = 'reagoodslot.GoodsID=' + me.GoodsID;
		}
		me.onSearch();
	},
	/**查询数据*/
