...
Ext.define('Shell.class.rea.client.apply.basic.ApplyDtGrid', {
	...
	/**获取数据服务路径*/
	selectUrl: '/ReaManageService.svc/ST_UDTO_SearchReaBmsReqDtlByHQLCommon?isPlanish=true',
	/**删除数据服务路径*/
	delUrl: '/ReaSysManageService.svc/ST_UDTO_DelReaBmsReqDtl',
	/**新增服务地址*/
	addUrl: '/ReaManageService.svc/ST_UDTO_AddReaBmsReqDocAndDt',
	/**修改服务(只更新明细,更新主单)地址*/
	editUrl: '/ReaManageService.svc/ST_UDTO_UpdateReaBmsReqDtlOfCheck',
	/**排序字段*/
	defaultOrderBy: [{
			...
			update: function(store, record) {
				if(record.dirty) {
					...
					for(var modified in changedObj) {
						if(modified == "ReaBmsReqDtl_ReqGoodsQty") {
							me.onReqGoodsQtyChanged(record);
						} else if(modified == "ReaBmsReqDtl_GoodsQty") {
	...
	initComponent: function() {
		...
		if(me.defaultOrderBy.length==0){
			me.defaultOrderBy=[{property: 'ReaBmsReqDtl_DispOrder',direction: 'ASC'}];
		}
		//自定义按钮功能栏
	...
	onReqGoodsQtyChanged: function(record) {
		var me = this;
		var ReqGoodsQty = record.get('ReaBmsReqDtl_ReqGoodsQty');
		if(!ReqGoodsQty)ReqGoodsQty=0;
		ReqGoodsQty=parseFloat(ReqGoodsQty);
		record.set('ReaBmsReqDtl_GoodsQty', ReqGoodsQty);
		var CurrentQty = record.get('ReaBmsReqDtl_CurrentQty');
		if(!CurrentQty)CurrentQty=0;
		CurrentQty=parseFloat(CurrentQty);
		var ExpectedStock=parseFloat(ReqGoodsQty)+parseFloat(CurrentQty);
		//预期库存量=当次采购数+现有库存量
		record.set('ReaBmsReqDtl_ExpectedStock', ExpectedStock);
		record.commit();
		//me.getView().refresh();
	},
	onGoodsQtyChanged: function(record) {
		...
		var Price = record.get('ReaBmsReqDtl_Price');
		if(!GoodsQty)GoodsQty=0;
		if(!Price)Price=0;
		Price=parseFloat(Price);
		GoodsQty=parseFloat(GoodsQty);
		var SumTotal=GoodsQty*Price;
		record.set('ReaBmsReqDtl_Price', Price);
		record.set('ReaBmsReqDtl_SumTotal', SumTotal);
		record.commit();
		//me.getView().refresh();
	},
	/**创建供应商数据列*/
		...
		var column = {
			...
			editor: new Ext.form.field.ComboBox({
				...
				store: new Ext.data.Store({ //Simple
					autoLoad: true,
					fields: ['CenOrgId', 'CenOrgCName', 'Id', 'CenOrgGoodsNo','Price'], //
					data: []
				}),
				listeners: {
	...
	/**创建申请数量数据列*/
	createReqGoodsQtyColumn: function() {
		var me = this;
		var column = {
			dataIndex: 'ReaBmsReqDtl_ReqGoodsQty',
	...
	createGoodsQtyColumn: function() {
		...
		var column = {
			...
			text: '<b style="color:blue;">实际审批数</b>',
			width: 70,
			editor: {
				xtype: 'numberfield',
				allowBlank: false,
	...
	createCurrentQtyColumn: function() {
		var me = this;
		var column = {
			dataIndex: 'ReaBmsReqDtl_CurrentQty',
			text: '库存数',
			width: 75
		};
		return column;
	...
	getReaGoodsCenOrgList: function(isRefresh) {
		...
		var url = JShell.System.Path.ROOT + '/ReaManageService.svc/ST_UDTO_SearchReaGoodsOrgLinkByHRDeptId?deptId=' + me.CurDeptId;
	...
	getDtSaveParams: function() {
		...
		me.store.each(function(record) {
			...
			var entity = {
				...
				"ReqGoodsQty": reqGoodsQty,
				"GoodsQty": goodsQty,
				"GoodsSName": record.get("ReaBmsReqDtl_GoodsSName"),
				"GoodsEName": record.get("ReaBmsReqDtl_GoodsEName"),
				"GoodsCName": record.get("ReaBmsReqDtl_GoodsCName"),
				"GoodsUnit": record.get("ReaBmsReqDtl_GoodsUnit"),
				"UnitMemo": record.get("ReaBmsReqDtl_UnitMemo"),
				"ProdOrgName": record.get("ReaBmsReqDtl_ProdOrgName"),
				"GoodsID": record.get("ReaBmsReqDtl_GoodsID"),
				"ReaGoodsNo": record.get("ReaBmsReqDtl_ReaGoodsNo"),
				"CenOrgGoodsNo": record.get("ReaBmsReqDtl_CenOrgGoodsNo"),
				"DispOrder":record.get("ReaBmsReqDtl_DispOrder"),
			...
			var sumTotal=record.get("ReaBmsReqDtl_SumTotal");
			if(!sumTotal) sumTotal = 0;
			sumTotal=price*goodsQty;
			sumTotal=parseFloat(sumTotal);
			
			entity.Price=price;
			entity.SumTotal=sumTotal;
			
			var monthlyUsage=record.get("ReaBmsReqDtl_MonthlyUsage");
			if(!monthlyUsage) monthlyUsage = 0;
			entity.MonthlyUsage = parseFloat(monthlyUsage);;
			
			var currentQty=record.get("ReaBmsReqDtl_CurrentQty");
			if(!currentQty) currentQty = 0;
			entity.CurrentQty = parseFloat(currentQty);
			
			var expectedStock=record.get("ReaBmsReqDtl_ExpectedStock");
			if(!expectedStock) expectedStock = 0;
			entity.ExpectedStock = parseFloat(expectedStock);
			
			if(record.get("ReaBmsReqDtl_ArrivalTime")) {
				entity.ArrivalTime = JShell.Date.toServerDate(record.get("ReaBmsReqDtl_ArrivalTime"));
			}
			if(record.get("ReaBmsReqDtl_GoodsUnitID")) {
				entity.GoodsUnitID = record.get("ReaBmsReqDtl_GoodsUnitID");
			}
			if(record.get("ReaBmsReqDtl_CompGoodsLinkID")) {
				entity.CompGoodsLinkID = record.get("ReaBmsReqDtl_CompGoodsLinkID");
			}
			var prodID = record.get("ReaBmsReqDtl_ProdID");
			if(prodID) {
				entity.ProdID = prodID;
			}
			var reaCenOrg = null;
	...
	onDeleteDtClick: function() {
		...
		me.delRecords(records);
	},
	delRecords: function(records) {
		var me = this;
		//区分前台删除及调用后台服务删除		
						...
						for(var i = 0; i < list.length; i++) {
							if(record.get("ReaBmsReqDtl_GoodsID") == list[i]["CurGoodsId"]) {
								...
								if(!parseFloat(currentQty))
									currentQty = 0;
								record.set("ReaBmsReqDtl_CurrentQty", currentQty);
								record.commit();
								break;
							}
						}
