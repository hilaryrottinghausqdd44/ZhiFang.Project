...
Ext.define('Shell.class.rea.client.deptgoods.Form', {
	...
	createItems:function(){
		...
		items.push({
			...
			name:'ReaDeptGoods_ReaGoods_CName',itemId:'ReaDeptGoods_ReaGoods_CName',
		...
		items.push({
				...
				JShell.Win.open('Shell.class.sysbase.org.CheckTree',{
					...
					listeners:{
						accept:function(p,record){
							var	ParentID = me.getComponent('ReaDeptGoods_DeptID'),
			...
			fieldLabel:'部门主键ID',hidden:true,name:'ReaDeptGoods_DeptID',itemId:'ReaDeptGoods_DeptID'
		});
	...
	getAddParams:function(){
		...
		if(values.ReaDeptGoods_DeptID){
			entity.DeptID=values.ReaDeptGoods_DeptID;
	...
	getEditParams:function(){
		...
		var fields = [
			'Id','DispOrder','Visible','ReaGoods_Id','DeptID','DeptName'
		];
	...
	initFilterListeners: function() {
		...
		var GoodsCName=me.getComponent('ReaDeptGoods_ReaGoods_CName');
        GoodsCName.on({
	...
	getLinkByIds:function(callback){
		var me = this,values = me.getForm().getValues(),
			url = JShell.System.Path.ROOT + me.selectUrl2.split('?')[0] + 
				...
				'&where=readeptgoods.ReaGoods.Id =' + values.ReaDeptGoods_ReaGoods_Id + '  and readeptgoods.DeptID='+ values.ReaDeptGoods_DeptID +' and readeptgoods.Id!='+me.PK ;
