 ...
 * @version 2018-04-26
 */
Ext.define('Shell.class.rea.client.reasale.comp.EditPanel', {
	...
	afterRender: function() {
		...
		me.DocForm.on({
			...
			//表单的订货方选择后联动供货明细列表
			onReaLabCheck: function(v, reaLab) {
				me.DtlPanel.setReaLabInfo(reaLab);
			},
			isEditAfter: function(form) {
				var bo = true;
				var bo = true;
				if(me.DtlPanel.DtlGrid.store.getCount() <= 0) bo = false;
				me.setLabcNameReadOnly(bo);
			}
		});
	...
	createItems: function() {
		...
		me.DocForm = Ext.create('Shell.class.rea.client.reasale.comp.add.DocForm', {
			header: false,
			itemId: 'DocForm',
			region: 'north',
			height: 195,
		...
		me.DtlPanel = Ext.create('Shell.class.rea.client.reasale.comp.DtlPanel', {
			header: false,
			itemId: 'DtlPanel',
			region: 'center',
			split: true,
	...
	/**@description 订货方是可编辑还是只读处理*/
	setLabcNameReadOnly: function(bo) {
		var me = this;
		var com = me.DocForm.getComponent('ReaBmsCenSaleDoc_LabcName');
		com.setReadOnly(bo);
	},
