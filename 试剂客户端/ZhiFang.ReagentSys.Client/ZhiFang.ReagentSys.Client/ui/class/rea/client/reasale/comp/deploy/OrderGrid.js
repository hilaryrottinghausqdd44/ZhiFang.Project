/**
 * @description 提取订单转供货单(实验室与平台不在同一数据库)
 * @author longfc
 * @version 2018-05-17
 */
Ext.define('Shell.class.rea.client.reasale.comp.deploy.OrderGrid', {
	...
	createGridColumns: function() {
		...
		var columns = [{
			...
			items: [{
				...
				handler: function(grid, rowIndex, colIndex) {
					var rec = grid.getStore().getAt(rowIndex);
					me.onOrderToSupplyOfDeploy(rec);
				}
			}]
		}, {
	...
	onBeforeLoad: function() {
		...
		if(!JcallShell.REA.RunParams.Lists["BSPlatformURL"].Value) {
			var error = me.errorFormat.replace(/{msg}/, "获取系统运行参数'访问BS平台的URL'配置项信息为空!");
			me.getView().update(error);
			return false;
		};
		//机构所属平台编码
	...
	getLoadUrl: function() {
		...
		var url =JcallShell.REA.RunParams.Lists["BSPlatformURL"].Value+ me.selectUrl;
	...
	onOrderToSupplyOfDeploy: function(rec) {
		...
		var url = JShell.System.Path.ROOT + "/ZFReaRestfulService.svc/";
		//封装上传的订单信息及订单明细
		var orderDoc = me.getOrderDocOfDeploy();
		var orderDtl = me.getOrderDtlOfDeploy();
		var params = {
			"orderDoc": orderDoc,
