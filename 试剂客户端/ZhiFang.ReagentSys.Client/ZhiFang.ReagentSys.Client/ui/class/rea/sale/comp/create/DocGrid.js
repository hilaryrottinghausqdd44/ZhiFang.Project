 ...
 * @version 2017-05-8
 */
Ext.define('Shell.class.rea.sale.comp.create.DocGrid', {
	extend: 'Shell.class.rea.sale.basic.DocGrid',
	title: '供应商供货审核-供货总单列表',
	...
	hasDel: true,
	 /**供货单审核服务（只审核不拆分)*/
	CheckSaleDocUrl:'/ReagentService.svc/RS_UDTO_CheckSaleDocByID',
	 /**供货单拆分服务*/
	...
	initComponent: function() {
		...
		me.buttonToolbarItems = ['refresh','-',{
			...
			itemId:'status',allowBlank:false,value:0,hidden:true,
	...
	showCheckButton:function(status){
		...
		for(var i=0;i<len;i++){
			...
			if(IsSplit=='1' || IsSplit=='true' || IsSplit==true){
				showSplit=false;
			}
			if(!records[i].get('BmsCenSaleDoc_SaleDocNo')){
				showSplit = false;
	...
	onSplitSaleClick:function(){
		...
		me.onSplitSaleUpdate(records);
	},
	
    /**条码拆分数据保存*/
	onSplitSaleUpdate:function(records){
		var me = this,
			len = records.length;
		...
		for(var i=0;i<len;i++){
			var rec = records[i];
			var id = rec.get(me.PKField);
			me.updateSplitSaleDoc(i,id);
		}
	},
	/**条码拆分*/
	updateSplitSaleDoc:function(index,id){
		var me = this,
			url = JShell.System.Path.ROOT + me.SplitSaleDocUrl;
		var params ={docID:id};
		setTimeout(function(){
			JShell.Server.post(url,Ext.JSON.encode(params),function(data){
				var record = me.store.findRecord(me.PKField,id);
