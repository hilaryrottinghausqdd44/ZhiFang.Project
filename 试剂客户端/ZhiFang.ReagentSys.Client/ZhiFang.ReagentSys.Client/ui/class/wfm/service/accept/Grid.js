...
Ext.define('Shell.class.wfm.service.accept.Grid', {
	...
	/**获取数据服务路径*/
	selectUrl: '/SingleTableService.svc/ST_UDTO_SearchPCustomerServiceByHQL?isPlanish=true',
	/**获取获取类字典列表服务路径*/
...
//			}
	...
	initFilterListeners: function() {
		...
		//受理人
		var ServiceAcceptanceMan = buttonsToolbar.getComponent('PCustomerService_ServiceAcceptanceMan'),
			ServiceAcceptanceManID = buttonsToolbar.getComponent('PCustomerService_ServiceAcceptanceManID');
	...
	getLoadUrl: function() {
		...
		if(buttonsToolbar) {
			...
			ServiceAcceptanceManID = buttonsToolbar.getComponent('PCustomerService_ServiceAcceptanceManID').getValue();
		}
		if(buttonsToolbar1) {
		...
		if(ServiceAcceptanceManID) {
			params.push("pcustomerservice.ServiceAcceptanceManID='" + ServiceAcceptanceManID + "'");
		}
		//状态
		if(Status) {
	...
	updateOneByIsUse: function(index, id, IsUse, Status) {
		var me = this;
		var url = "/SingleTableService.svc/ST_UDTO_UpdatePCustomerServiceByField";
	...
	getStatusData:function(Status){
		...
		for(var i in Status){
			...
			if(FontColor){
				me.StatusFColorEnum[obj.BDict_Id] = FontColor;
			}
			var style = ['font-weight:bold;text-align:center'];
			if(BGColor){style.push('color:' + BGColor);
			me.StatusBGColorEnum[obj.BDict_Id] = BGColor;
			}
			me.StatusEnum[obj.BDict_Id] = obj.BDict_CName;
			data.push([obj.BDict_Id,obj.BDict_CName,style.join(';')]);
		}
	...
	getStatusList:function(callback){
		...
		url += "&fields=BDict_CName,BDict_Id&where=	((pdict.BDictType.DictTypeCode='ServiceStatus') and pdict.IsUse=1)";
