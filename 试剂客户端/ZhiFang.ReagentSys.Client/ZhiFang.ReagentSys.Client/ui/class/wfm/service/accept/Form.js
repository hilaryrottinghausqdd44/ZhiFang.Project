...
Ext.define('Shell.class.wfm.service.accept.Form', {
	...
	/**获取数据服务路径*/
	selectUrl: '/SingleTableService.svc/ST_UDTO_SearchPCustomerServiceById?isPlanish=true',
	/**新增服务地址*/
	addUrl: '/SingleTableService.svc/ST_UDTO_AddPCustomerService',
	/**修改服务地址*/
	editUrl: '/SingleTableService.svc/ST_UDTO_UpdatePCustomerServiceByField',
	/**获取获取类字典列表服务路径*/
	...
	createExecuteFieldSet: function() {
		...
		var fieldset = {
			...
			items: [{
				...
				classConfig: {
					title: '状态选择',
					defaultWhere: "pdict.BDictType.DictTypeCode='" + this.ServiceStatus + "'"
				}
			}, {
				fieldLabel: '服务方式',
				xtype: 'uxCheckTrigger',
				...
				classConfig: {
					title: '服务方式选择',
					defaultWhere: "pdict.BDictType.DictTypeCode='" + this.ServiceMode + "'"
				},
	...
	createVisitFieldSet: function() {
		...
		var fieldset = {
			...
			items: [{
				...
				classConfig: {
					title: '用户满意度选择',
					defaultWhere: "pdict.BDictType.DictTypeCode='" + this.Satisfaction + "'"
				},
				fieldLabel: '用户满意度',
				...
				classConfig: {
					title: '完成情况选择',
					defaultWhere: "pdict.BDictType.DictTypeCode='" + this.CompletionStatus + "'"
				},
				fieldLabel: '完成情况',
	...
	changeResult: function(data) {
		...
		if(data.PCustomerService_Status){
		        me.getStatusName(data.PCustomerService_Status,function(data){
		        	if(data.value.list){
		        		StatusName.setValue(data.value.list[0].BDict_CName);
		        	}
		        });
			}else{
				StatusName.setValue('');
	...
	initFilterListeners: function() {
       ...
       if(ProvinceName) {
		...
		if(ServiceModeName) {
			ServiceModeName.on({
				check: function(p, record) {
					ServiceModeName.setValue(record ? record.get('BDict_CName') : '');
					ServiceModeID.setValue(record ? record.get('BDict_Id') : '');
					p.close();
				}
			});
		}
		...
		if(StatusName) {
			StatusName.on({
				check: function(p, record) {
					StatusName.setValue(record ? record.get('BDict_CName') : '');
					StatusID.setValue(record ? record.get('BDict_Id') : '');
					p.close();
				}
			});
		}
