...
Ext.define('Shell.class.wfm.authorization.ahsingle.basic.Form', {
	...
	PK: null,
	/**获取数据服务路径*/
	selectUrl: '/SingleTableService.svc/ST_UDTO_SearchAHSingleLicenceById?isPlanish=false',
	/**新增服务地址*/
	addUrl: '/SingleTableService.svc/ST_UDTO_AddAHSingleLicence',
	/**修改服务地址*/
		...
		EndDate.on({
			...
			change: function(com, newValue, oldValue, eOpts) {
				if(newValue && newValue != null && newValue != "") {
					if(checkHoliday.getValue() == true) {
						var EndDateValue = Ext.util.Format.date(newValue, 'Y-m-d');
	...
	getLicenceEndDate: function(endDate) {
		var me = this,
			Date = endDate;
		if(me.IsGetLicenceEndDate == true) {
			var url = '/SingleTableService.svc/ST_UDTO_GetLicenceEndDate';
	...
	getStartDateValueOfApply: function() {
		...
		if(me.formtype == "add" && LicenceTypeId.getValue() == info.Id) {
			...
			if(macValue != "" && sqhValue != "") {
				var url = '/SingleTableService.svc/ST_UDTO_GetStartDateValueOfApply';
				url = JShell.System.Path.getRootUrl(url);
