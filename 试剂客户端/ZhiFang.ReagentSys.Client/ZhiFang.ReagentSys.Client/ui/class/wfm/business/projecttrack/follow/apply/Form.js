...
Ext.define('Shell.class.wfm.business.projecttrack.follow.apply.Form', {
    ...
    /**修改服务地址*/
    editUrl:'/SingleTableService.svc/ST_UDTO_UpdatePContractFollowByField',
...
//  ProjectAttr: '5147931795319426326',
    ...
    createItems: function () {
        ...
        , {
			...
			xtype: 'uxCheckTrigger',
			itemId: 'PContractFollow_Content',
			className: 'Shell.class.wfm.dict.CheckGrid',
			classConfig: {
				title: '项目类型选择',
	...
	getAddParams:function(){
		...
		var entity = {
			...
			PaidServiceStartTime:JShell.Date.toServerDate(values.PContractFollow_PaidServiceStartTime),//有偿服务时间
	...
	isValidContractNumber:function(callback,ContractNumber){
		var me = this,
			values = me.getForm().getValues(),
			url = '/SingleTableService.svc/ST_UDTO_SearchPContractFollowByHQL';
	...
	onResetClick: function() {
		...
		if(!me.PK) {
			...
			me.getContractAttr(function(data){
				if(data && data.value){
					var idval=data.value.list[0].BDict_Id;
					var nameval=data.value.list[0].BDict_CName;
	...
	getContractAttr:function(callback){
		...
		url += '&fields=BDict_CName,BDict_Id&where=pdict.Id='+me.ProjectAttr;
		JShell.Server.get(url,function(data){
			if(data.success){
				callback(data);
			}else{
