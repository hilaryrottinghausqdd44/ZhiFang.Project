...
Ext.define('Shell.class.wfm.business.expenseaccount.basic.Form', {
	...
	height: 350,
	/**获取数据服务路径*/
	selectUrl: '/SingleTableService.svc/ST_UDTO_SearchPExpenseAccountById?isPlanish=true',
	/**新增服务地址*/
	addUrl: '/SingleTableService.svc/ST_UDTO_AddPExpenseAccount',
	/**修改服务地址*/
	...
	createDictItems: function() {
		...
		me.PExpenseAccount_ComponeName = {
			...
			classConfig: {
				title: '本公司名称选择',
				defaultWhere: "pdict.BDictType.DictTypeCode='" + this.ComponeName + "'"
			}
		};
		...
		me.PExpenseAccount_PExpenseAccounTypeName = {
			...
			classConfig: {
				title: '报销单类型选择',
				defaultWhere: "pdict.BDictType.DictTypeCode='" + this.PExpenseAccounTypeName + "'"
			}
		};
		...
		me.PExpenseAccount_PExpenseAccounContentTypeName = {
			...
			classConfig: {
				title: '内容类型选择',
				defaultWhere: "pdict.BDictType.DictTypeCode='" + this.PExpenseAccounContentTypeName + "'"
			}
		};
		...
		me.PExpenseAccount_OneLevelItemName = {
			...
			classConfig: {
				title: '一级科目选择',
				defaultWhere: "pdict.BDictType.DictTypeCode='" + this.OneLevelItemName + "'"
			}
		};
		//二级科目
		me.PExpenseAccount_TwoLevelItemName = {
			...
			classConfig: {
				title: '二级科目选择',
				defaultWhere: "pdict.BDictType.DictTypeCode='" + this.TwoLevelItemName + "'"
			}
		};
		//核算单位
	...
	doDictListeners: function(name) {
		...
		CName.on({
			check: function(p, record) {
				...
				//				DataTimeStamp.setValue(record ? record.get('BDict_DataTimeStamp') : '');
				p.close();
			}
		});
	},
