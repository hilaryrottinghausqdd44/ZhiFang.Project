 ...
 * @version 2015-07-02
 */
Ext.define('Shell.class.wfm.business.associate.invoice.contrast.App', {
	...
	operationType: "",
	GridCalss: 'Shell.class.wfm.business.associate.invoice.basic.Grid',
	
	editUrl: '/SingleTableService.svc/ST_UDTO_UpdatePInvoiceByFieldManager',
	...
	onCheckVerification: function() {
		...
		var oldPClientID = "",
			oldPayOrgID = "";
		if(isExec) {
			var CheckId = "" + recordPContract.get("PInvoice_CheckId");
			oldPClientID = "" + recordPContract.get('PInvoice_'+me.PClientID);
			oldPayOrgID = "" + recordPContract.get("PInvoice_PayOrgID");
			if(CheckId.length > 0) {
				isExec = false;
				...
				if(data.success) {
					...
					if(me.updateEntity != null) {
						...
						if(me.updateEntity.PayOrgID) {
							recordPContract.set("PInvoice_PayOrgID", me.updateEntity.PayOrgID);
						}
...
//						}
						if(me.updateEntity[me.PClientID]) {
							recordPContract.set('PInvoice_'+me.PClientID, me.updateEntity[me.PClientID]);
						}
...
//						}
						...
						if(me.updateEntity.ContrastId) {
							recordPContract.set("PInvoice_ContrastId", me.updateEntity.ContrastId);
						}
						if(me.updateEntity.ContrastCName) {
							recordPContract.set("PInvoice_ContrastCName", me.updateEntity.ContrastCName);
						}
						if(me.updateEntity.CheckId) {
							recordPContract.set("PInvoice_CheckId", me.updateEntity.CheckId);
						}
						if(me.updateEntity.CheckCName) {
							recordPContract.set("PInvoice_CheckCName", me.updateEntity.CheckCName);
						}
