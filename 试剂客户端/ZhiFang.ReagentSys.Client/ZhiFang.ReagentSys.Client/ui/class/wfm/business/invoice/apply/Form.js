...
//					width: 530,
					title: '开票内容类型选择',
					defaultWhere: "pdict.BDictType.Id=5106560944309337578"
				},
				width: me.defaults.width * 2
			}, {
				fieldLabel: '收票人姓名',
			...
			},
			  {
				...
				classConfig: {
					width: 530,
					title: '开票内容选择',
					defaultWhere: "pdict.BDictType.DictTypeCode='" + this.InvoiceContent + "'"
				},
				width: me.defaults.width * 2
			}, {
				fieldLabel: '开票金额',
		...
		if(ComponeName) {
			ComponeName.on({
				check: function(p, record) {
					...
					ComponeName.setValue(record ? record.get('BDict_CName') : '');
					...
					InvoiceContentName.classConfig={
						defaultWhere: "pdict.BDictType.DictTypeCode='" + ContentType + "'"
					};
					p.close();
				}
			});
...
//					PInvoice_PayOrgName.setValue(PayOrgName);
     ...
     * @author liangyl @version 2017-07-27*/
	 getInfo:function(Id,callback){
		var me = this,
			url = '/SingleTableService.svc/ST_UDTO_SearchPInvoiceByHQL?isPlanish=true';
