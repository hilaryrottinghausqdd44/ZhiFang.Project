...
function getAppObjectById(id,callback){
	var url = getRootPath()+'/ConstructionService.svc/CS_UDTO_SearchBTDAppComponentsById?id='+id;
...
function competence(id,callback){
 	...
 	var url = getRootPath() + '/RBACService.svc/RBAC_UDTO_SearchModuleBySessionHREmpIDAndCookieModuleID?id=' + id;
 	var c = function(text){
...
function getPinYinZiTouFromServer(CName,callback){
	var me = this;
	var url = encodeURI(getRootPath()+'/ConstructionService.svc/GetPinYin' + "?chinese=" + CName);
...
function getServerInformation(callback){
    var me = this;
    var url = getRootPath() + '/ConstructionService.svc/CS_UDTO_GetServerInformation';
    var c = function(text){
    	var result = Ext.JSON.decode(text);
        var value = null;
...
function getAppInfo(id,callback,fields,isPlanish){
	...
	if(id && id != -1){
		var url = getRootPath()+"/ConstructionService.svc/CS_UDTO_SearchBTDAppComponentsById?isPlanish=" + bo + "&id=" + id;
		url += "&fields=" + (fields || "");
		//回调函数
        var c = function(text){
        	var result = Ext.JSON.decode(text);
...
    Ext.each(cols,function(colIndex){ //逐列去操作tr
	var xtype="";
	if(grid.columns[colIndex-1]||grid.columns[colIndex-1].xtype)xtype=grid.columns[colIndex-1].xtype;
		var isCheckColumn = xtype== 'checkcolumn';//是否是勾选列
        var rowspan = 1;
