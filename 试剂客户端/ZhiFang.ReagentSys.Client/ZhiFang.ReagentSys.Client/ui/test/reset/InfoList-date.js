...
Ext.define('Ext.iqc.date.InfoList',{
	...
	serverUrl:{
		...
		add:getRootPath()+'/QCService.svc/QC_BA_ADDQCDataCustom',//新增质控数据
		update:getRootPath()+'/QCService.svc/QC_BA_UpdateQCDataCustom',//修改质控数据
		del:getRootPath()+'/QCService.svc/QC_UDTO_DelQCDValue',//根据ID删除质控数据
		updateIsUse:getRootPath()+'/QCService.svc/QC_UDTO_UpdateQCDValueByField'//逻辑删除
	},
	/**条件的ID属性*/
	tid:'',
	/**仪器ID*/
		...
		me.on({
			cellAvailable:function(editor,e){
				...
				if(e.value != value && e.field == 'ReportValue' && 
					(QCItemValueType == me.Enum.QCValueType[0] || 
						QCItemValueType == me.Enum.QCValueType[1])
				){
					//定量靶值标准差||定量靶值标准差
					//数值转换
