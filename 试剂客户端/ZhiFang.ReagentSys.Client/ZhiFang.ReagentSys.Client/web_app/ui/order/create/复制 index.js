$(function () {
	...
	function getCompListData(callback){
		var fields = [
			'ReaCenOrg_OrgNo','ReaCenOrg_CName','ReaCenOrg_Id'
		];
	...
	function initPageContent(){
		$("#UrgentFlag_Div").show();
		$('#UrgentFlag_Div').children("input").bootstrapSwitch('state',LOCAL_DATA.DocInfo.UrgentFlag == "1" ? false : true);
		...
		if(LOCAL_DATA.DocInfo.ReqDeliveryTime){
			var date = new Date(LOCAL_DATA.DocInfo.ReqDeliveryTime);;
		...
		$("#ReqDeliveryTime").on("change",function(data){
		...
		for(var i=0;i<len;i++){
			CompItems.push({
				title:(i+1) + '：' + list[i].ReaCenOrg_CName,
		...
		$("#Comp").select({
			...
			onChange: function(data) {
				var Id = data.values;
				var Name = data.titles;
				onChangeComp(Id,Name);
			}
		});
	...
	function onDocInfoChange(){
		var DocInfo = {
			...
			ReqDeliveryTime:$("#ReqDeliveryTime").val(),
			LabMemo:$("#LabMemo").val()
		};
...
//				Memo:values.ReaBmsCenOrderDoc_Memo.replace(/\\/g, '&#92').replace(/[\r\n]/g, '<br />');
