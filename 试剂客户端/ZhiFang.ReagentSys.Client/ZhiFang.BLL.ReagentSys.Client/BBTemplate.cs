...
namespace ZhiFang.BLL.ReagentSys.Client
{
    ...
    ///
    /// </summary>
    public class BBTemplate : BaseBLL<BTemplate>, ZhiFang.IBLL.ReagentSys.Client.IBBTemplate
    {
        /// <summary>
        ...
        public BaseResultDataValue AddTemplateUploadFile(HttpPostedFile file)
        {
            BaseResultDataValue brdv = new BaseResultDataValue();
            if (file != null && file.ContentLength > 0)
            {
                BaseResultBool brdv2 = UploadTemplateFile(file);
            }
        ...
        /// <returns></returns>
        private BaseResultBool UploadTemplateFile(HttpPostedFile file)
        {
            BaseResultBool brdv = new BaseResultBool();
            ...
            if (file != null && file.ContentLength > 0)
            {
                ...
                string reportSubDir = BTemplateType.GetStatusDic()[this.Entity.TypeID.ToString()].Name;
                string templateType = FrxToPdfReportHelp.PublicTemplateDir;
                if (fileExt.ToLower().Contains(".xls"))
                {
                    templateType = ExcelReportHelp.PublicTemplateDir;
                }
                string parentPath = ReportBTemplateHelp.GetBTemplateFullDir(this.Entity.LabID, templateType, reportSubDir);
                
                string fileName = filterFileName(file.FileName);
                try
                {
                    this.Entity.TemplateType = templateType;
                    this.Entity.FileSize = file.ContentLength;
                    this.Entity.ContentType = file.ContentType;
                    if (fileExt == ".frx")
                    {
                        this.Entity.ContentType = "application/octet-stream";
                    }
                    else if (fileExt == ".xlsx")
                    {
                        this.Entity.ContentType = "application/octet-stream";//application/ms-excel
                    }
                    else if (fileExt == ".xls")
                    {
                        this.Entity.ContentType = "application/vnd.ms-excel";
                    }
                    else
                    {
                        this.Entity.ContentType = "application/octet-stream";
                    }
                    this.Entity.FileName = file.FileName;
                    this.Entity.FileExt = fileExt;// ".frx";
                    this.Entity.FilePath = ReportBTemplateHelp.GetBTemplateSubDir(this.Entity.LabID, templateType, reportSubDir) + "\\" + file.FileName;

                    string filepath = Path.Combine(parentPath, file.FileName);
                    file.SaveAs(filepath);
                    //获取excel模板的规则内容信息
                    if (fileExt.ToLower().Contains(".xls"))
                    {
                        IList<string> listCell = ExcelRuleInfoHelp.ReadExcelMoudleFile(file.InputStream);
                        JObject jresult = ExcelRuleInfoHelp.GetExcelRuleInfo(listCell);
                        //ZhiFang.Common.Log.Log.Debug("jresult:" + jresult.ToString());
                        this.Entity.ExcelRuleInfo = jresult.ToString();
                    }
                }
                catch (Exception ex)
                {
                    ZhiFang.Common.Log.Log.Error("模板上传错误信息:" + ex.Message);
                }
            }
            else if (file.ContentLength <= 0)
            {
                ZhiFang.Common.Log.Log.Error("上传模板文件大小为空!");
            }
            return brdv;
        }
        public BaseResultBool UpdateTemplateUploadFileByField(string[] tempArray, HttpPostedFile file)
        {
            BaseResultBool brdv = new BaseResultBool();
            ...
            {
                BaseResultBool brdv2 = UploadTemplateFile(file);
                if (brdv2.success && !string.IsNullOrEmpty(this.Entity.FilePath))
                {
                    this.Entity.FileName = file.FileName;
                    var tmpa = tempArray.ToList();
                    tmpa.Add("FilePath='" + this.Entity.FilePath + "'");
                    if (file != null)
                    {
                        tmpa.Add("FileName='" + file.FileName + "'");
                        tmpa.Add("FileSize=" + file.ContentLength + "");
                        tmpa.Add("FileExt='" + this.Entity.FileExt + "'");
                        tmpa.Add("TemplateType='" + this.Entity.TemplateType + "'");
                        tmpa.Add("ContentType='" + this.Entity.ContentType + "'");
                    }
