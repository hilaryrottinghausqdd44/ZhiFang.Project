...
namespace ZhiFang.BLL.RBAC
{
    ...
    public class BPUser : BaseBLL<PUser, int>, IBPUser
    {
        ...
        public int GetMaxId()
        {
            return ((IDPUserDao)base.DBDao).GetMaxId();
        }
        /// <summary>
        /// 生成随机密码,默认6位
        /// </summary>
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public string RJ_ResetAccountPassword(long id, long empID, string empName)
        {
            PUser entity = this.Get(int.Parse(id.ToString()));
            string newPWD = RJ_GetRandomNumber(6);
            entity.Password = CovertPassWord(newPWD);
            this.Entity = entity;
            this.Edit();
            string[] arrFields = { "Password" };

            //AddSCOperation(entity, arrFields, empID, empName);

            return newPWD;
        }
        public BaseResultDataValue GetPUserByShortCode(PUser entity, string saveType)
        {
            BaseResultDataValue brdv = new BaseResultDataValue();
            brdv.success = true;

            //验证账号是否重复
            string sql = "puser.ShortCode='" + entity.ShortCode + "'";
            if (saveType == "edit")
            {
                sql = sql + " and puser.Id!=" + entity.Id;
            }
            IList<PUser> userList = ((IDPUserDao)base.DBDao).GetListByHQL(sql);
            if (userList.Count > 0)
            {
                brdv.success = false;
                brdv.ErrorInfo = "该账号已经存在！";
                ZhiFang.Common.Log.Log.Debug(brdv.ErrorInfo);
            }
            return brdv;
        }
        public BaseResultDataValue AddPUserOfReg()
        {
            BaseResultDataValue brdv = new BaseResultDataValue();

            if (string.IsNullOrEmpty(this.Entity.ShortCode))
            {
                brdv.success = false;
                brdv.ErrorInfo = "注册账号不允许为空！";
                ZhiFang.Common.Log.Log.Debug(brdv.ErrorInfo);
                return brdv;
            }
            //验证账号是否重复
            brdv = GetPUserByShortCode(this.Entity, "add");
            if (brdv.success == false)
            {
                return brdv;
            }

            if (string.IsNullOrEmpty(this.Entity.Code1))
                this.Entity.Code1 = this.Entity.ShortCode;
            if (string.IsNullOrEmpty(this.Entity.Code2))
                this.Entity.Code2 = this.Entity.ShortCode;
            if (string.IsNullOrEmpty(this.Entity.Code4))
                this.Entity.Code4 = this.Entity.Id.ToString();

            brdv.success = this.Add();
            //brdv.success = DBDao.Save(this.Entity);
            if (brdv.success && this.Entity.Department != null)
            {
                AddDepartmentUser(this.Entity, this.Entity.Department, "", "");
            }

            //绑定预置角色
            if (brdv.success && this.Entity.RBACRole != null)
            {
                AddRBACRole(this.Entity);
            }
        ...
        public override bool Add()
        {
            ...
            {
                this.Entity.Password = this.DefaultPWD;// this.Entity.ShortCode;
            }
            this.Entity.Password = CovertPassWord(this.Entity.Password);
            bool a = DBDao.Save(this.Entity);
            return a;
        }
        public bool AddRBACRole(PUser entity)
        {
            RBACRole role = IBRBACRole.Get(entity.RBACRole.Id);
            //
            if (role != null)
            {
        ...
        private void GetWard(ref PUser puser, string hisWardId, string hisDoctorCode, bool isAutoAddDepartmentUser)
        {
            ...
            else
            {
                ...
                else
                {
                    if (puser.Department != null)
                    {
                        ward = IDDepartmentDao.Get(puser.Department.Id);
                    }
                }
            }
        ...
        private void GetDepartment(ref PUser puser, string hisDeptCode, string hisDoctorCode, bool isAutoAddDepartmentUser)
        {
            ...
            else
            {
                ...
                {
                    //department = deptUserList[0].Department;
                    ZhiFang.Common.Log.Log.Debug("用户编码为:" + puser.Id + ",科室人员关系记录数为:" + deptUserList.Count + "!");
                    Department department2 = null;
                    if (deptUserList.Count > 1)
                        department2 = deptUserList.OrderBy(p => p.Department.DispOrder).ElementAt(0).Department;
                    else
                        department2 = deptUserList[0].Department;
                    department = IDDepartmentDao.Get(department2.Id);
                }
                else
                {
                    //从人员所属科室里查找
                    if (puser.Department != null)
                    {
                        department = IDDepartmentDao.Get(puser.Department.Id);
                    }
                }
            }
            if (department != null && department.CName != null)
            {
                ...
                ZhiFang.WebAssist.Common.Cookie.CookieHelper.Write("ULdept", department.CName);
            }
            else
            {
                ZhiFang.WebAssist.Common.Cookie.CookieHelper.Write("ULdept", null);
            }
            //判断是否存在科室人员关系信息
            if (isAutoAddDepartmentUser == true)
        ...
        public void SaveDepartmentUser(PUser puser, Department department)
        {
            DepartmentUser deptUser = new DepartmentUser();
            deptUser.PUser = puser;
            deptUser.Department = department;
        ...
        public void SetUserSession(PUser rbacUser)
        {
            if (rbacUser != null)
            {
                ...
                if (rbacUser.Department != null)
                {
                    ...
                    Cookie.CookieHelper.Write(DicCookieSession.MonitorType, rbacUser.Department.MonitorType.ToString());//感控监测类型
                }
            ...
            else
            {
                ...
                Cookie.CookieHelper.Write(DicCookieSession.MonitorType, "");//感控监测类型
        ...
        private RBACUser _getRBACUser(PUser entity)
        {
            ...
            {
                Department department = IDDepartmentDao.Get(entity.Department.Id);
        ...
        #region 导入功能
        public BaseResultDataValue CheckPUserExcelFormat(string excelFilePath, string serverPath)
        {
            BaseResultDataValue baseResultDataValue = new BaseResultDataValue();
            string xmlPUser = serverPath + "\\BaseTableXML\\PUser.xml";
            if (!System.IO.File.Exists(xmlPUser))
            {
                baseResultDataValue.success = false;
                baseResultDataValue.ErrorInfo = "人员导入配置文件不存在！";
                ZhiFang.Common.Log.Log.Info("人员导入配置文件不存在！");
                return baseResultDataValue;
            }

            IList<string> dicColumn = ExcelDataCommon.GetRequiredFieldByXml(xmlPUser);
            Dictionary<string, Type> dicType = ExcelDataCommon.GetFieldTypeByXml<PUser>(xmlPUser);
            baseResultDataValue.success = WebAssist.Common.MyNPOIHelper.CheckExcelFile(excelFilePath, dicColumn, dicType);
        ...
        public BaseResultDataValue AddPUserByExcel(string deptID, string roleId, string excelFilePath, string serverPath)
        {
            BaseResultDataValue baseResultDataValue = new BaseResultDataValue();

            Department dept = null;
            if (!string.IsNullOrWhiteSpace(deptID) && int.Parse(deptID) > 0)
                dept = IDDepartmentDao.Get(int.Parse(deptID));
            if (dept == null)
            {
                baseResultDataValue.success = false;
                baseResultDataValue.ErrorInfo = "未获取到选择导入的科室信息!";
                return baseResultDataValue;
            }
            string xmlPUser = serverPath + "\\BaseTableXML\\PUser.xml";
            if (!System.IO.File.Exists(xmlPUser))
            ...
            DataTable dt = WebAssist.Common.MyNPOIHelper.ImportExceltoDataTable(excelFilePath);
            if (dt == null || dt.Rows.Count <= 0)
            {
                baseResultDataValue.success = false;
                baseResultDataValue.ErrorInfo = "导入文件的人员数据信息为空！";
                return baseResultDataValue;
            }
            ...
            //导入的列信息
            Dictionary<string, string> dicColumn = ExcelDataCommon.GetColumnNameByDataSet(dataSet, xmlPUser, listPrimaryKey, dicDefaultValue);
            ...
            baseResultDataValue = _AddPUserData(dt, dept, roleId, dicColumn, listPrimaryKey, dicDefaultValue);
            baseResultDataValue.success = WebAssist.Common.MyNPOIHelper.GetInputExcelFileState(excelFilePath, dt);
            if (baseResultDataValue.success)
            {
        ...
        public BaseResultDataValue _AddPUserData(DataTable dataTable, Department dept, string roleId, Dictionary<string, string> dicColumn, IList<string> listPrimaryKey, Dictionary<string, string> dicDefaultValue)
        {
            BaseResultDataValue baseResultDataValue = new BaseResultDataValue();
            int isExistCount = 0;
            int isErrorCount = 0;
            ...
            if (!string.IsNullOrEmpty(roleId))
                role = IBRBACRole.Get(int.Parse(roleId));
            ...
            foreach (DataRow dataRow in dataTable.Rows)
            {
                ...
                PUser emp = ExcelDataCommon.AddExcelDataToDataBaseByInt<PUser>(dataRow, dicColumn, dicDefaultValue);
                if (emp == null) continue;

                long labid = 0;
                //labid =long.Parse(ZhiFang.Common.Public.Cookie.CookieHelper.Read(ZhiFang.Entity.Base.SysPublicSet.SysDicCookieSession.LabID));

                if (role == null)
                    emp.RBACRole = role;
                if (string.IsNullOrEmpty(emp.Usertype))
                    emp.Usertype = BloodIdentityType.护士.Key;
                emp.Visible = 1;
                emp.Department = dept;
                emp.DataAddTime = DateTime.Now;
                emp.LabID = labid;
                emp.DispOrder = emp.Id;
                ...
                if (baseResultDataValue.success == false)
                {
                    ...
                    dataRow["ExcelRowInputInfo"] = "导入失败:" + baseResultDataValue.ErrorInfo;
                    ZhiFang.Common.Log.Log.Error(dataRow["ExcelRowInputInfo"].ToString());
                }
                else
                {
