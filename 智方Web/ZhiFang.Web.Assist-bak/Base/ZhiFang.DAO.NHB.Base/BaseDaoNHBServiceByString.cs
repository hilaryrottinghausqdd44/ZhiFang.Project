...
{
    public abstract class BaseDaoNHBServiceByString<T, T1> : HibernateDaoSupport, IDBaseDao<T, T1> where T : BaseEntityServiceByString
    {
        #region IBaseDao<T,T1> 成员
        public string DataRowRoleHQLString = "";
        public BaseDaoNHBServiceByString()
        {
        ...
        public void GetDataRowRoleHQLString()
        {
            ...
            //ZhiFang.Common.Log.Log.Info("BaseDaoNHBService根据权限获取的where=" + DataRowRoleHQLString);
        }
        ...
        {
            if (!((BaseEntityServiceByString)entity).DataAddTime.HasValue)
                ((BaseEntityServiceByString)entity).DataAddTime = DateTime.Now;
