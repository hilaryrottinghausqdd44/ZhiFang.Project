...
namespace ZhiFang.DAO.NHB.Base
{
    public abstract class BaseDaoNHB<T, T1> : HibernateDaoSupport, IDBaseDao<T, T1> where T : BaseEntity
    {
        ...
        {
            if (!((BaseEntity)entity).DataAddTime.HasValue)
    ...
    public class DaoNHBGetStringMaxByHqlAction<T> : IHibernateCallback<string>
    {
        private string hql;
        public string HQL
        {
        ...
        public DaoNHBGetStringMaxByHqlAction(string hql)
        {
            this.hql = hql;
        }
        public string DoInHibernate(ISession session)
        {
            IQuery query = null;
            if (hql != null && hql.Length > 0)
            {
