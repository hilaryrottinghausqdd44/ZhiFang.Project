...
{
    public abstract class BaseDaoNHBServiceByInt<T, T1> : HibernateDaoSupport, IDBaseDao<T, T1> where T : BaseEntityServiceByInt
    {
        #region IBaseDao<T,T1> 成员
        public string DataRowRoleHQLString = "";
        public BaseDaoNHBServiceByInt()
        {
        ...
        {
            if (!((BaseEntityServiceByInt)entity).DataAddTime.HasValue)
                ((BaseEntityServiceByInt)entity).DataAddTime = DateTime.Now;
