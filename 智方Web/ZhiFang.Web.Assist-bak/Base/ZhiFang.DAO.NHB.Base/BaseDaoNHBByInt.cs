...
{
    public abstract class BaseDaoByIntNHB<T, T1> : HibernateDaoSupport, IDBaseDao<T, T1> where T : BaseEntityByInt
    {
        #region IBaseDao<T,T1> 成员
        public string DataRowRoleHQLString = "";
        public BaseDaoByIntNHB()
        {
        ...
        public void GetDataRowRoleHQLString()
        {
            ...
            string labid = ZhiFang.WebAssist.Common.Cookie.CookieHelper.Read(ZhiFang.Entity.Base.SysPublicSet.SysDicCookieSession.LabID);
            string IsLabFlag = ZhiFang.WebAssist.Common.Cookie.CookieHelper.Read(ZhiFang.Entity.Base.SysPublicSet.SysDicCookieSession.IsLabFlag);
        ...
        public void GetDataRowRoleHQLString(bool hasLabId)
        {
            IApplicationContext context = ContextRegistry.GetContext();
            object bslog = context.GetObject("DataRowRoleHQL");
            DataRowRoleHQLString = ((DataRowRoleHQL)bslog).Hql;
        ...
        public void GetDataRowRoleHQLString(string entityName)
        {
            IApplicationContext context = ContextRegistry.GetContext();
            object bslog = context.GetObject("DataRowRoleHQL");
            DataRowRoleHQLString = ((DataRowRoleHQL)bslog).Hql;
            ...
            if (IsLabFlag != null && IsLabFlag.Trim() == "1")
            {
                if (labid != null && labid.Trim() != "")
                {
                    if (!string.IsNullOrEmpty(entityName))
                        DataRowRoleHQLString += " and " + entityName.Trim().ToLower() + ".LabID=" + labid + " ";
                    else
                        DataRowRoleHQLString += " and LabID=" + labid + " ";
                }
            }
        ...
        public void GetDataRowRoleHQLString(string entityName, bool hasLabId)
        {
            IApplicationContext context = ContextRegistry.GetContext();
            object bslog = context.GetObject("DataRowRoleHQL");
            DataRowRoleHQLString = ((DataRowRoleHQL)bslog).Hql;
        ...
        public virtual int GetTotalCount(bool hasLabId)
        {
            GetDataRowRoleHQLString(hasLabId);
            int count = this.GetListCountByHQL(DataRowRoleHQLString, hasLabId);
            return count;
            //Dictionary<string, List<ICriterion>> dic = new Dictionary<string, List<ICriterion>>();
            //dic.Add(typeof(T).Name, null);
            //DaoNHBCriteriaCountAction<T> action = new DaoNHBCriteriaCountAction<T>(dic);
        ...
        public virtual T Get(T1 id, bool hasLabId)
        {
            GetDataRowRoleHQLString(hasLabId);
            IList<T> ilist = this.HibernateTemplate.Find<T>(" select " + typeof(T).Name.ToLower() + " from " + typeof(T).Name + " " + typeof(T).Name.ToLower() + " where " + DataRowRoleHQLString + " and " + typeof(T).Name.ToLower() + ".id=" + id);
            if (ilist != null && ilist.Count > 0)
        ...
        public virtual T FindById(T1 id)
        {
            GetDataRowRoleHQLString();
            IList<T> ilist = this.HibernateTemplate.Find<T>(" select " + typeof(T).Name.ToLower() + " from " + typeof(T).Name + " " + typeof(T).Name.ToLower() + " where " + DataRowRoleHQLString + " and " + typeof(T).Name.ToLower() + ".id=" + id);
            if (ilist != null && ilist.Count > 0)
        ...
        public virtual IList<T> Find<T>(string hql, bool hasLabId)
        {
            GetDataRowRoleHQLString(hasLabId);
            return this.HibernateTemplate.Find<T>(hql + " and " + DataRowRoleHQLString);
        }
        ...
        {
            if (!((BaseEntityByInt)entity).DataAddTime.HasValue)
                ((BaseEntityByInt)entity).DataAddTime = DateTime.Now;
        ...
        public virtual IList<T> LoadAll(bool hasLabId)
        {
            GetDataRowRoleHQLString(hasLabId);
            string strHQL = "select " + typeof(T).Name.ToLower() + " from " + typeof(T).Name + " " + typeof(T).Name.ToLower() + " where 1=1 ";
            IList<T> ilist = this.HibernateTemplate.Find<T>(FilterMacroCommand(strHQL + " and " + DataRowRoleHQLString));
        ...
        public IList<T> SelectListByHQL(string strHqlWhere)
        {
            GetDataRowRoleHQLString();
            string strHQL = "select " + typeof(T).Name.ToLower() + " from " + typeof(T).Name + " " + typeof(T).Name.ToLower() + " where 1=1 ";
            if (strHqlWhere != null && strHqlWhere.Length > 0)
        ...
        public IList<T> GetIListByHQL(string strHqlWhere, string Order, int start, int count)
        {
            GetDataRowRoleHQLString();
            ...
            DaoNHBSearchByHqlAction<List<T>, T> action = new DaoNHBSearchByHqlAction<List<T>, T>(strHQL, start1, count1);
            entityList = this.HibernateTemplate.Execute<List<T>>(action);
            return entityList;
        }
        ...
        public EntityList<T> GetListByHQL(string strHqlWhere, int start, int count, bool hasLabId)
        {
            GetDataRowRoleHQLString(hasLabId);
            Dictionary<string, BTDMacroCommand> dicmc = BTDMacroCommandDao.DicBTDMacroCommand;
            // IList<BTDMacroCommand> listmc1 = (List<BTDMacroCommand>)this.HibernateTemplate.Find<BTDMacroCommand>(" from BTDMacroCommand ");
            ...
            entityList.list = this.HibernateTemplate.Execute<List<T>>(action);
            entityList.count = this.GetListCountByHQL(strHqlWhere, hasLabId);
            //entityList.count = (entityList.list != null ? entityList.list.Count : 0);
            return entityList;
        }
        ...
        public EntityList<T> GetListByHQL(string strHqlWhere, string Order, int start, int count, bool hasLabId)
        {
            GetDataRowRoleHQLString(hasLabId);
        ...
        public int GetListCountByHQL(string strHqlWhere, bool hasLabId)
        {
            GetDataRowRoleHQLString(hasLabId);
            string strHQL = "select count(*) from " + typeof(T).Name + " " + typeof(T).Name.ToLower() + " where 1=1 ";
            strHQL += " and " + DataRowRoleHQLString;
        ...
        public object GetTotalByHQL(string strHqlWhere, string fields, bool hasLabId)
        {
            GetDataRowRoleHQLString(hasLabId);
            if (fields != null && fields.Trim() != "")
            {
        ...
        public IList<T> GetListByHQL(string strHqlWhere, bool hasLabId)
        {
            GetDataRowRoleHQLString(hasLabId);
            string strHQL = "select " + typeof(T).Name.ToLower() + " from " + typeof(T).Name + " " + typeof(T).Name.ToLower() + " where 1=1 ";
            if (strHqlWhere != null && strHqlWhere.Length > 0)
        ...
        public EntityList<T> GetListByHQL(string strHqlWhere, string Order, int start, int count, string strHQL, string strHQLCount)
        {
            ...
            if (!String.IsNullOrEmpty(strHQL))
            {
                ...
                strHQL += " and " + BaseDataFilter.GetDataRowRoleHQLString();
