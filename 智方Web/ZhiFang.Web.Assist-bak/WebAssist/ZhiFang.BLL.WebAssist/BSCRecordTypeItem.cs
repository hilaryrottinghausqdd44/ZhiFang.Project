...
namespace ZhiFang.BLL.WebAssist
{
	...
	///
	/// </summary>
	public  class BSCRecordTypeItem : BaseBLL<SCRecordTypeItem>, ZhiFang.IBLL.WebAssist.IBSCRecordTypeItem
	{
        IBSCOperation IBSCOperation { get; set; }

        #region 修改信息记录
        public void AddSCOperation(SCRecordTypeItem serverEntity, string[] arrFields, long empID, string empName)
        {
            StringBuilder strbMemo = new StringBuilder();
            EditGetUpdateMemoHelp.EditGetUpdateMemo<SCRecordTypeItem>(serverEntity, this.Entity, this.Entity.GetType(), arrFields, ref strbMemo);
            if (strbMemo.Length > 0)
            {
                ...
                sco.BusinessModuleCode = "SCRecordTypeItem";
                ...
                sco.IsUse = true;
                sco.Type = long.Parse(UpdateOperationType.公共记录项字典.Key);
