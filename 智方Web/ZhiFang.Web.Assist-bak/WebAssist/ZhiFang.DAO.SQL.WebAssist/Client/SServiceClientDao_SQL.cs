...
using System.Linq;
using System.Text;
using ZhiFang.DAO.SQL.WebAssist.Client;
...
namespace ZhiFang.DAO.SQL.WebAssist
{
    public class SServiceClientDao_SQL : IDBaseDao<SServiceClient, long>, IDSServiceClientDao_SQL
    {
        ...
        public SServiceClient ObtainById(long labId)
        {
            ...
            //SServiceClient model = new SServiceClient();
            DataSet ds = DbHelperSQL.ObtainSql(DbHelperSQL.ConnectionString, strSql.ToString());
            if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
            {
                return DataRowToModel(ds.Tables[0].Rows[0]);
            }
        ...
        public IList<SServiceClient> GetIListByHQL(string strSqlWhere, string Order, int start, int count)
        {
            IList<SServiceClient> entityList = new List<SServiceClient>();
            //entityList.count = GetRecordCount(strSqlWhere);
            //if (entityList.count <= 0) return entityList;
            ...
            if (ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
            {
                foreach (DataRow row in ds.Tables[0].Rows)
                {
                    entityList.Add(DataRowToModel(row));
                }
            }
            else
            {
        ...
        public SServiceClient FindById(long id)
        {
            throw new NotImplementedException();
        }

        public IList<SServiceClient> SelectListByHQL(string strHqlWhere)
        {
            throw new NotImplementedException();
        }
    }
