...
Ext.define('Shell.class.sysbase.department.Grid', {
	...
	/**删除数据服务路径*/
	delUrl: '/ServerWCF/WebAssistLisService.svc/WA_UDTO_DelDepartment',
	...
	initComponent: function() {
		...
		me.searchInfo = {
			width: 145,
			emptyText: '名称/快捷码/对照码',
			isLike: true,
			fields: ['department.Id', 'department.CName', 'department.Code1', 'department.Code2', 'department.ShortCode']
		};
	...
	createGridColumns: function() {
		...
		var columns = [{
			...
			text: '简称',
			dataIndex: 'Department_ShortName',
			width: 80,
			menuDisabled: true,
			defaultRenderer: true
		},  {
			...
			text: '感控监测类型',
			dataIndex: 'Department_MonitorType',
			width: 95,
			editor: {
	...
	updateOne: function(index, id, rec) {
		...
		var params = Ext.JSON.encode({
			entity: {
				Id: id,
				Visible: IsUse,
				MonitorType: rec.get('Department_MonitorType'),
			...
			empID: empID,
			empName: empName,
			fields: 'Id,Visible,MonitorType,Code1,Code2,Code3,Code4,Code5,DispOrder'
		});
	...
	onSync: function(type1) {
		var me = this;
		var url = JShell.System.Path.ROOT + "/ServerWCF/WebAssistManageService.svc/WA_UDTO_SyncDeptListToLIMP";
		var params = Ext.JSON.encode({
			syncType: type1
		});
		setTimeout(function() {
