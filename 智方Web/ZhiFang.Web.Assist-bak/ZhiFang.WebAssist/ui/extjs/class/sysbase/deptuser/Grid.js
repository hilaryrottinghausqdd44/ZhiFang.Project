...
Ext.define('Shell.class.sysbase.deptuser.Grid', {
	...
	/**新增数据服务路径*/
	addUrl: '/ServerWCF/WebAssistLisService.svc/WA_UDTO_AddDepartmentUser',
	/**修改服务地址*/
	editUrl: '/ServerWCF/WebAssistLisService.svc/WA_UDTO_UpdateDepartmentUserByField',
	/**删除数据服务路径*/
	delUrl: '/ServerWCF/WebAssistLisService.svc/WA_UDTO_DelDepartmentUser',
	...
	createGridColumns: function() {
		...
		var columns = [{
			...
			text: '登录账号',
			dataIndex: 'DepartmentUser_PUser_ShortCode',
			width: 95,
			defaultRenderer: true
		}, {
			text: '人员对照码1',
			...
			text: '人员对照码2',
			dataIndex: 'DepartmentUser_PUser_Code2',
			width: 95,
			hidden: true,
			defaultRenderer: true
		}, {
			text: '人员对照码3',
			dataIndex: 'DepartmentUser_PUser_Code3',
			width: 95,
			hidden: true,
			defaultRenderer: true
		}, {
			text: '人员对照码4',
			dataIndex: 'DepartmentUser_PUser_Code4',
			width: 95,
			hidden: true,
			defaultRenderer: true
		}, {
			text: '人员对照码5',
			dataIndex: 'DepartmentUser_PUser_Code5',
			width: 95,
			hidden: true,
			defaultRenderer: true
		}, {
	...
	onUserDownload: function() {
		var me = this;
		var url = JShell.System.Path.ROOT + '/Report/Excel模板/员工模板/用户信息模版V1.xls?v=' + new Date().getTime();
		window.open(url);
	},
	//用户导入
	onUserUpload: function(hasOrg) {
		var me = this;
		var tid = me.DepartmentId + '';
		if (tid == '0') {
			JShell.Msg.alert('请选择一个科室后再操作!', null, 2000);
			return;
		}

		JShell.Win.open('Shell.class.sysbase.deptuser.UploadUser', {
			DeptID: hasOrg ? me.DepartmentId : '',
			listeners: {
				save: function(p) {
					p.close();
					me.onSearch();
