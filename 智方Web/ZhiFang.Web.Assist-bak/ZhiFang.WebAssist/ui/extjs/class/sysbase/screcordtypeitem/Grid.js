 ...
 * @version 2020-02-11
 */
Ext.define('Shell.class.sysbase.screcordtypeitem.Grid', {
	...
	/**删除数据服务路径*/
	delUrl: '/ServerWCF/WebAssistManageService.svc/WA_UDTO_DelSCRecordTypeItem',
	...
	/**用户UI配置Key*/
	userUIKey: 'screcordtypeitem.Grid',
	/**用户UI配置Name*/
	userUIName: "记录项字典列表",
	...
	initComponent: function() {
		...
		me.searchInfo = {
			...
			emptyText: '名称/对照码',
			isLike: true,
			fields: ['screcordtypeitem.CName','screcordtypeitem.ItemCode']
		};
	...
	createGridColumns: function() {
		...
		var columns = [ {
			...
			text: '背景颜色',
			dataIndex: 'SCRecordTypeItem_BGColor',
			width: 80,
			hidden:true,
			menuDisabled: true,
			defaultRenderer: true
			...
			text: '默认值',
			dataIndex: 'SCRecordTypeItem_DefaultValue',
			width: 80,
			menuDisabled: true,
			defaultRenderer: true,
			editor: {}
			...
			xtype: 'checkcolumn',
			text: '使用',
			dataIndex: 'SCRecordTypeItem_IsUse',
			width: 40,
			align: 'center',
			sortable: false,
			menuDisabled: true,
			...
			text: '创建时间',
			dataIndex: 'SCRecordTypeItem_DataAddTime',
			width: 130,
			hidden: true,
			isDate: true,
			hasTime: true
	...
	updateOne: function(index, rec) {
		...
		var isUse = rec.get('SCRecordTypeItem_IsUse');
		...
		var params = Ext.JSON.encode({
			...
			entity: {
				Id: id,
				IsUse: isUse,
				CName: rec.get('SCRecordTypeItem_CName'),
				DispOrder: rec.get('SCRecordTypeItem_DispOrder'),
				DefaultValue: rec.get('SCRecordTypeItem_DefaultValue'),
				ItemCode: rec.get('SCRecordTypeItem_ItemCode')
			},
			fields: 'Id,IsUse,DispOrder,CName,ItemCode,DefaultValue'
		});
	...
	changeResult: function(data) {
		...
		for(var i = 0; i < len; i++) {
			var isVisible = data.list[i].IsUse;
			if(isVisible == 'True' || isVisible == 'true' || isVisible == '1' || isVisible == 1) {
				isVisible = true;
			} else {
				isVisible = false;
			}
			data.list[i].IsUse = isVisible;
		}
		return data;
	},
	onShowOperation: function(record) {
		...
		var config = {
			...
			title: '操作记录',
			defaultWhere: "scoperation.BusinessModuleCode='SCRecordTypeItem'"
		};
