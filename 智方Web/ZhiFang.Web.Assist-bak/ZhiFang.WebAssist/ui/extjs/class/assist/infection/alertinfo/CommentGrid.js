 ...
 * @version 2020-11-27
 */
Ext.define('Shell.class.assist.infection.alertinfo.CommentGrid', {
	...
	expiredDays: 1,
	/**用户UI配置Key*/
	userUIKey: 'alertinfo.CommentGrid',
	/**用户UI配置Name*/
	userUIName: "待评估处理",
	...
	defaultOrderBy: [{
		property: 'GKSampleRequestForm_CheckDate',
		direction: 'ASC'
	}],
	...
	initComponent: function() {
		...
		//结果回写标志为是 并且评估标志为否
		me.defaultWhere="gksamplerequestform.ResultFlag=1 and gksamplerequestform.EvaluatorFlag=0";
	...
	createGridColumns: function() {
		...
		var columns = [{
			...
			text: '审核日期',
			dataIndex: 'GKSampleRequestForm_CheckDate',
			width: 140,
			isDate: true,
			hasTime: true
		},{
	...
	getInternalWhere: function() {
		...
		var days = buttonsToolbar.getComponent('CheckDate').getValue();
		if(days && days >= 0) {
			var dateValue = me.calcDateArea(-days);
			//InvalidWarningDate
			if(dateValue.start)
				where.push("reabmsqtydtl.CheckDate>='" + JShell.Date.toString(dateValue.start, true) + " 00:00:00'");
			if(dateValue.end)
				where.push("reabmsqtydtl.CheckDate<'" + JShell.Date.toString(dateValue.end, true) + " 23:59:59'");
		}
	 ...
	 * @param {Object} rec
	 */
	updateInfo: function(index, rec, judgment) {
		var me = this;
		...
		var params = Ext.JSON.encode({
			"entity": {
				"Id": id,
				"EvaluatorFlag": 1,//评估标志
				"Judgment":judgment,//评估结果 (1:合格，2:不合格)
				"StatusID": 5,//已评价
				"EvaluatorId": empID,//评估者ID
				"Evaluators": empName//评估者名称
			},
			"fields": 'Id,StatusID,Judgment',
			"empID": empID,
			"empName": empName
		});
