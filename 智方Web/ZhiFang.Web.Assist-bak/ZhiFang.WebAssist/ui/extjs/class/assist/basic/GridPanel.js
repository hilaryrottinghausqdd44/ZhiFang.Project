 ...
 * @version 2020-02-11
 */
Ext.define('Shell.class.assist.basic.GridPanel', {
	...
	/**新增列表UI配置服务地址*/
	addUIUrl: '/ServerWCF/WebAssistManageService.svc/WA_UDTO_AddBUserUIConfig',
	/**修改列表UI配置服务地址*/
	editUIUrl: '/ServerWCF/WebAssistManageService.svc/WA_UDTO_UpdateBUserUIConfigByField',
	/**删除列表UI配置服务地址*/
	delUIUrl: '/ServerWCF/WebAssistManageService.svc/WA_UDTO_DelBUserUIConfig',
	...
	initComponent: function() {
		...
		if (me.defaultOrderBy && me.defaultOrderBy.length > 0) {
			...
			me.curOrderBy=JShell.JSON.decode(JShell.JSON.encode(me.defaultOrderBy));
		}
		me.callParent(arguments);
	},
	...
	initDefaultPageSize: function() {
		...
		if (!defaultPageSize) {
			//系统运行参数"列表默认分页记录数"
			JShell.BLTF.RunParams.getRunParamsValue("BLTFUIDefaultPageSize", false, function(data) {
				var defaultPageSize = JcallShell.BLTF.RunParams.Lists.BLTFUIDefaultPageSize.Value;
				if (!defaultPageSize) {
	...
	onSaveUIClick: function() {
		...
		JShell.Server.post(url, params, function(data) {
			...
			if (data.success) {
				...
				JcallShell.BLTF.BUserUIConfig.addListByKey(userUIKey1, params2);
		...
		setTimeout(function() {
			JShell.Server.get(url, function(data) {
				if (data.success) {
					...
					var empID = JcallShell.System.Cookie.get(JcallShell.System.Cookie.map.USERID);
					var userUIKey1 = empID + me.userUIKey;
					JcallShell.BLTF.BUserUIConfig.removeByKey(userUIKey1);
