layui.extend({
...
}).use(['uxutil', 'layer', 'uxdata', 'cachedata', 'webassistconfig'], function() {
	"use strict";
	...
	function getSysDoctorInfo(callback) {
		...
		var url = uxutil.path.ROOT + webassistconfig.Common.HISCALL_URL;
	...
	function onLoginByHisDoctorCode(callback) {
		var hisDoctorCode = defaultParams.HisDoctorId;
		var url = uxutil.path.ROOT + webassistconfig.Common.LOGINOFPUSERBYHISCODE_URL;
	...
	function onLoginByDefault(callback) {
		...
		var account = webassistconfig.Common.DEFAULT_ACCOUNT;
		var password = webassistconfig.Common.DEFAULT_PWD;
		var url = uxutil.path.ROOT + webassistconfig.Common.LOGINOFPUSER_URL;
