layui.extend({
...
}).use(['form', 'uxutil', 'layer', 'csserver', 'webassistconfig', 'cachedata'], function() {


	//'uxutil', 'csserver', 'webassistconfig'
	var $ = layui.$,
		uxutil = layui.uxutil,
		form = layui.form;
	...
	function onLogin() {
		if (webassistconfig.Common.ISHASCSLOGIN == true) {
			onCSLogin();
		} else {
			onBSLogin();
		}
	...
	function getSysDoctorInfo(account, callback) {
		var hisDeptCode = "";
		var url = uxutil.path.ROOT + "/ServerWCF/WebAssistManageService.svc/BT_SYS_GetSysCurDoctorInfoByAccount";
