layui.config({
	...
	uxutil: '../../../ux/util',
	defaultmodule: '../../defaultmodule', //无集成平台时,血库手工维护的功能模块	
	bloodClassDict: '../../../views/modules/bloodtransfusion/bloodClassDict'
}).use(['index', 'uxutil', 'defaultmodule','bloodClassDict'], function() {
	var $ = layui.$,
		uxutil = layui.uxutil;

	var bloodClassDict = layui.bloodClassDict;
	var defaultmodule=layui.defaultmodule;
	...
	function getModuleTree() {
		...
		var url = GET_MODULE_TREE_URL;// + "?id=" + 0;
		uxutil.server.ajax({
			url: url
		}, function(data) {
			layer.closeAll('loading');
			if(data.success) {
				//if(!data.value||!data.value.Tree||data.value.Tree.length<=0)data.value = defaultmodule.loadDefaultModule();
	...
	function getUrlHtml(url) {
		...
		if(href.slice(0, 1) == '#') { //ExtJS页面
			...
			href = uxutil.path.getUiPath() + url;
	...
	/**获取待评价及待归档处理提示信息*/
	function gkSysWarning() {
		var me = this;
		//判断当前用户的科室的感控监测类型是否为感控监测;“感控监测类型”：1:感控监测;0:科室监测;
		var monitorType = ""+uxutil.cookie.get(uxutil.cookie.map.MONITORTYPE);
		if(monitorType!="1")return;
		
		getGKWarning(function(data){
			var toCommentCount =0,toBeArchivedCount =0;
			 if(data.value){
	...
	/**获取待评价及待归档处理提示信息*/
	function getGKWarning(callback) {
		var me = this;
		var url = uxutil.path.ROOT + '/ServerWCF/WebAssistManageService.svc/WA_UDTO_GetGKWarningAlertInfo';
		uxutil.server.ajax({
			url: url
		}, function(data) {
			layer.closeAll('loading');
	...
	/**
	 * @description 显示院感系统的待评价及待归档处理
	 */
	function showGKAlertInfo() {
		var me = this;
		var url = uxutil.path.EXTJS+
			'/interface/one/index.html?className=Shell.class.assist.infection.alertinfo.App';
		layer.open({
			title: '待评价及待归档处理',
			type: 2,
			content: url,
	...
	var isLogin = uxutil.cookie.get(uxutil.cookie.map.ACCOUNTNAME);//USERNAME
